---
id: q011
title: "Bernoulli 参数 p 的 MLE 是什么？"
chapter: "B. 数理统计与统计推断"
difficulty: "★☆☆"
tags: ["MLE", "Bernoulli", "一致性"]
source_type: "高可信重构题型"
version: "2.0"
---

# 011. Bernoulli 参数 p 的 MLE 是什么？

[← Q010](../01-probability-expectation/q010-如何构造相同边缘分布-但联合行为完全不同的随机变量.md) · [总索引](../../docs/100-question-index.md) · [Q012 →](q012-正态分布均值-方差的-MLE-是什么-为什么样本方差常除以-n-1.md)

> **难度**：★☆☆  
> **标签**：MLE｜Bernoulli｜一致性  
> **题型口径**：高可信重构题型  
> **所属模块**：B. 数理统计与统计推断  
> **本题能力主线**：统计推断与研究可信度
...
Bernoulli 参数 p 的 MLE 是什么？

---

## 1. 面试官到底在考什么

本题表面属于 **MLE, Bernoulli, 一致性**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
对 x_i∈{0,1}，log L(p)=Σx_i log p+(n-Σx_i)log(1-p)，一阶条件给 p_hat=bar{x}。

**推荐面试表达：** 把 likelihood、score、一阶条件、统计性质按顺序说完即可。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

Bernoulli 样本的 sufficient statistic 是 $S=\sum_iX_i$。Score 为 $\ell'(p)=S/p-(n-S)/(1-p)$，解得 $\hat p=S/n$。Fisher information 为 $I_n(p)=n/[p(1-p)]$，渐近方差达到 $I_n^{-1}$。
...
它同时是样本成功率、无偏估计，Var(p_hat)=p(1-p)/n；由 CLT，sqrt(n)(p_hat-p) 渐近正态。

### 第二视角：如何验证主解

把解析推断与 simulation/resampling 并行：公式给出结构，bootstrap/permutation/Monte Carlo 检查有限样本下的偏差、方差和覆盖率。

对本题尤其值得继续追问的是：**Bernoulli MLE 同时也是 sufficient statistic 的函数；样本和 Σx_i 已经包含关于 p 的全部信息。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Bernoulli MLE 同时也是 sufficient statistic 的函数；样本和 Σx_i 已经包含关于 p 的全部信息。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Bernoulli MLE 同时也是 sufficient statistic 的函数；样本和 Σx_i 已经包含关于 p 的全部信息。
- **最值得保留的原始面试表达**：把 likelihood、score、一阶条件、统计性质按顺序说完即可。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

MLE 面试题不应只停在一阶条件；完整回答最好补 estimator 的 bias、variance、consistency、asymptotic normality 与边界行为。
...
**回答方向：** MLE invariance 来自参数重参数化后使 likelihood 最大的样本点不变；若 $\hat\theta$ 最大化 $L(\theta)$，则相应的 $g(\hat\theta)$ 给出 $g(\theta)$ 的 MLE。

### 追问 2：边界 p=0/1 时 Fisher information 如何？

**回答方向：** 形式上单个 Bernoulli 的 Fisher information $1/[p(1-p)]$ 在边界附近发散，但 $p=0,1$ 是正则性失效的边界点，标准渐近正态结论需谨慎。
...
> **第二步，主解。** 用最短的推导得到核心结论：对 x_i∈{0,1}，log L(p)=Σx_i log p+(n-Σx_i)log(1-p)，一阶条件给 p_hat=bar{x}。  
> **第三步，解释。** 关键结构是：Bernoulli MLE 同时也是 sufficient statistic 的函数；样本和 Σx_i 已经包含关于 p 的全部信息。  
...
- [010. 如何构造相同边缘分布、但联合行为完全不同的随机变量？](../01-probability-expectation/q010-如何构造相同边缘分布-但联合行为完全不同的随机变量.md)
