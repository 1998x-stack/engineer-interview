---
id: q016
title: "为什么普通 OLS t-stat 在金融时间序列里常失真？"
chapter: "B. 数理统计与统计推断"
difficulty: "★★☆"
tags: ["OLS", "HAC", "序列相关"]
source_type: "高可信重构题型"
...
# 016. 为什么普通 OLS t-stat 在金融时间序列里常失真？

[← Q015](q015-测试-10-000-个特征-总会出现显著结果-怎么处理.md) · [总索引](../../docs/100-question-index.md) · [Q017 →](q017-Ridge-与-Lasso-的本质区别.md)
...
为什么普通 OLS t-stat 在金融时间序列里常失真？

---

## 1. 面试官到底在考什么

本题表面属于 **OLS, HAC, 序列相关**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
普通标准误依赖误差独立同方差；金融数据常有 heteroskedasticity、serial correlation 和 overlapping labels，使标准误被低估或高估。

**推荐面试表达：** 回答顺序：假设被破坏 → 标准误为什么错 → 根据依赖结构选择修正。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

OLS 系数在一定外生性条件下仍可一致，但若误差存在 serial correlation/heteroskedasticity，经典方差公式 $\sigma^2(X'X)^{-1}$ 失效。HAC/Newey–West 用加权自协方差估计长期方差。
...
可使用 Newey-West/HAC、cluster-robust SE 或 block bootstrap。重点不是机械换一个标准误，而是先识别 dependence structure。

### 第二视角：如何验证主解

把解析推断与 simulation/resampling 并行：公式给出结构，bootstrap/permutation/Monte Carlo 检查有限样本下的偏差、方差和覆盖率。

对本题尤其值得继续追问的是：**HAC 只修正标准误，不会自动修正模型设定错误；如果遗漏了动态结构或目标定义有重叠，应该先重新建模依赖。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

HAC 只修正标准误，不会自动修正模型设定错误；如果遗漏了动态结构或目标定义有重叠，应该先重新建模依赖。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：HAC 只修正标准误，不会自动修正模型设定错误；如果遗漏了动态结构或目标定义有重叠，应该先重新建模依赖。
- **最值得保留的原始面试表达**：回答顺序：假设被破坏 → 标准误为什么错 → 根据依赖结构选择修正。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

金融回归常见重叠收益、滚动特征和波动聚集。一个漂亮的 t-stat 如果 standard error 假设错了，显著性可能完全是幻觉。
...
- **错误 1**：只看回归系数，不审查 residual。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：Newey-West bandwidth 怎么选？

**回答方向：** bandwidth 决定纳入多少阶 autocovariance。可按理论依赖长度、数据频率或自动 bandwidth rule 选择，并做 sensitivity；过小漏相关，过大增加估计方差。

### 追问 2：overlapping k-day returns 的相关结构是什么？

**回答方向：** 若使用 overlapping $k$-period returns，相邻观测共享底层单期收益，会诱导近似 MA($k-1$) 型相关结构，即使单期收益独立也如此。
...
> **第二步，主解。** 用最短的推导得到核心结论：普通标准误依赖误差独立同方差；金融数据常有 heteroskedasticity、serial correlation 和 overlapping labels，使标准误被低估或高估。  
> **第三步，解释。** 关键结构是：HAC 只修正标准误，不会自动修正模型设定错误；如果遗漏了动态结构或目标定义有重叠，应该先重新建模依赖。  
