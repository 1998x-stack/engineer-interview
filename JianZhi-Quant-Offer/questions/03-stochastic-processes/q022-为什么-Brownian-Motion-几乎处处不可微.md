---
id: q022
title: "为什么 Brownian Motion 几乎处处不可微？"
chapter: "C. 随机过程"
difficulty: "★★★"
tags: ["尺度分析", "quadratic variation", "Itô 直觉"]
source_type: "高可信重构题型"
version: "2.0"
---

# 022. 为什么 Brownian Motion 几乎处处不可微？

[← Q021](q021-Brownian-Motion-的核心性质是什么.md) · [总索引](../../docs/100-question-index.md) · [Q023 →](q023-什么是-Martingale.md)

> **难度**：★★★  
> **标签**：尺度分析｜quadratic variation｜Itô 直觉  
> **题型口径**：高可信重构题型  
> **所属模块**：C. 随机过程  
> **本题能力主线**：随机过程与状态演化
...
为什么 Brownian Motion 几乎处处不可微？

---

## 1. 面试官到底在考什么

本题表面属于 **尺度分析, quadratic variation, Itô 直觉**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
小时间间隔 h 上增量尺度约 sqrt(h)，差商尺度约 sqrt(h)/h=1/sqrt(h)，随 h→0 不稳定，因此不能像平滑函数那样微分。

**推荐面试表达：** 用 O(sqrt(h)) 对 O(h) 的尺度比值，是最短也最有说服力的回答。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

Brownian increment 大小是 $O_p(\sqrt{h})$，差商 $(W_{t+h}-W_t)/h$ 因此是 $O_p(h^{-1/2})$，不会收敛到有限导数。更严格地，Brownian path 几乎处处不可微，但具有有限二次变差。
...
更深层地，Brownian path 有非零 quadratic variation，而有限变差光滑路径的二次变差为 0。这导致 Itô 公式多出 1/2 f″ 项。

### 第二视角：如何验证主解

同一随机过程尽量从“状态递推/生成元”与“路径模拟”两条路线理解；前者解释结构，后者检查分布与 stopping/path-dependent 量。

对本题尤其值得继续追问的是：**非零 quadratic variation 是 Itô calculus 的关键；dW 的量级是 sqrt(dt)，因此二阶项 dW² 与 dt 同阶不能忽略。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

非零 quadratic variation 是 Itô calculus 的关键；dW 的量级是 sqrt(dt)，因此二阶项 dW² 与 dt 同阶不能忽略。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：非零 quadratic variation 是 Itô calculus 的关键；dW 的量级是 sqrt(dt)，因此二阶项 dW² 与 dt 同阶不能忽略。
- **最值得保留的原始面试表达**：用 O(sqrt(h)) 对 O(h) 的尺度比值，是最短也最有说服力的回答。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

这解释了为什么随机微积分保留 $(dW)^2\sim dt$ 而忽略更高阶项。若候选人只说“路径很粗糙”而不能连接 quadratic variation，理解通常还不够深。
...
**回答方向：** 在时间步长 $dt$ 上，$dW=O(\sqrt{dt})$，所以 $(dW)^2=O(dt)$；累积二次增量不会消失，极限正是 quadratic variation $dt$。

### 追问 2：为什么 ordinary chain rule 失效？

**回答方向：** ordinary chain rule 忽略二阶项，但 Brownian 的二阶增量与 $dt$ 同阶，必须保留 $\frac12f_{xx}(dW)^2$，得到 Itô correction。
...
> **第二步，主解。** 用最短的推导得到核心结论：小时间间隔 h 上增量尺度约 sqrt(h)，差商尺度约 sqrt(h)/h=1/sqrt(h)，随 h→0 不稳定，因此不能像平滑函数那样微分。  
> **第三步，解释。** 关键结构是：非零 quadratic variation 是 Itô calculus 的关键；dW 的量级是 sqrt(dt)，因此二阶项 dW² 与 dt 同阶不能忽略。  
...
- [021. Brownian Motion 的核心性质是什么？](q021-Brownian-Motion-的核心性质是什么.md)
