...
[← Q022](q022-为什么-Brownian-Motion-几乎处处不可微.md) · [总索引](../../docs/100-question-index.md) · [Q024 →](q024-Optional-Stopping-Theorem-为什么不能随便套.md)
...
本题表面属于 **条件期望, 公平游戏, 过滤**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
相对 filtration F_t，可积过程 X_t 若满足 E[X_{t+1}|F_t]=X_t（连续时间相应定义），则为 martingale。

**推荐面试表达：** 一定带上 filtration，否则定义不完整。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

过程 $M_t$ 相对于 filtration $\mathcal F_t$ 是 martingale，需 $E|M_t|<\infty$、adapted 且 $E[M_t\mid\mathcal F_s]=M_s$ 对 $s<t$。这是“条件均值无可预测漂移”，不是路径不变。
...
它表达“给定当前全部可用信息，下一步条件均值不变”。martingale 可以高度波动，也不要求独立增量。

### 第二视角：如何验证主解

同一随机过程尽量从“状态递推/生成元”与“路径模拟”两条路线理解；前者解释结构，后者检查分布与 stopping/path-dependent 量。

对本题尤其值得继续追问的是：**Martingale 是相对于信息流 filtration 定义的；改变信息集可能改变某过程是否为 martingale。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Martingale 是相对于信息流 filtration 定义的；改变信息集可能改变某过程是否为 martingale。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Martingale 是相对于信息流 filtration 定义的；改变信息集可能改变某过程是否为 martingale。
- **最值得保留的原始面试表达**：一定带上 filtration，否则定义不完整。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

金融里 risk-neutral discounted price 的 martingale 性质来自无套利与定价测度，不应与“真实市场价格不可预测”混为一谈。
...
- **错误 1**：把 martingale 说成“未来与现在独立”。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：submartingale/supermartingale？

**回答方向：** submartingale 满足 $E[X_t\mid\mathcal F_s]\ge X_s$，supermartingale 则相反；都需可积和 adapted。

### 追问 2：W_t²-t 为什么是 martingale？

**回答方向：** 由 Itô 公式 $d(W_t^2)=2W_tdW_t+dt$，因此 $d(W_t^2-t)=2W_tdW_t$，其条件漂移为 0，所以是 martingale。
...
> **第二步，主解。** 用最短的推导得到核心结论：相对 filtration F_t，可积过程 X_t 若满足 E[X_{t+1}|F_t]=X_t（连续时间相应定义），则为 martingale。  
> **第三步，解释。** 关键结构是：Martingale 是相对于信息流 filtration 定义的；改变信息集可能改变某过程是否为 martingale。  
