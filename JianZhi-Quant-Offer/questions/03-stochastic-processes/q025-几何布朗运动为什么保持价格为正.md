...
[← Q024](q024-Optional-Stopping-Theorem-为什么不能随便套.md) · [总索引](../../docs/100-question-index.md) · [Q026 →](q026-Ornstein-Uhlenbeck-Process-的长期均值是什么.md)
...
本题表面属于 **SDE, GBM, Itô**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
dS=μSdt+σSdW 的解为 S_t=S_0 exp[(μ-σ²/2)t+σW_t]，指数始终为正。

**推荐面试表达：** 现场推导时从 f(S)=log S 开始，是标准路径。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

对 $dS_t=\mu S_tdt+\sigma S_tdW_t$ 应用 Itô 到 $\log S_t$：$d\log S_t=(\mu-\frac12\sigma^2)dt+\sigma dW_t$，故 $S_t=S_0\exp[(\mu-\sigma^2/2)t+\sigma W_t]>0$。
...
对 log S 应用 Itô 公式得到 d log S=(μ-σ²/2)dt+σdW，积分后再指数化。σ²/2 是 Itô correction。

### 第二视角：如何验证主解

同一随机过程尽量从“状态递推/生成元”与“路径模拟”两条路线理解；前者解释结构，后者检查分布与 stopping/path-dependent 量。

对本题尤其值得继续追问的是：**GBM 的 lognormal 结构带来正值但也带来过薄尾部和连续路径假设，这正是后续 jump/stochastic-vol 模型出现的原因。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

GBM 的 lognormal 结构带来正值但也带来过薄尾部和连续路径假设，这正是后续 jump/stochastic-vol 模型出现的原因。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：GBM 的 lognormal 结构带来正值但也带来过薄尾部和连续路径假设，这正是后续 jump/stochastic-vol 模型出现的原因。
- **最值得保留的原始面试表达**：现场推导时从 f(S)=log S 开始，是标准路径。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

GBM 保证正价格但也带来 lognormal、连续路径和 constant volatility 等强假设。真实资产可能跳跃、停牌甚至接近零，因此“正值”并不意味着模型充分。
...
**回答方向：** GBM 下 $E[S_t]=S_0e^{\mu t}$。

### 追问 2：median path 与 mean path 有何差异？

**回答方向：** $\log S_t$ 的中位数对应 $W_t$ 的中位数 0，因此 median $=S_0e^{(\mu-\sigma^2/2)t}$；mean 更高，是 lognormal 右尾拉升的结果。
...
> **第二步，主解。** 用最短的推导得到核心结论：dS=μSdt+σSdW 的解为 S_t=S_0 exp[(μ-σ²/2)t+σW_t]，指数始终为正。  
> **第三步，解释。** 关键结构是：GBM 的 lognormal 结构带来正值但也带来过薄尾部和连续路径假设，这正是后续 jump/stochastic-vol 模型出现的原因。  
