---
id: q026
title: "Ornstein-Uhlenbeck Process 的长期均值是什么？"
chapter: "C. 随机过程"
difficulty: "★★☆"
tags: ["Mean reversion", "OU", "SDE"]
source_type: "高可信重构题型"
version: "2.0"
---

# 026. Ornstein-Uhlenbeck Process 的长期均值是什么？

[← Q025](q025-几何布朗运动为什么保持价格为正.md) · [总索引](../../docs/100-question-index.md) · [Q027 →](q027-Poisson-Process-的-inter-arrival-time-为什么是指数分布.md)

> **难度**：★★☆  
> **标签**：Mean reversion｜OU｜SDE  
> **题型口径**：高可信重构题型  
> **所属模块**：C. 随机过程  
> **本题能力主线**：随机过程与状态演化
...
Ornstein-Uhlenbeck Process 的长期均值是什么？

---

## 1. 面试官到底在考什么

本题表面属于 **Mean reversion, OU, SDE**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
对 dX=θ(μ-X)dt+σdW，条件均值为 E[X_t|X_0]=μ+(X_0-μ)e^{-θt}，θ>0 时长期趋于 μ。

**推荐面试表达：** 把长期均值、速度、长期方差三件事一起说，层次更完整。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

OU：$dX_t=\theta(\mu-X_t)dt+\sigma dW_t$。显式解为 $X_t=\mu+(X_0-\mu)e^{-\theta t}+\sigma\int_0^t e^{-\theta(t-s)}dW_s$。长期方差趋于 $\sigma^2/(2\theta)$。
...
显式解还给出长期方差 σ²/(2θ)。e^{-θt} 显示 mean-reversion speed；half-life 为 ln2/θ。

### 第二视角：如何验证主解

同一随机过程尽量从“状态递推/生成元”与“路径模拟”两条路线理解；前者解释结构，后者检查分布与 stopping/path-dependent 量。

对本题尤其值得继续追问的是：**OU 的 half-life ln2/θ 是把连续时间参数转成业务直觉的桥梁，但参数估计若跨 regime 使用会非常脆弱。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

OU 的 half-life ln2/θ 是把连续时间参数转成业务直觉的桥梁，但参数估计若跨 regime 使用会非常脆弱。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：OU 的 half-life ln2/θ 是把连续时间参数转成业务直觉的桥梁，但参数估计若跨 regime 使用会非常脆弱。
- **最值得保留的原始面试表达**：把长期均值、速度、长期方差三件事一起说，层次更完整。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

OU 常作为 mean-reverting latent factor/spread 的基线，但真实 spread 若不是 stationary，强行套 OU 会制造虚假“均值回归”。先做结构/稳定性诊断再估参数。
...
**回答方向：** 采样间隔 $\Delta$ 后，$X_{t+\Delta}-\mu=e^{-\theta\Delta}(X_t-\mu)+\eta_t$，因此离散形式是 AR(1)，系数 $\phi=e^{-\theta\Delta}$。

### 追问 2：θ<0 会怎样？

**回答方向：** $\theta<0$ 时 drift 把过程推离 $\mu$，形成不稳定/爆炸式均值反转的反面。

### 追问 3：估计 half-life 的陷阱？

**回答方向：** 由 $\hat\phi$ 估 half-life $-\ln2/\ln\hat\phi$ 时，$\hat\phi$ 接近 1 会使误差被严重放大；还要检查结构变化与 stationarity。
...
> **第二步，主解。** 用最短的推导得到核心结论：对 dX=θ(μ-X)dt+σdW，条件均值为 E[X_t|X_0]=μ+(X_0-μ)e^{-θt}，θ>0 时长期趋于 μ。  
> **第三步，解释。** 关键结构是：OU 的 half-life ln2/θ 是把连续时间参数转成业务直觉的桥梁，但参数估计若跨 regime 使用会非常脆弱。  
