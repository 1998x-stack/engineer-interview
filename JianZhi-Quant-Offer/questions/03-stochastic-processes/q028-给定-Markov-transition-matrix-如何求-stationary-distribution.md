---
id: q028
title: "给定 Markov transition matrix，如何求 stationary distribution？"
chapter: "C. 随机过程"
difficulty: "★★☆"
tags: ["Markov chain", "stationary", "ergodicity"]
source_type: "高可信重构题型"
version: "2.0"
---

# 028. 给定 Markov transition matrix，如何求 stationary distribution？

[← Q027](q027-Poisson-Process-的-inter-arrival-time-为什么是指数分布.md) · [总索引](../../docs/100-question-index.md) · [Q029 →](q029-Kalman-Filter-的本质是什么.md)

> **难度**：★★☆  
> **标签**：Markov chain｜stationary｜ergodicity  
> **题型口径**：高可信重构题型  
> **所属模块**：C. 随机过程  
> **本题能力主线**：随机过程与状态演化
...
给定 Markov transition matrix，如何求 stationary distribution？

---

## 1. 面试官到底在考什么

本题表面属于 **Markov chain, stationary, ergodicity**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
求行向量 π 满足 πP=π、Σπ_i=1、π_i≥0。有限不可约链必有唯一 stationary distribution；若再非周期，则分布从任意初值收敛到它。

**推荐面试表达：** 回答时分开说 existence、uniqueness、convergence 三个问题。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

有限状态链的 stationary distribution 满足左特征向量方程 $\pi P=\pi$、$\sum_i\pi_i=1$。不可约有限链总有唯一 stationary distribution；若再 aperiodic，则从任意初始分布收敛到它。
...
“存在/唯一”和“从任意初态收敛”是不同层次：aperiodicity 主要影响收敛行为。

### 第二视角：如何验证主解

同一随机过程尽量从“状态递推/生成元”与“路径模拟”两条路线理解；前者解释结构，后者检查分布与 stopping/path-dependent 量。

对本题尤其值得继续追问的是：**stationary distribution 解决的是长期占比，不等于从任意初始状态都立即收敛；收敛还需要关注 irreducibility 与 periodicity。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

stationary distribution 解决的是长期占比，不等于从任意初始状态都立即收敛；收敛还需要关注 irreducibility 与 periodicity。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：stationary distribution 解决的是长期占比，不等于从任意初始状态都立即收敛；收敛还需要关注 irreducibility 与 periodicity。
- **最值得保留的原始面试表达**：回答时分开说 existence、uniqueness、convergence 三个问题。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

regime model、订单状态、信用迁移都可抽象成 Markov chain。面试中要区分“stationary distribution 存在/唯一”与“链从初态收敛到它”。
...
- **错误 1**：把 stationary distribution 与某一步当前 distribution 混为一谈。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：reversible chain 与 detailed balance？

**回答方向：** reversible chain 满足 detailed balance $\pi_iP_{ij}=\pi_jP_{ji}$；这会自动推出 $\pi P=\pi$，但 stationary 不一定 reversible。

### 追问 2：eigenvalue 1 的意义？

**回答方向：** $P$ 是随机矩阵，因此 1 是右特征值；stationary distribution 是对应左特征向量。其它特征值模长决定 mixing/收敛速度。
...
> **第二步，主解。** 用最短的推导得到核心结论：求行向量 π 满足 πP=π、Σπ_i=1、π_i≥0。有限不可约链必有唯一 stationary distribution；若再非周期，则分布从任意初值收敛到它。  
> **第三步，解释。** 关键结构是：stationary distribution 解决的是长期占比，不等于从任意初始状态都立即收敛；收敛还需要关注 irreducibility 与 periodicity。  
...
- [027. Poisson Process 的 inter-arrival time 为什么是指数分布？](q027-Poisson-Process-的-inter-arrival-time-为什么是指数分布.md)
