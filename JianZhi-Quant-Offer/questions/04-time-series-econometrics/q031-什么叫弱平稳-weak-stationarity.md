---
id: q031
title: "什么叫弱平稳（weak stationarity）？"
chapter: "D. 时间序列与计量"
difficulty: "★☆☆"
tags: ["时间序列", "stationarity"]
source_type: "高可信重构题型"
version: "2.0"
---

# 031. 什么叫弱平稳（weak stationarity）？

[← Q030](../03-stochastic-processes/q030-HMM-与普通-Markov-Chain-的区别.md) · [总索引](../../docs/100-question-index.md) · [Q032 →](q032-AR-1-什么时候平稳-长期方差和-ACF-是什么.md)

> **难度**：★☆☆  
> **标签**：时间序列｜stationarity  
> **题型口径**：高可信重构题型  
> **所属模块**：D. 时间序列与计量  
> **本题能力主线**：时间序列与计量
...
什么叫弱平稳（weak stationarity）？

---

## 1. 面试官到底在考什么

本题表面属于 **时间序列, stationarity**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
均值 E[X_t]=μ 不随时间变；方差有限且恒定；Cov(X_t,X_{t-k}) 只依赖 lag k，不依赖绝对时间 t。

**推荐面试表达：** 定义题要完整列三条，并顺手区分 strict stationarity。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

弱平稳要求一阶矩常数、二阶矩有限且协方差只依赖 lag：$E[X_t]=\mu$，$\gamma(t,s)=\gamma(t-s)$。严格平稳要求任意有限维联合分布对时间平移不变，条件更强。
...
弱平稳只约束一、二阶矩；严格平稳要求任意有限维联合分布对时间平移不变。高斯过程中二者关系更紧。

### 第二视角：如何验证主解

解析时间序列模型之外，再做 rolling/expanding OOS；理论上的平稳参数如果在滚动窗口剧烈变化，就应把 structural break 当一等问题。

对本题尤其值得继续追问的是：**弱平稳是二阶矩意义上的稳定，不等于 strict stationarity；很多协方差方法只需要弱平稳。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

弱平稳是二阶矩意义上的稳定，不等于 strict stationarity；很多协方差方法只需要弱平稳。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：弱平稳是二阶矩意义上的稳定，不等于 strict stationarity；很多协方差方法只需要弱平稳。
- **最值得保留的原始面试表达**：定义题要完整列三条，并顺手区分 strict stationarity。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

许多估计器和检验依赖弱平稳，但金融序列常存在结构突变、季节性和波动状态变化。面试中应说明“平稳是局部建模近似，不是自然法则”。
...
**回答方向：** 严格平稳若二阶矩有限，则推出弱平稳；若二阶矩不存在，则未必能谈弱平稳。

### 追问 2：为什么趋势会破坏平稳？

**回答方向：** 确定性趋势会使 $E[X_t]$ 随时间变化，随机趋势还会让 variance/covariance 依赖绝对时间，因此破坏平稳条件。
...
> **第二步，主解。** 用最短的推导得到核心结论：均值 E[X_t]=μ 不随时间变；方差有限且恒定；Cov(X_t,X_{t-k}) 只依赖 lag k，不依赖绝对时间 t。  
> **第三步，解释。** 关键结构是：弱平稳是二阶矩意义上的稳定，不等于 strict stationarity；很多协方差方法只需要弱平稳。  
...
- [030. HMM 与普通 Markov Chain 的区别？](../03-stochastic-processes/q030-HMM-与普通-Markov-Chain-的区别.md)
