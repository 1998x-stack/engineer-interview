---
id: q033
title: "Random Walk 为什么 non-stationary？"
chapter: "D. 时间序列与计量"
difficulty: "★☆☆"
tags: ["Unit root", "随机游走"]
source_type: "高可信重构题型"
...
# 033. Random Walk 为什么 non-stationary？

[← Q032](q032-AR-1-什么时候平稳-长期方差和-ACF-是什么.md) · [总索引](../../docs/100-question-index.md) · [Q034 →](q034-什么是-Unit-Root-ADF-test-在检验什么.md)
...
Random Walk 为什么 non-stationary？

---

## 1. 面试官到底在考什么

本题表面属于 **Unit root, 随机游走**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
X_t=X_0+Σ_{i=1}^t ε_i，因此 Var(X_t)=tσ² 随 t 增长，自协方差也依赖绝对时间，不满足弱平稳。

**推荐面试表达：** 用方差随时间增长这个一行证明最直接。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

Random walk $X_t=X_{t-1}+\epsilon_t$ 等价于 $X_t=X_0+\sum_{i=1}^t\epsilon_i$，所以 $\mathrm{Var}(X_t)=t\sigma^2$，协方差也依赖绝对时间而非仅 lag。差分 $\Delta X_t=\epsilon_t$ 则可平稳。
...
但一阶差分 ΔX_t=ε_t 可以是平稳的，所以 random walk 是 I(1) 的典型例子。

### 第二视角：如何验证主解

解析时间序列模型之外，再做 rolling/expanding OOS；理论上的平稳参数如果在滚动窗口剧烈变化，就应把 structural break 当一等问题。

对本题尤其值得继续追问的是：**Random walk 的方差随时间线性增长，因此“均值附近波动”直觉失效；差分后才得到平稳 innovation。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Random walk 的方差随时间线性增长，因此“均值附近波动”直觉失效；差分后才得到平稳 innovation。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Random walk 的方差随时间线性增长，因此“均值附近波动”直觉失效；差分后才得到平稳 innovation。
- **最值得保留的原始面试表达**：用方差随时间增长这个一行证明最直接。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

价格 level 常近似 I(1)，return 更接近平稳。若在非平稳 level 上直接相关/回归，容易得到伪显著。
...
- **错误 1**：只说“它有趋势”，但随机游走并不一定有确定性趋势。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：带 drift 的 random walk？

**回答方向：** 带 drift：$X_t=c+X_{t-1}+\epsilon_t$，均值线性随时间漂移，仍是非平稳 I(1)。

### 追问 2：为什么 level 回归容易伪回归？

**回答方向：** 两个独立 level 都带强随机趋势时，OLS 可把共同持久性误识别为关系，经典 t-stat 失效并产生 spurious regression。
...
> **第二步，主解。** 用最短的推导得到核心结论：X_t=X_0+Σ_{i=1}^t ε_i，因此 Var(X_t)=tσ² 随 t 增长，自协方差也依赖绝对时间，不满足弱平稳。  
> **第三步，解释。** 关键结构是：Random walk 的方差随时间线性增长，因此“均值附近波动”直觉失效；差分后才得到平稳 innovation。  
