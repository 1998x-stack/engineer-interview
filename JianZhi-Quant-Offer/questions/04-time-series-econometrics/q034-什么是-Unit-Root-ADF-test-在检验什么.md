---
id: q034
title: "什么是 Unit Root？ADF test 在检验什么？"
chapter: "D. 时间序列与计量"
difficulty: "★★☆"
tags: ["ADF", "unit root", "检验方向"]
source_type: "高可信重构题型"
version: "2.0"
---

# 034. 什么是 Unit Root？ADF test 在检验什么？

[← Q033](q033-Random-Walk-为什么-non-stationary.md) · [总索引](../../docs/100-question-index.md) · [Q035 →](q035-为什么两个独立-Random-Walk-回归会产生-Spurious-Regression.md)

> **难度**：★★☆  
> **标签**：ADF｜unit root｜检验方向  
> **题型口径**：高可信重构题型  
> **所属模块**：D. 时间序列与计量  
> **本题能力主线**：时间序列与计量
...
什么是 Unit Root？ADF test 在检验什么？

---

## 1. 面试官到底在考什么

本题表面属于 **ADF, unit root, 检验方向**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
AR(1) 中 φ=1 是 unit root。ADF 的原假设 H0 是存在 unit root（非平稳）；只有显著拒绝 H0 才支持平稳/无 unit root。

**推荐面试表达：** 先明确 H0 的方向，是这题最重要的防错点。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

ADF 通过回归 $\Delta X_t=\alpha+\beta t+\gamma X_{t-1}+\sum_i\delta_i\Delta X_{t-i}+\epsilon_t$ 检验 $H_0:\gamma=0$（unit root）。临界值不是普通 t 分布。
...
ADF 通过加入滞后差分吸收高阶序列相关；趋势项/截距规格选择会影响临界值和结论。

### 第二视角：如何验证主解

解析时间序列模型之外，再做 rolling/expanding OOS；理论上的平稳参数如果在滚动窗口剧烈变化，就应把 structural break 当一等问题。

对本题尤其值得继续追问的是：**ADF 的原假设方向是高频面试坑；更高级的回答会补充 low power、结构突变和 deterministic terms 对检验的影响。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

ADF 的原假设方向是高频面试坑；更高级的回答会补充 low power、结构突变和 deterministic terms 对检验的影响。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：ADF 的原假设方向是高频面试坑；更高级的回答会补充 low power、结构突变和 deterministic terms 对检验的影响。
- **最值得保留的原始面试表达**：先明确 H0 的方向，是这题最重要的防错点。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

ADF 的 deterministic terms、lag length 和结构突变都会影响结论。单个“不拒绝”不是证明 unit root，更不是证明不可预测。
...
- **错误 1**：p-value 大就宣布“平稳”。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：KPSS 与 ADF 的原假设为何相反？

**回答方向：** ADF 的 null 是 unit root；KPSS 的 null 是 level/trend stationarity。二者原假设相反，因此组合使用能区分“无法拒绝”与真正证据。

### 追问 2：结构突变为什么会影响 unit-root test？

**回答方向：** 结构突变会让一个分段平稳过程看起来像高持久性/unit root，降低 ADF power；应考虑 break dummies 或专门的 break-aware tests。
...
> **第二步，主解。** 用最短的推导得到核心结论：AR(1) 中 φ=1 是 unit root。ADF 的原假设 H0 是存在 unit root（非平稳）；只有显著拒绝 H0 才支持平稳/无 unit root。  
> **第三步，解释。** 关键结构是：ADF 的原假设方向是高频面试坑；更高级的回答会补充 low power、结构突变和 deterministic terms 对检验的影响。  
