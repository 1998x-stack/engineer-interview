---
id: q035
title: "为什么两个独立 Random Walk 回归会产生 Spurious Regression？"
chapter: "D. 时间序列与计量"
difficulty: "★★☆"
tags: ["伪回归", "非平稳", "t-stat"]
source_type: "高可信重构题型"
...
# 035. 为什么两个独立 Random Walk 回归会产生 Spurious Regression？

[← Q034](q034-什么是-Unit-Root-ADF-test-在检验什么.md) · [总索引](../../docs/100-question-index.md) · [Q036 →](q036-什么是-Cointegration.md)
...
为什么两个独立 Random Walk 回归会产生 Spurious Regression？

---

## 1. 面试官到底在考什么

本题表面属于 **伪回归, 非平稳, t-stat**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
非平稳 level 含有持续随机趋势，使标准 OLS 的经典渐近分布失效；即使两过程独立，也可能出现高 R² 和显著 t-stat。

**推荐面试表达：** 明确说“高 R² 可以纯粹来自共同的积分阶数”，体现计量意识。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

独立随机游走共享强持久性，OLS 会把随机趋势误当稳定关系，经典 t-stat 的极限分布不再适用；$R^2$ 可很高而残差仍非平稳。关键诊断是回归残差是否 stationary，而不是系数显著。
...
这不是“数据太少”，而是 stochastic trend 让回归残差结构不满足标准假设。差分或 cointegration framework 才是正确处理方向。

### 第二视角：如何验证主解

解析时间序列模型之外，再做 rolling/expanding OOS；理论上的平稳参数如果在滚动窗口剧烈变化，就应把 structural break 当一等问题。

对本题尤其值得继续追问的是：**Spurious regression 是“共享低频随机趋势 + 错误标准推断”的产物；高 R² 和显著 t-stat 不能替代残差平稳性检查。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Spurious regression 是“共享低频随机趋势 + 错误标准推断”的产物；高 R² 和显著 t-stat 不能替代残差平稳性检查。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Spurious regression 是“共享低频随机趋势 + 错误标准推断”的产物；高 R² 和显著 t-stat 不能替代残差平稳性检查。
- **最值得保留的原始面试表达**：明确说“高 R² 可以纯粹来自共同的积分阶数”，体现计量意识。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

任何跨资产 level regression 都应先问变量的 integration order。伪回归是“漂亮统计量不等于可信研究”的典型教材案例。
...
### 追问 1：Durbin-Watson 会表现怎样？

**回答方向：** 伪回归中残差通常强正自相关，因此 Durbin–Watson 往往很低。

### 追问 2：为什么增加样本反而不自动解决？

**回答方向：** unit-root 下标准 OLS 的极限理论本身就错误；增加样本会让随机趋势更充分地表现，不能靠更多数据自动恢复经典 t 分布。
...
> **第二步，主解。** 用最短的推导得到核心结论：非平稳 level 含有持续随机趋势，使标准 OLS 的经典渐近分布失效；即使两过程独立，也可能出现高 R² 和显著 t-stat。  
> **第三步，解释。** 关键结构是：Spurious regression 是“共享低频随机趋势 + 错误标准推断”的产物；高 R² 和显著 t-stat 不能替代残差平稳性检查。  
