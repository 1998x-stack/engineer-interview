---
id: q036
title: "什么是 Cointegration？"
chapter: "D. 时间序列与计量"
difficulty: "★★★"
tags: ["Cointegration", "ECM", "长期关系"]
source_type: "高可信重构题型"
version: "2.0"
---

# 036. 什么是 Cointegration？

[← Q035](q035-为什么两个独立-Random-Walk-回归会产生-Spurious-Regression.md) · [总索引](../../docs/100-question-index.md) · [Q037 →](q037-Granger-Causality-是真正的因果吗.md)

> **难度**：★★★  
> **标签**：Cointegration｜ECM｜长期关系  
> **题型口径**：高可信重构题型  
> **所属模块**：D. 时间序列与计量  
> **本题能力主线**：时间序列与计量
...
本题表面属于 **Cointegration, ECM, 长期关系**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
若 X_t,Y_t 各自 I(1)，但存在 β 使 Y_t-βX_t 为 I(0)，则二者 cointegrated。

**推荐面试表达：** 回答里必须出现“非平稳序列的某个线性组合平稳”。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

若 $X_t,Y_t$ 均 I(1)，但 $u_t=Y_t-\beta X_t$ 为 I(0)，则它们 cointegrated。Engle-Granger 两步法先估长期关系，再对残差做 unit-root test，并可写 ECM：$\Delta Y_t=\lambda u_{t-1}+\cdots$。
...
它描述长期均衡误差是平稳的；误差修正模型 ECM 将短期变化与偏离长期均衡的误差项结合。高相关不等于 cointegration。

### 第二视角：如何验证主解

解析时间序列模型之外，再做 rolling/expanding OOS；理论上的平稳参数如果在滚动窗口剧烈变化，就应把 structural break 当一等问题。

对本题尤其值得继续追问的是：**Cointegration 不是简单高相关，而是存在平稳误差组合；ECM 把短期变化与长期均衡偏离同时放进模型。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Cointegration 不是简单高相关，而是存在平稳误差组合；ECM 把短期变化与长期均衡偏离同时放进模型。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Cointegration 不是简单高相关，而是存在平稳误差组合；ECM 把短期变化与长期均衡偏离同时放进模型。
- **最值得保留的原始面试表达**：回答里必须出现“非平稳序列的某个线性组合平稳”。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

pairs/spread 研究里最常见的错误是把高 correlation 当 cointegration。cointegration 关注长期 stochastic trend 是否共享，而 correlation 可来自共同趋势。
...
- **错误 1**：用 correlation 代替 cointegration test。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：Engle-Granger 两步法？

**回答方向：** Engle–Granger：先对 I(1) 变量估长期回归，再对残差做 unit-root test；残差平稳则 cointegrated，随后可构建 ECM。

### 追问 2：Johansen 适合什么场景？

**回答方向：** Johansen 适合多变量系统，可同时估计多个 cointegrating vectors 和 rank，而不需指定单一因变量。
...
> **第二步，主解。** 用最短的推导得到核心结论：若 X_t,Y_t 各自 I(1)，但存在 β 使 Y_t-βX_t 为 I(0)，则二者 cointegrated。  
> **第三步，解释。** 关键结构是：Cointegration 不是简单高相关，而是存在平稳误差组合；ECM 把短期变化与长期均衡偏离同时放进模型。  
