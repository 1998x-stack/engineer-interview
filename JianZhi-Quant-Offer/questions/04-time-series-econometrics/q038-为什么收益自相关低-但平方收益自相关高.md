---
id: q038
title: "为什么收益自相关低，但平方收益自相关高？"
chapter: "D. 时间序列与计量"
difficulty: "★★☆"
tags: ["Volatility clustering", "GARCH"]
source_type: "高可信重构题型"
version: "2.0"
---

# 038. 为什么收益自相关低，但平方收益自相关高？

[← Q037](q037-Granger-Causality-是真正的因果吗.md) · [总索引](../../docs/100-question-index.md) · [Q039 →](q039-为什么金融时间序列不能随机-train-test-split.md)

> **难度**：★★☆  
> **标签**：Volatility clustering｜GARCH  
> **题型口径**：高可信重构题型  
> **所属模块**：D. 时间序列与计量  
> **本题能力主线**：时间序列与计量
...
本题表面属于 **Volatility clustering, GARCH**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
市场方向可能接近不可预测，但波动强度具有持续性。于是 Corr(r_t,r_{t-k}) 小，而 Corr(r_t²,r_{t-k}²) 显著。

**推荐面试表达：** 先拆 direction 与 magnitude 两类可预测性。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

收益可近似 serially uncorrelated，但条件方差具有持久性。GARCH(1,1)：$\sigma_t^2=\omega+\alpha\epsilon_{t-1}^2+\beta\sigma_{t-1}^2$；弱平稳常要求 $\alpha+\beta<1$。
...
GARCH(1,1): σ_t²=ω+αε_{t-1}²+βσ_{t-1}²，用过去 shock 与过去条件方差描述 volatility persistence。

### 第二视角：如何验证主解

解析时间序列模型之外，再做 rolling/expanding OOS；理论上的平稳参数如果在滚动窗口剧烈变化，就应把 structural break 当一等问题。

对本题尤其值得继续追问的是：**收益均值可能近似不可预测，但条件方差可预测；这解释了为什么 volatility model 与 return-direction model 是不同任务。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

收益均值可能近似不可预测，但条件方差可预测；这解释了为什么 volatility model 与 return-direction model 是不同任务。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：收益均值可能近似不可预测，但条件方差可预测；这解释了为什么 volatility model 与 return-direction model 是不同任务。
- **最值得保留的原始面试表达**：先拆 direction 与 magnitude 两类可预测性。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

volatility clustering 是“均值可难预测、风险仍可预测”的核心例子。实际还需考虑 leverage/asymmetry、fat tails 和 intraday seasonality。
...
**回答方向：** $\alpha+\beta$ 接近 1 表示 volatility shock 衰减非常慢、持久性高；等于 1 的 IGARCH 有长期方差问题。

### 追问 2：为何 leverage effect 需要 EGARCH/GJR？

**回答方向：** 标准 GARCH 对正负冲击只看平方，无法表达负收益更强地抬升波动；EGARCH/GJR 增加符号/非对称项。
...
> **第二步，主解。** 用最短的推导得到核心结论：市场方向可能接近不可预测，但波动强度具有持续性。于是 Corr(r_t,r_{t-k}) 小，而 Corr(r_t²,r_{t-k}²) 显著。  
> **第三步，解释。** 关键结构是：收益均值可能近似不可预测，但条件方差可预测；这解释了为什么 volatility model 与 return-direction model 是不同任务。  
