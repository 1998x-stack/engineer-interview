---
id: q046
title: "如何判断分类概率是否 calibrated？"
chapter: "E. 机器学习与 Financial ML"
difficulty: "★★☆"
tags: ["Calibration", "Brier", "reliability"]
source_type: "高可信重构题型"
version: "2.0"
---

# 046. 如何判断分类概率是否 calibrated？

[← Q045](q045-一个-feature-的-IC-很低-是否一定没有价值.md) · [总索引](../../docs/100-question-index.md) · [Q047 →](q047-Finance-中为什么-accuracy-往往不是好指标.md)

> **难度**：★★☆  
> **标签**：Calibration｜Brier｜reliability  
> **题型口径**：高可信重构题型  
...
如何判断分类概率是否 calibrated？

---

## 1. 面试官到底在考什么

本题表面属于 **Calibration, Brier, reliability**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
把预测概率分桶，看预测 0.7 的样本是否约 70% 为正；可用 reliability diagram、Brier score、ECE。

**推荐面试表达：** 先明确 discrimination 与 calibration 是两个维度。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

完美 calibration 满足 $P(Y=1\mid \hat p=p)=p$。Brier score 为 $N^{-1}\sum(\hat p_i-y_i)^2$，同时受 calibration 与 refinement 影响。Reliability diagram 将预测概率分桶比较 empirical frequency。
...
若排序能力好但 calibration 差，可用 Platt scaling、isotonic regression、temperature scaling 等后处理；校准必须在独立 validation 上做。

### 第二视角：如何验证主解

复杂模型的任何提升都应对照 simple baseline + ablation + permutation/placebo；如果移除最可疑特征后性能崩塌，应首先怀疑泄漏或脆弱依赖。

对本题尤其值得继续追问的是：**好的概率模型应同时看 discrimination 与 calibration；AUC 高不代表概率数值可直接用于下游期望或风险计算。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

好的概率模型应同时看 discrimination 与 calibration；AUC 高不代表概率数值可直接用于下游期望或风险计算。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：好的概率模型应同时看 discrimination 与 calibration；AUC 高不代表概率数值可直接用于下游期望或风险计算。
- **最值得保留的原始面试表达**：先明确 discrimination 与 calibration 是两个维度。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

交易决策若以概率驱动头寸/阈值，calibration 往往比 accuracy 更关键。drift 后最先坏掉的可能就是 calibration。
...
- **错误 1**：把 classification accuracy 当 calibration。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：AUC 很高但 calibration 很差可能吗？

**回答方向：** 完全可能。AUC 只评价排序，任何单调变换都不改变 AUC；预测概率即使整体过度自信，排序仍可很好。

### 追问 2：Brier score 如何分解？

**回答方向：** Brier 可分解为 reliability/calibration、resolution 与 uncertainty（Murphy decomposition），帮助区分“概率偏”还是“本身区分度不足”。
...
> **第二步，主解。** 用最短的推导得到核心结论：把预测概率分桶，看预测 0.7 的样本是否约 70% 为正；可用 reliability diagram、Brier score、ECE。  
> **第三步，解释。** 关键结构是：好的概率模型应同时看 discrimination 与 calibration；AUC 高不代表概率数值可直接用于下游期望或风险计算。  
...
- [041. 训练准确率 99%，测试 52%，你首先检查什么？](q041-训练准确率-99%-测试-52%-你首先检查什么.md)
