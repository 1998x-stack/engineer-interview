---
id: q047
title: "Finance 中为什么 accuracy 往往不是好指标？"
chapter: "E. 机器学习与 Financial ML"
difficulty: "★☆☆"
tags: ["Imbalanced data", "metric", "utility"]
source_type: "高可信重构题型"
version: "2.0"
---

# 047. Finance 中为什么 accuracy 往往不是好指标？

[← Q046](q046-如何判断分类概率是否-calibrated.md) · [总索引](../../docs/100-question-index.md) · [Q048 →](q048-什么时候-Transformer-可能优于传统时间序列模型.md)

> **难度**：★☆☆  
> **标签**：Imbalanced data｜metric｜utility  
> **题型口径**：高可信重构题型  
...
Finance 中为什么 accuracy 往往不是好指标？

---

## 1. 面试官到底在考什么

本题表面属于 **Imbalanced data, metric, utility**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
类别不平衡时 accuracy 可被 majority class 轻易“刷高”；更关键的是业务损失/收益通常不对称，分类对错的价值并不相同。

**推荐面试表达：** 先问“错误的代价是否对称、基准率多少”，再选 metric。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

Accuracy 隐含对所有样本和两类错误赋相同损失。在不平衡事件中，一个常数预测器即可获得高 accuracy。正确 metric 应与 decision utility 和 class prevalence 匹配。
...
应根据任务选 PR-AUC、log loss、Brier、rank metrics、cost-sensitive metric 或更贴近预测任务的 utility proxy。

### 第二视角：如何验证主解

复杂模型的任何提升都应对照 simple baseline + ablation + permutation/placebo；如果移除最可疑特征后性能崩塌，应首先怀疑泄漏或脆弱依赖。

对本题尤其值得继续追问的是：**指标必须和真实决策目标对齐；类别不平衡时 accuracy 可能被多数类支配，排序问题、概率问题和效用问题应使用不同 metric。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

指标必须和真实决策目标对齐；类别不平衡时 accuracy 可能被多数类支配，排序问题、概率问题和效用问题应使用不同 metric。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：指标必须和真实决策目标对齐；类别不平衡时 accuracy 可能被多数类支配，排序问题、概率问题和效用问题应使用不同 metric。
- **最值得保留的原始面试表达**：先问“错误的代价是否对称、基准率多少”，再选 metric。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

金融预测常更关心 rank、PnL after cost、tail recall、calibration 或 expected utility。指标选择本身决定模型在优化什么。
...
**回答方向：** 极端不平衡时 ROC 的 FPR 分母是大量负类，即使产生很多 false positives，FPR 仍可能很小；PR-AUC 更聚焦正类 precision-recall。

### 追问 2：阈值选择如何依赖成本矩阵？

**回答方向：** 若 FP/FN 成本不同，应选择使期望成本最小的阈值，阈值依赖 posterior probability、base rate 和 cost matrix，而非固定 0.5。
...
> **第二步，主解。** 用最短的推导得到核心结论：类别不平衡时 accuracy 可被 majority class 轻易“刷高”；更关键的是业务损失/收益通常不对称，分类对错的价值并不相同。  
> **第三步，解释。** 关键结构是：指标必须和真实决策目标对齐；类别不平衡时 accuracy 可能被多数类支配，排序问题、概率问题和效用问题应使用不同 metric。  
