---
id: q055
title: "Price-Time Priority 是什么？"
chapter: "F. 市场微观结构"
difficulty: "★☆☆"
tags: ["Matching engine", "queue", "priority"]
source_type: "高可信重构题型"
version: "2.0"
---

# 055. Price-Time Priority 是什么？

[← Q054](q054-Order-Book-Imbalance-如何定义-为什么不能把它当永恒预测信号.md) · [总索引](../../docs/100-question-index.md) · [Q056 →](q056-为什么-Fill-Probability-是-execution-simulation-的关键.md)

> **难度**：★☆☆  
> **标签**：Matching engine｜queue｜priority  
> **题型口径**：高可信重构题型  
> **所属模块**：F. 市场微观结构  
> **本题能力主线**：市场微观结构
...
Price-Time Priority 是什么？

---

## 1. 面试官到底在考什么

本题表面属于 **Matching engine, queue, priority**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
通常先按价格优先，再在同价位按进入时间排序。因而同一 best price 上，不同订单的 queue position 会影响 fill probability。

**推荐面试表达：** 把 matching rule 与 simulation bias 联系起来。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

Price-time priority 下先按更优价格，再在同一价格按时间先后排序。对 limit order，是否成交不仅取决于市场是否触及价格，还取决于你前方 queue 的消耗、撤单和新增优先级规则。
...
研究中只看到 top-of-book price/size 不足以重构个人订单成交，需要追踪 order arrivals、cancellations、executions 与自身队列位置。

### 第二视角：如何验证主解

把公式/统计量放回 event stream 中 replay；微观结构指标只有在真实 queue、latency 和 fill 机制下仍成立，才有 execution 意义。

对本题尤其值得继续追问的是：**Price-time priority 使“同价订单”并不等价；成交概率必须知道 queue ahead，而不是只知道 best price。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Price-time priority 使“同价订单”并不等价；成交概率必须知道 queue ahead，而不是只知道 best price。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Price-time priority 使“同价订单”并不等价；成交概率必须知道 queue ahead，而不是只知道 best price。
- **最值得保留的原始面试表达**：把 matching rule 与 simulation bias 联系起来。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

因此只用 OHLC “touch” 假设成交是严重 optimistic bias。高频 backtest 应显式维护 queue state 或至少用保守 fill model。
...
- **错误 1**：把“价格触及”当“自己的订单一定成交”。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：pro-rata matching 与 price-time 有何区别？

**回答方向：** pro-rata 在同价位按订单 size 比例分配成交，而 price-time 是先到先得；因此最优下单规模、排队行为和 fill 模型完全不同。

### 追问 2：replace order 会不会丢 priority？

**回答方向：** 取决于 venue 规则。通常增量修改价格或数量可能丢失/重置 priority，而减量有时保留；生产系统必须按交易所 spec 实现。
...
> **第二步，主解。** 用最短的推导得到核心结论：通常先按价格优先，再在同价位按进入时间排序。因而同一 best price 上，不同订单的 queue position 会影响 fill probability。  
> **第三步，解释。** 关键结构是：Price-time priority 使“同价订单”并不等价；成交概率必须知道 queue ahead，而不是只知道 best price。  
...
- [060. Latency 为什么既是系统问题，也是统计问题？](q060-Latency-为什么既是系统问题-也是统计问题.md)
