---
id: q056
title: "为什么 Fill Probability 是 execution simulation 的关键？"
chapter: "F. 市场微观结构"
difficulty: "★★☆"
tags: ["Execution model", "queue", "simulation bias"]
source_type: "高可信重构题型"
version: "2.0"
---

# 056. 为什么 Fill Probability 是 execution simulation 的关键？

[← Q055](q055-Price-Time-Priority-是什么.md) · [总索引](../../docs/100-question-index.md) · [Q057 →](q057-Market-Impact-的-temporary-与-permanent-如何理解.md)

> **难度**：★★☆  
> **标签**：Execution model｜queue｜simulation bias  
> **题型口径**：高可信重构题型  
> **所属模块**：F. 市场微观结构  
> **本题能力主线**：市场微观结构
...
为什么 Fill Probability 是 execution simulation 的关键？

---

## 1. 面试官到底在考什么

本题表面属于 **Execution model, queue, simulation bias**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
价格到达你的限价不代表订单必然成交；前方 queue、trade size、cancel、latency、partial fill 都决定实际 fill。

**推荐面试表达：** 先指出“price path ≠ fill path”。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

Fill probability 可写成条件概率 $P(F_{t:t+h}=1\mid state_t,order_t)$，state 包括 queue ahead、order-flow、cancel rate、volatility、latency 等。触价不是充分条件。
...
若回测假设“touch=full fill”，会系统性高估 execution quality。研究应明确 fill model 的可观察状态、假设与校准方式。

### 第二视角：如何验证主解

把公式/统计量放回 event stream 中 replay；微观结构指标只有在真实 queue、latency 和 fill 机制下仍成立，才有 execution 意义。

对本题尤其值得继续追问的是：**若回测用“touch=fill”，会系统性高估执行；真实 fill 取决于队列位置、到达/撤单过程、延迟和成交量。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

若回测用“touch=fill”，会系统性高估执行；真实 fill 取决于队列位置、到达/撤单过程、延迟和成交量。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：若回测用“touch=fill”，会系统性高估执行；真实 fill 取决于队列位置、到达/撤单过程、延迟和成交量。
- **最值得保留的原始面试表达**：先指出“price path ≠ fill path”。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

若回测 fill model 过于乐观，会同时高估成交率并低估 adverse selection，因为真实最容易成交的时刻往往也是最不利的时刻。
...
- **错误 1**：用 mid-price signal 回测，却忽略 execution layer。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：如何用历史 order-level data 校准？

**回答方向：** 用真实 L3/order log 将下单时 state 与后续 fill/partial fill/latency 对齐，按 queue ahead、distance、flow、volatility 分桶或训练 hazard model。

### 追问 2：没有 L3 数据时如何做敏感性分析？

**回答方向：** 没有 L3 时应做上下界/多情景：touch=fill 的乐观模型、需要 trade-through 的保守模型、不同 queue proxy，并报告策略对 fill assumption 的敏感性。
...
> **第二步，主解。** 用最短的推导得到核心结论：价格到达你的限价不代表订单必然成交；前方 queue、trade size、cancel、latency、partial fill 都决定实际 fill。  
> **第三步，解释。** 关键结构是：若回测用“touch=fill”，会系统性高估执行；真实 fill 取决于队列位置、到达/撤单过程、延迟和成交量。  
