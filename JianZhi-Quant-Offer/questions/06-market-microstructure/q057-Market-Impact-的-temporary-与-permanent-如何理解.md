---
id: q057
title: "Market Impact 的 temporary 与 permanent 如何理解？"
chapter: "F. 市场微观结构"
difficulty: "★★☆"
tags: ["Impact", "execution cost", "information"]
source_type: "高可信重构题型"
version: "2.0"
---

# 057. Market Impact 的 temporary 与 permanent 如何理解？

[← Q056](q056-为什么-Fill-Probability-是-execution-simulation-的关键.md) · [总索引](../../docs/100-question-index.md) · [Q058 →](q058-Tick-Size-为什么会改变市场行为.md)

> **难度**：★★☆  
> **标签**：Impact｜execution cost｜information  
> **题型口径**：高可信重构题型  
> **所属模块**：F. 市场微观结构  
> **本题能力主线**：市场微观结构
...
Market Impact 的 temporary 与 permanent 如何理解？

---

## 1. 面试官到底在考什么

本题表面属于 **Impact, execution cost, information**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
temporary impact 随交易结束后部分回撤，更多反映流动性供需；permanent component 反映信息或持久的价格重估。

**推荐面试表达：** 答题重点是 identification problem，而不是背一个 impact 公式。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

Temporary impact 是执行压力造成、随后部分回撤的价格偏移；permanent impact 是与信息/长期价格更新相关的持久部分。经验上二者难完全识别，因为交易方向与信息通常内生。
...
真实估计很难，因为自然价格变化与自家交易影响纠缠，存在 selection bias；需要控制订单条件、市场状态及 post-trade horizon。

### 第二视角：如何验证主解

把公式/统计量放回 event stream 中 replay；微观结构指标只有在真实 queue、latency 和 fill 机制下仍成立，才有 execution 意义。

对本题尤其值得继续追问的是：**Temporary impact 更像流动性消耗，permanent impact 更像信息进入价格；二者的估计对执行成本模型和策略容量判断都重要。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Temporary impact 更像流动性消耗，permanent impact 更像信息进入价格；二者的估计对执行成本模型和策略容量判断都重要。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Temporary impact 更像流动性消耗，permanent impact 更像信息进入价格；二者的估计对执行成本模型和策略容量判断都重要。
- **最值得保留的原始面试表达**：答题重点是 identification problem，而不是背一个 impact 公式。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

execution cost model 若把所有 impact 当临时，会低估有信息订单的真实代价；反之把全部永久化也会高估。
...
- **错误 1**：把所有 post-trade move 都归因于自己的交易。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：平方根 impact law 是经验规律还是定律？

**回答方向：** 平方根 impact 是广泛观察到的经验 scaling law，不是无条件定律；指数、常数随市场、horizon、参与率和定义变化。

### 追问 2：怎么区分 impact 与 alpha？

**回答方向：** 订单方向通常由 alpha 决定，price move 同时含预测正确和自身冲击。可用控制组、自然实验、执行轨迹/参与率变化、post-trade decay 做识别。
...
> **第二步，主解。** 用最短的推导得到核心结论：temporary impact 随交易结束后部分回撤，更多反映流动性供需；permanent component 反映信息或持久的价格重估。  
> **第三步，解释。** 关键结构是：Temporary impact 更像流动性消耗，permanent impact 更像信息进入价格；二者的估计对执行成本模型和策略容量判断都重要。  
