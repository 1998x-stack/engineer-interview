---
id: q058
title: "Tick Size 为什么会改变市场行为？"
chapter: "F. 市场微观结构"
difficulty: "★★☆"
tags: ["Tick size", "queue competition", "liquidity"]
source_type: "高可信重构题型"
version: "2.0"
---

# 058. Tick Size 为什么会改变市场行为？

[← Q057](q057-Market-Impact-的-temporary-与-permanent-如何理解.md) · [总索引](../../docs/100-question-index.md) · [Q059 →](q059-什么是-Microprice-为什么可能不同于-Mid.md)

> **难度**：★★☆  
> **标签**：Tick size｜queue competition｜liquidity  
> **题型口径**：高可信重构题型  
> **所属模块**：F. 市场微观结构  
> **本题能力主线**：市场微观结构
...
Tick Size 为什么会改变市场行为？

---

## 1. 面试官到底在考什么

本题表面属于 **Tick size, queue competition, liquidity**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
tick 大时 spread 更容易被最小跳动约束、同价位 queue 竞争更强；tick 小时价格改善更细、quote 更新可能更频繁。

**推荐面试表达：** 从离散价格网格对行为的约束讲起。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

Tick size 是最小价格离散单位。若 tick 相对自然 spread 很大，spread 被价格网格绑定，竞争转移到 queue priority；若 tick 很小，价格层级更细但 depth 可能分散。
...
因此 feature 分布、queue dynamics、spread statistics 和模型参数都可能随 tick regime 改变，跨市场迁移要重新校准。

### 第二视角：如何验证主解

把公式/统计量放回 event stream 中 replay；微观结构指标只有在真实 queue、latency 和 fill 机制下仍成立，才有 execution 意义。

对本题尤其值得继续追问的是：**Tick size 是离散化约束，会改变 spread floor、queue competition 与 price improvement，从而改变同一 signal 的可交易性。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Tick size 是离散化约束，会改变 spread floor、queue competition 与 price improvement，从而改变同一 signal 的可交易性。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Tick size 是离散化约束，会改变 spread floor、queue competition 与 price improvement，从而改变同一 signal 的可交易性。
- **最值得保留的原始面试表达**：从离散价格网格对行为的约束讲起。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

同一 microstructure feature 跨 tick regimes 的含义会变，因此模型需要 tick-normalized features 或 regime stratification。
...
- **错误 1**：认为盘口模型只需做 price normalization 就可跨品种复用。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：tick/price ratio 为什么更重要？

**回答方向：** 绝对 tick 对不同价格资产不可比；$tick/price$ 或 tick 相对自然 spread 才反映价格离散化强度。

### 追问 2：tick change 的 natural experiment 如何研究？

**回答方向：** 利用交易所 tick rule 外生变化做 event study/difference-in-differences，比较 spread、depth、queue、volume，并控制同期市场变化和处理组选择。
...
> **第二步，主解。** 用最短的推导得到核心结论：tick 大时 spread 更容易被最小跳动约束、同价位 queue 竞争更强；tick 小时价格改善更细、quote 更新可能更频繁。  
> **第三步，解释。** 关键结构是：Tick size 是离散化约束，会改变 spread floor、queue competition 与 price improvement，从而改变同一 signal 的可交易性。  
