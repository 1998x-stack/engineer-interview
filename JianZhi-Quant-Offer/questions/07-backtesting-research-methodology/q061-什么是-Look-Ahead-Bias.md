---
id: q061
title: "什么是 Look-Ahead Bias？"
chapter: "G. 数据、回测与研究方法论"
difficulty: "★☆☆"
tags: ["Backtest", "leakage", "timestamp"]
source_type: "高可信重构题型"
version: "2.0"
---

# 061. 什么是 Look-Ahead Bias？

[← Q060](../06-market-microstructure/q060-Latency-为什么既是系统问题-也是统计问题.md) · [总索引](../../docs/100-question-index.md) · [Q062 →](q062-什么是-Survivorship-Bias.md)

> **难度**：★☆☆  
> **标签**：Backtest｜leakage｜timestamp  
> **题型口径**：高可信重构题型  
> **所属模块**：G. 数据、回测与研究方法论  
> **本题能力主线**：回测与研究方法论
...
本题表面属于 **Backtest, leakage, timestamp**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
在历史时点 t 的决策中使用了实际只能在 t 之后获得的信息。它可以来自 label、数据发布时间、修订值、全样本预处理或错误对齐。

**推荐面试表达：** 问自己：这一列在历史那一刻真的已经知道了吗？

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

Look-ahead bias 的形式是 feature/decision at time $t$ 使用了 information availability time $>t$ 的字段。必须区分 event time、publication time、vendor arrival time、processed time。
...
防线是 point-in-time feature construction：每个 feature 都应有 event time、available time/knowledge time，回放时只允许 available_time≤decision_time。

### 第二视角：如何验证主解

把 backtest 看成一个可证伪的软件实验：构造 placebo、延迟特征、打乱标签、提高成本、改变 universe 与 subperiod，观察结论是否按预期退化。

对本题尤其值得继续追问的是：**Look-ahead bias 的判定标准不是“代码有没有未来索引”，而是该字段在决策时刻是否真实可获得。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Look-ahead bias 的判定标准不是“代码有没有未来索引”，而是该字段在决策时刻是否真实可获得。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Look-ahead bias 的判定标准不是“代码有没有未来索引”，而是该字段在决策时刻是否真实可获得。
- **最值得保留的原始面试表达**：问自己：这一列在历史那一刻真的已经知道了吗？
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

很多泄漏不是显式“未来价格”，而是 revision 后宏观数据、当天最终成分股、收盘后才知道的 corporate action 或全日统计量。
...
- **错误 1**：用数据行上的“日期”代替真正可获得时间。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：财报数据发布日期与报告期末的区别？

**回答方向：** 报告期末只是经济数据所属日期，发布日期才决定何时市场能知道；回测必须用 publication/available_at，而不能把期末值提前到期末。

### 追问 2：延迟 vendor feed 怎么处理？

**回答方向：** vendor feed 有额外到达延迟，应按研究实际可用的 receive/ingest time 对齐，并做 latency buffer；不能用 vendor 后修正的理想时间。
...
> **第二步，主解。** 用最短的推导得到核心结论：在历史时点 t 的决策中使用了实际只能在 t 之后获得的信息。它可以来自 label、数据发布时间、修订值、全样本预处理或错误对齐。  
> **第三步，解释。** 关键结构是：Look-ahead bias 的判定标准不是“代码有没有未来索引”，而是该字段在决策时刻是否真实可获得。  
...
- [060. Latency 为什么既是系统问题，也是统计问题？](../06-market-microstructure/q060-Latency-为什么既是系统问题-也是统计问题.md)
