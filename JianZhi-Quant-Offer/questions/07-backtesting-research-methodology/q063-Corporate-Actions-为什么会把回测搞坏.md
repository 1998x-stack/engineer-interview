---
id: q063
title: "Corporate Actions 为什么会把回测搞坏？"
chapter: "G. 数据、回测与研究方法论"
difficulty: "★★☆"
tags: ["Adjusted data", "split", "dividend"]
source_type: "高可信重构题型"
version: "2.0"
---

# 063. Corporate Actions 为什么会把回测搞坏？

[← Q062](q062-什么是-Survivorship-Bias.md) · [总索引](../../docs/100-question-index.md) · [Q064 →](q064-Point-in-Time-Data-是什么.md)

> **难度**：★★☆  
> **标签**：Adjusted data｜split｜dividend  
> **题型口径**：高可信重构题型  
> **所属模块**：G. 数据、回测与研究方法论  
> **本题能力主线**：回测与研究方法论
...
Corporate Actions 为什么会把回测搞坏？

---

## 1. 面试官到底在考什么

本题表面属于 **Adjusted data, split, dividend**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
split、dividend、merger、spin-off、symbol change 会改变价格/股数/现金流。若不按研究目标正确调整，收益、volume、position accounting 都会出现假跳变。

**推荐面试表达：** 先明确“研究变量的经济含义”，再选 adjustment convention。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

split、dividend、rights issue、merger 会改变价格、股数和经济持有价值。使用 adjusted price 时必须理解 adjustment convention，并保证特征、标签和成交价处理一致。
...
例如 2-for-1 split 的原始价格约减半，但这不是 -50% 经济损失。应明确使用 raw、price-adjusted 还是 total-return series。

### 第二视角：如何验证主解

把 backtest 看成一个可证伪的软件实验：构造 placebo、延迟特征、打乱标签、提高成本、改变 universe 与 subperiod，观察结论是否按预期退化。

对本题尤其值得继续追问的是：**Corporate actions 会改变价格、股本、现金流和 ticker lineage；数据供应商的 adjustment convention 必须与 label/return 定义一致。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Corporate actions 会改变价格、股本、现金流和 ticker lineage；数据供应商的 adjustment convention 必须与 label/return 定义一致。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Corporate actions 会改变价格、股本、现金流和 ticker lineage；数据供应商的 adjustment convention 必须与 label/return 定义一致。
- **最值得保留的原始面试表达**：先明确“研究变量的经济含义”，再选 adjustment convention。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

最危险的是“研究特征用 adjusted、执行模拟用 raw”或反之，导致不可解释的 jump/return。
...
- **错误 1**：只调整价格，不调整份额/volume。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：volume 如何随 split 调整？

**回答方向：** 2-for-1 split 后 share volume 的经济单位也变化；历史 adjusted volume 通常需反向按 split factor 调整，使成交股数口径可比。

### 追问 2：dividend 在 close-to-close return 中怎么处理？

**回答方向：** close-to-close total return 应把除息现金流计入：$(P_t+D_t)/P_{t-1}-1$（概念式）；若用 adjusted close，需理解 vendor 已如何处理股息。
...
> **第二步，主解。** 用最短的推导得到核心结论：split、dividend、merger、spin-off、symbol change 会改变价格/股数/现金流。若不按研究目标正确调整，收益、volume、position accounting 都会出现假跳变。  
> **第三步，解释。** 关键结构是：Corporate actions 会改变价格、股本、现金流和 ticker lineage；数据供应商的 adjustment convention 必须与 label/return 定义一致。  
...
- [062. 什么是 Survivorship Bias？](q062-什么是-Survivorship-Bias.md)
