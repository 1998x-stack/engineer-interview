---
id: q064
title: "Point-in-Time Data 是什么？"
chapter: "G. 数据、回测与研究方法论"
difficulty: "★★☆"
tags: ["PIT", "revision", "data lineage"]
source_type: "高可信重构题型"
version: "2.0"
---

# 064. Point-in-Time Data 是什么？

[← Q063](q063-Corporate-Actions-为什么会把回测搞坏.md) · [总索引](../../docs/100-question-index.md) · [Q065 →](q065-为什么-transaction-cost-不能简单固定减-1bp.md)

> **难度**：★★☆  
> **标签**：PIT｜revision｜data lineage  
> **题型口径**：高可信重构题型  
> **所属模块**：G. 数据、回测与研究方法论  
> **本题能力主线**：回测与研究方法论
...
Point-in-Time Data 是什么？

---

## 1. 面试官到底在考什么

本题表面属于 **PIT, revision, data lineage**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
指在历史时点真正可获得的那一版数据，而不是今天数据库回填/修订后的最终历史。

**推荐面试表达：** 量化数据工程的核心：事实发生时间与我们知道事实的时间是两条轴。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

Point-in-time 数据要求查询历史时只能看到当时已发布且当时版本的数据。对可修订字段，需要保存 vintage/revision history，而不是今天数据库的最终值。
...
宏观数据、基本面、评级、指数成分等都可能 revision。理想数据模型至少区分 event/effective time 与 knowledge/ingestion time，实现 bitemporal lineage。

### 第二视角：如何验证主解

把 backtest 看成一个可证伪的软件实验：构造 placebo、延迟特征、打乱标签、提高成本、改变 universe 与 subperiod，观察结论是否按预期退化。

对本题尤其值得继续追问的是：**Point-in-time data 要同时记录 effective time、release time、ingestion time 和 revision；“今天看到的历史值”未必是过去当时看到的值。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Point-in-time data 要同时记录 effective time、release time、ingestion time 和 revision；“今天看到的历史值”未必是过去当时看到的值。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Point-in-time data 要同时记录 effective time、release time、ingestion time 和 revision；“今天看到的历史值”未必是过去当时看到的值。
- **最值得保留的原始面试表达**：量化数据工程的核心：事实发生时间与我们知道事实的时间是两条轴。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

财报、宏观数据、指数成分、评级、行业分类都常被后修订。PIT 是 fundamental/alternative research 的基础设施要求。
...
### 追问 1：vintage data 为什么重要？

**回答方向：** 宏观/基本面数据会修订；今天看到的最终历史值可能当时不存在。vintage data 保存每个发布日期版本，才能真实重建历史决策。

### 追问 2：如何存储 restatement？

**回答方向：** 将 observation date 与 revision/known-at time 双时间存储，保留每次 restatement 的值、来源、版本和生效时间，而不是覆盖旧值。
...
> **第二步，主解。** 用最短的推导得到核心结论：指在历史时点真正可获得的那一版数据，而不是今天数据库回填/修订后的最终历史。  
> **第三步，解释。** 关键结构是：Point-in-time data 要同时记录 effective time、release time、ingestion time 和 revision；“今天看到的历史值”未必是过去当时看到的值。  
