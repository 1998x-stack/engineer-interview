---
id: q068
title: "Alternative Data 有大量 missing values，怎么处理？"
chapter: "G. 数据、回测与研究方法论"
difficulty: "★★☆"
tags: ["Missingness", "MNAR", "数据质量"]
source_type: "高可信重构题型"
version: "2.0"
---

# 068. Alternative Data 有大量 missing values，怎么处理？

[← Q067](q067-Walk-Forward-Validation-怎么做.md) · [总索引](../../docs/100-question-index.md) · [Q069 →](q069-如何判断-improvement-是真-signal-还是-research-overfitting.md)

> **难度**：★★☆  
> **标签**：Missingness｜MNAR｜数据质量  
> **题型口径**：高可信重构题型  
> **所属模块**：G. 数据、回测与研究方法论  
> **本题能力主线**：回测与研究方法论
...
Alternative Data 有大量 missing values，怎么处理？

---

## 1. 面试官到底在考什么

本题表面属于 **Missingness, MNAR, 数据质量**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
先判断 missing mechanism 和业务过程：MCAR/MAR/MNAR。缺失可能本身携带信息，不能默认 fillna(0)；更不能用未来值回填。

**推荐面试表达：** 先问“为什么缺失”，再问“怎么填”。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

先识别 missing mechanism：MCAR、MAR、MNAR。金融 alternative data 中 MNAR 很常见，例如“没有记录”可能意味着店铺关闭、用户不活跃或覆盖不足。
...
可增加 missing indicator、按可获得历史做 forward-fill、模型化缺失、分群处理，并检查 vendor coverage 变化是否制造伪 signal。

### 第二视角：如何验证主解

把 backtest 看成一个可证伪的软件实验：构造 placebo、延迟特征、打乱标签、提高成本、改变 universe 与 subperiod，观察结论是否按预期退化。

对本题尤其值得继续追问的是：**Missingness 可能是信号本身；先识别 MCAR/MAR/MNAR，再决定插补、missing indicator、模型原生处理或样本过滤。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Missingness 可能是信号本身；先识别 MCAR/MAR/MNAR，再决定插补、missing indicator、模型原生处理或样本过滤。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Missingness 可能是信号本身；先识别 MCAR/MAR/MNAR，再决定插补、missing indicator、模型原生处理或样本过滤。
- **最值得保留的原始面试表达**：先问“为什么缺失”，再问“怎么填”。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

缺失处理必须 point-in-time；forward fill 的最大 horizon、freshness 与 source coverage 都应成为显式特征/约束。
...
- **错误 1**：缺失=0、缺失=均值的机械处理。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：forward-fill 最大允许间隔？

**回答方向：** 按变量语义设最大 staleness：超过阈值视为 missing，而非无限 forward-fill；阈值应基于更新频率和业务有效期做 OOS sensitivity。

### 追问 2：coverage expansion 如何造成 backtest bias？

**回答方向：** 供应商覆盖随时间扩大时，后期 missing 更少，模型可能学到“数据是否被覆盖”而不是经济 signal；若回填历史覆盖会产生不可获得信息偏差。
...
> **第二步，主解。** 用最短的推导得到核心结论：先判断 missing mechanism 和业务过程：MCAR/MAR/MNAR。缺失可能本身携带信息，不能默认 fillna(0)；更不能用未来值回填。  
> **第三步，解释。** 关键结构是：Missingness 可能是信号本身；先识别 MCAR/MAR/MNAR，再决定插补、missing indicator、模型原生处理或样本过滤。  
...
- [063. Corporate Actions 为什么会把回测搞坏？](q063-Corporate-Actions-为什么会把回测搞坏.md)
