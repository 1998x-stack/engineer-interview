---
id: q071
title: "推导 Minimum-Variance Portfolio。"
chapter: "H. 组合、风险与优化"
difficulty: "★★★"
tags: ["Portfolio optimization", "Lagrange"]
source_type: "高可信重构题型"
version: "2.0"
---

# 071. 推导 Minimum-Variance Portfolio。

[← Q070](../07-backtesting-research-methodology/q070-一个可复现-Quant-Research-Experiment-应保存什么.md) · [总索引](../../docs/100-question-index.md) · [Q072 →](q072-为什么直接使用-sample-covariance-matrix-很危险.md)

> **难度**：★★★  
> **标签**：Portfolio optimization｜Lagrange  
> **题型口径**：高可信重构题型  
> **所属模块**：H. 组合、风险与优化  
> **本题能力主线**：组合、风险与优化
...
推导 Minimum-Variance Portfolio。

---

## 1. 面试官到底在考什么

本题表面属于 **Portfolio optimization, Lagrange**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
求 min_w wᵀΣw s.t. 1ᵀw=1。Lagrange 一阶条件 2Σw-λ1=0，得到 w*=Σ^-1 1/(1ᵀΣ^-1 1)。

**推荐面试表达：** 先写优化问题与约束，再推一阶条件。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

问题 $\min_w w^T\Sigma w$ s.t. $1^Tw=1$。Lagrangian $L=w^T\Sigma w-\lambda(1^Tw-1)$，一阶条件 $2\Sigma w-\lambda1=0$，故 $w^*=\Sigma^{-1}1/(1^T\Sigma^{-1}1)$。
...
若再加目标收益约束 wᵀμ=μ0，则引入第二个 multiplier，得到经典 efficient frontier。

### 第二视角：如何验证主解

解析 KKT/闭式解说明结构，数值 solver + perturbation 说明稳定性；真正可信的最优解应对输入误差和约束变化有可解释反应。

对本题尤其值得继续追问的是：**最小方差组合的闭式解暴露了 Σ^{-1} 的中心地位，也暴露了它对 covariance estimation error 的敏感性。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

最小方差组合的闭式解暴露了 Σ^{-1} 的中心地位，也暴露了它对 covariance estimation error 的敏感性。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：最小方差组合的闭式解暴露了 Σ^{-1} 的中心地位，也暴露了它对 covariance estimation error 的敏感性。
- **最值得保留的原始面试表达**：先写优化问题与约束，再推一阶条件。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

推导只是开始；真实问题还要问 $\Sigma$ 是否可逆、估计是否稳定、是否有 short/leverage/turnover 约束。
...
- **错误 1**：忘记 covariance matrix 必须可逆/需正则化。
...
**回答方向：** 若 $\Sigma$ singular，可用 pseudo-inverse，但更稳健的是 shrinkage、factor model、删除冗余维度或加 ridge，使问题正定。

### 追问 2：允许 cash/risk-free asset 后如何变化？

**回答方向：** 有 risk-free asset 时预算可分配到 cash，风险资产切点组合取决于预期收益与无风险率；单纯 minimum-variance 若允许全投 cash 会退化为 100% cash。
...
> **第二步，主解。** 用最短的推导得到核心结论：求 min_w wᵀΣw s.t. 1ᵀw=1。Lagrange 一阶条件 2Σw-λ1=0，得到 w*=Σ^-1 1/(1ᵀΣ^-1 1)。  
> **第三步，解释。** 关键结构是：最小方差组合的闭式解暴露了 Σ^{-1} 的中心地位，也暴露了它对 covariance estimation error 的敏感性。  
...
- [070. 一个可复现 Quant Research Experiment 应保存什么？](../07-backtesting-research-methodology/q070-一个可复现-Quant-Research-Experiment-应保存什么.md)
