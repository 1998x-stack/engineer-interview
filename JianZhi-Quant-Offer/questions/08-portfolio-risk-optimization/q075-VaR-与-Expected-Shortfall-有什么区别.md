---
id: q075
title: "VaR 与 Expected Shortfall 有什么区别？"
chapter: "H. 组合、风险与优化"
difficulty: "★★☆"
tags: ["Tail risk", "VaR", "ES"]
source_type: "高可信重构题型"
version: "2.0"
---

# 075. VaR 与 Expected Shortfall 有什么区别？

[← Q074](q074-PCA-在风险模型里是什么意思.md) · [总索引](../../docs/100-question-index.md) · [Q076 →](q076-为什么-Volatility-Targeting-不等于风险恒定.md)
...
VaR 与 Expected Shortfall 有什么区别？

---

## 1. 面试官到底在考什么

本题表面属于 **Tail risk, VaR, ES**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
VaR_α 是损失分布的 α 分位数；ES_α 是超越 VaR 尾部条件下的平均损失，更刻画 tail severity。

**推荐面试表达：** 明确“quantile vs conditional tail mean”。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

VaR$_\alpha$ 是损失分布的 $\alpha$ 分位数；ES$_\alpha=E[L\mid L\ge VaR_\alpha]$（连续情形）。ES 考虑尾部严重程度，并满足 coherent risk measure 的 subadditivity 等性质。
...
VaR 不告诉你阈值之后有多糟；ES 在常见条件下具有更好的 coherent risk measure 性质。估计两者都高度依赖尾部分布与样本量。

### 第二视角：如何验证主解

解析 KKT/闭式解说明结构，数值 solver + perturbation 说明稳定性；真正可信的最优解应对输入误差和约束变化有可解释反应。

对本题尤其值得继续追问的是：**VaR 只给分位阈值，ES 关注越过阈值后的平均损失；尾部分布估计误差会同时影响二者。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

VaR 只给分位阈值，ES 关注越过阈值后的平均损失；尾部分布估计误差会同时影响二者。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：VaR 只给分位阈值，ES 关注越过阈值后的平均损失；尾部分布估计误差会同时影响二者。
- **最值得保留的原始面试表达**：明确“quantile vs conditional tail mean”。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

二者都依赖分布/历史窗口，不能把单一数值当真实最大损失。stress testing 与 scenario analysis 是必要补充。
...
- **错误 1**：把 99% VaR 解读为“最大损失”。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：正态分布下 VaR/ES 闭式？

**回答方向：** 若损失 $L\sim N(\mu,\sigma^2)$，VaR$_\alpha=\mu+\sigma z_\alpha$，ES$_\alpha=\mu+\sigma\phi(z_\alpha)/(1-\alpha)$。

### 追问 2：backtesting tail risk 有何困难？

**回答方向：** tail 事件样本极少、依赖强且 regime 变化大，VaR/ES backtest 的统计功效低；极端分位还受模型错设和数据稀缺影响。
...
> **第二步，主解。** 用最短的推导得到核心结论：VaR_α 是损失分布的 α 分位数；ES_α 是超越 VaR 尾部条件下的平均损失，更刻画 tail severity。  
> **第三步，解释。** 关键结构是：VaR 只给分位阈值，ES 关注越过阈值后的平均损失；尾部分布估计误差会同时影响二者。  
