---
id: q077
title: "什么是 Marginal Contribution to Risk？"
chapter: "H. 组合、风险与优化"
difficulty: "★★☆"
tags: ["Risk decomposition", "gradient"]
source_type: "高可信重构题型"
version: "2.0"
---

# 077. 什么是 Marginal Contribution to Risk？

[← Q076](q076-为什么-Volatility-Targeting-不等于风险恒定.md) · [总索引](../../docs/100-question-index.md) · [Q078 →](q078-为什么-Portfolio-Optimization-容易产生极端权重.md)

> **难度**：★★☆  
> **标签**：Risk decomposition｜gradient  
> **题型口径**：高可信重构题型  
> **所属模块**：H. 组合、风险与优化  
> **本题能力主线**：组合、风险与优化
...
什么是 Marginal Contribution to Risk？

---

## 1. 面试官到底在考什么

本题表面属于 **Risk decomposition, gradient**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
σ_p=sqrt(wᵀΣw)，边际风险 ∂σ_p/∂w_i=(Σw)_i/σ_p；component risk contribution 为 RC_i=w_i(Σw)_i/σ_p。

**推荐面试表达：** 先算 gradient，再乘 position，逻辑最清晰。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

组合波动 $\sigma_p=\sqrt{w^T\Sigma w}$，marginal risk $\partial\sigma_p/\partial w_i=(\Sigma w)_i/\sigma_p$；component risk $RC_i=w_i(\Sigma w)_i/\sigma_p$，并满足 $\sum_iRC_i=\sigma_p$。
...
Euler homogeneous property 下，Σ_i RC_i=σ_p。它用于理解某个 position 对组合总体风险的贡献，而不是单看 standalone volatility。

### 第二视角：如何验证主解

解析 KKT/闭式解说明结构，数值 solver + perturbation 说明稳定性；真正可信的最优解应对输入误差和约束变化有可解释反应。

对本题尤其值得继续追问的是：**Risk contribution 是对 portfolio volatility 的梯度分解；它把“总风险”拆成各头寸边际贡献，便于约束和归因。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Risk contribution 是对 portfolio volatility 的梯度分解；它把“总风险”拆成各头寸边际贡献，便于约束和归因。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Risk contribution 是对 portfolio volatility 的梯度分解；它把“总风险”拆成各头寸边际贡献，便于约束和归因。
- **最值得保留的原始面试表达**：先算 gradient，再乘 position，逻辑最清晰。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

Risk parity、risk budgeting 都基于这种 Euler decomposition，但输入 covariance 的误差仍会传入 risk contribution。
...
- **错误 1**：把风险贡献等同于权重乘单资产波动率。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：variance contribution 版本怎么写？

**回答方向：** 对 variance $V=w^T\Sigma w$，marginal contribution 是 $2(\Sigma w)_i$，component contribution 可写 $w_i(\Sigma w)_i$，总和等于 portfolio variance。

### 追问 2：risk parity 的条件？

**回答方向：** 典型 risk parity 要求各资产 component risk contribution 相等，并需 covariance 正定、约束可行；有 short/复杂约束时解与解释更复杂。
...
> **第二步，主解。** 用最短的推导得到核心结论：σ_p=sqrt(wᵀΣw)，边际风险 ∂σ_p/∂w_i=(Σw)_i/σ_p；component risk contribution 为 RC_i=w_i(Σw)_i/σ_p。  
> **第三步，解释。** 关键结构是：Risk contribution 是对 portfolio volatility 的梯度分解；它把“总风险”拆成各头寸边际贡献，便于约束和归因。  
...
- [076. 为什么 Volatility Targeting 不等于风险恒定？](q076-为什么-Volatility-Targeting-不等于风险恒定.md)
