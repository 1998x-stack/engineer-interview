---
id: q081
title: "Put-Call Parity 是什么？如何从无套利推导？"
chapter: "I. 衍生品与定价"
difficulty: "★★☆"
tags: ["Options", "no-arbitrage", "parity"]
source_type: "高可信重构题型"
version: "2.0"
---

# 081. Put-Call Parity 是什么？如何从无套利推导？

[← Q080](../08-portfolio-risk-optimization/q080-Factor-Neutralization-怎么理解.md) · [总索引](../../docs/100-question-index.md) · [Q082 →](q082-Black-Scholes-的核心假设是什么-哪些明显不现实.md)

> **难度**：★★☆  
> **标签**：Options｜no-arbitrage｜parity  
> **题型口径**：高可信重构题型  
> **所属模块**：I. 衍生品与定价  
> **本题能力主线**：衍生品与定价
...
Put-Call Parity 是什么？如何从无套利推导？

---

## 1. 面试官到底在考什么

本题表面属于 **Options, no-arbitrage, parity**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
无股息欧式期权：C-P=S_0-Ke^{-rT}。比较组合 A：long call + PV(K) 与组合 B：long put + stock；到期两者 payoff 都是 max(S_T,K)。

**推荐面试表达：** 无套利题先构造“到期 payoff 完全相同的两个组合”。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

构造组合 A：long call + PV(K) cash；组合 B：long put + stock。到期两者 payoff 都是 $\max(S_T,K)$，无套利故 $C+Ke^{-rT}=P+S_0$（无分红）。
...
若当前价格不相等，则可构造初始净现金流并在到期得到同一 payoff，违背无套利。

### 第二视角：如何验证主解

把 replication/PDE 与 risk-neutral expectation/Monte Carlo 对照；两个框架在同一假设下应给一致价格，不一致通常意味着 convention 或实现错误。

对本题尤其值得继续追问的是：**Put-call parity 是复制组合等价关系；任何偏离都应先检查现金流、carry、dividend、exercise style 和交易约束。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Put-call parity 是复制组合等价关系；任何偏离都应先检查现金流、carry、dividend、exercise style 和交易约束。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Put-call parity 是复制组合等价关系；任何偏离都应先检查现金流、carry、dividend、exercise style 和交易约束。
- **最值得保留的原始面试表达**：无套利题先构造“到期 payoff 完全相同的两个组合”。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

parity 是发现报价一致性/模型 sanity 的基础，但实际需考虑 dividend、borrow、rates、American exercise 与交易成本。
...
**回答方向：** 连续股息率 $q$ 下，$C-P=S_0e^{-qT}-Ke^{-rT}$。

### 追问 2：American option parity 为什么变成不等式？

**回答方向：** American 可提前行权，持有期权的最优停止价值破坏简单到期 payoff 等价，因此 parity 通常变为上下界/不等式。
...
> **第二步，主解。** 用最短的推导得到核心结论：无股息欧式期权：C-P=S_0-Ke^{-rT}。比较组合 A：long call + PV(K) 与组合 B：long put + stock；到期两者 payoff 都是 max(S_T,K)。  
> **第三步，解释。** 关键结构是：Put-call parity 是复制组合等价关系；任何偏离都应先检查现金流、carry、dividend、exercise style 和交易约束。  
...
- [080. Factor Neutralization 怎么理解？](../08-portfolio-risk-optimization/q080-Factor-Neutralization-怎么理解.md)
