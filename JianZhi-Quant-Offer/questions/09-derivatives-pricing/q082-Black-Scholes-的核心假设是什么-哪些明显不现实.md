---
id: q082
title: "Black-Scholes 的核心假设是什么？哪些明显不现实？"
chapter: "I. 衍生品与定价"
difficulty: "★★☆"
tags: ["Black-Scholes", "assumptions", "model risk"]
source_type: "高可信重构题型"
version: "2.0"
---

# 082. Black-Scholes 的核心假设是什么？哪些明显不现实？

[← Q081](q081-Put-Call-Parity-是什么-如何从无套利推导.md) · [总索引](../../docs/100-question-index.md) · [Q083 →](q083-为什么-Black-Scholes-PDE-中真实-drift-μ-消失.md)

> **难度**：★★☆  
> **标签**：Black-Scholes｜assumptions｜model risk  
> **题型口径**：高可信重构题型  
> **所属模块**：I. 衍生品与定价  
> **本题能力主线**：衍生品与定价
...
Black-Scholes 的核心假设是什么？哪些明显不现实？

---

## 1. 面试官到底在考什么

本题表面属于 **Black-Scholes, assumptions, model risk**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
经典模型假设 GBM、常数 σ/r、连续交易、无摩擦、可连续对冲、无套利、欧式行权等。现实存在 smile/skew、jumps、stochastic vol、离散对冲、交易成本和流动性约束。

**推荐面试表达：** 列假设后至少举 2-3 个现实偏离与后果。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

BS 假设底层连续 GBM、常数 $r,\sigma$、无摩擦、可连续交易、无套利、通常 European payoff。核心不是逐条背，而是知道每条假设对应哪种现实 model risk。
...
面试更关注“假设-结果”映射：哪个假设被破坏，会影响哪部分定价/对冲结论。

### 第二视角：如何验证主解

把 replication/PDE 与 risk-neutral expectation/Monte Carlo 对照；两个框架在同一假设下应给一致价格，不一致通常意味着 convention 或实现错误。

对本题尤其值得继续追问的是：**Black-Scholes 更重要的是“哪些风险被假设掉了”；vol smile、jump、交易成本和离散对冲都可视作 assumption failure 的后果。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Black-Scholes 更重要的是“哪些风险被假设掉了”；vol smile、jump、交易成本和离散对冲都可视作 assumption failure 的后果。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Black-Scholes 更重要的是“哪些风险被假设掉了”；vol smile、jump、交易成本和离散对冲都可视作 assumption failure 的后果。
- **最值得保留的原始面试表达**：列假设后至少举 2-3 个现实偏离与后果。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

vol smile 直接否定 constant vol；jump/gap 否定连续路径；交易成本/离散 hedge 否定完美复制。BS 更像基准坐标系而不是现实真相。
...
- **错误 1**：把模型假设背成清单但不理解用途。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：local vol/stochastic vol 分别放松什么？

**回答方向：** local vol 让 $\sigma=\sigma(S,t)$，可拟合当前 smile；stochastic vol 引入随机波动状态，重点改善 smile dynamics 与 volatility clustering。

### 追问 2：jump diffusion 解决什么尾部特征？

**回答方向：** jump diffusion 加入不连续跳跃，能产生更厚尾、短期限 smile/skew 与 gap risk，补足纯连续 GBM 的尾部不足。
...
> **第二步，主解。** 用最短的推导得到核心结论：经典模型假设 GBM、常数 σ/r、连续交易、无摩擦、可连续对冲、无套利、欧式行权等。现实存在 smile/skew、jumps、stochastic vol、离散对冲、交易成本和流动性约束。  
> **第三步，解释。** 关键结构是：Black-Scholes 更重要的是“哪些风险被假设掉了”；vol smile、jump、交易成本和离散对冲都可视作 assumption failure 的后果。  
...
- [081. Put-Call Parity 是什么？如何从无套利推导？](q081-Put-Call-Parity-是什么-如何从无套利推导.md)
