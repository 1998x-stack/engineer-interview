...
chapter: "I. 衍生品与定价"
difficulty: "★★☆"
tags: ["Greeks", "convexity", "hedging error"]
source_type: "高可信重构题型"
version: "2.0"
---
...
[← Q084](q084-Delta-是什么-它是不是“期权上涨概率”.md) · [总索引](../../docs/100-question-index.md) · [Q086 →](q086-Vega-是什么-为什么-vanilla-option-通常-Vega-0.md)

> **难度**：★★☆  
> **标签**：Greeks｜convexity｜hedging error  
> **题型口径**：高可信重构题型  
> **所属模块**：I. 衍生品与定价  
> **本题能力主线**：衍生品与定价
...
本题表面属于 **Greeks, convexity, hedging error**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
Gamma=∂²V/∂S²，描述 delta 对 S 的变化，也即 payoff/value 的局部 convexity。二阶近似 ΔV≈Delta*ΔS+0.5*Gamma*(ΔS)²。

**推荐面试表达：** 把数学敏感度与 hedge stability 联系起来。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

Gamma $\Gamma=\partial^2V/\partial S^2$ 衡量 delta 对 spot 的敏感度。二阶 Taylor：$\Delta V\approx\Delta\,dS+\frac12\Gamma(dS)^2$。Long vanilla option 通常 gamma>0。
...
高 gamma 意味着 delta 变化快，离散对冲误差和再平衡频率更敏感；near-expiry ATM option 常见 gamma 集中。

### 第二视角：如何验证主解

把 replication/PDE 与 risk-neutral expectation/Monte Carlo 对照；两个框架在同一假设下应给一致价格，不一致通常意味着 convention 或实现错误。

对本题尤其值得继续追问的是：**Gamma 控制 delta 对标的变化的速度，也是离散对冲误差与 convexity exposure 的核心二阶项。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Gamma 控制 delta 对标的变化的速度，也是离散对冲误差与 convexity exposure 的核心二阶项。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Gamma 控制 delta 对标的变化的速度，也是离散对冲误差与 convexity exposure 的核心二阶项。
- **最值得保留的原始面试表达**：把数学敏感度与 hedge stability 联系起来。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

高 gamma 意味着 hedge 需要更频繁调整，也意味着 realized volatility 与 implied volatility 差异对 PnL 更敏感。
...
- **错误 1**：只说“gamma 是二阶导数”而不连接动态 hedging。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：gamma 与 theta 的 BS 关系？

**回答方向：** BS PDE 可重排出 theta-gamma-carry 关系，例如无股息时 $\Theta+\frac12\sigma^2S^2\Gamma+rS\Delta-rV=0$。

### 追问 2：long gamma 为什么通常 long convexity？

**回答方向：** long gamma 表示 payoff/价值对 spot 具有正凸性；大幅上下波动的二阶项都对 long gamma 有利，但通常要付出 theta/vol premium。
...
> **第二步，主解。** 用最短的推导得到核心结论：Gamma=∂²V/∂S²，描述 delta 对 S 的变化，也即 payoff/value 的局部 convexity。二阶近似 ΔV≈Delta*ΔS+0.5*Gamma*(ΔS)²。  
> **第三步，解释。** 关键结构是：Gamma 控制 delta 对标的变化的速度，也是离散对冲误差与 convexity exposure 的核心二阶项。  
