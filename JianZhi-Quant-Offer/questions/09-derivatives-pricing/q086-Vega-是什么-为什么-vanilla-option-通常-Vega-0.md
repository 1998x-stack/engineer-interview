---
id: q086
title: "Vega 是什么？为什么 vanilla option 通常 Vega>0？"
chapter: "I. 衍生品与定价"
difficulty: "★★☆"
tags: ["Greeks", "volatility", "convexity"]
source_type: "高可信重构题型"
version: "2.0"
---

# 086. Vega 是什么？为什么 vanilla option 通常 Vega>0？

[← Q085](q085-Gamma-为什么重要.md) · [总索引](../../docs/100-question-index.md) · [Q087 →](q087-Implied-Volatility-如何从期权价格反求.md)

> **难度**：★★☆  
> **标签**：Greeks｜volatility｜convexity  
> **题型口径**：高可信重构题型  
> **所属模块**：I. 衍生品与定价  
> **本题能力主线**：衍生品与定价
...
Vega 是什么？为什么 vanilla option 通常 Vega>0？

---

## 1. 面试官到底在考什么

本题表面属于 **Greeks, volatility, convexity**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
Vega=∂V/∂σ。对普通欧式 vanilla，增加不确定性会扩大终值分布，而凸性 payoff 受益，所以 vega 通常为正。

**推荐面试表达：** 用 convexity + risk-neutral expectation 解释正 vega。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

Vega $\nu=\partial V/\partial\sigma$。在 BS 中 call/put vega 相同且为 $S_0\phi(d_1)\sqrt T$（无 dividend 形式需相应修正），通常为正。
...
严格说是模型参数敏感度，不是 realized vol 本身。不同期限/执行价的 vega 形成 volatility surface risk。

### 第二视角：如何验证主解

把 replication/PDE 与 risk-neutral expectation/Monte Carlo 对照；两个框架在同一假设下应给一致价格，不一致通常意味着 convention 或实现错误。

对本题尤其值得继续追问的是：**Vanilla option 对更高波动通常受益于 payoff convexity；但组合层面的 vega 可以正负混合，且 surface move 不是单参数 σ 变化。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Vanilla option 对更高波动通常受益于 payoff convexity；但组合层面的 vega 可以正负混合，且 surface move 不是单参数 σ 变化。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Vanilla option 对更高波动通常受益于 payoff convexity；但组合层面的 vega 可以正负混合，且 surface move 不是单参数 σ 变化。
- **最值得保留的原始面试表达**：用 convexity + risk-neutral expectation 解释正 vega。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

“vega>0”来自期权 payoff 的凸性与波动扩散，但 exotic/路径依赖产品的局部 vega 结构可能更复杂。
...
- **错误 1**：把 implied vol 上升与 realized vol 上升视为同一件事。
...
**回答方向：** vanilla vega 通常在接近 ATM、到期尚有时间时最大；极深 ITM/OTM 或临近到期时 vega 较小。

### 追问 2：volga/vomma 是什么？

**回答方向：** volga/vomma 是对 volatility 的二阶敏感度 $\partial^2V/\partial\sigma^2$，描述 vega 随 vol 变化。
...
> **第二步，主解。** 用最短的推导得到核心结论：Vega=∂V/∂σ。对普通欧式 vanilla，增加不确定性会扩大终值分布，而凸性 payoff 受益，所以 vega 通常为正。  
> **第三步，解释。** 关键结构是：Vanilla option 对更高波动通常受益于 payoff convexity；但组合层面的 vega 可以正负混合，且 surface move 不是单参数 σ 变化。  
