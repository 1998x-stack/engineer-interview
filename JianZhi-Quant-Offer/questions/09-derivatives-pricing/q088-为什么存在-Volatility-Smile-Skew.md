---
id: q088
title: "为什么存在 Volatility Smile/Skew？"
chapter: "I. 衍生品与定价"
difficulty: "★★☆"
tags: ["Vol surface", "fat tails", "model misspecification"]
source_type: "高可信重构题型"
version: "2.0"
---

# 088. 为什么存在 Volatility Smile/Skew？

[← Q087](q087-Implied-Volatility-如何从期权价格反求.md) · [总索引](../../docs/100-question-index.md) · [Q089 →](q089-American-与-European-Option-最大区别是什么.md)

> **难度**：★★☆  
> **标签**：Vol surface｜fat tails｜model misspecification  
> **题型口径**：高可信重构题型  
> **所属模块**：I. 衍生品与定价  
> **本题能力主线**：衍生品与定价
...
为什么存在 Volatility Smile/Skew？

---

## 1. 面试官到底在考什么

本题表面属于 **Vol surface, fat tails, model misspecification**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
真实收益不是 constant-vol lognormal；jumps、stochastic vol、leverage effect、fat tails 及供需共同使不同 strike/maturity 的 BS implied vol 不同。

**推荐面试表达：** 把 IV surface 视为“model-implied coordinate system”。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

若市场真是 constant-vol lognormal，所有 strike/maturity 反推 IV 应相同。实际 smile/skew 表明 risk-neutral return distribution 有非高斯尾部、偏度/峰度，或 volatility dynamics 非常数。
...
所以 implied volatility 是把市场价格映射回 BS 参数空间的“报价坐标”，不是某个真正恒定 σ 的直接观测。

### 第二视角：如何验证主解

把 replication/PDE 与 risk-neutral expectation/Monte Carlo 对照；两个框架在同一假设下应给一致价格，不一致通常意味着 convention 或实现错误。

对本题尤其值得继续追问的是：**Smile/skew 是市场对非 lognormal 尾部、jump、stochastic vol 和供需风险的综合反映；它本质上是模型错配的价格化。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Smile/skew 是市场对非 lognormal 尾部、jump、stochastic vol 和供需风险的综合反映；它本质上是模型错配的价格化。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Smile/skew 是市场对非 lognormal 尾部、jump、stochastic vol 和供需风险的综合反映；它本质上是模型错配的价格化。
- **最值得保留的原始面试表达**：把 IV surface 视为“model-implied coordinate system”。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

equity 常见 downside skew 与 crash/leverage risk 相关。surface 既是价格摘要，也是模型输入；需要 no-arbitrage interpolation。
...
- **错误 1**：看到 smile 就简单说“市场错定价”。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：smile 与 skew 的区别？

**回答方向：** smile 指 IV 两端都高于中间的弯曲形状；skew/smirk 指不同 strike 的 IV 明显单向倾斜，如 equity downside IV 更高。

### 追问 2：term structure 说明什么？

**回答方向：** term structure 是不同 maturity 的 IV 水平/形状，反映市场对不同 horizon 波动、事件和均值回归的定价。
...
> **第二步，主解。** 用最短的推导得到核心结论：真实收益不是 constant-vol lognormal；jumps、stochastic vol、leverage effect、fat tails 及供需共同使不同 strike/maturity 的 BS implied vol 不同。  
> **第三步，解释。** 关键结构是：Smile/skew 是市场对非 lognormal 尾部、jump、stochastic vol 和供需风险的综合反映；它本质上是模型错配的价格化。  
...
- [087. Implied Volatility 如何从期权价格反求？](q087-Implied-Volatility-如何从期权价格反求.md)
