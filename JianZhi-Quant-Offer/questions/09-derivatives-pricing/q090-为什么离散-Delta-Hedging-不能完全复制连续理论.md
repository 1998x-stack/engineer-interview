---
id: q090
title: "为什么离散 Delta Hedging 不能完全复制连续理论？"
chapter: "I. 衍生品与定价"
difficulty: "★★★"
tags: ["Discrete hedging", "model risk", "P&L"]
source_type: "高可信重构题型"
version: "2.0"
---

# 090. 为什么离散 Delta Hedging 不能完全复制连续理论？

[← Q089](q089-American-与-European-Option-最大区别是什么.md) · [总索引](../../docs/100-question-index.md) · [Q091 →](../10-coding-quant-systems/q091-长度-N-的序列-如何-O-N-计算-rolling-mean.md)

> **难度**：★★★  
> **标签**：Discrete hedging｜model risk｜P&L  
> **题型口径**：高可信重构题型  
> **所属模块**：I. 衍生品与定价  
> **本题能力主线**：衍生品与定价
...
为什么离散 Delta Hedging 不能完全复制连续理论？

---

## 1. 面试官到底在考什么

本题表面属于 **Discrete hedging, model risk, P&L**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
连续复制假设可无限频繁、无成本交易，真实世界存在离散时间、交易成本、jump、vol misspecification 和 latency，因此会出现 hedging error。

**推荐面试表达：** 把“discretization error vs transaction cost”作为核心 trade-off。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

连续复制依赖 infinitesimal rebalancing 与连续路径。实际离散对冲会留下 gamma exposure；近似 hedging error 与 $\Gamma[(\Delta S)^2-\sigma^2S^2\Delta t]$ 等项相关，并受 jump/cost 进一步影响。
...
在小步长近似下，gamma、realized vs implied variance、rebalancing interval 等共同决定误差结构。交易更频繁可降离散误差，却提高成本。

### 第二视角：如何验证主解

把 replication/PDE 与 risk-neutral expectation/Monte Carlo 对照；两个框架在同一假设下应给一致价格，不一致通常意味着 convention 或实现错误。

对本题尤其值得继续追问的是：**连续复制在现实中被离散时间、交易成本、跳跃和参数变化打破；hedging P&L 是模型假设与执行误差共同作用的结果。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

连续复制在现实中被离散时间、交易成本、跳跃和参数变化打破；hedging P&L 是模型假设与执行误差共同作用的结果。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：连续复制在现实中被离散时间、交易成本、跳跃和参数变化打破；hedging P&L 是模型假设与执行误差共同作用的结果。
- **最值得保留的原始面试表达**：把“discretization error vs transaction cost”作为核心 trade-off。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

真实 hedge PnL 是模型验证工具：区分 implied vs realized vol、surface move、jumps、transaction cost 与 execution slippage。
...
### 追问 1：hedging frequency 如何权衡？

**回答方向：** 更高频 hedge 降低离散 gamma risk，但增加 spread/fee/impact 与 operational noise；最优频率是风险与成本的权衡。

### 追问 2：jump risk 为什么无法用连续 delta 完全对冲？

**回答方向：** jump 发生时 spot 瞬间跨过有限区间，连续 delta 调整来不及，中间不可交易风险无法由仅底层股票完全复制。
...
> **第二步，主解。** 用最短的推导得到核心结论：连续复制假设可无限频繁、无成本交易，真实世界存在离散时间、交易成本、jump、vol misspecification 和 latency，因此会出现 hedging error。  
> **第三步，解释。** 关键结构是：连续复制在现实中被离散时间、交易成本、跳跃和参数变化打破；hedging P&L 是模型假设与执行误差共同作用的结果。  
...
- [091. 长度 N 的序列，如何 O(N) 计算 rolling mean？](../10-coding-quant-systems/q091-长度-N-的序列-如何-O-N-计算-rolling-mean.md)
