---
id: q091
title: "长度 N 的序列，如何 O(N) 计算 rolling mean？"
chapter: "J. Coding、算法与量化系统"
difficulty: "★☆☆"
tags: ["Sliding window", "算法", "streaming"]
source_type: "高可信重构题型"
version: "2.0"
---

# 091. 长度 N 的序列，如何 O(N) 计算 rolling mean？

[← Q090](../09-derivatives-pricing/q090-为什么离散-Delta-Hedging-不能完全复制连续理论.md) · [总索引](../../docs/100-question-index.md) · [Q092 →](q092-Streaming-数据中实时维护-median-怎么做.md)

> **难度**：★☆☆  
> **标签**：Sliding window｜算法｜streaming  
> **题型口径**：高可信重构题型  
...
长度 N 的序列，如何 O(N) 计算 rolling mean？

---

## 1. 面试官到底在考什么

本题表面属于 **Sliding window, 算法, streaming**，但真正考察的是：你能否把口头问题迅速变成一个**定义清楚、假设透明、可推导、可验证**的模型。
...
维护窗口和 S_t=S_{t-1}+x_t-x_{t-W}，每步 O(1)，总 O(N)，空间可做到 O(W) 或若只需均值则视输入结构更低。

**推荐面试表达：** 先说“reuse overlapping work”，再给递推式。

如果只有 30 秒，优先说清楚：**定义 → 关键式子/算法 → 结论 → 一个最重要的坑**。不要先铺背景。

## 3. Formalization：变量、假设与数学对象

维护窗口和 $S_t$：初始化前 $W$ 个元素，之后 $S_t=S_{t-1}+x_t-x_{t-W}$，每步 $O(1)$、总 $O(N)$、额外空间 $O(1)$（若数据流不需保存全序列）。
...
数值实现应考虑浮点累计误差、NaN、窗口启动阶段和并行/分块边界。

### 第二视角：如何验证主解

先写 reference implementation 保证正确，再做 optimized implementation；用同一 golden replay 做逐事件 diff，性能优化不得改变状态语义。

对本题尤其值得继续追问的是：**Sliding window 的核心是不重复计算；同一思想可扩展到 rolling variance、deque-based rolling max/min 等流式统计。**

一个成熟回答应能说明：如果解析解、数值实验和经验数据三者不一致，优先检查哪一层假设，而不是简单选择“看起来最漂亮”的结果。

## 5. Why：为什么这个方法有效

Sliding window 的核心是不重复计算；同一思想可扩展到 rolling variance、deque-based rolling max/min 等流式统计。

这一层是区分“会做题”和“理解题”的关键。面试官继续追问时，最常见的方向不是让你重复公式，而是问：**为什么这个结构成立、什么情况下失效、能否推广**。

### 高级面试层：从答案到研究判断

- **本题的高级抽象**：Sliding window 的核心是不重复计算；同一思想可扩展到 rolling variance、deque-based rolling max/min 等流式统计。
- **最值得保留的原始面试表达**：先说“reuse overlapping work”，再给递推式。
- **研究者视角**：不要只问“公式是否正确”，还要问“这个结论对哪些 perturbation 稳定、什么观测会证伪它、实现中哪一层最容易引入偏差”。

## 6. 量化金融 / 工程语境中的对应问题

工程上要处理 NaN、可变窗口、数值累计误差和时间窗口而非固定条数。流式计算的关键是明确 state invariant。
...
- **错误 1**：每个窗口重新 sum，做成 O(NW)。

一个专业回答不应该只说“答案是 X”，而应至少能补一句：**“这个结论依赖于……；如果……不成立，我会改用……”**。

## 8. 追问树：不只列问题，还要会接

### 追问 1：rolling variance 如何 O(N)？

**回答方向：** rolling variance 可维护窗口内 sum 与 sumsq 得 $E[x^2]-E[x]^2$，每步 $O(1)$；但数值取消可能严重，生产上需更稳定算法/重算策略。

### 追问 2：Welford algorithm 适合什么场景？

**回答方向：** Welford 适合单遍 streaming 的均值/方差，数值稳定且无需保存全数据；标准形式不直接支持移除旧值，滑窗需扩展。
...
> **第二步，主解。** 用最短的推导得到核心结论：维护窗口和 S_t=S_{t-1}+x_t-x_{t-W}，每步 O(1)，总 O(N)，空间可做到 O(W) 或若只需均值则视输入结构更低。  
> **第三步，解释。** 关键结构是：Sliding window 的核心是不重复计算；同一思想可扩展到 rolling variance、deque-based rolling max/min 等流式统计。  
...
- [090. 为什么离散 Delta Hedging 不能完全复制连续理论？](../09-derivatives-pricing/q090-为什么离散-Delta-Hedging-不能完全复制连续理论.md)
