---
id: q001
title: "LLM、Workflow 和 Agent 有什么区别？什么时候不应该使用 Agent？"
...
# Q001 · LLM、Workflow 和 Agent 有什么区别？什么时候不应该使用 Agent？
...
| 失败类别 | Control / State |
| 风险等级 | 中 |
| 核心 Invariant | 控制流复杂度与业务不确定性匹配：确定步骤不交给概率模型。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

Agent 的价值来自运行时适应性；代价也来自运行时不确定性。面试官在看你是否有‘先选控制范式，再选框架’的工程意识。
...
### 1. 用‘控制流由谁决定’区分三者，而不是按产品名称区分。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 2. 评估自治带来的收益是否覆盖不确定性、成本、延迟和审计复杂度。

Agent Eval 至少拆成 component、trajectory、end-to-end 三层。这样最终失败时能判断是检索、规划、工具还是 verifier 出错，并把回归定位到具体 transition。

### 3. 高风险动作可采用 Workflow 外壳 + Agent 局部决策的混合结构。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`先画任务 DAG，标出不可预先确定的节点；只有这些节点交给模型决策，其余用确定性代码约束。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**控制流复杂度与业务不确定性匹配：确定步骤不交给概率模型。**。然后按下面四层回答：
...
审批节点、付款提交、权限校验采用 Workflow；“根据自然语言材料决定需要查哪些系统”可由 Agent 局部完成。

## Failure Modes：方案最容易坏在哪里

- **把“调用了 LLM”误判为 Agent，导致无谓的自治和成本**
- **审批/交易等高风险流程交给模型自由编排，审计和合规不可控**
- **用 Agent 解决一个本可由规则/DAG 解决的问题，延迟和失败面扩大**

遇到异常时，不要立即跳到“重试”。建议统一使用：

`Detect → Classify → Contain → Recover → Preserve → Verify`

本题最重要的 Preserve 对象是：**控制流复杂度与业务不确定性匹配：确定步骤不交给概率模型。**。
...
### 1. 固定审批流程为什么通常不该做成 Agent？

**回答方向**：审批步骤、权限和状态迁移可预先枚举且审计要求高，因此确定性 Workflow 更合适；Agent 可以用于读取非结构化材料、生成建议，但不应拥有最终审批控制流。

### 2. 怎样判断一个流程值得 Agent 化？

**回答方向**：先以“用‘控制流由谁决定’区分三者，而不是按产品名称区分。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

## 易错回答

> 把‘用了 LLM’等同于‘Agent’，或默认 Agent 一定比 Workflow 高级。
...
更深一层看，本题的本质是保护这个系统 invariant：**控制流复杂度与业务不确定性匹配：确定步骤不交给概率模型。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

先画任务 DAG，标出不可预先确定的节点；只有这些节点交给模型决策，其余用确定性代码约束。
...
- [ ] 核心 invariant 已写成可验证条件：控制流复杂度与业务不确定性匹配：确定步骤不交给概率模型。
