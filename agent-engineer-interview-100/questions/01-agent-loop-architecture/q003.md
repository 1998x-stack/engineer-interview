---
id: q003
title: "为什么说 Agent Loop 本质上可以看成状态机？State 里应该保存什么？"
...
# Q003 · 为什么说 Agent Loop 本质上可以看成状态机？State 里应该保存什么？
...
| 失败类别 | Control / State |
| 风险等级 | 中 |
| 核心 Invariant | 状态可序列化、可版本化、可比较；消息只是状态的一种视图。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

长任务、暂停恢复、故障重试都依赖显式状态；如果关键事实只存在模型上下文中，就无法可靠恢复。
...
### 1. 把不可丢失的业务状态和可压缩的语言上下文分开。

消息适合承载对话叙事，不适合作为唯一数据库。业务游标、预算、operation id、权限与任务状态应结构化保存，否则无法做可靠恢复、并发控制和版本迁移。

### 2. 状态迁移应尽量显式，便于恢复、回放和测试。

消息适合承载对话叙事，不适合作为唯一数据库。业务游标、预算、operation id、权限与任务状态应结构化保存，否则无法做可靠恢复、并发控制和版本迁移。

### 3. 对于并发 worker，还需版本号或乐观锁字段。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`将状态分成 control state、business state、context state 三层，并为每层定义持久化与生命周期策略。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**状态可序列化、可版本化、可比较；消息只是状态的一种视图。**。然后按下面四层回答：
...
- **把 transient context 与 durable state 混在一起**
- **状态字段没有 owner，多个节点覆盖彼此结果**
- **checkpoint 保存了 prompt 文本却没保存业务游标/operation id**

遇到异常时，不要立即跳到“重试”。建议统一使用：

`Detect → Classify → Contain → Recover → Preserve → Verify`

本题最重要的 Preserve 对象是：**状态可序列化、可版本化、可比较；消息只是状态的一种视图。**。
...
### 1. 哪些字段不能只存在 prompt 里？

**回答方向**：先以“把不可丢失的业务状态和可压缩的语言上下文分开。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**状态可序列化、可版本化、可比较；消息只是状态的一种视图。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

将状态分成 control state、business state、context state 三层，并为每层定义持久化与生命周期策略。
...
- [ ] 核心 invariant 已写成可验证条件：状态可序列化、可版本化、可比较；消息只是状态的一种视图。
...
[← Q002](q002.md) · [本章目录](README.md) · [Q004 →](q004.md)
