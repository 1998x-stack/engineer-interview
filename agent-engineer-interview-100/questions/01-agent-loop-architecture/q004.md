---
id: q004
title: "生产 Agent 中，哪些逻辑交给 LLM，哪些逻辑必须写死在代码里？"
...
# Q004 · 生产 Agent 中，哪些逻辑交给 LLM，哪些逻辑必须写死在代码里？
...
| 失败类别 | Control / State |
| 风险等级 | 高 |
| 核心 Invariant | 模型只能在被授权的决策空间内选择；系统 invariant 必须由代码或策略引擎强制。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

LLM 是概率系统，不能保证 invariant 始终成立。生产系统要把‘可能遵守’变成‘无法绕过’。
...
### 1. 模型适合处理 fuzzy policy，不适合作为安全边界。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 2. 所有 side-effect tool 在执行前做确定性校验。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 3. 模型输出用结构化 schema 缩小自由度。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`建立 policy engine / tool gateway；LLM 输出 action intent，执行层再做 ACL、额度和参数校验。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**模型只能在被授权的决策空间内选择；系统 invariant 必须由代码或策略引擎强制。**。然后按下面四层回答：
...
- **让 LLM 自己判断“自己有没有权限”**
- **把金额上限、租户隔离、重试次数写进 prompt 当安全规则**
- **用自然语言输出作为数据库事务提交条件**
...
本题最重要的 Preserve 对象是：**模型只能在被授权的决策空间内选择；系统 invariant 必须由代码或策略引擎强制。**。
...
**回答方向**：先以“模型适合处理 fuzzy policy，不适合作为安全边界。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 业务规则很复杂时如何避免全写 prompt？
...
把所有“绝不能由模型决定”的事项列出来：租户、金额上限、ACL、schema、重试上限、审批状态、最终资源授权。再说明模型只负责在允许集合中选择。
...
> 把关键安全规则写进 system prompt 后就认为足够安全。
...
更深一层看，本题的本质是保护这个系统 invariant：**模型只能在被授权的决策空间内选择；系统 invariant 必须由代码或策略引擎强制。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

建立 policy engine / tool gateway；LLM 输出 action intent，执行层再做 ACL、额度和参数校验。
...
- [ ] 核心 invariant 已写成可验证条件：模型只能在被授权的决策空间内选择；系统 invariant 必须由代码或策略引擎强制。
...
[← Q003](q003.md) · [本章目录](README.md) · [Q005 →](q005.md)
