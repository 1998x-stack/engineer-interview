---
id: q005
title: "Graph Engineer 和 Loop Engineer 的核心区别是什么？"
...
# Q005 · Graph Engineer 和 Loop Engineer 的核心区别是什么？
...
这实际上是‘自由度放在哪一层’的问题。面试官关注你如何在自治与控制之间做边界设计。
...
### 1. Graph 提高可观测性和可恢复性，但降低自由度。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 2. 纯 Loop 适合探索性任务；强业务流程更适合显式图。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 3. 最佳实践往往是 deterministic shell + agentic core。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`先从 trace 中识别稳定重复的阶段，把它们固化成 graph 节点；把真正不可枚举的步骤保留为 loop。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开
...
**回答方向**：先以“Graph 提高可观测性和可恢复性，但降低自由度。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 如何从自由 Loop 迁移到显式 Graph？

**回答方向**：先以“Graph 提高可观测性和可恢复性，但降低自由度。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

## 易错回答

> 把 Graph 和 Loop 当成互斥技术选型。
...
先从 trace 中识别稳定重复的阶段，把它们固化成 graph 节点；把真正不可枚举的步骤保留为 loop。
...
[← Q004](q004.md) · [本章目录](README.md) · [Q006 →](q006.md)
