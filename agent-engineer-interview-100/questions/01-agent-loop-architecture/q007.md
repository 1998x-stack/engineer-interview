---
id: q007
title: "Agent 的 Structured Output 校验失败怎么办？"
...
# Q007 · Agent 的 Structured Output 校验失败怎么办？
...
| 失败类别 | Control / State |
| 风险等级 | 中高 |
| 核心 Invariant | schema validation 失败是一个显式状态，必须有有界修复策略和可观测错误。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

结构化输出是 Agent 与程序世界的契约；契约失败不应直接污染后续状态。
...
### 1. 区分 syntax error 与 semantic error。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 2. 优先使用受约束输出或原生 schema。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 3. 重试提示中反馈具体 validation error，而不是只说‘格式错误’。

Retry 必须同时满足：错误瞬时可恢复、操作可安全重放、仍在父 deadline 内、没有超过 retry budget。否则重试只是把局部故障放大成队列/下游雪崩。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`用 Pydantic/JSON Schema 做边界校验，错误分类后决定 retry、repair、fallback 或 fail-fast。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**schema validation 失败是一个显式状态，必须有有界修复策略和可观测错误。**。然后按下面四层回答：
...
- **无限让模型“修一下 JSON”**
- **parser 静默丢字段或类型强转**
- **把 schema error 当模型能力问题而忽略契约设计**

遇到异常时，不要立即跳到“重试”。建议统一使用：

`Detect → Classify → Contain → Recover → Preserve → Verify`

本题最重要的 Preserve 对象是：**schema validation 失败是一个显式状态，必须有有界修复策略和可观测错误。**。
...
### 1. 连续三次 schema 都失败怎么办？

**回答方向**：先以“区分 syntax error 与 semantic error。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. parser repair 会不会掩盖模型问题？

**回答方向**：先以“区分 syntax error 与 semantic error。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

## 易错回答

> 无限 retry，或者静默丢字段继续执行。
...
更深一层看，本题的本质是保护这个系统 invariant：**schema validation 失败是一个显式状态，必须有有界修复策略和可观测错误。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

用 Pydantic/JSON Schema 做边界校验，错误分类后决定 retry、repair、fallback 或 fail-fast。
...
- [ ] 核心 invariant 已写成可验证条件：schema validation 失败是一个显式状态，必须有有界修复策略和可观测错误。
...
[← Q006](q006.md) · [本章目录](README.md) · [Q008 →](q008.md)
