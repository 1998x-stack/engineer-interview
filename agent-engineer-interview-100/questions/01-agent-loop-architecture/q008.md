...
### 1. Run State 随一次执行结束；Session State 跨多轮对话；User State 跨会话；Business State 属于外部系统真相。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`为每个字段写 owner、source of truth、TTL、consistency、privacy 五个属性，再决定存储位置。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开
...
### 1. Memory 属于 Session 还是 User State？

**回答方向**：先以“Run State 随一次执行结束；Session State 跨多轮对话；User State 跨会话；Business State 属于外部系统真相。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
> 用一个 Redis key 存所有状态，不区分生命周期和权威性。
...
为每个字段写 owner、source of truth、TTL、consistency、privacy 五个属性，再决定存储位置。
...
[← Q007](q007.md) · [本章目录](README.md) · [Q009 →](q009.md)
