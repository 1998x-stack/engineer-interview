---
id: q009
title: "什么是 Agent Harness？它和 Prompt、Agent Framework 的区别是什么？"
...
# Q009 · 什么是 Agent Harness？它和 Prompt、Agent Framework 的区别是什么？
...
| 失败类别 | Control / State |
| 风险等级 | 中 |
| 核心 Invariant | 控制流可解释、状态可重放、完成条件可验证 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

复杂 Agent 的性能取决于模型和环境共同作用。好的 Harness 把模型的概率能力约束成可操作、可恢复的系统行为。
...
### 1. Harness 是‘模型周围的工程系统’，不是单一库。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 2. 同一模型换 Harness，成功率可能显著变化。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 3. 生产成熟度通常体现在 Harness，而不是 prompt 技巧数量。

把这个点落到状态机：明确输入状态、允许的 transition、产生的 side effect、失败后的合法下一状态。这样才能从概念讨论进入可测试实现。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`建立统一 runtime 层：context builder、tool gateway、state store、policy、trace、eval hooks。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**控制流可解释、状态可重放、完成条件可验证**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**控制流可解释、状态可重放、完成条件可验证**。
...
### 1. 为什么换模型不一定解决 Agent 稳定性问题？

**回答方向**：先以“Harness 是‘模型周围的工程系统’，不是单一库。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
> 把 Harness 仅理解为 system prompt 或工具集合。
...
更深一层看，本题的本质是保护这个系统 invariant：**控制流可解释、状态可重放、完成条件可验证**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

建立统一 runtime 层：context builder、tool gateway、state store、policy、trace、eval hooks。
...
- [ ] 核心 invariant 已写成可验证条件：控制流可解释、状态可重放、完成条件可验证
...
[← Q008](q008.md) · [本章目录](README.md) · [Q010 →](q010.md)
