---
id: q011
title: "ReAct、Plan-and-Execute、Reflection 分别适合什么任务？"
...
# Q011 · ReAct、Plan-and-Execute、Reflection 分别适合什么任务？
...
| 风险等级 | 中 |
| 核心 Invariant | 每一步都推动目标进度，偏离可检测，计划可局部修复 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

规划范式选择的本质是用多少前瞻换多少运行时灵活性。
...
### 1. ReAct 的优势是即时适应，缺点是局部贪心与循环。

把这个点落到 trajectory 控制：定义 progress signal、触发阈值和后续动作。只有“发现异常”而没有 replan/stop/clarify 策略仍不完整。

### 2. Plan-and-Execute 降低每步规划成本，但初始计划可能过时。

成本优化必须和任务成功率绑定。减少 token、换小模型、增加缓存都要看 `cost_per_success`，否则可能只是把费用转成失败率和人工升级成本。

### 3. Reflection 只有在反馈信号有信息量时才值得付额外 token。

成本优化必须和任务成功率绑定。减少 token、换小模型、增加缓存都要看 `cost_per_success`，否则可能只是把费用转成失败率和人工升级成本。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`用任务长度、环境变化率、可验证性、工具延迟四个维度选择策略，并通过 eval 验证。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**每一步都推动目标进度，偏离可检测，计划可局部修复**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**每一步都推动目标进度，偏离可检测，计划可局部修复**。
...
### 1. 什么时候 Reflection 反而降低效果？

**回答方向**：先以“ReAct 的优势是即时适应，缺点是局部贪心与循环。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**每一步都推动目标进度，偏离可检测，计划可局部修复**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

用任务长度、环境变化率、可验证性、工具延迟四个维度选择策略，并通过 eval 验证。
...
- [ ] 核心 invariant 已写成可验证条件：每一步都推动目标进度，偏离可检测，计划可局部修复
...
[← Q010](../01-agent-loop-architecture/q010.md) · [本章目录](README.md) · [Q012 →](q012.md)
