...
| 风险等级 | 中高 |
| 核心 Invariant | trajectory 必须持续满足 goal invariant，并在 milestone 上产生可测进展。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

Agent 错误会沿后续步骤放大。越早检测，blast radius 越小，恢复成本越低。
...
### 1. Drift detection 既有规则信号，也可有 evaluator。

Agent Eval 至少拆成 component、trajectory、end-to-end 三层。这样最终失败时能判断是检索、规划、工具还是 verifier 出错，并把回归定位到具体 transition。

### 2. trajectory-level 评估比只看 final answer 更早暴露问题。

Agent Eval 至少拆成 component、trajectory、end-to-end 三层。这样最终失败时能判断是检索、规划、工具还是 verifier 出错，并把回归定位到具体 transition。

### 3. 高风险动作前必须强制 precondition check。

把这个点落到 trajectory 控制：定义 progress signal、触发阈值和后续动作。只有“发现异常”而没有 replan/stop/clarify 策略仍不完整。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`记录 expected_next_state 与 actual_state；配置 repeated-call detector、constraint checker 和 milestone verifier。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**trajectory 必须持续满足 goal invariant，并在 milestone 上产生可测进展。**。然后按下面四层回答：
...
- **只在最后做 eval，错误已经产生副作用**
- **progress 只靠模型自评**
- **漂移检测没有触发 stop/replan 的动作策略**

遇到异常时，不要立即跳到“重试”。建议统一使用：

`Detect → Classify → Contain → Recover → Preserve → Verify`

本题最重要的 Preserve 对象是：**trajectory 必须持续满足 goal invariant，并在 milestone 上产生可测进展。**。
...
**回答方向**：先以“Drift detection 既有规则信号，也可有 evaluator。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. Evaluator 也可能判断错怎么办？
...
故意让第 4 步工具返回与目标无关但语义合理的结果，看系统是否能通过 milestone/progress evaluator 在最终答案之前拦截。
...
更深一层看，本题的本质是保护这个系统 invariant：**trajectory 必须持续满足 goal invariant，并在 milestone 上产生可测进展。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

记录 expected_next_state 与 actual_state；配置 repeated-call detector、constraint checker 和 milestone verifier。
...
- [ ] 核心 invariant 已写成可验证条件：trajectory 必须持续满足 goal invariant，并在 milestone 上产生可测进展。
...
[← Q012](q012.md) · [本章目录](README.md) · [Q014 →](q014.md)
