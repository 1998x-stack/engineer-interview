---
id: q015
title: "ReAct Agent 为什么经常死循环？怎么识别？"
...
# Q015 · ReAct Agent 为什么经常死循环？怎么识别？
...
| 风险等级 | 中高 |
| 核心 Invariant | 重复动作必须带来可量化的新信息或状态变化，否则触发 cycle/stagnation 保护。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

死循环是 Agent 自治的典型失效模式；识别循环模式比硬限步更能保持成功率。
...
### 1. step limit 只能兜底，不能解释根因。

把这个点落到 trajectory 控制：定义 progress signal、触发阈值和后续动作。只有“发现异常”而没有 replan/stop/clarify 策略仍不完整。

### 2. 区分 retry loop、tool-selection loop、reasoning loop。

Retry 必须同时满足：错误瞬时可恢复、操作可安全重放、仍在父 deadline 内、没有超过 retry budget。否则重试只是把局部故障放大成队列/下游雪崩。

### 3. 循环可用 trajectory signature 检测。

把这个点落到 trajectory 控制：定义 progress signal、触发阈值和后续动作。只有“发现异常”而没有 replan/stop/clarify 策略仍不完整。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`定义 normalized action signature；连续重复超过阈值且 observation 无变化时进入 replan 或 fallback。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**重复动作必须带来可量化的新信息或状态变化，否则触发 cycle/stagnation 保护。**。然后按下面四层回答：
...
- **同一 query/tool args 连续出现**
- **Observation 重复但模型继续尝试同一路径**
- **Reflection 自我循环但没有 external evidence**

遇到异常时，不要立即跳到“重试”。建议统一使用：

`Detect → Classify → Contain → Recover → Preserve → Verify`

本题最重要的 Preserve 对象是：**重复动作必须带来可量化的新信息或状态变化，否则触发 cycle/stagnation 保护。**。
...
**回答方向**：先以“step limit 只能兜底，不能解释根因。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
连续返回相同 observation，验证 repeated-call detector、state hash、information-gain/stagnation threshold 是否能触发改写、replan 或 stop。
...
更深一层看，本题的本质是保护这个系统 invariant：**重复动作必须带来可量化的新信息或状态变化，否则触发 cycle/stagnation 保护。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

定义 normalized action signature；连续重复超过阈值且 observation 无变化时进入 replan 或 fallback。
...
- [ ] 核心 invariant 已写成可验证条件：重复动作必须带来可量化的新信息或状态变化，否则触发 cycle/stagnation 保护。
...
[← Q014](q014.md) · [本章目录](README.md) · [Q016 →](q016.md)
