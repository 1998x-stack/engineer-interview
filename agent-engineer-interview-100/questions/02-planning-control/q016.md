---
id: q016
title: "max_steps 设置 10 还是 100？依据是什么？"
...
# Q016 · max_steps 设置 10 还是 100？依据是什么？
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`从离线 trace 统计成功任务步数分布，按任务复杂度 bucket 配额，并在线监控 budget exhaustion rate。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开
...
### 1. 动态 step budget 怎么做？

**回答方向**：先以“分析 P50/P95 successful steps。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
从离线 trace 统计成功任务步数分布，按任务复杂度 bucket 配额，并在线监控 budget exhaustion rate。
...
[← Q015](q015.md) · [本章目录](README.md) · [Q017 →](q017.md)
