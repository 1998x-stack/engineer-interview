---
id: q018
title: "Reflection 为什么不是简单让模型‘再想一次’？"
...
# Q018 · Reflection 为什么不是简单让模型‘再想一次’？
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`优先接入测试、schema、检索证据和独立 judge；只有检测到可修复问题才触发 reflection。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开
...
### 1. 什么时候 self-reflection 有效？

**回答方向**：先以“Reflection 的价值来自 error signal。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. Actor 和 Judge 用同一模型有什么风险？
...
优先接入测试、schema、检索证据和独立 judge；只有检测到可修复问题才触发 reflection。
...
[← Q017](q017.md) · [本章目录](README.md) · [Q019 →](q019.md)
