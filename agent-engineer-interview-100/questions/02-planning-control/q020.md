---
id: q020
title: "用户在 Agent 执行第 18 步时点击取消，系统如何正确停止？"
...
# Q020 · 用户在 Agent 执行第 18 步时点击取消，系统如何正确停止？
...
| 风险等级 | 高 |
| 核心 Invariant | 取消必须从 parent run 传播到正在运行的 tool/worker，并对已发生副作用做 reconciliation。 |
| 面试频率 | 高频 |
| 难度 | 难 |

## 面试官在考什么

真正的工作可能已经在外部系统继续执行；错误取消会制造幽灵任务和重复副作用。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`run_id 绑定 cancellation token；所有 worker/工具在安全点检查；对外部动作使用 operation_id 做后续 reconciliation。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**取消必须从 parent run 传播到正在运行的 tool/worker，并对已发生副作用做 reconciliation。**。然后按下面四层回答：
...
- **UI 显示取消但后台 worker 继续执行**
- **子 Agent 没收到 cancellation token**
- **取消后 checkpoint 仍标记 RUNNING，形成幽灵任务**

遇到异常时，不要立即跳到“重试”。建议统一使用：

`Detect → Classify → Contain → Recover → Preserve → Verify`

本题最重要的 Preserve 对象是：**取消必须从 parent run 传播到正在运行的 tool/worker，并对已发生副作用做 reconciliation。**。
...
**回答方向**：先以“区分 cooperative cancellation 与 hard kill。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 子 Agent 收不到取消信号怎么办？
...
更深一层看，本题的本质是保护这个系统 invariant：**取消必须从 parent run 传播到正在运行的 tool/worker，并对已发生副作用做 reconciliation。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

run_id 绑定 cancellation token；所有 worker/工具在安全点检查；对外部动作使用 operation_id 做后续 reconciliation。
...
- [ ] 核心 invariant 已写成可验证条件：取消必须从 parent run 传播到正在运行的 tool/worker，并对已发生副作用做 reconciliation。
...
[← Q019](q019.md) · [本章目录](README.md) · [Q021 →](../03-tools-mcp/q021.md)
