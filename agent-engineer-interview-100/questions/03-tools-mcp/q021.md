---
id: q021
title: "Function Calling 背后模型如何决定调用哪个工具？"
...
# Q021 · Function Calling 背后模型如何决定调用哪个工具？
...
| 风险等级 | 中 |
| 核心 Invariant | 模型的工具选择只是一项提议；候选工具集合、schema、权限由 runtime 约束。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

工具选择是模型推理的一部分；设计 action space 直接影响 Agent 行为。
...
### 1. 工具描述要同时写能力边界和不适用条件。

模型的工具选择是语义匹配问题。除了更清晰的描述，还要控制候选集、减少重叠职责、加入正反例，并在 runtime 对最终调用做 schema 与 policy 校验。

### 2. 参数 schema 越明确，调用稳定性通常越高。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`建立 tool-selection eval，统计 confusion matrix；迭代名称、description、examples 和 router。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**模型的工具选择只是一项提议；候选工具集合、schema、权限由 runtime 约束。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**模型的工具选择只是一项提议；候选工具集合、schema、权限由 runtime 约束。**。
...
### 1. 为什么 tool description 会显著影响准确率？

**回答方向**：先以“工具描述要同时写能力边界和不适用条件。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
构造两个名字相近但权限/副作用不同的工具，测试描述、负面条件、候选集和 policy gate 是否共同降低误选。
...
更深一层看，本题的本质是保护这个系统 invariant：**模型的工具选择只是一项提议；候选工具集合、schema、权限由 runtime 约束。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

建立 tool-selection eval，统计 confusion matrix；迭代名称、description、examples 和 router。
...
- [ ] 核心 invariant 已写成可验证条件：模型的工具选择只是一项提议；候选工具集合、schema、权限由 runtime 约束。
...
[← Q020](../02-planning-control/q020.md) · [本章目录](README.md) · [Q022 →](q022.md)
