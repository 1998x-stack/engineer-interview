...
| 风险等级 | 中 |
| 核心 Invariant | 相似工具必须通过清晰职责边界和候选集缩减降低 decision entropy。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

模型在高相似 action space 中会增加选择熵，根因往往是工具设计而非模型智力。
...
### 3. 工具数量过多时做 capability discovery。

把这个点落到 Tool Gateway：模型只提出调用，可信层负责验证 schema、权限、deadline、幂等和结果规范化。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`用离线样本跑 tool routing eval；按错误类型调整 schema、可见性和 deterministic gating。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**相似工具必须通过清晰职责边界和候选集缩减降低 decision entropy。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**相似工具必须通过清晰职责边界和候选集缩减降低 decision entropy。**。
...
**回答方向**：先以“分析错误对的 confusion matrix。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**相似工具必须通过清晰职责边界和候选集缩减降低 decision entropy。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

用离线样本跑 tool routing eval；按错误类型调整 schema、可见性和 deterministic gating。
...
- [ ] 核心 invariant 已写成可验证条件：相似工具必须通过清晰职责边界和候选集缩减降低 decision entropy。
...
[← Q021](q021.md) · [本章目录](README.md) · [Q023 →](q023.md)
