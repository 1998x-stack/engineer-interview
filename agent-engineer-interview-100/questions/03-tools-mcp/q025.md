---
id: q025
title: "Tool 返回 partial success，接口应该怎么表达？"
...
# Q025 · Tool 返回 partial success，接口应该怎么表达？
...
| 风险等级 | 中高 |
| 核心 Invariant | 部分成功必须可机器区分“已完成、失败、未知、可重试”，并保留 item-level operation identity。 |
| 面试频率 | 中频 |
| 难度 | 中 |

## 面试官在考什么

Agent 的恢复能力受工具契约限制。工具越结构化，越容易做精确恢复。
...
### 1. 批量操作尤其需要 item-level result。

把这个点落到 Tool Gateway：模型只提出调用，可信层负责验证 schema、权限、deadline、幂等和结果规范化。

### 2. partial success 要明确是否可安全重试。

Retry 必须同时满足：错误瞬时可恢复、操作可安全重放、仍在父 deadline 内、没有超过 retry budget。否则重试只是把局部故障放大成队列/下游雪崩。

### 3. 结果 schema 应便于机器解析，而不是长自然语言。

把这个点落到 Tool Gateway：模型只提出调用，可信层负责验证 schema、权限、deadline、幂等和结果规范化。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`为 tool result 定义统一 envelope：status/data/errors/retry_after/operation_id。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**部分成功必须可机器区分“已完成、失败、未知、可重试”，并保留 item-level operation identity。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**部分成功必须可机器区分“已完成、失败、未知、可重试”，并保留 item-level operation identity。**。
...
### 1. 如何处理 100 个子任务里失败 3 个？

**回答方向**：先以“批量操作尤其需要 item-level result。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**部分成功必须可机器区分“已完成、失败、未知、可重试”，并保留 item-level operation identity。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

为 tool result 定义统一 envelope：status/data/errors/retry_after/operation_id。
...
- [ ] 核心 invariant 已写成可验证条件：部分成功必须可机器区分“已完成、失败、未知、可重试”，并保留 item-level operation identity。
...
[← Q024](q024.md) · [本章目录](README.md) · [Q026 →](q026.md)
