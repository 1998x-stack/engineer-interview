...
| 风险等级 | 中高 |
| 核心 Invariant | 只有无依赖、无冲突、可独立重试的动作才适合并行；并行度受 deadline 与下游容量约束。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

Agent 的并行性本质上仍受经典并发控制约束。
...
### 1. 并行优化 latency，但会提高资源峰值和结果合并复杂度。

能并行不等于应该并行。需要检查依赖、资源冲突、副作用顺序、下游限流和尾延迟；工程目标是缩短 critical path，而不是最大化并发数。

### 2. 对相同 downstream 设置 concurrency limit。

把这个点落到 Tool Gateway：模型只提出调用，可信层负责验证 schema、权限、deadline、幂等和结果规范化。

### 3. 并行结果要有稳定 correlation id。

能并行不等于应该并行。需要检查依赖、资源冲突、副作用顺序、下游限流和尾延迟；工程目标是缩短 critical path，而不是最大化并发数。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`构建 dependency DAG；标注 read/write set 和 rate limit；调度器按资源冲突决定并行度。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**只有无依赖、无冲突、可独立重试的动作才适合并行；并行度受 deadline 与下游容量约束。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**只有无依赖、无冲突、可独立重试的动作才适合并行；并行度受 deadline 与下游容量约束。**。
...
**回答方向**：先以“并行优化 latency，但会提高资源峰值和结果合并复杂度。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**只有无依赖、无冲突、可独立重试的动作才适合并行；并行度受 deadline 与下游容量约束。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

构建 dependency DAG；标注 read/write set 和 rate limit；调度器按资源冲突决定并行度。
...
- [ ] 核心 invariant 已写成可验证条件：只有无依赖、无冲突、可独立重试的动作才适合并行；并行度受 deadline 与下游容量约束。
...
[← Q025](q025.md) · [本章目录](README.md) · [Q027 →](q027.md)
