---
id: q027
title: "Tool 返回 100K tokens，直接塞回 context 会发生什么？"
...
# Q027 · Tool 返回 100K tokens，直接塞回 context 会发生什么？
...
| 风险等级 | 中高 |
| 核心 Invariant | 工具大输出保存在外部 artifact store，进入 context 的只是当前决策所需切片和引用。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

Context 是稀缺的工作内存，不是持久存储。大工具结果是典型 context pollution 来源。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`Tool Gateway 将大响应写 artifact store，返回摘要+handle；Agent 用 handle 再检索。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**工具大输出保存在外部 artifact store，进入 context 的只是当前决策所需切片和引用。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**工具大输出保存在外部 artifact store，进入 context 的只是当前决策所需切片和引用。**。
...
**回答方向**：先以“区分 raw artifact 与 context view。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 什么时候让 Agent 自己搜索 tool result？
...
更深一层看，本题的本质是保护这个系统 invariant：**工具大输出保存在外部 artifact store，进入 context 的只是当前决策所需切片和引用。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

Tool Gateway 将大响应写 artifact store，返回摘要+handle；Agent 用 handle 再检索。
...
- [ ] 核心 invariant 已写成可验证条件：工具大输出保存在外部 artifact store，进入 context 的只是当前决策所需切片和引用。
...
[← Q026](q026.md) · [本章目录](README.md) · [Q028 →](q028.md)
