---
id: q029
title: "几百个 MCP Tool 全部暴露给模型为什么是坏设计？"
...
# Q029 · 几百个 MCP Tool 全部暴露给模型为什么是坏设计？
...
| 风险等级 | 中高 |
| 核心 Invariant | 每一步只向模型暴露最小、相关、被授权的 capability set。 |
| 面试频率 | 高频 |
| 难度 | 难 |

## 面试官在考什么

Action space 越大，决策难度和攻击面都越大。
...
### 2. 可分 namespace、domain、risk tier。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`做两阶段路由：task -> capability set -> concrete tool；每阶段记录 top-k 与选择置信度。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**每一步只向模型暴露最小、相关、被授权的 capability set。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**每一步只向模型暴露最小、相关、被授权的 capability set。**。
...
**回答方向**：先以“动态工具加载同时提升准确率和安全性。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**每一步只向模型暴露最小、相关、被授权的 capability set。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

做两阶段路由：task -> capability set -> concrete tool；每阶段记录 top-k 与选择置信度。
...
- [ ] 核心 invariant 已写成可验证条件：每一步只向模型暴露最小、相关、被授权的 capability set。
...
[← Q028](q028.md) · [本章目录](README.md) · [Q030 →](q030.md)
