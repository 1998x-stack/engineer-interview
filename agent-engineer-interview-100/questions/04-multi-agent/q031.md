---
id: q031
title: "什么时候 Multi-Agent 比 Single-Agent 更好？什么时候反而更差？"
...
# Q031 · 什么时候 Multi-Agent 比 Single-Agent 更好？什么时候反而更差？
...
| 风险等级 | 中 |
| 核心 Invariant | 只有当并行性/专业化/隔离收益超过协调成本时才拆成 Multi-Agent。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

多 Agent 是架构复杂度换能力，不是免费提升智力。
...
### 1. 收益来自 specialization、parallelism、independent context。

能并行不等于应该并行。需要检查依赖、资源冲突、副作用顺序、下游限流和尾延迟；工程目标是缩短 critical path，而不是最大化并发数。

### 2. 成本来自 coordination、handoff loss、重复 token、错误传播。

消息正确性需要业务状态机兜底：传输层可以 at-least-once，但 consumer 必须按 `message_id/task_id/attempt` 去重，并拒绝旧 attempt 迟到结果覆盖新状态。

### 3. 先证明单 Agent 瓶颈，再引入多 Agent。

把这个点落到分布式任务语义：需要 task ownership、消息 identity、持久化状态和失败传播策略。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`对同一任务做 single vs multi 的 task success/latency/cost 对照实验，再决定。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**只有当并行性/专业化/隔离收益超过协调成本时才拆成 Multi-Agent。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**只有当并行性/专业化/隔离收益超过协调成本时才拆成 Multi-Agent。**。
...
### 1. 如何用 eval 证明 Multi-Agent 有收益？

**回答方向**：用“专业化、并行、上下文隔离”作为收益，用“协调、消息、重复工作、成本和故障面”作为代价；只有净收益经过 eval 证明才拆分。

### 2. 多 Agent 会不会只是 prompt role-play？

**回答方向**：先以“收益来自 specialization、parallelism、independent context。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**只有当并行性/专业化/隔离收益超过协调成本时才拆成 Multi-Agent。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

对同一任务做 single vs multi 的 task success/latency/cost 对照实验，再决定。
...
- [ ] 核心 invariant 已写成可验证条件：只有当并行性/专业化/隔离收益超过协调成本时才拆成 Multi-Agent。
...
[← Q030](../03-tools-mcp/q030.md) · [本章目录](README.md) · [Q032 →](q032.md)
