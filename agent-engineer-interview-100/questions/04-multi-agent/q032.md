---
id: q032
title: "Orchestrator-Worker 架构怎么设计？"
...
# Q032 · Orchestrator-Worker 架构怎么设计？
...
| 风险等级 | 中高 |
| 核心 Invariant | Orchestrator 拥有任务 DAG 与完成判定，Worker 只拥有明确 scoped task。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

真正的多 Agent 系统需要显式控制面，否则会变成不可观测的对话网络。
...
### 1. 任务应有 task_id、acceptance criteria、deadline、budget。

把这个点落到分布式任务语义：需要 task ownership、消息 identity、持久化状态和失败传播策略。

### 2. Worker 输出结构化 artifact 而非只返回聊天。

把这个点落到分布式任务语义：需要 task ownership、消息 identity、持久化状态和失败传播策略。

### 3. Orchestrator 必须处理失败、超时和重分配。

Timeout 代表调用方在 deadline 内没有得到确定结果，不能推导“远端没有执行”。只读请求可相对积极重试；副作用请求要先查状态或用同一幂等键重放。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`用 TaskGraph + queue；orchestrator 维护 task state machine，worker 无权直接修改全局状态。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**Orchestrator 拥有任务 DAG 与完成判定，Worker 只拥有明确 scoped task。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**Orchestrator 拥有任务 DAG 与完成判定，Worker 只拥有明确 scoped task。**。
...
### 1. Orchestrator 自己跑偏怎么办？

**回答方向**：先以“任务应有 task_id、acceptance criteria、deadline、budget。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. Worker 能否创建新的 Worker？
...
更深一层看，本题的本质是保护这个系统 invariant：**Orchestrator 拥有任务 DAG 与完成判定，Worker 只拥有明确 scoped task。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

用 TaskGraph + queue；orchestrator 维护 task state machine，worker 无权直接修改全局状态。
...
- [ ] 核心 invariant 已写成可验证条件：Orchestrator 拥有任务 DAG 与完成判定，Worker 只拥有明确 scoped task。
...
- [Q036 · Handoff 时传整个聊天记录还是最小上下文？](q036.md)
...
[← Q031](q031.md) · [本章目录](README.md) · [Q033 →](q033.md)
