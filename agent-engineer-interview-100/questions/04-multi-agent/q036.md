...
| 风险等级 | 中高 |
| 核心 Invariant | handoff 传递最小充分上下文：contract + relevant state + artifacts + provenance + constraints。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

跨 Agent 复制全部历史既昂贵又扩大污染和泄露面。
...
### 1. handoff 是 context engineering 的一部分。

Partial success 必须保留 item-level 状态，至少区分 `succeeded / failed / unknown / retryable`。如果只返回布尔 `success=false`，上层无法知道哪些子动作已经产生副作用。

### 2. 保留 provenance，避免摘要变成无来源事实。

Context 是模型当轮工作集。关键事实必须在 Durable State / Artifact Store 中 lossless 保存；compaction 只压缩可有损叙事，并保留指向原始证据的引用。

### 3. 不同 Agent 只获得完成其职责所需的信息。

完成语义要拆成三层：调用返回、Agent 声明完成、业务条件满足。只有最后一层能作为真正终止依据，通常由结构化 artifact、外部资源状态或独立 verifier 判断。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`定义 handoff schema：goal/constraints/facts/artifacts/open_questions；由 receiver 缺什么再按需取。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**handoff 传递最小充分上下文：contract + relevant state + artifacts + provenance + constraints。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**handoff 传递最小充分上下文：contract + relevant state + artifacts + provenance + constraints。**。
...
**回答方向**：先以“handoff 是 context engineering 的一部分。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
> 把全量 conversation dump 给每个 Agent。
...
更深一层看，本题的本质是保护这个系统 invariant：**handoff 传递最小充分上下文：contract + relevant state + artifacts + provenance + constraints。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

定义 handoff schema：goal/constraints/facts/artifacts/open_questions；由 receiver 缺什么再按需取。
...
- [ ] 核心 invariant 已写成可验证条件：handoff 传递最小充分上下文：contract + relevant state + artifacts + provenance + constraints。
...
[← Q035](q035.md) · [本章目录](README.md) · [Q037 →](q037.md)
