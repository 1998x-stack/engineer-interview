...
| 风险等级 | 中高 |
| 核心 Invariant | 冲突由证据质量、authority 和 verifier 解决，不默认多数票。 |
| 面试频率 | 高频 |
| 难度 | 难 |

## 面试官在考什么

多 Agent 的数量不等于独立信息源。核心是证据质量和决策权边界。
...
### 2. 不同 Agent 可能共享同一模型偏差。

把这个点落到分布式任务语义：需要 task ownership、消息 identity、持久化状态和失败传播策略。

### 3. 投票适合独立、同质风险较低的判断，不适合事实权限冲突。

ACL 是可信数据平面的强约束。未授权文档最好在候选召回前就被排除；即使做二次过滤，也不能依赖模型“看到了但不使用”。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`让每个 Agent 返回 claim-evidence；裁决器按 source quality、freshness、policy 排序。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**冲突由证据质量、authority 和 verifier 解决，不默认多数票。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**冲突由证据质量、authority 和 verifier 解决，不默认多数票。**。
...
### 2. 如何降低 correlated error？

**回答方向**：先以“置信度不能脱离证据校准。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**冲突由证据质量、authority 和 verifier 解决，不默认多数票。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

让每个 Agent 返回 claim-evidence；裁决器按 source quality、freshness、policy 排序。
...
- [ ] 核心 invariant 已写成可验证条件：冲突由证据质量、authority 和 verifier 解决，不默认多数票。
...
[← Q036](q036.md) · [本章目录](README.md) · [Q038 →](q038.md)
