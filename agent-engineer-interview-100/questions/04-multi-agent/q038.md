---
id: q038
title: "多个 Agent 相互调用进入无限‘甩锅循环’，怎么发现和阻断？"
...
# Q038 · 多个 Agent 相互调用进入无限‘甩锅循环’，怎么发现和阻断？
...
| 风险等级 | 高 |
| 核心 Invariant | 跨 Agent 调用图必须有 depth/TTL/budget/cycle/stagnation 五类上限。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

多 Agent 的循环可能跨进程，单 Agent 局部看不见，因此必须有全局控制面。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`每次 delegation 写 call edge；orchestrator 检查 depth、cycle、budget、progress，再签发 delegation token。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**跨 Agent 调用图必须有 depth/TTL/budget/cycle/stagnation 五类上限。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**跨 Agent 调用图必须有 depth/TTL/budget/cycle/stagnation 五类上限。**。
...
**回答方向**：先以“子 Agent 不应无限拥有 delegation 权。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
构造 A→B→C→A 的环；runtime 通过 call graph + max depth + TTL + budget + repeated-edge detector 在产生大量 token 前终止。
...
> 仅给每个 Agent 一个 max_steps，忽略跨 Agent 循环。
...
更深一层看，本题的本质是保护这个系统 invariant：**跨 Agent 调用图必须有 depth/TTL/budget/cycle/stagnation 五类上限。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

每次 delegation 写 call edge；orchestrator 检查 depth、cycle、budget、progress，再签发 delegation token。
...
- [ ] 核心 invariant 已写成可验证条件：跨 Agent 调用图必须有 depth/TTL/budget/cycle/stagnation 五类上限。
...
[← Q037](q037.md) · [本章目录](README.md) · [Q039 →](q039.md)
