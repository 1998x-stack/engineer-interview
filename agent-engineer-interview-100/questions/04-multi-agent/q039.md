---
id: q039
title: "8 个 Worker 中一个失败，要不要整个任务失败？"
...
# Q039 · 8 个 Worker 中一个失败，要不要整个任务失败？
...
| 风险等级 | 中 |
| 核心 Invariant | 失败传播策略由任务依赖关系与可降级目标决定，而不是“一人失败全体失败”。 |
| 面试频率 | 中频 |
| 难度 | 中 |

## 面试官在考什么

可靠性来自任务级故障域设计，而不是 worker 永不失败。
...
### 3. 重分配时继承已有 artifact，避免从零重复。

消息正确性需要业务状态机兜底：传输层可以 at-least-once，但 consumer 必须按 `message_id/task_id/attempt` 去重，并拒绝旧 attempt 迟到结果覆盖新状态。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`为每个 task 定义 retry_policy、fallback_worker、required_for_success；orchestrator 按 DAG 传播影响。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**失败传播策略由任务依赖关系与可降级目标决定，而不是“一人失败全体失败”。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**失败传播策略由任务依赖关系与可降级目标决定，而不是“一人失败全体失败”。**。
...
### 1. 什么时候采用 speculative redundancy？

**回答方向**：先以“任务图应标注 critical/optional。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 如何防止两个 Worker 同时写同一资源？

**回答方向**：先以“任务图应标注 critical/optional。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

## 易错回答

> 任何 worker 失败都 fail-all，或无条件忽略失败。
...
更深一层看，本题的本质是保护这个系统 invariant：**失败传播策略由任务依赖关系与可降级目标决定，而不是“一人失败全体失败”。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

为每个 task 定义 retry_policy、fallback_worker、required_for_success；orchestrator 按 DAG 传播影响。
...
- [ ] 核心 invariant 已写成可验证条件：失败传播策略由任务依赖关系与可降级目标决定，而不是“一人失败全体失败”。
...
[← Q038](q038.md) · [本章目录](README.md) · [Q040 →](q040.md)
