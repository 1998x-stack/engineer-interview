---
id: q040
title: "多 Agent 如何实现 Context、Storage、Permission 完全隔离，又允许必要交换？"
...
# Q040 · 多 Agent 如何实现 Context、Storage、Permission 完全隔离，又允许必要交换？
...
| 风险等级 | 高 |
| 核心 Invariant | 租户/Agent 默认隔离；跨边界共享通过显式 capability、artifact contract 与 policy gateway。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

多 Agent 增加数据流路径，最容易出现跨上下文污染和越权。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`使用 tenant_id/task_id 贯穿 storage/query/tool token；artifact 分享通过显式 grant。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**租户/Agent 默认隔离；跨边界共享通过显式 capability、artifact contract 与 policy gateway。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**租户/Agent 默认隔离；跨边界共享通过显式 capability、artifact contract 与 policy gateway。**。
...
**回答方向**：先以“隔离边界需要在存储和工具层强制执行。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 同一 Worker 服务多个租户如何防串数据？

**回答方向**：先以“隔离边界需要在存储和工具层强制执行。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

## 易错回答

> 依靠 Agent 名称或 system prompt 约束数据边界。
...
更深一层看，本题的本质是保护这个系统 invariant：**租户/Agent 默认隔离；跨边界共享通过显式 capability、artifact contract 与 policy gateway。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

使用 tenant_id/task_id 贯穿 storage/query/tool token；artifact 分享通过显式 grant。
...
- [ ] 核心 invariant 已写成可验证条件：租户/Agent 默认隔离；跨边界共享通过显式 capability、artifact contract 与 policy gateway。
...
[← Q039](q039.md) · [本章目录](README.md) · [Q041 →](../05-context-memory/q041.md)
