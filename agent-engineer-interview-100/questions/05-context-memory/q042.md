---
id: q042
title: "Context Compaction 和 Context Reset 有什么区别？"
...
# Q042 · Context Compaction 和 Context Reset 有什么区别？
...
| 失败类别 | Context / Memory |
| 风险等级 | 中高 |
| 核心 Invariant | Compaction 保持同一运行连续性；Reset 用结构化 handoff 开启新上下文边界。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

上下文连续性和状态连续性不是一回事。把状态外置后，模型上下文可以安全重建。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`在 milestone checkpoint 生成 handoff package；新 session 从 package + artifact handles 启动。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**Compaction 保持同一运行连续性；Reset 用结构化 handoff 开启新上下文边界。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**Compaction 保持同一运行连续性；Reset 用结构化 handoff 开启新上下文边界。**。
...
更深一层看，本题的本质是保护这个系统 invariant：**Compaction 保持同一运行连续性；Reset 用结构化 handoff 开启新上下文边界。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

在 milestone checkpoint 生成 handoff package；新 session 从 package + artifact handles 启动。
...
- [ ] 核心 invariant 已写成可验证条件：Compaction 保持同一运行连续性；Reset 用结构化 handoff 开启新上下文边界。
...
[← Q041](q041.md) · [本章目录](README.md) · [Q043 →](q043.md)
