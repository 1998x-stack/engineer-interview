---
id: q045
title: "100K token context budget 应该怎么分配？"
...
# Q045 · 100K token context budget 应该怎么分配？
...
| 失败类别 | Context / Memory |
| 风险等级 | 中 |
| 核心 Invariant | Context budget 是资源预算：先留足输出和工具 schema，再按边际信息价值分配输入。 |
| 面试频率 | 中频 |
| 难度 | 中 |

## 面试官在考什么

上下文选择是资源调度问题；目标是最大化每 token 的决策价值。
...
### 1. token budget 是 runtime resource。
...
### 3. 可用 marginal utility 决定是否加入下一段上下文。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`构建 context packer，按 priority score/token cost 做装箱，并记录被裁剪项供调试。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**Context budget 是资源预算：先留足输出和工具 schema，再按边际信息价值分配输入。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**Context budget 是资源预算：先留足输出和工具 schema，再按边际信息价值分配输入。**。
...
### 1. 如何知道某段 context 是否有价值？

**回答方向**：先以“token budget 是 runtime resource。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 预留 output budget 为什么重要？

**回答方向**：先以“token budget 是 runtime resource。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

## 易错回答

> 按固定 20/20/20/20/20 比例切。
...
更深一层看，本题的本质是保护这个系统 invariant：**Context budget 是资源预算：先留足输出和工具 schema，再按边际信息价值分配输入。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

构建 context packer，按 priority score/token cost 做装箱，并记录被裁剪项供调试。
...
- [ ] 核心 invariant 已写成可验证条件：Context budget 是资源预算：先留足输出和工具 schema，再按边际信息价值分配输入。
...
[← Q044](q044.md) · [本章目录](README.md) · [Q046 →](q046.md)
