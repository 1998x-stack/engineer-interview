---
id: q046
title: "多 Agent 是否应该共享同一个 Context？"
...
# Q046 · 多 Agent 是否应该共享同一个 Context？
...
| 失败类别 | Context / Memory |
| 风险等级 | 中 |
| 核心 Invariant | Agent 之间默认不共享 raw context；共享结构化任务事实和受控 artifact。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

多 Agent 的价值之一就是独立上下文；全量共享会抵消这种优势并增加耦合。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`设计 public task state + private agent context；跨边界只能通过 typed message 或 artifact。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**Agent 之间默认不共享 raw context；共享结构化任务事实和受控 artifact。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**Agent 之间默认不共享 raw context；共享结构化任务事实和受控 artifact。**。
...
**回答方向**：先以“隔离能降低干扰和 token 成本。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
> 所有 Agent 读写同一个大 message buffer。
...
更深一层看，本题的本质是保护这个系统 invariant：**Agent 之间默认不共享 raw context；共享结构化任务事实和受控 artifact。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

设计 public task state + private agent context；跨边界只能通过 typed message 或 artifact。
...
- [ ] 核心 invariant 已写成可验证条件：Agent 之间默认不共享 raw context；共享结构化任务事实和受控 artifact。
...
[← Q045](q045.md) · [本章目录](README.md) · [Q047 →](q047.md)
