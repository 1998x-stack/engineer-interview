...
| 失败类别 | Context / Memory |
| 风险等级 | 中高 |
| 核心 Invariant | 只有未来任务可复用、稳定、可信且用户允许的信息才写长期记忆。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

记忆错误会跨任务持续放大，比单轮幻觉更危险。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`写入前经过 memory gate：utility、sensitivity、confidence、TTL、consent；低置信只做 episodic note。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**只有未来任务可复用、稳定、可信且用户允许的信息才写长期记忆。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**只有未来任务可复用、稳定、可信且用户允许的信息才写长期记忆。**。
...
**回答方向**：先以“Memory 需要 write policy 与 read policy。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**只有未来任务可复用、稳定、可信且用户允许的信息才写长期记忆。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

写入前经过 memory gate：utility、sensitivity、confidence、TTL、consent；低置信只做 episodic note。
...
- [ ] 核心 invariant 已写成可验证条件：只有未来任务可复用、稳定、可信且用户允许的信息才写长期记忆。
...
[← Q046](q046.md) · [本章目录](README.md) · [Q048 →](q048.md)
