...
| 失败类别 | Context / Memory |
| 风险等级 | 中高 |
| 核心 Invariant | Memory 是带版本和 provenance 的事实集合，不是简单覆盖式 KV。 |
| 面试频率 | 高频 |
| 难度 | 难 |

## 面试官在考什么

现实世界事实会变化；缺少 provenance 的 memory 无法判断新旧和可信度。
...
### 1. Memory 是 versioned knowledge，而不是 key-value 永久真相。
...
### 3. 对不可判定冲突返回 uncertainty。

把这个点落到 context builder：先确定必须保留的事实，再决定压缩、检索和优先级，而不是按时间顺序简单截断。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`memory record 加 valid_from/valid_to/source/confidence/supersedes；读取时做 conflict resolution。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**Memory 是带版本和 provenance 的事实集合，不是简单覆盖式 KV。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**Memory 是带版本和 provenance 的事实集合，不是简单覆盖式 KV。**。
...
**回答方向**：先以“Memory 是 versioned knowledge，而不是 key-value 永久真相。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 企业事实谁有更高 authority？
...
更深一层看，本题的本质是保护这个系统 invariant：**Memory 是带版本和 provenance 的事实集合，不是简单覆盖式 KV。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

memory record 加 valid_from/valid_to/source/confidence/supersedes；读取时做 conflict resolution。
...
- [ ] 核心 invariant 已写成可验证条件：Memory 是带版本和 provenance 的事实集合，不是简单覆盖式 KV。
...
[← Q047](q047.md) · [本章目录](README.md) · [Q049 →](q049.md)
