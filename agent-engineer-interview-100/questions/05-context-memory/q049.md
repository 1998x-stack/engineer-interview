---
id: q049
title: "Semantic、Episodic、Procedural Memory 分别解决什么问题？"
...
# Q049 · Semantic、Episodic、Procedural Memory 分别解决什么问题？
...
| 失败类别 | Context / Memory |
| 风险等级 | 中 |
| 核心 Invariant | 语义记忆存事实，情景记忆存经历，程序记忆存可复用策略；生命周期和检索方式不同。 |
| 面试频率 | 中频 |
| 难度 | 中 |

## 面试官在考什么

不同记忆类型的‘相似度’和‘新鲜度’含义不同，混在一个向量库会难以治理。
...
### 2. Episodic 用于经验回忆和案例检索。

把 RAG 看成证据供应链：trigger → retrieve → filter → rerank → validate → consume。每一层都有独立 failure mode 和指标，不能用最终回答准确率掩盖检索故障。

### 3. Procedural 应谨慎更新，避免错误经验固化。

知识新鲜度必须显式建模成版本/时间戳/index lag。对高风险动作，若索引可能落后，就应回源 authoritative system 或拒绝以旧证据执行。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`分别定义 schema/index/retrieval policy；统一通过 memory router 按任务查询。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**语义记忆存事实，情景记忆存经历，程序记忆存可复用策略；生命周期和检索方式不同。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**语义记忆存事实，情景记忆存经历，程序记忆存可复用策略；生命周期和检索方式不同。**。
...
**回答方向**：先以“Semantic 更接近知识库。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. Skill 文件算 procedural memory 吗？
...
更深一层看，本题的本质是保护这个系统 invariant：**语义记忆存事实，情景记忆存经历，程序记忆存可复用策略；生命周期和检索方式不同。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

分别定义 schema/index/retrieval policy；统一通过 memory router 按任务查询。
...
- [ ] 核心 invariant 已写成可验证条件：语义记忆存事实，情景记忆存经历，程序记忆存可复用策略；生命周期和检索方式不同。
...
[← Q048](q048.md) · [本章目录](README.md) · [Q050 →](q050.md)
