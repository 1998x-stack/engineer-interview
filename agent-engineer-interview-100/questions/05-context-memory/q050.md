...
| 失败类别 | Context / Memory |
| 风险等级 | 中高 |
| 核心 Invariant | Memory 的价值必须通过 ablation 证明：加入后任务成功率提升且 stale/contradiction 风险可控。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

Memory 的目标是改善任务行为，不是把数据库查得更像。
...
### 1. Memory 可能提高体验也可能引入错误。

长期 Memory 需要 read policy 和 write policy：什么值得写、来源是谁、何时过期、冲突怎么处理。没有 provenance/TTL 的向量记忆很容易把旧事实当成新事实。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`建立 memory eval set：需记忆/不该记忆/冲突/过时/敏感五类；比较 end-to-end delta。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**Memory 的价值必须通过 ablation 证明：加入后任务成功率提升且 stale/contradiction 风险可控。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**Memory 的价值必须通过 ablation 证明：加入后任务成功率提升且 stale/contradiction 风险可控。**。
...
### 1. Memory 命中了但模型没用怎么办？

**回答方向**：先以“Memory 可能提高体验也可能引入错误。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 怎样构造 memory benchmark？
...
更深一层看，本题的本质是保护这个系统 invariant：**Memory 的价值必须通过 ablation 证明：加入后任务成功率提升且 stale/contradiction 风险可控。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

建立 memory eval set：需记忆/不该记忆/冲突/过时/敏感五类；比较 end-to-end delta。
...
- [ ] 核心 invariant 已写成可验证条件：Memory 的价值必须通过 ablation 证明：加入后任务成功率提升且 stale/contradiction 风险可控。
...
[← Q049](q049.md) · [本章目录](README.md) · [Q051 →](../06-agentic-rag/q051.md)
