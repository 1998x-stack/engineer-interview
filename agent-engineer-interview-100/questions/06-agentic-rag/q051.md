---
id: q051
title: "Agent 中 RAG 应该一直执行，还是作为 Tool 按需调用？"
chapter: "Agentic RAG"
chapter_no: 6
frequency: "高频"
difficulty: "中"
...
# Q051 · Agent 中 RAG 应该一直执行，还是作为 Tool 按需调用？
...
| 风险等级 | 中 |
| 核心 Invariant | 检索触发由任务的知识需求决定；always-retrieve 与 on-demand 都需要用 eval 选型。 |
| 面试频率 | 高频 |
| 难度 | 中 |
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`按 domain/risk/freshness 定义 mandatory retrieval；其他场景由 router 判断并记录 decision reason。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**检索触发由任务的知识需求决定；always-retrieve 与 on-demand 都需要用 eval 选型。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**检索触发由任务的知识需求决定；always-retrieve 与 on-demand 都需要用 eval 选型。**。
...
**回答方向**：先以“检索本身有 latency/cost。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**检索触发由任务的知识需求决定；always-retrieve 与 on-demand 都需要用 eval 选型。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

按 domain/risk/freshness 定义 mandatory retrieval；其他场景由 router 判断并记录 decision reason。
...
- [ ] 核心 invariant 已写成可验证条件：检索触发由任务的知识需求决定；always-retrieve 与 on-demand 都需要用 eval 选型。
...
[← Q050](../05-context-memory/q050.md) · [本章目录](README.md) · [Q052 →](q052.md)
