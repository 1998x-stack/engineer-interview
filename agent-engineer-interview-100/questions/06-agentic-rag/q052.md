...
| 风险等级 | 中高 |
| 核心 Invariant | retrieval trigger 要把“不知道”转成可校准决策，并在高风险场景使用强制检索规则。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

RAG 的第一步不是检索算法，而是是否检索。漏检会导致无依据生成，过检索则浪费资源。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`构造 must-retrieve / optional / no-retrieve 数据集，评估触发准确率，再联合端到端指标调阈值。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**retrieval trigger 要把“不知道”转成可校准决策，并在高风险场景使用强制检索规则。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**retrieval trigger 要把“不知道”转成可校准决策，并在高风险场景使用强制检索规则。**。
...
### 1. Retrieval trigger 的标签怎么标？

**回答方向**：先以“self-confidence 未必校准。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**retrieval trigger 要把“不知道”转成可校准决策，并在高风险场景使用强制检索规则。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

构造 must-retrieve / optional / no-retrieve 数据集，评估触发准确率，再联合端到端指标调阈值。
...
- [ ] 核心 invariant 已写成可验证条件：retrieval trigger 要把“不知道”转成可校准决策，并在高风险场景使用强制检索规则。
...
[← Q051](q051.md) · [本章目录](README.md) · [Q053 →](q053.md)
