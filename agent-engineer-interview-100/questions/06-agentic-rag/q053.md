...
| 风险等级 | 中 |
| 核心 Invariant | chunk 粒度要在语义完整性、检索分辨率和上下文成本之间优化。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

Chunk 是检索单元，也是生成上下文单元，影响精确率和完整性两个方向。
...
### 2. 可用 parent-child / small-to-big retrieval。

把 RAG 看成证据供应链：trigger → retrieve → filter → rerank → validate → consume。每一层都有独立 failure mode 和指标，不能用最终回答准确率掩盖检索故障。

### 3. chunk 参数必须以任务评估为依据。

把 RAG 看成证据供应链：trigger → retrieve → filter → rerank → validate → consume。每一层都有独立 failure mode 和指标，不能用最终回答准确率掩盖检索故障。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`先按标题/段落/语义切分，网格搜索 chunk_size/overlap/top_k，联合评估 Recall 与 answer quality。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**chunk 粒度要在语义完整性、检索分辨率和上下文成本之间优化。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**chunk 粒度要在语义完整性、检索分辨率和上下文成本之间优化。**。
...
**回答方向**：先以“固定字符切分往往破坏语义边界。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**chunk 粒度要在语义完整性、检索分辨率和上下文成本之间优化。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

先按标题/段落/语义切分，网格搜索 chunk_size/overlap/top_k，联合评估 Recall 与 answer quality。
...
- [ ] 核心 invariant 已写成可验证条件：chunk 粒度要在语义完整性、检索分辨率和上下文成本之间优化。
...
[← Q052](q052.md) · [本章目录](README.md) · [Q054 →](q054.md)
