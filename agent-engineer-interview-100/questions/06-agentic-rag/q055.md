---
id: q055
title: "Retriever 找错资料，Agent 根据错误资料执行错误动作，怎么解决？"
chapter: "Agentic RAG"
chapter_no: 6
frequency: "必考"
difficulty: "难"
...
# Q055 · Retriever 找错资料，Agent 根据错误资料执行错误动作，怎么解决？
...
| 风险等级 | 高 |
| 核心 Invariant | 证据错误不能直接越过 verifier 触发高风险动作；action threshold 高于 answer threshold。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

错误知识进入 Agent 后会转化为真实动作，blast radius 高于普通问答。
...
### 2. RAG evidence 需要 provenance。

把这个点落到证据质量：相关性、权限、freshness、provenance 和 sufficiency 都要在动作前被显式验证。

### 3. 使用 action gate 将知识置信度映射到执行权限。

ACL 是可信数据平面的强约束。未授权文档最好在候选召回前就被排除；即使做二次过滤，也不能依赖模型“看到了但不使用”。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`tool precondition 中加入 evidence policy：source tier、age、cross-check、confidence；不足则 ask/HITL。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**证据错误不能直接越过 verifier 触发高风险动作；action threshold 高于 answer threshold。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**证据错误不能直接越过 verifier 触发高风险动作；action threshold 高于 answer threshold。**。
...
### 1. 怎样评估 evidence sufficiency？

**回答方向**：先以“答案错误和动作错误的风险级别不同。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 多个来源互相冲突怎么办？

**回答方向**：多数票只在独立同分布且错误近似独立时有意义。生产场景更应比较证据 provenance、authority、freshness 和 verifier 规则，必要时升级人工裁决。

## 易错回答

> 只优化 Recall@K，认为检索到 top1 就可信。
...
更深一层看，本题的本质是保护这个系统 invariant：**证据错误不能直接越过 verifier 触发高风险动作；action threshold 高于 answer threshold。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

tool precondition 中加入 evidence policy：source tier、age、cross-check、confidence；不足则 ask/HITL。
...
- [ ] 核心 invariant 已写成可验证条件：证据错误不能直接越过 verifier 触发高风险动作；action threshold 高于 answer threshold。
...
- [Q057 · 有 ACL 的企业知识库，是 retrieval 前过滤还是 retrieval 后过滤？](q057.md)
- [Q059 · RAG 到底怎么评估？](q059.md)
...
[← Q054](q054.md) · [本章目录](README.md) · [Q056 →](q056.md)
