---
id: q056
title: "Agent 如何做 Multi-hop / Iterative Retrieval？"
chapter: "Agentic RAG"
chapter_no: 6
frequency: "高频"
difficulty: "难"
...
# Q056 · Agent 如何做 Multi-hop / Iterative Retrieval？
...
| 风险等级 | 中高 |
| 核心 Invariant | 每轮检索必须缩小未知量或增加证据，否则触发停止/改写/澄清。 |
| 面试频率 | 高频 |
| 难度 | 难 |

## 面试官在考什么

复杂问题的答案往往分散在多个实体或文档中，单次相似度检索难覆盖完整链路。
...
### 1. 下一跳 query 应由 evidence gap 驱动。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`维护 claim/evidence graph；每轮只为未覆盖 claim 生成 query，并做 dedup + max_hops。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**每轮检索必须缩小未知量或增加证据，否则触发停止/改写/澄清。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**每轮检索必须缩小未知量或增加证据，否则触发停止/改写/澄清。**。
...
**回答方向**：先以“下一跳 query 应由 evidence gap 驱动。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**每轮检索必须缩小未知量或增加证据，否则触发停止/改写/澄清。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

维护 claim/evidence graph；每轮只为未覆盖 claim 生成 query，并做 dedup + max_hops。
...
- [ ] 核心 invariant 已写成可验证条件：每轮检索必须缩小未知量或增加证据，否则触发停止/改写/澄清。
...
[← Q055](q055.md) · [本章目录](README.md) · [Q057 →](q057.md)
