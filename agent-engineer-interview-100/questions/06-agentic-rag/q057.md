---
id: q057
title: "有 ACL 的企业知识库，是 retrieval 前过滤还是 retrieval 后过滤？"
chapter: "Agentic RAG"
chapter_no: 6
frequency: "必考"
difficulty: "难"
...
# Q057 · 有 ACL 的企业知识库，是 retrieval 前过滤还是 retrieval 后过滤？
...
| 风险等级 | 高 |
| 核心 Invariant | ACL 必须在可信检索层强制，未授权文档不应进入候选集；后过滤不能作为唯一防线。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

只要无权限内容进入模型上下文，就已经越过了安全边界。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`索引记录携带 tenant_id/acl_version；query 必须注入可信 identity filter，并做审计。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**ACL 必须在可信检索层强制，未授权文档不应进入候选集；后过滤不能作为唯一防线。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**ACL 必须在可信检索层强制，未授权文档不应进入候选集；后过滤不能作为唯一防线。**。
...
**回答方向**：先以“权限过滤应尽量早。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**ACL 必须在可信检索层强制，未授权文档不应进入候选集；后过滤不能作为唯一防线。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

索引记录携带 tenant_id/acl_version；query 必须注入可信 identity filter，并做审计。
...
- [ ] 核心 invariant 已写成可验证条件：ACL 必须在可信检索层强制，未授权文档不应进入候选集；后过滤不能作为唯一防线。
...
[← Q056](q056.md) · [本章目录](README.md) · [Q058 →](q058.md)
