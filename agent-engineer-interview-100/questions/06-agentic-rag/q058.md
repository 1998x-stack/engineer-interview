---
id: q058
title: "知识库一分钟更新一次，但向量索引十分钟更新一次，Agent 如何避免读旧数据？"
chapter: "Agentic RAG"
chapter_no: 6
frequency: "高频"
difficulty: "难"
...
# Q058 · 知识库一分钟更新一次，但向量索引十分钟更新一次，Agent 如何避免读旧数据？
...
| 风险等级 | 中高 |
| 核心 Invariant | 每条知识有版本/freshness；读路径能识别 index lag 并回源或拒绝使用过期证据。 |
| 面试频率 | 高频 |
| 难度 | 难 |

## 面试官在考什么

RAG 引入了新的数据延迟层，实时性要求必须显式设计。
...
### 1. 知识源有 source of truth 与 derived index 之分。
...
### 3. 缓存也要参与 freshness 判断。

知识新鲜度必须显式建模成版本/时间戳/index lag。对高风险动作，若索引可能落后，就应回源 authoritative system 或拒绝以旧证据执行。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`为每个 domain 定义 freshness SLA；retriever 返回 version；超阈值自动查询原系统或阻断高风险回答。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**每条知识有版本/freshness；读路径能识别 index lag 并回源或拒绝使用过期证据。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**每条知识有版本/freshness；读路径能识别 index lag 并回源或拒绝使用过期证据。**。
...
**回答方向**：先以“知识源有 source of truth 与 derived index 之分。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**每条知识有版本/freshness；读路径能识别 index lag 并回源或拒绝使用过期证据。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

为每个 domain 定义 freshness SLA；retriever 返回 version；超阈值自动查询原系统或阻断高风险回答。
...
- [ ] 核心 invariant 已写成可验证条件：每条知识有版本/freshness；读路径能识别 index lag 并回源或拒绝使用过期证据。
...
[← Q057](q057.md) · [本章目录](README.md) · [Q059 →](q059.md)
