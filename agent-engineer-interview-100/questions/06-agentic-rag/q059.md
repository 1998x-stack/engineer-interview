...
| 风险等级 | 中高 |
| 核心 Invariant | RAG Eval 分 Retrieval、Grounding/Generation、End-to-End Action 三层，避免最终指标掩盖检索故障。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

分层 eval 才能做 failure attribution 和定向优化。
...
### 3. 按 query taxonomy 分层统计。

把这个点落到证据质量：相关性、权限、freshness、provenance 和 sufficiency 都要在动作前被显式验证。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`建立 golden queries + judged docs；每次索引、embedding、rerank、prompt 变更跑 regression suite。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**RAG Eval 分 Retrieval、Grounding/Generation、End-to-End Action 三层，避免最终指标掩盖检索故障。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**RAG Eval 分 Retrieval、Grounding/Generation、End-to-End Action 三层，避免最终指标掩盖检索故障。**。
...
**回答方向**：先以“检索好不代表生成一定好。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. LLM judge 可以评 RAG 吗？
...
更深一层看，本题的本质是保护这个系统 invariant：**RAG Eval 分 Retrieval、Grounding/Generation、End-to-End Action 三层，避免最终指标掩盖检索故障。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

建立 golden queries + judged docs；每次索引、embedding、rerank、prompt 变更跑 regression suite。
...
- [ ] 核心 invariant 已写成可验证条件：RAG Eval 分 Retrieval、Grounding/Generation、End-to-End Action 三层，避免最终指标掩盖检索故障。
...
- [Q054 · 为什么生产 RAG 常采用 Hybrid Retrieval + Rerank？](q054.md)
...
[← Q058](q058.md) · [本章目录](README.md) · [Q060 →](q060.md)
