...
| 风险等级 | 中高 |
| 核心 Invariant | 依赖故障时走显式 degradation policy，不能静默把“无检索”伪装成“有依据”。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

故障模式需要提前设计，否则事故现场才临时决策会放大风险。
...
### 2. 不同业务定义不同 degraded mode。

把这个点落到证据质量：相关性、权限、freshness、provenance 和 sufficiency 都要在动作前被显式验证。

### 3. 高风险场景可选择 fail-closed。

把这个点落到证据质量：相关性、权限、freshness、provenance 和 sufficiency 都要在动作前被显式验证。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`定义 retriever circuit breaker + fallback chain；每条路径附 mode/source/freshness，trace 中可区分。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**依赖故障时走显式 degradation policy，不能静默把“无检索”伪装成“有依据”。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**依赖故障时走显式 degradation policy，不能静默把“无检索”伪装成“有依据”。**。
...
**回答方向**：先以“Fallback 必须在正常时验证。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**依赖故障时走显式 degradation policy，不能静默把“无检索”伪装成“有依据”。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

定义 retriever circuit breaker + fallback chain；每条路径附 mode/source/freshness，trace 中可区分。
...
- [ ] 核心 invariant 已写成可验证条件：依赖故障时走显式 degradation policy，不能静默把“无检索”伪装成“有依据”。
...
[← Q059](q059.md) · [本章目录](README.md) · [Q061 →](../07-durable-execution/q061.md)
