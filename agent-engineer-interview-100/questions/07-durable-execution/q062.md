...
| 风险等级 | 中高 |
| 核心 Invariant | checkpoint 保存可恢复执行所需的最小完整状态，并带 schema/code version。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

恢复需要知道‘世界已经发生了什么’，而不只是模型看过什么。
...
### 2. 大 artifact 存引用，不必内嵌。

把这个点放进 crash/replay 模型：假设进程在任意两行代码之间崩溃，检查恢复是否会丢状态或重复副作用。

### 3. checkpoint 要原子写入或可判定状态。

Checkpoint 只解决“本地执行状态从哪里恢复”。如果步骤已经对外产生副作用，仍需要额外的 idempotency/reconciliation 机制；否则从旧 checkpoint 重放会重复执行外部动作。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`定义 checkpoint schema；事件流记录每次 transition，checkpoint 保存最近可恢复快照，两者结合 replay。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**checkpoint 保存可恢复执行所需的最小完整状态，并带 schema/code version。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**checkpoint 保存可恢复执行所需的最小完整状态，并带 schema/code version。**。
...
### 1. 保存模型 hidden reasoning 吗？

**回答方向**：先以“关键是足够恢复且可版本化。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**checkpoint 保存可恢复执行所需的最小完整状态，并带 schema/code version。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

定义 checkpoint schema；事件流记录每次 transition，checkpoint 保存最近可恢复快照，两者结合 replay。
...
- [ ] 核心 invariant 已写成可验证条件：checkpoint 保存可恢复执行所需的最小完整状态，并带 schema/code version。
...
- [Q066 · 整个 Agent SLA 30 秒，内部 5 个工具 timeout 怎么分？](q066.md)
...
[← Q061](q061.md) · [本章目录](README.md) · [Q063 →](q063.md)
