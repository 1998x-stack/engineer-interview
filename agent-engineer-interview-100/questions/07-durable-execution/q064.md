---
id: q064
title: "哪些错误应该 retry，哪些绝对不应该 retry？"
...
# Q064 · 哪些错误应该 retry，哪些绝对不应该 retry？
...
| 风险等级 | 高 |
| 核心 Invariant | retry 只用于“可恢复 + 在 deadline 内 + 重试不会放大副作用”的错误。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

错误类型不同，重试可能修复问题，也可能扩大副作用或拖垮下游。
...
### 2. Retry budget 防止重试风暴。

Retry 必须同时满足：错误瞬时可恢复、操作可安全重放、仍在父 deadline 内、没有超过 retry budget。否则重试只是把局部故障放大成队列/下游雪崩。

### 3. 使用 exponential backoff + jitter。

把这个点放进 crash/replay 模型：假设进程在任意两行代码之间崩溃，检查恢复是否会丢状态或重复副作用。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`建立 error taxonomy -> policy table：retry/backoff/fallback/HITL/fail-fast。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**retry 只用于“可恢复 + 在 deadline 内 + 重试不会放大副作用”的错误。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**retry 只用于“可恢复 + 在 deadline 内 + 重试不会放大副作用”的错误。**。
...
### 1. LLM 输出错误算 retryable 吗？

**回答方向**：先以“错误码需要携带 retryable。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 429 的 retry-after 怎么处理？

**回答方向**：先以“错误码需要携带 retryable。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

## 易错回答

> 所有 exception 统一 retry 3 次。
...
更深一层看，本题的本质是保护这个系统 invariant：**retry 只用于“可恢复 + 在 deadline 内 + 重试不会放大副作用”的错误。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

建立 error taxonomy -> policy table：retry/backoff/fallback/HITL/fail-fast。
...
- [ ] 核心 invariant 已写成可验证条件：retry 只用于“可恢复 + 在 deadline 内 + 重试不会放大副作用”的错误。
...
[← Q063](q063.md) · [本章目录](README.md) · [Q065 →](q065.md)
