---
id: q065
title: "Agent 为什么需要 Circuit Breaker？"
...
# Q065 · Agent 为什么需要 Circuit Breaker？
...
| 风险等级 | 中高 |
| 核心 Invariant | 持续失败的依赖必须快速失败，保护调用方线程、token、队列和下游。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

自治系统会自动重复尝试，因此比人工调用更容易形成故障放大器。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`在 Tool Gateway 记录 rolling error rate；OPEN 时返回结构化 degraded result 并触发 fallback。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**持续失败的依赖必须快速失败，保护调用方线程、token、队列和下游。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**持续失败的依赖必须快速失败，保护调用方线程、token、队列和下游。**。
...
**回答方向**：先以“状态通常 CLOSED/OPEN/HALF_OPEN。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 多个实例如何共享 breaker 状态？
...
更深一层看，本题的本质是保护这个系统 invariant：**持续失败的依赖必须快速失败，保护调用方线程、token、队列和下游。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

在 Tool Gateway 记录 rolling error rate；OPEN 时返回结构化 degraded result 并触发 fallback。
...
- [ ] 核心 invariant 已写成可验证条件：持续失败的依赖必须快速失败，保护调用方线程、token、队列和下游。
...
[← Q064](q064.md) · [本章目录](README.md) · [Q066 →](q066.md)
