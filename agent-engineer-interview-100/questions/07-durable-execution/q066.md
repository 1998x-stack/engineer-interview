---
id: q066
title: "整个 Agent SLA 30 秒，内部 5 个工具 timeout 怎么分？"
...
# Q066 · 整个 Agent SLA 30 秒，内部 5 个工具 timeout 怎么分？
...
| 风险等级 | 中高 |
| 核心 Invariant | 子调用 deadline 从父 deadline 递减传播，并保留尾部预算用于聚合/响应。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

局部 timeout 不考虑全局 deadline 会导致尾延迟叠加。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`run_context 保存 deadline_at；每次调用计算 remaining_ms 并设置子 timeout + reserve。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**子调用 deadline 从父 deadline 递减传播，并保留尾部预算用于聚合/响应。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**子调用 deadline 从父 deadline 递减传播，并保留尾部预算用于聚合/响应。**。
...
**回答方向**：先以“timeout budget 应包含模型和排队时间。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**子调用 deadline 从父 deadline 递减传播，并保留尾部预算用于聚合/响应。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

run_context 保存 deadline_at；每次调用计算 remaining_ms 并设置子 timeout + reserve。
...
- [ ] 核心 invariant 已写成可验证条件：子调用 deadline 从父 deadline 递减传播，并保留尾部预算用于聚合/响应。
...
[← Q065](q065.md) · [本章目录](README.md) · [Q067 →](q067.md)
