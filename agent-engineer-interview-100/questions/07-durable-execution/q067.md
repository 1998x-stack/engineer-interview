---
id: q067
title: "流量突然增长 20 倍，Agent 队列怎么保护系统？"
...
# Q067 · 流量突然增长 20 倍，Agent 队列怎么保护系统？
...
| 风险等级 | 中高 |
| 核心 Invariant | 系统在过载时主动拒绝/降级，而不是让队列无限增长导致所有请求超时。 |
| 面试频率 | 高频 |
| 难度 | 难 |

## 面试官在考什么

高并发下最重要的是限制负载进入，而不是让每个请求都慢慢失败。
...
### 3. 监控 queue age 而不只看 queue length。

把这个点放进 crash/replay 模型：假设进程在任意两行代码之间崩溃，检查恢复是否会丢状态或重复副作用。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`设置 token-bucket quota、max queue age、worker concurrency；过载触发 degraded policy。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**系统在过载时主动拒绝/降级，而不是让队列无限增长导致所有请求超时。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**系统在过载时主动拒绝/降级，而不是让队列无限增长导致所有请求超时。**。
...
**回答方向**：先以“Agent 任务耗时长，队列容量比普通 API 更敏感。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**系统在过载时主动拒绝/降级，而不是让队列无限增长导致所有请求超时。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

设置 token-bucket quota、max queue age、worker concurrency；过载触发 degraded policy。
...
- [ ] 核心 invariant 已写成可验证条件：系统在过载时主动拒绝/降级，而不是让队列无限增长导致所有请求超时。
...
[← Q066](q066.md) · [本章目录](README.md) · [Q068 →](q068.md)
