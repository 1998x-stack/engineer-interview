---
id: q068
title: "同一用户同时启动两个修改同一资源的 Agent，怎么办？"
...
# Q068 · 同一用户同时启动两个修改同一资源的 Agent，怎么办？
...
| 风险等级 | 高 |
| 核心 Invariant | 并发写基于 version/lock/compare-and-swap 检测冲突，Agent 不自行“最后写覆盖”。 |
| 面试频率 | 高频 |
| 难度 | 难 |

## 面试官在考什么

Agent 的执行时间长，更容易发生并发冲突和陈旧读取。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`读取资源时保存 version；写入时带 expected_version；冲突返回 typed error 触发 replan。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**并发写基于 version/lock/compare-and-swap 检测冲突，Agent 不自行“最后写覆盖”。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**并发写基于 version/lock/compare-and-swap 检测冲突，Agent 不自行“最后写覆盖”。**。
...
**回答方向**：先以“Agent 长事务不宜一直持锁。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**并发写基于 version/lock/compare-and-swap 检测冲突，Agent 不自行“最后写覆盖”。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

读取资源时保存 version；写入时带 expected_version；冲突返回 typed error 触发 replan。
...
- [ ] 核心 invariant 已写成可验证条件：并发写基于 version/lock/compare-and-swap 检测冲突，Agent 不自行“最后写覆盖”。
...
[← Q067](q067.md) · [本章目录](README.md) · [Q069 →](q069.md)
