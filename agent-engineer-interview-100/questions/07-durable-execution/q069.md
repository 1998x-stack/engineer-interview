---
id: q069
title: "Agent 已执行 A、B、C，D 失败，需要 rollback，怎么设计？"
...
# Q069 · Agent 已执行 A、B、C，D 失败，需要 rollback，怎么设计？
...
| 风险等级 | 高 |
| 核心 Invariant | 跨多个副作用步骤用 Saga/compensation 表达可逆性，无法补偿的步骤推迟到最后或 HITL。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

Agent 常跨多个系统，分布式事务不可得，业务补偿是更现实的可靠性机制。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`TaskGraph 标记 compensation handler；失败触发 saga state machine，并对失败补偿升级人工。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**跨多个副作用步骤用 Saga/compensation 表达可逆性，无法补偿的步骤推迟到最后或 HITL。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**跨多个副作用步骤用 Saga/compensation 表达可逆性，无法补偿的步骤推迟到最后或 HITL。**。
...
**回答方向**：先以“补偿不是数据库 rollback，可能失败。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 什么时候选择 roll-forward 而非 rollback？

**回答方向**：先以“补偿不是数据库 rollback，可能失败。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

## 易错回答

> 认为可以把所有外部 API 包进一个事务。
...
更深一层看，本题的本质是保护这个系统 invariant：**跨多个副作用步骤用 Saga/compensation 表达可逆性，无法补偿的步骤推迟到最后或 HITL。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

TaskGraph 标记 compensation handler；失败触发 saga state machine，并对失败补偿升级人工。
...
- [ ] 核心 invariant 已写成可验证条件：跨多个副作用步骤用 Saga/compensation 表达可逆性，无法补偿的步骤推迟到最后或 HITL。
...
[← Q068](q068.md) · [本章目录](README.md) · [Q070 →](q070.md)
