---
id: q071
title: "Agent 每次 Run 到底应该 Trace 什么？"
...
# Q071 · Agent 每次 Run 到底应该 Trace 什么？
...
| 风险等级 | 中高 |
| 核心 Invariant | Trace 能回答“谁在何时基于什么输入做了什么决定，并产生了什么状态变化”。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

Agent 错误可能发生在任意中间步骤，没有 trajectory 就无法归因。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`采用 OpenTelemetry 类似 span 模型；run_id/task_id/correlation_id 全链路透传，按风险与错误采样。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**Trace 能回答“谁在何时基于什么输入做了什么决定，并产生了什么状态变化”。**。然后按下面四层回答：
...
```text
trace(run_id, user/tenant, workflow, version)
 ├─ agent_span
 │   ├─ model_span(input_ref, model, tokens, output_ref)
 │   ├─ retrieval_span(query, filters, doc_ids, scores)
 │   ├─ tool_span(tool, operation_id, args_hash, status, latency)
 │   ├─ handoff_span(from, to, task_id)
 │   └─ guardrail_span(policy, decision)
 └─ final_eval(task_success, failure_class)
```
...
本题最重要的 Preserve 对象是：**Trace 能回答“谁在何时基于什么输入做了什么决定，并产生了什么状态变化”。**。
...
**回答方向**：默认不记录敏感 payload，改存 hash/reference；必要内容先脱敏并限制访问、加密和保留期。调试环境也要保持 tenant 与权限隔离。

### 2. 如何关联多 Agent 跨服务 span？

**回答方向**：先以“Trace 是 trajectory 的可重放证据。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

## ⭐ 必刷题：深水区

### Trace 可因果测试

给一条失败 run，要求 5 分钟内找 first bad transition。若 trace 缺 input/output ref、state diff 或 correlation，说明 observability 设计不足。
...
更深一层看，本题的本质是保护这个系统 invariant：**Trace 能回答“谁在何时基于什么输入做了什么决定，并产生了什么状态变化”。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

采用 OpenTelemetry 类似 span 模型；run_id/task_id/correlation_id 全链路透传，按风险与错误采样。
...
- [ ] 核心 invariant 已写成可验证条件：Trace 能回答“谁在何时基于什么输入做了什么决定，并产生了什么状态变化”。
...
- [Q079 · 用户说‘Agent 答错了’，怎么判断是 Model、RAG、Tool 还是 Planner 的问题？](q079.md)
...
[← Q070](../07-durable-execution/q070.md) · [本章目录](README.md) · [Q072 →](q072.md)
