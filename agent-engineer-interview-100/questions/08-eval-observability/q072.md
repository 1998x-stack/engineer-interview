---
id: q072
title: "线上 Agent 成功率从 85% 降到 70%，怎么定位？"
...
# Q072 · 线上 Agent 成功率从 85% 降到 70%，怎么定位？
...
| 风险等级 | 中高 |
| 核心 Invariant | 成功率下降先按版本、模型、工具、RAG、租户、任务类型和 failure class 分层切片，寻找首个显著变化。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

聚合成功率无法告诉你根因；分层归因是生产调试的第一步。
...
### 2. 区分模型质量下降与 infra/tool 失败。

把这个点落到 trace/eval：给出可聚合 signal 和 failure class，使线上回归可以被定位而不是靠人工猜测。

### 3. 看 success denominator 是否变化。

把这个点落到 trace/eval：给出可聚合 signal 和 failure class，使线上回归可以被定位而不是靠人工猜测。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`建立 dashboard：task success -> failure type -> component -> version；支持一键 drill-down 到 trace。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**成功率下降先按版本、模型、工具、RAG、租户、任务类型和 failure class 分层切片，寻找首个显著变化。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**成功率下降先按版本、模型、工具、RAG、租户、任务类型和 failure class 分层切片，寻找首个显著变化。**。
...
**回答方向**：先以“检查输入分布和下游依赖是否变化。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
> 随机看几个 bad case 后改 system prompt。
...
更深一层看，本题的本质是保护这个系统 invariant：**成功率下降先按版本、模型、工具、RAG、租户、任务类型和 failure class 分层切片，寻找首个显著变化。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

建立 dashboard：task success -> failure type -> component -> version；支持一键 drill-down 到 trace。
...
- [ ] 核心 invariant 已写成可验证条件：成功率下降先按版本、模型、工具、RAG、租户、任务类型和 failure class 分层切片，寻找首个显著变化。
...
[← Q071](q071.md) · [本章目录](README.md) · [Q073 →](q073.md)
