...
| 风险等级 | 中高 |
| 核心 Invariant | 同一 final answer 可能来自安全路径或侥幸路径，必须同时评 trajectory 与 side effects。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

Agent 的价值在行动；错误轨迹可能已经造成不可逆副作用，即使最终文本看起来正确。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`定义 hard invariants + soft trajectory rubric；对高风险动作硬判，对路径效率用统计评分。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**同一 final answer 可能来自安全路径或侥幸路径，必须同时评 trajectory 与 side effects。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**同一 final answer 可能来自安全路径或侥幸路径，必须同时评 trajectory 与 side effects。**。
...
### 1. 如何评价‘合理轨迹’而不限制创新路径？

**回答方向**：先以“final success 与 process quality 分离。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 哪些步骤必须 deterministic 检查？
...
更深一层看，本题的本质是保护这个系统 invariant：**同一 final answer 可能来自安全路径或侥幸路径，必须同时评 trajectory 与 side effects。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

定义 hard invariants + soft trajectory rubric；对高风险动作硬判，对路径效率用统计评分。
...
- [ ] 核心 invariant 已写成可验证条件：同一 final answer 可能来自安全路径或侥幸路径，必须同时评 trajectory 与 side effects。
...
[← Q073](q073.md) · [本章目录](README.md) · [Q075 →](q075.md)
