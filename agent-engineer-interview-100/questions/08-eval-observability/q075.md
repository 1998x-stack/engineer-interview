...
| 风险等级 | 中高 |
| 核心 Invariant | Judge 也是概率模型，必须校准、做 pairwise/多 judge/人工抽检，并监测漂移。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

评估器如果不稳定，所有优化决策都会被噪声误导。
...
### 1. Judge 也要做 reliability eval。

Agent Eval 至少拆成 component、trajectory、end-to-end 三层。这样最终失败时能判断是检索、规划、工具还是 verifier 出错，并把回归定位到具体 transition。

### 2. 不要把单次 judge score 当客观真值。

把这个点落到 trace/eval：给出可聚合 signal 和 failure class，使线上回归可以被定位而不是靠人工猜测。

### 3. 可计算 inter-rater agreement。

把这个点落到 trace/eval：给出可聚合 signal 和 failure class，使线上回归可以被定位而不是靠人工猜测。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`先用人工标注小集校准 judge；记录 agreement、false positive/negative，关键任务加 deterministic checks。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**Judge 也是概率模型，必须校准、做 pairwise/多 judge/人工抽检，并监测漂移。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**Judge 也是概率模型，必须校准、做 pairwise/多 judge/人工抽检，并监测漂移。**。
...
### 1. 同模型做 actor 和 judge 可不可以？

**回答方向**：使用固定 rubric、pairwise 比较、多个 judge/模型和人工抽样校准；持续监控 judge 与人工一致率，并把 judge 版本写入 eval 结果。

### 2. 什么时候必须人工评？

**回答方向**：先以“Judge 也要做 reliability eval。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

## 易错回答

> 把 LLM Judge 当作万能 ground truth。
...
更深一层看，本题的本质是保护这个系统 invariant：**Judge 也是概率模型，必须校准、做 pairwise/多 judge/人工抽检，并监测漂移。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

先用人工标注小集校准 judge；记录 agreement、false positive/negative，关键任务加 deterministic checks。
...
- [ ] 核心 invariant 已写成可验证条件：Judge 也是概率模型，必须校准、做 pairwise/多 judge/人工抽检，并监测漂移。
...
[← Q074](q074.md) · [本章目录](README.md) · [Q076 →](q076.md)
