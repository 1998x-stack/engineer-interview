---
id: q076
title: "修改一句 System Prompt，怎么确定没有让其他任务退化？"
...
# Q076 · 修改一句 System Prompt，怎么确定没有让其他任务退化？
...
| 风险等级 | 中高 |
| 核心 Invariant | 任何 prompt/model/tool 变更都通过固定回归集 + 关键安全集 + 线上 canary 才扩大流量。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

LLM 行为高度耦合，局部提示修改可能改变其他能力，因此必须回归测试。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`prompt_version 写入 trace；CI 中跑固定 eval；上线 5% canary，指标异常自动回滚。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**任何 prompt/model/tool 变更都通过固定回归集 + 关键安全集 + 线上 canary 才扩大流量。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**任何 prompt/model/tool 变更都通过固定回归集 + 关键安全集 + 线上 canary 才扩大流量。**。
...
**回答方向**：先以“Prompt 也是代码，要版本化。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 如何做 prompt rollback？
...
更深一层看，本题的本质是保护这个系统 invariant：**任何 prompt/model/tool 变更都通过固定回归集 + 关键安全集 + 线上 canary 才扩大流量。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

prompt_version 写入 trace；CI 中跑固定 eval；上线 5% canary，指标异常自动回滚。
...
- [ ] 核心 invariant 已写成可验证条件：任何 prompt/model/tool 变更都通过固定回归集 + 关键安全集 + 线上 canary 才扩大流量。
...
[← Q075](q075.md) · [本章目录](README.md) · [Q077 →](q077.md)
