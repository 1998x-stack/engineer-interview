...
| 风险等级 | 中高 |
| 核心 Invariant | SLO 围绕用户完成目标定义，并同时包含可用性、成功率、尾延迟、成本和安全预算。 |
| 面试频率 | 高频 |
| 难度 | 难 |

## 面试官在考什么

Agent 是复合系统，任一组件成功都不代表用户任务成功。
...
### 1. 模型 API availability 不等于 Agent availability。
...
### 3. 预算与安全也可成为 SLO/guardrail。

时延要按 critical path 分解为 budget，并把 deadline 传到每个子调用。只优化平均时延没有意义；P95/P99、queue wait 和超时后的降级策略才决定生产体验。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`为关键 user journey 定义 SLI；按 severity 设置 error budget，并用 error budget 驱动发布节奏。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**SLO 围绕用户完成目标定义，并同时包含可用性、成功率、尾延迟、成本和安全预算。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**SLO 围绕用户完成目标定义，并同时包含可用性、成功率、尾延迟、成本和安全预算。**。
...
### 1. 创意 Agent 的 task success 怎么定义？

**回答方向**：先以“模型 API availability 不等于 Agent availability。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**SLO 围绕用户完成目标定义，并同时包含可用性、成功率、尾延迟、成本和安全预算。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

为关键 user journey 定义 SLI；按 severity 设置 error budget，并用 error budget 驱动发布节奏。
...
- [ ] 核心 invariant 已写成可验证条件：SLO 围绕用户完成目标定义，并同时包含可用性、成功率、尾延迟、成本和安全预算。
...
[← Q079](q079.md) · [本章目录](README.md) · [Q081 →](../09-security-hitl/q081.md)
