---
id: q081
title: "网页里的 Prompt Injection 告诉 Agent‘忽略之前指令’，怎么办？"
...
# Q081 · 网页里的 Prompt Injection 告诉 Agent‘忽略之前指令’，怎么办？
...
| 失败类别 | Security / Trust |
| 风险等级 | 高 |
| 核心 Invariant | 外部内容永远是数据，不因文本声称而获得 system/tool authority。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

Prompt injection 的根因是模型同时处理指令和数据，而传统文本边界并非真正安全边界。
...
### 1. 区分 control plane instruction 与 data plane content。

把这个点放到 trust boundary：不可信输入只能提出意图，不能获得权限；最终授权发生在可审计的可信执行层。

### 2. 对网页/文档中的 tool-like 指令做隔离。
...
### 3. 高权限 Agent 必须最小化可访问工具。

ACL 是可信数据平面的强约束。未授权文档最好在候选召回前就被排除；即使做二次过滤，也不能依赖模型“看到了但不使用”。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`外部内容包装为 quoted evidence；Tool Gateway 按 user intent + policy 验证 action，不接受网页直接授权。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**外部内容永远是数据，不因文本声称而获得 system/tool authority。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**外部内容永远是数据，不因文本声称而获得 system/tool authority。**。
...
### 1. Indirect prompt injection 怎么测？

**回答方向**：先以“区分 control plane instruction 与 data plane content。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**外部内容永远是数据，不因文本声称而获得 system/tool authority。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

外部内容包装为 quoted evidence；Tool Gateway 按 user intent + policy 验证 action，不接受网页直接授权。
...
- [ ] 核心 invariant 已写成可验证条件：外部内容永远是数据，不因文本声称而获得 system/tool authority。
...
- [Q089 · 权限控制应该放在 Prompt、Agent、Tool Gateway 还是业务 API？](q089.md)
- [Q090 · Memory Poisoning 怎么解决？](q090.md)
...
[← Q080](../08-eval-observability/q080.md) · [本章目录](README.md) · [Q082 →](q082.md)
