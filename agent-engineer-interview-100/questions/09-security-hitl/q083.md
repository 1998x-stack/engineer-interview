---
id: q083
title: "什么操作应该 Human-in-the-Loop？"
...
| 失败类别 | Security / Trust |
| 风险等级 | 高 |
| 核心 Invariant | HITL 由影响、可逆性、不确定性和权限决定，不是“所有 tool 都弹窗”。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

HITL 的目标是把人力放在错误代价最高的决策点。
...
### 3. 批准后签发一次性 approval token。

成本优化必须和任务成功率绑定。减少 token、换小模型、增加缓存都要看 `cost_per_success`，否则可能只是把费用转成失败率和人工升级成本。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`建立 risk scoring；超过阈值生成 immutable action preview，批准 token 绑定 action hash。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**HITL 由影响、可逆性、不确定性和权限决定，不是“所有 tool 都弹窗”。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**HITL 由影响、可逆性、不确定性和权限决定，不是“所有 tool 都弹窗”。**。
...
**回答方向**：先以“HITL 不是所有工具都确认。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 用户批准后参数被 Agent 改了怎么办？

**回答方向**：先以“HITL 不是所有工具都确认。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

## 易错回答

> 按工具名称硬编码‘这个工具必须人工’而不看参数和影响。
...
更深一层看，本题的本质是保护这个系统 invariant：**HITL 由影响、可逆性、不确定性和权限决定，不是“所有 tool 都弹窗”。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

建立 risk scoring；超过阈值生成 immutable action preview，批准 token 绑定 action hash。
...
- [ ] 核心 invariant 已写成可验证条件：HITL 由影响、可逆性、不确定性和权限决定，不是“所有 tool 都弹窗”。
...
- [Q062 · Checkpoint 应该保存什么？](../07-durable-execution/q062.md)
...
[← Q082](q082.md) · [本章目录](README.md) · [Q084 →](q084.md)
