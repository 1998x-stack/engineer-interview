---
id: q084
title: "执行代码的 Agent 为什么要 Sandbox？"
...
| 失败类别 | Security / Trust |
| 风险等级 | 高 |
| 核心 Invariant | 不可信代码在文件、进程、网络、系统调用、CPU/内存和凭证上都被隔离。 |
| 面试频率 | 高频 |
| 难度 | 难 |

## 面试官在考什么

Agent 生成的代码不可完全信任，必须假设它可能意外或恶意访问系统资源。
...
### 3. 产物通过受控 artifact channel 输出。

把这个点放到 trust boundary：不可信输入只能提出意图，不能获得权限；最终授权发生在可审计的可信执行层。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`使用容器/微虚拟机+seccomp/cgroup/network policy；凭证不写进环境，按任务临时注入最小权限。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**不可信代码在文件、进程、网络、系统调用、CPU/内存和凭证上都被隔离。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**不可信代码在文件、进程、网络、系统调用、CPU/内存和凭证上都被隔离。**。
...
### 1. Docker 就等于安全 sandbox 吗？

**回答方向**：先以“sandbox 也要考虑数据 exfiltration。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 如何限制 shell command？
...
更深一层看，本题的本质是保护这个系统 invariant：**不可信代码在文件、进程、网络、系统调用、CPU/内存和凭证上都被隔离。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

使用容器/微虚拟机+seccomp/cgroup/network policy；凭证不写进环境，按任务临时注入最小权限。
...
- [ ] 核心 invariant 已写成可验证条件：不可信代码在文件、进程、网络、系统调用、CPU/内存和凭证上都被隔离。
...
[← Q083](q083.md) · [本章目录](README.md) · [Q085 →](q085.md)
