---
id: q085
title: "Tracing 很重要，但 Trace 中包含用户敏感信息怎么办？"
...
# Q085 · Tracing 很重要，但 Trace 中包含用户敏感信息怎么办？
...
| 失败类别 | Security / Trust |
| 风险等级 | 高 |
| 核心 Invariant | 可观测性默认数据最小化；敏感 payload 采用引用、脱敏、分级访问和保留期。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

可观测性和隐私是双重约束，不能为了 debug 创造新的数据泄漏面。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`日志 SDK 在采集前做 field-level redaction；保存 hash/summary/metadata，原始内容按 break-glass 流程访问。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**可观测性默认数据最小化；敏感 payload 采用引用、脱敏、分级访问和保留期。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**可观测性默认数据最小化；敏感 payload 采用引用、脱敏、分级访问和保留期。**。
...
**回答方向**：先以“trace 本身是敏感数据资产。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
> 为了可调试把所有 prompt、headers、API key 全存下来。
...
更深一层看，本题的本质是保护这个系统 invariant：**可观测性默认数据最小化；敏感 payload 采用引用、脱敏、分级访问和保留期。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

日志 SDK 在采集前做 field-level redaction；保存 hash/summary/metadata，原始内容按 break-glass 流程访问。
...
- [ ] 核心 invariant 已写成可验证条件：可观测性默认数据最小化；敏感 payload 采用引用、脱敏、分级访问和保留期。
...
[← Q084](q084.md) · [本章目录](README.md) · [Q086 →](q086.md)
