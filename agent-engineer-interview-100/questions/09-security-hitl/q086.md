---
id: q086
title: "Multi-Tenant Agent 如何保证 A 公司数据不会进入 B 公司 Context？"
...
# Q086 · Multi-Tenant Agent 如何保证 A 公司数据不会进入 B 公司 Context？
...
| 失败类别 | Security / Trust |
| 风险等级 | 高 |
| 核心 Invariant | tenant identity 从入口贯穿 context、retrieval、storage、tool、cache、trace，每层都强制隔离。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

Agent 会跨很多组件流转数据，任何一层遗漏租户边界都可能串数据。
...
### 2. 缓存 key 必须包含 tenant/security context。

隔离应由身份和存储边界实现，而不是在 prompt 中提醒“不要看别人的数据”。`tenant_id` 必须成为 retrieval filter、cache key、storage partition、tool auth 和 trace metadata 的共同维度。

### 3. 异步消息也要携带 signed identity。

消息正确性需要业务状态机兜底：传输层可以 at-least-once，但 consumer 必须按 `message_id/task_id/attempt` 去重，并拒绝旧 attempt 迟到结果覆盖新状态。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`identity 从认证层签发并不可由模型修改；每个 storage/tool query 由服务端注入 scope。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**tenant identity 从入口贯穿 context、retrieval、storage、tool、cache、trace，每层都强制隔离。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**tenant identity 从入口贯穿 context、retrieval、storage、tool、cache、trace，每层都强制隔离。**。
...
**回答方向**：先以“向量检索必须 tenant filter。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**tenant identity 从入口贯穿 context、retrieval、storage、tool、cache、trace，每层都强制隔离。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

identity 从认证层签发并不可由模型修改；每个 storage/tool query 由服务端注入 scope。
...
- [ ] 核心 invariant 已写成可验证条件：tenant identity 从入口贯穿 context、retrieval、storage、tool、cache、trace，每层都强制隔离。
...
[← Q085](q085.md) · [本章目录](README.md) · [Q087 →](q087.md)
