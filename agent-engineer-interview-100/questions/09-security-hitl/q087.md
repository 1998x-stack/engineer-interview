---
id: q087
title: "工具标记为 read-only、destructive、idempotent 有什么意义？"
...
# Q087 · 工具标记为 read-only、destructive、idempotent 有什么意义？
...
| 失败类别 | Security / Trust |
| 风险等级 | 中 |
| 核心 Invariant | 工具风险元数据是 policy 输入；实际权限仍由可信执行层验证。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

把工具语义机器化后，才能把可靠性策略从 prompt 转成可执行规则。
...
### 3. 还可加入 cost、latency、data sensitivity。

时延要按 critical path 分解为 budget，并把 deadline 传到每个子调用。只优化平均时延没有意义；P95/P99、queue wait 和超时后的降级策略才决定生产体验。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`注册工具时强制填写 side_effect/idempotent/risk/data_scope；Gateway 根据元数据生成默认 policy。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**工具风险元数据是 policy 输入；实际权限仍由可信执行层验证。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**工具风险元数据是 policy 输入；实际权限仍由可信执行层验证。**。
...
**回答方向**：先以“Tool metadata 是 policy 输入。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**工具风险元数据是 policy 输入；实际权限仍由可信执行层验证。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

注册工具时强制填写 side_effect/idempotent/risk/data_scope；Gateway 根据元数据生成默认 policy。
...
- [ ] 核心 invariant 已写成可验证条件：工具风险元数据是 policy 输入；实际权限仍由可信执行层验证。
...
[← Q086](q086.md) · [本章目录](README.md) · [Q088 →](q088.md)
