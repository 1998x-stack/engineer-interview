---
id: q088
title: "Agent 调第三方工具需要 Credential，Credential 应该放在哪里？"
...
# Q088 · Agent 调第三方工具需要 Credential，Credential 应该放在哪里？
...
| 失败类别 | Security / Trust |
| 风险等级 | 高 |
| 核心 Invariant | credential 存在 secrets/identity 系统，由 runtime 注入；永不进入 prompt、memory 或普通 trace。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

模型上下文可能被记录、回显或受 prompt injection 影响，不是 secret storage。
...
### 2. 短期 token 优于长期静态 key。

成本优化必须和任务成功率绑定。减少 token、换小模型、增加缓存都要看 `cost_per_success`，否则可能只是把费用转成失败率和人工升级成本。

### 3. trace 默认屏蔽 Authorization。

Trace 的目的不是“日志更多”，而是可因果定位。每个 span 应记录输入/输出引用、版本、时延、状态差异与错误码，并用同一 `trace_id` 串起模型、工具、检索和 handoff。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`使用 secret broker；tool request 带 credential_handle，Gateway 在可信边界解析并注入。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**credential 存在 secrets/identity 系统，由 runtime 注入；永不进入 prompt、memory 或普通 trace。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**credential 存在 secrets/identity 系统，由 runtime 注入；永不进入 prompt、memory 或普通 trace。**。
...
### 1. 用户自己的 OAuth token 怎么处理？

**回答方向**：先以“凭据要按 tenant/tool/scope 隔离。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. MCP server 需要凭据怎么办？

**回答方向**：先以“凭据要按 tenant/tool/scope 隔离。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

## 易错回答

> 把 API key 写进 system prompt 或 tool description。
...
更深一层看，本题的本质是保护这个系统 invariant：**credential 存在 secrets/identity 系统，由 runtime 注入；永不进入 prompt、memory 或普通 trace。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

使用 secret broker；tool request 带 credential_handle，Gateway 在可信边界解析并注入。
...
- [ ] 核心 invariant 已写成可验证条件：credential 存在 secrets/identity 系统，由 runtime 注入；永不进入 prompt、memory 或普通 trace。
...
- [Q082 · 什么叫 Least Privilege Agent？](q082.md)
...
[← Q087](q087.md) · [本章目录](README.md) · [Q089 →](q089.md)
