---
id: q089
title: "权限控制应该放在 Prompt、Agent、Tool Gateway 还是业务 API？"
...
# Q089 · 权限控制应该放在 Prompt、Agent、Tool Gateway 还是业务 API？
...
| 失败类别 | Security / Trust |
| 风险等级 | 高 |
| 核心 Invariant | 授权必须在最接近资源的可信层再次验证；上层只做预检和 UX。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

任何单层都可能被绕过或出 bug；最终资源拥有者必须独立验证。
...
### 2. 业务 API 是最终 source of truth。

把这个点放到 trust boundary：不可信输入只能提出意图，不能获得权限；最终授权发生在可审计的可信执行层。

### 3. 上层校验用于体验和减少无效调用，不取代后端授权。

把这个点放到 trust boundary：不可信输入只能提出意图，不能获得权限；最终授权发生在可审计的可信执行层。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`identity/claims 采用签名 token 透传；Gateway 与 API 都校验 scope/resource/action。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**授权必须在最接近资源的可信层再次验证；上层只做预检和 UX。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**授权必须在最接近资源的可信层再次验证；上层只做预检和 UX。**。
...
**回答方向**：先以“Defense in depth。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**授权必须在最接近资源的可信层再次验证；上层只做预检和 UX。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

identity/claims 采用签名 token 透传；Gateway 与 API 都校验 scope/resource/action。
...
- [ ] 核心 invariant 已写成可验证条件：授权必须在最接近资源的可信层再次验证；上层只做预检和 UX。
...
[← Q088](q088.md) · [本章目录](README.md) · [Q090 →](q090.md)
