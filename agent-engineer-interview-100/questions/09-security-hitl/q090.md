...
| 失败类别 | Security / Trust |
| 风险等级 | 高 |
| 核心 Invariant | Memory 写入前验证来源和权限；读取时带 trust/provenance/expiry，关键事实需重新确认。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

一次错误记忆会跨多轮、跨任务持续影响决策，因此它是持久化攻击面。
...
### 2. Agent 推断与用户明确事实要区分 trust level。

把这个点放到 trust boundary：不可信输入只能提出意图，不能获得权限；最终授权发生在可审计的可信执行层。

### 3. procedural memory 更新尤其谨慎。

长期 Memory 需要 read policy 和 write policy：什么值得写、来源是谁、何时过期、冲突怎么处理。没有 provenance/TTL 的向量记忆很容易把旧事实当成新事实。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`memory record 携带 source/trust/verified_by；高影响写入进入 quarantine 或用户确认。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**Memory 写入前验证来源和权限；读取时带 trust/provenance/expiry，关键事实需重新确认。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**Memory 写入前验证来源和权限；读取时带 trust/provenance/expiry，关键事实需重新确认。**。
...
**回答方向**：先以“外部网页内容不能直接变长期记忆。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**Memory 写入前验证来源和权限；读取时带 trust/provenance/expiry，关键事实需重新确认。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

memory record 携带 source/trust/verified_by；高影响写入进入 quarantine 或用户确认。
...
- [ ] 核心 invariant 已写成可验证条件：Memory 写入前验证来源和权限；读取时带 trust/provenance/expiry，关键事实需重新确认。
...
[← Q089](q089.md) · [本章目录](README.md) · [Q091 →](../10-performance-system-design/q091.md)
