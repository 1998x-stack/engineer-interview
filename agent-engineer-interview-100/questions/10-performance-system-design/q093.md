...
| 风险等级 | 中高 |
| 核心 Invariant | 缓存键包含所有影响语义的版本与权限维度，并有 freshness/invalidation 策略。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

缓存能降低成本和延迟，也可能制造陈旧、越权和错误复用。
...
### 3. prompt cache 与业务 cache 是不同层。

成本优化必须和任务成功率绑定。减少 token、换小模型、增加缓存都要看 `cost_per_success`，否则可能只是把费用转成失败率和人工升级成本。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`定义 cache key = input hash + version + tenant + policy context；每类缓存独立 TTL/invalidator。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**缓存键包含所有影响语义的版本与权限维度，并有 freshness/invalidation 策略。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**缓存键包含所有影响语义的版本与权限维度，并有 freshness/invalidation 策略。**。
...
**回答方向**：先以“cache invalidation 是核心。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**缓存键包含所有影响语义的版本与权限维度，并有 freshness/invalidation 策略。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

定义 cache key = input hash + version + tenant + policy context；每类缓存独立 TTL/invalidator。
...
- [ ] 核心 invariant 已写成可验证条件：缓存键包含所有影响语义的版本与权限维度，并有 freshness/invalidation 策略。
...
[← Q092](q092.md) · [本章目录](README.md) · [Q094 →](q094.md)
