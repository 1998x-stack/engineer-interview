...
| 风险等级 | 中高 |
| 核心 Invariant | 并行只在 critical path 变短且协调/限流成本可控时有收益。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

并行优化不是越多越好，而是受 Amdahl 定律和下游瓶颈限制。
...
### 3. 并行成本要计入 token 和 coordination。

能并行不等于应该并行。需要检查依赖、资源冲突、副作用顺序、下游限流和尾延迟；工程目标是缩短 critical path，而不是最大化并发数。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`压测不同 concurrency，观察 throughput/P95/error/cost，找 sweet spot 并设置动态限流。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**并行只在 critical path 变短且协调/限流成本可控时有收益。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**并行只在 critical path 变短且协调/限流成本可控时有收益。**。
...
**回答方向**：先以“关键看 critical path。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. 什么时候 speculative execution 值得？
...
更深一层看，本题的本质是保护这个系统 invariant：**并行只在 critical path 变短且协调/限流成本可控时有收益。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

压测不同 concurrency，观察 throughput/P95/error/cost，找 sweet spot 并设置动态限流。
...
- [ ] 核心 invariant 已写成可验证条件：并行只在 critical path 变短且协调/限流成本可控时有收益。
...
[← Q093](q093.md) · [本章目录](README.md) · [Q095 →](q095.md)
