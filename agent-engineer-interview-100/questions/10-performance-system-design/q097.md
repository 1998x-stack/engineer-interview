...
| 风险等级 | 中高 |
| 核心 Invariant | rate limit 通过 admission、queue、backoff、quota、fallback 保护，而不是所有请求同步重试。 |
| 面试频率 | 高频 |
| 难度 | 中 |

## 面试官在考什么

Agent 会生成突发多轮模型调用，rate limit 比单轮聊天更容易被放大。
...
### 3. 提前做 capacity planning。

把这个点落到容量模型：说明它对 throughput、P95、成本、queue 和下游限流的影响，并给出扩容/降级阈值。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`中央 model gateway 做 token-bucket、queue、retry scheduling；provider error 触发 circuit breaker。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**rate limit 通过 admission、queue、backoff、quota、fallback 保护，而不是所有请求同步重试。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**rate limit 通过 admission、queue、backoff、quota、fallback 保护，而不是所有请求同步重试。**。
...
### 1. 多个模型 provider 怎么路由？

**回答方向**：先以“尊重 retry-after。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。

### 2. fallback 模型能力不同如何保证结果？
...
更深一层看，本题的本质是保护这个系统 invariant：**rate limit 通过 admission、queue、backoff、quota、fallback 保护，而不是所有请求同步重试。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

中央 model gateway 做 token-bucket、queue、retry scheduling；provider error 触发 circuit breaker。
...
- [ ] 核心 invariant 已写成可验证条件：rate limit 通过 admission、queue、backoff、quota、fallback 保护，而不是所有请求同步重试。
...
[← Q096](q096.md) · [本章目录](README.md) · [Q098 →](q098.md)
