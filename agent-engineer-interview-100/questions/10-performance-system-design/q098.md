---
id: q098
title: "100 个用户和 100 万用户的 Agent architecture 最大区别在哪里？"
...
# Q098 · 100 个用户和 100 万用户的 Agent architecture 最大区别在哪里？
...
| 风险等级 | 中高 |
| 核心 Invariant | 百万用户需要多租户隔离、队列与背压、横向 worker、状态存储、配额、灾备和可观测性。 |
| 面试频率 | 必考 |
| 难度 | 难 |

## 面试官在考什么

规模放大后，最难的是状态、一致性、尾延迟和租户隔离，而不是 LLM API 调用本身。
...
### 2. 长任务需要 scheduler/worker fleet。
...
把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`采用 API -> scheduler -> durable queue -> workers -> gateways -> state/artifact stores；所有组件水平扩展。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**百万用户需要多租户隔离、队列与背压、横向 worker、状态存储、配额、灾备和可观测性。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**百万用户需要多租户隔离、队列与背压、横向 worker、状态存储、配额、灾备和可观测性。**。
...
**回答方向**：先以“控制面和执行面分离。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**百万用户需要多租户隔离、队列与背压、横向 worker、状态存储、配额、灾备和可观测性。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

采用 API -> scheduler -> durable queue -> workers -> gateways -> state/artifact stores；所有组件水平扩展。
...
- [ ] 核心 invariant 已写成可验证条件：百万用户需要多租户隔离、队列与背压、横向 worker、状态存储、配额、灾备和可观测性。
...
[← Q097](q097.md) · [本章目录](README.md) · [Q099 →](q099.md)
