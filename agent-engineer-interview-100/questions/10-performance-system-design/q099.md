...
| 风险等级 | 中高 |
| 核心 Invariant | 每一笔模型/token/tool/检索成本都能归因到 tenant/task/run/agent/step，并关联 task outcome。 |
| 面试频率 | 必考 |
| 难度 | 中 |

## 面试官在考什么

Agent 的成本由轨迹决定，同一用户请求可能触发数量差异很大的内部调用。
...
### 2. 关联 task success 计算 cost per successful task。

把这个点落到容量模型：说明它对 throughput、P95、成本、queue 和下游限流的影响，并给出扩容/降级阈值。

### 3. 对多 Agent 汇总 parent-child cost。

把这个点落到容量模型：说明它对 throughput、P95、成本、queue 和下游限流的影响，并给出扩容/降级阈值。

### 从机制到系统

把上面几点合起来，本题不能只用一个局部 API 或 Prompt 技巧解决。需要把 **`每个 span 计算 cost，聚合到 run；设置 per-task budget，超预算触发 smaller model/stop/HITL。`** 变成 runtime 中可测试的状态迁移，并给关键 transition 设计失败分支。
## 3 分钟专业展开

先把问题抽象成一个工程约束：**每一笔模型/token/tool/检索成本都能归因到 tenant/task/run/agent/step，并关联 task outcome。**。然后按下面四层回答：
...
本题最重要的 Preserve 对象是：**每一笔模型/token/tool/检索成本都能归因到 tenant/task/run/agent/step，并关联 task outcome。**。
...
**回答方向**：先以“把 retry cost 单独统计。”为主结论，再补充状态所有权、失败窗口和验证指标。回答必须给出一个明确边界：什么情况下采用该方案，什么情况下应 fail-safe / clarify / HITL。
...
更深一层看，本题的本质是保护这个系统 invariant：**每一笔模型/token/tool/检索成本都能归因到 tenant/task/run/agent/step，并关联 task outcome。**。Agent 引入的是概率决策，因此工程层需要把不可违反的规则外置为 deterministic guardrails/state machines，并给非确定路径准备恢复机制。

## Know-How

每个 span 计算 cost，聚合到 run；设置 per-task budget，超预算触发 smaller model/stop/HITL。
...
- [ ] 核心 invariant 已写成可验证条件：每一笔模型/token/tool/检索成本都能归因到 tenant/task/run/agent/step，并关联 task outcome。
...
[← Q098](q098.md) · [本章目录](README.md) · [Q100 →](q100.md)
