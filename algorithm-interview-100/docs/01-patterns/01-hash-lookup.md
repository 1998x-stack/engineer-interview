...
**识别信号：** 查补数、去重、映射
**核心原则：** 把历史值映射为 O(1) 平均查询，重点检查查询/写入顺序。
...
识别信号：**查补数、去重、映射**。但不要只做关键词匹配：真正判断依据是题目是否存在与该模式一致的**信息复用方式或单调结构**。

### 不变量优先

> 把历史值映射为 O(1) 平均查询，重点检查查询/写入顺序。

面试里最重要的不是说出 “我用 Hash Lookup”，而是把容器中的每个元素、指针边界或 DP 状态的语义说精确。
