...
**识别信号：** 连续区间、计数、历史状态
**核心原则：** 把区间条件改写成两个前缀状态的关系；计数存频次，最长通常存最早位置。
...
识别信号：**连续区间、计数、历史状态**。但不要只做关键词匹配：真正判断依据是题目是否存在与该模式一致的**信息复用方式或单调结构**。

### 不变量优先

> 把区间条件改写成两个前缀状态的关系；计数存频次，最长通常存最早位置。

面试里最重要的不是说出 “我用 Prefix State”，而是把容器中的每个元素、指针边界或 DP 状态的语义说精确。
...
- [006. LC525 连续数组](../../problems/01-array-hash-prefix/006-contiguous-array.md) - `Prefix State Compression`
