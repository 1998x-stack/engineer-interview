...
**识别信号：** O(1) 空间、值域映射到下标
**核心原则：** 复用输入数组作为状态存储，必须证明覆盖旧值不会破坏尚未读取的信息。
...
识别信号：**O(1) 空间、值域映射到下标**。但不要只做关键词匹配：真正判断依据是题目是否存在与该模式一致的**信息复用方式或单调结构**。

### 不变量优先

> 复用输入数组作为状态存储，必须证明覆盖旧值不会破坏尚未读取的信息。

面试里最重要的不是说出 “我用 In-place Mapping”，而是把容器中的每个元素、指针边界或 DP 状态的语义说精确。
...
- [008. LC73 矩阵置零](../../problems/01-array-hash-prefix/008-set-matrix-zeroes.md) - `State Reuse`
