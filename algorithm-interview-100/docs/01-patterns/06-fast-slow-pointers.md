...
**识别信号：** 链表环、中点、入口
**核心原则：** 通过不同速度建立相遇关系；入口题需要能解释距离方程而非只背代码。
...
识别信号：**链表环、中点、入口**。但不要只做关键词匹配：真正判断依据是题目是否存在与该模式一致的**信息复用方式或单调结构**。

### 不变量优先

> 通过不同速度建立相遇关系；入口题需要能解释距离方程而非只背代码。

面试里最重要的不是说出 “我用 Fast / Slow Pointers”，而是把容器中的每个元素、指针边界或 DP 状态的语义说精确。
