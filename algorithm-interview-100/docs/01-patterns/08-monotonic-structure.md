...
**识别信号：** 右侧第一个更大/小、窗口极值
**核心原则：** 结构中只保留未来仍可能成为答案的候选；每个元素通常至多入/出一次。
...
识别信号：**右侧第一个更大/小、窗口极值**。但不要只做关键词匹配：真正判断依据是题目是否存在与该模式一致的**信息复用方式或单调结构**。

### 不变量优先

> 结构中只保留未来仍可能成为答案的候选；每个元素通常至多入/出一次。

面试里最重要的不是说出 “我用 Monotonic Stack / Deque”，而是把容器中的每个元素、指针边界或 DP 状态的语义说精确。
