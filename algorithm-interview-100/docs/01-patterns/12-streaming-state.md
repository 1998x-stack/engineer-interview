...
识别信号：**在线插入、随时查询**。但不要只做关键词匹配：真正判断依据是题目是否存在与该模式一致的**信息复用方式或单调结构**。

### 不变量优先

> 每次更新后立即恢复不变量，使查询无需回看全部历史。

面试里最重要的不是说出 “我用 Streaming State”，而是把容器中的每个元素、指针边界或 DP 状态的语义说精确。
...
- [050. LC346 数据流中的移动平均值](../../problems/06-heap-streaming-sampling/050-moving-average-from-data-stream.md) - `Fixed-size Stream State`
