...
**识别信号：** 直径、路径、全局答案
**核心原则：** 区分“向父节点返回的单边贡献”和“在当前节点结算的全局答案”。
...
识别信号：**直径、路径、全局答案**。但不要只做关键词匹配：真正判断依据是题目是否存在与该模式一致的**信息复用方式或单调结构**。

### 不变量优先

> 区分“向父节点返回的单边贡献”和“在当前节点结算的全局答案”。

面试里最重要的不是说出 “我用 Tree DP”，而是把容器中的每个元素、指针边界或 DP 状态的语义说精确。
