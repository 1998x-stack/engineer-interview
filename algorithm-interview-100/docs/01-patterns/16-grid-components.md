...
**识别信号：** 岛屿、连通块、面积
**核心原则：** visited/原地标记保证每格只属于一个搜索过程。
...
识别信号：**岛屿、连通块、面积**。但不要只做关键词匹配：真正判断依据是题目是否存在与该模式一致的**信息复用方式或单调结构**。

### 不变量优先

> visited/原地标记保证每格只属于一个搜索过程。

面试里最重要的不是说出 “我用 Grid Connected Components”，而是把容器中的每个元素、指针边界或 DP 状态的语义说精确。
