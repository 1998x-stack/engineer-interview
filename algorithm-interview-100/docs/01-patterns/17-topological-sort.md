...
识别信号：**先修、依赖、DAG**。但不要只做关键词匹配：真正判断依据是题目是否存在与该模式一致的**信息复用方式或单调结构**。

### 不变量优先

> 入度为 0 才可进入队列；处理数不足说明存在环。

面试里最重要的不是说出 “我用 Topological Sort”，而是把容器中的每个元素、指针边界或 DP 状态的语义说精确。
