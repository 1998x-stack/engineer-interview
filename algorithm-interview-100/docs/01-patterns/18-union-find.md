...
**识别信号：** 集合合并、动态连通、冗余边
**核心原则：** find 给集合代表；路径压缩与按秩/大小合并控制复杂度。
...
识别信号：**集合合并、动态连通、冗余边**。但不要只做关键词匹配：真正判断依据是题目是否存在与该模式一致的**信息复用方式或单调结构**。

### 不变量优先

> find 给集合代表；路径压缩与按秩/大小合并控制复杂度。

面试里最重要的不是说出 “我用 Union-Find”，而是把容器中的每个元素、指针边界或 DP 状态的语义说精确。
...
- [075. LC684 冗余连接](../../problems/08-graph/075-redundant-connection.md) - `Union-Find Cycle Detection`
