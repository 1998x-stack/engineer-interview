...
识别信号：**前缀、多词、自动补全**。但不要只做关键词匹配：真正判断依据是题目是否存在与该模式一致的**信息复用方式或单调结构**。

### 不变量优先

> 路径代表前缀，终止标记区分完整单词与仅前缀。

面试里最重要的不是说出 “我用 Trie”，而是把容器中的每个元素、指针边界或 DP 状态的语义说精确。
