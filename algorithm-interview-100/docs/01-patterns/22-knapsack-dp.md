...
**识别信号：** 目标和、每个物品一次或无限次
**核心原则：** 明确 0/1 与完全背包的遍历方向，防止同一物品被错误复用。
...
识别信号：**目标和、每个物品一次或无限次**。但不要只做关键词匹配：真正判断依据是题目是否存在与该模式一致的**信息复用方式或单调结构**。

### 不变量优先

> 明确 0/1 与完全背包的遍历方向，防止同一物品被错误复用。

面试里最重要的不是说出 “我用 Knapsack DP”，而是把容器中的每个元素、指针边界或 DP 状态的语义说精确。
