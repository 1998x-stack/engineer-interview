---
id: "Q002"
title: "反向传播的本质是什么？Autograd 在做什么？"
chapter: 1
...
考察链式法则、计算图和 reverse-mode AD。

**高质量回答标准：** 能给出定义和一个最小公式；能解释梯度/表达能力的因果关系；能指出一个失败边界。

## 一句话结论

反向传播本质是链式法则在计算图上的高效复用。

## 60–90 秒面试回答

反向传播本质是链式法则在计算图上的高效复用。Forward 保存必要的中间量并建立依赖关系；Backward 从标量损失出发，按拓扑逆序计算 vector-Jacobian product，并把来自多条路径的梯度累加。
dL/dW = (dL/da) · (da/dz) · (dz/dW)

## 深度解析

- 对标量损失 L，参数很多而输出少，reverse-mode AD 比 forward-mode 更合适。
- 节点有多条后继路径时，梯度必须相加，这是计算图“累计梯度”的来源。
- PyTorch 的 backward() 默认把梯度累加到 .grad，因此训练循环中通常要 zero_grad。



## 数学、Shape 与复杂度

对复合函数 $L=f_n\circ\cdots\circ f_1(x)$，反向传播重复应用链式法则。对标量损失，reverse-mode AD 的优势是一次反向即可得到大量参数的梯度：

$$
\frac{\partial L}{\partial x}=\frac{\partial L}{\partial h_n}\prod_{i=n}^{1}\frac{\partial h_i}{\partial h_{i-1}}.
$$
...
对一个 2 层 MLP 使用 `torch.autograd.grad` 与中心有限差分对同一参数做 gradient check，并打印相对误差。
...
- **易错：** 把 Autograd 说成数值差分。
- **易错：** 忽略广播、共享参数或多路径导致的梯度求和。
...
反向模式 AD 对标量损失尤其高效：一次反传可得到损失对大量参数的梯度；核心对象是 vector-Jacobian product，而不是显式构造巨大 Jacobian。

### 工程与实验抓手

用 `retain_grad()`、hook 与 `torch.autograd.grad` 检查中间节点梯度；对一个两层 MLP 用有限差分做 gradient check。

### 失败边界 / 反例

in-place 修改、detach、跨设备/跨 dtype 操作都可能切断或破坏计算图；‘有 grad 属性’不等于梯度路径正确。

### 白板专项练习

手推 `y=(Wx+b)^2` 的 VJP，再用 PyTorch 对比数值；解释为什么反向传播的内存瓶颈常来自保存 activation。
...
- **本题特定证据：** 用 `retain_grad()`、hook 与 `torch.autograd.grad` 检查中间节点梯度；对一个两层 MLP 用有限差分做 gradient check。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**in-place 修改、detach、跨设备/跨 dtype 操作都可能切断或破坏计算图；‘有 grad 属性’不等于梯度路径正确。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
