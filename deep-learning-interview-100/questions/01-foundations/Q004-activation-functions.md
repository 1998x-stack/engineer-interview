---
id: "Q004"
title: "Sigmoid、Tanh、ReLU、GELU、SiLU 如何比较？"
chapter: 1
...
# Q004 · Sigmoid、Tanh、ReLU、GELU、SiLU 如何比较？
...
考察激活函数的梯度、饱和性与现代网络选择。

**高质量回答标准：** 能给出定义和一个最小公式；能解释梯度/表达能力的因果关系；能指出一个失败边界。

## 一句话结论

Sigmoid/Tanh 在大绝对值区域会饱和，深层传播时梯度易衰减；ReLU 简单高效但存在 dead ReLU；GELU 是平滑的概率式门控，Transformer 常用；SiLU/Swish 是 x·sigmoid(x)，在现代 LLM/视觉网络中也常见。
...
- Sigmoid 输出非零中心，可能让优化方向产生偏置。
- ReLU 正半轴梯度为 1，缓解饱和，但负半轴长期为 0 时神经元可能死亡。
- GELU/SiLU 保留负值的小幅响应，梯度更平滑。
...
采样输入网格，画各激活值与导数；在同一深 MLP 上记录 dead-unit ratio 与 grad RMS。
...
- LeakyReLU 为什么能缓解 dead ReLU？
- GELU 和 ReLU 的计算代价差多少？
- 为什么门控 FFN 常用 SiLU/SwiGLU？
...
- **易错：** 把“GELU 一定优于 ReLU”说成普遍定律。
- **易错：** 只比较函数值，不比较梯度形状与任务环境。
...
比较激活函数不能只看曲线，还要看导数、输出均值、饱和区、计算成本与硬件 kernel。GELU/SiLU 的平滑门控与 ReLU 的硬阈值是关键差异。

### 工程与实验抓手

统计同一 MLP 在 ReLU/GELU/SiLU 下的 activation mean/std、dead ratio、gradient norm；保持初始化与 seed 不变。

### 失败边界 / 反例

Sigmoid 仍适合作为门控或概率输出，不能因为深层 hidden layer 少用就断言它‘过时’。

### 白板专项练习

给出输入从 -10 到 10 的导数曲线，解释每种激活在大正/大负区间的梯度行为。
...
- **本题特定证据：** 统计同一 MLP 在 ReLU/GELU/SiLU 下的 activation mean/std、dead ratio、gradient norm；保持初始化与 seed 不变。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**Sigmoid 仍适合作为门控或概率输出，不能因为深层 hidden layer 少用就断言它‘过时’。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
