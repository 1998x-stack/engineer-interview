---
id: "Q009"
title: "MSE、BCE、Cross Entropy 分别用于什么场景？"
chapter: 1
...
# Q009 · MSE、BCE、Cross Entropy 分别用于什么场景？
...
MSE 常用于连续值回归；BCE 用于二分类或多标签分类，每个标签独立建模；Softmax Cross Entropy 用于互斥多分类。

## 60–90 秒面试回答

MSE 常用于连续值回归；BCE 用于二分类或多标签分类，每个标签独立建模；Softmax Cross Entropy 用于互斥多分类。关键不是“记名字”，而是先确定目标变量的概率结构。

## 深度解析

- 二分类可用单 logit + BCEWithLogitsLoss。
- 多标签每一类独立 sigmoid；多分类各类概率和为 1。
- BCEWithLogitsLoss 比 sigmoid 后再 BCE 更数值稳定。
...
构造 Gaussian regression、binary multi-label、categorical 三个 toy task，对应验证 MSE/BCE/CE。
...
- **易错：** 混淆 multi-class 与 multi-label。
- **易错：** 在框架 loss 前重复做 sigmoid/softmax。
...
Loss 的选择应由数据生成假设与输出结构决定：MSE 对应高斯噪声的负对数似然，BCE 对应独立 Bernoulli，CE 对应 categorical。

### 工程与实验抓手

对同一错误幅度比较 MSE、MAE、Huber 的梯度；对多标签任务验证 sigmoid+BCE 与 softmax+CE 的行为差异。

### 失败边界 / 反例

不要根据‘回归/分类’标签机械选择 loss；例如排序、metric learning、分割、检测常需要结构化或组合目标。

### 白板专项练习

从最大似然角度分别写出 Gaussian/Bernoulli/Categorical 的 NLL，并对应到常见 loss。
...
- **本题特定证据：** 对同一错误幅度比较 MSE、MAE、Huber 的梯度；对多标签任务验证 sigmoid+BCE 与 softmax+CE 的行为差异。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**不要根据‘回归/分类’标签机械选择 loss；例如排序、metric learning、分割、检测常需要结构化或组合目标。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
