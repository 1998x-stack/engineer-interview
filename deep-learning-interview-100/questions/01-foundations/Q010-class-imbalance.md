...
先确认业务代价，再从采样、损失、表征、阈值与指标五层处理。

## 60–90 秒面试回答

先确认业务代价，再从采样、损失、表征、阈值与指标五层处理。可做重采样、class weight、Focal Loss、hard negative mining，并根据 precision/recall 成本选择阈值。极不平衡任务不要只看 accuracy，通常 PR-AUC 比 ROC-AUC 更直观。

## 深度解析

- 负采样会改变训练先验，线上概率可能需要校准。
- 阈值不是固定 0.5，应基于业务成本和验证集选择。
- 当正类极稀少时，PR 曲线更直接展示正类识别质量。
...
在 1:1000 数据上比较 Accuracy、ROC-AUC、PR-AUC、F1 与固定 Recall 下 Precision，并做 threshold sweep。
...
- ROC-AUC 为什么在极不平衡时可能显得“虚高”？
- Focal Loss 的 γ 有什么作用？
- 重采样后如何做概率校准？
...
类别不平衡需区分先验不平衡、采样不平衡、难例不平衡与代价不对称；解决手段分别作用于 data、loss、sampling、threshold、metric。

### 工程与实验抓手

用同一模型比较 class weight、focal loss、balanced sampler、threshold tuning；报告 PR-AUC 与固定 recall 下 precision。

### 失败边界 / 反例

ROC-AUC 在极端负样本占比时可能看起来很好但业务 precision 很差；重采样后概率还可能需要校准。

### 白板专项练习

给出 1:1000 数据，解释为什么 accuracy=99.9% 仍无意义，并设计离线评估表。
...
- **本题特定证据：** 用同一模型比较 class weight、focal loss、balanced sampler、threshold tuning；报告 PR-AUC 与固定 recall 下 precision。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**ROC-AUC 在极端负样本占比时可能看起来很好但业务 precision 很差；重采样后概率还可能需要校准。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
