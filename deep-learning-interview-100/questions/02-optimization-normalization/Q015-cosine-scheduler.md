---
id: "Q015"
title: "Cosine Learning Rate Scheduler 为什么常用？"
chapter: 2
...
# Q015 · Cosine Learning Rate Scheduler 为什么常用？
...
Cosine decay 让学习率从高到低平滑下降，前期保留探索和快速学习，后期逐渐减小步长以精细收敛；相比阶梯下降，它没有突变，常与 warmup 组合。
...
- 周期末可保留 η_min 而非降到 0。
- Cosine 本身不是魔法，关键是合适的峰值 LR、总步数和 warmup。
- Cosine restart 是另一类周期性策略。
...
画 constant/linear/cosine 的 LR 与 update norm 曲线；比较相同 area-under-LR 的实验。
...
- Linear decay 与 cosine 如何选？
- 训练步数变化时 scheduler 怎么同步调整？
...
Cosine decay 的优势是平滑、少拐点、后期逐渐减小更新；真正影响结果的还包括 min LR、warmup、总 token 与是否 restart。
...
不要把 scheduler 与 optimizer 独立讨论：AdamW 的 effective update 并不等于表面 LR。

### 白板专项练习

写出 cosine 公式，计算训练 25%、50%、75% 时的 LR 比例。
...
- **本题特定证据：** 画 constant/linear/cosine 的 LR 与 update norm 曲线；比较相同 area-under-LR 的实验。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**不要把 scheduler 与 optimizer 独立讨论：AdamW 的 effective update 并不等于表面 LR。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
