---
id: "Q020"
title: "Dropout 为什么能防止过拟合？训练和推理如何处理？"
chapter: 2
...
# Q020 · Dropout 为什么能防止过拟合？训练和推理如何处理？
...
训练时随机屏蔽部分激活，相当于不断扰动可用子网络，抑制神经元之间过强的协同适配；inverted dropout会在训练时按 1/(1-p) 缩放，使推理时可以直接关闭 dropout 而保持激活期望一致。
...
- Dropout 主要是训练期随机性，不是推理时 ensemble 多次采样的必要条件。
- 在大模型里 dropout 比早期 CNN/MLP 的使用更依赖数据规模和正则化需求。
- MC Dropout 属于另一个不确定性估计用法。
...
验证 inverted dropout：训练时 `E[y]≈x`，eval 时不再随机 mask；测试 seed 与 train/eval 切换。
...
- 为什么 inverted dropout 推理不用缩放？
- Dropout 和 BatchNorm 混用有什么注意点？
...
Dropout 可理解为训练时随机采样子网络并用期望尺度校正；实际作用与 batch size、Norm、数据规模和模型规模耦合。
...
在大规模 LLM 预训练中 dropout 可能很低甚至为 0；不能把它当成必需的泛化手段。

### 白板专项练习

推导 keep probability=p 时为何训练输出通常除以 p。
...
- **本题特定证据：** 验证 inverted dropout：训练时 `E[y]≈x`，eval 时不再随机 mask；测试 seed 与 train/eval 切换。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**在大规模 LLM 预训练中 dropout 可能很低甚至为 0；不能把它当成必需的泛化手段。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
