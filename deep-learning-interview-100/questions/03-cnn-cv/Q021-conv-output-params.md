...
卷积输出尺寸由输入 H、padding P、dilation D、kernel K、stride S 决定；参数量与空间尺寸无关，只取决于输入/输出通道和卷积核大小。
...
- 二维卷积输出 H_out=floor((H+2P-D(K-1)-1)/S+1)。
- 参数量=C_out·C_in/groups·K_h·K_w，若有 bias 再加 C_out。
- groups/depthwise 会改变参数量。
...
标准卷积参数量（含 bias）为 $C_{out}C_{in}K_hK_w+C_{out}$。

## 工程实现 / PyTorch 验证

### 推荐验证协议

写一个函数随机生成 H/W/K/S/P/D，和 `nn.Conv2d` 输出 shape 自动对拍；再计算 MACs。
...
- dilation 为什么会增大有效感受野？
- same padding 如何计算？
- depthwise 卷积的 groups 等于多少？
...
卷积输出尺寸应同时考虑 dilation；参数量与空间分辨率无关，而 FLOPs 与输出 H×W 成正比。
...
常见错误是把 padding 或 dilation 公式漏掉，以及把 groups 卷积参数量仍按普通卷积计算。

### 白板专项练习

给定 `B=8,Cin=64,H=W=56,Cout=128,K=3,S=2,P=1`，现场算输出、参数量和近似 MACs。
...
- **本题特定证据：** 写一个函数随机生成 H/W/K/S/P/D，和 `nn.Conv2d` 输出 shape 自动对拍；再计算 MACs。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**常见错误是把 padding 或 dilation 公式漏掉，以及把 groups 卷积参数量仍按普通卷积计算。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
