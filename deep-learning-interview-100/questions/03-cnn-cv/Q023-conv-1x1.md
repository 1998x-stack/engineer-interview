...
1×1 卷积不扩大空间邻域，但会在每个 spatial location 上对通道向量做线性映射，因此可用于通道升/降维、 特征融合和 bottleneck，显著减少后续大卷积计算量。

## 60–90 秒面试回答

1×1 卷积不扩大空间邻域，但会在每个 spatial location 上对通道向量做线性映射，因此可用于通道升/降维、
特征融合和 bottleneck，显著减少后续大卷积计算量。

## 深度解析

- 对每个像素位置，它等价于共享参数的全连接层。
- ResNet bottleneck 常用 1×1 降维→3×3→1×1 升维。
- 配合非线性后可增加网络深度和表达能力。
...
比较 3×3 Cin→Cout 与先 1×1 降维再 3×3 的参数/FLOPs，给出实际数字。
...
1×1 卷积本质是每个 spatial location 上共享权重的 channel mixing，可用于 bottleneck、升降维、跨通道投影。
...
1×1 不直接扩大空间感受野，但堆在已有空间算子后可混合来自不同感受野的通道特征。

### 白板专项练习

计算 `256→64→256` bottleneck 与直接 `256→256 3×3` 的参数量比例。
...
- **本题特定证据：** 比较 3×3 Cin→Cout 与先 1×1 降维再 3×3 的参数/FLOPs，给出实际数字。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**1×1 不直接扩大空间感受野，但堆在已有空间算子后可混合来自不同感受野的通道特征。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
