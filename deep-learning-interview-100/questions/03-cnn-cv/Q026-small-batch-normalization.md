---
id: "Q026"
title: "Batch 很小时为什么 BatchNorm 容易失效？有哪些替代？"
chapter: 3
...
# Q026 · Batch 很小时为什么 BatchNorm 容易失效？有哪些替代？
...
BN 依赖 mini-batch 对均值和方差的估计；batch 很小时估计噪声大，且不同 GPU 的局部 batch 更小。

## 60–90 秒面试回答

BN 依赖 mini-batch 对均值和方差的估计；batch 很小时估计噪声大，且不同 GPU 的局部 batch 更小。可用SyncBN 汇总多卡统计，或改用 GroupNorm/LayerNorm，也可以冻结预训练 BN。

## 深度解析

- SyncBN 提高统计稳定性但增加通信。
- GroupNorm 按通道组统计，与 batch size 基本无关。
- 目标检测常受高分辨率限制，batch 小是常见场景。
...
固定全局 batch，改变每卡 batch 与是否 SyncBN，比较输出统计和吞吐通信成本。
...
- Frozen BN 的 running stats 从哪里来？
- GroupNorm 的 group 数怎么选？
...
- **易错：** 认为“只要 batch=1 BN 就完全不能用”，实际推理 eval 模式可用 running stats。
...
小 batch BN 的核心问题是样本统计估计方差大；检测/分割因高分辨率导致 per-GPU batch 小，因此 GN/FrozenBN/SyncBN 常见。
...
SyncBN 解决统计样本不足但引入跨卡通信；FrozenBN 适合迁移学习但可能受 domain shift 影响。

### 白板专项练习

给出 batch=1 的极端例子，解释 BN 统计为什么不可靠以及 GN 为什么不依赖 batch。
...
- **本题特定证据：** 固定全局 batch，改变每卡 batch 与是否 SyncBN，比较输出统计和吞吐通信成本。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**SyncBN 解决统计样本不足但引入跨卡通信；FrozenBN 适合迁移学习但可能受 domain shift 影响。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
