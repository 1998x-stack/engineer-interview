---
id: "Q035"
title: "Encoder-only、Decoder-only、Encoder-Decoder 如何选择？"
chapter: 4
...
# Q035 · Encoder-only、Decoder-only、Encoder-Decoder 如何选择？
...
Encoder-only 擅长对完整输入做双向表示；Decoder-only 用 causal attention 统一为 next-token prediction，最自然地支持开放生成；Encoder-Decoder 将输入编码与输出生成分离，适合翻译、摘要等显式seq2seq。
...
- 架构选择与任务、训练数据、部署方式相关，不是能力排名。
- Decoder-only 的统一接口和 scaling 便利推动了 LLM 主流化。
- Encoder 模型在 embedding/retrieval 场景仍非常重要。
...
画三种 attention mask / cross-attention 数据流，并比较同 token budget 的训练/推理缓存结构。
...
- T5 为什么属于 encoder-decoder？
- 为什么 decoder-only 也能做分类？
...
- **易错：** 认为 decoder-only 可以无代价替代所有 encoder 模型。
...
架构选择本质由信息流约束决定：Encoder 双向表征；Decoder 因 causal mask 适合生成；Encoder-Decoder 用 cross-attention 建模输入输出两套序列。
...
不要用‘理解 vs 生成’一刀切：decoder-only 也能做分类/表征，encoder 也可配 decoder 完成生成。

### 白板专项练习

给搜索排序、机器翻译、对话生成三个任务，说明为什么选对应架构并列出替代方案。
...
- **本题特定证据：** 画三种 attention mask / cross-attention 数据流，并比较同 token budget 的训练/推理缓存结构。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**不要用‘理解 vs 生成’一刀切：decoder-only 也能做分类/表征，encoder 也可配 decoder 完成生成。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
