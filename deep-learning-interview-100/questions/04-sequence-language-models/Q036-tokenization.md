---
id: "Q036"
title: "BPE、WordPiece、SentencePiece 为什么存在？"
chapter: 4
...
# Q036 · BPE、WordPiece、SentencePiece 为什么存在？
...
考察 Tokenization 与词表权衡。

**高质量回答标准：** 能区分训练与自回归推理；能正确处理 tokenization、mask、shift 等序列细节。

## 一句话结论

词级词表容易 OOV 且规模巨大，字符级序列又太长；subword 在两者之间折中，把常见词或词根保留为较长token，把罕见词拆成较短单位。

## 60–90 秒面试回答

词级词表容易 OOV 且规模巨大，字符级序列又太长；subword 在两者之间折中，把常见词或词根保留为较长token，把罕见词拆成较短单位。BPE 基于频繁 pair merge，WordPiece 使用不同的合并准则，SentencePiece 更像直接在原始字符串上训练 tokenizer 的框架。

## 深度解析

- Tokenizer 会影响序列长度、跨语言公平性和推理成本。
- 数字、代码、中文和低资源语言的切分特征差异明显。
- 词表扩大可缩短序列，但增大 embedding/output matrix。
...
训练一个极小 BPE tokenizer，逐轮打印 merge；比较中文、英文、代码文本的 token fertility。
...
Subword tokenizer 在词表大小与序列长度之间折中；BPE/WordPiece 的训练准则不同，SentencePiece 还强调从 raw text 直接建模并可处理空格。
...
tokenizer 是模型接口的一部分：改词表会影响 embedding、长度分布、成本和兼容性，不能随意替换。

### 白板专项练习

对一个罕见词手算 3 轮 BPE merge，并解释 byte-level fallback 对 OOV 的意义。
...
- **本题特定证据：** 训练一个极小 BPE tokenizer，逐轮打印 merge；比较中文、英文、代码文本的 token fertility。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**tokenizer 是模型接口的一部分：改词表会影响 embedding、长度分布、成本和兼容性，不能随意替换。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
...
- [Q037 · Padding Token 为什么通常不参与 Loss？](../04-sequence-language-models/Q037-padding-loss-mask.md)
- [Q034 · BERT 的预训练目标是什么？为什么它适合理解类任务？](../04-sequence-language-models/Q034-bert-pretraining.md)
- [Q038 · Causal Mask 与 Padding Mask 有什么区别？](../04-sequence-language-models/Q038-causal-padding-mask.md)
