---
id: "Q037"
title: "Padding Token 为什么通常不参与 Loss？"
chapter: 4
...
# Q037 · Padding Token 为什么通常不参与 Loss？
...
不同长度序列组成 batch 时会补 PAD；PAD 不是真实监督信号，如果参与交叉熵，模型会被大量无意义 token牵引。

## 60–90 秒面试回答

不同长度序列组成 batch 时会补 PAD；PAD 不是真实监督信号，如果参与交叉熵，模型会被大量无意义 token牵引。通常把对应 label 设为 ignore_index（如 -100），并在 attention 中也用 padding mask 防止无效位置被关注。

## 深度解析

- loss mask 和 attention mask 是两件不同的事。
- Instruction tuning 还常对 user prompt 部分屏蔽 loss，只监督 assistant answer。
- 右 padding/左 padding 会影响位置与生成实现。
...
构造左右 padding 两个 batch，逐 token 打印 input/labels/loss mask，确认 shift 后监督位置正确。
...
- 为什么 prompt token 也可能 mask loss？
- pad_token_id 和 eos_token_id 能否设相同？
...
- **易错：** 只处理 loss，不处理 attention mask。
...
Causal LM 的 label shift 与 padding mask 是两件事：padding 位置通常设 ignore_index；是否 mask prompt 取决于训练目标。
...
只改 attention_mask 不等于 loss 会忽略 pad；必须理解模型 forward 内部如何 shift labels。

### 白板专项练习

给一段 prompt+assistant token 序列，标出每个位置的 label 与 -100。
...
- **本题特定证据：** 构造左右 padding 两个 batch，逐 token 打印 input/labels/loss mask，确认 shift 后监督位置正确。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**只改 attention_mask 不等于 loss 会忽略 pad；必须理解模型 forward 内部如何 shift labels。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
