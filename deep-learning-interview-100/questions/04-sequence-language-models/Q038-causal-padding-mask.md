---
id: "Q038"
title: "Causal Mask 与 Padding Mask 有什么区别？"
chapter: 4
...
# Q038 · Causal Mask 与 Padding Mask 有什么区别？
...
考察 Attention mask 语义。

**高质量回答标准：** 能区分训练与自回归推理；能正确处理 tokenization、mask、shift 等序列细节。

## 一句话结论

Causal mask 限制位置 i 只能看到 j≤i，防止自回归训练偷看未来；padding mask 屏蔽补齐位置，避免无效token 参与注意力。

## 60–90 秒面试回答

Causal mask 限制位置 i 只能看到 j≤i，防止自回归训练偷看未来；padding mask 屏蔽补齐位置，避免无效token 参与注意力。实际实现常把两者组合为一个 additive/bool mask。
M_ij=0 (j≤i), -∞ (j>i)

## 深度解析

- 加性 mask 常对禁止位置加一个极小值，使 softmax 后近似 0。
- 训练时 causal mask 让所有位置可并行计算，但每个位置只用左上下文。
- FlashAttention 等 kernel 对 mask 支持方式可能不同。



## 数学、Shape 与复杂度

Causal mask 常以加性形式进入 attention logits：未来位置填 $-\infty$，softmax 后概率为 0；padding mask 则屏蔽无效 token。两者语义不同但可组合。

## 工程实现 / PyTorch 验证

### 推荐验证协议

对 batch 内不同长度序列可视化最终 mask matrix；测试全 mask 行避免 softmax NaN。
...
- 为什么 causal LM 训练仍能并行？
- mask 放在 softmax 前还是后？
...
Padding mask 屏蔽无效 token；causal mask 屏蔽未来 token。二者常通过逻辑与合并并广播到 `[B,1,Tq,Tk]` 或 kernel 特定格式。
...
左 padding + KV cache/position_ids 是常见坑，不能只看二维 attention_mask。

### 白板专项练习

手画 T=5 的下三角 causal mask，并叠加一个长度=3 的 padding mask。
...
- **本题特定证据：** 对 batch 内不同长度序列可视化最终 mask matrix；测试全 mask 行避免 softmax NaN。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**左 padding + KV cache/position_ids 是常见坑，不能只看二维 attention_mask。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
