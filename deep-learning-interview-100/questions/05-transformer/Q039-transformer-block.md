---
id: "Q039"
title: "从头讲一个现代 Transformer Block。"
chapter: 5
...
# Q039 · 从头讲一个现代 Transformer Block。
...
现代 decoder block 通常是 Pre-Norm：输入先 RMSNorm/LayerNorm，经过 causal self-attention 后与残差相加；再 norm，经过 FFN/SwiGLU 后再残差相加。

## 60–90 秒面试回答

现代 decoder block 通常是 Pre-Norm：输入先 RMSNorm/LayerNorm，经过 causal self-attention 后与残差相加；再 norm，经过 FFN/SwiGLU 后再残差相加。多层堆叠后再做 final norm 和输出头。

## 深度解析

- 原始 Transformer 使用 Post-Norm，现代 LLM 大量使用 Pre-Norm。
- Attention 负责 token mixing，FFN 负责每个 token 的 channel/feature mixing。
- Residual、Normalization、position encoding 与 attention kernel 是理解整个模型的四个轴。
...
打印一个真实开源模型 config，把 textbook Transformer 与实际 block 参数逐项对照。
...
- 画出 QKV、RoPE、GQA 在 block 中的位置。
- 为什么 FFN 隐藏维度通常大于 d_model？
...
- **易错：** 只会背“Attention+FFN”但画不出 tensor flow。
...
现代 decoder block 应说清 Norm→Attention→Residual→Norm→FFN/MoE→Residual，并注明具体模型可能使用 RMSNorm、SwiGLU、RoPE、GQA。
...
不要把原始 2017 Transformer 的 Post-Norm、sinusoidal PE 当成所有现代 LLM 的默认实现。

### 白板专项练习

给定 D、H、FFN hidden，估算一层 attention/FFN 参数量并判断哪部分更大。
...
- **本题特定证据：** 打印一个真实开源模型 config，把 textbook Transformer 与实际 block 参数逐项对照。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**不要把原始 2017 Transformer 的 Post-Norm、sinusoidal PE 当成所有现代 LLM 的默认实现。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
