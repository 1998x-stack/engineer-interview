...
若 q、k 各维近似独立、零均值、单位方差，则点积 q·k 是 d_k 个乘积之和，方差随 d_k 增长。

## 60–90 秒面试回答

若 q、k 各维近似独立、零均值、单位方差，则点积 q·k 是 d_k 个乘积之和，方差随 d_k 增长。logit 过大时softmax 进入饱和区、梯度很小；除以 √d_k 使尺度大致恢复到 O(1)。
Var(q·k)≈d_k ⇒ std≈√d_k

## 深度解析

- 这是 variance scaling，不是单纯“防止数值溢出”。
- 如果使用 cosine attention，尺度问题会以另一种方式处理。
- 训练可学习 temperature 也是相似思想。



## 数学、Shape 与复杂度

若 $q_i,k_i$ 独立、零均值、单位方差，则 $q^Tk$ 的方差约为 $d_k$。除以 $\sqrt{d_k}$ 后把 logit 的标准差拉回常数量级，避免 softmax 过早进入饱和区。

## 工程实现 / PyTorch 验证

### 推荐验证协议

随机采样不同 d_k 的 Q/K，画未缩放/缩放 logits std 与 softmax entropy。
...
除以 √d_k 是控制 dot-product variance 的近似；若 Q/K 已有其他归一化或 QK-Norm，最佳 scaling 机制可能变化。
...
不要说‘防止梯度爆炸’就结束；直接机制是避免 logits 随维度变大导致 softmax 饱和。

### 白板专项练习

假设 q_i,k_i 独立零均值方差 1，推导 dot-product 方差为 d_k。
...
- **本题特定证据：** 随机采样不同 d_k 的 Q/K，画未缩放/缩放 logits std 与 softmax entropy。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**不要说‘防止梯度爆炸’就结束；直接机制是避免 logits 随维度变大导致 softmax 饱和。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
