---
id: "Q046"
title: "Absolute Position、Relative Position、RoPE 有什么区别？"
chapter: 5
...
# Q046 · Absolute Position、Relative Position、RoPE 有什么区别？
...
绝对位置直接给第 i 个 token 一个位置向量；相对位置让注意力显式依赖 i-j；RoPE 对 Q、K 的不同二维子空间按位置做旋转，使 q_m^T k_n 自然包含 m-n 的相对位置信息，同时不直接修改 V。
...
- RoPE 的关键是 R_m^T R_n = R_{n-m}。
- 不同频率对应不同位置尺度。
- 长上下文扩展常对 RoPE 频率/尺度做插值或重标定。



## 数学、Shape 与复杂度

RoPE 对 Q/K 施加位置相关旋转。二维旋转满足 $R_m^TR_n=R_{n-m}$，因此内积天然依赖相对位移：
...
实现最小 RoPE，验证对 Q/K 同时旋转后内积满足相对位移性质；测试超训练长度时角频率变化。
...
RoPE 将每对隐藏维度视作二维平面按位置旋转，使 `q_m^T k_n` 只与相对位置差的旋转有关；频率分布决定不同尺度的位置敏感性。
...
长上下文扩展通常不是‘直接把 max_position 改大’，还涉及频率缩放与训练/微调分布。

### 白板专项练习

用二维旋转矩阵推导 `R_m^T R_n = R_{n-m}`。
...
- **本题特定证据：** 实现最小 RoPE，验证对 Q/K 同时旋转后内积满足相对位移性质；测试超训练长度时角频率变化。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**长上下文扩展通常不是‘直接把 max_position 改大’，还涉及频率缩放与训练/微调分布。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
