---
id: "Q047"
title: "Transformer 的 FFN 到底做什么？为什么不能删？"
chapter: 5
...
# Q047 · Transformer 的 FFN 到底做什么？为什么不能删？
...
考察 Attention 与 FFN 分工。

**高质量回答标准：** 能写公式与 shape；能给复杂度；能把训练、显存、kernel 与 serving 影响串起来。

## 一句话结论

Attention 主要在 token 之间路由和混合信息；FFN 对每个 token 独立地做高维非线性特征变换，提供大量参数容量。

## 60–90 秒面试回答

Attention 主要在 token 之间路由和混合信息；FFN 对每个 token 独立地做高维非线性特征变换，提供大量参数容量。删掉 FFN 会显著降低模型的逐位置表示变换能力和总体容量。

## 深度解析

- 经典 FFN: W2·σ(W1x)，中间维度常扩张。
- 现代 LLM 常用 GLU/SwiGLU 门控结构。
- FFN 往往占模型参数和 FLOPs 的大头之一。
...
统计一层 attention 与 FFN 参数/FLOPs；把 FFN hidden ratio 从 4x 调整，观察容量与速度。
...
- SwiGLU 为什么有两路投影？
- 为什么 FFN 可以视为 key-value memory 的一种近似解释？
...
Attention 主要做 token mixing，FFN/MoE 主要做每 token 的 channel-wise 非线性变换；SwiGLU 还引入门控乘法。
...
删除 FFN 并不让网络完全线性，因为 attention 有 softmax，但会大幅限制逐位置特征变换容量。

### 白板专项练习

写出 GELU FFN 与 SwiGLU 公式，比较参数量在相同 intermediate size 下的变化。
...
- **本题特定证据：** 统计一层 attention 与 FFN 参数/FLOPs；把 FFN hidden ratio 从 4x 调整，观察容量与速度。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**删除 FFN 并不让网络完全线性，因为 attention 有 softmax，但会大幅限制逐位置特征变换容量。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
