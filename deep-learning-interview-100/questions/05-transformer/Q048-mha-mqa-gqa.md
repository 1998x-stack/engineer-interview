---
id: "Q048"
title: "MHA、MQA、GQA 的区别？为什么 GQA 能省 KV Cache？"
chapter: 5
...
# Q048 · MHA、MQA、GQA 的区别？为什么 GQA 能省 KV Cache？
...
考察现代 LLM inference architecture。

**高质量回答标准：** 能写公式与 shape；能给复杂度；能把训练、显存、kernel 与 serving 影响串起来。

## 一句话结论

MHA 每个 query head 都有自己的 K/V head；MQA 让所有 query head 共享一组 K/V；GQA 则让一组 queryheads 共享一个 K/V head。

## 60–90 秒面试回答

MHA 每个 query head 都有自己的 K/V head；MQA 让所有 query head 共享一组 K/V；GQA 则让一组 queryheads 共享一个 K/V head。由于 decode 需要缓存历史 K/V，减少 KV head 数会近似按比例降低 KV Cache。
KV cache ∝ layers × batch × seq × kv_heads × head_dim × 2(K,V)

## 深度解析

- Q head 数保持多头表达，KV head 数减少主要优化 memory bandwidth。
- GQA 在质量与推理成本之间折中，是现代 LLM 常用方案。
- MLA 进一步用低秩潜表示压缩 K/V，可作为高级追问。



## 数学、Shape 与复杂度

KV cache 规模与 KV head 数线性相关。若 query head 数为 $H_q$、KV head 数为 $H_{kv}$，GQA 通过 $H_{kv}<H_q$ 显著降低每层缓存，而保持更多 query 表达子空间。

## 工程实现 / PyTorch 验证

### 推荐验证协议

给定 Hq=32、Hkv=8/1，计算 KV cache 和 decode KV bandwidth 比例；检查 tensor shape 重复/广播方式。
...
MHA/MQA/GQA 的核心区别是 Q head 与 KV head 的共享比例；GQA 用少量 KV heads 在质量与 cache/带宽之间折中。
...
GQA 省的是 KV 相关内存/带宽，不会把所有 attention 计算按同样比例降低；Q heads 仍然存在。

### 白板专项练习

画出 32 query heads 映射到 8 KV groups 的关系，并写出 cache 公式。
...
- **本题特定证据：** 给定 Hq=32、Hkv=8/1，计算 KV cache 和 decode KV bandwidth 比例；检查 tensor shape 重复/广播方式。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**GQA 省的是 KV 相关内存/带宽，不会把所有 attention 计算按同样比例降低；Q heads 仍然存在。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
