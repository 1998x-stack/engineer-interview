---
id: "Q052"
title: "为什么现代通用大模型大量采用 Decoder-only？"
chapter: 5
...
# Q052 · 为什么现代通用大模型大量采用 Decoder-only？
...
Decoder-only 用 next-token prediction 把各种文本、代码、指令和工具轨迹统一成序列建模问题；训练目标简单、数据可混合、架构易扩展，并天然支持开放式生成和 in-context learning，因此在通用大模型中形成强工程优势。
...
- 不是因为 encoder “能力弱”。
- 很多检索/embedding 系统仍更适合 encoder。
- 多模态也可把视觉 token 序列化后接入 decoder-only 主干。
...
同一任务构造成 classifier encoder 与 prompted decoder，比较 token/latency/训练目标。
...
- 为什么 decoder-only 能做理解任务？
- Prefix LM 与 causal LM 有何差异？
...
Decoder-only 用统一 next-token objective 覆盖生成、in-context learning 与任务序列化，工程栈和 scaling 简洁；并非因为 encoder ‘没有用’。
...
检索 embedding、token classification 等场景 encoder-only 仍可能更高效；架构选择要结合输出模式。

### 白板专项练习

从 causal factorization `P(x)=∏P(x_t|x_<t)` 解释统一生成目标。
...
- **本题特定证据：** 同一任务构造成 classifier encoder 与 prompted decoder，比较 token/latency/训练目标。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**检索 embedding、token classification 等场景 encoder-only 仍可能更高效；架构选择要结合输出模式。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
