...
考察不仅会背 context window。

**高质量回答标准：** 能写公式与 shape；能给复杂度；能把训练、显存、kernel 与 serving 影响串起来。

## 一句话结论

问题至少包含：Attention 计算/IO、KV Cache 显存、位置编码外推、长文信息利用效率、lost-in-the-middle、检索噪声与上下文稀释。

## 60–90 秒面试回答

问题至少包含：Attention 计算/IO、KV Cache 显存、位置编码外推、长文信息利用效率、lost-in-the-middle、检索噪声与上下文稀释。一个模型宣称支持更长窗口，不等于能稳定利用整个窗口。

## 深度解析

- 训练 context length 与推理最大长度要区分。
- 长文本 benchmark 要测 needle retrieval、multi-hop、global aggregation 等不同能力。
- 系统层可通过 chunking、RAG、prefix cache、KV 压缩降低成本。
...
做 needle-in-a-haystack 的 position×length 网格测试，同时加入多 needle 与 distractor，避免单一 benchmark。
...
- 如何评价 1M context 是否“真的有用”？
- RoPE scaling 与 continued pretraining 的关系？
...
长上下文问题至少分 capacity、compute/memory、position extrapolation、有效利用与数据/eval；窗口大只是 capacity，不代表 retrieval/use 能力等比例增加。
...
Lost-in-the-middle、attention sink、RoPE extrapolation、KV capacity 是不同层面问题，不应混成一个‘长上下文退化’。
...
- **本题特定证据：** 做 needle-in-a-haystack 的 position×length 网格测试，同时加入多 needle 与 distractor，避免单一 benchmark。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**Lost-in-the-middle、attention sink、RoPE extrapolation、KV capacity 是不同层面问题，不应混成一个‘长上下文退化’。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
