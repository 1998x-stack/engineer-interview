---
id: "Q057"
title: "LoRA Rank 怎么选？为什么经典初始化常让一侧为零？"
chapter: 6
...
# Q057 · LoRA Rank 怎么选？为什么经典初始化常让一侧为零？
...
rank 控制适配容量；过小可能欠拟合，过大增加显存并降低 PEFT 优势。

## 60–90 秒面试回答

rank 控制适配容量；过小可能欠拟合，过大增加显存并降低 PEFT 优势。经典做法让一侧随机、一侧零，使初始ΔW=0，模型从 base behavior 起步，同时另一侧能在第一步获得非零梯度并逐渐打开更新通道。

## 深度解析

- 若 A、B 都为 0，乘积结构会导致两侧初始梯度都可能为 0。
- 不同层的最佳 rank 可能不同，AdaLoRA 等方法做动态分配。
- rank 选择要结合 domain shift、数据量与 target modules。

### Rank 不是越大越好

Rank 决定 adapter 的容量、显存和通信量。实践中应按任务和 target module 做实验，而不是迷信固定 `r=8/16/64`。建议记录 `trainable_params / total_params`、验证集收益、过拟合速度和多任务可迁移性。

### 初始化细节

默认 LoRA 的设计目标是 **no-op initialization**：训练第 0 步模型函数与 base 完全一致，再逐渐学习增量。当前 PEFT 生态还提供 PiSSA、LoftQ 等初始化变体；面试里应区分“经典 LoRA 原理”和“现代库里的可选初始化”。

## 数学、Shape 与复杂度

经典 LoRA 初始化让整个增量分支一开始为 0（例如一侧随机、一侧为 0），使初始函数严格等于 base model。若两侧都为 0，则两侧梯度会在起点同时退化，无法有效打破对称。

## 工程实现 / PyTorch 验证

### 推荐验证协议

打印 LoRA A/B 第一 step 的 grad norm，验证双零初始化退化；比较 rank 与 `alpha/r`、`alpha/sqrt(r)` scaling。
...
经典 LoRA 让 B=0、A 随机，使初始 ΔW=0 但第一步 B 有梯度；若 A、B 都为 0，则两侧梯度都可能为 0。现代 PEFT 还支持 PiSSA/EVA/LoftQ/rsLoRA 等初始化或缩放。
...
rank 不是越大越好：更大容量增加显存/通信且可能需要不同 scaling；target module 的选择往往和 rank 同样重要。

### 白板专项练习

手推 `ΔW=BA` 时 `∂L/∂A=B^T G`、`∂L/∂B=GA^T`，解释初始化梯度。
...
- **本题特定证据：** 打印 LoRA A/B 第一 step 的 grad norm，验证双零初始化退化；比较 rank 与 `alpha/r`、`alpha/sqrt(r)` scaling。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**rank 不是越大越好：更大容量增加显存/通信且可能需要不同 scaling；target module 的选择往往和 rank 同样重要。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
...
- [PEFT LoRA package reference (current)](https://huggingface.co/docs/peft/package_reference/lora)
