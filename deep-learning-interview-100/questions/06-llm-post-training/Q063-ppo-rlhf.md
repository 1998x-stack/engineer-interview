---
id: "Q063"
title: "经典 PPO-based RLHF 由哪些模型组成？目标是什么？"
chapter: 6
...
# Q063 · 经典 PPO-based RLHF 由哪些模型组成？目标是什么？
...
经典 RLHF 常包含 policy/actor、reference policy、reward model，以及 PPO 中用于估计 value 的 critic。

## 60–90 秒面试回答

经典 RLHF 常包含 policy/actor、reference policy、reward model，以及 PPO 中用于估计 value 的 critic。
policy 生成回答，reward model 给序列奖励，critic 估计优势；PPO 通过 clipped policy ratio 限制单次策略更新，reference KL 进一步约束模型不要偏离基座过远。
L_clip=E[min(r_t A_t, clip(r_t,1-ε,1+ε)A_t)]

## 深度解析

- PPO clip 与 reference KL 是两种不同约束。
- reward 往往是 sequence-level，需要用 advantage/return 分配到 token-level 更新。
- RLHF 训练昂贵的关键在 rollout 和多个模型常驻显存。
...
clip objective 控制每轮 policy 更新幅度；RLHF 中还常加入 reference-policy KL penalty。

## 工程实现 / PyTorch 验证

### 推荐验证协议

画一次迭代的数据流并估算四模型驻留/推理成本；记录 ratio、clip fraction、KL、entropy、value loss。
...
- Actor/Critic/Reference/Reward 各是否更新？
- GAE 是什么？
- 为什么 PPO 训练不稳定？
...
- **易错：** 把 reward model 和 critic 混为一谈。
...
经典 PPO-RLHF 需区分 policy、reference、reward model、value/critic；rollout 后基于 reward+KL 构造 return/advantage，再做 clipped policy/value update。
...
PPO 的稳定性来自一组约束而非单一 clip；reward scale、GAE、batch reuse 和 policy lag 都会影响训练。

### 白板专项练习

写出 clipped surrogate 并解释 `min` 为什么对正负 advantage 形成不同截断。
...
- **本题特定证据：** 画一次迭代的数据流并估算四模型驻留/推理成本；记录 ratio、clip fraction、KL、entropy、value loss。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**PPO 的稳定性来自一组约束而非单一 clip；reward scale、GAE、batch reuse 和 policy lag 都会影响训练。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
