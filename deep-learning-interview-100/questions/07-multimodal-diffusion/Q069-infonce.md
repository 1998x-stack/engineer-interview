---
id: "Q069"
title: "InfoNCE Loss 是什么？为什么适合对比学习？"
chapter: 7
...
# Q069 · InfoNCE Loss 是什么？为什么适合对比学习？
...
InfoNCE 把一个 query 的正样本与一组负样本一起做 softmax 分类：提高 q 与 k+ 相似度，同时降低与 k- 的相似度。

## 60–90 秒面试回答

InfoNCE 把一个 query 的正样本与一组负样本一起做 softmax 分类：提高 q 与 k+ 相似度，同时降低与 k- 的相似度。它既可视为对比分类目标，也与互信息下界估计有关。
L=-log exp(sim(q,k+)/τ) / Σ_j exp(sim(q,k_j)/τ)

## 深度解析

- 通常先对 embedding L2 normalize，再用 cosine/dot-product similarity。
- in-batch negatives 能高效扩大负样本数量，但引入 false negative 风险。
- 对称 CLIP loss 会同时 image→text 和 text→image。
...
$$
L_i=-\log\frac{\exp(sim(q_i,k_i^+)/\tau)}{\sum_j\exp(sim(q_i,k_j)/\tau)}.
$$

它可以看作“在一组候选 key 中识别正样本”的交叉熵。
...
def infonce(q, k, temperature=0.07):
...
实现 symmetric InfoNCE，比较 batch size/negative 数量对 loss 与 retrieval recall 的影响。
...
- 为什么 batch size 对对比学习很重要？
- InfoNCE 和 triplet loss 有什么差异？
...
InfoNCE 可视为 batch 内分类：正 pair 是目标类，其他 key 为 negatives；温度控制 logit scale，表示通常先 L2 normalize。
...
in-batch negative 默认假设其他样本为负，false negative 会造成偏差；分布式 all-gather negatives 还需注意 gradient 语义。

### 白板专项练习

从 cross-entropy 视角推导一个 query 对 N keys 的 InfoNCE。
...
- **本题特定证据：** 实现 symmetric InfoNCE，比较 batch size/negative 数量对 loss 与 retrieval recall 的影响。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**in-batch negative 默认假设其他样本为负，false negative 会造成偏差；分布式 all-gather negatives 还需注意 gradient 语义。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
...
- [Q070 · InfoNCE 中 Temperature τ 起什么作用？](../07-multimodal-diffusion/Q070-temperature.md)
