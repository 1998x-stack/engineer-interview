---
id: "Q071"
title: "对比学习中的 False Negative 如何处理？"
chapter: 7
...
# Q071 · 对比学习中的 False Negative 如何处理？
...
False negative 是语义上应该接近、却被采样为负的样本，会把正确邻居错误推远。

## 60–90 秒面试回答

False negative 是语义上应该接近、却被采样为负的样本，会把正确邻居错误推远。可利用标签/行为图过滤、multi-positive objective、debiasing、teacher similarity、语义阈值，以及更谨慎的 hard-negative mining。

## 深度解析

- 热门 item 作为大量负样本会改变训练先验，推荐场景尤其明显。
- hard negative 不是越难越好，过难可能就是 false negative。
- 要评估采样分布与线上候选分布是否一致。
...
用 metadata/teacher similarity 标记疑似 false negatives，比较过滤、multi-positive、debias loss。
...
- **易错：** 把“相似但不是正标签”一律当 hard negative。
...
False negative 本质是语义上正但被 objective 当负，会把本应相近的表示推远；在推荐、多标签、近重复数据尤其常见。
...
hard negative mining 会同时提高有效难例与 false-negative 风险，必须配合语义过滤。

### 白板专项练习

设计一个 batch 内多正样本 loss，让同一 query 的多个 positives 不互相竞争。
...
- **本题特定证据：** 用 metadata/teacher similarity 标记疑似 false negatives，比较过滤、multi-positive、debias loss。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**hard negative mining 会同时提高有效难例与 false-negative 风险，必须配合语义过滤。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
