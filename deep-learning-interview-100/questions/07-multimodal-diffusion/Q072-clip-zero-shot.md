...
CLIP 在大规模图文对上用对比学习把匹配图片与文本映射到同一 embedding 空间。

## 60–90 秒面试回答

CLIP 在大规模图文对上用对比学习把匹配图片与文本映射到同一 embedding 空间。分类时把类别写成自然语言prompt，得到各类别文本向量，再比较图片向量与文本向量相似度，因此无需为新类别训练独立分类头。

## 深度解析

- prompt wording 会影响 zero-shot 性能，因此出现 prompt ensemble/prompt learning。
- CLIP 学的是跨模态相似度，不是传统固定标签分类器。
- 对比训练规模与数据质量是泛化关键。
...
对同类别写多种 prompt template 做 prompt ensembling；比较 raw class name 与自然语言 prompt。
...
CLIP 通过大规模 image-text contrastive alignment 把两模态映射到共享 embedding space；zero-shot 分类本质是 image 与 class prompt embeddings 的相似度分类。
...
zero-shot 受 prompt、domain shift、类别名语义和训练数据偏差影响；不是无监督万能分类器。

### 白板专项练习

写出 image→embedding、text prompts→embedding、cosine×temperature→softmax 的完整推理链。
...
- **本题特定证据：** 对同类别写多种 prompt template 做 prompt ensembling；比较 raw class name 与自然语言 prompt。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**zero-shot 受 prompt、domain shift、类别名语义和训练数据偏差影响；不是无监督万能分类器。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
...
- [Q073 · 如何把 Text-only LLM 改造成多模态模型？](../07-multimodal-diffusion/Q073-multimodal-llm.md)
