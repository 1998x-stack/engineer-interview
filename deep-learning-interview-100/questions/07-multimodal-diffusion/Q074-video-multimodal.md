---
id: "Q074"
title: "多模态模型如何处理视频？视觉 token 爆炸怎么办？"
chapter: 7
...
# Q074 · 多模态模型如何处理视频？视觉 token 爆炸怎么办？
...
视频先做帧/clip 采样，经视觉编码器得到空间 token，再做 temporal pooling、token merging、resampler或层次编码，压缩成可被 LLM 消化的视频 token。

## 60–90 秒面试回答

视频先做帧/clip 采样，经视觉编码器得到空间 token，再做 temporal pooling、token merging、resampler或层次编码，压缩成可被 LLM 消化的视频 token。核心矛盾是时空信息完整性与上下文/计算成本。

## 深度解析

- 均匀抽帧可能错过短关键事件，应考虑 shot/event-aware sampling。
- 视觉 token 远多于文本时，loss 和梯度可能被视觉模态主导，需要采样/权重/对齐策略。
- 长视频还需处理时间位置编码。
...
用不同 FPS/scene-based sampling/temporal pooling 测事件定位准确率和 token 成本。
...
视频同时有空间与时间冗余，关键是 frame sampling、temporal encoder、token compression 与长上下文预算；均匀抽帧可能漏掉短事件。
...
把每帧所有 patch token 直接拼接会迅速爆上下文；压缩过强又会丢动作与时序细节。

### 白板专项练习

给 120 秒视频、2 fps、每帧 576 tokens，算原始 token 数并设计压缩到 8k 的方案。
...
- **本题特定证据：** 用不同 FPS/scene-based sampling/temporal pooling 测事件定位准确率和 token 成本。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**把每帧所有 patch token 直接拼接会迅速爆上下文；压缩过强又会丢动作与时序细节。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
