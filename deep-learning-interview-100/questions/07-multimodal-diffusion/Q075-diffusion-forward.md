---
id: "Q075"
title: "Diffusion 的 Forward Process 是什么？"
chapter: 7
...
# Q075 · Diffusion 的 Forward Process 是什么？
...
Forward diffusion 逐步向数据 x0 加 Gaussian noise，构造一个已知的 Markov chain，最终 xT 接近标准高斯；由于高斯性质，可以直接从 x0 一步采样任意 xt，而无需真的循环 t 次。
...
- q(xt|x_{t-1}) 由 β_t 控制噪声强度。
- 累积 ᾱ_t 后可得到 closed-form xt=√ᾱ_t x0+√(1-ᾱ_t)ε。
- 训练学习逆过程的参数化。
...
DDPM forward process 逐步加噪：

$$
q(x_t|x_{t-1})=\mathcal N(\sqrt{1-\beta_t}x_{t-1},\beta_tI).
$$

利用累积系数可直接从 $x_0$ 一步采样 $x_t$，训练无需真的逐步执行前向链。

## 工程实现 / PyTorch 验证

### 推荐验证协议

实现 q_sample，验证 t 增大时 SNR 降低；画不同 noise schedule 的 SNR 曲线。
...
- 为什么 forward process 不需要学习？
- noise schedule 会影响什么？
...
Forward diffusion 是固定 Markov 加噪过程；利用 `bar_alpha_t` 可直接从 x0 采样任意 xt，这是训练高效的关键。
...
β schedule 影响不同时间步难度和采样质量；forward process 本身通常不学习参数。

### 白板专项练习

从逐步高斯转移推导 closed-form `q(x_t|x_0)`。
...
- **本题特定证据：** 实现 q_sample，验证 t 增大时 SNR 降低；画不同 noise schedule 的 SNR 曲线。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**β schedule 影响不同时间步难度和采样质量；forward process 本身通常不学习参数。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
