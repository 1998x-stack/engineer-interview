---
id: "Q077"
title: "Latent Diffusion 为什么比 Pixel Diffusion 更便宜？"
chapter: 7
...
# Q077 · Latent Diffusion 为什么比 Pixel Diffusion 更便宜？
...
先用 autoencoder 把高分辨率图像压缩到低空间尺寸 latent，再在 latent 上运行 diffusion，大幅减少每一步U-Net/Transformer 的 token/像素数量；最终 decoder 把 latent 还原成图像。
...
- 压缩带来计算收益，但 autoencoder 的重建质量决定细节上限。
- latent 通道数虽增加，空间降采样通常仍带来巨大节省。
- 文本条件常通过 cross-attention 注入。
...
比较 pixel/latent 的元素数与 U-Net/DiT FLOPs；检查 VAE reconstruction 质量作为上限之一。
...
Latent diffusion 将高维像素压到 VAE latent，再做 denoising，大幅减少空间 token；成本是引入 encoder/decoder 失真与 latent scaling。
...
latent 过度压缩会丢文字、细节和高频信息；生成质量不只由 diffusion backbone 决定。

### 白板专项练习

给 1024² RGB 与 8× downsample latent，计算空间元素缩减倍数。
...
- **本题特定证据：** 比较 pixel/latent 的元素数与 U-Net/DiT FLOPs；检查 VAE reconstruction 质量作为上限之一。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**latent 过度压缩会丢文字、细节和高频信息；生成质量不只由 diffusion backbone 决定。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
