---
id: "Q082"
title: "Gradient Checkpointing 为什么能省显存？代价是什么？"
chapter: 8
...
# Q082 · Gradient Checkpointing 为什么能省显存？代价是什么？
...
考察 activation memory。

**高质量回答标准：** 能先定位 compute/memory/communication/input 哪类瓶颈，再选择工具；能解释优化手段的代价。

## 一句话结论

普通反向传播需要保存大量 forward activations；checkpointing 只保留少数边界激活，在 backward 时重新执行部分 forward 恢复中间值，因此用额外计算换 activation memory。
...
- 它不直接减少参数/optimizer state。
- 重计算区间越大，显存越省但额外 FLOPs 越多。
- 随机算子需要正确保存/恢复 RNG state 才能保持一致。
...
用 `torch.utils.checkpoint` 比较 peak memory 与 step time；分别 checkpoint attention/FFN block。
...
- 为什么 activation 在长序列训练中可能超过参数显存？
- checkpoint 每一层是否最优？
...
Checkpointing 省的是需要保存的 activation，用 backward 重算 forward 片段换显存；最优切分取决于 activation 大小和 recompute 成本。
...
如果模型本来受 optimizer/parameter memory 主导，checkpoint 收益有限；随机算子还需注意 RNG state。

### 白板专项练习

画出不 checkpoint 与 checkpoint 的 activation 生命周期，说明为什么 compute 增加。
...
- **本题特定证据：** 用 `torch.utils.checkpoint` 比较 peak memory 与 step time；分别 checkpoint attention/FFN block。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**如果模型本来受 optimizer/parameter memory 主导，checkpoint 收益有限；随机算子还需注意 RNG state。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
