...
建立数值断点：输入→关键层激活/logits→loss→grad→optimizer state，逐步检查 isfinite，并记录首次出现 异常的 step/rank/sample。

## 60–90 秒面试回答

建立数值断点：输入→关键层激活/logits→loss→grad→optimizer state，逐步检查 isfinite，并记录首次出现
异常的 step/rank/sample。常见原因包括 LR 过高、FP16 overflow、除零/log(0)、异常数据、梯度爆炸、归一化方差过小或自定义 kernel bug。

## 深度解析

- anomaly detection 很慢，适合缩小范围后使用。
- gradient clipping 若只能延迟 NaN，根因可能仍在。
- 某个 rank 单独 NaN 时优先看 shard data/custom op/collective。
...
用 hooks/`detect_anomaly` 在最小复现上定位；保存异常 batch、RNG 与 checkpoint 可重放。
...
- 为什么 softmax 手写容易 overflow？
- 如何做 numerically stable log-softmax？
...
NaN/Inf 诊断遵循首次污染原则：找第一个非有限 tensor/step，而不是只看最终 loss；同时追踪 logits、loss components、grad norm、optimizer state。
...
`detect_anomaly` 很慢，不适合长期生产训练；gradient clipping 也可能掩盖根因而非修复。

### 白板专项练习

设计二分法：层级 hook 找首次 NaN 层，再缩到具体 op 与输入范围。
...
- **本题特定证据：** 用 hooks/`detect_anomaly` 在最小复现上定位；保存异常 batch、RNG 与 checkpoint 可重放。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**`detect_anomaly` 很慢，不适合长期生产训练；gradient clipping 也可能掩盖根因而非修复。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
