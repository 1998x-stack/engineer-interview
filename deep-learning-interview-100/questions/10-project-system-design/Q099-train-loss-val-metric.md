---
id: "Q099"
title: "训练 Loss 一直下降，但验证指标不涨，怎么办？"
chapter: 10
...
先确认评估 pipeline 和数据切分，再区分过拟合、objective mismatch、label noise、distribution shift、metric saturation 与训练/验证预处理不一致。

## 60–90 秒面试回答

先确认评估 pipeline 和数据切分，再区分过拟合、objective mismatch、label noise、distribution shift、metric saturation 与训练/验证预处理不一致。只有确定是模型泛化问题后，再谈正则化、数据增强、容量与训练策略。

## 深度解析

- 若 val loss 也降但业务 metric 不涨，优先怀疑 objective/metric mismatch。
- 若只有某 slice 不涨，应做分桶分析而不是看总平均。
- 验证集污染会产生相反的虚假乐观。
...
固定 checkpoint 做 prediction dump，按 slice 比较 loss、calibration、ranking metric；验证评估代码与 label 时间窗。
...
Train loss↓而 val metric 不涨首先检查 metric/objective 对齐、split/leakage、label noise 与 evaluation bug，再谈正则或模型容量。
...
训练 loss 与业务 metric 非单调并不罕见，尤其 surrogate loss；不能把所有情况都归为 overfitting。

### 白板专项练习

列出至少 8 个假设，并按‘最便宜验证优先’排序排障。
...
- **本题特定证据：** 固定 checkpoint 做 prediction dump，按 slice 比较 loss、calibration、ranking metric；验证评估代码与 label 时间窗。

### 方案选择与停止条件

当收益只存在于单一 benchmark、无法复现到多个 seed/slice、引入明显 latency/显存/维护成本，或触发本题的失败边界——**训练 loss 与业务 metric 非单调并不罕见，尤其 surrogate loss；不能把所有情况都归为 overfitting。**——就不应继续用“模型更复杂”掩盖问题。面试中主动说出停止条件，通常比继续堆方案更像真实工程负责人。
