...
## Professional Edition · 深度增强版

本版对 **100/100** 道题逐题增厚，不是统一追加空泛模板。每题在 PDF 基线之上新增：

- **专家级白板推导**：把结论还原成 compute / HBM / communication / scheduling 成本模型；
- **源码 / Runtime 视角**：指出应追踪的对象、状态、关键决策点和 fallback；
- **Benchmark Lab**：给出控制变量、自变量、观测量和验收方式；
- **Senior 面试变体**：硬件、流量、模型、SLO、故障条件变化后的迁移思考；
- **版本边界**：对 vLLM/SGLang/TensorRT-LLM 等快速演进系统明确版本敏感性。

建议先读 [Hands-on Labs](docs/hands-on-labs.md)，再按章节刷题。
