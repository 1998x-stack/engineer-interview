---
id: Q001
title: "为什么 LLM 的 Prefill 和 Decode 是两种完全不同的计算？"
chapter: "推理性能基本原理"
difficulty: "★★★★★"
tags: ["prefill", "decode", "roofline"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q001｜为什么 LLM 的 Prefill 和 Decode 是两种完全不同的计算？

> **定位**：推理性能基本原理 · **难度**：★★★★★  
> **关键词**：`prefill` · `decode` · `roofline`

## 30 秒面试回答

> Prefill 一次处理整段 prompt，矩阵 M 维较大，GEMM/Attention 更容易提高算术强度，常偏 compute-bound； Decode 每步只新增少量 token，却要反复读取模型权重与历史 KV，低 batch 下常偏 memory-bandwidth-bound；两阶段分别主导 TTFT 与 TPOT，因此不能用同一套优化策略。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：Prefill 一次处理整段 prompt，矩阵 M 维较大，GEMM/Attention 更容易提高算术强度，常偏 compute-bound；Decode 每步只新增少量 token，却要反复读取模型权重与历史 KV，低 batch 下常偏 memory-bandwidth-bound；两阶段分别主导 TTFT 与 TPOT，因此不能用同一套优化策略。
...
公式 Prefill: Q,K,V∈R^{T×d}; Decode: Q∈R^{B×d}. 低并发 Decode 的下界常近似受 weight bytes / HBM bandwidth 限制。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. Prefill 一次处理整段 prompt，矩阵 M 维较大，GEMM/Attention 更容易提高算术强度，常偏 compute-bound
- 2. Decode 每步只新增少量 token，却要反复读取模型权重与历史 KV，低 batch 下常偏 memory-bandwidth-bound
- 3. 两阶段分别主导 TTFT 与 TPOT，因此不能用同一套优化策略。
...
现场推演 32K prompt + 100 输出 token 的服务，优先关心 Prefill 切分与 TTFT；1K prompt + 16K 推理输出则更应优化
Decode、KV 与 speculative decoding。
...
- 围绕“为什么 LLM 的 Prefill 和 Decode 是两种完全不同的计算？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

“Prefill compute-bound / Decode memory-bound”是常见区间，不是定律；大 batch、长 KV、MoE 通信或特殊 kernel 会改变主瓶颈。

- ✗ 把“Prefill compute-bound、Decode memory-bound”当成绝对结论
- ✗ 高 batch、MoE、长上下文或通信开销都可能改变瓶颈。
...
- → 为什么增大 batch 对 Decode 特别有效？
- → 为什么 Chunked Prefill 能同时改善吞吐与 ITL？
- → 如何用 Roofline 验证你的判断？
...
关键不是“Prefill=矩阵乘、Decode=向量乘”这句口号，而是算术强度随 M 维、batch 和 KV 长度变化。高 batch decode、超长 KV attention、MoE A2A 都可能让瓶颈迁移。

### 白板推导

对单层 Linear，可把 Prefill 写成 $[B\cdot T,d]\times[d,h]$，而 Decode 近似 $[B,d]\times[d,h]$。当 $B\cdot T$ 很大时，权重被大量 token 复用，算术强度上升；Decode 低 batch 时复用不足。
...
分别构造 1K/8K/32K prompt 与 32/512/4K output，保持总 token 接近，比较 TTFT/TPOT 与 HBM/Tensor Core。
...
- [Q035 CUDA Graph 为什么适合 Decode？](../04-kernel-runtime/q035-cuda-graph-decode.md)
...
[01 推理性能基本原理](index.md) · [Q002 →](q002-metrics-slo-goodput.md)
