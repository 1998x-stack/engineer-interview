...
指标必须定义时间边界：TTFT 是否包含排队、网络、tokenization；TPOT 是均值还是逐 token 分布；Goodput 的 SLO 需要明确 AND/OR 条件。否则跨团队数字不可比较。

### 白板推导

建议明确：$T_{E2E}=T_{queue}+T_{prefill}+\sum_i T_{decode,i}+T_{network}$。如果把 queue 从 TTFT 定义里拿掉，必须另报 queue latency；Goodput 可写为 $\lambda\cdot P(\mathrm{TTFT}<S_1\land\mathrm{TPOT}<S_2)$。
