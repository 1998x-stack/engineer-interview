---
id: Q003
title: "为什么 batch=1 的 Decode 经常是 Memory Bound？"
chapter: "推理性能基本原理"
difficulty: "★★★★★"
tags: ["decode", "memory-bandwidth"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q003｜为什么 batch=1 的 Decode 经常是 Memory Bound？

> **定位**：推理性能基本原理 · **难度**：★★★★★  
> **关键词**：`decode` · `memory-bandwidth`

## 30 秒面试回答

> 每生成一个 token 都要执行全模型层。batch=1 时同一份权重只服务一个 token，权重读取复用极低；现代 GPU FLOPS 增长往往快于 HBM 带宽增长，所以算力没有吃满时 HBM 已接近瓶颈。增大 batch 可以摊薄每 token 的权重读取成本。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：每生成一个 token 都要执行全模型层。batch=1 时同一份权重只服务一个 token，权重读取复用极低；现代 GPU FLOPS 增长往往快于 HBM 带宽增长，所以算力没有吃满时 HBM 已接近瓶颈。增大 batch 可以摊薄每 token 的权重读取成本。
...
公式粗略上界 tokens/s ≤ HBM_BW / model_weight_bytes；70B BF16 权重约 140 GB。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 每生成一个 token 都要执行全模型层。batch=1 时同一份权重只服务一个 token，权重读取复用极低
- 2. 现代 GPU FLOPS 增长往往快于 HBM 带宽增长，所以算力没有吃满时 HBM 已接近瓶颈。增大 batch 可以摊薄每 token 的权重读取成本。
...
现场推演若设备有效带宽 3 TB/s，单纯按权重流量得到的理想上限约 21 token/s；实际还要扣除 KV、kernel、同步和通信。
...
- 围绕“为什么 batch=1 的 Decode 经常是 Memory Bound？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- ✗ 直接用理论 HBM 峰值计算线上速度
- ✗ 忽略权重并非每层都完美流式、cache/quantization/TP 都会改变 bytes。
...
- → 为什么量化在 batch=1 往往更有效？
- → batch 大到什么程度后可能转为 compute-bound？
- → 如何用 Nsight 验证？
...
一阶模型把每 token 权重流量近似为参数字节数/batch；二阶模型还要加入 KV 读取、量化 scale、embedding/lm_head 与 TP collective。batch 增大只摊薄权重，不会同样摊薄所有开销。

### 白板推导

低 batch 的极简模型：$t_{token}\gtrsim W_{bytes}/BW_{HBM}$；batch 为 $B$ 时，权重项可近似摊为 $W_{bytes}/B$，但 KV 项大致仍随每个序列 context 增长。
...
固定 output=512，sweep batch=1/2/4/8/16；观察 HBM 是否先饱和、SM/Tensor 是否随 batch 上升。
...
[← Q002](q002-metrics-slo-goodput.md) · [01 推理性能基本原理](index.md) · [Q004 →](q004-roofline-profiling.md)
