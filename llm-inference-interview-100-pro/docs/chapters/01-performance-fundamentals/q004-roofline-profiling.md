---
id: Q004
title: "什么是 Roofline Model？如何用于分析 LLM inference？"
chapter: "推理性能基本原理"
difficulty: "★★★★★"
tags: ["roofline", "profiling"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q004｜什么是 Roofline Model？如何用于分析 LLM inference？

> **定位**：推理性能基本原理 · **难度**：★★★★★  
> **关键词**：`roofline` · `profiling`

## 30 秒面试回答

> Roofline 用算术强度 AI=FLOPs/Bytes 把 kernel 放到“带宽屋顶”或“算力屋顶”下。先估算某算子的 FLOPs 与 HBM/缓存流量，再比较 AI 与设备 machine balance；这能回答应该优化访存、量化还是 Tensor Core/GEMM。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：Roofline 用算术强度 AI=FLOPs/Bytes 把 kernel 放到“带宽屋顶”或“算力屋顶”下。先估算某算子的 FLOPs 与 HBM/缓存流量，再比较 AI 与设备 machine balance；这能回答应该优化访存、量化还是 Tensor Core/GEMM。
...
公式 Performance ≤ min(PeakFLOPS, AI×MemoryBandwidth)。临界 AI*=PeakFLOPS/MemoryBandwidth。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. Roofline 用算术强度 AI=FLOPs/Bytes 把 kernel 放到“带宽屋顶”或“算力屋顶”下。先估算某算子的 FLOPs 与
HBM/缓存流量，再比较 AI 与设备 machine balance
- 2. 这能回答应该优化访存、量化还是 Tensor Core/GEMM。
...
现场推演同一 Linear 在 Prefill 的 M 很大时更接近 GEMM，AI 提升；Decode 小 batch 时近似 GEMV/small GEMM，AI 明显下降。
...
- 围绕“什么是 Roofline Model？如何用于分析 LLM inference？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 只看 GPU Utilization=100% 就判断 compute-bound
- ✗ utilization 不等于 SM/Tensor/HBM 的有效利用率。
...
- → Attention 的 AI 如何随 context 变化？
- → 量化为什么会移动 Roofline 工作点？
- → 通信如何加入扩展 Roofline？
...
Roofline 最有价值的是“排除错误优化方向”：若点落在带宽屋顶附近，继续做更多 Tensor Core 优化收益有限；若 AI 高但达不到算力屋顶，则应看 shape、occupancy、pipeline 和 kernel implementation。

### 白板推导

Machine balance 为 $AI^*=P_{peak}/BW_{HBM}$。若某 kernel 的 $AI\ll AI^*$，优先减少 bytes；若 $AI\gg AI^*$，再看 Tensor Core、occupancy 与指令效率。
...
[← Q003](q003-decode-memory-bandwidth.md) · [01 推理性能基本原理](index.md) · [Q005 →](q005-estimation-bandwidth.md)
