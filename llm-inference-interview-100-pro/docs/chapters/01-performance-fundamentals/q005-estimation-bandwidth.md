...
理论 token/s 应拆成 compute、HBM、communication、overhead 四个候选下界，再取 max 得到 token latency。把峰值带宽替换为 profiler 测得的 effective bandwidth，估算会更接近线上。

### 白板推导

可以用 $t_{token}\approx \max(t_{compute},t_{HBM},t_{comm})+t_{launch/sched}$，再取倒数得到 token/s。多卡时 $t_{comm}$ 不能忽略。
