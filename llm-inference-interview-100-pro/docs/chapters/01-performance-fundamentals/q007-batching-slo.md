...
> batch 增大提高权重复用与吞吐，但会增加排队、KV 占用、单 step 工作量与尾延迟。线上最优点由 SLO、到达率、长度分布和显存共同决定，而不是最大可装 batch。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：batch 增大提高权重复用与吞吐，但会增加排队、KV 占用、单
step 工作量与尾延迟。线上最优点由 SLO、到达率、长度分布和显存共同决定，而不是最大可装 batch。
...
公式目标常写为 max Goodput s.t. p99(TTFT)<SLO₁, p99(TPOT)<SLO₂, memory≤budget。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. batch 增大提高权重复用与吞吐，但会增加排队、KV 占用、单 step 工作量与尾延迟。线上最优点由 SLO、到达率、长度分布和显存共同决定，而不是最大可装 batch。
...
现场推演离线生成可用更大 batch 追求 tokens/s；交互式 Chat 需限制 iteration token budget 保护 ITL。
...
- 围绕“为什么 Batch Size 不是越大越好？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 把 OOM 前的最大 batch 当最优
- ✗ 忽略长短请求混批造成 head-of-line blocking。
...
- → Continuous batching 如何改变 batch 定义？
- → 如何根据 SLO 自动调 batch？
- → 为什么 MoE 对 batch 更敏感？
...
batch 是吞吐和排队的控制旋钮。最佳 batch 取决于 SLO 与 offered load；当 batch 增大导致排队、KV 占用或长尾上升时，aggregate tokens/s 仍可能增加而 Goodput 下降。
...
- [Q080 MoE 为什么低并发和高并发性能差异特别大？](../08-moe-mla-codesign/q080-moe-batching.md)
...
[← Q006](q006-sequence-length-cost.md) · [01 推理性能基本原理](index.md) · [Q008 →](q008-moe-active-params.md)
