...
KV 公式最容易漏掉 GQA/MQA 的 H_kv、数据类型字节数、TP/CP 分片方式和 block padding。容量规划还要留出 weights、workspace、CUDA graph 与 allocator margin。

### 白板推导

标准 decoder-only KV 容量近似 $M=2\cdot L\cdot T\cdot H_{kv}\cdot d_h\cdot b$；batch 为 $B$ 时再乘 $B$。若 TP/CP 对 KV 分片，应根据实际布局除以对应分片度。
...
用同一模型在 1K→128K context 下记录 allocator 报告的 KV bytes，并用公式反推 H_kv/分片是否一致。
