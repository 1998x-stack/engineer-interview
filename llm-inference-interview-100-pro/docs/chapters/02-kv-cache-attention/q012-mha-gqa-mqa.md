---
id: Q012
title: "MHA、MQA、GQA 为什么会极大影响推理成本？"
...
> 三者主要差在多个 query head 是否共享 K/V。MQA 只有一组 KV，GQA 用若干组 KV，显著降低 KV 容量和 Decode 读取带宽；代价是模型表示能力与 kernel 实现需要折中。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：三者主要差在多个 query head 是否共享 K/V。MQA 只有一组
KV，GQA 用若干组 KV，显著降低 KV 容量和 Decode 读取带宽；代价是模型表示能力与 kernel 实现需要折中。
...
公式 KV memory ∝ H_kv；MHA H_kv=H_q，MQA H_kv=1，GQA 介于两者。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 三者主要差在多个 query head 是否共享 K/V。MQA 只有一组 KV，GQA 用若干组 KV，显著降低 KV 容量和 Decode 读取带宽
- 2. 代价是模型表示能力与 kernel 实现需要折中。
...
现场推演对长上下文和大并发，KV 往往先于权重成为容量瓶颈，因此 GQA 的服务收益可能大于其训练侧直觉。
...
- 围绕“MHA、MQA、GQA 为什么会极大影响推理成本？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- ✗ 只说“GQA 更快”却解释不了为什么
- ✗ 忽略不同 attention backend 对 GQA shape 的优化差异。
...
GQA 的核心是多个 Q head 共享较少的 K/V head；它降低 KV 容量和 decode attention bandwidth，但并不按相同比例降低 Q projection、MLP 或全部计算。

### 白板推导

相对 MHA，KV 容量缩放约为 $H_{kv}/H_q$。例如 32 个 Q heads、8 个 KV heads 的 GQA，KV 主体约是 MHA 的 1/4。
...
- [Q011 KV Cache 显存怎么计算？](q011-kv-cache-memory.md)
...
[← Q011](q011-kv-cache-memory.md) · [02 KV Cache 与 Attention](index.md) · [Q013 →](q013-pagedattention-kv.md)
