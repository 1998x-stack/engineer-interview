...
page 太小会增加 page-table 元数据、寻址和 kernel gather 开销；太大则增加尾页浪费并降低细粒度 prefix reuse。最优值与 attention backend、context 分布和 cache reuse 一起决定。

### 白板推导

若 page size 为 $P$、序列尾部有效 token 数为 $r$，单序列尾页浪费为 $P-r$；在长度对 $P$ 近似均匀时，平均尾页浪费约 $(P-1)/2$ tokens。
