...
Prefix caching 的收益近似由“可复用 token × 命中概率 × 被避免的 prefill 成本”决定；还要扣除 lookup、hash、缓存容量竞争和 eviction。高复用 system prompt/RAG 模板最适合。

### 白板推导

粗略收益：$T_{saved}\approx N_{hit}\cdot C_{prefill/token}-T_{lookup}-T_{evict/IO}$。命中长度比“命中请求数”更有解释力。
...
人为设置共享前缀比例 0/25/50/75/100%，并分别改变共享 prefix 长度，画 hit tokens 与 TTFT 的关系。
