---
id: Q017
title: "RadixAttention 与普通 Prefix Cache 有什么不同？"
chapter: "KV Cache 与 Attention"
difficulty: "★★★★★"
tags: ["SGLang", "RadixAttention"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q017｜RadixAttention 与普通 Prefix Cache 有什么不同？

> **定位**：KV Cache 与 Attention · **难度**：★★★★★  
> **关键词**：`SGLang` · `RadixAttention`

## 30 秒面试回答

> RadixAttention 用 radix tree 保存可共享的 token prefix，从而支持最长前缀匹配、分支复用与 cache-aware scheduling。它不仅是缓存结构，还把“哪些请求共享 prefix”暴露给调度器。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：RadixAttention 用 radix tree 保存可共享的 token prefix，从而支持最长前缀匹配、分支复用与 cache-aware scheduling。它不仅是缓存结构，还把“哪些请求共享 prefix”暴露给调度器。
...
公式公共 token 序列形成树的共享边；请求的 longest-prefix match 决定可复用 KV。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. RadixAttention 用 radix tree 保存可共享的 token prefix，从而支持最长前缀匹配、分支复用与 cache-aware
scheduling。它不仅是缓存结构，还把“哪些请求共享 prefix”暴露给调度器。
...
现场推演 ABCDEF、ABCGHI、ABCXYZ 共享 ABC；若调度器连续执行相近前缀请求，可以提高缓存局部性。
...
- 围绕“RadixAttention 与普通 Prefix Cache 有什么不同？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 把它等同于字符串 trie 而忽略 KV 引用、eviction 与调度
- ✗ 忽略 tokenization 后才是匹配单位。
...
- → 为什么 cache-aware scheduling 可能伤害公平性？
- → 多模态 prefix 怎么处理？
- → 与 hash-based prefix caching 比较。
...
RadixAttention 的关键是 longest-prefix match + radix tree 中的 KV 生命周期管理，并可把 cache locality 反馈给 scheduler；它因此同时是缓存结构和调度信号。
...
- [Q083 SGLang RadixAttention 相比普通 Prefix Caching 的核心优势？](../09-serving-runtimes/q083-sglang-radixattention.md)
- [Q086 SGLang HiCache 是什么？](../09-serving-runtimes/q086-hicache-sglang.md)
...
[← Q016](q016-prefix-cache-cache.md) · [02 KV Cache 与 Attention](index.md) · [Q018 →](q018-kv-quant-fp8.md)
