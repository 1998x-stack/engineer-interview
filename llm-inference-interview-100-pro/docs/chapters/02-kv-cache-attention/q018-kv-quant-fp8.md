...
> 长上下文与大并发下 KV 容量按 B×T 线性增长，而且 Decode 每步都要读历史 KV。将 BF16 KV 降为 FP8 理论上可约减半容量和流量，从而增加并发或上下文；但需要 runtime quant/dequant 与 attention kernel 原生支持。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：长上下文与大并发下 KV 容量按 B×T 线性增长，而且 Decode
每步都要读历史 KV。将 BF16 KV 降为 FP8 理论上可约减半容量和流量，从而增加并发或上下文；但需要 runtime
quant/dequant 与 attention kernel 原生支持。
...
公式 KV bytes ∝ dtype_bytes；BF16→FP8 约 2B→1B。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 长上下文与大并发下 KV 容量按 B×T 线性增长，而且 Decode 每步都要读历史 KV。将 BF16 KV 降为 FP8 理论上可约减半容量和流量，从而增加并发或上下文
- 2. 但需要 runtime quant/dequant 与 attention kernel 原生支持。
...
现场推演模型权重已经 INT4 但 128K context 仍 OOM 时，继续压权重收益有限，KV quantization 可能更直接。
...
- 围绕“为什么 KV Cache 也值得量化？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
截至 2026 年 8 月的 TensorRT-LLM 文档，KV Cache 已有 FP8 路径；在部分 Blackwell 配置上还提供 NVFP4 KV Cache。工程上应把“KV 容量减半/进一步压缩”的理论收益与目标 GPU 的实际 kernel 支持矩阵分开验证。

## 10.1 专家级深挖：把结论推到白板上

### 核心机制再抽象

长上下文时 KV 容量按 token 线性增长，decode attention 的读流量也随上下文增大。KV 量化同时改善容量和带宽，但需要考虑在线量化/反量化及长上下文误差。

### 白板推导

KV 从 BF16(2B) 降到 FP8(1B)，主体容量理论减半；但实际系统还包含 scale/metadata 与对齐，所以应以 allocator 实测为准。
...
- [Q045 FP8 与 INT8 的工程区别是什么？](../05-quantization/q045-fp8-int8.md)
- [Q048 为什么 KV Quantization 和 Weight Quantization 是两个问题？](../05-quantization/q048-kv-quant-dynamic.md)
...
[← Q017](q017-sglang-radixattention.md) · [02 KV Cache 与 Attention](index.md) · [Q019 →](q019-sliding-window-long-context.md)
