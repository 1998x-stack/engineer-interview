---
id: Q021
title: "Static Batching 和 Continuous Batching 有什么区别？"
chapter: "Batching 与 Scheduling"
difficulty: "★★★★★"
tags: ["continuous-batching", "scheduler"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q021｜Static Batching 和 Continuous Batching 有什么区别？

> **定位**：Batching 与 Scheduling · **难度**：★★★★★  
> **关键词**：`continuous-batching` · `scheduler`

## 30 秒面试回答

> Static batch 在一批请求全部完成前很难插入新请求，短请求会被长请求拖住；Continuous batching 在每个 decoding iteration 动态移除完成请求、加入新请求，提高 GPU 利用率并降低 head-of-line blocking。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：Static batch 在一批请求全部完成前很难插入新请求，短请求会被长请求拖住；Continuous batching 在每个 decoding iteration 动态移除完成请求、加入新请求，提高 GPU 利用率并降低 head-of-line blocking。
...
公式调度粒度从 request/batch 生命周期缩小到 iteration/token step。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. Static batch 在一批请求全部完成前很难插入新请求，短请求会被长请求拖住
- 2. Continuous batching 在每个 decoding iteration 动态移除完成请求、加入新请求，提高 GPU 利用率并降低 head-of-
line blocking。
...
现场推演一个 10-token 请求和 1000-token 请求同批，静态方式可能让短请求等待整个长请求；continuous batching 可在短请求完成后立刻释放 slot。
...
- 围绕“Static Batching 和 Continuous Batching 有什么区别？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 把 continuous batching 等同于动态 batch size
- ✗ 核心是 iteration-level admission/retirement。
...
- → 为什么 continuous batching 需要更灵活的 KV allocator？
- → 如何处理不同 sequence length？
- → 对 CUDA Graph 有何挑战？
...
Static batching 的 head-of-line blocking 来自 batch 生命周期绑定；continuous batching 把调度粒度降到 iteration/token，使完成请求及时退出、新请求及时加入。

### 白板推导

Static batch 的完成时间近似受 $\max_i L_{out,i}$ 支配；continuous batching 允许每个请求在完成时释放 slot，因此减少 head-of-line blocking。
...
- [Q082 vLLM V1 Scheduler 与早期设计有什么重要变化？](../09-serving-runtimes/q082-vllm-v1-scheduler.md)
...
[← Q020](../02-kv-cache-attention/q020-mla-deepseek.md) · [03 Batching 与 Scheduling](index.md) · [Q022 →](q022-orca-scheduling.md)
