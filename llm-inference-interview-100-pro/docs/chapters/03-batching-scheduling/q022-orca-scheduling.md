---
id: Q022
title: "Orca 的 Iteration-level Scheduling 为什么重要？"
chapter: "Batching 与 Scheduling"
difficulty: "★★★★☆"
tags: ["Orca", "scheduling"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q022｜Orca 的 Iteration-level Scheduling 为什么重要？
...
> Orca 把调度单位从完整 request 改成 iteration，允许对正在生成的序列与新请求细粒度重组；同时利用 selective batching 处理不同算子可否批处理的问题。它是现代 LLM serving 调度思想的重要起点。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：Orca 把调度单位从完整 request 改成 iteration，允许对正在生成的序列与新请求细粒度重组；同时利用 selective batching 处理不同算子可否批处理的问题。它是现代 LLM serving 调度思想的重要起点。
...
公式每个 decode iteration 都可重新形成 batch。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. Orca 把调度单位从完整 request 改成 iteration，允许对正在生成的序列与新请求细粒度重组
- 2. 同时利用 selective batching 处理不同算子可否批处理的问题。它是现代 LLM serving 调度思想的重要起点。
...
现场推演请求 A 结束后，下一 iteration 立即让等待队列中的 B 进入，无需等 batch 其余序列结束。
...
- 围绕“Orca 的 Iteration-level Scheduling 为什么重要？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 只记论文名不理解“为什么 request-level batching 浪费”。
...
- → Selective batching 是什么？
- → 它和 vLLM continuous batching 有何关系？
- → 迭代调度如何影响公平性？
...
Orca 的价值在于把“请求级调度”变为“迭代级调度”，并配合 selective batching 处理不同算子可否批处理的问题；它是现代 continuous batching 的重要思想来源。
...
[← Q021](q021-continuous-batching-scheduler.md) · [03 Batching 与 Scheduling](index.md) · [Q023 →](q023-prefill-interference-itl.md)
