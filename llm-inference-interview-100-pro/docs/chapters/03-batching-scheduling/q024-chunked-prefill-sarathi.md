---
id: Q024
title: "Chunked Prefill 为什么能解决这个问题？"
chapter: "Batching 与 Scheduling"
difficulty: "★★★★★"
tags: ["chunked-prefill", "Sarathi"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q024｜Chunked Prefill 为什么能解决这个问题？

> **定位**：Batching 与 Scheduling · **难度**：★★★★★  
> **关键词**：`chunked-prefill` · `Sarathi`

## 30 秒面试回答

> 将长 prompt 拆成若干 token chunk，每轮只用剩余 token budget 处理一部分 Prefill，并优先保护 Decode。这样限制单个 Prefill 的最长阻塞时间，同时可以把 compute-heavy Prefill 与 memory-heavy Decode 混到同一 iteration 提高利用率。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：将长 prompt 拆成若干 token chunk，每轮只用剩余 token
budget 处理一部分 Prefill，并优先保护 Decode。这样限制单个 Prefill 的最长阻塞时间，同时可以把 compute-heavy
Prefill 与 memory-heavy Decode 混到同一 iteration 提高利用率。
...
公式 Long Prefill T → ceil(T/C) chunks；C 受 max_num_batched_tokens/token budget 控制。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 将长 prompt 拆成若干 token chunk，每轮只用剩余 token budget 处理一部分 Prefill，并优先保护 Decode。这样限制单个 Prefill 的最长阻塞时间，同时可以把 compute-heavy Prefill 与 memory-heavy Decode 混到同一 iteration 提高利用率。
...
现场推演 32K prompt 不再一次吞掉整个 iteration，而可按 2K/4K chunk 与 decode 交替。
...
- 围绕“Chunked Prefill 为什么能解决这个问题？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

Chunk 太小会增加调度/launch 开销，太大又会重新制造 decode stall；最优值依 workload 与 SLO。

- ✗ chunk 越小越好
- ✗ 过小会增加调度、kernel 和重复边界处理开销。
...
- → chunk size 如何影响 TTFT 和 TPOT？
- → 为什么 vLLM V1 默认优先 Decode？
- → 多模态 Prefill 怎么切？
...
Chunked prefill 把长 prompt 切为 token chunks，允许 decode 在 chunk 之间插入；本质是用更多调度边界换取更稳定的 TPOT，同时控制每轮 token budget。

### 白板推导

长 prompt $T$ 切成 chunk $C$ 后约需 $\lceil T/C\rceil$ 个调度轮次。$C$ 越小，decode 插入机会越多，但调度/launch 开销上升。
...
sweep chunk size 512/1K/2K/4K/8K；观察长 prompt TTFT 与已有 decode TPOT 的 Pareto frontier。
...
[← Q023](q023-prefill-interference-itl.md) · [03 Batching 与 Scheduling](index.md) · [Q025 →](q025-vllm-token-budget.md)
