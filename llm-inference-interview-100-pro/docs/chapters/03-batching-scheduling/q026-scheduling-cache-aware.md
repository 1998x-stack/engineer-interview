---
id: Q026
title: "FCFS 和 Cache-aware Scheduling 怎么选？"
chapter: "Batching 与 Scheduling"
difficulty: "★★★★☆"
tags: ["scheduling", "cache-aware"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q026｜FCFS 和 Cache-aware Scheduling 怎么选？

> **定位**：Batching 与 Scheduling · **难度**：★★★★☆  
> **关键词**：`scheduling` · `cache-aware`

## 30 秒面试回答

> FCFS 简单、公平、可预测；Cache-aware 策略优先让 prefix 相近或缓存命中高的请求执行，可减少 Prefill 重算并提高吞吐。生产系统通常需在 cache benefit 与 waiting time/priority 之间加权，避免 starvation。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：FCFS 简单、公平、可预测；Cache-aware 策略优先让 prefix
相近或缓存命中高的请求执行，可减少 Prefill 重算并提高吞吐。生产系统通常需在 cache benefit 与 waiting time/priority
之间加权，避免 starvation。
...
公式 priority≈α·cache_saved_compute + β·waiting_age + γ·tenant_priority。
...
- 1. FCFS 简单、公平、可预测
- 2. Cache-aware 策略优先让 prefix 相近或缓存命中高的请求执行，可减少 Prefill 重算并提高吞吐。生产系统通常需在
cache benefit 与 waiting time/priority 之间加权，避免 starvation。
...
现场推演 Agent workload 大量共享 tool/system prefix 时 LPM/cache-aware 很有效；完全随机 chat 则收益弱。
...
- 围绕“FCFS 和 Cache-aware Scheduling 怎么选？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- → 如何设计 aging？
- → Cache locality 和 SLO 冲突时怎么办？
- → 如何量化 cache_saved_compute？
...
FCFS 优先公平与可预测；cache-aware 优先减少重复 prefill。生产上常加入 aging/deadline，让 cache benefit 不能无限压过等待时间。
...
[← Q025](q025-vllm-token-budget.md) · [03 Batching 与 Scheduling](index.md) · [Q027 →](q027-fairness-starvation.md)
