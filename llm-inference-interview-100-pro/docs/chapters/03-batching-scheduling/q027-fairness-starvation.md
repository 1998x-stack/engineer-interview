---
id: Q027
title: "如何解决调度中的 Starvation？"
chapter: "Batching 与 Scheduling"
difficulty: "★★★★☆"
tags: ["fairness", "starvation"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q027｜如何解决调度中的 Starvation？

> **定位**：Batching 与 Scheduling · **难度**：★★★★☆  
> **关键词**：`fairness` · `starvation`

## 30 秒面试回答

> 给等待时间引入 aging、设置最大等待时长或 deadline、按租户配额和优先级做加权；任何只追求 locality/短作业优先的策略都必须证明不会让某类请求无限等待。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：给等待时间引入 aging、设置最大等待时长或 deadline、按租户配额和优先级做加权；任何只追求 locality/短作业优先的策略都必须证明不会让某类请求无限等待。
...
公式 effective_priority = base_priority + f(wait_time)，其中 f 随等待时间单调增强。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 给等待时间引入 aging、设置最大等待时长或 deadline、按租户配额和优先级做加权
- 2. 任何只追求 locality/短作业优先的策略都必须证明不会让某类请求无限等待。
...
现场推演缓存命中高的请求不断到来时，纯 LPM 可能长期压住无共享 prefix 的请求；aging 能逐步提升后者权重。
...
- 围绕“如何解决调度中的 Starvation？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 只平均公平，不看 p99 tenant fairness
- ✗ 优先级倒置。
...
- → SRPT 是否适合 LLM？
- → 输出长度未知怎么估计？
- → 多租户 quota 如何和 cache locality 共存？
...
防 starvation 的常见方法是 aging、deadline、max-wait 或分层队列。高分回答应指出公平性本身也要定义：按 request、token、租户还是 SLO class。
...
[← Q026](q026-scheduling-cache-aware.md) · [03 Batching 与 Scheduling](index.md) · [Q028 →](q028-preemption-kv.md)
