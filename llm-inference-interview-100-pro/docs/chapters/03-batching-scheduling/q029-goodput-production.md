---
id: Q029
title: "为什么生产系统应该优化 Goodput，而不是最高 Throughput？"
chapter: "Batching 与 Scheduling"
difficulty: "★★★★★"
tags: ["goodput", "production"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q029｜为什么生产系统应该优化 Goodput，而不是最高 Throughput？

> **定位**：Batching 与 Scheduling · **难度**：★★★★★  
> **关键词**：`goodput` · `production`

## 30 秒面试回答

> Throughput 把超时请求也算“完成”，但业务只关心满足 TTFT/TPOT/端到端 SLO 的请求。Goodput 把性能目标和用户体验直接绑定，能够解释为什么过度 batch 虽提高 tokens/s 却让系统实际可服务能力下降。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：Throughput 把超时请求也算“完成”，但业务只关心满足
TTFT/TPOT/端到端 SLO 的请求。Goodput 把性能目标和用户体验直接绑定，能够解释为什么过度 batch 虽提高 tokens/s
却让系统实际可服务能力下降。
...
公式 Goodput(λ)=λ×P(request meets all SLOs)。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. Throughput 把超时请求也算“完成”，但业务只关心满足 TTFT/TPOT/端到端 SLO 的请求。Goodput 把性能目标和用户体验直接绑定，能够解释为什么过度 batch 虽提高 tokens/s 却让系统实际可服务能力下降。
...
现场推演调大 batch 后 throughput +15%，但 p99 TPOT 超标请求从 2% 升到 30%，Goodput 可能下降。
...
- 围绕“为什么生产系统应该优化 Goodput，而不是最高 Throughput？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- ✗ 只使用单一 SLO
- ✗ 真实服务往往有不同 priority/tenant/model 的多 SLO。
...
- → 如何测最大可持续 arrival rate？
- → Goodput 与 cost/token 如何联合优化？
- → SLO 违约是排队还是执行导致？
...
Throughput 把所有完成工作都算成功，Goodput 只计算满足 SLO 的请求；因此 batch/并发继续增大后通常先看到 throughput 增长、再看到 Goodput 在 knee point 后下降。

### 白板推导

若 offered load 为 $\lambda$，SLO 成功概率为 $p_{slo}$，则 Goodput=$\lambda p_{slo}$（在系统稳定且完成率接近 offered load 的区间）。
...
[← Q028](q028-preemption-kv.md) · [03 Batching 与 Scheduling](index.md) · [Q030 →](q030-pd-disaggregation-distserve.md)
