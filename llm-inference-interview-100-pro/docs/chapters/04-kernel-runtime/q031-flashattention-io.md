...
FlashAttention 通过 tiling 和 online softmax 避免显式物化完整 attention score matrix，减少 HBM 往返；数学结果仍是 exact attention。

### 白板推导

标准 attention 的 score matrix 是 $T\times T$；FlashAttention 通过 tile + online softmax 不在 HBM 物化完整矩阵，减少 $O(T^2)$ 级中间 IO。
...
同一 attention shape 比较 baseline SDPA/FA backend；记录 HBM bytes、kernel 时间和长序列 scaling，而不仅是 tokens/s。
