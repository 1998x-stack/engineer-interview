---
id: Q034
title: "Kernel Fusion 为什么能提升性能？"
chapter: "CUDA、Attention Kernel 与 Runtime"
difficulty: "★★★★☆"
tags: ["kernel-fusion", "CUDA"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q034｜Kernel Fusion 为什么能提升性能？

> **定位**：CUDA、Attention Kernel 与 Runtime · **难度**：★★★★☆  
> **关键词**：`kernel-fusion` · `CUDA`

## 30 秒面试回答

> 融合相邻算子可以减少中间 tensor 的 HBM 往返和 kernel launch，尤其适合 RMSNorm、bias、activation、 residual 等 memory-bound 小算子。是否值得融合取决于寄存器压力、occupancy 与可复用 kernel。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：融合相邻算子可以减少中间 tensor 的 HBM 往返和 kernel
launch，尤其适合 RMSNorm、bias、activation、residual 等 memory-bound 小算子。是否值得融合取决于寄存器压力、occupancy 与可复用 kernel。
...
公式 T_unfused≈Σ(T_launch+T_read+T_compute+T_write)；fusion 尽量合并 read/write 与 launch。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 融合相邻算子可以减少中间 tensor 的 HBM 往返和 kernel launch，尤其适合 RMSNorm、bias、activation、residual 等
memory-bound 小算子。是否值得融合取决于寄存器压力、occupancy 与可复用 kernel。
...
现场推演 RMSNorm→Linear 前后的某些 elementwise 逻辑融合后，可避免额外 HBM round-trip。
...
- 围绕“Kernel Fusion 为什么能提升性能？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- ✗ 无限融合
- ✗ 过大 kernel 可能导致 register spilling/occupancy 下降。
...
Fusion 的收益来自减少中间张量 HBM round-trip 与 launch；但融合过大也可能增加 register pressure、降低 occupancy 或妨碍调度，因此不是越多越好。

### 白板推导

Amdahl：若被融合算子只占端到端 $f$，即使该段无限加速，整体上限也只有 $1/(1-f)$。
...
[← Q033](q033-flashinfer-attention-backend.md) · [04 CUDA、Attention Kernel 与 Runtime](index.md) · [Q035 →](q035-cuda-graph-decode.md)
