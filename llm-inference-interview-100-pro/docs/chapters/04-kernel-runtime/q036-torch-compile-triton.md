---
id: Q036
title: "torch.compile / Triton 在推理优化中解决什么？"
chapter: "CUDA、Attention Kernel 与 Runtime"
difficulty: "★★★★☆"
tags: ["torch.compile", "Triton"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q036｜torch.compile / Triton 在推理优化中解决什么？

> **定位**：CUDA、Attention Kernel 与 Runtime · **难度**：★★★★☆  
> **关键词**：`torch.compile` · `Triton`

## 30 秒面试回答

> torch.compile 可做图级融合、常量传播、shape specialization 和 codegen；Triton 允许更快开发定制 GPU kernel。它们降低 Python/框架 overhead 并针对实际 shape 生成实现，但动态 shape、graph break 与 recompilation 可能导致 cold-start 或抖动。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：torch.compile 可做图级融合、常量传播、shape
specialization 和 codegen；Triton 允许更快开发定制 GPU kernel。它们降低 Python/框架 overhead 并针对实际 shape
生成实现，但动态 shape、graph break 与 recompilation 可能导致 cold-start 或抖动。
...
公式收益 = 更少 graph break/launch + 更优 fusion/kernel；代价 = compile time + code cache + specialization
complexity。
...
- 1. torch.compile 可做图级融合、常量传播、shape specialization 和 codegen
- 2. Triton 允许更快开发定制 GPU kernel。它们降低 Python/框架 overhead 并针对实际 shape 生成实现，但动态 shape、graph break 与 recompilation 可能导致 cold-start 或抖动。
...
现场推演同一线上服务 shape 分布稳定时 compile 缓存可复用；高度动态多模态输入可能导致大量 specialization。
...
- 围绕“torch.compile / Triton 在推理优化中解决什么？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 看到 compile=True 就期待稳定提速
- ✗ 未区分首次编译和 steady-state。
...
torch.compile 优化图级融合与 Python overhead，Triton 用于生成/定制 kernel；真正风险是 graph break、dynamic shape specialization、compile latency 与 recompilation。
...
[← Q035](q035-cuda-graph-decode.md) · [04 CUDA、Attention Kernel 与 Runtime](index.md) · [Q037 →](q037-launch-overhead-cuda.md)
