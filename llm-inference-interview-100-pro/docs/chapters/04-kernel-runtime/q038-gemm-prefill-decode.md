---
id: Q038
title: "为什么 Prefill GEMM 和 Decode GEMM 优化完全不同？"
chapter: "CUDA、Attention Kernel 与 Runtime"
difficulty: "★★★★★"
tags: ["GEMM", "prefill-decode"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q038｜为什么 Prefill GEMM 和 Decode GEMM 优化完全不同？

> **定位**：CUDA、Attention Kernel 与 Runtime · **难度**：★★★★★  
> **关键词**：`GEMM` · `prefill-decode`

## 30 秒面试回答

> Prefill 的矩阵 M 通常是 token 数×batch，较大；Decode M 可能只有 batch，形成 small-GEMM/GEMV。不同  M/N/K 使 tile、split-K、warp 数、epilogue 与 Tensor Core 利用率的最优点不同，因此不能用一个 kernel 配置覆盖全部。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：Prefill 的矩阵 M 通常是 token 数×batch，较大；Decode M
可能只有 batch，形成 small-GEMM/GEMV。不同 M/N/K 使 tile、split-K、warp 数、epilogue 与 Tensor Core 利用率的最优点不同，因此不能用一个 kernel 配置覆盖全部。
...
公式 Prefill [B·T,K]×[K,N]；Decode [B,K]×[K,N]。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. Prefill 的矩阵 M 通常是 token 数×batch，较大
- 2. Decode M 可能只有 batch，形成 small-GEMM/GEMV。不同 M/N/K 使 tile、split-K、warp 数、epilogue 与 Tensor
Core 利用率的最优点不同，因此不能用一个 kernel 配置覆盖全部。
...
现场推演同一个 FFN Linear，在 8K Prefill 可接近大 GEMM 峰值，在 batch=1 Decode 则受权重读取和 launch 限制。
...
- 围绕“为什么 Prefill GEMM 和 Decode GEMM 优化完全不同？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 只根据模型 hidden size 选 kernel，不看 M 维与实际 batch。
...
Prefill GEMM 的 M 往往是 token 数×batch，较大；decode 的 M 近似 batch，常很小。二者在 tile、split-K、persistent kernel 和 epilogue 上的最佳实现不同。
...
- [Q040 Nsight 里看到 GPU Util=100%，为什么模型仍可能很慢？](q040-nsight-profiling.md)
- [Q035 CUDA Graph 为什么适合 Decode？](q035-cuda-graph-decode.md)

---

[← Q037](q037-launch-overhead-cuda.md) · [04 CUDA、Attention Kernel 与 Runtime](index.md) · [Q039 →](q039-tensor-core-shape.md)
