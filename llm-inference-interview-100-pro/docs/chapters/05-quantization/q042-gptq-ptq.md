...
> GPTQ 是 post-training weight quantization，通过对一小批 calibration activation 估计二阶敏感度，逐列/分组量化并对尚未量化的权重做误差补偿，从而在 3/4-bit 下控制输出误差。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：GPTQ 是 post-training weight quantization，通过对一小批
calibration activation 估计二阶敏感度，逐列/分组量化并对尚未量化的权重做误差补偿，从而在 3/4-bit 下控制输出误差。
...
公式目标近似最小化 ||WX-W_qX||²，并用 Hessian H≈XXᵀ 的逆/近似指导补偿。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. GPTQ 是 post-training weight quantization，通过对一小批 calibration activation 估计二阶敏感度，逐列/分组量化并对尚未量化的权重做误差补偿，从而在 3/4-bit 下控制输出误差。
...
现场推演相同 4-bit，简单 round-to-nearest 可能破坏敏感方向，GPTQ 用校准数据捕捉这些方向。
...
- 围绕“GPTQ 原理是什么？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- → 为什么需要 calibration data？
- → group size 如何影响？
- → GPTQ 与 AWQ 的敏感度定义有何不同？
...
GPTQ 利用近似二阶信息，在逐块/逐列量化时用剩余权重补偿误差。工程上 calibration 数据、group size、act-order 和 kernel format 都会影响结果。
...
选择 3 个 calibration 分布（通用、代码、目标域），比较 GPTQ 后各域 accuracy 与 kernel 性能，验证 calibration shift。
...
[← Q041](q041-quantization-w4a16-w8a8.md) · [05 量化与低精度推理](index.md) · [Q043 →](q043-awq-quantization.md)
