...
SmoothQuant 利用等价变换 XW=(XS^{-1})(SW)，把 activation outlier 的量化难度迁移到更容易离线处理的 weight；关键超参决定迁移强度。

### 白板推导

对对角缩放矩阵 $S$，$XW=(XS^{-1})(SW)$。选择 $S$ 的目标是降低 activation 动态范围，同时不让 weight 变得过难量化。
