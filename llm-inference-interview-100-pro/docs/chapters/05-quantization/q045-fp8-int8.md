...
> INT8 是定点/整数表示，需要 scale 映射，范围与精度由 scale 强烈决定；FP8 带 exponent，动态范围更自然，并可由 Hopper/Blackwell Transformer Engine 原生支持。工程上需看具体 E4M3/E5M2、scale 粒度、accumulation dtype 和 kernel。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：INT8 是定点/整数表示，需要 scale 映射，范围与精度由 scale
强烈决定；FP8 带 exponent，动态范围更自然，并可由 Hopper/Blackwell Transformer Engine 原生支持。工程上需看具体 E4M3/E5M2、scale 粒度、accumulation dtype 和 kernel。
...
公式 FP8 的 exponent/mantissa 在动态范围与精度间折中；通常仍使用更高精度累加。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. INT8 是定点/整数表示，需要 scale 映射，范围与精度由 scale 强烈决定
- 2. FP8 带 exponent，动态范围更自然，并可由 Hopper/Blackwell Transformer Engine 原生支持。工程上需看具体
E4M3/E5M2、scale 粒度、accumulation dtype 和 kernel。
...
现场推演激活范围变化大的模型在 FP8 下可能比 INT8 更容易保持稳定，同时享受原生 Tensor Core。
...
- 围绕“FP8 与 INT8 的工程区别是什么？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- → E4M3 vs E5M2 怎么选？
- → FP8 weight+activation 如何校准？
- → FP8 KV 与 FP8 GEMM 是否同一问题？
...
FP8 有 exponent，动态范围通常比 INT8 更宽；INT8 scale/calibration 更敏感。硬件是否原生支持、accumulation dtype 与 scaling granularity 会决定实际收益。
...
[← Q044](q044-smoothquant-int8.md) · [05 量化与低精度推理](index.md) · [Q046 →](q046-fp4-nvfp4-blackwell.md)
