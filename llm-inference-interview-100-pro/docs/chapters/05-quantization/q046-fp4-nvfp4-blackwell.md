...
NVFP4 的价值来自极低权重带宽与 Blackwell 原生低精度 Tensor Core 路径，并配合细粒度 scaling 改善数值范围；若硬件/内核不支持，4-bit 只会变成存储格式。
...
同一 Blackwell GPU 上比较 FP8 与 NVFP4 的 weight bandwidth、GEMM throughput 与质量；若在非原生硬件上再测一次可验证硬件依赖。
