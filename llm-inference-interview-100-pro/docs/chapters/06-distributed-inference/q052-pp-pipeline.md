...
PP 按 layer 分 stage，通信主要是 stage 间 activation；推理时 bubble、microbatch 和单请求串行跨 stage 决定延迟。它常用于跨较慢节点减少频繁 all-reduce。

### 白板推导

PP 吞吐效率常用 bubble fraction 直觉理解：microbatch 越少、stage 越不均衡，bubble 占比越高；推理单请求还会串行穿过所有 stage。
