---
id: Q053
title: "Data Parallel inference 有什么意义？"
chapter: "分布式推理与通信"
difficulty: "★★★★☆"
tags: ["DP", "replica"]
...
# Q053｜Data Parallel inference 有什么意义？
...
> 每个 replica 持有完整模型或完整 TP/PP group，不同请求路由到不同 replica。它是最直接的横向扩容方式，适合独立请求和高 QPS；挑战在负载均衡、prefix/KV locality 与不同 replica 的队列状态。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：每个 replica 持有完整模型或完整 TP/PP group，不同请求路由到不同 replica。它是最直接的横向扩容方式，适合独立请求和高 QPS；挑战在负载均衡、prefix/KV locality 与不同
replica 的队列状态。
...
公式总容量≈replicas×单 replica goodput，前提是请求可均衡。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 每个 replica 持有完整模型或完整 TP/PP group，不同请求路由到不同 replica。它是最直接的横向扩容方式，适合独立请求和高 QPS
- 2. 挑战在负载均衡、prefix/KV locality 与不同 replica 的队列状态。
...
现场推演 TP=2 的模型组复制 4 份得到 4 个 DP replica，通常比单纯 TP=8 更适合高并发。
...
- 围绕“Data Parallel inference 有什么意义？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- → 如何做 KV-aware routing？
- → DP autoscaling 看 QPS 还是 token rate？
- → 不同 replica cache 如何共享？
...
DP 复制完整模型并分配不同请求，最适合独立请求横向扩展；它不能降低单 replica 的模型显存，但能提高 aggregate throughput 与容错/扩缩容灵活性。
...
[← Q052](q052-pp-pipeline.md) · [06 分布式推理与通信](index.md) · [Q054 →](q054-ep-moe.md)
