...
> EP 按 expert 维度分布 MoE 权重。router 决定 token 目标 expert，系统做 dispatch all-to-all、local grouped GEMM、再 combine。EP 让巨大的 expert 权重自然分摊，但通信模式从 TP 的 collective 变成数据依赖的 all-to-all。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：EP 按 expert 维度分布 MoE 权重。router 决定 token 目标
expert，系统做 dispatch all-to-all、local grouped GEMM、再 combine。EP 让巨大的 expert 权重自然分摊，但通信模式从 TP 的 collective 变成数据依赖的 all-to-all。
...
公式 token→top-k experts；T≈dispatch + expert_compute + combine。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. EP 按 expert 维度分布 MoE 权重。router 决定 token 目标 expert，系统做 dispatch all-to-all、local grouped GEMM、再 combine。EP 让巨大的 expert 权重自然分摊，但通信模式从 TP 的 collective 变成数据依赖的 all-to-all。
...
现场推演如果每 GPU 放一组 experts，token 可能跨节点发送；expert placement 与网络拓扑直接影响性能。
...
- 围绕“Expert Parallelism 是什么？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
EP 按 expert 放置权重，token 由 router 发送到目标 expert；它把容量分摊得很自然，但引入 all-to-all 和 expert imbalance。
...
[← Q053](q053-dp-replica.md) · [06 分布式推理与通信](index.md) · [Q055 →](q055-cp-long-context.md)
