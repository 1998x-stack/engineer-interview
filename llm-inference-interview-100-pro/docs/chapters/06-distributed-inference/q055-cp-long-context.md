---
id: Q055
title: "Context Parallelism 为什么越来越重要？"
chapter: "分布式推理与通信"
difficulty: "★★★★★"
tags: ["CP", "long-context"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q055｜Context Parallelism 为什么越来越重要？
...
> 超长 context 的 Prefill compute 和 KV 容量可能超过单 GPU。CP 沿 sequence/context 维切分，使多个 GPU 分担 attention/KV；Prefill CP 关注长 prompt 并行，Decode CP 还要高效聚合分片 KV 的 attention 结果。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：超长 context 的 Prefill compute 和 KV 容量可能超过单
GPU。CP 沿 sequence/context 维切分，使多个 GPU 分担 attention/KV；Prefill CP 关注长 prompt 并行，Decode CP 还要高效聚合分片 KV 的 attention 结果。
...
公式 Context T 分到 C ranks，每 rank 持有约 T/C 的 KV；需额外通信聚合 attention。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 超长 context 的 Prefill compute 和 KV 容量可能超过单 GPU。CP 沿 sequence/context 维切分，使多个 GPU 分担
attention/KV
- 2. Prefill CP 关注长 prompt 并行，Decode CP 还要高效聚合分片 KV 的 attention 结果。
...
现场推演 1M token context 即使模型权重可放下，KV 也可能无法单卡容纳，CP 成为容量手段。
...
- 围绕“Context Parallelism 为什么越来越重要？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

CP 解决长上下文容量/并行问题，但引入上下文维度通信与调度，不应只按“能放下”判断。

- ✗ 认为 CP 免费把显存除 C
- ✗ attention 需要跨分片规约，通信不可忽略。
...
- → Ring attention/sequence parallel 与 CP 的关系？
- → Decode CP 的通信是什么？
- → CP 和 P/D 怎么组合？
...
CP 按 sequence/context 维切分 attention/KV，长上下文时可分摊 KV 容量和 attention 工作；代价是跨 rank 合并 attention 状态或通信。

### 白板推导

从本题给出的基础公式出发，对关键自变量做敏感性分析：分别让 TP/PP/DP/EP/CP degree、microbatch、node placement、NVLink/IB topology、collective algorithm 中一个变量变化，判断容量、带宽、计算或通信项如何变化。现场推导时重点说明数量级和主导项，而不是追求小数点精度。
...
先单机 scale-up，再跨节点 scale-out；比较 1/2/4/8 卡效率并记录通信占比，避免只报 aggregate tokens/s。
...
- [Q019 Sliding Window Attention 如何降低推理复杂度？](../02-kv-cache-attention/q019-sliding-window-long-context.md)
...
[← Q054](q054-ep-moe.md) · [06 分布式推理与通信](index.md) · [Q056 →](q056-tp-pp-topology.md)
