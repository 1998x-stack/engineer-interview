...
> 看模型容量、单请求 latency、节点内/跨节点带宽与目标并发。高带宽 NVLink 域内适合 TP；跨节点若 AllReduce 成本太高，可增加 PP 或 DP。最常见是 topology-aware hybrid：节点内 TP，节点间 PP/EP/DP。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：看模型容量、单请求 latency、节点内/跨节点带宽与目标并发。高带宽 NVLink 域内适合 TP；跨节点若 AllReduce 成本太高，可增加 PP 或 DP。最常见是 topology-aware hybrid：节点内 TP，节点间 PP/EP/DP。
...
公式 Scale-up bandwidth 高 → TP；scale-out bandwidth/latency 较差 → 减少高频 collective。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 看模型容量、单请求 latency、节点内/跨节点带宽与目标并发。高带宽 NVLink 域内适合 TP
- 2. 跨节点若 AllReduce 成本太高，可增加 PP 或 DP。最常见是 topology-aware hybrid：节点内 TP，节点间 PP/EP/DP。
...
现场推演 2 节点×8 GPU，可考虑每节点 TP=8、PP=2，而不是 TP=16 跨 IB。
...
- 围绕“TP 和 PP 怎么选？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- ✗ 只看显存是否放得下
- ✗ 忽略 collective frequency 与 batch。
...
TP 更依赖低延迟高带宽 collective，PP 更依赖 stage balance 和 pipeline efficiency。拓扑从 NVLink 到跨节点 IB 后，最佳切分点会明显变化。
...
[← Q055](q055-cp-long-context.md) · [06 分布式推理与通信](index.md) · [Q057 →](q057-nccl-collectives.md)
