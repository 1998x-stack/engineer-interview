---
id: Q057
title: "LLM Tensor Parallel 最常出现哪些 collective？"
chapter: "分布式推理与通信"
difficulty: "★★★★☆"
...
# Q057｜LLM Tensor Parallel 最常出现哪些 collective？

> **定位**：分布式推理与通信 · **难度**：★★★★☆  
> **关键词**：`NCCL` · `collectives`

## 30 秒面试回答

> 主要是 AllReduce、AllGather、ReduceScatter，具体取决于 row/column parallel、sequence parallel 与 attention/MLP 切法。理解“哪一步传多少字节、每层几次”比死记框架配置更重要。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：主要是 AllReduce、AllGather、ReduceScatter，具体取决于
row/column parallel、sequence parallel 与 attention/MLP 切法。理解“哪一步传多少字节、每层几次”比死记框架配置更重要。
...
公式 T_comm≈α·messages + bytes/effective_bandwidth；小消息 latency α 主导，大消息 bandwidth 主导。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 主要是 AllReduce、AllGather、ReduceScatter，具体取决于 row/column parallel、sequence parallel 与
attention/MLP 切法。理解“哪一步传多少字节、每层几次”比死记框架配置更重要。
...
现场推演 Decode 的 token batch 小，collective payload 可能较小，此时 latency 比带宽更关键；Prefill 大矩阵则更看
bandwidth。
...
- 围绕“LLM Tensor Parallel 最常出现哪些 collective？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 把 NCCL bandwidth 峰值当实际
- ✗ 忽略算法 ring/tree 与拓扑。
...
- → 为什么 sequence parallel 常配 ReduceScatter？
- → NVLS 有何作用？
...
列并行/行并行 Linear 常配合 AllGather、ReduceScatter、AllReduce。需要能根据 tensor shape 估算 message bytes，并区分 latency-dominated 与 bandwidth-dominated 区间。

### 白板推导

对 tensor 元素数 $N$、dtype bytes $b$，collective payload 量级为 $Nb$；ring all-reduce 的链路流量还会乘与 world size 相关的因子。现场估算先算 payload，再谈算法常数。
...
[← Q056](q056-tp-pp-topology.md) · [06 分布式推理与通信](index.md) · [Q058 →](q058-topology-nvlink-ib.md)
