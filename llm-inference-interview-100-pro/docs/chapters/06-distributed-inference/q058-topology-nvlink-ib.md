...
拓扑决定“同样 8 卡”的通信成本。应画出 GPU—NVLink/NVSwitch—NIC—IB 的层级，把频繁 collective 尽量限制在 scale-up 域，把跨节点通信留给更粗粒度并行。
...
使用 nvidia-smi topo/实际拓扑，把 TP group 限制在 NVLink 域与跨节点两种配置，比较 collective 和 TPOT。
