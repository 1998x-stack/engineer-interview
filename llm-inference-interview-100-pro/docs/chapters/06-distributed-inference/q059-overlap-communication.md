---
id: Q059
title: "Communication/Computation Overlap 怎么做？"
chapter: "分布式推理与通信"
difficulty: "★★★★★"
tags: ["overlap", "communication"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q059｜Communication/Computation Overlap 怎么做？

> **定位**：分布式推理与通信 · **难度**：★★★★★  
> **关键词**：`overlap` · `communication`

## 30 秒面试回答

> 将独立或可分块的通信与 GEMM 放在不同 stream/pipeline 中重叠，目标把串行 T_compute+T_comm 逼近 max(T_compute,T_comm)。需要细粒度 chunk、正确依赖和足够 SM/Copy Engine/网络资源，否则会资源争抢。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：将独立或可分块的通信与 GEMM 放在不同 stream/pipeline 中重叠，目标把串行 T_compute+T_comm 逼近 max(T_compute,T_comm)。需要细粒度 chunk、正确依赖和足够
SM/Copy Engine/网络资源，否则会资源争抢。
...
公式理想 T≈max(T_compute,T_comm)，实际还受 synchronization 与 contention 影响。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 将独立或可分块的通信与 GEMM 放在不同 stream/pipeline 中重叠，目标把串行 T_compute+T_comm 逼近
max(T_compute,T_comm)。需要细粒度 chunk、正确依赖和足够 SM/Copy Engine/网络资源，否则会资源争抢。
...
现场推演 MoE 可让一批 expert GEMM 计算时异步 dispatch 下一批 token，或把 TP collective 与后续可独立计算重叠。
...
- 围绕“Communication/Computation Overlap 怎么做？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 以为异步 API 就自动 overlap
- ✗ 若 compute 占满 SM 或通信依赖未切开，仍串行。
...
Overlap 需要有真正独立的 compute/comm 工作，并合理使用 stream、分块和调度。若二者争用同一 HBM/NVLink 资源，理论上的 max(compute,comm) 很难实现。
...
- [Q058 为什么 NVLink、PCIe、InfiniBand 拓扑会直接改变最优Parallelism？](q058-topology-nvlink-ib.md)
...
[← Q058](q058-topology-nvlink-ib.md) · [06 分布式推理与通信](index.md) · [Q060 →](q060-moe-system-design.md)
