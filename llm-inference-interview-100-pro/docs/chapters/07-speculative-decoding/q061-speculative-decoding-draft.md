...
Spec decode 用便宜 proposal 一次提出多个 token，再由 target 并行验证，减少 target 串行迭代数。加速上限取决于 accepted tokens/target step，而不是 draft 生成速度单独决定。

### 白板推导

定义一次 target verification 平均推进 $A$ 个 token，target verification 时间 $T_v$、draft 时间 $T_d$，则有效速度约 $A/(T_d+T_v)$；与 baseline $1/T_t$ 比较即可判断净收益。
...
对 spec length 1/2/4/8、不同 draft，记录 accepted tokens/step 与 TPOT；在并发 1、8、64 下重复。
