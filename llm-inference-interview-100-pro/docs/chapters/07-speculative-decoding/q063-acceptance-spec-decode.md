---
id: Q063
title: "Acceptance Rate 为什么是 Spec Decode 最关键指标之一？"
chapter: "Speculative Decoding"
difficulty: "★★★★★"
tags: ["acceptance", "spec-decode"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q063｜Acceptance Rate 为什么是 Spec Decode 最关键指标之一？

> **定位**：Speculative Decoding · **难度**：★★★★★  
> **关键词**：`acceptance` · `spec-decode`

## 30 秒面试回答

> 它直接决定每次昂贵 target verification 能推进多少 token。acceptance 高但 draft 太慢、或 acceptance 低但 draft 很便宜，都需要综合成本；因此更准确指标是平均 accepted length 与单位时间推进率。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：它直接决定每次昂贵 target verification 能推进多少 token。acceptance 高但 draft 太慢、或 acceptance 低但 draft 很便宜，都需要综合成本；因此更准确指标是平均 accepted
length 与单位时间推进率。
...
公式 effective_tokens_per_target_call≈E[accepted_prefix_length]+可能的 correction token。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 它直接决定每次昂贵 target verification 能推进多少 token。acceptance 高但 draft 太慢、或 acceptance 低但 draft 很便宜，都需要综合成本
- 2. 因此更准确指标是平均 accepted length 与单位时间推进率。
...
现场推演代码/模板重复场景通常更可预测，acceptance 可能高；开放式高温生成 acceptance 往往下降。

### 建议实验

记录每请求 accepted length 分布，而不只平均 acceptance；对 draft depth 做 sweep，计算 (accepted tokens)/(draft+verify time) 与真实 TPOT speedup 的相关性。
...
- 围绕“Acceptance Rate 为什么是 Spec Decode 最关键指标之一？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 只报 acceptance percentage，不报 draft/verify latency。
...
- → 如何在线调 speculative length？
- → 高温为什么影响 acceptance？
- → 不同 domain 是否应该用不同 draft？
...
acceptance 决定一次昂贵 target verification 能推进多少 token。必须结合 draft/verify 成本看“有效接受 token/毫秒”，不能只追求百分比。

### 白板推导

平均 accepted length 比单纯 acceptance percentage 更直接。若 spec length 为 $k$，则 $E[A]=\sum_{i=1}^{k}P(\text{前 }i\text{ 个均接受})$。
...
**典型执行路径**：Draft/proposal → candidate tree/sequence → target verification → accept/reject → KV commit。加速来自减少 target 串行 decode steps，而不是凭空减少所有计算。

- 记录每轮 proposal length、accepted length、rejection position 与 verify latency，才能解释波动。
- 确认 KV commit/rollback 是否造成额外复制或 allocator 压力。
...
在低/中/高并发分别 sweep speculation length；同时记录 acceptance 和 target batch，确认收益不是 workload 变化造成。
...
draft size、spec length、temperature、acceptance、target batch、verification backend。一次实验尽量只改变一个关键变量，复杂系统再用二维 sweep 验证交互项。

### 观测量

accepted tokens/step、acceptance rate、draft/verify latency、TPOT、target forward 次数、GPU utilization。此外保存 profiler trace 与原始 per-request 数据，不只保存聚合平均值。
...
高级回答应以 accepted tokens per target step 与成本模型讨论，而不是只说“多预测几个 token”。
...
[← Q062](q062-speculative-sampling-correctness.md) · [07 Speculative Decoding](index.md) · [Q064 →](q064-draft-model-tradeoff.md)
