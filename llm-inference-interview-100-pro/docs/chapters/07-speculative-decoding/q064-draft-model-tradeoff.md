---
id: Q064
title: "Draft Model 为什么不能越小越好？"
chapter: "Speculative Decoding"
difficulty: "★★★★☆"
tags: ["draft-model", "tradeoff"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q064｜Draft Model 为什么不能越小越好？

> **定位**：Speculative Decoding · **难度**：★★★★☆  
> **关键词**：`draft-model` · `tradeoff`

## 30 秒面试回答

> 更小 draft 计算便宜，但和 target 分布偏差大，acceptance 降低；更大 draft 更准确却消耗更多算力/显存。最优点取决于 target 大小、batch、硬件、输出域和 speculative length。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：更小 draft 计算便宜，但和 target 分布偏差大，acceptance
降低；更大 draft 更准确却消耗更多算力/显存。最优点取决于 target 大小、batch、硬件、输出域和 speculative length。
...
公式优化目标 max E[accepted_tokens]/(T_draft+T_verify)。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 更小 draft 计算便宜，但和 target 分布偏差大，acceptance 降低
- 2. 更大 draft 更准确却消耗更多算力/显存。最优点取决于 target 大小、batch、硬件、输出域和 speculative length。
...
现场推演若 draft 从 7B 换 1B 速度 3×，但平均接受从 4 token 降到 1.2 token，整体可能反而变慢。
...
- 围绕“Draft Model 为什么不能越小越好？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- → 如何选择 draft/target ratio？
- → draft 与 target 是否必须同词表？
- → 共享层/feature 能否替代完整 draft？
...
draft 越小通常越快但预测质量更差；越大 acceptance 高但自身成本也高。最佳 draft 是 target、batch、温度和模型相似度共同决定的。
...
[← Q063](q063-acceptance-spec-decode.md) · [07 Speculative Decoding](index.md) · [Q065 →](q065-ngram-speculation.md)
