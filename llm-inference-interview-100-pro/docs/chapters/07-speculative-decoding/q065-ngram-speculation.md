---
id: Q065
title: "N-gram / Suffix speculation 为什么不需要 Draft Model？"
chapter: "Speculative Decoding"
difficulty: "★★★★☆"
tags: ["ngram", "speculation"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q065｜N-gram / Suffix speculation 为什么不需要 Draft Model？

> **定位**：Speculative Decoding · **难度**：★★★★☆  
> **关键词**：`ngram` · `speculation`

## 30 秒面试回答

> 它从当前上下文中寻找已出现的 token n-gram/suffix 模式，用其后续 token 作为 proposal。对代码、JSON、复制型文本成本极低，不需要额外权重与 KV；泛化能力则明显弱于 learned draft。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：它从当前上下文中寻找已出现的 token n-gram/suffix 模式，用其后续 token 作为 proposal。对代码、JSON、复制型文本成本极低，不需要额外权重与 KV；泛化能力则明显弱于
learned draft。
...
公式 proposal 来自 context reuse，而非神经网络 forward。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 它从当前上下文中寻找已出现的 token n-gram/suffix 模式，用其后续 token 作为 proposal。对代码、JSON、复制型文本成本极低，不需要额外权重与 KV
- 2. 泛化能力则明显弱于 learned draft。
...
现场推演生成结构化 JSON 或复制文档片段时，上下文已有相同模式，n-gram proposal 很可能连续命中。
...
- 围绕“N-gram / Suffix speculation 为什么不需要 Draft Model？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- → 如何实现高效 suffix lookup？
- → 与 prefix cache 是否共享数据结构？
- → 什么时候自动关闭？
...
N-gram/suffix 利用当前上下文中已经出现的 token pattern 作为 proposal，几乎无模型开销；代码、JSON、重复文本场景命中率高，自由文本则可能很低。
...
[← Q064](q064-draft-model-tradeoff.md) · [07 Speculative Decoding](index.md) · [Q066 →](q066-medusa-spec-decode.md)
