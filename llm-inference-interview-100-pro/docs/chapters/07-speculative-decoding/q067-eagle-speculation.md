---
id: Q067
title: "EAGLE 与普通 Draft Model 的关键区别？"
chapter: "Speculative Decoding"
difficulty: "★★★★★"
tags: ["EAGLE", "speculation"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q067｜EAGLE 与普通 Draft Model 的关键区别？

> **定位**：Speculative Decoding · **难度**：★★★★★  
> **关键词**：`EAGLE` · `speculation`

## 30 秒面试回答

> EAGLE 类方法利用 target 的内部 hidden feature，预测未来 feature/token，再让 target 验证；因为 feature 比纯 token 分布包含更多目标模型信息，proposal 质量通常更高。EAGLE-2 进一步根据置信度动态扩展树。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：EAGLE 类方法利用 target 的内部 hidden feature，预测未来
feature/token，再让 target 验证；因为 feature 比纯 token 分布包含更多目标模型信息，proposal 质量通常更高。EAGLE-2 进一步根据置信度动态扩展树。
...
公式 feature-conditioned draft → dynamic tree → target verification。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. EAGLE 类方法利用 target 的内部 hidden feature，预测未来 feature/token，再让 target 验证
- 2. 因为 feature 比纯 token 分布包含更多目标模型信息，proposal 质量通常更高。EAGLE-2 进一步根据置信度动态扩展树。
...
现场推演相比完全独立的小语言模型，EAGLE 与 target 表示空间更贴近，可能在相同 draft cost 下提高 acceptance。
...
- 围绕“EAGLE 与普通 Draft Model 的关键区别？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 只记“EAGLE 更快”不理解需要额外训练与 runtime support。
...
- → 为什么 feature prediction 更容易？
- → dynamic tree 如何选择节点？
- → EAGLE 对 batch throughput 有何影响？
...
EAGLE 在 feature space 预测未来，并利用 target 内部特征提高 proposal 质量；EAGLE-2 等方法进一步动态构造候选树，避免固定宽度浪费。

### 白板推导

从本题给出的基础公式出发，对关键自变量做敏感性分析：分别让 draft size、spec length、temperature、acceptance、target batch、verification backend 中一个变量变化，判断容量、带宽、计算或通信项如何变化。现场推导时重点说明数量级和主导项，而不是追求小数点精度。
...
[← Q066](q066-medusa-spec-decode.md) · [07 Speculative Decoding](index.md) · [Q068 →](q068-mtp-deepseek.md)
