---
id: Q068
title: "MTP 为什么既可以用于训练，也可以用于推理加速？"
...
> Multi-Token Prediction 在训练时让模型同时预测未来多个 token，作为辅助目标提高表示学习；推理时这些额外 head/模块可直接产出 speculative proposals，因此训练目标与 serving 加速共享结构。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：Multi-Token Prediction 在训练时让模型同时预测未来多个
token，作为辅助目标提高表示学习；推理时这些额外 head/模块可直接产出 speculative proposals，因此训练目标与
serving 加速共享结构。
...
公式 L = L_next-token + Σ_k λ_k L_{t+k}；推理时多步预测作为 draft。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. Multi-Token Prediction 在训练时让模型同时预测未来多个 token，作为辅助目标提高表示学习
- 2. 推理时这些额外 head/模块可直接产出 speculative proposals，因此训练目标与 serving 加速共享结构。
...
现场推演模型原生带 MTP head 时，无需再部署完全独立 draft，可在特定框架里做 speculative decoding。
...
- 围绕“MTP 为什么既可以用于训练，也可以用于推理加速？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 把训练 MTP 等同于一次 forward 直接无损输出多个 token
- ✗ 最终仍需验证/接受机制。
...
MTP 把未来多个 token 预测加入训练目标，使模型天然拥有 proposal 信号；推理时可复用为 speculative decoding，体现 training-inference co-design。
...
[← Q067](q067-eagle-speculation.md) · [07 Speculative Decoding](index.md) · [Q069 →](q069-tree-attention-verification.md)
