...
高 QPS 时 target 已经通过 batching 获得很好利用率，speculation 增加 draft 和 verification 反而可能抢资源；短输出和低 acceptance 也常负优化。

### 白板推导

当 $T_d+T_v>A\cdot T_t$ 时 speculation 负优化；高 batch 下 $T_t$ 可能已经很小，因此更容易触发。
