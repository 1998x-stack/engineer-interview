---
id: Q075
title: "Shared Expert 有什么意义？"
chapter: "MoE、MLA 与模型-系统协同"
difficulty: "★★★★☆"
tags: ["shared-expert", "MoE"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q075｜Shared Expert 有什么意义？

> **定位**：MoE、MLA 与模型-系统协同 · **难度**：★★★★☆  
> **关键词**：`shared-expert` · `MoE`

## 30 秒面试回答

> 把跨 token 普遍需要的能力放入始终激活的 shared expert，把稀疏 specialist 放 routed experts，可减少某些共享知识被重复学习、改善专家分工。系统上 shared expert 是固定计算，可与 routed expert 计算/通信做 overlap。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：把跨 token 普遍需要的能力放入始终激活的 shared expert，把稀疏 specialist 放 routed experts，可减少某些共享知识被重复学习、改善专家分工。系统上 shared expert 是固定计算，可与 routed expert 计算/通信做 overlap。
...
公式 output = shared_expert(x) + Σ gate_i·expert_i(x)。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 把跨 token 普遍需要的能力放入始终激活的 shared expert，把稀疏 specialist 放 routed experts，可减少某些共享知识被重复学习、改善专家分工。系统上 shared expert 是固定计算，可与 routed expert 计算/通信做 overlap。
...
现场推演如果 shared expert 与 A2A dispatch 同时执行，可用本地计算隐藏部分网络延迟。
...
- 围绕“Shared Expert 有什么意义？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 只从模型质量解释 shared expert
- ✗ 它还改变 runtime 的可重叠工作。
...
- → shared expert 是否也做 TP？
- → 如何 overlap？
- → shared expert 太大有什么问题？
...
shared expert 为所有 token 提供公共容量，减少全部知识都由 routed expert 承担；系统上它是 dense path，会影响每 token 固定计算与 TP placement。
...
[← Q074](q074-alltoall-moe.md) · [08 MoE、MLA 与模型-系统协同](index.md) · [Q076 →](q076-mla-gqa-kv.md)
