...
> GQA 通过减少 KV head 数压缩缓存；MLA 进一步缓存低维 latent representation，通常能把 KV 压得更激进。比较不能只看名词，要代入模型具体 latent dim、RoPE dim 与缓存布局。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：GQA 通过减少 KV head 数压缩缓存；MLA 进一步缓存低维
latent representation，通常能把 KV 压得更激进。比较不能只看名词，要代入模型具体 latent dim、RoPE dim 与缓存布局。
...
公式 GQA KV∝H_kv·D_head；MLA KV≈D_latent + 部分 positional components。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. GQA 通过减少 KV head 数压缩缓存
- 2. MLA 进一步缓存低维 latent representation，通常能把 KV 压得更激进。比较不能只看名词，要代入模型具体 latent
dim、RoPE dim 与缓存布局。
...
现场推演在相同 hidden size 下，MLA 若 latent dim 远小于 H_kv·D_head，长上下文显存优势显著。
...
- 围绕“MLA 与 GQA 谁更省 KV Cache？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

MLA 的 KV 压缩潜力大，但需要模型结构支持与专用 kernel；不能把它作为对任意 GQA 模型的运行时开关。

- ✗ 忽略 MLA 需要重构 K/V 的额外计算与 kernel 复杂度。
...
- → 如何从 config 计算 MLA 每 token KV？
- → 为什么 decode kernel 可能更复杂？
- → 量化 MLA cache 有何不同？
...
GQA 通过减少 KV heads 降低 KV；MLA 更进一步压缩到 latent。比较时应以 bytes/token、attention kernel 和重构计算综合衡量，而不是只看 head 数。

### 白板推导

比较方案时统一换算为 KV bytes/token，而不是只比 heads：$bytes/token=2L\times\text{stored-KV-dim}\times b$。
...
[← Q075](q075-shared-expert-moe.md) · [08 MoE、MLA 与模型-系统协同](index.md) · [Q077 →](q077-deepseek-framework.md)
