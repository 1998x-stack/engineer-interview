---
id: Q078
title: "MoE 量化为什么比 Dense 模型更复杂？"
chapter: "MoE、MLA 与模型-系统协同"
difficulty: "★★★★☆"
tags: ["MoE-quant", "calibration"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q078｜MoE 量化为什么比 Dense 模型更复杂？

> **定位**：MoE、MLA 与模型-系统协同 · **难度**：★★★★☆  
> **关键词**：`MoE-quant` · `calibration`

## 30 秒面试回答

> 不同 expert 的 activation/权重分布、调用频率和 calibration coverage 不一致。冷门 expert 在校准集上样本不足，统一 scale 或统一策略可能损伤严重；量化后 expert kernel 还要支持 grouped GEMM 与不规则 M。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：不同 expert 的 activation/权重分布、调用频率和 calibration
coverage 不一致。冷门 expert 在校准集上样本不足，统一 scale 或统一策略可能损伤严重；量化后 expert kernel 还要支持
grouped GEMM 与不规则 M。
...
公式 Calibration should cover routing distribution; scale may need per-expert/per-group granularity。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 不同 expert 的 activation/权重分布、调用频率和 calibration coverage 不一致。冷门 expert 在校准集上样本不足，统一
scale 或统一策略可能损伤严重
- 2. 量化后 expert kernel 还要支持 grouped GEMM 与不规则 M。
...
现场推演若某 expert 只在数学输入被激活，而 calibration 主要是通用聊天，它的 INT4 scale 可能失真。
...
- 围绕“MoE 量化为什么比 Dense 模型更复杂？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- ✗ 随机抽几百条文本校准所有 expert
- ✗ 不统计 expert activation coverage。
...
- → 如何构造 calibration routing coverage？
- → 不同 expert 可否用不同 bit？
- → 量化会改变 routing 吗？
...
MoE calibration 必须覆盖不同 expert。若某 expert 在 calibration 数据中很少激活，它的 scale/clip 估计可能失真；还要分别考虑 router、shared expert 和 routed expert。

### 白板推导

从本题给出的基础公式出发，对关键自变量做敏感性分析：分别让 top-k、expert count、EP/TP degree、tokens/expert、capacity/load balance、KV representation、dtype 中一个变量变化，判断容量、带宽、计算或通信项如何变化。现场推导时重点说明数量级和主导项，而不是追求小数点精度。
...
- [Q077 DeepSeek 类模型为什么对推理框架提出新要求？](q077-deepseek-framework.md)
- [Q079 Wide Expert Parallelism 为什么可能比大规模 TP 更适合 MoE？](q079-wide-ep-moe.md)
- [Q076 MLA 与 GQA 谁更省 KV Cache？](q076-mla-gqa-kv.md)
- [Q080 MoE 为什么低并发和高并发性能差异特别大？](q080-moe-batching.md)
- [Q075 Shared Expert 有什么意义？](q075-shared-expert-moe.md)

---

[← Q077](q077-deepseek-framework.md) · [08 MoE、MLA 与模型-系统协同](index.md) · [Q079 →](q079-wide-ep-moe.md)
