---
id: Q079
title: "Wide Expert Parallelism 为什么可能比大规模 TP 更适合 MoE？"
...
# Q079｜Wide Expert Parallelism 为什么可能比大规模 TP 更适合 MoE？
...
> 把更多 GPU 用于放不同 expert，可以避免每个小 expert GEMM 再做高频 TP collective；代价转为 all-to-all。若网络支持高效 dispatch，wide EP 能让 expert 计算本地化并提升总容量。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：把更多 GPU 用于放不同 expert，可以避免每个小 expert
GEMM 再做高频 TP collective；代价转为 all-to-all。若网络支持高效 dispatch，wide EP 能让 expert 计算本地化并提升总容量。
...
公式 Tradeoff: smaller TP collective per expert vs larger EP all-to-all domain。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 把更多 GPU 用于放不同 expert，可以避免每个小 expert GEMM 再做高频 TP collective
- 2. 代价转为 all-to-all。若网络支持高效 dispatch，wide EP 能让 expert 计算本地化并提升总容量。
...
现场推演单 expert 已能一张 GPU 高效计算时，继续 TP=8 只会让每 expert M 更小并增加 collective；EP 扩展可能更合理。
...
- 围绕“Wide Expert Parallelism 为什么可能比大规模 TP 更适合 MoE？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- ✗ 一味增大 EP
- ✗ all-to-all peer 数和跨节点 latency 也会恶化。
...
- → 如何根据 expert hidden size 决定 TP？
- → EP domain 是否限制在节点内？
- → expert replication 如何降低跨节点？
...
Wide EP 让一个 expert 尽量保持完整 GEMM，避免给每个 expert 再做大 TP；收益前提是 A2A 网络强、每 expert token batch 足够大。
...
[← Q078](q078-moe-quant-calibration.md) · [08 MoE、MLA 与模型-系统协同](index.md) · [Q080 →](q080-moe-batching.md)
