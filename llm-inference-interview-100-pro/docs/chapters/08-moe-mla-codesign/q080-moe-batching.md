...
> 低并发时每 expert 得到的 token 很少，形成 tiny GEMM、kernel launch 与通信延迟占主导；高并发可以跨请求聚合更多 token，让 grouped GEMM 变大、通信消息更饱满，提高硬件利用率。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：低并发时每 expert 得到的 token 很少，形成 tiny GEMM、kernel launch 与通信延迟占主导；高并发可以跨请求聚合更多 token，让 grouped GEMM 变大、通信消息更饱满，提高硬件利用率。
...
公式 tokens_per_expert≈batch_tokens×top_k/num_experts（只是一阶均匀估算）。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 低并发时每 expert 得到的 token 很少，形成 tiny GEMM、kernel launch 与通信延迟占主导
- 2. 高并发可以跨请求聚合更多 token，让 grouped GEMM 变大、通信消息更饱满，提高硬件利用率。
...
现场推演 EP=64、每 step 仅 64 token、top-2 时平均每 expert 只有约 2 token，几乎无法形成高效 GEMM。
...
- 围绕“MoE 为什么低并发和高并发性能差异特别大？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 只在高并发 benchmark 得出模型“很快”
- ✗ 交互低 QPS 可能完全不同。
...
- → 如何 microbatch expert tokens？
- → spec decode 会改变 expert batch 吗？
- → P/D 分离对 MoE 有何帮助？
...
从框架内部抽象到技术选型，理解缓存、调度、kernel、量化与 P/D 能力。

本章题目 Q081 - Q090 · 共 10 题

本章学习目标
- 能解释三个主流框架的内部重点与版本差异。
- 能做 feature fit + benchmark 的技术选型。
- 理解 cache、structured output、LoRA 与 cold-start。
...
并发低时每 expert 只有很少 token，grouped GEMM 形状差且 A2A 小消息占比高；并发升高后 token 聚合改善 GEMM，但过高又会碰到 queue/SLO。
...
[← Q079](q079-wide-ep-moe.md) · [08 MoE、MLA 与模型-系统协同](index.md) · [Q081 →](../09-serving-runtimes/q081-vllm-architecture.md)
