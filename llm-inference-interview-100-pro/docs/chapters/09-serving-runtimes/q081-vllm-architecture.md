---
id: Q081
title: "请描述 vLLM 的核心架构思想。"
chapter: "vLLM / SGLang / TensorRT-LLM"
difficulty: "★★★★★"
tags: ["vLLM", "architecture"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q081｜请描述 vLLM 的核心架构思想。

> **定位**：vLLM / SGLang / TensorRT-LLM · **难度**：★★★★★  
> **关键词**：`vLLM` · `architecture`

## 30 秒面试回答

> 从服务视角可拆为 API/request processor、scheduler、KV cache manager、model runner/executor、 attention/GEMM kernels 与 sampler。核心价值是把动态请求调度、分页 KV、连续批处理和硬件 kernel 统一到高吞吐 runtime 中。V1 又进一步统一 token budget 与执行架构。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：从服务视角可拆为 API/request processor、scheduler、KV
cache manager、model runner/executor、attention/GEMM kernels 与 sampler。核心价值是把动态请求调度、分页
KV、连续批处理和硬件 kernel 统一到高吞吐 runtime 中。V1 又进一步统一 token budget 与执行架构。
...
公式 Request → Scheduler → KV allocation → Model Runner → Kernels → Sampler/Output。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 从服务视角可拆为 API/request processor、scheduler、KV cache manager、model
runner/executor、attention/GEMM kernels 与 sampler。核心价值是把动态请求调度、分页 KV、连续批处理和硬件
kernel 统一到高吞吐 runtime 中。V1 又进一步统一 token budget 与执行架构。
...
现场推演理解架构后才能解释为什么 max_num_batched_tokens、prefix cache 或 preemption 会影响同一条请求的
TTFT/TPOT。

### 建议实验

对一个固定模型记录 scheduler token budget、KV usage、batch composition 与 attention backend；用一次请求从 API 层追踪到 executor/kernel，画出最小架构图。
...
- 围绕“请描述 vLLM 的核心架构思想。”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- → SchedulerOutput 包含什么？
- → KV manager 和 scheduler 如何交互？
- → model runner 如何利用 CUDA Graph？
...
理解 vLLM 应以“scheduler + KV cache manager + model runner + worker/executor + kernels”分层，而不是只背 PagedAttention。V1 进一步统一核心抽象以减少历史分支。
...
- [Q025 Scheduler 中的 max_num_batched_tokens 本质是什么？](../03-batching-scheduling/q025-vllm-token-budget.md)
- [Q082 vLLM V1 Scheduler 与早期设计有什么重要变化？](q082-vllm-v1-scheduler.md)
...
[← Q080](../08-moe-mla-codesign/q080-moe-batching.md) · [09 vLLM / SGLang / TensorRT-LLM](index.md) · [Q082 →](q082-vllm-v1-scheduler.md)
