---
id: Q083
title: "SGLang RadixAttention 相比普通 Prefix Caching 的核心优势？"
...
# Q083｜SGLang RadixAttention 相比普通 Prefix Caching 的核心优势？
...
> 它将共享 prefix 组织成 radix tree，可精确追踪部分前缀、分支与引用，并让 scheduler 利用 longest-prefix match 做 locality-aware routing。对多轮、agent、few-shot 等结构化程序尤其有价值。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：它将共享 prefix 组织成 radix tree，可精确追踪部分前缀、分支与引用，并让 scheduler 利用 longest-prefix match 做 locality-aware routing。对多轮、agent、few-shot 等结构化程序尤其有价值。
...
公式 tree node/edge 对应 token prefix 与 KV blocks；scheduler 依据 match length 评估 reuse。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 它将共享 prefix 组织成 radix tree，可精确追踪部分前缀、分支与引用，并让 scheduler 利用 longest-prefix match 做
locality-aware routing。对多轮、agent、few-shot 等结构化程序尤其有价值。
...
现场推演多个 agent 请求共享系统工具描述和前几轮消息时，树结构可复用到分叉点。
...
- 围绕“SGLang RadixAttention 相比普通 Prefix Caching 的核心优势？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 只比较 cache lookup 数据结构速度
- ✗ 真正价值是缓存+调度共同设计。
...
- → eviction 怎么做？
- → Radix tree 与 paged KV 如何映射？
- → DP replica 之间如何做 cache-aware routing？
...
SGLang 的 RadixAttention 把 prefix KV 存在 radix tree，并将 longest-prefix reuse 与 cache-aware scheduling 联动；这比“是否命中完整 prompt hash”更细粒度。
...
[← Q082](q082-vllm-v1-scheduler.md) · [09 vLLM / SGLang / TensorRT-LLM](index.md) · [Q084 →](q084-framework-selection-trtllm-vllm.md)
