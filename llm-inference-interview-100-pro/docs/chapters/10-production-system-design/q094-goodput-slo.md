...
Goodput 把 SLO 直接放进吞吐定义。画 offered load—throughput—goodput 曲线时，Goodput 的峰值附近通常就是容量规划和 admission control 的关键区间。

### 白板推导

Goodput=$\#\{request\ meets\ SLO\}/\Delta t$。如果只有 TPOT SLO 或只有 TTFT SLO，应明确成功谓词。
