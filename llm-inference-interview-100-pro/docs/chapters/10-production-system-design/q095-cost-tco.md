...
成本不能只用 GPU 租金/峰值 tokens/s；还要计 idle headroom、replica redundancy、prefill/输出差异、失败重试、CPU/网络和利用率。建议分 input/output token 成本。

### 白板推导

$Cost/Mtok=\frac{C_{gpu/h}\cdot N_{gpu}+C_{other/h}}{tokens/h}\times10^6$。实际还应把平均利用率和冗余 headroom 放进分母/分子。
