---
id: Q096
title: "LLM Serving 如何做 Admission Control 和 Autoscaling？"
chapter: "Benchmark、生产部署与系统设计"
difficulty: "★★★★★"
tags: ["admission-control", "autoscaling"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q096｜LLM Serving 如何做 Admission Control 和 Autoscaling？

> **定位**：Benchmark、生产部署与系统设计 · **难度**：★★★★★  
> **关键词**：`admission-control` · `autoscaling`

## 30 秒面试回答

> 不能只看 requests/s，要估计 token workload：input token rate、active decode sequences、预期 output、 KV occupancy、queue 与 SLO。Admission control 防止超载进入 thrashing；autoscaling 依据未来 token demand 与冷启动时间提前扩容。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：不能只看 requests/s，要估计 token workload：input token
rate、active decode sequences、预期 output、KV occupancy、queue 与 SLO。Admission control 防止超载进入
thrashing；autoscaling 依据未来 token demand 与冷启动时间提前扩容。
...
公式 Load≈λ×E[prefill_cost+decode_steps×per_step_cost]，同时受 KV 生命周期约束。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 不能只看 requests/s，要估计 token workload：input token rate、active decode sequences、预期 output、KV
occupancy、queue 与 SLO。Admission control 防止超载进入 thrashing
- 2. autoscaling 依据未来 token demand 与冷启动时间提前扩容。
...
现场推演 10 个 100K prompt 请求的压力可能远大于 100 个短 chat；单纯 QPS 会误判。
...
- 围绕“LLM Serving 如何做 Admission Control 和 Autoscaling？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
admission 要按“预计工作量”而不是请求数：输入 token、预期输出、KV 占用和 SLO class。autoscaling 还要考虑模型冷启动时间，不能只看当前 QPS。
...
[← Q095](q095-cost-tco.md) · [10 Benchmark、生产部署与系统设计](index.md) · [Q097 →](q097-oom-kv-operations.md)
