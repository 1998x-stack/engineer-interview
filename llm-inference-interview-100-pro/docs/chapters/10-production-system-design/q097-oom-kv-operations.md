---
id: Q097
title: "线上发生 KV Cache OOM，你怎么处理？"
chapter: "Benchmark、生产部署与系统设计"
difficulty: "★★★★★"
tags: ["OOM", "KV", "operations"]
source: "LLM_Inference_Interview_100_2026.pdf"
edition: "2026.09"
---

# Q097｜线上发生 KV Cache OOM，你怎么处理？

> **定位**：Benchmark、生产部署与系统设计 · **难度**：★★★★★  
> **关键词**：`OOM` · `KV` · `operations`

## 30 秒面试回答

> 先做内存分解：weights、KV、workspace、graph、allocator fragmentation；确认是容量不足还是泄漏/配置。短期降低并发/最大 context、启用 admission；中期 KV quant、GQA/MLA、page/allocator 优化、offload/HiCache/CP；长期 scale-out 或模型结构调整。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：先做内存分解：weights、KV、workspace、graph、allocator fragmentation；确认是容量不足还是泄漏/配置。短期降低并发/最大 context、启用 admission；中期 KV
quant、GQA/MLA、page/allocator 优化、offload/HiCache/CP；长期 scale-out 或模型结构调整。
...
公式 M_total=M_weights+M_KV+M_workspace+M_graph+M_misc < VRAM_budget。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 先做内存分解：weights、KV、workspace、graph、allocator fragmentation
- 2. 确认是容量不足还是泄漏/配置。短期降低并发/最大 context、启用 admission
- 3. 中期 KV quant、GQA/MLA、page/allocator 优化、offload/HiCache/CP
- 4. 长期 scale-out 或模型结构调整。
...
现场推演如果权重仅占 40% 显存、KV 55%，继续把权重从 INT8 降 INT4 只释放有限总空间，KV 优化更直接。
...
- 围绕“线上发生 KV Cache OOM，你怎么处理？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。
...
- → 如何判断 fragmentation？
- → 如何设置 max concurrency？
- → preemption storm 的症状是什么？
...
KV OOM 先分容量不足还是碎片/配置问题；再看可接受的动作：降低并发、缩短 context、KV quant、prefix eviction、hierarchical cache、CP 或扩容。
...
[← Q096](q096-admission-control-autoscaling.md) · [10 Benchmark、生产部署与系统设计](index.md) · [Q098 →](q098-observability-metrics.md)
