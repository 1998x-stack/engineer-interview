---
id: Q098
title: "生产 LLM Server 最重要的监控指标有哪些？"
...
# Q098｜生产 LLM Server 最重要的监控指标有哪些？

> **定位**：Benchmark、生产部署与系统设计 · **难度**：★★★★★  
> **关键词**：`observability` · `metrics`

## 30 秒面试回答

> 需要 request、scheduler、cache、GPU/network 四层可观测性，并可用 request_id 串联。请求层看 QPS/queue/TTFT/TPOT/p99/error；scheduler 看 running/waiting/batch/tokens/preemption；cache 看 occupancy/hit/eviction；硬件看 SM/Tensor/HBM/VRAM/NCCL/power。

## 1. 面试官到底在考什么

- 这道题不是考名词定义，而是看你能否从系统资源出发解释：需要 request、scheduler、cache、GPU/network 四层可观测性，并可用 request_id 串联。请求层看 QPS/queue/TTFT/TPOT/p99/error；scheduler 看
running/waiting/batch/tokens/preemption；cache 看 occupancy/hit/eviction；硬件看
SM/Tensor/HBM/VRAM/NCCL/power。
...
公式可观测性目标：从一次 SLO violation 能追到对应 scheduler step、GPU rank 与 kernel。

> **使用公式的原则**：先做一阶上界/下界估算，再用 profiler 校正有效带宽、kernel efficiency、通信 overlap 与排队时间。不要把理论峰值直接当线上值。

## 3. 深入原理：Know-Why

- 1. 需要 request、scheduler、cache、GPU/network 四层可观测性，并可用 request_id 串联。请求层看
QPS/queue/TTFT/TPOT/p99/error
- 2. scheduler 看 running/waiting/batch/tokens/preemption
- 3. cache 看 occupancy/hit/eviction
- 4. 硬件看 SM/Tensor/HBM/VRAM/NCCL/power。
...
现场推演 prefix hit rate 下降可能先表现为 TTFT 增加，再提高 GPU Prefill 负载并造成整体 queue。
...
- 围绕“生产 LLM Server 最重要的监控指标有哪些？”至少设计一个可证伪的 A/B 实验，明确控制变量与验收指标。

## 5. 边界条件与反例

避免把局部规律绝对化；必须说明 workload、并发、上下文长度、硬件拓扑、精度和 SLO，才能判断该结论是否成立。

- ✗ 指标很多但没有 trace correlation
- ✗ 无法回答“哪一批请求导致 p99”。
...
监控必须能把 request_id 映射到 scheduler state 和 GPU timeline。只有请求指标没有 KV/GPU 证据，或只有 GPU 指标没有 SLO，都无法快速 RCA。

### 白板推导

从本题给出的基础公式出发，对关键自变量做敏感性分析：分别让 traffic trace、SLO、replica count、parallelism、autoscaling、admission、cache、failure mode 中一个变量变化，判断容量、带宽、计算或通信项如何变化。现场推导时重点说明数量级和主导项，而不是追求小数点精度。
...
用真实或合成 trace 回放，加入突发流量、长 prompt、节点故障和版本回归；以 SLO 和成本而非峰值吞吐验收。
...
[← Q097](q097-oom-kv-operations.md) · [10 Benchmark、生产部署与系统设计](index.md) · [Q099 →](q099-system-design-h100-70b.md)
