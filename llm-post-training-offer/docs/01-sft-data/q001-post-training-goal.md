---
id: Q001
title: "什么是 Post-Training？为什么 Pretraining 后仍需要后训练？"
chapter: 1
...
# Q001 · 什么是 Post-Training？为什么 Pretraining 后仍需要后训练？

> **题型**：高频题 · **难度**：L1 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[章节首页](README.md) · [总索引](../question-index.md) · [Q002 →](../01-sft-data/q002-sft-behavior-cloning.md)
...
Pretraining 学“哪些 token 在上下文中可能出现”，后训练则把这种通用建模能力塑造成可控行为：指令遵 循、偏好、安全、推理、工具使用与领域适应。

### PDF 原始深入理解

把两种目标函数区分开：语言建模是分布拟合，post-training 是行为分布重塑。强 base model 的很多能力已 经存在，后训练经常更像 capability elicitation 与 policy shaping，而非从零注入知识。

### PDF 原始常见失分点

不要把 post-training 等同于 RLHF；SFT、偏好优化、RLVR、Agentic RL 都属于后训练。
...
1. 先给核心判断：Pretraining 学“哪些 token 在上下文中可能出现”，后训练则把这种通用建模能力塑造成可控行为：指令遵 循、偏好、安全、推理、工具使用与领域适应。
2. 再解释机制：把两种目标函数区分开：语言建模是分布拟合，post-training 是行为分布重塑。强 base model 的很多能力已 经存在，后训练经常更像 capability elicitation 与 policy shaping，而非从零注入知识。
3. 最后补一个边界条件或失败模式：区分“分布建模”与“行为策略塑形”。后训练的增益通常来自把已有能力重新放到正确的条件分布上，而不是简单增加事实知识。
...
区分“分布建模”与“行为策略塑形”。后训练的增益通常来自把已有能力重新放到正确的条件分布上，而不是简单增加事实知识。
...
\[\mathcal L_{LM}=-\sum_t\log p_\theta(x_t\mid x_{<t}),\qquad \mathcal L_{SFT}=-\sum_t\log \pi_\theta(y_t\mid x,y_{<t})\]
...
```text
Problem: 什么是 Post-Training？为什么 Pretraining 后仍需要后训练？
...
- 不要把 post-training 等同于 RLHF；SFT、偏好优化、RLVR、Agentic RL 都属于后训练。
- 边界条件：区分“分布建模”与“行为策略塑形”。后训练的增益通常来自把已有能力重新放到正确的条件分布上，而不是简单增加事实知识。
...
1. 如果 base model 已经很强，为什么少量高质量 SFT 仍然有效？
...
> **Q001：Pretraining 学“哪些 token 在上下文中可能出现”，后训练则把这种通用建模能力塑造成可控行为：指令遵 循、偏好、安全、推理、工具使用与领域适应。**
...
本题不能停在“Pretraining 学“哪些 token 在上下文中可能出现”，后训练则把这种通用建模能力塑造成可控行为：指令遵 循、偏好、安全、推理、工具使用与领域适应”。更专业的表述是先确定五个对象：
...
对 **Q001**，最关键的机制判断是：**区分“分布建模”与“行为策略塑形”。后训练的增益通常来自把已有能力重新放到正确的条件分布上，而不是简单增加事实知识。**

再进一步，工程上真正需要落地的是：把后训练拆成“能力是否存在、行为是否被触发、偏好是否一致、环境是否可交互”四层。项目复盘时分别给出 base→SFT→preference/RL 的增益与回退，避免把所有变化都归因于“模型更聪明”。
...
本题 PDF 的深入结论是：把两种目标函数区分开：语言建模是分布拟合，post-training 是行为分布重塑。强 base model 的很多能力已 经存在，后训练经常更像 capability elicitation 与 policy shaping，而非从零注入知识。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q001
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：Pretraining 学“哪些 token 在上下文中可能出现”，后训练则把这种通用建模能力塑造成可控行为：指令遵 循、偏好、安全、推理、工具使用与领域适应 |
