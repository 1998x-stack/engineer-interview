---
id: Q002
title: "SFT 到底在学什么？为什么说它是 behavior cloning？"
chapter: 1
...
# Q002 · SFT 到底在学什么？为什么说它是 behavior cloning？

> **题型**：高频题 · **难度**：L1 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q001](../01-sft-data/q001-post-training-goal.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q003 →](../01-sft-data/q003-sft-data-filter-sampling.md)
...
SFT 用目标回答作为唯一轨迹做 teacher forcing，本质是最大化专家轨迹的条件似然。它最擅长输出格式、基 本指令遵循、工具调用语法、风格与 cold-start reasoning。

### PDF 原始深入理解

关键局限是它只“模仿看过的轨迹”，不会主动探索训练集没有的解法，也无法自然表达多个候选答案之间的相 对偏好。

### PDF 原始常见失分点

把 SFT 说成“让模型更聪明”过于粗糙；应区分能力、行为与分布。
...
1. 先给核心判断：SFT 用目标回答作为唯一轨迹做 teacher forcing，本质是最大化专家轨迹的条件似然。它最擅长输出格式、基 本指令遵循、工具调用语法、风格与 cold-start reasoning。
2. 再解释机制：关键局限是它只“模仿看过的轨迹”，不会主动探索训练集没有的解法，也无法自然表达多个候选答案之间的相 对偏好。
3. 最后补一个边界条件或失败模式：SFT 的梯度只来自专家轨迹；teacher forcing 让训练稳定，但也制造 exposure bias 与“只会模仿、不负责探索”的边界。
...
SFT 的梯度只来自专家轨迹；teacher forcing 让训练稳定，但也制造 exposure bias 与“只会模仿、不负责探索”的边界。
...
\[\mathcal L_{SFT}=-\sum_{t=1}^{T}\log \pi_\theta(y_t\mid x,y_{<t})\]
...
```text
Problem: SFT 到底在学什么？为什么说它是 behavior cloning？
...
- 把 SFT 说成“让模型更聪明”过于粗糙；应区分能力、行为与分布。
- 边界条件：SFT 的梯度只来自专家轨迹；teacher forcing 让训练稳定，但也制造 exposure bias 与“只会模仿、不负责探索”的边界。
...
1. 为什么最佳 SFT checkpoint 不一定是最佳 RL initialization？
...
> **Q002：SFT 用目标回答作为唯一轨迹做 teacher forcing，本质是最大化专家轨迹的条件似然。它最擅长输出格式、基 本指令遵循、工具调用语法、风格与 cold-start reasoning。**
...
本题不能停在“SFT 用目标回答作为唯一轨迹做 teacher forcing，本质是最大化专家轨迹的条件似然。它最擅长输出格式、基 本指令遵循、工具调用语法、风格与 cold-start reasoning”。更专业的表述是先确定五个对象：
...
对 **Q002**，最关键的机制判断是：**SFT 的梯度只来自专家轨迹；teacher forcing 让训练稳定，但也制造 exposure bias 与“只会模仿、不负责探索”的边界。**

再进一步，工程上真正需要落地的是：检查 label mask 是否只覆盖 assistant token、chat template 是否与推理一致、长回答是否因 token 数获得不成比例梯度。比较 response-only loss 与全序列 loss，能直接暴露实现是否混入 prompt 学习。
...
本题 PDF 的深入结论是：关键局限是它只“模仿看过的轨迹”，不会主动探索训练集没有的解法，也无法自然表达多个候选答案之间的相 对偏好。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q002
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：SFT 用目标回答作为唯一轨迹做 teacher forcing，本质是最大化专家轨迹的条件似然。它最擅长输出格式、基 本指令遵循、工具调用语法、风格与 cold-start reasoning |
...
**本题难度 L1 的最低合格线**：能给出准确边界与核心定义；如果只能背名词，不足以通过算法面。

## 20. 复习与项目化清单

在认为自己“掌握 Q002”之前，至少能独立完成：
