...
# Q003 · 公开真题：SFT 数据如何筛选和采样？

> **题型**：公开真题 · **难度**：L2 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q002](../01-sft-data/q002-sft-behavior-cloning.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q004 →](../01-sft-data/q004-sft-data-quality-diversity-difficulty.md)
...
按数据工程链路回答：来源治理 → 去重/泄漏 → 正确性 → 指令质量 → 难度 → 多样性 → 格式 → 安全 → 领域配 比 → train/eval 隔离。

### PDF 原始深入理解

面试官真正看的是你能否把“数据质量”变成可执行规则。对 reasoning 数据应额外检查最终答案、推理一致 性、长度、语言混用和可验证性。采样可采用 temperature、domain cap、hard-example mining 与 capability-aware sampling。

### PDF 原始常见失分点

只讲“清洗、去重、过滤低质”没有区分度；一定要讲筛选信号和错误样本如何回流。
...
1. 先给核心判断：按数据工程链路回答：来源治理 → 去重/泄漏 → 正确性 → 指令质量 → 难度 → 多样性 → 格式 → 安全 → 领域配 比 → train/eval 隔离。
2. 再解释机制：面试官真正看的是你能否把“数据质量”变成可执行规则。对 reasoning 数据应额外检查最终答案、推理一致 性、长度、语言混用和可验证性。采样可采用 temperature、domain cap、hard-example mining 与 capability-aware sampling。
3. 最后补一个边界条件或失败模式：把“质量”拆成可执行信号：正确性、去重、难度、覆盖、格式、安全、泄漏和领域权重；真正高级的答案必须说明筛选器误差与回流机制。
...
把“质量”拆成可执行信号：正确性、去重、难度、覆盖、格式、安全、泄漏和领域权重；真正高级的答案必须说明筛选器误差与回流机制。
...
raw data
  ...
  -> frozen validation & regression sets
  -> training
  -> failure mining -> next data round
```
...
- 只讲“清洗、去重、过滤低质”没有区分度；一定要讲筛选信号和错误样本如何回流。
- 边界条件：把“质量”拆成可执行信号：正确性、去重、难度、覆盖、格式、安全、泄漏和领域权重；真正高级的答案必须说明筛选器误差与回流机制。
...
1. 如果只允许保留 20% SFT 数据，你会依据什么排序？ [I7]
2. 筛选器自身的 false positive/false negative 如何标定？
3. 怎样构造数据质量评分而不让 teacher judge 形成单点偏差？
4. 如果把数据量减半，怎样保证信息量不减半？
5. 如何用一个离线实验证明改动来自数据，而不是更长训练？
6. 哪些指标能提前发现 catastrophic forgetting？
...
> **Q003：按数据工程链路回答：来源治理 → 去重/泄漏 → 正确性 → 指令质量 → 难度 → 多样性 → 格式 → 安全 → 领域配 比 → train/eval 隔离。**
...
- [Q008 · 公开真题：CoT 数据怎么构建、筛选与验证？](q008-cot-data-build-filter-verify.md)
...
本题不能停在“按数据工程链路回答：来源治理 → 去重/泄漏 → 正确性 → 指令质量 → 难度 → 多样性 → 格式 → 安全 → 领域配 比 → train/eval 隔离”。更专业的表述是先确定五个对象：
...
对 **Q003**，最关键的机制判断是：**把“质量”拆成可执行信号：正确性、去重、难度、覆盖、格式、安全、泄漏和领域权重；真正高级的答案必须说明筛选器误差与回流机制。**

再进一步，工程上真正需要落地的是：把筛选器做成可校准的多阶段 cascade：规则/哈希先挡廉价错误，模型 judge 处理语义质量，verifier 处理可验证正确性；记录每一级保留率与误杀率，才能做数据 ROI。
...
本题 PDF 的深入结论是：面试官真正看的是你能否把“数据质量”变成可执行规则。对 reasoning 数据应额外检查最终答案、推理一致 性、长度、语言混用和可验证性。采样可采用 temperature、domain cap、hard-example mining 与 capability-aware sampling。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：按数据工程链路回答：来源治理 → 去重/泄漏 → 正确性 → 指令质量 → 难度 → 多样性 → 格式 → 安全 → 领域配 比 → train/eval 隔离 |
