---
id: Q004
title: "SFT 数据是越多越好吗？如何理解 data quality × diversity × difficulty？"
chapter: 1
...
# Q004 · SFT 数据是越多越好吗？如何理解 data quality × diversity × difficulty？

> **题型**：高频题 · **难度**：L2 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q003](../01-sft-data/q003-sft-data-filter-sampling.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q005 →](../01-sft-data/q005-sft-forgetting-alignment-tax.md)
...
不是。大量低质量、重复或过易数据会放大错误梯度、模板化和 verbosity bias，并可能覆盖 base model 原有 能力。

### PDF 原始深入理解

有效数据价值更接近“边际训练信号”：正确、非重复、覆盖能力盲区、难度贴近当前 frontier 的样本贡献更 高。对强模型而言，100 万条简单指令可能不如 5 万条高质量 hard cases。

### PDF 原始常见失分点

只按 token 数汇报数据规模；忽略数据重复率、错误率和难度分布。
...
1. 先给核心判断：不是。大量低质量、重复或过易数据会放大错误梯度、模板化和 verbosity bias，并可能覆盖 base model 原有 能力。
2. 再解释机制：有效数据价值更接近“边际训练信号”：正确、非重复、覆盖能力盲区、难度贴近当前 frontier 的样本贡献更 高。对强模型而言，100 万条简单指令可能不如 5 万条高质量 hard cases。
3. 最后补一个边界条件或失败模式：数据规模要看边际信息增益。重复、过易或错误样本会消耗 token budget，同时把模型推向模板化和错误捷径。
...
数据规模要看边际信息增益。重复、过易或错误样本会消耗 token budget，同时把模型推向模板化和错误捷径。
...
```text
Problem: SFT 数据是越多越好吗？如何理解 data quality × diversity × difficulty？
...
- 只按 token 数汇报数据规模；忽略数据重复率、错误率和难度分布。
- 边界条件：数据规模要看边际信息增益。重复、过易或错误样本会消耗 token budget，同时把模型推向模板化和错误捷径。
...
1. 怎么做 data ablation 才能证明“数据质量”而不是训练步数带来的提升？
...
> **Q004：不是。大量低质量、重复或过易数据会放大错误梯度、模板化和 verbosity bias，并可能覆盖 base model 原有 能力。**
...
本题不能停在“不是。大量低质量、重复或过易数据会放大错误梯度、模板化和 verbosity bias，并可能覆盖 base model 原有 能力”。更专业的表述是先确定五个对象：
...
对 **Q004**，最关键的机制判断是：**数据规模要看边际信息增益。重复、过易或错误样本会消耗 token budget，同时把模型推向模板化和错误捷径。**

再进一步，工程上真正需要落地的是：用“有效信息量/token”而不是样本数评价扩容：对重复簇、难度分桶、能力分桶做等 token ablation，并观察 scaling curve 是否已进入低边际收益区。
...
本题 PDF 的深入结论是：有效数据价值更接近“边际训练信号”：正确、非重复、覆盖能力盲区、难度贴近当前 frontier 的样本贡献更 高。对强模型而言，100 万条简单指令可能不如 5 万条高质量 hard cases。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q004
...
| 3 | `effective tokens` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `dedup rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `filter keep-rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `train/heldout loss` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `capability regression` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `domain/difficulty coverage` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：不是。大量低质量、重复或过易数据会放大错误梯度、模板化和 verbosity bias，并可能覆盖 base model 原有 能力 |
