---
id: Q005
title: "为什么 SFT 会造成 catastrophic forgetting 或 alignment tax？"
chapter: 1
...
# Q005 · 为什么 SFT 会造成 catastrophic forgetting 或 alignment tax？

> **题型**：原理推导 · **难度**：L2 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q004](../01-sft-data/q004-sft-data-quality-diversity-difficulty.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q006 →](../01-sft-data/q006-sft-domain-mixture.md)
...
因为优化分布从超大规模 pretraining distribution 收缩到较窄的 instruction distribution，梯度会持续推动参 数适配新分布。

### PDF 原始深入理解

缓解手段包括较小 LR、pretraining replay、领域混合、参数高效微调、early stopping 和回归评测。所谓 alignment tax 是行为对齐带来的通用能力/某些 benchmark 回退，应通过多目标数据与阶段化训练控制。

### PDF 原始常见失分点

只会说“学习率太大”；forgetting 首先是分布与目标的问题。
...
1. 先给核心判断：因为优化分布从超大规模 pretraining distribution 收缩到较窄的 instruction distribution，梯度会持续推动参 数适配新分布。
2. 再解释机制：缓解手段包括较小 LR、pretraining replay、领域混合、参数高效微调、early stopping 和回归评测。所谓 alignment tax 是行为对齐带来的通用能力/某些 benchmark 回退，应通过多目标数据与阶段化训练控制。
3. 最后补一个边界条件或失败模式：遗忘首先是目标分布改变，不只是学习率问题。回放数据、混合训练和回归评测本质上都在约束参数不要只服务窄分布。
...
遗忘首先是目标分布改变，不只是学习率问题。回放数据、混合训练和回归评测本质上都在约束参数不要只服务窄分布。
...
```text
Problem: 为什么 SFT 会造成 catastrophic forgetting 或 alignment tax？
...
- 只会说“学习率太大”；forgetting 首先是分布与目标的问题。
- 边界条件：遗忘首先是目标分布改变，不只是学习率问题。回放数据、混合训练和回归评测本质上都在约束参数不要只服务窄分布。
...
1. 为什么 replay 一定要有？什么情况下 LoRA 反而限制最终能力？
...
> **Q005：因为优化分布从超大规模 pretraining distribution 收缩到较窄的 instruction distribution，梯度会持续推动参 数适配新分布。**
...
- [Q010 · SFT、DPO 与 Online RL 应该如何选？](q010-sft-dpo-online-rl-choice.md)
...
本题不能停在“因为优化分布从超大规模 pretraining distribution 收缩到较窄的 instruction distribution，梯度会持续推动参 数适配新分布”。更专业的表述是先确定五个对象：
...
对 **Q005**，最关键的机制判断是：**遗忘首先是目标分布改变，不只是学习率问题。回放数据、混合训练和回归评测本质上都在约束参数不要只服务窄分布。**

再进一步，工程上真正需要落地的是：构建旧能力 regression suite，并把 forgetting 定义为可量化的 delta；混入 replay 时分别 sweep replay ratio、LR、训练步数，确认是梯度冲突还是单纯过训练。
...
本题 PDF 的深入结论是：缓解手段包括较小 LR、pretraining replay、领域混合、参数高效微调、early stopping 和回归评测。所谓 alignment tax 是行为对齐带来的通用能力/某些 benchmark 回退，应通过多目标数据与阶段化训练控制。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q005
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：因为优化分布从超大规模 pretraining distribution 收缩到较窄的 instruction distribution，梯度会持续推动参 数适配新分布 |
