...
# Q006 · 多领域 SFT 数据应该怎么配比？

> **题型**：高频题 · **难度**：L2 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q005](../01-sft-data/q005-sft-forgetting-alignment-tax.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q007 →](../01-sft-data/q007-prompt-diversity.md)
...
不应按原始数据量等比例采样，而应围绕业务价值、质量、稀缺性、难度、遗忘风险和当前能力短板设权重。

### PDF 原始深入理解

可以将 mixture weight 视作控制训练梯度来源的策略变量；常见方法包括 temperature sampling、上限截断、 curriculum、loss-aware sampling 与分阶段 mixture。

### PDF 原始常见失分点

把“英语 50%、中文 50%”当成原则；真正应回答为什么这么配、如何在线调整。
...
1. 先给核心判断：不应按原始数据量等比例采样，而应围绕业务价值、质量、稀缺性、难度、遗忘风险和当前能力短板设权重。
2. 再解释机制：可以将 mixture weight 视作控制训练梯度来源的策略变量；常见方法包括 temperature sampling、上限截断、 curriculum、loss-aware sampling 与分阶段 mixture。
3. 最后补一个边界条件或失败模式：mixture weight 是“梯度预算”的分配，不是静态比例。需要同时看业务价值、稀缺度、当前能力缺口与遗忘风险。
...
mixture weight 是“梯度预算”的分配，不是静态比例。需要同时看业务价值、稀缺度、当前能力缺口与遗忘风险。
...
- 把“英语 50%、中文 50%”当成原则；真正应回答为什么这么配、如何在线调整。
- 边界条件：mixture weight 是“梯度预算”的分配，不是静态比例。需要同时看业务价值、稀缺度、当前能力缺口与遗忘风险。
...
1. 某领域 loss 很高，是该加大权重还是数据质量有问题？如何区分？
...
> **Q006：不应按原始数据量等比例采样，而应围绕业务价值、质量、稀缺性、难度、遗忘风险和当前能力短板设权重。**
...
本题不能停在“不应按原始数据量等比例采样，而应围绕业务价值、质量、稀缺性、难度、遗忘风险和当前能力短板设权重”。更专业的表述是先确定五个对象：
...
对 **Q006**，最关键的机制判断是：**mixture weight 是“梯度预算”的分配，不是静态比例。需要同时看业务价值、稀缺度、当前能力缺口与遗忘风险。**

再进一步，工程上真正需要落地的是：把 mixture 当成资源分配优化：每个 domain 维护当前能力、目标能力、数据质量和边际收益估计；定期重估，而不是整个训练周期固定比例。
...
本题 PDF 的深入结论是：可以将 mixture weight 视作控制训练梯度来源的策略变量；常见方法包括 temperature sampling、上限截断、 curriculum、loss-aware sampling 与分阶段 mixture。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q006
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：不应按原始数据量等比例采样，而应围绕业务价值、质量、稀缺性、难度、遗忘风险和当前能力短板设权重 |
