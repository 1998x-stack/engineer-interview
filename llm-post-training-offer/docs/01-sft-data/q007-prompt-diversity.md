---
id: Q007
title: "为什么 instruction data 需要 prompt diversity，而不只是 response diversity？"
chapter: 1
...
# Q007 · 为什么 instruction data 需要 prompt diversity，而不只是 response diversity？

> **题型**：原理推导 · **难度**：L1 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q006](../01-sft-data/q006-sft-domain-mixture.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q008 →](../01-sft-data/q008-cot-data-build-filter-verify.md)
...
同一意图的多种表达可以降低模板记忆，提高对意图而非表面形式的泛化。

### PDF 原始深入理解

一个完善的数据扩增要同时改变措辞、上下文、约束组合、角色、语言和信息缺失方式，并避免 synthetic paraphrase 造成模式单一。
...
1. 先给核心判断：同一意图的多种表达可以降低模板记忆，提高对意图而非表面形式的泛化。
2. 再解释机制：一个完善的数据扩增要同时改变措辞、上下文、约束组合、角色、语言和信息缺失方式，并避免 synthetic paraphrase 造成模式单一。
3. 最后补一个边界条件或失败模式：prompt diversity 让模型学习 intent invariance；只改答案而不改问题形式，会把数据增强做成表面同义改写。
...
prompt diversity 让模型学习 intent invariance；只改答案而不改问题形式，会把数据增强做成表面同义改写。
...
```text
Problem: 为什么 instruction data 需要 prompt diversity，而不只是 response diversity？
...
- 只做答案改写而 prompt 基本不变。
- 边界条件：prompt diversity 让模型学习 intent invariance；只改答案而不改问题形式，会把数据增强做成表面同义改写。
...
1. 如何检测 synthetic data 的“模型味”和模板塌缩？
...
> **Q007：同一意图的多种表达可以降低模板记忆，提高对意图而非表面形式的泛化。**
...
本题不能停在“同一意图的多种表达可以降低模板记忆，提高对意图而非表面形式的泛化”。更专业的表述是先确定五个对象：
...
对 **Q007**，最关键的机制判断是：**prompt diversity 让模型学习 intent invariance；只改答案而不改问题形式，会把数据增强做成表面同义改写。**

再进一步，工程上真正需要落地的是：对 prompt 做意图级聚类和表述级扰动，分别测“同意图不同问法”的一致性；若只有 lexical diversity 而 intent coverage 不变，增强价值往往有限。
...
本题 PDF 的深入结论是：一个完善的数据扩增要同时改变措辞、上下文、约束组合、角色、语言和信息缺失方式，并避免 synthetic paraphrase 造成模式单一。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q007
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：同一意图的多种表达可以降低模板记忆，提高对意图而非表面形式的泛化 |
