---
id: Q008
title: "公开真题：CoT 数据怎么构建、筛选与验证？"
chapter: 1
chapter_title: "后训练全景与 SFT"
type: "公开真题"
level: L3
priority: normal
tags: [sft, verifier, cot]
source_refs: [I3, I7, P14]
---

# Q008 · 公开真题：CoT 数据怎么构建、筛选与验证？

> **题型**：公开真题 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q007](../01-sft-data/q007-prompt-diversity.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q009 →](../01-sft-data/q009-sft-cold-start-pure-rl.md)
...
Teacher 多样采样 → rule/model verifier → 正确轨迹保留 → 去重与多样性筛选 → 长度/语言/格式约束 → 难度 分层 → SFT 或 rejection sampling。

### PDF 原始深入理解

final answer 正确不代表 reasoning 合理。数学/代码可用强 verifier；开放任务可组合 answer correctness、过 程一致性与对抗检查。还要防止 teacher error 被 student 放大。

### PDF 原始常见失分点

把“答案对”当成“链路对”；只依赖一个 judge。
...
1. 先给核心判断：Teacher 多样采样 → rule/model verifier → 正确轨迹保留 → 去重与多样性筛选 → 长度/语言/格式约束 → 难度 分层 → SFT 或 rejection sampling。
2. 再解释机制：final answer 正确不代表 reasoning 合理。数学/代码可用强 verifier；开放任务可组合 answer correctness、过 程一致性与对抗检查。还要防止 teacher error 被 student 放大。
3. 最后补一个边界条件或失败模式：CoT 数据必须把“最终答案正确”和“过程可信”分开评估；teacher-generated reasoning 还要控制同质化、错误传播和长度偏差。
...
CoT 数据必须把“最终答案正确”和“过程可信”分开评估；teacher-generated reasoning 还要控制同质化、错误传播和长度偏差。
...
for prompt in prompts:
    trajectories = teacher.sample(prompt, n=N, temperature=t)
    trajectories = verify_final_answer(trajectories)
    trajectories = check_reasoning_consistency(trajectories)
    trajectories = dedup_and_diversify(trajectories)
    keep(hard_but_valid(trajectories))
```
...
- 把“答案对”当成“链路对”；只依赖一个 judge。
- 边界条件：CoT 数据必须把“最终答案正确”和“过程可信”分开评估；teacher-generated reasoning 还要控制同质化、错误传播和长度偏差。
...
1. 如果两个 CoT 最终都正确，一个短一个长，你怎么选？ [I3][I7]
...
> **Q008：Teacher 多样采样 → rule/model verifier → 正确轨迹保留 → 去重与多样性筛选 → 长度/语言/格式约束 → 难度 分层 → SFT 或 rejection sampling。**
...
- [Q003 · 公开真题：SFT 数据如何筛选和采样？](q003-sft-data-filter-sampling.md)
- [Q004 · SFT 数据是越多越好吗？如何理解 data quality × diversity × difficulty？](q004-sft-data-quality-diversity-difficulty.md)
- [Q006 · 多领域 SFT 数据应该怎么配比？](q006-sft-domain-mixture.md)
- [Q007 · 为什么 instruction data 需要 prompt diversity，而不只是 response diversity？](q007-prompt-diversity.md)
- [Q089 · 如何构建 hard-example data flywheel？](../09-eval-debug/q089-hard-example-data-flywheel.md)
...
本题不能停在“Teacher 多样采样 → rule/model verifier → 正确轨迹保留 → 去重与多样性筛选 → 长度/语言/格式约束 → 难度 分层 → SFT 或 rejection sampling”。更专业的表述是先确定五个对象：
...
对 **Q008**，最关键的机制判断是：**CoT 数据必须把“最终答案正确”和“过程可信”分开评估；teacher-generated reasoning 还要控制同质化、错误传播和长度偏差。**

再进一步，工程上真正需要落地的是：CoT 管线至少区分 final-answer verifier、过程一致性 judge、格式/语言过滤与去模板化；对正确答案但错误推理的样本单独统计，否则会把伪推理当监督信号。
...
本题 PDF 的深入结论是：final answer 正确不代表 reasoning 合理。数学/代码可用强 verifier；开放任务可组合 answer correctness、过 程一致性与对抗检查。还要防止 teacher error 被 student 放大。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
| # | 指标 | 如何解释 |
|---:|---|---|
| 1 | `effective tokens` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 2 | `dedup rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 3 | `filter keep-rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `train/heldout loss` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `capability regression` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `domain/difficulty coverage` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：Teacher 多样采样 → rule/model verifier → 正确轨迹保留 → 去重与多样性筛选 → 长度/语言/格式约束 → 难度 分层 → SFT 或 rejection sampling |
