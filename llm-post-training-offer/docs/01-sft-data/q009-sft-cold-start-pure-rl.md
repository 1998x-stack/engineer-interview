---
id: Q009
title: "为什么 SFT 经常是 RL 的 cold start？DeepSeek-R1-Zero 又为什么可以跳 过？"
chapter: 1
chapter_title: "后训练全景与 SFT"
type: "原理推导"
level: L3
priority: normal
tags: [sft, reward, zero]
source_refs: [P8, P14]
---

# Q009 · 为什么 SFT 经常是 RL 的 cold start？DeepSeek-R1-Zero 又为什么可以跳 过？

> **题型**：原理推导 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q008](../01-sft-data/q008-cot-data-build-filter-verify.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q010 →](../01-sft-data/q010-sft-dpo-online-rl-choice.md)
...
RL 需要策略在初始分布中能采到非零 reward 轨迹；若几乎全错，梯度信号稀缺。SFT 可把策略移到可探索区 域。

### PDF 原始深入理解

但若 base model 足够强、任务可验证且 sampling budget 大，纯 RL 也可能直接激发 reasoning。DeepSeek- R1-Zero 展示了这种可能，同时也出现可读性与语言混用问题，后续 R1 又引入 cold-start data 与多阶段训练。[P8]
...
1. 先给核心判断：RL 需要策略在初始分布中能采到非零 reward 轨迹；若几乎全错，梯度信号稀缺。SFT 可把策略移到可探索区 域。
2. 再解释机制：但若 base model 足够强、任务可验证且 sampling budget 大，纯 RL 也可能直接激发 reasoning。DeepSeek- R1-Zero 展示了这种可能，同时也出现可读性与语言混用问题，后续 R1 又引入 cold-start data 与多阶段训练。[P8]
3. 最后补一个边界条件或失败模式：cold start 的作用是提高初始策略采到正 reward 轨迹的概率；pure RL 可行的前提是 base model、verifier 与 sampling budget 足够强。
...
cold start 的作用是提高初始策略采到正 reward 轨迹的概率；pure RL 可行的前提是 base model、verifier 与 sampling budget 足够强。
...
```text
Problem: 为什么 SFT 经常是 RL 的 cold start？DeepSeek-R1-Zero 又为什么可以跳 过？
...
- 把“必须先 SFT”当成铁律。
- 边界条件：cold start 的作用是提高初始策略采到正 reward 轨迹的概率；pure RL 可行的前提是 base model、verifier 与 sampling budget 足够强。
...
> **Q009：RL 需要策略在初始分布中能采到非零 reward 轨迹；若几乎全错，梯度信号稀缺。SFT 可把策略移到可探索区 域。**
...
本题不能停在“RL 需要策略在初始分布中能采到非零 reward 轨迹；若几乎全错，梯度信号稀缺。SFT 可把策略移到可探索区 域”。更专业的表述是先确定五个对象：
...
对 **Q009**，最关键的机制判断是：**cold start 的作用是提高初始策略采到正 reward 轨迹的概率；pure RL 可行的前提是 base model、verifier 与 sampling budget 足够强。**

再进一步，工程上真正需要落地的是：先估计初始策略的 success probability 与 pass@k；若在可承受 rollout budget 下几乎采不到正样本，cold-start SFT 比盲目扩大 RL batch 更有效。
...
本题 PDF 的深入结论是：但若 base model 足够强、任务可验证且 sampling budget 大，纯 RL 也可能直接激发 reasoning。DeepSeek- R1-Zero 展示了这种可能，同时也出现可读性与语言混用问题，后续 R1 又引入 cold-start data 与多阶段训练。[P8]。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q009
...
| 3 | `optimizer/grad/param shard bytes` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `data keep-rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `contamination rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：RL 需要策略在初始分布中能采到非零 reward 轨迹；若几乎全错，梯度信号稀缺。SFT 可把策略移到可探索区 域 |
