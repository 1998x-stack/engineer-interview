---
id: Q011
title: "Reward Model 如何训练？Bradley-Terry 假设是什么？"
chapter: 2
...
# Q011 · Reward Model 如何训练？Bradley-Terry 假设是什么？

> **题型**：高频题 · **难度**：L2 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q010](../01-sft-data/q010-sft-dpo-online-rl-choice.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q012 →](../02-reward-model/q012-reward-model-data-source.md)
...
对同一 prompt 的 preferred / rejected 输出学习标量 reward，使 r(chosen) > r(rejected)。经典损失是 -log σ(r_w-r_l)。

### PDF 原始深入理解

Bradley-Terry 把偏好概率表示为 reward 差的 logistic 函数；它只约束相对顺序，不要求 reward 是可解释的绝 对分数。

### PDF 原始常见失分点

把 RM 输出当“真实质量分”；忽略 calibration 与分布外问题。
...
1. 先给核心判断：对同一 prompt 的 preferred / rejected 输出学习标量 reward，使 r(chosen) > r(rejected)。经典损失是 -log σ(r_w-r_l)。
2. 再解释机制：Bradley-Terry 把偏好概率表示为 reward 差的 logistic 函数；它只约束相对顺序，不要求 reward 是可解释的绝 对分数。
3. 最后补一个边界条件或失败模式：Bradley–Terry 只要求相对排序可由标量差表示；它并不保证 reward 有绝对意义，也无法天然表达循环偏好和多维 Pareto 偏好。
...
Bradley–Terry 只要求相对排序可由标量差表示；它并不保证 reward 有绝对意义，也无法天然表达循环偏好和多维 Pareto 偏好。
...
\[P(y_w\succ y_l\mid x)=\sigma(r_\phi(x,y_w)-r_\phi(x,y_l))\] \[\mathcal L_{RM}=-\log\sigma(r_w-r_l)\]
...
```text
Problem: Reward Model 如何训练？Bradley-Terry 假设是什么？
...
- 把 RM 输出当“真实质量分”；忽略 calibration 与分布外问题。
- 边界条件：Bradley–Terry 只要求相对排序可由标量差表示；它并不保证 reward 有绝对意义，也无法天然表达循环偏好和多维 Pareto 偏好。
...
1. 如果 preference 出现循环 A>B、B>C、C>A，标量 RM 会发生什么？
2. Bradley–Terry 为什么无法自然表达 intransitive preference？
3. RM score 是否应该跨 prompt 比较？
4. 这个 reward 的 OOD 边界在哪里？
5. 如果两个 judge 冲突，你如何决定最终信号？
6. 如何证明模型没有在利用 grader shortcut？
...
> **Q011：对同一 prompt 的 preferred / rejected 输出学习标量 reward，使 r(chosen) > r(rejected)。经典损失是 -log σ(r_w-r_l)。**
...
本题不能停在“对同一 prompt 的 preferred / rejected 输出学习标量 reward，使 r(chosen) > r(rejected)。经典损失是 -log σ(r_w-r_l)”。更专业的表述是先确定五个对象：
...
对 **Q011**，最关键的机制判断是：**Bradley–Terry 只要求相对排序可由标量差表示；它并不保证 reward 有绝对意义，也无法天然表达循环偏好和多维 Pareto 偏好。**

再进一步，工程上真正需要落地的是：训练 RM 时同时报告 pair accuracy、AUC、calibration、不同 margin 的准确率；BT 假设只给相对效用，不能把 reward 数值跨版本直接比较。
...
本题 PDF 的深入结论是：Bradley-Terry 把偏好概率表示为 reward 差的 logistic 函数；它只约束相对顺序，不要求 reward 是可解释的绝 对分数。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q011
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：对同一 prompt 的 preferred / rejected 输出学习标量 reward，使 r(chosen) > r(rejected)。经典损失是 -log σ(r_w-r_l) |
