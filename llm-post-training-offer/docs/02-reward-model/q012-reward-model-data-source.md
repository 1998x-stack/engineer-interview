---
id: Q012
title: "公开真题：PPO 中的 Reward Model 数据从哪里来？"
chapter: 2
...
# Q012 · 公开真题：PPO 中的 Reward Model 数据从哪里来？

> **题型**：公开真题 · **难度**：L2 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q011](../02-reward-model/q011-reward-model-bradley-terry.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q013 →](../02-reward-model/q013-preference-margin-agreement.md)
...
通常由 prompt 上的 policy 多样生成，再由人工或 judge 排序，构造成 preference pairs 训练。

### PDF 原始深入理解

RM 数据应覆盖当前 policy 真正会产生的失败模式。policy 演化后，老 RM 可能进入 OOD 区域，出现 reward extrapolation 与 exploitation，因此需要迭代采样/重训或 ensemble。

### PDF 原始常见失分点

只说“人工标注”；不说明 policy distribution。
...
1. 先给核心判断：通常由 prompt 上的 policy 多样生成，再由人工或 judge 排序，构造成 preference pairs 训练。
2. 再解释机制：RM 数据应覆盖当前 policy 真正会产生的失败模式。policy 演化后，老 RM 可能进入 OOD 区域，出现 reward extrapolation 与 exploitation，因此需要迭代采样/重训或 ensemble。
3. 最后补一个边界条件或失败模式：RM 数据要追随 policy 分布。一个静态 RM 在策略持续变强后会逐渐进入 OOD，最终变成可被 optimizer 利用的代理模型。
...
RM 数据要追随 policy 分布。一个静态 RM 在策略持续变强后会逐渐进入 OOD，最终变成可被 optimizer 利用的代理模型。
...
```text
Problem: 公开真题：PPO 中的 Reward Model 数据从哪里来？
...
- 只说“人工标注”；不说明 policy distribution。
- 边界条件：RM 数据要追随 policy 分布。一个静态 RM 在策略持续变强后会逐渐进入 OOD，最终变成可被 optimizer 利用的代理模型。
...
> **Q012：通常由 prompt 上的 policy 多样生成，再由人工或 judge 排序，构造成 preference pairs 训练。**
...
本题不能停在“通常由 prompt 上的 policy 多样生成，再由人工或 judge 排序，构造成 preference pairs 训练”。更专业的表述是先确定五个对象：
...
对 **Q012**，最关键的机制判断是：**RM 数据要追随 policy 分布。一个静态 RM 在策略持续变强后会逐渐进入 OOD，最终变成可被 optimizer 利用的代理模型。**

再进一步，工程上真正需要落地的是：建立 policy-version→RM-dataset-version 映射；定期用最新 policy 采 hard negatives。若 RM 长期只看旧模型输出，online optimizer 会快速进入 RM 的 OOD 区域。
...
本题 PDF 的深入结论是：RM 数据应覆盖当前 policy 真正会产生的失败模式。policy 演化后，老 RM 可能进入 OOD 区域，出现 reward extrapolation 与 exploitation，因此需要迭代采样/重训或 ensemble。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q012
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：通常由 prompt 上的 policy 多样生成，再由人工或 judge 排序，构造成 preference pairs 训练 |
