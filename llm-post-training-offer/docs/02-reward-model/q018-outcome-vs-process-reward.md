---
id: Q018
title: "Outcome Reward 与 Process Reward 有什么本质差异？"
chapter: 2
...
# Q018 · Outcome Reward 与 Process Reward 有什么本质差异？

> **题型**：高频题 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q017](../02-reward-model/q017-detect-reward-hacking.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q019 →](../02-reward-model/q019-multi-reward-composition.md)
...
Outcome Reward 只评价最终结果，保留探索自由但 credit assignment 粗；Process Reward 对中间步骤提供 更密集信号，但可能把人工偏见写入搜索路径。

### PDF 原始深入理解

在长 CoT 和 Agent 任务中，PRM 能降低信用分配难度；但若过程标签错误，会把错误“正确化”。常见策略是 outcome 为主、selective process 为辅。
...
1. 先给核心判断：Outcome Reward 只评价最终结果，保留探索自由但 credit assignment 粗；Process Reward 对中间步骤提供 更密集信号，但可能把人工偏见写入搜索路径。
2. 再解释机制：在长 CoT 和 Agent 任务中，PRM 能降低信用分配难度；但若过程标签错误，会把错误“正确化”。常见策略是 outcome 为主、selective process 为辅。
3. 最后补一个边界条件或失败模式：Outcome reward 给探索自由但 credit 稀疏；process reward 改善 credit 但会把“正确过程”先验写进训练目标，可能限制新策略。
...
Outcome reward 给探索自由但 credit 稀疏；process reward 改善 credit 但会把“正确过程”先验写进训练目标，可能限制新策略。
...
```text
Problem: Outcome Reward 与 Process Reward 有什么本质差异？
...
- 认为 PRM 一定优于 ORM。
- 边界条件：Outcome reward 给探索自由但 credit 稀疏；process reward 改善 credit 但会把“正确过程”先验写进训练目标，可能限制新策略。
...
1. 什么时候宁愿保持 sparse outcome reward？
...
> **Q018：Outcome Reward 只评价最终结果，保留探索自由但 credit assignment 粗；Process Reward 对中间步骤提供 更密集信号，但可能把人工偏见写入搜索路径。**
...
本题不能停在“Outcome Reward 只评价最终结果，保留探索自由但 credit assignment 粗；Process Reward 对中间步骤提供 更密集信号，但可能把人工偏见写入搜索路径”。更专业的表述是先确定五个对象：
...
对 **Q018**，最关键的机制判断是：**Outcome reward 给探索自由但 credit 稀疏；process reward 改善 credit 但会把“正确过程”先验写进训练目标，可能限制新策略。**

再进一步，工程上真正需要落地的是：如果引入 PRM，先验证 step label precision，并做“PRM on/off + outcome reward 固定”的 ablation；错误的 dense reward 会比 sparse reward 更系统性地误导策略。
...
本题 PDF 的深入结论是：在长 CoT 和 Agent 任务中，PRM 能降低信用分配难度；但若过程标签错误，会把错误“正确化”。常见策略是 outcome 为主、selective process 为辅。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q018
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：Outcome Reward 只评价最终结果，保留探索自由但 credit assignment 粗；Process Reward 对中间步骤提供 更密集信号，但可能把人工偏见写入搜索路径 |
