...
# Q019 · 多个 Reward / Judge 如何组合？

> **题型**：系统设计 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q018](../02-reward-model/q018-outcome-vs-process-reward.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q020 →](../02-reward-model/q020-rule-verifier-rlvr.md)
...
不要无脑线性加权。先检查量纲、相关性和冲突，再选择 normalization、constraint、gating、lexicographic priority 或 Pareto 方法。

### PDF 原始深入理解

例如安全可作为 hard constraint，任务成功为 primary reward，风格与成本做 secondary shaping，比把所有 目标放在一个任意加权和更稳。
...
1. 先给核心判断：不要无脑线性加权。先检查量纲、相关性和冲突，再选择 normalization、constraint、gating、lexicographic priority 或 Pareto 方法。
2. 再解释机制：例如安全可作为 hard constraint，任务成功为 primary reward，风格与成本做 secondary shaping，比把所有 目标放在一个任意加权和更稳。
3. 最后补一个边界条件或失败模式：多 reward 不只是线性加权问题，还涉及量纲、冲突和约束。很多安全/合法性条件更适合 hard gate，而不是可被其它 reward 抵消的负分。
...
多 reward 不只是线性加权问题，还涉及量纲、冲突和约束。很多安全/合法性条件更适合 hard gate，而不是可被其它 reward 抵消的负分。
...
- 把不同 reward 原始分数直接相加。
- 边界条件：多 reward 不只是线性加权问题，还涉及量纲、冲突和约束。很多安全/合法性条件更适合 hard gate，而不是可被其它 reward 抵消的负分。
...
1. 如果 helpfulness 与 safety judge 冲突，如何确定优化优先级？
...
> **Q019：不要无脑线性加权。先检查量纲、相关性和冲突，再选择 normalization、constraint、gating、lexicographic priority 或 Pareto 方法。**
...
本题不能停在“不要无脑线性加权。先检查量纲、相关性和冲突，再选择 normalization、constraint、gating、lexicographic priority 或 Pareto 方法”。更专业的表述是先确定五个对象：
...
对 **Q019**，最关键的机制判断是：**多 reward 不只是线性加权问题，还涉及量纲、冲突和约束。很多安全/合法性条件更适合 hard gate，而不是可被其它 reward 抵消的负分。**

再进一步，工程上真正需要落地的是：先把不可违反项变成 gate/constraint，再对可权衡目标做归一化和组合；线性加权前必须处理 reward 尺度漂移，否则一个分量会因数值尺度垄断梯度。
...
本题 PDF 的深入结论是：例如安全可作为 hard constraint，任务成功为 primary reward，风格与成本做 secondary shaping，比把所有 目标放在一个任意加权和更稳。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q019
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：不要无脑线性加权。先检查量纲、相关性和冲突，再选择 normalization、constraint、gating、lexicographic priority 或 Pareto 方法 |
