---
id: Q020
title: "为什么 rule-based verifier 是 RLVR 的关键基础设施？"
chapter: 2
chapter_title: "Preference、Reward Model 与 Reward Design"
type: "原理推导"
level: L2
priority: normal
tags: [grpo, dapo, verifier, rlvr]
source_refs: [P2, P8, P14]
---

# Q020 · 为什么 rule-based verifier 是 RLVR 的关键基础设施？

> **题型**：原理推导 · **难度**：L2 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q019](../02-reward-model/q019-multi-reward-composition.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q021 →](../03-ppo-gae/q021-ppo-on-vs-off-policy.md)
...
数学、代码、逻辑等任务可以用规则或可执行环境给低噪声、可扩展、便宜的正确性反馈。

### PDF 原始深入理解

GRPO/DAPO 的成功很大程度依赖可规模化 verifier，而不是“RL 算法本身万能”。要同时关注 verifier 的覆盖 率、误判与可被利用的漏洞。[P2][P8]

### PDF 原始常见失分点

只讨论算法，不讨论 reward 可验证性。
...
1. 先给核心判断：数学、代码、逻辑等任务可以用规则或可执行环境给低噪声、可扩展、便宜的正确性反馈。
2. 再解释机制：GRPO/DAPO 的成功很大程度依赖可规模化 verifier，而不是“RL 算法本身万能”。要同时关注 verifier 的覆盖 率、误判与可被利用的漏洞。[P2][P8]
3. 最后补一个边界条件或失败模式：RLVR 的真正稀缺资源是高质量、低成本、难被利用的 verifier。算法只是放大 verifier 提供的可学习信号。
...
RLVR 的真正稀缺资源是高质量、低成本、难被利用的 verifier。算法只是放大 verifier 提供的可学习信号。
...
```text
Problem: 为什么 rule-based verifier 是 RLVR 的关键基础设施？
...
- 只讨论算法，不讨论 reward 可验证性。
- 边界条件：RLVR 的真正稀缺资源是高质量、低成本、难被利用的 verifier。算法只是放大 verifier 提供的可学习信号。
...
1. 哪些开放任务可以被重新表述成可验证子任务？ 第三章 PPO / RLHF：面试必须能手推的经典主干 本章目标 完成面试题 21-30。要求不是记忆名词，而是把每个方法与其输入、目标函数、failure mode 和工程代价连接起来。
...
> **Q020：数学、代码、逻辑等任务可以用规则或可执行环境给低噪声、可扩展、便宜的正确性反馈。**
...
本题不能停在“数学、代码、逻辑等任务可以用规则或可执行环境给低噪声、可扩展、便宜的正确性反馈”。更专业的表述是先确定五个对象：
...
对 **Q020**，最关键的机制判断是：**RLVR 的真正稀缺资源是高质量、低成本、难被利用的 verifier。算法只是放大 verifier 提供的可学习信号。**

再进一步，工程上真正需要落地的是：verifier 要单独做测试集：等价答案、格式扰动、边界值、超时、对抗输出。RLVR 的上限通常先受 verifier precision 限制，再受 policy optimizer 限制。
...
本题 PDF 的深入结论是：GRPO/DAPO 的成功很大程度依赖可规模化 verifier，而不是“RL 算法本身万能”。要同时关注 verifier 的覆盖 率、误判与可被利用的漏洞。[P2][P8]。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q020
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：数学、代码、逻辑等任务可以用规则或可执行环境给低噪声、可扩展、便宜的正确性反馈 |
