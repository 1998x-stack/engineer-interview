---
id: Q022
title: "公开真题：为什么 PPO 需要 Importance Sampling？"
chapter: 3
...
priority: normal
tags: [ppo]
source_refs: [I1, I11, P1, P3, P14]
---

# Q022 · 公开真题：为什么 PPO 需要 Importance Sampling？

> **题型**：公开真题 · **难度**：L2 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q021](../03-ppo-gae/q021-ppo-on-vs-off-policy.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q023 →](../03-ppo-gae/q023-ppo-clipped-objective.md)
...
数据来自 π_old，但目标在更新 π_θ；用 π_θ/π_old 将旧分布下的期望校正到新策略附近。

### PDF 原始深入理解

它不是魔法：ratio 方差会随策略差异增大，支持集不重合时也无法可靠修正，所以需要 clipping / trust-region 思想限制每次更新幅度。
...
1. 先给核心判断：数据来自 π_old，但目标在更新 π_θ；用 π_θ/π_old 将旧分布下的期望校正到新策略附近。
2. 再解释机制：它不是魔法：ratio 方差会随策略差异增大，支持集不重合时也无法可靠修正，所以需要 clipping / trust-region 思想限制每次更新幅度。
3. 最后补一个边界条件或失败模式：importance ratio 来自换测度：数据由 old policy 采样，但目标想评价 current policy。它只能校正 action likelihood，不会自动修复严重 distribution shift。
...
importance ratio 来自换测度：数据由 old policy 采样，但目标想评价 current policy。它只能校正 action likelihood，不会自动修复严重 distribution shift。
...
```text
Problem: 公开真题：为什么 PPO 需要 Importance Sampling？
...
- 只背“修正分布偏差”，不解释期望变换。
- 边界条件：importance ratio 来自换测度：数据由 old policy 采样，但目标想评价 current policy。它只能校正 action likelihood，不会自动修复严重 distribution shift。
...
1. importance ratio 为什么容易爆？ [I1][I11]
...
> **Q022：数据来自 π_old，但目标在更新 π_θ；用 π_θ/π_old 将旧分布下的期望校正到新策略附近。**
...
本题不能停在“数据来自 π_old，但目标在更新 π_θ；用 π_θ/π_old 将旧分布下的期望校正到新策略附近”。更专业的表述是先确定五个对象：
...
对 **Q022**，最关键的机制判断是：**importance ratio 来自换测度：数据由 old policy 采样，但目标想评价 current policy。它只能校正 action likelihood，不会自动修复严重 distribution shift。**

再进一步，工程上真正需要落地的是：importance ratio 用 logprob 差计算并先在 log-space 观察分布；极端 ratio 往往不是正常学习，而是版本错位、mask/tokenizer 不一致或 stale rollout 的信号。
...
本题 PDF 的深入结论是：它不是魔法：ratio 方差会随策略差异增大，支持集不重合时也无法可靠修正，所以需要 clipping / trust-region 思想限制每次更新幅度。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q022
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：数据来自 π_old，但目标在更新 π_θ；用 π_θ/π_old 将旧分布下的期望校正到新策略附近 |
