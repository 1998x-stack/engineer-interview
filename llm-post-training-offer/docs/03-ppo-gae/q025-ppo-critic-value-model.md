---
id: Q025
title: "PPO 为什么需要 Critic / Value Model？"
chapter: 3
...
# Q025 · PPO 为什么需要 Critic / Value Model？

> **题型**：高频题 · **难度**：L2 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q024](../03-ppo-gae/q024-ppo-clip-positive-negative-advantage.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q026 →](../03-ppo-gae/q026-gae-derivation-lambda.md)
...
policy gradient 需要 advantage；Critic 估计 V(s) 作为 baseline，降低 Monte-Carlo return 的方差。

### PDF 原始深入理解

Critic 是性能与成本的 trade-off：估得准可显著降方差，但 LLM critic 本身几乎是另一个大模型，训练和显存成 本高，也是 GRPO 去 critic 的主要动机。

### PDF 原始常见失分点

说 critic “负责打 reward”；reward model 与 value model 职责不同。
...
1. 先给核心判断：policy gradient 需要 advantage；Critic 估计 V(s) 作为 baseline，降低 Monte-Carlo return 的方差。
2. 再解释机制：Critic 是性能与成本的 trade-off：估得准可显著降方差，但 LLM critic 本身几乎是另一个大模型，训练和显存成 本高，也是 GRPO 去 critic 的主要动机。
3. 最后补一个边界条件或失败模式：Critic 的本质是 baseline/bootstrapping，用少量 bias 换显著 variance reduction；LLM 长序列下 critic 还带来一整套显存与稳定性成本。
...
Critic 的本质是 baseline/bootstrapping，用少量 bias 换显著 variance reduction；LLM 长序列下 critic 还带来一整套显存与稳定性成本。
...
```text
Problem: PPO 为什么需要 Critic / Value Model？
...
- 说 critic “负责打 reward”；reward model 与 value model 职责不同。
- 边界条件：Critic 的本质是 baseline/bootstrapping，用少量 bias 换显著 variance reduction；LLM 长序列下 critic 还带来一整套显存与稳定性成本。
...
> **Q025：policy gradient 需要 advantage；Critic 估计 V(s) 作为 baseline，降低 Monte-Carlo return 的方差。**
...
本题不能停在“policy gradient 需要 advantage；Critic 估计 V(s) 作为 baseline，降低 Monte-Carlo return 的方差”。更专业的表述是先确定五个对象：
...
对 **Q025**，最关键的机制判断是：**Critic 的本质是 baseline/bootstrapping，用少量 bias 换显著 variance reduction；LLM 长序列下 critic 还带来一整套显存与稳定性成本。**

再进一步，工程上真正需要落地的是：critic 需要 value loss、explained variance、value clipping 和 return scale 监控；value loss 下降并不代表 advantage 好，关键看 baseline 是否真正降低 policy-gradient variance。
...
本题 PDF 的深入结论是：Critic 是性能与成本的 trade-off：估得准可显著降方差，但 LLM critic 本身几乎是另一个大模型，训练和显存成 本高，也是 GRPO 去 critic 的主要动机。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q025
...
| # | 指标 | 如何解释 |
|---:|---|---|
| 1 | `approx KL` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 2 | `clip fraction` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 3 | `ratio quantiles` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `entropy` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `value loss/explained variance` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `grad norm` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `data age` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：policy gradient 需要 advantage；Critic 估计 V(s) 作为 baseline，降低 Monte-Carlo return 的方差 |
