...
priority: high
tags: [gae]
source_refs: [I1, I6, P3, P1, P14]
---

# Q026 · 公开真题：GAE 如何计算？λ 控制什么？

> **题型**：公开真题 · **难度**：L3 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q025](../03-ppo-gae/q025-ppo-critic-value-model.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q027 →](../03-ppo-gae/q027-ppo-four-models.md)
...
δ_t = r_t + γV(s_{t+1}) - V(s_t)；GAE = Σ_l (γλ)^l δ_{t+l}。

### PDF 原始深入理解

λ 在 bias/variance 之间插值：低 λ 更依赖 bootstrapped value、方差低偏差高；高 λ 更接近 Monte Carlo、偏 差低方差高。[P3]
...
1. 先给核心判断：δ_t = r_t + γV(s_{t+1}) - V(s_t)；GAE = Σ_l (γλ)^l δ_{t+l}。
2. 再解释机制：λ 在 bias/variance 之间插值：低 λ 更依赖 bootstrapped value、方差低偏差高；高 λ 更接近 Monte Carlo、偏 差低方差高。[P3]
3. 最后补一个边界条件或失败模式：GAE 把不同 horizon 的 TD residual 指数加权；lambda 越大越接近 Monte Carlo，bias 下降但 variance 上升。
...
GAE 把不同 horizon 的 TD residual 指数加权；lambda 越大越接近 Monte Carlo，bias 下降但 variance 上升。
...
\[\delta_t=r_t+\gamma V(s_{t+1})-V(s_t),\qquad \hat A_t^{GAE}=\sum_{l=0}^{\infty}(\gamma\lambda)^l\delta_{t+l}\]
...
- 把 λ 与 γ 混淆。
- 边界条件：GAE 把不同 horizon 的 TD residual 指数加权；lambda 越大越接近 Monte Carlo，bias 下降但 variance 上升。
...
1. 如果 episode 很长且 value 很差，你会把 λ 调高还是调低？为什么？ [I1][I6]
2. lambda=0 与 lambda=1 分别退化成什么直觉？
3. 终局 reward 的 LLM episode 中 GAE 怎样处理中间 token reward？
...
> **Q026：δ_t = r_t + γV(s_{t+1}) - V(s_t)；GAE = Σ_l (γλ)^l δ_{t+l}。**
...
本题不能停在“δ_t = r_t + γV(s_{t+1}) - V(s_t)；GAE = Σ_l (γλ)^l δ_{t+l}”。更专业的表述是先确定五个对象：
...
对 **Q026**，最关键的机制判断是：**GAE 把不同 horizon 的 TD residual 指数加权；lambda 越大越接近 Monte Carlo，bias 下降但 variance 上升。**

再进一步，工程上真正需要落地的是：GAE 的调参不能脱离 horizon 与 reward sparsity：短任务可用更低 lambda，长稀疏任务通常需要更长信用传播；同时检查 bootstrap 截断是否正确处理 EOS/timeout。
...
本题 PDF 的深入结论是：λ 在 bias/variance 之间插值：低 λ 更依赖 bootstrapped value、方差低偏差高；高 λ 更接近 Monte Carlo、偏 差低方差高。[P3]。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q026
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：δ_t = r_t + γV(s_{t+1}) - V(s_t)；GAE = Σ_l (γλ)^l δ_{t+l} |
