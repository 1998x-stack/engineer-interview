---
id: Q027
title: "为什么经典 PPO-RLHF 常说需要四个模型？"
chapter: 3
chapter_title: "PPO / GAE / 经典 RLHF"
type: "高频题"
level: L2
priority: high
tags: [ppo, grpo, reward]
source_refs: [P1, P3, P14]
---

# Q027 · 为什么经典 PPO-RLHF 常说需要四个模型？

> **题型**：高频题 · **难度**：L2 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q026](../03-ppo-gae/q026-gae-derivation-lambda.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q028 →](../03-ppo-gae/q028-reference-model-kl.md)
...
Actor 更新策略；Critic 估值；Reward Model 评分；Reference Model 提供 KL anchor。

### PDF 原始深入理解

系统成本不只模型权重，还包括 optimizer state、activation、rollout KV cache 与跨卡通信。GRPO 去 critic 后通常能明显降低训练内存，但 rollout 采样仍很贵。

### PDF 原始常见失分点

以为 Reference 与 old policy 是同一个概念。
...
1. 先给核心判断：Actor 更新策略；Critic 估值；Reward Model 评分；Reference Model 提供 KL anchor。
2. 再解释机制：系统成本不只模型权重，还包括 optimizer state、activation、rollout KV cache 与跨卡通信。GRPO 去 critic 后通常能明显降低训练内存，但 rollout 采样仍很贵。
3. 最后补一个边界条件或失败模式：四模型并不意味着一定四份完整权重常驻，但逻辑角色确实是 actor、critic、reward、reference；工程优化围绕参数共享、offload 与 colocate 展开。
...
四模型并不意味着一定四份完整权重常驻，但逻辑角色确实是 actor、critic、reward、reference；工程优化围绕参数共享、offload 与 colocate 展开。
...
```text
Problem: 为什么经典 PPO-RLHF 常说需要四个模型？
...
- 以为 Reference 与 old policy 是同一个概念。
- 边界条件：四模型并不意味着一定四份完整权重常驻，但逻辑角色确实是 actor、critic、reward、reference；工程优化围绕参数共享、offload 与 colocate 展开。
...
1. π_old、π_ref、π_rollout 在工程系统里什么时候可能不同？
...
> **Q027：Actor 更新策略；Critic 估值；Reward Model 评分；Reference Model 提供 KL anchor。**
...
本题不能停在“Actor 更新策略；Critic 估值；Reward Model 评分；Reference Model 提供 KL anchor”。更专业的表述是先确定五个对象：
...
对 **Q027**，最关键的机制判断是：**四模型并不意味着一定四份完整权重常驻，但逻辑角色确实是 actor、critic、reward、reference；工程优化围绕参数共享、offload 与 colocate 展开。**

再进一步，工程上真正需要落地的是：四个逻辑角色可通过共享底座、冻结、量化或分时加载降低物理常驻成本，但任何共享都会引入耦合；面试要同时说“逻辑角色”和“实际部署方式”。
...
本题 PDF 的深入结论是：系统成本不只模型权重，还包括 optimizer state、activation、rollout KV cache 与跨卡通信。GRPO 去 critic 后通常能明显降低训练内存，但 rollout 采样仍很贵。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q027
...
| 6 | `approx KL` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `clip fraction` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：Actor 更新策略；Critic 估值；Reward Model 评分；Reference Model 提供 KL anchor |
