---
id: Q028
title: "Reference Model 与 KL penalty 的本质作用是什么？"
chapter: 3
...
priority: normal
tags: [reward]
source_refs: [P1, P3, P14]
---

# Q028 · Reference Model 与 KL penalty 的本质作用是什么？

> **题型**：原理推导 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q027](../03-ppo-gae/q027-ppo-four-models.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q029 →](../03-ppo-gae/q029-adaptive-kl-beta.md)
...
常见目标是 E[R] - β KL(π||π_ref)，用 reference 作为语言/行为先验，防止 policy 为追 reward 漂向异常分布。

### PDF 原始深入理解

KL 也是稳定性约束与 alignment anchor。注意 RL 算法中的 old policy 用于 trust region/importance ratio， reference 则用于长期行为锚定，两者概念不同。
...
1. 先给核心判断：常见目标是 E[R] - β KL(π||π_ref)，用 reference 作为语言/行为先验，防止 policy 为追 reward 漂向异常分布。
2. 再解释机制：KL 也是稳定性约束与 alignment anchor。注意 RL 算法中的 old policy 用于 trust region/importance ratio， reference 则用于长期行为锚定，两者概念不同。
3. 最后补一个边界条件或失败模式：reference model 是行为先验，KL 是“不要为了 proxy reward 远离语言分布”的软约束；它同时影响稳定性、探索和 reward hacking 风险。
...
reference model 是行为先验，KL 是“不要为了 proxy reward 远离语言分布”的软约束；它同时影响稳定性、探索和 reward hacking 风险。
...
\[J(\pi)=\mathbb E_{y\sim\pi}[R(x,y)]-\beta D_{KL}(\pi(\cdot|x)\|\pi_{ref}(\cdot|x))\]
...
```text
Problem: Reference Model 与 KL penalty 的本质作用是什么？
...
- 把所有“旧模型”都叫 ref。
- 边界条件：reference model 是行为先验，KL 是“不要为了 proxy reward 远离语言分布”的软约束；它同时影响稳定性、探索和 reward hacking 风险。
...
1. 如果不断把 ref 更新成最新 policy，会发生什么？
2. 这个估计量的 bias/variance 从哪里来？
3. 如果 ratio/advantage 数值极端，你先查哪三个日志？
4. 在 LLM token 级实现时 mask 与 normalization 怎么做？
...
> **Q028：常见目标是 E[R] - β KL(π||π_ref)，用 reference 作为语言/行为先验，防止 policy 为追 reward 漂向异常分布。**
...
- [Q021 · 公开真题：PPO 是 on-policy 还是 off-policy？为什么答案不能只说一个词？](q021-ppo-on-vs-off-policy.md)
...
本题不能停在“常见目标是 E[R] - β KL(π||π_ref)，用 reference 作为语言/行为先验，防止 policy 为追 reward 漂向异常分布”。更专业的表述是先确定五个对象：
...
对 **Q028**，最关键的机制判断是：**reference model 是行为先验，KL 是“不要为了 proxy reward 远离语言分布”的软约束；它同时影响稳定性、探索和 reward hacking 风险。**

再进一步，工程上真正需要落地的是：KL 的实现需明确 forward/reverse、token-level/sequence-level、是否采样估计；reference tokenizer/template 不一致会让 KL 失去行为锚点含义。
...
本题 PDF 的深入结论是：KL 也是稳定性约束与 alignment anchor。注意 RL 算法中的 old policy 用于 trust region/importance ratio， reference 则用于长期行为锚定，两者概念不同。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q028
...
| 4 | `approx KL` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `clip fraction` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `ratio quantiles` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `entropy` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `value loss/explained variance` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：常见目标是 E[R] - β KL(π||π_ref)，用 reference 作为语言/行为先验，防止 policy 为追 reward 漂向异常分布 |
