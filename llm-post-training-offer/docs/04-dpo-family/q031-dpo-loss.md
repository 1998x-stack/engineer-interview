---
id: Q031
title: "DPO loss 怎么写？四个 log-prob 项各在做什么？"
chapter: 4
...
# Q031 · DPO loss 怎么写？四个 log-prob 项各在做什么？

> **题型**：高频题 · **难度**：L3 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q030](../03-ppo-gae/q030-ppo-engineering-cost.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q032 →](../04-dpo-family/q032-dpo-derivation.md)
...
DPO 比较 chosen 与 rejected 相对 reference 的 log-prob 改变量：β[(logπθ(yw)-logπref(yw))-(logπθ(yl)- logπref(yl))]，再过 log-sigmoid。

### PDF 原始深入理解

它并非简单“提高 chosen、降低 rejected”，而是相对 reference 比较两者隐式 reward 的 margin。[P4]

### PDF 原始常见失分点

漏掉 reference 或把序列 logprob 写成单 token。
...
1. 先给核心判断：DPO 比较 chosen 与 rejected 相对 reference 的 log-prob 改变量：β[(logπθ(yw)-logπref(yw))-(logπθ(yl)- logπref(yl))]，再过 log-sigmoid。
2. 再解释机制：它并非简单“提高 chosen、降低 rejected”，而是相对 reference 比较两者隐式 reward 的 margin。[P4]
3. 最后补一个边界条件或失败模式：DPO 的四个 log-prob 项构造“相对 reference 的 winner 增益减 loser 增益”。它优化的是隐式 reward margin，不是单纯提高 chosen 概率。
...
DPO 的四个 log-prob 项构造“相对 reference 的 winner 增益减 loser 增益”。它优化的是隐式 reward margin，不是单纯提高 chosen 概率。
...
```text
logp_w  = policy(chosen)
logp_l  = policy(rejected)
ref_w   = reference(chosen)
ref_l   = reference(rejected)
...
- 漏掉 reference 或把序列 logprob 写成单 token。
- 边界条件：DPO 的四个 log-prob 项构造“相对 reference 的 winner 增益减 loser 增益”。它优化的是隐式 reward margin，不是单纯提高 chosen 概率。
...
1. 为什么 DPO 的序列长度偏差值得关注？
2. 为什么 DPO 不是简单 chosen CE - rejected CE？
3. reference logprob 是否可以预计算？什么时候不行？
...
> **Q031：DPO 比较 chosen 与 rejected 相对 reference 的 log-prob 改变量：β[(logπθ(yw)-logπref(yw))-(logπθ(yl)- logπref(yl))]，再过 log-sigmoid。**
...
本题不能停在“DPO 比较 chosen 与 rejected 相对 reference 的 log-prob 改变量：β[(logπθ(yw)-logπref(yw))-(logπθ(yl)- logπref(yl))]，再过 log-sigmoid”。更专业的表述是先确定五个对象：
...
对 **Q031**，最关键的机制判断是：**DPO 的四个 log-prob 项构造“相对 reference 的 winner 增益减 loser 增益”。它优化的是隐式 reward margin，不是单纯提高 chosen 概率。**

再进一步，工程上真正需要落地的是：DPO 实现最容易错在 chosen/rejected mask、sequence logprob 聚合、reference logprob 缓存和 beta 符号；用可控 toy pair 验证 preference margin 是否朝正确方向更新。
...
本题 PDF 的深入结论是：它并非简单“提高 chosen、降低 rejected”，而是相对 reference 比较两者隐式 reward 的 margin。[P4]。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：DPO 比较 chosen 与 rejected 相对 reference 的 log-prob 改变量：β[(logπθ(yw)-logπref(yw))-(logπθ(yl)- logπref(yl))]，再过 log-sigmoid |
