---
id: Q032
title: "DPO 如何从 KL-regularized RLHF 推导出来？"
chapter: 4
...
# Q032 · DPO 如何从 KL-regularized RLHF 推导出来？

> **题型**：原理推导 · **难度**：L4 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q031](../04-dpo-family/q031-dpo-loss.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q033 →](../04-dpo-family/q033-why-dpo-simpler-than-ppo.md)
...
从 max_π E[r]-βKL(π||π_ref) 得到最优策略 π*(y|x) ∝ π_ref(y|x) exp(r/β)。重排得 r = β log(π*/π_ref)+C(x)， 代回 Bradley-Terry 偏好概率，C(x) 在 pair 差分中消失。

### PDF 原始深入理解

这道题的价值在于说明 DPO 不是“拍脑袋的分类损失”，而是把 RLHF 的 reward parameterization 解析化。 [P4]

### PDF 原始常见失分点

只背最终 loss，不会讲 partition function 为什么消掉。
...
1. 先给核心判断：从 max_π E[r]-βKL(π||π_ref) 得到最优策略 π*(y|x) ∝ π_ref(y|x) exp(r/β)。重排得 r = β log(π*/π_ref)+C(x)， 代回 Bradley-Terry 偏好概率，C(x) 在 pair 差分中消失。
2. 再解释机制：这道题的价值在于说明 DPO 不是“拍脑袋的分类损失”，而是把 RLHF 的 reward parameterization 解析化。 [P4]
3. 最后补一个边界条件或失败模式：DPO 推导的关键是 KL-regularized RL 的最优 policy 有闭式形式；把 reward 用 log policy ratio 表示后，Bradley–Terry 中的 partition term 会相消。
...
DPO 推导的关键是 KL-regularized RL 的最优 policy 有闭式形式；把 reward 用 log policy ratio 表示后，Bradley–Terry 中的 partition term 会相消。
...
\[\pi^*(y|x)\propto \pi_{ref}(y|x)e^{r(x,y)/\beta}\] \[r(x,y)=\beta\log\frac{\pi^*(y|x)}{\pi_{ref}(y|x)}+C(x)\]
...
```text
Problem: DPO 如何从 KL-regularized RLHF 推导出来？
...
- 只背最终 loss，不会讲 partition function 为什么消掉。
- 边界条件：DPO 推导的关键是 KL-regularized RL 的最优 policy 有闭式形式；把 reward 用 log policy ratio 表示后，Bradley–Terry 中的 partition term 会相消。
...
1. DPO 的哪些假设在真实数据里可能不成立？
2. partition function 为什么会在 pair difference 中消掉？
3. 这个推导依赖哪些 reward/policy 假设？
4. 如果 preference pair 来自旧模型，怎么处理 distribution shift？
5. reference-free 真正省掉了什么，又失去了什么？
6. 长度偏差怎样做 controlled evaluation？
...
> **Q032：从 max_π E[r]-βKL(π||π_ref) 得到最优策略 π*(y|x) ∝ π_ref(y|x) exp(r/β)。重排得 r = β log(π*/π_ref)+C(x)， 代回 Bradley-Terry 偏好概率，C(x) 在 pair 差分中消失。**
...
- [Q033 · 为什么 DPO 比 PPO 简单很多？](q033-why-dpo-simpler-than-ppo.md)
...
本题不能停在“从 max_π E[r]-βKL(π||π_ref) 得到最优策略 π*(y|x) ∝ π_ref(y|x) exp(r/β)。重排得 r = β log(π*/π_ref)+C(x)， 代回 Bradley-Terry 偏好概率，C(x) 在 pair 差分中消失”。更专业的表述是先确定五个对象：
...
对 **Q032**，最关键的机制判断是：**DPO 推导的关键是 KL-regularized RL 的最优 policy 有闭式形式；把 reward 用 log policy ratio 表示后，Bradley–Terry 中的 partition term 会相消。**

再进一步，工程上真正需要落地的是：推导时必须说出 C(x) 为什么在 pair difference 中消掉，以及 BT preference likelihood 如何把不可辨识的 reward 常数消去；这是从公式背诵到真正理解的分水岭。
...
本题 PDF 的深入结论是：这道题的价值在于说明 DPO 不是“拍脑袋的分类损失”，而是把 RLHF 的 reward parameterization 解析化。 [P4]。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q032
...
| # | 指标 | 如何解释 |
|---:|---|---|
| 1 | `KL target/error` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 2 | `reward quantiles` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 3 | `reward calibration` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `chosen-rejected margin` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `preference margin` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `chosen/rejected logprob` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `implicit reward gap` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `reference KL` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：从 max_π E[r]-βKL(π||π_ref) 得到最优策略 π*(y|x) ∝ π_ref(y|x) exp(r/β)。重排得 r = β log(π*/π_ref)+C(x)， 代回 Bradley-Terry 偏好概率，C(x) 在 pair 差分中消失 |
