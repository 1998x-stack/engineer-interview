---
id: Q034
title: "公开真题：PPO 与 DPO 怎么选？"
chapter: 4
chapter_title: "DPO 与 Offline Preference Optimization"
type: "公开真题"
level: L3
priority: high
tags: [sft, ppo, dpo, reward]
source_refs: [I6, I10, P4]
---

# Q034 · 公开真题：PPO 与 DPO 怎么选？

> **题型**：公开真题 · **难度**：L3 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q033](../04-dpo-family/q033-why-dpo-simpler-than-ppo.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q035 →](../04-dpo-family/q035-dpo-offline-distribution-shift.md)
...
如果目标主要是静态偏好和风格对齐，DPO 简洁稳定；如果 reward 可计算、需要探索新策略或环境交互， online RL 更自然。

### PDF 原始深入理解

选择要由任务约束推出，而不是算法排名。很多生产 pipeline 会先 SFT/DPO 做行为对齐，再在 verifiable domain 上做 RL。

### PDF 原始常见失分点

把“DPO 离线、PPO 在线”说完就停；还要讨论 reward 与 exploration。
...
1. 先给核心判断：如果目标主要是静态偏好和风格对齐，DPO 简洁稳定；如果 reward 可计算、需要探索新策略或环境交互， online RL 更自然。
2. 再解释机制：选择要由任务约束推出，而不是算法排名。很多生产 pipeline 会先 SFT/DPO 做行为对齐，再在 verifiable domain 上做 RL。
3. 最后补一个边界条件或失败模式：PPO/DPO 不是“性能谁更高”的静态比较：主观偏好、静态 pair、低系统预算偏向 DPO；可验证任务、需要探索时更偏 online RL。
...
PPO/DPO 不是“性能谁更高”的静态比较：主观偏好、静态 pair、低系统预算偏向 DPO；可验证任务、需要探索时更偏 online RL。
...
- 把“DPO 离线、PPO 在线”说完就停；还要讨论 reward 与 exploration。
- 边界条件：PPO/DPO 不是“性能谁更高”的静态比较：主观偏好、静态 pair、低系统预算偏向 DPO；可验证任务、需要探索时更偏 online RL。
...
1. 开放聊天 + 数学 reasoning 的统一模型如何分阶段？ [I6][I10]
...
> **Q034：如果目标主要是静态偏好和风格对齐，DPO 简洁稳定；如果 reward 可计算、需要探索新策略或环境交互， online RL 更自然。**

## 13. 参考资料

- **[I6] 公开面经**：[牛客：百川大模型算法二面](https://www.nowcoder.com/enterprise/267815/interview)
- **[I10] 公开面经**：[牛客：阿里大模型算法（SFT / DPO / PPO / GRPO / Agent）](https://www.nowcoder.com/discuss/923309821460221952)
...
本题不能停在“如果目标主要是静态偏好和风格对齐，DPO 简洁稳定；如果 reward 可计算、需要探索新策略或环境交互， online RL 更自然”。更专业的表述是先确定五个对象：
...
对 **Q034**，最关键的机制判断是：**PPO/DPO 不是“性能谁更高”的静态比较：主观偏好、静态 pair、低系统预算偏向 DPO；可验证任务、需要探索时更偏 online RL。**

再进一步，工程上真正需要落地的是：决策时比较“数据是否覆盖目标策略分布”和“探索是否有价值”。若任务存在可验证新策略空间，online RL 的潜在增益更大；若主要做风格偏好，DPO 往往性价比更高。
...
本题 PDF 的深入结论是：选择要由任务约束推出，而不是算法排名。很多生产 pipeline 会先 SFT/DPO 做行为对齐，再在 verifiable domain 上做 RL。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q034
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：如果目标主要是静态偏好和风格对齐，DPO 简洁稳定；如果 reward 可计算、需要探索新策略或环境交互， online RL 更自然 |
