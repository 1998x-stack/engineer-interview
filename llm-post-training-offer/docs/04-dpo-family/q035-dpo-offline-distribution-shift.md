---
id: Q035
title: "DPO 的 offline distribution shift 问题是什么？"
chapter: 4
...
# Q035 · DPO 的 offline distribution shift 问题是什么？

> **题型**：原理推导 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q034](../04-dpo-family/q034-ppo-vs-dpo.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q036 →](../04-dpo-family/q036-dpo-overfit-length-bias.md)
...
偏好数据来自旧 policy/其他模型，而训练后的 πθ 会离开该分布；pair 可能不再覆盖模型新出现的错误。

### PDF 原始深入理解

因此 DPO 缺少“采样 → 反馈 → 再优化”的闭环。可以用 iterative DPO、online preference collection 或主动 数据挖掘改善。
...
1. 先给核心判断：偏好数据来自旧 policy/其他模型，而训练后的 πθ 会离开该分布；pair 可能不再覆盖模型新出现的错误。
2. 再解释机制：因此 DPO 缺少“采样 → 反馈 → 再优化”的闭环。可以用 iterative DPO、online preference collection 或主动 数据挖掘改善。
3. 最后补一个边界条件或失败模式：offline shift 的关键是训练 pair 由旧 policy 产生，而最终 policy 在另一个分布上生成；loss 继续下降并不保证新分布上的 preference 仍可靠。
...
offline shift 的关键是训练 pair 由旧 policy 产生，而最终 policy 在另一个分布上生成；loss 继续下降并不保证新分布上的 preference 仍可靠。
...
```text
Problem: DPO 的 offline distribution shift 问题是什么？
...
- 认为训练集越大就自动解决 shift。
- 边界条件：offline shift 的关键是训练 pair 由旧 policy 产生，而最终 policy 在另一个分布上生成；loss 继续下降并不保证新分布上的 preference 仍可靠。
...
> **Q035：偏好数据来自旧 policy/其他模型，而训练后的 πθ 会离开该分布；pair 可能不再覆盖模型新出现的错误。**
...
本题不能停在“偏好数据来自旧 policy/其他模型，而训练后的 πθ 会离开该分布；pair 可能不再覆盖模型新出现的错误”。更专业的表述是先确定五个对象：
...
对 **Q035**，最关键的机制判断是：**offline shift 的关键是训练 pair 由旧 policy 产生，而最终 policy 在另一个分布上生成；loss 继续下降并不保证新分布上的 preference 仍可靠。**

再进一步，工程上真正需要落地的是：用 policy checkpoint 距离、generation behavior shift 与新旧数据 win-rate 衡量 offline shift；必要时迭代采样/重标，而不是在旧 pair 上无限加 epoch。
...
本题 PDF 的深入结论是：因此 DPO 缺少“采样 → 反馈 → 再优化”的闭环。可以用 iterative DPO、online preference collection 或主动 数据挖掘改善。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q035
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：偏好数据来自旧 policy/其他模型，而训练后的 πθ 会离开该分布；pair 可能不再覆盖模型新出现的错误 |
