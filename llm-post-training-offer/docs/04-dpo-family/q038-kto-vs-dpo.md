...
# Q038 · KTO 与 DPO 的数据要求有何差异？

> **题型**：高频题 · **难度**：L2 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q037](../04-dpo-family/q037-dpo-beta.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q039 →](../04-dpo-family/q039-orpo-reference-free.md)
...
DPO 依赖 chosen/rejected pair；KTO 可从单个样本的 desirable/undesirable 二元信号学习，并使用 prospect-theoretic utility。[P5]

### PDF 原始深入理解

当 pair 难采集、只有点赞/点踩式反馈时 KTO 更自然，但仍需考虑标签偏差和正负比例。

### PDF 原始常见失分点

说 KTO “不需要偏好数据”；它仍需要人类价值信号，只是不必成对。
...
1. 先给核心判断：DPO 依赖 chosen/rejected pair；KTO 可从单个样本的 desirable/undesirable 二元信号学习，并使用 prospect-theoretic utility。[P5]
2. 再解释机制：当 pair 难采集、只有点赞/点踩式反馈时 KTO 更自然，但仍需考虑标签偏差和正负比例。
3. 最后补一个边界条件或失败模式：KTO 把 pairwise preference 降为单样本 desirable/undesirable 信号，优势是标注门槛低；代价是失去同 prompt 直接比较提供的局部排序信息。
...
KTO 把 pairwise preference 降为单样本 desirable/undesirable 信号，优势是标注门槛低；代价是失去同 prompt 直接比较提供的局部排序信息。
...
- 说 KTO “不需要偏好数据”；它仍需要人类价值信号，只是不必成对。
- 边界条件：KTO 把 pairwise preference 降为单样本 desirable/undesirable 信号，优势是标注门槛低；代价是失去同 prompt 直接比较提供的局部排序信息。
...
> **Q038：DPO 依赖 chosen/rejected pair；KTO 可从单个样本的 desirable/undesirable 二元信号学习，并使用 prospect-theoretic utility。[P5]**

## 13. 参考资料

- **[P5] PDF/论文参考**：[Ethayarajh et al. — KTO: Model Alignment as Prospect Theoretic Optimization](https://arxiv.org/abs/2402.01306)
...
本题不能停在“DPO 依赖 chosen/rejected pair；KTO 可从单个样本的 desirable/undesirable 二元信号学习，并使用 prospect-theoretic utility。[P5]”。更专业的表述是先确定五个对象：
...
对 **Q038**，最关键的机制判断是：**KTO 把 pairwise preference 降为单样本 desirable/undesirable 信号，优势是标注门槛低；代价是失去同 prompt 直接比较提供的局部排序信息。**

再进一步，工程上真正需要落地的是：KTO 的工程价值在于不要求同 prompt pair，但单点 desirable/undesirable 标签的基准与类比例更重要；需检查类别不平衡和 reference anchor 的影响。
...
本题 PDF 的深入结论是：当 pair 难采集、只有点赞/点踩式反馈时 KTO 更自然，但仍需考虑标签偏差和正负比例。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q038
...
| # | 指标 | 如何解释 |
|---:|---|---|
| 1 | `chosen-rejected margin` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 2 | `preference margin` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 3 | `chosen/rejected logprob` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `implicit reward gap` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `reference KL` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `length-controlled win-rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `offline coverage` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：DPO 依赖 chosen/rejected pair；KTO 可从单个样本的 desirable/undesirable 二元信号学习，并使用 prospect-theoretic utility。[P5] |
