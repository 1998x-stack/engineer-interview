---
id: Q040
title: "SimPO 相比 DPO 改了什么？为什么使用平均 log probability？"
chapter: 4
...
# Q040 · SimPO 相比 DPO 改了什么？为什么使用平均 log probability？

> **题型**：高频题 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q039](../04-dpo-family/q039-orpo-reference-free.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q041 →](../05-grpo/q041-ppo-vs-grpo.md)
...
SimPO 使用 sequence average log probability 作为隐式 reward，reference-free，并加入 target reward margin。[P7]

### PDF 原始深入理解

平均 logprob 对长度做归一，更贴近实际生成评分，并降低 reference 计算成本。

### PDF 原始常见失分点

把 length normalization 只看作数值技巧；它直接影响偏好目标。
...
1. 先给核心判断：SimPO 使用 sequence average log probability 作为隐式 reward，reference-free，并加入 target reward margin。[P7]
2. 再解释机制：平均 logprob 对长度做归一，更贴近实际生成评分，并降低 reference 计算成本。
3. 最后补一个边界条件或失败模式：SimPO 用平均 log-prob 作为 implicit reward，让 reward 与生成时 sequence scoring 更一致，并用 margin 强化 chosen/rejected 分离。
...
SimPO 用平均 log-prob 作为 implicit reward，让 reward 与生成时 sequence scoring 更一致，并用 margin 强化 chosen/rejected 分离。
...
```text
Problem: SimPO 相比 DPO 改了什么？为什么使用平均 log probability？
...
- 把 length normalization 只看作数值技巧；它直接影响偏好目标。
- 边界条件：SimPO 用平均 log-prob 作为 implicit reward，让 reward 与生成时 sequence scoring 更一致，并用 margin 强化 chosen/rejected 分离。
...
1. GSPO 的几何平均 ratio 与 SimPO 的平均 logprob 有什么思想共性？ 举一反三 / 章节检查 尝试不看答案，用“任务 → 数据 → reward → exploration → credit → compute → failure → eval”重新讲本章。若能把 3 个以上问题串成一条因果链，而不是逐题背诵，本章才算掌 握。 第五章 GRPO：“去 Critic”到真实工程问题 本章目标 完成面试题 41-50。要求不是记忆名词，而是把每个方法与其输入、目标函数、failure mode 和工程代价连接起来。
...
> **Q040：SimPO 使用 sequence average log probability 作为隐式 reward，reference-free，并加入 target reward margin。[P7]**

## 13. 参考资料

- **[P7] PDF/论文参考**：[Meng et al. — SimPO: Simple Preference Optimization with a Reference-Free Reward](https://arxiv.org/abs/2405.14734)
...
本题不能停在“SimPO 使用 sequence average log probability 作为隐式 reward，reference-free，并加入 target reward margin。[P7]”。更专业的表述是先确定五个对象：
...
对 **Q040**，最关键的机制判断是：**SimPO 用平均 log-prob 作为 implicit reward，让 reward 与生成时 sequence scoring 更一致，并用 margin 强化 chosen/rejected 分离。**

再进一步，工程上真正需要落地的是：SimPO 的平均 logprob 与 margin 设计缓解长度效应，但不是自动消除所有 length bias；仍要做长度分桶与生成端控制实验。
...
本题 PDF 的深入结论是：平均 logprob 对长度做归一，更贴近实际生成评分，并降低 reference 计算成本。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q040
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：SimPO 使用 sequence average log probability 作为隐式 reward，reference-free，并加入 target reward margin。[P7] |
