...
source_refs: [I3, I11, P2, P8]
---

# Q041 · 公开真题：PPO 与 GRPO 最大区别是什么？

> **题型**：公开真题 · **难度**：L2 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q040](../04-dpo-family/q040-simpo-average-logprob.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q042 →](../05-grpo/q042-group-relative-baseline.md)
...
GRPO 用同一 prompt 下多条采样的 group-relative reward 构造 baseline/advantage，从而不训练 critic。

### PDF 原始深入理解

这减少了 value model 的显存和训练复杂度，但需要每个 prompt 采 G 条 response，实际是“用 sampling compute 换 critic”。GRPO 最早在 DeepSeekMath 中提出。[P2]

### PDF 原始常见失分点

只说“GRPO 没有 value model”，不讲代价。
...
1. 先给核心判断：GRPO 用同一 prompt 下多条采样的 group-relative reward 构造 baseline/advantage，从而不训练 critic。
2. 再解释机制：这减少了 value model 的显存和训练复杂度，但需要每个 prompt 采 G 条 response，实际是“用 sampling compute 换 critic”。GRPO 最早在 DeepSeekMath 中提出。[P2]
3. 最后补一个边界条件或失败模式：GRPO 去 critic，但没有消灭 variance 与成本：它用同 prompt 的 group samples 构造 baseline，并把成本转移到 rollout。
...
GRPO 去 critic，但没有消灭 variance 与成本：它用同 prompt 的 group samples 构造 baseline，并把成本转移到 rollout。
...
```text
for prompt in batch:
    responses = rollout(policy_old, prompt, G)
    rewards   = verifier(responses)
    advantage = normalize_within_group(rewards)
    update_policy(responses, advantage, old_logprobs)
```
...
- 只说“GRPO 没有 value model”，不讲代价。
- 边界条件：GRPO 去 critic，但没有消灭 variance 与成本：它用同 prompt 的 group samples 构造 baseline，并把成本转移到 rollout。
...
1. group size G 对方差和吞吐有什么影响？ [I3][I11]
2. 没有 critic 后 variance 会发生什么？
3. GRPO 与 RLOO/ReMax 的 baseline 思路有何关系？
...
> **Q041：GRPO 用同一 prompt 下多条采样的 group-relative reward 构造 baseline/advantage，从而不训练 critic。**
...
本题不能停在“GRPO 用同一 prompt 下多条采样的 group-relative reward 构造 baseline/advantage，从而不训练 critic”。更专业的表述是先确定五个对象：
...
对 **Q041**，最关键的机制判断是：**GRPO 去 critic，但没有消灭 variance 与成本：它用同 prompt 的 group samples 构造 baseline，并把成本转移到 rollout。**

再进一步，工程上真正需要落地的是：GRPO 不是“PPO 去 critic”这么简单：baseline 的统计来源从 learned value 变成同 prompt group；variance、采样成本和可用 reward 结构都随之改变。
...
本题 PDF 的深入结论是：这减少了 value model 的显存和训练复杂度，但需要每个 prompt 采 G 条 response，实际是“用 sampling compute 换 critic”。GRPO 最早在 DeepSeekMath 中提出。[P2]。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：GRPO 用同一 prompt 下多条采样的 group-relative reward 构造 baseline/advantage，从而不训练 critic |
