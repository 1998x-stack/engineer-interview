---
id: Q042
title: "为什么 group-relative baseline 能替代 Critic？"
chapter: 5
chapter_title: "GRPO"
type: "原理推导"
level: L3
...
# Q042 · 为什么 group-relative baseline 能替代 Critic？

> **题型**：原理推导 · **难度**：L3 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q041](../05-grpo/q041-ppo-vs-grpo.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q043 →](../05-grpo/q043-grpo-cost-transfer.md)
...
同一 prompt 的 G 个 Monte-Carlo samples 提供局部相对基线：A_i=(r_i-mean(r))/std(r)。

### PDF 原始深入理解

它不学习跨状态的 value function，而是用“同题竞争”消除 prompt 难度的部分尺度差异。优势是简单；代价 是对 group 质量和 reward 方差敏感。[P8]

### PDF 原始常见失分点

说它等价于精确 advantage；其实只是 estimator。
...
1. 先给核心判断：同一 prompt 的 G 个 Monte-Carlo samples 提供局部相对基线：A_i=(r_i-mean(r))/std(r)。
2. 再解释机制：它不学习跨状态的 value function，而是用“同题竞争”消除 prompt 难度的部分尺度差异。优势是简单；代价 是对 group 质量和 reward 方差敏感。[P8]
3. 最后补一个边界条件或失败模式：group mean/std 是 context-conditioned Monte Carlo baseline。它避免训练 value model，但依赖组内 reward 有足够方差。
...
group mean/std 是 context-conditioned Monte Carlo baseline。它避免训练 value model，但依赖组内 reward 有足够方差。
...
\[\hat A_i=\frac{r_i-\mu_G}{\sigma_G+\varepsilon},\qquad \mu_G=\frac1G\sum_i r_i\]
...
```text
Problem: 为什么 group-relative baseline 能替代 Critic？
...
- 说它等价于精确 advantage；其实只是 estimator。
- 边界条件：group mean/std 是 context-conditioned Monte Carlo baseline。它避免训练 value model，但依赖组内 reward 有足够方差。
...
> **Q042：同一 prompt 的 G 个 Monte-Carlo samples 提供局部相对基线：A_i=(r_i-mean(r))/std(r)。**
...
本题不能停在“同一 prompt 的 G 个 Monte-Carlo samples 提供局部相对基线：A_i=(r_i-mean(r))/std(r)”。更专业的表述是先确定五个对象：
...
对 **Q042**，最关键的机制判断是：**group mean/std 是 context-conditioned Monte Carlo baseline。它避免训练 value model，但依赖组内 reward 有足够方差。**

再进一步，工程上真正需要落地的是：group normalization 要处理 std≈0、reward ties、不同 task reward scale；建议记录 group std 分布与有效 group 比率，而不是只看 global reward。
...
本题 PDF 的深入结论是：它不学习跨状态的 value function，而是用“同题竞争”消除 prompt 难度的部分尺度差异。优势是简单；代价 是对 group 质量和 reward 方差敏感。[P8]。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q042
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：同一 prompt 的 G 个 Monte-Carlo samples 提供局部相对基线：A_i=(r_i-mean(r))/std(r) |
