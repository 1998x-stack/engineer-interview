---
id: Q043
title: "GRPO 去掉 Critic 后，真正的成本转移到了哪里？"
chapter: 5
chapter_title: "GRPO"
type: "系统设计"
level: L3
...
# Q043 · GRPO 去掉 Critic 后，真正的成本转移到了哪里？

> **题型**：系统设计 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q042](../05-grpo/q042-group-relative-baseline.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q044 →](../05-grpo/q044-sequence-reward-token-credit.md)
...
转移到 rollout：每个 prompt 要生成多条长 response；reasoning 场景的 autoregressive decoding 极其昂 贵。

### PDF 原始深入理解

因此系统优化重点会转向 inference throughput、continuous batching、KV cache、长尾管理、policy versioning 和训练/推理解耦。
...
1. 先给核心判断：转移到 rollout：每个 prompt 要生成多条长 response；reasoning 场景的 autoregressive decoding 极其昂 贵。
2. 再解释机制：因此系统优化重点会转向 inference throughput、continuous batching、KV cache、长尾管理、policy versioning 和训练/推理解耦。
3. 最后补一个边界条件或失败模式：“省 critic”不等于“省算力”。如果每个 prompt 要采 G 条长 CoT，decode token 往往成为总成本第一项。
...
“省 critic”不等于“省算力”。如果每个 prompt 要采 G 条长 CoT，decode token 往往成为总成本第一项。
...
```text
Problem: GRPO 去掉 Critic 后，真正的成本转移到了哪里？
...
- 认为“少一个模型 = 总成本一定更低”。
- 边界条件：“省 critic”不等于“省算力”。如果每个 prompt 要采 G 条长 CoT，decode token 往往成为总成本第一项。
...
> **Q043：转移到 rollout：每个 prompt 要生成多条长 response；reasoning 场景的 autoregressive decoding 极其昂 贵。**
...
本题不能停在“转移到 rollout：每个 prompt 要生成多条长 response；reasoning 场景的 autoregressive decoding 极其昂 贵”。更专业的表述是先确定五个对象：
...
对 **Q043**，最关键的机制判断是：**“省 critic”不等于“省算力”。如果每个 prompt 要采 G 条长 CoT，decode token 往往成为总成本第一项。**

再进一步，工程上真正需要落地的是：算清楚成本转移：critic forward/backward 省掉多少，G 倍 rollout 增加多少；在长 CoT 场景，decode token 往往比省下的 critic 计算更贵。
...
本题 PDF 的深入结论是：因此系统优化重点会转向 inference throughput、continuous batching、KV cache、长尾管理、policy versioning 和训练/推理解耦。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q043
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：转移到 rollout：每个 prompt 要生成多条长 response；reasoning 场景的 autoregressive decoding 极其昂 贵 |
