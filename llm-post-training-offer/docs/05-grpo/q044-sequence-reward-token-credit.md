---
id: Q044
title: "公开真题：Sequence-level reward 如何传到 token？credit assignment 有什么问题？"
chapter: 5
chapter_title: "GRPO"
type: "公开真题"
level: L3
priority: high
tags: [reward, cot]
source_refs: [I1, I8, P2, P8]
---

# Q044 · 公开真题：Sequence-level reward 如何传到 token？credit assignment 有什么问题？

> **题型**：公开真题 · **难度**：L3 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q043](../05-grpo/q043-grpo-cost-transfer.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q045 →](../05-grpo/q045-grpo-all-correct-all-wrong.md)
...
最简单是把同一 sequence advantage broadcast 给所有 response token，再对每个 token 的 logprob 做 policy gradient。

### PDF 原始深入理解

这隐含“整条轨迹 token 共享相同归因”，对长 CoT/Agent 非常粗。错误步骤和关键步骤被同样奖励，推动了 PRM、token weighting 与更细 credit 的研究。
...
1. 先给核心判断：最简单是把同一 sequence advantage broadcast 给所有 response token，再对每个 token 的 logprob 做 policy gradient。
2. 再解释机制：这隐含“整条轨迹 token 共享相同归因”，对长 CoT/Agent 非常粗。错误步骤和关键步骤被同样奖励，推动了 PRM、token weighting 与更细 credit 的研究。
3. 最后补一个边界条件或失败模式：把 sequence advantage 广播到所有 token 是最简单 credit assignment，但会把无关/错误中间 token 与真正关键 token 一起强化或抑制。
...
把 sequence advantage 广播到所有 token 是最简单 credit assignment，但会把无关/错误中间 token 与真正关键 token 一起强化或抑制。
...
\[\nabla_\theta J\approx \sum_{t=1}^{T}\hat A_{seq}\nabla_\theta\log\pi_\theta(y_t\mid x,y_{<t})\]
...
```text
Problem: 公开真题：Sequence-level reward 如何传到 token？credit assignment 有什么问题？
...
- 把 broadcast 说成精确归因。
- 边界条件：把 sequence advantage 广播到所有 token 是最简单 credit assignment，但会把无关/错误中间 token 与真正关键 token 一起强化或抑制。
...
1. 如果答案正确但前半段有大量无效推理，梯度应如何处理？ [I1][I8]
2. 相同 sequence reward 广播给所有 token 会强化哪些错误 token？
3. 能否用 process reward 或 token-level verifier 改善？
4. group std 为 0 时怎样处理？
5. G 翻倍后应该观察哪些质量与吞吐指标？
6. policy lag 允许多大，如何测？
...
> **Q044：最简单是把同一 sequence advantage broadcast 给所有 response token，再对每个 token 的 logprob 做 policy gradient。**
...
- [Q046 · Group size G 越大越好吗？](q046-grpo-group-size.md)
...
本题不能停在“最简单是把同一 sequence advantage broadcast 给所有 response token，再对每个 token 的 logprob 做 policy gradient”。更专业的表述是先确定五个对象：
...
对 **Q044**，最关键的机制判断是：**把 sequence advantage 广播到所有 token 是最简单 credit assignment，但会把无关/错误中间 token 与真正关键 token 一起强化或抑制。**

再进一步，工程上真正需要落地的是：sequence reward 广播到每个 token 会把“结果好”视为整条轨迹都好；若过程含冗余/错误修正，token credit 会非常粗。至少比较 token/sequence aggregation 与长度控制结果。
...
本题 PDF 的深入结论是：这隐含“整条轨迹 token 共享相同归因”，对长 CoT/Agent 非常粗。错误步骤和关键步骤被同样奖励，推动了 PRM、token weighting 与更细 credit 的研究。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q044
...
| 3 | `group reward mean/std` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `effective-group ratio` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `pass@k` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `ratio quantiles` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `entropy` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `policy-version lag` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：最简单是把同一 sequence advantage broadcast 给所有 response token，再对每个 token 的 logprob 做 policy gradient |
