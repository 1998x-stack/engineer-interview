---
id: Q045
title: "为什么 group 内全对或全错时 GRPO 基本没有有效梯度？"
chapter: 5
chapter_title: "GRPO"
type: "原理推导"
level: L3
priority: normal
tags: [grpo, dapo]
source_refs: [P9, P2, P8]
---

# Q045 · 为什么 group 内全对或全错时 GRPO 基本没有有效梯度？

> **题型**：原理推导 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q044](../05-grpo/q044-sequence-reward-token-credit.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q046 →](../05-grpo/q046-grpo-group-size.md)
...
若 r_i 全相同，组内标准化后 A_i≈0，无法区分哪条轨迹更优。

### PDF 原始深入理解

这意味着过易/过难 prompt 虽然消耗 rollout，但不提供 relative learning signal；DAPO Dynamic Sampling 就针对这一问题。[P9]

### PDF 原始常见失分点

只认为全错样本无用；全对也可能无 relative signal。
...
1. 先给核心判断：若 r_i 全相同，组内标准化后 A_i≈0，无法区分哪条轨迹更优。
2. 再解释机制：这意味着过易/过难 prompt 虽然消耗 rollout，但不提供 relative learning signal；DAPO Dynamic Sampling 就针对这一问题。[P9]
3. 最后补一个边界条件或失败模式：组内全同 reward 时 normalized advantage 退化为 0；这解释了为什么难度调度和 dynamic sampling 直接影响有效 token 利用率。
...
组内全同 reward 时 normalized advantage 退化为 0；这解释了为什么难度调度和 dynamic sampling 直接影响有效 token 利用率。
...
\[r_1=\cdots=r_G\Rightarrow \sigma_G=0,\quad \hat A_i\approx0\]
...
```text
Problem: 为什么 group 内全对或全错时 GRPO 基本没有有效梯度？
...
- 只认为全错样本无用；全对也可能无 relative signal。
- 边界条件：组内全同 reward 时 normalized advantage 退化为 0；这解释了为什么难度调度和 dynamic sampling 直接影响有效 token 利用率。
...
> **Q045：若 r_i 全相同，组内标准化后 A_i≈0，无法区分哪条轨迹更优。**
...
本题不能停在“若 r_i 全相同，组内标准化后 A_i≈0，无法区分哪条轨迹更优”。更专业的表述是先确定五个对象：
...
对 **Q045**，最关键的机制判断是：**组内全同 reward 时 normalized advantage 退化为 0；这解释了为什么难度调度和 dynamic sampling 直接影响有效 token 利用率。**

再进一步，工程上真正需要落地的是：all-correct/all-wrong group 应单独统计；若比例高，说明题太易/太难或 G/temperature 不合适。Dynamic Sampling 本质是提高有效梯度样本占比。
...
本题 PDF 的深入结论是：这意味着过易/过难 prompt 虽然消耗 rollout，但不提供 relative learning signal；DAPO Dynamic Sampling 就针对这一问题。[P9]。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q045
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：若 r_i 全相同，组内标准化后 A_i≈0，无法区分哪条轨迹更优 |
