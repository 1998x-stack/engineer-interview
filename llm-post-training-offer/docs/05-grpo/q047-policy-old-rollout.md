---
id: Q047
title: "公开真题：πθ、πold、πrollout 分别是什么？为什么工程中可能不相等？"
chapter: 5
chapter_title: "GRPO"
type: "公开真题"
level: L3
priority: normal
tags: [grpo]
source_refs: [I6, P2, P8]
---

# Q047 · 公开真题：πθ、πold、πrollout 分别是什么？为什么工程中可能不相等？

> **题型**：公开真题 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q046](../05-grpo/q046-grpo-group-size.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q048 →](../05-grpo/q048-grpo-large-batch-off-policy.md)
...
πrollout 是实际生成样本的版本，πold 是优化 ratio 的行为策略快照，πθ 是当前 learner。同步系统可近似 πrollout=πold；异步或大 batch 时会产生版本滞后。

### PDF 原始深入理解

一旦不相等，importance ratio、clip 与 on-policy 假设都需要重新审视。

### PDF 原始常见失分点

把 reference policy 也混入这三个概念。
...
1. 先给核心判断：πrollout 是实际生成样本的版本，πold 是优化 ratio 的行为策略快照，πθ 是当前 learner。同步系统可近似 πrollout=πold；异步或大 batch 时会产生版本滞后。
2. 再解释机制：一旦不相等，importance ratio、clip 与 on-policy 假设都需要重新审视。
3. 最后补一个边界条件或失败模式：rollout policy 负责生成数据，old policy 定义 importance denominator，current policy 被更新；异步系统里三者可能是不同版本。
...
rollout policy 负责生成数据，old policy 定义 importance denominator，current policy 被更新；异步系统里三者可能是不同版本。
...
\[y\sim\pi_{rollout},\qquad \rho=\frac{\pi_\theta(y)}{\pi_{old}(y)},\qquad \pi_{rollout}\not\equiv\pi_{old}\not\equiv\pi_\theta\ \text{(async)}\]
...
```text
Problem: 公开真题：πθ、πold、πrollout 分别是什么？为什么工程中可能不相等？
...
- 把 reference policy 也混入这三个概念。
- 边界条件：rollout policy 负责生成数据，old policy 定义 importance denominator，current policy 被更新；异步系统里三者可能是不同版本。
...
1. 如何记录 policy_version 并丢弃过旧 rollout？ [I6]
...
> **Q047：πrollout 是实际生成样本的版本，πold 是优化 ratio 的行为策略快照，πθ 是当前 learner。同步系统可近似 πrollout=πold；异步或大 batch 时会产生版本滞后。**
...
本题不能停在“πrollout 是实际生成样本的版本，πold 是优化 ratio 的行为策略快照，πθ 是当前 learner。同步系统可近似 πrollout=πold；异步或大 batch 时会产生版本滞后”。更专业的表述是先确定五个对象：
...
对 **Q047**，最关键的机制判断是：**rollout policy 负责生成数据，old policy 定义 importance denominator，current policy 被更新；异步系统里三者可能是不同版本。**

再进一步，工程上真正需要落地的是：在异步系统中给每条 trajectory 写入 rollout version、old-logprob source 与 learner version；没有版本追踪，任何 off-policy 诊断都只能猜。
...
本题 PDF 的深入结论是：一旦不相等，importance ratio、clip 与 on-policy 假设都需要重新审视。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q047
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：πrollout 是实际生成样本的版本，πold 是优化 ratio 的行为策略快照，πθ 是当前 learner。同步系统可近似 πrollout=πold；异步或大 batch 时会产生版本滞后 |
