---
id: Q048
title: "公开真题：batch 很大时为什么 GRPO 会越来越 off-policy？"
chapter: 5
chapter_title: "GRPO"
type: "公开真题"
level: L4
...
# Q048 · 公开真题：batch 很大时为什么 GRPO 会越来越 off-policy？

> **题型**：公开真题 · **难度**：L4 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q047](../05-grpo/q047-policy-old-rollout.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q049 →](../05-grpo/q049-policy-lag-sync-async.md)
...
rollout 生成、reward、排队、batch assemble 需要时间，learner 同期更新多个 step；数据到达 learner 时已 来自更旧 policy。

### PDF 原始深入理解

缓解方式：缩短 staleness、减少 inner epochs、versioned queue、importance correction、同步 barrier、 限制 queue depth、丢弃 stale batch。
...
1. 先给核心判断：rollout 生成、reward、排队、batch assemble 需要时间，learner 同期更新多个 step；数据到达 learner 时已 来自更旧 policy。
2. 再解释机制：缓解方式：缩短 staleness、减少 inner epochs、versioned queue、importance correction、同步 barrier、 限制 queue depth、丢弃 stale batch。
3. 最后补一个边界条件或失败模式：batch 越大、queue 越长、update 越多，sample 从生成到消费的延迟越大，policy lag 导致 ratio 重尾和 off-policy 偏差。
...
batch 越大、queue 越长、update 越多，sample 从生成到消费的延迟越大，policy lag 导致 ratio 重尾和 off-policy 偏差。
...
```text
Problem: 公开真题：batch 很大时为什么 GRPO 会越来越 off-policy？
...
- 只说“batch 大方差小”。
- 边界条件：batch 越大、queue 越长、update 越多，sample 从生成到消费的延迟越大，policy lag 导致 ratio 重尾和 off-policy 偏差。
...
1. 吞吐与 freshness 如何做量化 trade-off？ [I6]
...
> **Q048：rollout 生成、reward、排队、batch assemble 需要时间，learner 同期更新多个 step；数据到达 learner 时已 来自更旧 policy。**
...
本题不能停在“rollout 生成、reward、排队、batch assemble 需要时间，learner 同期更新多个 step；数据到达 learner 时已 来自更旧 policy”。更专业的表述是先确定五个对象：
...
对 **Q048**，最关键的机制判断是：**batch 越大、queue 越长、update 越多，sample 从生成到消费的延迟越大，policy lag 导致 ratio 重尾和 off-policy 偏差。**

再进一步，工程上真正需要落地的是：large batch 的问题不是 batch size 本身，而是排队和更新周期延长导致 data age 增长；监控 staleness histogram 比只记录 batch size 更有解释力。
...
本题 PDF 的深入结论是：缓解方式：缩短 staleness、减少 inner epochs、versioned queue、importance correction、同步 barrier、 限制 queue depth、丢弃 stale batch。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q048
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：rollout 生成、reward、排队、batch assemble 需要时间，learner 同期更新多个 step；数据到达 learner 时已 来自更旧 policy |
