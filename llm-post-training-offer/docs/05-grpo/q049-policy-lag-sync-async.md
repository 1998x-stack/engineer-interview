---
id: Q049
title: "如何缓解 policy lag？同步 RL 与异步 RL 各自适合什么？"
chapter: 5
chapter_title: "GRPO"
type: "系统设计"
level: L4
...
# Q049 · 如何缓解 policy lag？同步 RL 与异步 RL 各自适合什么？

> **题型**：系统设计 · **难度**：L4 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q048](../05-grpo/q048-grpo-large-batch-off-policy.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q050 →](../05-grpo/q050-grpo-verifiable-tasks.md)
...
同步：fresh、目标清晰，但 straggler 严重；异步：吞吐高，但 staleness 与 off-policy bias 增加。

### PDF 原始深入理解

工程上常做有界异步：固定最大版本差、队列深度、权重同步频率与 stale sample 策略。
...
1. 先给核心判断：同步：fresh、目标清晰，但 straggler 严重；异步：吞吐高，但 staleness 与 off-policy bias 增加。
2. 再解释机制：工程上常做有界异步：固定最大版本差、队列深度、权重同步频率与 stale sample 策略。
3. 最后补一个边界条件或失败模式：同步系统追求 policy freshness，异步系统追求吞吐；工程上通常通过 version id、staleness bound、queue backpressure 和 discard policy 折中。
...
同步系统追求 policy freshness，异步系统追求吞吐；工程上通常通过 version id、staleness bound、queue backpressure 和 discard policy 折中。
...
```text
Problem: 如何缓解 policy lag？同步 RL 与异步 RL 各自适合什么？
...
- 认为“异步一定更快更好”。
- 边界条件：同步系统追求 policy freshness，异步系统追求吞吐；工程上通常通过 version id、staleness bound、queue backpressure 和 discard policy 折中。
...
1. 如果 rollout 是训练总成本 80%，你会优先优化哪一侧？
...
> **Q049：同步：fresh、目标清晰，但 straggler 严重；异步：吞吐高，但 staleness 与 off-policy bias 增加。**
...
本题不能停在“同步：fresh、目标清晰，但 straggler 严重；异步：吞吐高，但 staleness 与 off-policy bias 增加”。更专业的表述是先确定五个对象：
...
对 **Q049**，最关键的机制判断是：**同步系统追求 policy freshness，异步系统追求吞吐；工程上通常通过 version id、staleness bound、queue backpressure 和 discard policy 折中。**

再进一步，工程上真正需要落地的是：同步策略是 throughput—freshness trade-off。可以用最大 version lag、队列 TTL、stale sample drop 和 importance correction 构成分层防线。
...
本题 PDF 的深入结论是：工程上常做有界异步：固定最大版本差、队列深度、权重同步频率与 stale sample 策略。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q049
...
| # | 指标 | 如何解释 |
|---:|---|---|
| 1 | `group reward mean/std` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 2 | `effective-group ratio` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 3 | `pass@k` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `ratio quantiles` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `entropy` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `policy-version lag` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `useful rollout tokens/s` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：同步：fresh、目标清晰，但 straggler 严重；异步：吞吐高，但 staleness 与 off-policy bias 增加 |
