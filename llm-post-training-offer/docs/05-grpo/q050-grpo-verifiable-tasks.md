---
id: Q050
title: "为什么 GRPO 特别适合数学/代码等 verifiable tasks？"
chapter: 5
chapter_title: "GRPO"
type: "原理推导"
level: L2
priority: normal
tags: [grpo, reward, verifier, rlvr]
source_refs: [P2, P8]
---

# Q050 · 为什么 GRPO 特别适合数学/代码等 verifiable tasks？

> **题型**：原理推导 · **难度**：L2 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q049](../05-grpo/q049-policy-lag-sync-async.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q051 →](../06-dapo-gspo/q051-dapo-vs-grpo.md)
...
这些任务可以获得稳定的 correctness verifier，避免昂贵且可被 exploit 的主观 RM；group samples 也能提供 明确的相对成功信号。

### PDF 原始深入理解

真正关键组合是 GRPO + RLVR：算法负责稳定更新，verifier 决定 reward 质量与可扩展性。[P2][P8]

### PDF 原始常见失分点

把 GRPO 当成通用聊天对齐的默认算法。
...
1. 先给核心判断：这些任务可以获得稳定的 correctness verifier，避免昂贵且可被 exploit 的主观 RM；group samples 也能提供 明确的相对成功信号。
2. 再解释机制：真正关键组合是 GRPO + RLVR：算法负责稳定更新，verifier 决定 reward 质量与可扩展性。[P2][P8]
3. 最后补一个边界条件或失败模式：数学/code 的核心优势不是“模型更会推理”，而是 reward 能可靠自动化，允许低噪声、大规模、持续在线探索。
...
数学/code 的核心优势不是“模型更会推理”，而是 reward 能可靠自动化，允许低噪声、大规模、持续在线探索。
...
```text
Problem: 为什么 GRPO 特别适合数学/代码等 verifiable tasks？
...
- 把 GRPO 当成通用聊天对齐的默认算法。
- 边界条件：数学/code 的核心优势不是“模型更会推理”，而是 reward 能可靠自动化，允许低噪声、大规模、持续在线探索。
...
1. 开放问答如何创造“半可验证”reward？ 举一反三 / 章节检查 尝试不看答案，用“任务 → 数据 → reward → exploration → credit → compute → failure → eval”重新讲本章。若能把 3 个以上问题串成一条因果链，而不是逐题背诵，本章才算掌 握。 第六章 DAPO / GSPO：2026 面试高频前沿 本章目标 完成面试题 51-60。要求不是记忆名词，而是把每个方法与其输入、目标函数、failure mode 和工程代价连接起来。
...
> **Q050：这些任务可以获得稳定的 correctness verifier，避免昂贵且可被 exploit 的主观 RM；group samples 也能提供 明确的相对成功信号。**
...
本题不能停在“这些任务可以获得稳定的 correctness verifier，避免昂贵且可被 exploit 的主观 RM；group samples 也能提供 明确的相对成功信号”。更专业的表述是先确定五个对象：
...
对 **Q050**，最关键的机制判断是：**数学/code 的核心优势不是“模型更会推理”，而是 reward 能可靠自动化，允许低噪声、大规模、持续在线探索。**

再进一步，工程上真正需要落地的是：数学/代码适合 GRPO/RLVR的关键是 verifier 高 precision 且成本低；若 verifier 弱，group relative 只会高效放大错误信号。
...
本题 PDF 的深入结论是：真正关键组合是 GRPO + RLVR：算法负责稳定更新，verifier 决定 reward 质量与可扩展性。[P2][P8]。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q050
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：这些任务可以获得稳定的 correctness verifier，避免昂贵且可被 exploit 的主观 RM；group samples 也能提供 明确的相对成功信号 |
