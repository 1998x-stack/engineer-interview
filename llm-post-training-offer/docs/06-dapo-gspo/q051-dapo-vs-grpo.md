---
id: Q051
title: "公开真题：DAPO 相比 GRPO 做了哪些核心改进？"
chapter: 6
chapter_title: "DAPO / GSPO"
type: "公开真题"
level: L3
priority: high
tags: [grpo, dapo, reward, cot]
source_refs: [I3, I7, P9, P10]
---

# Q051 · 公开真题：DAPO 相比 GRPO 做了哪些核心改进？

> **题型**：公开真题 · **难度**：L3 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q050](../05-grpo/q050-grpo-verifiable-tasks.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q052 →](../06-dapo-gspo/q052-dapo-clip-higher.md)
...
四项必须脱口而出：Clip-Higher、Dynamic Sampling、Token-level Policy Gradient Loss、Overlong Reward Shaping。[P9]

### PDF 原始深入理解

这四项分别对应 entropy collapse、无效组采样、长 CoT 长度 weighting、截断边界 reward noise。高分答案 要把 trick 与 failure mode 一一配对。
...
1. 先给核心判断：四项必须脱口而出：Clip-Higher、Dynamic Sampling、Token-level Policy Gradient Loss、Overlong Reward Shaping。[P9]
2. 再解释机制：这四项分别对应 entropy collapse、无效组采样、长 CoT 长度 weighting、截断边界 reward noise。高分答案 要把 trick 与 failure mode 一一配对。
3. 最后补一个边界条件或失败模式：DAPO 是对 GRPO 大规模 long-CoT 训练 failure mode 的 recipe 化修复：Clip-Higher、Dynamic Sampling、Token-level PG、Overlong Reward Shaping。
...
DAPO 是对 GRPO 大规模 long-CoT 训练 failure mode 的 recipe 化修复：Clip-Higher、Dynamic Sampling、Token-level PG、Overlong Reward Shaping。
...
```text
GRPO baseline
  + decoupled / higher positive clip
  + dynamic sampling of informative prompt groups
  + token-level loss aggregation
  + smooth overlong reward shaping
```
...
- 只背四个名词。
- 边界条件：DAPO 是对 GRPO 大规模 long-CoT 训练 failure mode 的 recipe 化修复：Clip-Higher、Dynamic Sampling、Token-level PG、Overlong Reward Shaping。
...
1. 每项如果去掉，训练曲线最可能出现什么症状？ [I3][I7]
2. 四个 DAPO trick 哪个先做 ablation？为什么？
3. Dynamic Sampling 会不会引入新的 task-distribution bias？
...
> **Q051：四项必须脱口而出：Clip-Higher、Dynamic Sampling、Token-level Policy Gradient Loss、Overlong Reward Shaping。[P9]**
...
本题不能停在“四项必须脱口而出：Clip-Higher、Dynamic Sampling、Token-level Policy Gradient Loss、Overlong Reward Shaping。[P9]”。更专业的表述是先确定五个对象：
...
对 **Q051**，最关键的机制判断是：**DAPO 是对 GRPO 大规模 long-CoT 训练 failure mode 的 recipe 化修复：Clip-Higher、Dynamic Sampling、Token-level PG、Overlong Reward Shaping。**

再进一步，工程上真正需要落地的是：回答 DAPO 时必须把每个 trick 对应到具体 failure：Clip-Higher→探索，Dynamic Sampling→无效 group，Token-level loss→长度权重，Overlong shaping→边界噪声。
...
本题 PDF 的深入结论是：这四项分别对应 entropy collapse、无效组采样、长 CoT 长度 weighting、截断边界 reward noise。高分答案 要把 trick 与 failure mode 一一配对。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
| 4 | `entropy` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `positive/negative clip fraction` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `effective sample rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `response length p50/p95` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `sequence ratio` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：四项必须脱口而出：Clip-Higher、Dynamic Sampling、Token-level Policy Gradient Loss、Overlong Reward Shaping。[P9] |
