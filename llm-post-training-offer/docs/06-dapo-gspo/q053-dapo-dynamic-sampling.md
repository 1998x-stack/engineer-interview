---
id: Q053
title: "公开真题：Dynamic Sampling 为什么有效？"
chapter: 6
...
priority: normal
tags: [dapo-gspo]
source_refs: [I7, P9, P10]
---

# Q053 · 公开真题：Dynamic Sampling 为什么有效？

> **题型**：公开真题 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q052](../06-dapo-gspo/q052-dapo-clip-higher.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q054 →](../06-dapo-gspo/q054-long-cot-length-normalization.md)
...
过滤组内 accuracy=0 或 1 的 prompt，使每个 batch 保持较稳定数量的有效梯度样本。[P9]

### PDF 原始深入理解

它本质上是按“当前策略可学习性”重采样：太易/太难都降低权重，从而把 rollout budget 投到 frontier。
...
1. 先给核心判断：过滤组内 accuracy=0 或 1 的 prompt，使每个 batch 保持较稳定数量的有效梯度样本。[P9]
2. 再解释机制：它本质上是按“当前策略可学习性”重采样：太易/太难都降低权重，从而把 rollout budget 投到 frontier。
3. 最后补一个边界条件或失败模式：Dynamic Sampling 把 rollout 预算从“无相对梯度”的全对/全错 prompt 转向有判别信号的 prompt，本质上提升 effective batch efficiency。
...
Dynamic Sampling 把 rollout 预算从“无相对梯度”的全对/全错 prompt 转向有判别信号的 prompt，本质上提升 effective batch efficiency。
...
```text
Problem: 公开真题：Dynamic Sampling 为什么有效？
...
- 只说“过滤简单题”。全错也会被过滤。
- 边界条件：Dynamic Sampling 把 rollout 预算从“无相对梯度”的全对/全错 prompt 转向有判别信号的 prompt，本质上提升 effective batch efficiency。
...
> **Q053：过滤组内 accuracy=0 或 1 的 prompt，使每个 batch 保持较稳定数量的有效梯度样本。[P9]**
...
本题不能停在“过滤组内 accuracy=0 或 1 的 prompt，使每个 batch 保持较稳定数量的有效梯度样本。[P9]”。更专业的表述是先确定五个对象：
...
对 **Q053**，最关键的机制判断是：**Dynamic Sampling 把 rollout 预算从“无相对梯度”的全对/全错 prompt 转向有判别信号的 prompt，本质上提升 effective batch efficiency。**

再进一步，工程上真正需要落地的是：Dynamic Sampling 要考虑数据分布被重新加权：过滤易/极难题提高梯度效率，但也可能改变 curriculum；必须保留原分布 eval，防止只优化中等难度。
...
本题 PDF 的深入结论是：它本质上是按“当前策略可学习性”重采样：太易/太难都降低权重，从而把 rollout budget 投到 frontier。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q053
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：过滤组内 accuracy=0 或 1 的 prompt，使每个 batch 保持较稳定数量的有效梯度样本。[P9] |
