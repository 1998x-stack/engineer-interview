---
id: Q054
title: "长 CoT 下 sequence-level normalization 为什么可能产生长度偏差？"
chapter: 6
...
# Q054 · 长 CoT 下 sequence-level normalization 为什么可能产生长度偏差？

> **题型**：原理推导 · **难度**：L4 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q053](../06-dapo-gspo/q053-dapo-dynamic-sampling.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q055 →](../06-dapo-gspo/q055-token-level-policy-gradient.md)
...
若先对每条 response 做均值再让每条 sequence 权重相等，500 token 与 10k token 的总梯度预算相近，长轨 迹单 token 权重被显著稀释。

### PDF 原始深入理解

应明确“我们希望等权 sequence，还是等权 token？”这是目标定义问题，不只是实现细节。

### PDF 原始常见失分点

认为 length normalization 总是公平。
...
1. 先给核心判断：若先对每条 response 做均值再让每条 sequence 权重相等，500 token 与 10k token 的总梯度预算相近，长轨 迹单 token 权重被显著稀释。
2. 再解释机制：应明确“我们希望等权 sequence，还是等权 token？”这是目标定义问题，不只是实现细节。
3. 最后补一个边界条件或失败模式：当长度差异巨大时，先按 sequence 平均会让每条 response 权重相近，但长序列单 token 梯度被稀释；这会形成隐含长度 weighting。
...
当长度差异巨大时，先按 sequence 平均会让每条 response 权重相近，但长序列单 token 梯度被稀释；这会形成隐含长度 weighting。
...
```text
Problem: 长 CoT 下 sequence-level normalization 为什么可能产生长度偏差？
...
- 认为 length normalization 总是公平。
- 边界条件：当长度差异巨大时，先按 sequence 平均会让每条 response 权重相近，但长序列单 token 梯度被稀释；这会形成隐含长度 weighting。
...
1. 如何设计一个不偏短也不偏长的 loss aggregation？
...
> **Q054：若先对每条 response 做均值再让每条 sequence 权重相等，500 token 与 10k token 的总梯度预算相近，长轨 迹单 token 权重被显著稀释。**
...
本题不能停在“若先对每条 response 做均值再让每条 sequence 权重相等，500 token 与 10k token 的总梯度预算相近，长轨 迹单 token 权重被显著稀释”。更专业的表述是先确定五个对象：
...
对 **Q054**，最关键的机制判断是：**当长度差异巨大时，先按 sequence 平均会让每条 response 权重相近，但长序列单 token 梯度被稀释；这会形成隐含长度 weighting。**

再进一步，工程上真正需要落地的是：长 CoT 下要明确 denominator 是 sequence、token 还是有效 token；不同 normalization 会隐式改变“长回答每 token 的梯度预算”。
...
本题 PDF 的深入结论是：应明确“我们希望等权 sequence，还是等权 token？”这是目标定义问题，不只是实现细节。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q054
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：若先对每条 response 做均值再让每条 sequence 权重相等，500 token 与 10k token 的总梯度预算相近，长轨 迹单 token 权重被显著稀释 |
