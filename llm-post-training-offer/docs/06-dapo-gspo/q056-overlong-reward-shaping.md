---
id: Q056
title: "Overlong Reward Shaping 为什么比硬截断惩罚更稳？"
chapter: 6
...
# Q056 · Overlong Reward Shaping 为什么比硬截断惩罚更稳？

> **题型**：原理推导 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q055](../06-dapo-gspo/q055-token-level-policy-gradient.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q057 →](../06-dapo-gspo/q057-gspo-vs-grpo.md)
...
硬阈值会制造 reward cliff：刚超 1 个 token 的正确解可能突然从高 reward 变低 reward，引入与质量无关的高 噪声梯度。

### PDF 原始深入理解

平滑 shaping 在接近上限时逐渐惩罚，降低边界不连续。[P9]
...
1. 先给核心判断：硬阈值会制造 reward cliff：刚超 1 个 token 的正确解可能突然从高 reward 变低 reward，引入与质量无关的高 噪声梯度。
2. 再解释机制：平滑 shaping 在接近上限时逐渐惩罚，降低边界不连续。[P9]
3. 最后补一个边界条件或失败模式：硬截断产生 reward cliff，使边界附近几乎相同的轨迹得到巨大 reward 差；平滑 shaping 把长度约束变成连续、更低噪的学习信号。
...
硬截断产生 reward cliff，使边界附近几乎相同的轨迹得到巨大 reward 差；平滑 shaping 把长度约束变成连续、更低噪的学习信号。
...
```text
Problem: Overlong Reward Shaping 为什么比硬截断惩罚更稳？
...
- 认为它只是让回答更短。
- 边界条件：硬截断产生 reward cliff，使边界附近几乎相同的轨迹得到巨大 reward 差；平滑 shaping 把长度约束变成连续、更低噪的学习信号。
...
1. 如果模型学会故意在阈值前提前给答案，会不会产生新 hacking？
...
> **Q056：硬阈值会制造 reward cliff：刚超 1 个 token 的正确解可能突然从高 reward 变低 reward，引入与质量无关的高 噪声梯度。**
...
本题不能停在“硬阈值会制造 reward cliff：刚超 1 个 token 的正确解可能突然从高 reward 变低 reward，引入与质量无关的高 噪声梯度”。更专业的表述是先确定五个对象：
...
对 **Q056**，最关键的机制判断是：**硬截断产生 reward cliff，使边界附近几乎相同的轨迹得到巨大 reward 差；平滑 shaping 把长度约束变成连续、更低噪的学习信号。**

再进一步，工程上真正需要落地的是：overlong shaping 应画 reward-vs-length 曲线检查是否连续；硬 cliff 会让临界长度附近样本产生高方差甚至错误优化方向。
...
本题 PDF 的深入结论是：平滑 shaping 在接近上限时逐渐惩罚，降低边界不连续。[P9]。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q056
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：硬阈值会制造 reward cliff：刚超 1 个 token 的正确解可能突然从高 reward 变低 reward，引入与质量无关的高 噪声梯度 |
