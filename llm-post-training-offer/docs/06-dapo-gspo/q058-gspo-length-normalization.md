...
# Q058 · GSPO 为什么需要 1/|y| 的长度归一化？

> **题型**：原理推导 · **难度**：L4 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q057](../06-dapo-gspo/q057-gspo-vs-grpo.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q059 →](../06-dapo-gspo/q059-gspo-moe-routing.md)
...
sequence probability 是 token probability 连乘，ratio 的尺度会随长度迅速变化。取 log 后平均、再 exp，相 当于几何平均 token ratio，使不同长度 sequence 更可比。[P10]

### PDF 原始深入理解

这是目标尺度设计，不只是防 underflow。

### PDF 原始常见失分点

把长度归一化只理解为数值稳定技巧，而忽略它对不同长度样本权重的影响。
...
1. 先给核心判断：sequence probability 是 token probability 连乘，ratio 的尺度会随长度迅速变化。取 log 后平均、再 exp，相 当于几何平均 token ratio，使不同长度 sequence 更可比。[P10]
2. 再解释机制：这是目标尺度设计，不只是防 underflow。
3. 最后补一个边界条件或失败模式：sequence 概率是 token 概率乘积，长度越长数值越小、ratio 方差越大；1/|y| 等价于对 log-ratio 取平均，也就是 geometric mean。
...
sequence 概率是 token 概率乘积，长度越长数值越小、ratio 方差越大；1/|y| 等价于对 log-ratio 取平均，也就是 geometric mean。
...
\[\log s_i=\frac1{|y_i|}\sum_t\log\frac{\pi_\theta(y_{i,t}|\cdot)}{\pi_{old}(y_{i,t}|\cdot)}\]
...
- 把长度归一化只理解为数值稳定技巧，而忽略它对不同长度样本权重的影响。
- 边界条件：sequence 概率是 token 概率乘积，长度越长数值越小、ratio 方差越大；1/|y| 等价于对 log-ratio 取平均，也就是 geometric mean。
...
> **Q058：sequence probability 是 token probability 连乘，ratio 的尺度会随长度迅速变化。取 log 后平均、再 exp，相 当于几何平均 token ratio，使不同长度 sequence 更可比。[P10]**
...
本题不能停在“sequence probability 是 token probability 连乘，ratio 的尺度会随长度迅速变化。取 log 后平均、再 exp，相 当于几何平均 token ratio，使不同长度 sequence 更可比。[P10]”。更专业的表述是先确定五个对象：
...
对 **Q058**，最关键的机制判断是：**sequence 概率是 token 概率乘积，长度越长数值越小、ratio 方差越大；1/|y| 等价于对 log-ratio 取平均，也就是 geometric mean。**

再进一步，工程上真正需要落地的是：长度归一化是对 log-ratio 求 token 平均再指数化；数值上必须在 log-space 聚合，避免直接乘 token probability 的下溢。
...
本题 PDF 的深入结论是：这是目标尺度设计，不只是防 underflow。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q058
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：sequence probability 是 token probability 连乘，ratio 的尺度会随长度迅速变化。取 log 后平均、再 exp，相 当于几何平均 token ratio，使不同长度 sequence 更可比。[P10] |
