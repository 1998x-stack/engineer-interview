---
id: Q059
title: "公开真题：为什么 GSPO 对 MoE routing mismatch 更友好？"
chapter: 6
...
priority: normal
tags: [grpo, gspo, moe]
source_refs: [I8, P10, P9]
---

# Q059 · 公开真题：为什么 GSPO 对 MoE routing mismatch 更友好？

> **题型**：公开真题 · **难度**：L5 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q058](../06-dapo-gspo/q058-gspo-length-normalization.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q060 →](../06-dapo-gspo/q060-dapo-vs-gspo.md)
...
MoE 的 token-level likelihood 对 expert routing 的微小差异很敏感；GRPO 可能需要 Routing Replay 保持训 推一致。GSPO 只依赖 sequence-level likelihood，对局部 token mismatch 更容忍，可减少基础设施负担。[P10]

### PDF 原始深入理解

关键不是“MoE 更快”，而是 optimization granularity 对 routing perturbation 的敏感性不同。
...
1. 先给核心判断：MoE 的 token-level likelihood 对 expert routing 的微小差异很敏感；GRPO 可能需要 Routing Replay 保持训 推一致。GSPO 只依赖 sequence-level likelihood，对局部 token mismatch 更容忍，可减少基础设施负担。[P10]
2. 再解释机制：关键不是“MoE 更快”，而是 optimization granularity 对 routing perturbation 的敏感性不同。
3. 最后补一个边界条件或失败模式：MoE 的 routing 变化会放大逐 token logprob mismatch；sequence-level ratio 对局部 token 扰动更鲁棒，因此减少对 routing replay 的依赖。
...
MoE 的 routing 变化会放大逐 token logprob mismatch；sequence-level ratio 对局部 token 扰动更鲁棒，因此减少对 routing replay 的依赖。
...
```text
Problem: 公开真题：为什么 GSPO 对 MoE routing mismatch 更友好？
...
- 忽略推理引擎与训练引擎精度/路由差异。
- 边界条件：MoE 的 routing 变化会放大逐 token logprob mismatch；sequence-level ratio 对局部 token 扰动更鲁棒，因此减少对 routing replay 的依赖。
...
1. Routing Replay 为什么有内存与通信成本？ [I8]
...
> **Q059：MoE 的 token-level likelihood 对 expert routing 的微小差异很敏感；GRPO 可能需要 Routing Replay 保持训 推一致。GSPO 只依赖 sequence-level likelihood，对局部 token mismatch 更容忍，可减少基础设施负担。[P10]**
...
本题不能停在“MoE 的 token-level likelihood 对 expert routing 的微小差异很敏感；GRPO 可能需要 Routing Replay 保持训 推一致。GSPO 只依赖 sequence-level likelihood，对局部 token mismatch 更容忍，可减少基础设施负担。[P10]”。更专业的表述是先确定五个对象：
...
对 **Q059**，最关键的机制判断是：**MoE 的 routing 变化会放大逐 token logprob mismatch；sequence-level ratio 对局部 token 扰动更鲁棒，因此减少对 routing replay 的依赖。**

再进一步，工程上真正需要落地的是：MoE 需额外记录 routing mismatch、expert load 与 train/rollout backend 差异；若 token-level ratio 被 routing perturbation 放大，sequence-level objective 往往更稳。
...
本题 PDF 的深入结论是：关键不是“MoE 更快”，而是 optimization granularity 对 routing perturbation 的敏感性不同。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q059
...
| # | 指标 | 如何解释 |
|---:|---|---|
| 1 | `expert load balance` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 2 | `routing mismatch` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 8 | `training stability` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：MoE 的 token-level likelihood 对 expert routing 的微小差异很敏感；GRPO 可能需要 Routing Replay 保持训 推一致。GSPO 只依赖 sequence-level likelihood，对局部 token mismatch 更容忍，可减少基础设施负担。[P10] |
