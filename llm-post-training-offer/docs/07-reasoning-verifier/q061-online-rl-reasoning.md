---
id: Q061
title: "为什么 Online RL 可能提升 reasoning，而 SFT 不一定？"
chapter: 7
...
# Q061 · 为什么 Online RL 可能提升 reasoning，而 SFT 不一定？

> **题型**：原理推导 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q060](../06-dapo-gspo/q060-dapo-vs-gspo.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q062 →](../07-reasoning-verifier/q062-rl-create-vs-elicit.md)
...
SFT 模仿 teacher 轨迹；online RL 在 reward 指引下从当前 policy 多次采样、探索并提高成功轨迹概率，可以 发现训练数据未显式给出的策略组合。

### PDF 原始深入理解

RL 的价值包含 elicitation、search 与 policy improvement；但前提是 reward 足够可靠且 exploration 能触达 成功区域。[P8]
...
1. 先给核心判断：SFT 模仿 teacher 轨迹；online RL 在 reward 指引下从当前 policy 多次采样、探索并提高成功轨迹概率，可以 发现训练数据未显式给出的策略组合。
2. 再解释机制：RL 的价值包含 elicitation、search 与 policy improvement；但前提是 reward 足够可靠且 exploration 能触达 成功区域。[P8]
3. 最后补一个边界条件或失败模式：SFT 只能提高数据中轨迹的 likelihood；online RL 可以在采样—评价—更新闭环中发现低概率新轨迹，这是 reasoning 改进的重要来源。
...
SFT 只能提高数据中轨迹的 likelihood；online RL 可以在采样—评价—更新闭环中发现低概率新轨迹，这是 reasoning 改进的重要来源。
...
```text
Problem: 为什么 Online RL 可能提升 reasoning，而 SFT 不一定？
...
- 把 RL 神化成“创造新知识”。
- 边界条件：SFT 只能提高数据中轨迹的 likelihood；online RL 可以在采样—评价—更新闭环中发现低概率新轨迹，这是 reasoning 改进的重要来源。
...
1. 如何证明提升来自 exploration 而不是训练 token 更多？
...
> **Q061：SFT 模仿 teacher 轨迹；online RL 在 reward 指引下从当前 policy 多次采样、探索并提高成功轨迹概率，可以 发现训练数据未显式给出的策略组合。**
...
- [Q067 · Entropy collapse 是什么？为什么 GRPO/DAPO 特别关注？](q067-entropy-collapse.md)
...
本题不能停在“SFT 模仿 teacher 轨迹；online RL 在 reward 指引下从当前 policy 多次采样、探索并提高成功轨迹概率，可以 发现训练数据未显式给出的策略组合”。更专业的表述是先确定五个对象：
...
对 **Q061**，最关键的机制判断是：**SFT 只能提高数据中轨迹的 likelihood；online RL 可以在采样—评价—更新闭环中发现低概率新轨迹，这是 reasoning 改进的重要来源。**

再进一步，工程上真正需要落地的是：验证“RL 提升 reasoning”时要控制 inference compute：若 RL 模型只是输出更长，能力提升可能来自更多 test-time compute，而非策略本身更优。
...
本题 PDF 的深入结论是：RL 的价值包含 elicitation、search 与 policy improvement；但前提是 reward 足够可靠且 exploration 能触达 成功区域。[P8]。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q061
...
| 3 | `pass@1/pass@k` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `verifier precision` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `reward sparsity` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `trajectory diversity` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `accuracy/token` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `entropy` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：SFT 模仿 teacher 轨迹；online RL 在 reward 指引下从当前 policy 多次采样、探索并提高成功轨迹概率，可以 发现训练数据未显式给出的策略组合 |
