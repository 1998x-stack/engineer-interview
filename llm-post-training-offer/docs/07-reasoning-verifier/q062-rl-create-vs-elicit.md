...
# Q062 · RL 是“创造能力”还是“激活已有能力”？

> **题型**：系统设计 · **难度**：L4 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q061](../07-reasoning-verifier/q061-online-rl-reasoning.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q063 →](../07-reasoning-verifier/q063-verifiable-vs-open-ended-rl.md)
...
更准确的答案是两者混合：重权预训练中极低概率行为，同时通过迭代搜索组合出新策略。

### PDF 原始深入理解

可以用 behavior support 的观点讨论：RL 很难从完全零支持的行为凭空学习，但能把微弱概率路径放大并组 合。
...
1. 先给核心判断：更准确的答案是两者混合：重权预训练中极低概率行为，同时通过迭代搜索组合出新策略。
2. 再解释机制：可以用 behavior support 的观点讨论：RL 很难从完全零支持的行为凭空学习，但能把微弱概率路径放大并组 合。
3. 最后补一个边界条件或失败模式：“创造”与“激活”不是互斥：RL 会 reweight 既有 latent behavior，也会通过组合与搜索让极低概率策略成为稳定策略。
...
“创造”与“激活”不是互斥：RL 会 reweight 既有 latent behavior，也会通过组合与搜索让极低概率策略成为稳定策略。
...
- 非黑即白回答。
- 边界条件：“创造”与“激活”不是互斥：RL 会 reweight 既有 latent behavior，也会通过组合与搜索让极低概率策略成为稳定策略。
...
1. 如何用 base model pass@k 与 RL 后 pass@1 判断 elicitation 成分？
...
> **Q062：更准确的答案是两者混合：重权预训练中极低概率行为，同时通过迭代搜索组合出新策略。**
...
本题不能停在“更准确的答案是两者混合：重权预训练中极低概率行为，同时通过迭代搜索组合出新策略”。更专业的表述是先确定五个对象：
...
对 **Q062**，最关键的机制判断是：**“创造”与“激活”不是互斥：RL 会 reweight 既有 latent behavior，也会通过组合与搜索让极低概率策略成为稳定策略。**

再进一步，工程上真正需要落地的是：用 base/SFT/RL 的 pass@k、trajectory pattern 与可复现新策略分析“elicitation vs creation”；结论通常是连续谱，而不是二元命题。
...
本题 PDF 的深入结论是：可以用 behavior support 的观点讨论：RL 很难从完全零支持的行为凭空学习，但能把微弱概率路径放大并组 合。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q062
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：更准确的答案是两者混合：重权预训练中极低概率行为，同时通过迭代搜索组合出新策略 |
