---
id: Q063
title: "为什么数学/代码 RL 比 open-ended chat RL 更容易？"
chapter: 7
...
# Q063 · 为什么数学/代码 RL 比 open-ended chat RL 更容易？

> **题型**：原理推导 · **难度**：L2 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q062](../07-reasoning-verifier/q062-rl-create-vs-elicit.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q064 →](../07-reasoning-verifier/q064-process-vs-outcome-reward.md)
...
前者有客观 verifier，reward 信噪比高；聊天的 helpfulness、truthfulness、style、安全往往主观且多目标冲 突。

### PDF 原始深入理解

因此“能不能规模化 RL”常由 reward verifiability 决定，而不是 optimizer 是否先进。
...
1. 先给核心判断：前者有客观 verifier，reward 信噪比高；聊天的 helpfulness、truthfulness、style、安全往往主观且多目标冲 突。
2. 再解释机制：因此“能不能规模化 RL”常由 reward verifiability 决定，而不是 optimizer 是否先进。
3. 最后补一个边界条件或失败模式：可验证任务的 reward 更接近 objective truth，能低成本扩展；开放聊天往往依赖 judge/RM，proxy gap 和主观性决定了 RL 上限。
...
可验证任务的 reward 更接近 objective truth，能低成本扩展；开放聊天往往依赖 judge/RM，proxy gap 和主观性决定了 RL 上限。
...
```text
Problem: 为什么数学/代码 RL 比 open-ended chat RL 更容易？
...
- 只说数学数据更多。
- 边界条件：可验证任务的 reward 更接近 objective truth，能低成本扩展；开放聊天往往依赖 judge/RM，proxy gap 和主观性决定了 RL 上限。
...
> **Q063：前者有客观 verifier，reward 信噪比高；聊天的 helpfulness、truthfulness、style、安全往往主观且多目标冲 突。**
...
本题不能停在“前者有客观 verifier，reward 信噪比高；聊天的 helpfulness、truthfulness、style、安全往往主观且多目标冲 突”。更专业的表述是先确定五个对象：
...
对 **Q063**，最关键的机制判断是：**可验证任务的 reward 更接近 objective truth，能低成本扩展；开放聊天往往依赖 judge/RM，proxy gap 和主观性决定了 RL 上限。**

再进一步，工程上真正需要落地的是：先测 reward/verifier 的可判定率和错误率。开放任务若 judge noise 高，应优先改善评价协议，而不是直接套数学 RL 的 recipe。
...
本题 PDF 的深入结论是：因此“能不能规模化 RL”常由 reward verifiability 决定，而不是 optimizer 是否先进。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q063
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：前者有客观 verifier，reward 信噪比高；聊天的 helpfulness、truthfulness、style、安全往往主观且多目标冲 突 |
