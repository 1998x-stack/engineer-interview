---
id: Q064
title: "Process Reward 一定比 Outcome Reward 好吗？"
chapter: 7
...
# Q064 · Process Reward 一定比 Outcome Reward 好吗？

> **题型**：系统设计 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q063](../07-reasoning-verifier/q063-verifiable-vs-open-ended-rl.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q065 →](../07-reasoning-verifier/q065-sparse-reward.md)
...
不一定。PRM 提供更密 credit，但过程标签可能固化人类路径、限制探索并引入额外噪声；ORM 更稀疏却更自 由。

### PDF 原始深入理解

长 horizon 任务常用 outcome 主目标 + 关键节点 process signal 的折中。
...
1. 先给核心判断：不一定。PRM 提供更密 credit，但过程标签可能固化人类路径、限制探索并引入额外噪声；ORM 更稀疏却更自 由。
2. 再解释机制：长 horizon 任务常用 outcome 主目标 + 关键节点 process signal 的折中。
3. 最后补一个边界条件或失败模式：PRM 改善局部 credit，但会把监督者的过程偏好写进模型；ORM 更自由，却把所有因果贡献压缩到最终 reward。
...
PRM 改善局部 credit，但会把监督者的过程偏好写进模型；ORM 更自由，却把所有因果贡献压缩到最终 reward。
...
```text
Problem: Process Reward 一定比 Outcome Reward 好吗？
...
- 默认步骤越多监督越好。
- 边界条件：PRM 改善局部 credit，但会把监督者的过程偏好写进模型；ORM 更自由，却把所有因果贡献压缩到最终 reward。
...
1. 什么时候 process reward 会伤害 emergent reasoning？
...
> **Q064：不一定。PRM 提供更密 credit，但过程标签可能固化人类路径、限制探索并引入额外噪声；ORM 更稀疏却更自 由。**
...
本题不能停在“不一定。PRM 提供更密 credit，但过程标签可能固化人类路径、限制探索并引入额外噪声；ORM 更稀疏却更自 由”。更专业的表述是先确定五个对象：
...
对 **Q064**，最关键的机制判断是：**PRM 改善局部 credit，但会把监督者的过程偏好写进模型；ORM 更自由，却把所有因果贡献压缩到最终 reward。**

再进一步，工程上真正需要落地的是：PRM 与 ORM 的比较必须等总 reward compute、等 rollout budget；PRM 的 dense signal 若来自昂贵模型 judge，成本与延迟可能改变最优方案。
...
本题 PDF 的深入结论是：长 horizon 任务常用 outcome 主目标 + 关键节点 process signal 的折中。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q064
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：不一定。PRM 提供更密 credit，但过程标签可能固化人类路径、限制探索并引入额外噪声；ORM 更稀疏却更自 由 |
