...
# Q065 · Sparse Reward 如何改善？

> **题型**：系统设计 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q064](../07-reasoning-verifier/q064-process-vs-outcome-reward.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q066 →](../07-reasoning-verifier/q066-cot-length-vs-capability.md)
...
curriculum、难度分层、reward shaping、subgoal verifier、process reward、rejection sampling warm- start、提高 sampling diversity。

### PDF 原始深入理解

目标是提高 P(R>0) 并降低 credit variance，但 shaping 不能改变最优策略到错误方向。
...
1. 先给核心判断：curriculum、难度分层、reward shaping、subgoal verifier、process reward、rejection sampling warm- start、提高 sampling diversity。
2. 再解释机制：目标是提高 P(R>0) 并降低 credit variance，但 shaping 不能改变最优策略到错误方向。
3. 最后补一个边界条件或失败模式：稀疏 reward 的核心问题是正信号密度。curriculum、shaping、process signal、分解任务都在提高有效 gradient 的出现概率。
...
稀疏 reward 的核心问题是正信号密度。curriculum、shaping、process signal、分解任务都在提高有效 gradient 的出现概率。
...
- 随意加 dense reward。
- 边界条件：稀疏 reward 的核心问题是正信号密度。curriculum、shaping、process signal、分解任务都在提高有效 gradient 的出现概率。
...
1. potential-based shaping 为什么更安全？
...
> **Q065：curriculum、难度分层、reward shaping、subgoal verifier、process reward、rejection sampling warm- start、提高 sampling diversity。**
...
本题不能停在“curriculum、难度分层、reward shaping、subgoal verifier、process reward、rejection sampling warm- start、提高 sampling diversity”。更专业的表述是先确定五个对象：
...
对 **Q065**，最关键的机制判断是：**稀疏 reward 的核心问题是正信号密度。curriculum、shaping、process signal、分解任务都在提高有效 gradient 的出现概率。**

再进一步，工程上真正需要落地的是：稀疏 reward 先从 curriculum 和 sampling 提高正例率，再考虑 shaping；shaping 一定要检查是否改变了真正最优策略。
...
本题 PDF 的深入结论是：目标是提高 P(R>0) 并降低 credit variance，但 shaping 不能改变最优策略到错误方向。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q065
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：curriculum、难度分层、reward shaping、subgoal verifier、process reward、rejection sampling warm- start、提高 sampling diversity |
