...
# Q066 · 如何避免模型把“更长 CoT”误学成“更高能力”？

> **题型**：系统设计 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q065](../07-reasoning-verifier/q065-sparse-reward.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q067 →](../07-reasoning-verifier/q067-entropy-collapse.md)
...
同时监控 accuracy、length、accuracy/token、pass@k、不同长度 bucket 的收益；采用 efficiency reward、 overlong shaping、长度控制 eval 与蒸馏。

### PDF 原始深入理解

长 reasoning 有时是探索结果，但长度本身不应成为 proxy。
...
1. 先给核心判断：同时监控 accuracy、length、accuracy/token、pass@k、不同长度 bucket 的收益；采用 efficiency reward、 overlong shaping、长度控制 eval 与蒸馏。
2. 再解释机制：长 reasoning 有时是探索结果，但长度本身不应成为 proxy。
3. 最后补一个边界条件或失败模式：长 CoT 常与高难样本相关，模型容易把相关性当因果。必须做 length-controlled eval、效率指标和相同正确率下的 token 成本比较。
...
长 CoT 常与高难样本相关，模型容易把相关性当因果。必须做 length-controlled eval、效率指标和相同正确率下的 token 成本比较。
...
```text
Problem: 如何避免模型把“更长 CoT”误学成“更高能力”？
...
- 只设 max_length 粗暴截断。
- 边界条件：长 CoT 常与高难样本相关，模型容易把相关性当因果。必须做 length-controlled eval、效率指标和相同正确率下的 token 成本比较。
...
1. 如何设计 length-controlled A/B evaluation？
...
> **Q066：同时监控 accuracy、length、accuracy/token、pass@k、不同长度 bucket 的收益；采用 efficiency reward、 overlong shaping、长度控制 eval 与蒸馏。**
...
本题不能停在“同时监控 accuracy、length、accuracy/token、pass@k、不同长度 bucket 的收益；采用 efficiency reward、 overlong shaping、长度控制 eval 与蒸馏”。更专业的表述是先确定五个对象：
...
对 **Q066**，最关键的机制判断是：**长 CoT 常与高难样本相关，模型容易把相关性当因果。必须做 length-controlled eval、效率指标和相同正确率下的 token 成本比较。**

再进一步，工程上真正需要落地的是：把长度当成本而不是能力 proxy：报告 accuracy-vs-tokens、reward-vs-length、同长度分桶能力，才能判断“思考更久”是否真的更有效。
...
本题 PDF 的深入结论是：长 reasoning 有时是探索结果，但长度本身不应成为 proxy。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q066
...
| 5 | `pass@1/pass@k` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `verifier precision` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `reward sparsity` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `trajectory diversity` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：同时监控 accuracy、length、accuracy/token、pass@k、不同长度 bucket 的收益；采用 efficiency reward、 overlong shaping、长度控制 eval 与蒸馏 |
