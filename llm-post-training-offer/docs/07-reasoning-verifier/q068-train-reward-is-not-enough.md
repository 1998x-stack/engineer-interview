...
# Q068 · 为什么只看训练 reward 非常危险？

> **题型**：高频题 · **难度**：L2 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q067](../07-reasoning-verifier/q067-entropy-collapse.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q069 →](../07-reasoning-verifier/q069-verifier-hacking.md)
...
proxy reward 可能被 exploit，且 reward 分布本身可能漂移。必须联合 held-out verifier、benchmark、 human/judge、KL、entropy、length 与 error taxonomy。

### PDF 原始深入理解

一条健康训练曲线应展示 reward 与真实任务指标共同改善，而不是 reward 单边上涨。

### PDF 原始常见失分点

把 reward 当 ground truth。
...
1. 先给核心判断：proxy reward 可能被 exploit，且 reward 分布本身可能漂移。必须联合 held-out verifier、benchmark、 human/judge、KL、entropy、length 与 error taxonomy。
2. 再解释机制：一条健康训练曲线应展示 reward 与真实任务指标共同改善，而不是 reward 单边上涨。
3. 最后补一个边界条件或失败模式：proxy reward 是训练目标，不是最终评测；必须有独立的 held-out task score、judge、人评、长度、KL、entropy 等交叉信号。
...
proxy reward 是训练目标，不是最终评测；必须有独立的 held-out task score、judge、人评、长度、KL、entropy 等交叉信号。
...
- 把 reward 当 ground truth。
- 边界条件：proxy reward 是训练目标，不是最终评测；必须有独立的 held-out task score、judge、人评、长度、KL、entropy 等交叉信号。
...
> **Q068：proxy reward 可能被 exploit，且 reward 分布本身可能漂移。必须联合 held-out verifier、benchmark、 human/judge、KL、entropy、length 与 error taxonomy。**
...
本题不能停在“proxy reward 可能被 exploit，且 reward 分布本身可能漂移。必须联合 held-out verifier、benchmark、 human/judge、KL、entropy、length 与 error taxonomy”。更专业的表述是先确定五个对象：
...
对 **Q068**，最关键的机制判断是：**proxy reward 是训练目标，不是最终评测；必须有独立的 held-out task score、judge、人评、长度、KL、entropy 等交叉信号。**

再进一步，工程上真正需要落地的是：训练 reward 是优化器看到的 proxy；评估必须至少有一个独立于训练 reward 的 evaluator，并审计高 reward 尾部行为。
...
本题 PDF 的深入结论是：一条健康训练曲线应展示 reward 与真实任务指标共同改善，而不是 reward 单边上涨。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q068
...
| 4 | `KL target/error` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `reward quantiles` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `reward calibration` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `pass@1/pass@k` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `verifier precision` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：proxy reward 可能被 exploit，且 reward 分布本身可能漂移。必须联合 held-out verifier、benchmark、 human/judge、KL、entropy、length 与 error taxonomy |
