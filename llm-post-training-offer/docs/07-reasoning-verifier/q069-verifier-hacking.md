---
id: Q069
title: "Rule-based verifier 也会被 hacking 吗？"
chapter: 7
...
# Q069 · Rule-based verifier 也会被 hacking 吗？

> **题型**：系统设计 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q068](../07-reasoning-verifier/q068-train-reward-is-not-enough.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q070 →](../07-reasoning-verifier/q070-good-reward-design.md)
...
会。数学有等价表达/单位/格式漏洞，代码有弱测试、hardcode、超时/环境 exploit。

### PDF 原始深入理解

应做 adversarial verifier tests、hidden tests、canonicalization、sandbox 与多 verifier 交叉验证。
...
1. 先给核心判断：会。数学有等价表达/单位/格式漏洞，代码有弱测试、hardcode、超时/环境 exploit。
2. 再解释机制：应做 adversarial verifier tests、hidden tests、canonicalization、sandbox 与多 verifier 交叉验证。
3. 最后补一个边界条件或失败模式：verifier 也是程序/模型，会有 parser、格式、测试覆盖和 judge 偏差。任何自动 reward 都要像安全边界一样做 adversarial testing。
...
verifier 也是程序/模型，会有 parser、格式、测试覆盖和 judge 偏差。任何自动 reward 都要像安全边界一样做 adversarial testing。
...
```text
Problem: Rule-based verifier 也会被 hacking 吗？
...
- 认为规则 verifier 天然安全。
- 边界条件：verifier 也是程序/模型，会有 parser、格式、测试覆盖和 judge 偏差。任何自动 reward 都要像安全边界一样做 adversarial testing。
...
1. 如何估计 verifier false positive 对 RL 的危害？
...
> **Q069：会。数学有等价表达/单位/格式漏洞，代码有弱测试、hardcode、超时/环境 exploit。**
...
本题不能停在“会。数学有等价表达/单位/格式漏洞，代码有弱测试、hardcode、超时/环境 exploit”。更专业的表述是先确定五个对象：
...
对 **Q069**，最关键的机制判断是：**verifier 也是程序/模型，会有 parser、格式、测试覆盖和 judge 偏差。任何自动 reward 都要像安全边界一样做 adversarial testing。**

再进一步，工程上真正需要落地的是：verifier hacking 要做 adversarial test suite，并把已发现漏洞固化成 regression；仅靠换一个更大 judge，往往只是延后被利用。
...
本题 PDF 的深入结论是：应做 adversarial verifier tests、hidden tests、canonicalization、sandbox 与多 verifier 交叉验证。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q069
...
| # | 指标 | 如何解释 |
|---:|---|---|
| 1 | `pass@1/pass@k` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 2 | `verifier precision` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 3 | `reward sparsity` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `trajectory diversity` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `accuracy/token` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `entropy` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `independent eval gap` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：会。数学有等价表达/单位/格式漏洞，代码有弱测试、hardcode、超时/环境 exploit |
