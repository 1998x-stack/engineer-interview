...
# Q070 · 如何设计一个“好 Reward”？

> **题型**：系统设计 · **难度**：L4 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q069](../07-reasoning-verifier/q069-verifier-hacking.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q071 →](../08-rl-systems/q071-grpo-dataflow.md)
...
四个关键词：目标相关、信号足够、难以作弊、可规模化。必要时再加稳定尺度与可解释拆分。

### PDF 原始深入理解

这四者互相冲突：越 dense 越可能泄漏路径，越复杂 judge 越贵且越可被 exploit。reward design 是后训练的 核心产品定义。
...
1. 先给核心判断：四个关键词：目标相关、信号足够、难以作弊、可规模化。必要时再加稳定尺度与可解释拆分。
2. 再解释机制：这四者互相冲突：越 dense 越可能泄漏路径，越复杂 judge 越贵且越可被 exploit。reward design 是后训练的 核心产品定义。
3. 最后补一个边界条件或失败模式：好 reward 同时追求相关性、可学习密度、抗 hacking、低成本和可扩展；若这些目标冲突，优先把关键条件做成 constraint/gate。
...
好 reward 同时追求相关性、可学习密度、抗 hacking、低成本和可扩展；若这些目标冲突，优先把关键条件做成 constraint/gate。
...
\[R=w_1R_{task}+w_2R_{quality}-\lambda C\] 但对安全、合法性、格式有效性等“不可被抵消”的条件，优先使用 hard constraint/gate。
...
- 从公式出发而不是从真实目标出发。
- 边界条件：好 reward 同时追求相关性、可学习密度、抗 hacking、低成本和可扩展；若这些目标冲突，优先把关键条件做成 constraint/gate。
...
1. 如果 reward 只能满足其中三项，你优先牺牲哪一项？ 举一反三 / 章节检查 尝试不看答案，用“任务 → 数据 → reward → exploration → credit → compute → failure → eval”重新讲本章。若能把 3 个以上问题串成一条因果链，而不是逐题背诵，本章才算掌 握。 第八章 RL 系：Rollout、分布式与显存 本章目标 完成面试题 71-80。要求不是记忆名词，而是把每个方法与其输入、目标函数、failure mode 和工程代价连接起来。
...
> **Q070：四个关键词：目标相关、信号足够、难以作弊、可规模化。必要时再加稳定尺度与可解释拆分。**
...
本题不能停在“四个关键词：目标相关、信号足够、难以作弊、可规模化。必要时再加稳定尺度与可解释拆分”。更专业的表述是先确定五个对象：
...
对 **Q070**，最关键的机制判断是：**好 reward 同时追求相关性、可学习密度、抗 hacking、低成本和可扩展；若这些目标冲突，优先把关键条件做成 constraint/gate。**

再进一步，工程上真正需要落地的是：一个可用 reward 设计文档应写清 true objective、proxy、不可抵消约束、scale/normalization、attack surface、cost 和回归评测。
...
本题 PDF 的深入结论是：这四者互相冲突：越 dense 越可能泄漏路径，越复杂 judge 越贵且越可被 exploit。reward design 是后训练的 核心产品定义。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q070
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：四个关键词：目标相关、信号足够、难以作弊、可规模化。必要时再加稳定尺度与可解释拆分 |
