---
id: Q071
title: "公开真题：一个完整 GRPO 数据流是什么？"
chapter: 8
chapter_title: "RL 系统、Rollout 与分布式训练"
type: "公开真题"
level: L3
priority: high
tags: [grpo, reward, verifier]
source_refs: [I6, P11, P12, P13, R1]
---

# Q071 · 公开真题：一个完整 GRPO 数据流是什么？

> **题型**：公开真题 · **难度**：L3 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q070](../07-reasoning-verifier/q070-good-reward-design.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q072 →](../08-rl-systems/q072-rollout-vs-teacher-forcing-cost.md)
...
Prompt → rollout 生成 G 个 response → reward/verifier → group normalization → advantages → logprob/importance ratio → policy gradient → actor update → 权重同步 → 下一轮 rollout。

### PDF 原始深入理解

回答时把每一步对应到 tensor shape、设备位置与 policy version，会显著区分“读过论文”和“跑过系统”。

### PDF 原始常见失分点

漏掉 old policy logprob/weight sync。
...
1. 先给核心判断：Prompt → rollout 生成 G 个 response → reward/verifier → group normalization → advantages → logprob/importance ratio → policy gradient → actor update → 权重同步 → 下一轮 rollout。
2. 再解释机制：回答时把每一步对应到 tensor shape、设备位置与 policy version，会显著区分“读过论文”和“跑过系统”。
3. 最后补一个边界条件或失败模式：完整 GRPO 数据流要能落到 tensor 与 version：prompt→rollout→reward→group stats→logprob/ratio→loss→update→weight sync。
...
完整 GRPO 数据流要能落到 tensor 与 version：prompt→rollout→reward→group stats→logprob/ratio→loss→update→weight sync。
...
```text
Prompt -> Rollout Workers -> Responses
       -> Reward / Verifier
       -> Group Statistics / Advantage
       -> Old & Current LogProbs
       -> PPO/GRPO-style Loss
       -> Learner Update
...
- 漏掉 old policy logprob/weight sync。
- 边界条件：完整 GRPO 数据流要能落到 tensor 与 version：prompt→rollout→reward→group stats→logprob/ratio→loss→update→weight sync。
...
1. 哪些数据应该在 rollout 端缓存？ [I6]
2. 哪些 tensor 必须携带 policy_version？
3. reward 计算和 old_logprob 计算应该在哪一侧完成？
4. 瓶颈到底是 compute、memory、communication 还是 queue？
5. 如何定义 useful tokens/s 而不是 raw tokens/s？
6. 权重同步频率减半会对 throughput 和 staleness 各有什么影响？
...
> **Q071：Prompt → rollout 生成 G 个 response → reward/verifier → group normalization → advantages → logprob/importance ratio → policy gradient → actor update → 权重同步 → 下一轮 rollout。**

## 13. 参考资料

- **[I6] 公开面经**：[牛客：百川大模型算法二面](https://www.nowcoder.com/enterprise/267815/interview)
...
本题不能停在“Prompt → rollout 生成 G 个 response → reward/verifier → group normalization → advantages → logprob/importance ratio → policy gradient → actor update → 权重同步 → 下一轮 rollout”。更专业的表述是先确定五个对象：
...
对 **Q071**，最关键的机制判断是：**完整 GRPO 数据流要能落到 tensor 与 version：prompt→rollout→reward→group stats→logprob/ratio→loss→update→weight sync。**

再进一步，工程上真正需要落地的是：画 GRPO 数据流时标清每个 tensor 的 producer/consumer、shape、dtype、device 和 version；这能把算法理解直接落到框架实现。
...
本题 PDF 的深入结论是：回答时把每一步对应到 tensor shape、设备位置与 policy version，会显著区分“读过论文”和“跑过系统”。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
| 5 | `group std` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `tie rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `rollout tokens/s` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `learner tokens/s` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：Prompt → rollout 生成 G 个 response → reward/verifier → group normalization → advantages → logprob/importance ratio → policy gradient → actor update → 权重同步 → 下一轮 rollout |
