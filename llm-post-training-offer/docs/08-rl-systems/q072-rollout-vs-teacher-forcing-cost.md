---
id: Q072
title: "为什么 RL rollout 比 SFT teacher-forcing 贵？"
chapter: 8
...
# Q072 · 为什么 RL rollout 比 SFT teacher-forcing 贵？

> **题型**：原理推导 · **难度**：L2 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q071](../08-rl-systems/q071-grpo-dataflow.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q073 →](../08-rl-systems/q073-rollout-tail-gpu-utilization.md)
...
generation 是逐 token autoregressive，无法像 SFT 一样对整条目标序列一次并行前向；长 CoT 让 decode token 成为主要成本。

### PDF 原始深入理解

训练侧偏 FLOPs，rollout 侧偏 memory bandwidth + KV cache + sequential latency，两者最佳并行策略不 同。
...
1. 先给核心判断：generation 是逐 token autoregressive，无法像 SFT 一样对整条目标序列一次并行前向；长 CoT 让 decode token 成为主要成本。
2. 再解释机制：训练侧偏 FLOPs，rollout 侧偏 memory bandwidth + KV cache + sequential latency，两者最佳并行策略不 同。
3. 最后补一个边界条件或失败模式：teacher forcing 能并行处理整段 target，decode 必须逐 token 自回归；因此 rollout 的 wall-clock 由输出长度、KV cache 和调度效率主导。
...
teacher forcing 能并行处理整段 target，decode 必须逐 token 自回归；因此 rollout 的 wall-clock 由输出长度、KV cache 和调度效率主导。
...
```text
Problem: 为什么 RL rollout 比 SFT teacher-forcing 贵？
...
- 只比较 forward 次数。
- 边界条件：teacher forcing 能并行处理整段 target，decode 必须逐 token 自回归；因此 rollout 的 wall-clock 由输出长度、KV cache 和调度效率主导。
...
1. 为什么 rollout engine 常与 learner 分离？
...
> **Q072：generation 是逐 token autoregressive，无法像 SFT 一样对整条目标序列一次并行前向；长 CoT 让 decode token 成为主要成本。**
...
本题不能停在“generation 是逐 token autoregressive，无法像 SFT 一样对整条目标序列一次并行前向；长 CoT 让 decode token 成为主要成本”。更专业的表述是先确定五个对象：
...
对 **Q072**，最关键的机制判断是：**teacher forcing 能并行处理整段 target，decode 必须逐 token 自回归；因此 rollout 的 wall-clock 由输出长度、KV cache 和调度效率主导。**

再进一步，工程上真正需要落地的是：teacher forcing 可并行处理整序列，decode 必须自回归；因此同 token 数下 rollout 的 wall-clock 与 KV-cache 压力通常更高。
...
本题 PDF 的深入结论是：训练侧偏 FLOPs，rollout 侧偏 memory bandwidth + KV cache + sequential latency，两者最佳并行策略不 同。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q072
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：generation 是逐 token autoregressive，无法像 SFT 一样对整条目标序列一次并行前向；长 CoT 让 decode token 成为主要成本 |
