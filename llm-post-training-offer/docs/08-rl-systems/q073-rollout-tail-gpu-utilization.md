---
id: Q073
title: "公开真题：rollout 长尾为什么降低 GPU 利用率？"
chapter: 8
...
priority: normal
tags: [rl-systems]
source_refs: [I7, I8, P11, P12, P13, R1]
---

# Q073 · 公开真题：rollout 长尾为什么降低 GPU 利用率？

> **题型**：公开真题 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q072](../08-rl-systems/q072-rollout-vs-teacher-forcing-cost.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q074 →](../08-rl-systems/q074-rollout-tail-solutions.md)
...
同批 response 长度差异大，短序列结束后仍需等待最长序列，造成 decode slot 空闲与同步等待。

### PDF 原始深入理解

问题可视为 straggler + dynamic sequence length；在 reasoning RL 中 1k 与 30k token 可同批出现，长尾非 常严重。

### PDF 原始常见失分点

只说 padding 浪费；decode 阶段更关键是活跃序列数量下降。
...
1. 先给核心判断：同批 response 长度差异大，短序列结束后仍需等待最长序列，造成 decode slot 空闲与同步等待。
2. 再解释机制：问题可视为 straggler + dynamic sequence length；在 reasoning RL 中 1k 与 30k token 可同批出现，长尾非 常严重。
3. 最后补一个边界条件或失败模式：一个 batch 的完成时间被最长 response 拖住，短任务提前结束后形成空槽；长 CoT 尤其容易产生严重 straggler。
...
一个 batch 的完成时间被最长 response 拖住，短任务提前结束后形成空槽；长 CoT 尤其容易产生严重 straggler。
...
```text
Problem: 公开真题：rollout 长尾为什么降低 GPU 利用率？
...
- 只说 padding 浪费；decode 阶段更关键是活跃序列数量下降。
- 边界条件：一个 batch 的完成时间被最长 response 拖住，短任务提前结束后形成空槽；长 CoT 尤其容易产生严重 straggler。
...
1. 如何用 length bucket 和 continuous batching 缓解？ [I7][I8]
...
> **Q073：同批 response 长度差异大，短序列结束后仍需等待最长序列，造成 decode slot 空闲与同步等待。**
...
本题不能停在“同批 response 长度差异大，短序列结束后仍需等待最长序列，造成 decode slot 空闲与同步等待”。更专业的表述是先确定五个对象：
...
对 **Q073**，最关键的机制判断是：**一个 batch 的完成时间被最长 response 拖住，短任务提前结束后形成空槽；长 CoT 尤其容易产生严重 straggler。**

再进一步，工程上真正需要落地的是：长尾问题用输出长度 histogram、p95/p99 decode latency、batch active slots、GPU active ratio 联合定位；平均长度不足以解释尾部拖累。
...
本题 PDF 的深入结论是：问题可视为 straggler + dynamic sequence length；在 reasoning RL 中 1k 与 30k token 可同批出现，长尾非 常严重。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q073
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：同批 response 长度差异大，短序列结束后仍需等待最长序列，造成 decode slot 空闲与同步等待 |
