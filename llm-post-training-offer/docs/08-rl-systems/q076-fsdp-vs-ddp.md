...
priority: normal
tags: [fsdp]
source_refs: [I1, I7, P12, P11, P13, R1]
---

# Q076 · 公开真题：FSDP 与 DDP 的核心区别？

> **题型**：公开真题 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q075](../08-rl-systems/q075-vllm-for-rollout.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q077 →](../08-rl-systems/q077-zero-stages.md)
...
DDP 每 rank 保持完整参数/优化器，梯度 all-reduce；FSDP 对参数、梯度、optimizer state 做 shard，并在计 算前 all-gather、之后 reshard/reduce-scatter。[P12]

### PDF 原始深入理解

FSDP 节省内存但增加通信与调度复杂度；FSDP2 以 per-parameter sharding 提升可用性。
...
1. 先给核心判断：DDP 每 rank 保持完整参数/优化器，梯度 all-reduce；FSDP 对参数、梯度、optimizer state 做 shard，并在计 算前 all-gather、之后 reshard/reduce-scatter。[P12]
2. 再解释机制：FSDP 节省内存但增加通信与调度复杂度；FSDP2 以 per-parameter sharding 提升可用性。
3. 最后补一个边界条件或失败模式：DDP 复制模型状态、聚合梯度；FSDP 通过 all-gather/reduce-scatter 在计算前后动态 shard 参数、梯度和 optimizer state。
...
DDP 复制模型状态、聚合梯度；FSDP 通过 all-gather/reduce-scatter 在计算前后动态 shard 参数、梯度和 optimizer state。
...
- 说 FSDP 只是“参数切片”。
- 边界条件：DDP 复制模型状态、聚合梯度；FSDP 通过 all-gather/reduce-scatter 在计算前后动态 shard 参数、梯度和 optimizer state。
...
1. FULL_SHARD 与 SHARD_GRAD_OP 的通信/显存 trade-off？ [I1][I7]
...
> **Q076：DDP 每 rank 保持完整参数/优化器，梯度 all-reduce；FSDP 对参数、梯度、optimizer state 做 shard，并在计 算前 all-gather、之后 reshard/reduce-scatter。[P12]**
...
本题不能停在“DDP 每 rank 保持完整参数/优化器，梯度 all-reduce；FSDP 对参数、梯度、optimizer state 做 shard，并在计 算前 all-gather、之后 reshard/reduce-scatter。[P12]”。更专业的表述是先确定五个对象：
...
对 **Q076**，最关键的机制判断是：**DDP 复制模型状态、聚合梯度；FSDP 通过 all-gather/reduce-scatter 在计算前后动态 shard 参数、梯度和 optimizer state。**

再进一步，工程上真正需要落地的是：FSDP 对参数/梯度/optimizer state 做分片并引入 all-gather/reduce-scatter；面试应同时解释内存下降和通信/碎片化/重计算代价。
...
本题 PDF 的深入结论是：FSDP 节省内存但增加通信与调度复杂度；FSDP2 以 per-parameter sharding 提升可用性。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q076
...
| # | 指标 | 如何解释 |
|---:|---|---|
| 1 | `all-gather time` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 2 | `shard memory` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 3 | `rollout tokens/s` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `learner tokens/s` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `GPU active ratio` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `p95/p99 latency` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `peak memory` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `network bytes/s` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：DDP 每 rank 保持完整参数/优化器，梯度 all-reduce；FSDP 对参数、梯度、optimizer state 做 shard，并在计 算前 all-gather、之后 reshard/reduce-scatter。[P12] |
