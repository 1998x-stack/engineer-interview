---
id: Q077
title: "公开真题：ZeRO-1/2/3 分别 shard 什么？"
chapter: 8
...
priority: normal
tags: [zero]
source_refs: [I1, P11, P12, P13, R1]
---

# Q077 · 公开真题：ZeRO-1/2/3 分别 shard 什么？

> **题型**：公开真题 · **难度**：L2 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q076](../08-rl-systems/q076-fsdp-vs-ddp.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q078 →](../08-rl-systems/q078-rl-memory-vs-sft.md)
...
Stage 1 shard optimizer states；Stage 2 再 shard gradients；Stage 3 再 shard parameters。

### PDF 原始深入理解

记忆表格只是第一层，第二层要能估算 Adam 场景参数/梯度/一二阶状态的显存占比，并讨论通信成本。[P11]
...
1. 先给核心判断：Stage 1 shard optimizer states；Stage 2 再 shard gradients；Stage 3 再 shard parameters。
2. 再解释机制：记忆表格只是第一层，第二层要能估算 Adam 场景参数/梯度/一二阶状态的显存占比，并讨论通信成本。[P11]
3. 最后补一个边界条件或失败模式：ZeRO 的演进就是逐步消除数据并行冗余：Stage 1 shard optimizer，Stage 2 再 shard gradient，Stage 3 再 shard parameter。
...
ZeRO 的演进就是逐步消除数据并行冗余：Stage 1 shard optimizer，Stage 2 再 shard gradient，Stage 3 再 shard parameter。
...
```text
Problem: 公开真题：ZeRO-1/2/3 分别 shard 什么？
...
- 把 stage 顺序记反。
- 边界条件：ZeRO 的演进就是逐步消除数据并行冗余：Stage 1 shard optimizer，Stage 2 再 shard gradient，Stage 3 再 shard parameter。
...
1. bf16 参数 + fp32 optimizer 时每参数大约多少字节？ [I1]
...
> **Q077：Stage 1 shard optimizer states；Stage 2 再 shard gradients；Stage 3 再 shard parameters。**
...
本题不能停在“Stage 1 shard optimizer states；Stage 2 再 shard gradients；Stage 3 再 shard parameters”。更专业的表述是先确定五个对象：
...
对 **Q077**，最关键的机制判断是：**ZeRO 的演进就是逐步消除数据并行冗余：Stage 1 shard optimizer，Stage 2 再 shard gradient，Stage 3 再 shard parameter。**

再进一步，工程上真正需要落地的是：ZeRO-1/2/3 记忆表只是起点；进一步要能估算每阶段每卡状态量，并解释 stage 越高为什么通信与实现复杂度越大。
...
本题 PDF 的深入结论是：记忆表格只是第一层，第二层要能估算 Adam 场景参数/梯度/一二阶状态的显存占比，并讨论通信成本。[P11]。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q077
...
| # | 指标 | 如何解释 |
|---:|---|---|
| 1 | `optimizer/grad/param shard bytes` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 2 | `rollout tokens/s` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 3 | `learner tokens/s` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `GPU active ratio` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `p95/p99 latency` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `peak memory` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `network bytes/s` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `weight-sync time` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：Stage 1 shard optimizer states；Stage 2 再 shard gradients；Stage 3 再 shard parameters |
