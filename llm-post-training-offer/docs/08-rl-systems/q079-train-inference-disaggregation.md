---
id: Q079
title: "RL 训推分离如何设计？weight sync 的 trade-off 是什么？"
chapter: 8
...
# Q079 · RL 训推分离如何设计？weight sync 的 trade-off 是什么？

> **题型**：系统设计 · **难度**：L4 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q078](../08-rl-systems/q078-rl-memory-vs-sft.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q080 →](../08-rl-systems/q080-trl-verl-openrlhf.md)
...
rollout workers 优化 decode，learner GPUs 优化训练；learner 更新后需把新权重同步到 rollout。同步越频 繁，policy fresh 但通信贵；越稀疏，吞吐高但 staleness 大。

### PDF 原始深入理解

大模型可用分片权重广播、增量/异步更新与版本控制。真正指标是 tokens/sec × sample freshness × learner utilization。
...
1. 先给核心判断：rollout workers 优化 decode，learner GPUs 优化训练；learner 更新后需把新权重同步到 rollout。同步越频 繁，policy fresh 但通信贵；越稀疏，吞吐高但 staleness 大。
2. 再解释机制：大模型可用分片权重广播、增量/异步更新与版本控制。真正指标是 tokens/sec × sample freshness × learner utilization。
3. 最后补一个边界条件或失败模式：训推分离把 decode 与 learner 放到不同资源池，换来各自最优并行方式；真正难点是权重同步频率与 policy freshness。
...
训推分离把 decode 与 learner 放到不同资源池，换来各自最优并行方式；真正难点是权重同步频率与 policy freshness。
...
\[\text{throughput}\uparrow \Longleftrightarrow \text{sync frequency}\downarrow \Longleftrightarrow \text{policy staleness}\uparrow\]
...
```text
[rollout pool] --trajectories--> [reward] --> [learner pool]
      ^                                      |
      |--------- versioned weight sync ------|
metrics: version lag, sync bytes/s, rollout tokens/s, learner MFU
```
...
- 只最大化单侧吞吐。
- 边界条件：训推分离把 decode 与 learner 放到不同资源池，换来各自最优并行方式；真正难点是权重同步频率与 policy freshness。
...
1. 怎么定义“可接受的 policy version lag”？
...
> **Q079：rollout workers 优化 decode，learner GPUs 优化训练；learner 更新后需把新权重同步到 rollout。同步越频 繁，policy fresh 但通信贵；越稀疏，吞吐高但 staleness 大。**
...
本题不能停在“rollout workers 优化 decode，learner GPUs 优化训练；learner 更新后需把新权重同步到 rollout。同步越频 繁，policy fresh 但通信贵；越稀疏，吞吐高但 staleness 大”。更专业的表述是先确定五个对象：
...
对 **Q079**，最关键的机制判断是：**训推分离把 decode 与 learner 放到不同资源池，换来各自最优并行方式；真正难点是权重同步频率与 policy freshness。**

再进一步，工程上真正需要落地的是：训推分离要有版本协议：何时发布新权重、rollout 如何标记版本、最大 staleness、同步失败怎么处理；没有协议的异步只是在制造不可控 off-policy。
...
本题 PDF 的深入结论是：大模型可用分片权重广播、增量/异步更新与版本控制。真正指标是 tokens/sec × sample freshness × learner utilization。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：rollout workers 优化 decode，learner GPUs 优化训练；learner 更新后需把新权重同步到 rollout。同步越频 繁，policy fresh 但通信贵；越稀疏，吞吐高但 staleness 大 |
