...
priority: normal
tags: [reward]
source_refs: [I7, P8, P9, P10]
---

# Q081 · 公开真题：怎么判断一次 RL 训练“质量达标”？

> **题型**：公开真题 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q080](../08-rl-systems/q080-trl-verl-openrlhf.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q082 →](../09-eval-debug/q082-reward-up-benchmark-flat.md)
...
分五层：optimization（loss/grad norm）、policy（KL/entropy）、reward（均值/方差/分桶）、capability （pass@1/benchmark）、behavior（judge/human/safety）。

### PDF 原始深入理解

训练合格不是 reward 最高，而是 held-out 能力改善、行为没有退化、曲线稳定且结果可复现。
...
1. 先给核心判断：分五层：optimization（loss/grad norm）、policy（KL/entropy）、reward（均值/方差/分桶）、capability （pass@1/benchmark）、behavior（judge/human/safety）。
2. 再解释机制：训练合格不是 reward 最高，而是 held-out 能力改善、行为没有退化、曲线稳定且结果可复现。
3. 最后补一个边界条件或失败模式：“质量达标”至少包含 optimization stability、policy behavior、reward validity、held-out capability 和成本五层 gate。
...
“质量达标”至少包含 optimization stability、policy behavior、reward validity、held-out capability 和成本五层 gate。
...
```text
Problem: 公开真题：怎么判断一次 RL 训练“质量达标”？
...
- 只给最终 benchmark。
- 边界条件：“质量达标”至少包含 optimization stability、policy behavior、reward validity、held-out capability 和成本五层 gate。
...
> **Q081：分五层：optimization（loss/grad norm）、policy（KL/entropy）、reward（均值/方差/分桶）、capability （pass@1/benchmark）、behavior（judge/human/safety）。**
...
本题不能停在“分五层：optimization（loss/grad norm）、policy（KL/entropy）、reward（均值/方差/分桶）、capability （pass@1/benchmark）、behavior（judge/human/safety）”。更专业的表述是先确定五个对象：
...
对 **Q081**，最关键的机制判断是：**“质量达标”至少包含 optimization stability、policy behavior、reward validity、held-out capability 和成本五层 gate。**

再进一步，工程上真正需要落地的是：质量 gate 应覆盖 optimization、policy、reward、capability、behavior、system 六层；每层定义“继续训练/回滚/人工审计”的阈值。
...
本题 PDF 的深入结论是：训练合格不是 reward 最高，而是 held-out 能力改善、行为没有退化、曲线稳定且结果可复现。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q081
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：分五层：optimization（loss/grad norm）、policy（KL/entropy）、reward（均值/方差/分桶）、capability （pass@1/benchmark）、behavior（judge/human/safety） |
