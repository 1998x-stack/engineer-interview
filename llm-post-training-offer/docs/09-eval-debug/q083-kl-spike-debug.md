...
[← Q082](../09-eval-debug/q082-reward-up-benchmark-flat.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q084 →](../09-eval-debug/q084-entropy-down-debug.md)
...
可能是 LR/epoch 过大、clip 失效、reward scale 异常、stale rollout、reference/old policy 混淆、logprob 数 值错误。

### PDF 原始深入理解

应同时看 ratio 分布、clip fraction、policy version、grad norm 和 reward scale，按数据流定位。
...
1. 先给核心判断：可能是 LR/epoch 过大、clip 失效、reward scale 异常、stale rollout、reference/old policy 混淆、logprob 数 值错误。
2. 再解释机制：应同时看 ratio 分布、clip fraction、policy version、grad norm 和 reward scale，按数据流定位。
3. 最后补一个边界条件或失败模式：KL spike 是 policy 快速漂移的信号；需要联查 LR、clip fraction、ratio tail、staleness、reward scale 与 reference/version 是否一致。
...
KL spike 是 policy 快速漂移的信号；需要联查 LR、clip fraction、ratio tail、staleness、reward scale 与 reference/version 是否一致。
...
- 只调大 β。
- 边界条件：KL spike 是 policy 快速漂移的信号；需要联查 LR、clip fraction、ratio tail、staleness、reward scale 与 reference/version 是否一致。
...
> **Q083：可能是 LR/epoch 过大、clip 失效、reward scale 异常、stale rollout、reference/old policy 混淆、logprob 数 值错误。**
...
本题不能停在“可能是 LR/epoch 过大、clip 失效、reward scale 异常、stale rollout、reference/old policy 混淆、logprob 数 值错误”。更专业的表述是先确定五个对象：
...
对 **Q083**，最关键的机制判断是：**KL spike 是 policy 快速漂移的信号；需要联查 LR、clip fraction、ratio tail、staleness、reward scale 与 reference/version 是否一致。**

再进一步，工程上真正需要落地的是：KL spike 的第一目标是定位“从哪一步开始分叉”；联合 ratio、clip fraction、grad norm、policy version、reward scale 能快速区分算法更新过猛与数据错位。
...
本题 PDF 的深入结论是：应同时看 ratio 分布、clip fraction、policy version、grad norm 和 reward scale，按数据流定位。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q083
...
| 4 | `queue depth` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `data age` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：可能是 LR/epoch 过大、clip 失效、reward scale 异常、stale rollout、reference/old policy 混淆、logprob 数 值错误 |
