...
# Q084 · Entropy 一路下降怎么办？

> **题型**：系统设计 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q083](../09-eval-debug/q083-kl-spike-debug.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q085 →](../09-eval-debug/q085-reward-variance-debug.md)
...
先区分健康收敛与 collapse：若 pass@1 上升且 pass@k/diversity 仍好，可能正常；若 reward plateau、 group similarity 上升，则要增强 exploration。

### PDF 原始深入理解

可调 sampling temperature、clip-higher、entropy bonus、harder prompts、diversity sampling，并检查 reward 是否偏好模板化。
...
1. 先给核心判断：先区分健康收敛与 collapse：若 pass@1 上升且 pass@k/diversity 仍好，可能正常；若 reward plateau、 group similarity 上升，则要增强 exploration。
2. 再解释机制：可调 sampling temperature、clip-higher、entropy bonus、harder prompts、diversity sampling，并检查 reward 是否偏好模板化。
3. 最后补一个边界条件或失败模式：entropy 下降可能是正常收敛，也可能是探索塌缩。判断标准是 reward/held-out/diversity 是否同步改善，以及 group variance 是否消失。
...
entropy 下降可能是正常收敛，也可能是探索塌缩。判断标准是 reward/held-out/diversity 是否同步改善，以及 group variance 是否消失。
...
- “entropy 越高越好”也错误。
- 边界条件：entropy 下降可能是正常收敛，也可能是探索塌缩。判断标准是 reward/held-out/diversity 是否同步改善，以及 group variance 是否消失。
...
1. 应该监控 token entropy 还是 sequence diversity？
2. 你会先加日志还是先调超参？为什么？
...
> **Q084：先区分健康收敛与 collapse：若 pass@1 上升且 pass@k/diversity 仍好，可能正常；若 reward plateau、 group similarity 上升，则要增强 exploration。**
...
- [Q082 · Reward 一直涨但 benchmark 不涨，怎么排查？](q082-reward-up-benchmark-flat.md)
...
本题不能停在“先区分健康收敛与 collapse：若 pass@1 上升且 pass@k/diversity 仍好，可能正常；若 reward plateau、 group similarity 上升，则要增强 exploration”。更专业的表述是先确定五个对象：
...
对 **Q084**，最关键的机制判断是：**entropy 下降可能是正常收敛，也可能是探索塌缩。判断标准是 reward/held-out/diversity 是否同步改善，以及 group variance 是否消失。**

再进一步，工程上真正需要落地的是：entropy down 不一定坏；关键看 reward/benchmark/diversity 是否同步改善。若 reward plateau 同时 entropy 继续掉，才更像 collapse。
...
本题 PDF 的深入结论是：可调 sampling temperature、clip-higher、entropy bonus、harder prompts、diversity sampling，并检查 reward 是否偏好模板化。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q084
...
| 4 | `group std` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `tie rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `reward/benchmark delta` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `KL/entropy/grad norm` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `metric slices` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：先区分健康收敛与 collapse：若 pass@1 上升且 pass@k/diversity 仍好，可能正常；若 reward plateau、 group similarity 上升，则要增强 exploration |
