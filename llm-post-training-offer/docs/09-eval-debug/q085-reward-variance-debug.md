...
# Q085 · Reward variance 很大怎么办？

> **题型**：系统设计 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q084](../09-eval-debug/q084-entropy-down-debug.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q086 →](../09-eval-debug/q086-offline-vs-online-eval.md)
...
先按 task/difficulty/length 分桶，判断是异质性还是 judge 噪声。再考虑 per-task normalization、clipping、 calibration、group baseline 或 reward decomposition。
...
1. 先给核心判断：先按 task/difficulty/length 分桶，判断是异质性还是 judge 噪声。再考虑 per-task normalization、clipping、 calibration、group baseline 或 reward decomposition。
2. 再解释机制：简单全局标准化可能掩盖不同任务尺度问题。
3. 最后补一个边界条件或失败模式：reward variance 可能来自任务混合、长度、grader 噪声或 reward scale；先分层统计，再决定 normalization，而不是盲目全局 clip。
...
reward variance 可能来自任务混合、长度、grader 噪声或 reward scale；先分层统计，再决定 normalization，而不是盲目全局 clip。
...
- 只做 z-score。
- 边界条件：reward variance 可能来自任务混合、长度、grader 噪声或 reward scale；先分层统计，再决定 normalization，而不是盲目全局 clip。
...
1. 如果不同任务 reward 量纲不同，如何做 multi-task RL？
2. 你会先加日志还是先调超参？为什么？
...
> **Q085：先按 task/difficulty/length 分桶，判断是异质性还是 judge 噪声。再考虑 per-task normalization、clipping、 calibration、group baseline 或 reward decomposition。**
...
本题不能停在“先按 task/difficulty/length 分桶，判断是异质性还是 judge 噪声。再考虑 per-task normalization、clipping、 calibration、group baseline 或 reward decomposition”。更专业的表述是先确定五个对象：
...
对 **Q085**，最关键的机制判断是：**reward variance 可能来自任务混合、长度、grader 噪声或 reward scale；先分层统计，再决定 normalization，而不是盲目全局 clip。**

再进一步，工程上真正需要落地的是：reward variance 先分解为 task variance、judge noise、length/scale、sampling variance；直接全局 normalize 可能把真实难度信息一并抹掉。
...
本题 PDF 的深入结论是：简单全局标准化可能掩盖不同任务尺度问题。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q085
...
| 7 | `reward/benchmark delta` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `KL/entropy/grad norm` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：先按 task/difficulty/length 分桶，判断是异质性还是 judge 噪声。再考虑 per-task normalization、clipping、 calibration、group baseline 或 reward decomposition |
