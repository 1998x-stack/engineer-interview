...
# Q087 · 怎么做后训练 ablation 才可信？

> **题型**：系统设计 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q086](../09-eval-debug/q086-offline-vs-online-eval.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q088 →](../09-eval-debug/q088-sft-better-rl-worse.md)
...
逐项增加变量，控制 rollout tokens、training steps/compute、seed、eval protocol 与初始 checkpoint。

### PDF 原始深入理解

例如 DAPO 应 baseline GRPO → +Clip-Higher → +Dynamic Sampling → +Token PG → +Overlong，而不是 full recipe 对 baseline 一把梭。

### PDF 原始常见失分点

只控制 step，不控制 generated tokens。
...
1. 先给核心判断：逐项增加变量，控制 rollout tokens、training steps/compute、seed、eval protocol 与初始 checkpoint。
2. 再解释机制：例如 DAPO 应 baseline GRPO → +Clip-Higher → +Dynamic Sampling → +Token PG → +Overlong，而不是 full recipe 对 baseline 一把梭。
3. 最后补一个边界条件或失败模式：可信 ablation 要控制 compute/data/token budget，只改一个变量，并报告均值、方差和训练曲线，不只报最佳 checkpoint。
...
可信 ablation 要控制 compute/data/token budget，只改一个变量，并报告均值、方差和训练曲线，不只报最佳 checkpoint。
...
- 只控制 step，不控制 generated tokens。
- 边界条件：可信 ablation 要控制 compute/data/token budget，只改一个变量，并报告均值、方差和训练曲线，不只报最佳 checkpoint。
...
1. 为什么 RL ablation 特别需要报告 variance？
2. 你会先加日志还是先调超参？为什么？
...
> **Q087：逐项增加变量，控制 rollout tokens、training steps/compute、seed、eval protocol 与初始 checkpoint。**
...
本题不能停在“逐项增加变量，控制 rollout tokens、training steps/compute、seed、eval protocol 与初始 checkpoint”。更专业的表述是先确定五个对象：
...
对 **Q087**，最关键的机制判断是：**可信 ablation 要控制 compute/data/token budget，只改一个变量，并报告均值、方差和训练曲线，不只报最佳 checkpoint。**

再进一步，工程上真正需要落地的是：ablation 必须控制 compute/data/token budget；对系统 trick 还要控制 hardware/placement。否则“更好”可能只是多花了 rollout。
...
本题 PDF 的深入结论是：例如 DAPO 应 baseline GRPO → +Clip-Higher → +Dynamic Sampling → +Token PG → +Overlong，而不是 full recipe 对 baseline 一把梭。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q087
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：逐项增加变量，控制 rollout tokens、training steps/compute、seed、eval protocol 与初始 checkpoint |
