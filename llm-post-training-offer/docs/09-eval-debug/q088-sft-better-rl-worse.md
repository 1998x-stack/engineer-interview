...
# Q088 · 为什么 SFT 变好但 RL 可能变差？

> **题型**：原理推导 · **难度**：L4 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q087](../09-eval-debug/q087-post-training-ablation.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q089 →](../09-eval-debug/q089-hard-example-data-flywheel.md)
...
SFT 可能降低 entropy、变得过度 deterministic，导致 RL exploration space 变窄；也可能过拟合一种 reasoning style。

### PDF 原始深入理解

所以最佳 SFT checkpoint 与最佳 RL init 的目标不同。应通过 base pass@k、entropy 与 exploration eval 选 择 RL 起点。
...
1. 先给核心判断：SFT 可能降低 entropy、变得过度 deterministic，导致 RL exploration space 变窄；也可能过拟合一种 reasoning style。
2. 再解释机制：所以最佳 SFT checkpoint 与最佳 RL init 的目标不同。应通过 base pass@k、entropy 与 exploration eval 选 择 RL 起点。
3. 最后补一个边界条件或失败模式：SFT 可能提高 greedy accuracy 却降低 entropy / exploration，使 RL 更难找到新轨迹；所以 RL initialization 要单独用 rollout statistics 评估。
...
SFT 可能提高 greedy accuracy 却降低 entropy / exploration，使 RL 更难找到新轨迹；所以 RL initialization 要单独用 rollout statistics 评估。
...
- 认为先把 SFT 做到最强一定更好。
- 边界条件：SFT 可能提高 greedy accuracy 却降低 entropy / exploration，使 RL 更难找到新轨迹；所以 RL initialization 要单独用 rollout statistics 评估。
...
> **Q088：SFT 可能降低 entropy、变得过度 deterministic，导致 RL exploration space 变窄；也可能过拟合一种 reasoning style。**
...
本题不能停在“SFT 可能降低 entropy、变得过度 deterministic，导致 RL exploration space 变窄；也可能过拟合一种 reasoning style”。更专业的表述是先确定五个对象：
...
对 **Q088**，最关键的机制判断是：**SFT 可能提高 greedy accuracy 却降低 entropy / exploration，使 RL 更难找到新轨迹；所以 RL initialization 要单独用 rollout statistics 评估。**

再进一步，工程上真正需要落地的是：SFT 变好但 RL 变差时，检查 entropy、pass@k、sampling diversity 与初始化 KL；SFT 可能把 policy 锁进一个高似然但探索不足的局部区域。
...
本题 PDF 的深入结论是：所以最佳 SFT checkpoint 与最佳 RL init 的目标不同。应通过 base pass@k、entropy 与 exploration eval 选 择 RL 起点。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q088
...
| 2 | `reward/benchmark delta` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 3 | `KL/entropy/grad norm` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `metric slices` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `seed variance` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `regression failures` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `time-to-reproduce` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：SFT 可能降低 entropy、变得过度 deterministic，导致 RL exploration space 变窄；也可能过拟合一种 reasoning style |
