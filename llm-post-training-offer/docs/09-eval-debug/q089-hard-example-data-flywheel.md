---
id: Q089
title: "如何构建 hard-example data flywheel？"
chapter: 9
...
# Q089 · 如何构建 hard-example data flywheel？

> **题型**：系统设计 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q088](../09-eval-debug/q088-sft-better-rl-worse.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q090 →](../09-eval-debug/q090-data-vs-algorithm-vs-system.md)
...
Model → Eval → Failure mining → 聚类/归因 → 生成/标注/验证 → SFT/Preference/RL → Regression → 新模 型。

### PDF 原始深入理解

关键是根据当前 frontier 动态增加边际训练信号，而不是随机扩数据。要记录 failure taxonomy 与每轮修复 率。

### PDF 原始常见失分点

只做“错题加入训练集”，导致 contamination 和过拟合。
...
1. 先给核心判断：Model → Eval → Failure mining → 聚类/归因 → 生成/标注/验证 → SFT/Preference/RL → Regression → 新模 型。
2. 再解释机制：关键是根据当前 frontier 动态增加边际训练信号，而不是随机扩数据。要记录 failure taxonomy 与每轮修复 率。
3. 最后补一个边界条件或失败模式：data flywheel 的核心是 failure-conditioned data acquisition：用当前模型的错误边界决定下一轮数据，而不是持续随机扩容。
...
data flywheel 的核心是 failure-conditioned data acquisition：用当前模型的错误边界决定下一轮数据，而不是持续随机扩容。
...
```text
model -> eval -> failure clustering -> hard-case acquisition
      -> verify/label -> curriculum/mixture -> train
      -> regression suite -> model'
      -> repeat at the new frontier
```
...
- 只做“错题加入训练集”，导致 contamination 和过拟合。
- 边界条件：data flywheel 的核心是 failure-conditioned data acquisition：用当前模型的错误边界决定下一轮数据，而不是持续随机扩容。
...
> **Q089：Model → Eval → Failure mining → 聚类/归因 → 生成/标注/验证 → SFT/Preference/RL → Regression → 新模 型。**
...
- [Q003 · 公开真题：SFT 数据如何筛选和采样？](../01-sft-data/q003-sft-data-filter-sampling.md)
- [Q004 · SFT 数据是越多越好吗？如何理解 data quality × diversity × difficulty？](../01-sft-data/q004-sft-data-quality-diversity-difficulty.md)
- [Q006 · 多领域 SFT 数据应该怎么配比？](../01-sft-data/q006-sft-domain-mixture.md)
- [Q007 · 为什么 instruction data 需要 prompt diversity，而不只是 response diversity？](../01-sft-data/q007-prompt-diversity.md)
- [Q008 · 公开真题：CoT 数据怎么构建、筛选与验证？](../01-sft-data/q008-cot-data-build-filter-verify.md)
...
本题不能停在“Model → Eval → Failure mining → 聚类/归因 → 生成/标注/验证 → SFT/Preference/RL → Regression → 新模 型”。更专业的表述是先确定五个对象：
...
对 **Q089**，最关键的机制判断是：**data flywheel 的核心是 failure-conditioned data acquisition：用当前模型的错误边界决定下一轮数据，而不是持续随机扩容。**

再进一步，工程上真正需要落地的是：hard-example flywheel 要防“只追当前模型盲点”导致分布越来越窄；保留稳定 anchor set，并监控新数据对旧能力的回归。
...
本题 PDF 的深入结论是：关键是根据当前 frontier 动态增加边际训练信号，而不是随机扩数据。要记录 failure taxonomy 与每轮修复 率。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
| # | 指标 | 如何解释 |
|---:|---|---|
| 1 | `data keep-rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 2 | `contamination rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 3 | `reward/benchmark delta` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `KL/entropy/grad norm` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `metric slices` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `seed variance` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `regression failures` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `time-to-reproduce` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：Model → Eval → Failure mining → 聚类/归因 → 生成/标注/验证 → SFT/Preference/RL → Regression → 新模 型 |
