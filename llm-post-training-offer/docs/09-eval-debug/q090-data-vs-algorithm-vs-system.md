...
# Q090 · 如何判断问题来自数据、算法还是系统实现？

> **题型**：系统设计 · **难度**：L4 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q089](../09-eval-debug/q089-hard-example-data-flywheel.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q091 →](../10-agentic-rl/q091-agentic-rl-state-space.md)
...
训练集也学不上：先查实现/优化；train 好 eval 差：查数据泛化；reward ↑ task ↓：查 objective；单卡正确 多卡异常：查 distributed；同步好异步坏：查 staleness。

### PDF 原始深入理解

建立最小可复现实验和 invariants，比盲调超参有效。
...
1. 先给核心判断：训练集也学不上：先查实现/优化；train 好 eval 差：查数据泛化；reward ↑ task ↓：查 objective；单卡正确 多卡异常：查 distributed；同步好异步坏：查 staleness。
2. 再解释机制：建立最小可复现实验和 invariants，比盲调超参有效。
3. 最后补一个边界条件或失败模式：归因要沿数据流做切片：单卡/多卡、固定 batch/在线采样、真 reward/假 reward、旧 checkpoint/新 checkpoint；用最小可复现实验隔离层级。
...
归因要沿数据流做切片：单卡/多卡、固定 batch/在线采样、真 reward/假 reward、旧 checkpoint/新 checkpoint；用最小可复现实验隔离层级。
...
- 一次同时改多个变量。
- 边界条件：归因要沿数据流做切片：单卡/多卡、固定 batch/在线采样、真 reward/假 reward、旧 checkpoint/新 checkpoint；用最小可复现实验隔离层级。
...
1. 请设计一个 GRPO loss 数值单测。 举一反三 / 章节检查 尝试不看答案，用“任务 → 数据 → reward → exploration → credit → compute → failure → eval”重新讲本章。若能把 3 个以上问题串成一条因果链，而不是逐题背诵，本章才算掌 握。 第十章 Agentic RL 与项目系统设计：三面真正拉开差距 本章目标 完成面试题 91-100。要求不是记忆名词，而是把每个方法与其输入、目标函数、failure mode 和工程代价连接起来。
2. 你会先加日志还是先调超参？为什么？
...
> **Q090：训练集也学不上：先查实现/优化；train 好 eval 差：查数据泛化；reward ↑ task ↓：查 objective；单卡正确 多卡异常：查 distributed；同步好异步坏：查 staleness。**
...
本题不能停在“训练集也学不上：先查实现/优化；train 好 eval 差：查数据泛化；reward ↑ task ↓：查 objective；单卡正确 多卡异常：查 distributed；同步好异步坏：查 staleness”。更专业的表述是先确定五个对象：
...
对 **Q090**，最关键的机制判断是：**归因要沿数据流做切片：单卡/多卡、固定 batch/在线采样、真 reward/假 reward、旧 checkpoint/新 checkpoint；用最小可复现实验隔离层级。**

再进一步，工程上真正需要落地的是：用最小复现矩阵定位层级：常量 reward、固定数据、单卡/多卡、同步/异步、冻结 policy。最早导致分叉的层通常就是根因所在。
...
本题 PDF 的深入结论是：建立最小可复现实验和 invariants，比盲调超参有效。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q090
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：训练集也学不上：先查实现/优化；train 好 eval 差：查数据泛化；reward ↑ task ↓：查 objective；单卡正确 多卡异常：查 distributed；同步好异步坏：查 staleness |
