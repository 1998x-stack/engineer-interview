---
id: Q091
title: "公开真题：什么是 Agentic RL？与单轮 reasoning RL 的状态空间有何不同？"
chapter: 10
chapter_title: "Agentic RL 与系统设计"
type: "公开真题"
level: L3
priority: normal
tags: [reward, agentic-rl]
source_refs: [I1, I7, P8, R1]
---

# Q091 · 公开真题：什么是 Agentic RL？与单轮 reasoning RL 的状态空间有何不同？

> **题型**：公开真题 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q090](../09-eval-debug/q090-data-vs-algorithm-vs-system.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q092 →](../10-agentic-rl/q092-agentic-credit-assignment.md)
...
Agent trajectory 是 state → action/tool → observation → next action 的环境交互，不只是一次 x→y。

### PDF 原始深入理解

状态包含对话、工具输出、环境状态和历史动作；horizon 更长、reward 更迟、执行可能有副作用，因此 credit、成本与安全都更复杂。

### PDF 原始常见失分点

把 Agentic RL 说成“对 function calling 做 GRPO”。
...
1. 先给核心判断：Agent trajectory 是 state → action/tool → observation → next action 的环境交互，不只是一次 x→y。
2. 再解释机制：状态包含对话、工具输出、环境状态和历史动作；horizon 更长、reward 更迟、执行可能有副作用，因此 credit、成本与安全都更复杂。
3. 最后补一个边界条件或失败模式：Agentic RL 的状态包含历史对话、工具 observation、环境状态与预算；action 既可能是 token，也可能是工具调用/高层决策。
...
Agentic RL 的状态包含历史对话、工具 observation、环境状态与预算；action 既可能是 token，也可能是工具调用/高层决策。
...
```text
Problem: 公开真题：什么是 Agentic RL？与单轮 reasoning RL 的状态空间有何不同？
...
- 把 Agentic RL 说成“对 function calling 做 GRPO”。
- 边界条件：Agentic RL 的状态包含历史对话、工具 observation、环境状态与预算；action 既可能是 token，也可能是工具调用/高层决策。
...
1. multi-turn rollout 的终止条件如何定义？ [I1][I7]
...
> **Q091：Agent trajectory 是 state → action/tool → observation → next action 的环境交互，不只是一次 x→y。**
...
本题不能停在“Agent trajectory 是 state → action/tool → observation → next action 的环境交互，不只是一次 x→y”。更专业的表述是先确定五个对象：
...
对 **Q091**，最关键的机制判断是：**Agentic RL 的状态包含历史对话、工具 observation、环境状态与预算；action 既可能是 token，也可能是工具调用/高层决策。**

再进一步，工程上真正需要落地的是：Agentic RL 需要明确 state 里包含哪些 observation、tool result、history 和 hidden environment state；state 定义不清，credit 与 Markov 假设都无从讨论。
...
本题 PDF 的深入结论是：状态包含对话、工具输出、环境状态和历史动作；horizon 更长、reward 更迟、执行可能有副作用，因此 credit、成本与安全都更复杂。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q091
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：Agent trajectory 是 state → action/tool → observation → next action 的环境交互，不只是一次 x→y |
