---
id: Q092
title: "公开真题：Agentic RL 的 credit assignment 怎么做？"
chapter: 10
chapter_title: "Agentic RL 与系统设计"
type: "公开真题"
level: L4
priority: normal
tags: [reward, verifier, agentic-rl]
source_refs: [I7, I8, P8, R1]
---

# Q092 · 公开真题：Agentic RL 的 credit assignment 怎么做？

> **题型**：公开真题 · **难度**：L4 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q091](../10-agentic-rl/q091-agentic-rl-state-space.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q093 →](../10-agentic-rl/q093-tool-calling-data.md)
...
最粗是 terminal reward broadcast；更细可用 step reward、tool validity、process verifier、value/advantage decomposition、hierarchical credit。

### PDF 原始深入理解

关键是避免把最终成功错误归因给所有 token。对工具任务可把 action span 作为更自然的 credit unit。

### PDF 原始常见失分点

把每一步都手工打 dense reward。
...
1. 先给核心判断：最粗是 terminal reward broadcast；更细可用 step reward、tool validity、process verifier、value/advantage decomposition、hierarchical credit。
2. 再解释机制：关键是避免把最终成功错误归因给所有 token。对工具任务可把 action span 作为更自然的 credit unit。
3. 最后补一个边界条件或失败模式：长程 credit 的难点是终局 reward 与早期 action 相隔很远；可用 step reward、hierarchical value、trajectory decomposition，但都要防错误归因。
...
长程 credit 的难点是终局 reward 与早期 action 相隔很远；可用 step reward、hierarchical value、trajectory decomposition，但都要防错误归因。
...
```text
Problem: 公开真题：Agentic RL 的 credit assignment 怎么做？
...
- 把每一步都手工打 dense reward。
- 边界条件：长程 credit 的难点是终局 reward 与早期 action 相隔很远；可用 step reward、hierarchical value、trajectory decomposition，但都要防错误归因。
...
1. action-level advantage 如何回传到 tool-call token？ [I7][I8]
...
> **Q092：最粗是 terminal reward broadcast；更细可用 step reward、tool validity、process verifier、value/advantage decomposition、hierarchical credit。**

## 13. 参考资料

- **[I7] 公开面经**：[牛客：阿里国际 AI 应用算法（SFT / GRPO / reward）](https://www.nowcoder.com/feed/main/detail/0427c1fcd66640c295d8b9f867f3497e)
- **[I8] 公开面经**：[牛客：字节大模型算法岗（rollout / MoE / GSPO）](https://www.nowcoder.com/discuss/922308546966847488)
...
本题不能停在“最粗是 terminal reward broadcast；更细可用 step reward、tool validity、process verifier、value/advantage decomposition、hierarchical credit”。更专业的表述是先确定五个对象：
...
对 **Q092**，最关键的机制判断是：**长程 credit 的难点是终局 reward 与早期 action 相隔很远；可用 step reward、hierarchical value、trajectory decomposition，但都要防错误归因。**

再进一步，工程上真正需要落地的是：长程 credit 可从终局 return、step reward、value、hierarchical option 等多层处理；先确认 reward attribution 是否可信，再追求更“dense”。
...
本题 PDF 的深入结论是：关键是避免把最终成功错误归因给所有 token。对工具任务可把 action span 作为更自然的 credit unit。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q092
...
| 3 | `tool success/error rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `task success` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `steps/trajectory` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `tool calls` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `invalid-action rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `retry rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：最粗是 terminal reward broadcast；更细可用 step reward、tool validity、process verifier、value/advantage decomposition、hierarchical credit |
