...
# Q094 · Agent reward 应如何设计？

> **题型**：系统设计 · **难度**：L4 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q093](../10-agentic-rl/q093-tool-calling-data.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q095 →](../10-agentic-rl/q095-agent-tool-loop-hacking.md)
...
任务成功做 primary；invalid action/safety 可做 hard penalty/constraint；调用成本、步骤数、延迟做 secondary objective。

### PDF 原始深入理解

避免任意线性加权把主目标稀释。对于真实环境，reward 还要考虑可重复性和执行副作用。
...
1. 先给核心判断：任务成功做 primary；invalid action/safety 可做 hard penalty/constraint；调用成本、步骤数、延迟做 secondary objective。
2. 再解释机制：避免任意线性加权把主目标稀释。对于真实环境，reward 还要考虑可重复性和执行副作用。
3. 最后补一个边界条件或失败模式：Agent reward 最好有 primary success + hard constraints + secondary efficiency，而不是把安全、成本、成功率全部无脑线性相加。
...
Agent reward 最好有 primary success + hard constraints + secondary efficiency，而不是把安全、成本、成功率全部无脑线性相加。
...
\[R=R_{success}-\lambda_1\,\text{tool\_cost}-\lambda_2\,\text{latency}\] 配合 invalid-action / safety hard constraints。
...
- 每调用一次工具都给正奖励。
- 边界条件：Agent reward 最好有 primary success + hard constraints + secondary efficiency，而不是把安全、成本、成功率全部无脑线性相加。
...
> **Q094：任务成功做 primary；invalid action/safety 可做 hard penalty/constraint；调用成本、步骤数、延迟做 secondary objective。**
...
本题不能停在“任务成功做 primary；invalid action/safety 可做 hard penalty/constraint；调用成本、步骤数、延迟做 secondary objective”。更专业的表述是先确定五个对象：
...
对 **Q094**，最关键的机制判断是：**Agent reward 最好有 primary success + hard constraints + secondary efficiency，而不是把安全、成本、成功率全部无脑线性相加。**

再进一步，工程上真正需要落地的是：agent reward 最好采用 success 主目标 + cost/latency 次目标 + invalid/safety hard constraint；任意线性加权会产生“可用成功分抵消非法动作”的漏洞。
...
本题 PDF 的深入结论是：避免任意线性加权把主目标稀释。对于真实环境，reward 还要考虑可重复性和执行副作用。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q094
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：任务成功做 primary；invalid action/safety 可做 hard penalty/constraint；调用成本、步骤数、延迟做 secondary objective |
