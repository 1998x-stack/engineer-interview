---
id: Q095
title: "如何防止 Agent 为了 reward 无限调用工具或重复搜索？"
chapter: 10
...
# Q095 · 如何防止 Agent 为了 reward 无限调用工具或重复搜索？

> **题型**：系统设计 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q094](../10-agentic-rl/q094-agent-reward-design.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q096 →](../10-agentic-rl/q096-multi-turn-vs-single-turn-rl.md)
...
设置 task-success 主奖励、per-call cost、step budget、duplicate-action penalty、loop detection、 invalid-call 约束。

### PDF 原始深入理解

但不能只靠惩罚，否则模型会学“不调用”。应确保不调用时无法获得任务 success reward。
...
1. 先给核心判断：设置 task-success 主奖励、per-call cost、step budget、duplicate-action penalty、loop detection、 invalid-call 约束。
2. 再解释机制：但不能只靠惩罚，否则模型会学“不调用”。应确保不调用时无法获得任务 success reward。
3. 最后补一个边界条件或失败模式：工具循环是典型 reward loophole。除了 step cost，还需要 max steps、重复检测、invalid-action gate、状态去重与终止条件。
...
工具循环是典型 reward loophole。除了 step cost，还需要 max steps、重复检测、invalid-action gate、状态去重与终止条件。
...
\[R'=R_{task}-\lambda N_{calls}\] 只是第一层；还需要状态/动作级约束避免模型用重复调用“刷 reward”。
...
```text
Problem: 如何防止 Agent 为了 reward 无限调用工具或重复搜索？
...
- 把 max_steps 当唯一方案。
- 边界条件：工具循环是典型 reward loophole。除了 step cost，还需要 max steps、重复检测、invalid-action gate、状态去重与终止条件。
...
> **Q095：设置 task-success 主奖励、per-call cost、step budget、duplicate-action penalty、loop detection、 invalid-call 约束。**
...
本题不能停在“设置 task-success 主奖励、per-call cost、step budget、duplicate-action penalty、loop detection、 invalid-call 约束”。更专业的表述是先确定五个对象：
...
对 **Q095**，最关键的机制判断是：**工具循环是典型 reward loophole。除了 step cost，还需要 max steps、重复检测、invalid-action gate、状态去重与终止条件。**

再进一步，工程上真正需要落地的是：重复调用、无意义搜索、刷中间分都是 tool-loop hacking；除了每次调用 cost，还需要去重、预算、状态机约束与终止条件。
...
本题 PDF 的深入结论是：但不能只靠惩罚，否则模型会学“不调用”。应确保不调用时无法获得任务 success reward。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q095
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：设置 task-success 主奖励、per-call cost、step budget、duplicate-action penalty、loop detection、 invalid-call 约束 |
