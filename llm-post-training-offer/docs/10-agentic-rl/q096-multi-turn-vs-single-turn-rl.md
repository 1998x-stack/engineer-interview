---
id: Q096
title: "Multi-turn RL 与 single-turn RL 最大区别是什么？"
chapter: 10
...
# Q096 · Multi-turn RL 与 single-turn RL 最大区别是什么？

> **题型**：原理推导 · **难度**：L3 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q095](../10-agentic-rl/q095-agent-tool-loop-hacking.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q097 →](../10-agentic-rl/q097-long-horizon-grpo-vs-ppo.md)
...
multi-turn horizon 长、environment non-stationary、trajectory variable-length、credit 更稀疏，且 rollout 与工具延迟显著增加。

### PDF 原始深入理解

系统还要保存/恢复环境 state，并处理 tool timeout、partial observation 与并行 episode scheduling。
...
1. 先给核心判断：multi-turn horizon 长、environment non-stationary、trajectory variable-length、credit 更稀疏，且 rollout 与工具延迟显著增加。
2. 再解释机制：系统还要保存/恢复环境 state，并处理 tool timeout、partial observation 与并行 episode scheduling。
3. 最后补一个边界条件或失败模式：multi-turn 把 horizon、环境随机性和 state partial observability 都放大；因此 credit、staleness、replay 和 rollout 成本同时更难。
...
multi-turn 把 horizon、环境随机性和 state partial observability 都放大；因此 credit、staleness、replay 和 rollout 成本同时更难。
...
```text
Problem: Multi-turn RL 与 single-turn RL 最大区别是什么？
...
- 只说上下文更长。
- 边界条件：multi-turn 把 horizon、环境随机性和 state partial observability 都放大；因此 credit、staleness、replay 和 rollout 成本同时更难。
...
> **Q096：multi-turn horizon 长、environment non-stationary、trajectory variable-length、credit 更稀疏，且 rollout 与工具延迟显著增加。**
...
本题不能停在“multi-turn horizon 长、environment non-stationary、trajectory variable-length、credit 更稀疏，且 rollout 与工具延迟显著增加”。更专业的表述是先确定五个对象：
...
对 **Q096**，最关键的机制判断是：**multi-turn 把 horizon、环境随机性和 state partial observability 都放大；因此 credit、staleness、replay 和 rollout 成本同时更难。**

再进一步，工程上真正需要落地的是：multi-turn 的数据单位是 trajectory，不是单 response；日志要能回放每个 environment transition，否则失败只能看到终局，无法定位哪一步偏离。
...
本题 PDF 的深入结论是：系统还要保存/恢复环境 state，并处理 tool timeout、partial observation 与并行 episode scheduling。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q096
...
| 5 | `tool success/error rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：multi-turn horizon 长、environment non-stationary、trajectory variable-length、credit 更稀疏，且 rollout 与工具延迟显著增加 |
