---
id: Q097
title: "公开真题：长程任务为什么可能选 GRPO 而不是 PPO？"
chapter: 10
chapter_title: "Agentic RL 与系统设计"
type: "公开真题"
level: L4
priority: normal
tags: [ppo, grpo, reward, verifier]
source_refs: [I12, P8, R1]
---

# Q097 · 公开真题：长程任务为什么可能选 GRPO 而不是 PPO？

> **题型**：公开真题 · **难度**：L4 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q096](../10-agentic-rl/q096-multi-turn-vs-single-turn-rl.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q098 →](../10-agentic-rl/q098-agent-process-reward.md)
...
长 horizon 的 value model 难训练且成本高；若有可靠 terminal verifier，用 group-relative outcome 可能更 简单。

### PDF 原始深入理解

但这不是绝对结论：若有高质量 dense value/process signal，critic 仍可能明显降低 variance。算法选择取决 于 reward 与 horizon，而不是流行度。
...
1. 先给核心判断：长 horizon 的 value model 难训练且成本高；若有可靠 terminal verifier，用 group-relative outcome 可能更 简单。
2. 再解释机制：但这不是绝对结论：若有高质量 dense value/process signal，critic 仍可能明显降低 variance。算法选择取决 于 reward 与 horizon，而不是流行度。
3. 最后补一个边界条件或失败模式：长程任务中 critic 可能难以稳定估计 value，且成本高；若最终 outcome 可验证，group-relative 方法是可行替代，但不是无条件优于 PPO。
...
长程任务中 critic 可能难以稳定估计 value，且成本高；若最终 outcome 可验证，group-relative 方法是可行替代，但不是无条件优于 PPO。
...
```text
Problem: 公开真题：长程任务为什么可能选 GRPO 而不是 PPO？
...
- 回答“GRPO 一定更适合长程”。
- 边界条件：长程任务中 critic 可能难以稳定估计 value，且成本高；若最终 outcome 可验证，group-relative 方法是可行替代，但不是无条件优于 PPO。
...
> **Q097：长 horizon 的 value model 难训练且成本高；若有可靠 terminal verifier，用 group-relative outcome 可能更 简单。**

## 13. 参考资料

- **[I12] 公开面经**：[牛客：百川医疗大模型后训练（2026-08）](https://www.nowcoder.com/enterprise/267815/interview)
...
本题不能停在“长 horizon 的 value model 难训练且成本高；若有可靠 terminal verifier，用 group-relative outcome 可能更 简单”。更专业的表述是先确定五个对象：
...
对 **Q097**，最关键的机制判断是：**长程任务中 critic 可能难以稳定估计 value，且成本高；若最终 outcome 可验证，group-relative 方法是可行替代，但不是无条件优于 PPO。**

再进一步，工程上真正需要落地的是：GRPO 对 critic 难学的长 horizon 有工程吸引力，但 group-level outcome credit 很粗；若有可靠 dense value/process signal，PPO/actor-critic 仍可能更有效。
...
本题 PDF 的深入结论是：但这不是绝对结论：若有高质量 dense value/process signal，critic 仍可能明显降低 variance。算法选择取决 于 reward 与 horizon，而不是流行度。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q097
...
| 5 | `task success` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `steps/trajectory` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `tool calls` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `invalid-action rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：长 horizon 的 value model 难训练且成本高；若有可靠 terminal verifier，用 group-relative outcome 可能更 简单 |
