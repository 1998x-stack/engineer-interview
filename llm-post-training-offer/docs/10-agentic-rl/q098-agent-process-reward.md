---
id: Q098
title: "如何给 Agent 过程打 reward，而不把探索路径写死？"
chapter: 10
...
# Q098 · 如何给 Agent 过程打 reward，而不把探索路径写死？

> **题型**：系统设计 · **难度**：L4 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q097](../10-agentic-rl/q097-long-horizon-grpo-vs-ppo.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q099 →](../10-agentic-rl/q099-70b-post-training-pipeline.md)
...
只对可验证关键事件给予 selective shaping：合法工具选择、关键证据获得、状态推进；最终 task success 保 持主导。

### PDF 原始深入理解

过程 reward 应奖励“进展”而非固定步骤顺序，避免把人工脚本当唯一最优路径。
...
1. 先给核心判断：只对可验证关键事件给予 selective shaping：合法工具选择、关键证据获得、状态推进；最终 task success 保 持主导。
2. 再解释机制：过程 reward 应奖励“进展”而非固定步骤顺序，避免把人工脚本当唯一最优路径。
3. 最后补一个边界条件或失败模式：process reward 应奖励“可验证进展”而非预写固定路径；例如成功获取必要 observation 可奖励，但不强迫必须先做某个具体工具序列。
...
process reward 应奖励“可验证进展”而非预写固定路径；例如成功获取必要 observation 可奖励，但不强迫必须先做某个具体工具序列。
...
```text
Problem: 如何给 Agent 过程打 reward，而不把探索路径写死？
...
- 逐步照标准答案打分。
- 边界条件：process reward 应奖励“可验证进展”而非预写固定路径；例如成功获取必要 observation 可奖励，但不强迫必须先做某个具体工具序列。
...
1. 如何定义 progress metric 而非 path imitation？
...
> **Q098：只对可验证关键事件给予 selective shaping：合法工具选择、关键证据获得、状态推进；最终 task success 保 持主导。**
...
本题不能停在“只对可验证关键事件给予 selective shaping：合法工具选择、关键证据获得、状态推进；最终 task success 保 持主导”。更专业的表述是先确定五个对象：
...
对 **Q098**，最关键的机制判断是：**process reward 应奖励“可验证进展”而非预写固定路径；例如成功获取必要 observation 可奖励，但不强迫必须先做某个具体工具序列。**

再进一步，工程上真正需要落地的是：process reward 的设计要测“是否帮助任务成功”而不是“是否符合人工偏好的步骤”；过强 shaping 会让 agent 拒绝发现更短或新颖路径。
...
本题 PDF 的深入结论是：过程 reward 应奖励“进展”而非固定步骤顺序，避免把人工脚本当唯一最优路径。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
```text
# Interview-to-engineering skeleton for Q098
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：只对可验证关键事件给予 selective shaping：合法工具选择、关键证据获得、状态推进；最终 task success 保 持主导 |
