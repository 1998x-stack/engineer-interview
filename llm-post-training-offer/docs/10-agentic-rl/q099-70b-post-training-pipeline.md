---
id: Q099
title: "系统设计题：如果让你做一个 70B reasoning model 的完整 Post-Training pipeline？"
chapter: 10
chapter_title: "Agentic RL 与系统设计"
type: "系统设计"
level: L5
priority: normal
tags: [sft, reward, agentic-rl, rlvr]
source_refs: [P8, R1]
---

# Q099 · 系统设计题：如果让你做一个 70B reasoning model 的完整 Post-Training pipeline？

> **题型**：系统设计 · **难度**：L5 · **优先级**：常规
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q098](../10-agentic-rl/q098-agent-process-reward.md) · [章节首页](README.md) · [总索引](../question-index.md) · [Q100 →](../10-agentic-rl/q100-algorithm-choice-project-defense.md)
...
Base → 高质量 instruction SFT → reasoning cold-start → RLVR（math/code/logic）→ general preference → safety → Agentic RL → failure mining → iterative post-training。

### PDF 原始深入理解

每一阶段都要有独立目标、数据、reward、release gate 与 regression set。不要把所有目标一次混进 RL，否 则很难定位冲突。
...
1. 先给核心判断：Base → 高质量 instruction SFT → reasoning cold-start → RLVR（math/code/logic）→ general preference → safety → Agentic RL → failure mining → iterative post-training。
2. 再解释机制：每一阶段都要有独立目标、数据、reward、release gate 与 regression set。不要把所有目标一次混进 RL，否 则很难定位冲突。
3. 最后补一个边界条件或失败模式：70B pipeline 的关键是阶段化：先行为冷启动，再可验证 RL，再偏好/安全，再 Agent；每阶段都有独立数据、reward、回归集和停止标准。
...
70B pipeline 的关键是阶段化：先行为冷启动，再可验证 RL，再偏好/安全，再 Agent；每阶段都有独立数据、reward、回归集和停止标准。
...
Base 70B
 ...
 -> RLVR (math/code/logic)
 -> preference & style alignment
 -> safety / policy constraints
 -> Agentic RL / tool use
 -> failure mining + regression
 -> next iteration
```
...
- 只列算法名，不讲阶段依赖和 eval。
- 边界条件：70B pipeline 的关键是阶段化：先行为冷启动，再可验证 RL，再偏好/安全，再 Agent；每阶段都有独立数据、reward、回归集和停止标准。
...
1. 每阶段哪些 checkpoint 应冻结/复用？如何避免 alignment tax？
...
> **Q099：Base → 高质量 instruction SFT → reasoning cold-start → RLVR（math/code/logic）→ general preference → safety → Agentic RL → failure mining → iterative post-training。**
...
本题不能停在“Base → 高质量 instruction SFT → reasoning cold-start → RLVR（math/code/logic）→ general preference → safety → Agentic RL → failure mining → iterative post-training”。更专业的表述是先确定五个对象：

| 维度 | 本题应明确的内容 |
|---|---|
| Optimization objective | 设计一个可回滚、可评测、分阶段放大的 70B reasoning 后训练 release pipeline |
| Statistical unit | training stage / checkpoint / data & reward gate |
| 关键估计误差 | 阶段间干扰、能力回退、reward/数据版本漂移 |
| 系统承载 | SFT + RLVR + preference/safety + Agentic RL + eval/release train |
| Scale variable | 70B sharding、长 CoT rollout、阶段 GPU budget 与回归门槛 |

对 **Q099**，最关键的机制判断是：**70B pipeline 的关键是阶段化：先行为冷启动，再可验证 RL，再偏好/安全，再 Agent；每阶段都有独立数据、reward、回归集和停止标准。**

再进一步，工程上真正需要落地的是：70B pipeline 必须把阶段目标、数据来源、模型 checkpoint、reward、GPU budget、eval gate、回滚条件写成 release train；否则只是算法列表。
...
本题 PDF 的深入结论是：每一阶段都要有独立目标、数据、reward、release gate 与 regression set。不要把所有目标一次混进 RL，否 则很难定位冲突。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
| 3 | `task success` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `steps/trajectory` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `tool calls` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `invalid-action rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `retry rate` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `latency/cost` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
本题不是孤立知识点。它应被放回本章主线：**设计一个可回滚、可评测、分阶段放大的 70B reasoning 后训练 release pipeline**。面试中如果能主动说明“当前方法解决了什么 failure，同时引入了什么新成本/新偏差”，通常比继续罗列算法名更有区分度。
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：Base → 高质量 instruction SFT → reasoning cold-start → RLVR（math/code/logic）→ general preference → safety → Agentic RL → failure mining → iterative post-training |
