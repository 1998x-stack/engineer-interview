---
id: Q100
title: "终极项目题：为什么你的项目选 GRPO/DAPO/GSPO，而不是 PPO/DPO？"
chapter: 10
chapter_title: "Agentic RL 与系统设计"
type: "公开真题"
level: L5
priority: high
tags: [ppo, dpo, grpo, dapo, gspo, reward, verifier, cot, moe]
source_refs: [I3, I7, I11, P8, R1]
---

# Q100 · 终极项目题：为什么你的项目选 GRPO/DAPO/GSPO，而不是 PPO/DPO？

> **题型**：公开真题 · **难度**：L5 · **优先级**：🔥 高频 20 题
>
> **来源说明**：下方“PDF 原始提要”严格来自《剑指 LLM 后训练 Offer》；“扩展讲义”是在此基础上结合公开论文与工程实践做的补充分析。

[← Q099](../10-agentic-rl/q099-70b-post-training-pipeline.md) · [章节首页](README.md) · [总索引](../question-index.md)
...
按“任务 → 数据 → reward → exploration → credit → compute → staleness”推导选择。例如数学 reasoning 有可靠 verifier、需要在线探索、32B critic 昂贵，所以 GRPO 合理；若出现全对/全错组、熵塌缩、长 CoT weighting，再引入 DAPO；若 MoE routing/训推 logprob mismatch 与长期稳定性突出，再考虑 GSPO。

### PDF 原始深入理解

这道题不是考算法记忆，而是考研究判断：每个算法必须对应一个具体约束或 failure mode。
...
1. 先给核心判断：按“任务 → 数据 → reward → exploration → credit → compute → staleness”推导选择。例如数学 reasoning 有可靠 verifier、需要在线探索、32B critic 昂贵，所以 GRPO 合理；若出现全对/全错组、熵塌缩、长 CoT weighting，再引入 DAPO；若 MoE routing/训推 logprob mismatch 与长期稳定性突出，再考虑 GSPO。
2. 再解释机制：这道题不是考算法记忆，而是考研究判断：每个算法必须对应一个具体约束或 failure mode。
3. 最后补一个边界条件或失败模式：项目答辩的高分结构是从约束自然推出算法，再用 ablation 证明选择必要；算法名必须对应一个明确 failure mode 或系统瓶颈。
...
项目答辩的高分结构是从约束自然推出算法，再用 ablation 证明选择必要；算法名必须对应一个明确 failure mode 或系统瓶颈。
...
Task constraints
 -> data regime
 -> reward/verifier reliability
 -> need for exploration
 -> credit assignment horizon
 -> model size / GPU budget
 -> rollout staleness / MoE constraints
 -> choose baseline
 -> observe failure mode
 -> upgrade algorithm only when failure justifies it
```
...
- “因为论文效果更好/更新”。
- 边界条件：项目答辩的高分结构是从约束自然推出算法，再用 ablation 证明选择必要；算法名必须对应一个明确 failure mode 或系统瓶颈。
...
1. 请给出你项目的三项关键 ablation，证明算法选择是必要的。 [I3][I7][I11]
2. 如果 GRPO 与 DPO 最终 benchmark 接近，你如何用成本/风险决定？
3. 你能给出一次“原先判断错了，实验推翻假设”的项目案例吗？
4. 如果工具返回错误/超时，策略应如何恢复？
5. 如何给长程任务做 credit 而不把路径写死？
6. Agent 成功率与工具成本冲突时，什么是 primary objective？
...
> **Q100：按“任务 → 数据 → reward → exploration → credit → compute → staleness”推导选择。例如数学 reasoning 有可靠 verifier、需要在线探索、32B critic 昂贵，所以 GRPO 合理；若出现全对/全错组、熵塌缩、长 CoT weighting，再引入 DAPO；若 MoE routing/训推 logprob mismatch 与长期稳定性突出，再考虑 GSPO。**

## 13. 参考资料

- **[I3] 公开面经**：[牛客：字节 大模型算法岗（CoT / GRPO / DAPO / GSPO）](https://www.nowcoder.com/discuss/921960727672299520)
- **[I7] 公开面经**：[牛客：阿里国际 AI 应用算法（SFT / GRPO / reward）](https://www.nowcoder.com/feed/main/detail/0427c1fcd66640c295d8b9f867f3497e)
...
- [Q093 · 公开真题：Tool Calling / Function Calling 数据怎么构造？](q093-tool-calling-data.md)
...
本题不能停在“按“任务 → 数据 → reward → exploration → credit → compute → staleness”推导选择。例如数学 reasoning 有可靠 verifier、需要在线探索、32B critic 昂贵，所以 GRPO 合理；若出现全对/全错组、熵塌缩、长 CoT weighting，再引入 DAPO；若 MoE routing/训推 logprob mismatch 与长期稳定性突出，再考虑 GSPO”。更专业的表述是先确定五个对象：

| 维度 | 本题应明确的内容 |
|---|---|
| Optimization objective | 在任务、reward、探索、credit 与系统预算约束下做可辩护的算法选择 |
| Statistical unit | decision hypothesis / baseline / failure / controlled ablation |
| 关键估计误差 | 新算法偏好、实验混杂、proxy 与系统成本遗漏 |
| 系统承载 | 端到端 post-training pipeline + experiment/evaluation stack |
| Scale variable | quality-per-compute、稳定性、policy freshness、维护复杂度 |

对 **Q100**，最关键的机制判断是：**项目答辩的高分结构是从约束自然推出算法，再用 ablation 证明选择必要；算法名必须对应一个明确 failure mode 或系统瓶颈。**

再进一步，工程上真正需要落地的是：项目答辩用决策树：任务→可用数据→reward可靠性→探索价值→credit horizon→系统预算→baseline→failure→升级。最重要的是说出“为什么没选其它方法”。
...
本题 PDF 的深入结论是：这道题不是考算法记忆，而是考研究判断：每个算法必须对应一个具体约束或 failure mode。把它变成研究问题时，需要追问：**哪一个可观测量能够证伪这句话？** 如果无法设计证伪实验，说明理解仍停留在概念层。
...
| 3 | `expert load balance` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 4 | `routing mismatch` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 5 | `chosen-rejected margin` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 6 | `task success` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 7 | `steps/trajectory` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
| 8 | `tool calls` | 用来验证本题对应机制是否按预期工作；必须与质量指标联合解释 |
...
本题不是孤立知识点。它应被放回本章主线：**在任务、reward、探索、credit 与系统预算约束下做可辩护的算法选择**。面试中如果能主动说明“当前方法解决了什么 failure，同时引入了什么新成本/新偏差”，通常比继续罗列算法名更有区分度。
...
| 档位 | 面试表现 |
|---|---|
| 60–70 | 能复述核心结论：按“任务 → 数据 → reward → exploration → credit → compute → staleness”推导选择。例如数学 reasoning 有可靠 verifier、需要在线探索、32B critic 昂贵，所以 GRPO 合理；若出现全对/全错组、熵塌缩、长 CoT weighting，再引入 DAPO；若 MoE routing/训推 logprob mismatch 与长期稳定性突出，再考虑 GSPO |
