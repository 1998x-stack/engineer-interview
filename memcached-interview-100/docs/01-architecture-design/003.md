...
面试官通常不会满足于“术语定义”，而会沿着 `Memcached 为什么快？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> RAM + Hash O(1) + 简单协议 + Slab + 轻量并发 + 不做持久化/事务/复杂查询，共同缩短热路径。
...
2. **把关键路径画出来：** 对 `Memcached 为什么快？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 估算网络上限：QPS × (request bytes + response bytes) × 8，对比 NIC 安全带宽，再与单节点压测 QPS 取更紧约束。
4. **准备一个故障反例：** 小 value/高 QPS 常受 PPS、syscall、锁和 CPU 限制；大 value/中 QPS 则更容易先撞到 NIC 吞吐，因此“Memcached 每秒能多少 QPS”没有脱离 workload 的单值答案。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 高速缓存经常受网络 RTT、锁竞争、cache miss 和对象序列化限制，而非 RAM 本身。
...
### 追问 1：为什么只回答“因为内存快”不够？

**回答提示：** 先复述边界，再用本题核心机制回答：RAM + Hash O(1) + 简单协议 + Slab + 轻量并发 + 不做持久化/事务/复杂查询，共同缩短热路径。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：性能瓶颈可能从 CPU 转移到哪里？
...
> **第一句给结论：** RAM + Hash O(1) + 简单协议 + Slab + 轻量并发 + 不做持久化/事务/复杂查询，共同缩短热路径。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 内存访问不等于零成本：链式指针追踪会产生 CPU cache miss；热点 key 会制造锁和 NIC 瓶颈；大 value 会转为带宽瓶颈。  
> **第四层落生产：** 给出一个指标或算式，例如：估算网络上限：QPS × (request bytes + response bytes) × 8，对比 NIC 安全带宽，再与单节点压测 QPS 取更紧约束。

## 专家级展开（V2）

### A. 从第一性原理推导

“快”来自整条热路径做减法：没有持久化提交、没有复杂查询优化、Hash 平均常数寻址、Slab 复用内存、事件驱动连接复用，以及尽量缩小共享锁域。
...
```bash
git grep -n "\bdrive_machine\b" -- memcached.c assoc.c items.c
git grep -n "\bassoc_find\b" -- memcached.c assoc.c items.c
git grep -n "\bslabs_alloc\b" -- memcached.c assoc.c items.c
git grep -n "\bitem_lock\b" -- memcached.c assoc.c items.c
git grep -n "\bitem_link_q\b" -- memcached.c assoc.c items.c
```
...
内存访问不等于零成本：链式指针追踪会产生 CPU cache miss；热点 key 会制造锁和 NIC 瓶颈；大 value 会转为带宽瓶颈。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

小 value/高 QPS 常受 PPS、syscall、锁和 CPU 限制；大 value/中 QPS 则更容易先撞到 NIC 吞吐，因此“Memcached 每秒能多少 QPS”没有脱离 workload 的单值答案。
...
估算网络上限：QPS × (request bytes + response bytes) × 8，对比 NIC 安全带宽，再与单节点压测 QPS 取更紧约束。
...
[← 上一题](../01-architecture-design/002.md) | [本章目录](README.md) | [下一题 →](../01-architecture-design/004.md)
