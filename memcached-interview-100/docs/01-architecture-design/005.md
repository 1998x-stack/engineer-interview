...
**Memcached 是不是一个分布式系统？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **架构边界、KV 语义与“为什么做减法”**。

面试官通常不会满足于“术语定义”，而会沿着 `Memcached 是不是一个分布式系统？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 整体部署可以是分布式缓存，但传统 Memcached server 之间默认互不感知，客户端/代理负责节点选择。
...
2. **把关键路径画出来：** 对 `Memcached 是不是一个分布式系统？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 把控制面单独列出：membership 来源、配置发布延迟、hash ring 版本、节点摘除策略，都应该可观测。
4. **准备一个故障反例：** 如果节点列表由不同应用实例独立配置，一部分实例漏掉一个节点，就会出现相同 key 路由不一致、重复缓存和异常 MISS。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> “distributed memory object caching system”不等于“server 端自带一致性集群协议”。
...
### 追问 1：服务器 A 知道服务器 B 吗？

**回答提示：** 先复述边界，再用本题核心机制回答：整体部署可以是分布式缓存，但传统 Memcached server 之间默认互不感知，客户端/代理负责节点选择。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：加入新节点时谁决定 key 去哪里？
...
> **第一句给结论：** 整体部署可以是分布式缓存，但传统 Memcached server 之间默认互不感知，客户端/代理负责节点选择。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 不要把“多节点部署”与“分布式数据库”混为一谈；前者没有自动复制、成员共识和 durable rebalancing。  
> **第四层落生产：** 给出一个指标或算式，例如：把控制面单独列出：membership 来源、配置发布延迟、hash ring 版本、节点摘除策略，都应该可观测。

## 专家级展开（V2）

### A. 从第一性原理推导

“Memcached 是否分布式”必须区分 server implementation 与 deployment topology：多个独立 server 可以组成分布式缓存，但传统 server 之间不做共识、分片元数据交换或自动数据迁移。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `server 内部 assoc_*；分片通常属于客户端或 built-in proxy`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\bserver\b" -- memcached.c assoc.c items.c
```
...
不要把“多节点部署”与“分布式数据库”混为一谈；前者没有自动复制、成员共识和 durable rebalancing。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

如果节点列表由不同应用实例独立配置，一部分实例漏掉一个节点，就会出现相同 key 路由不一致、重复缓存和异常 MISS。
...
把控制面单独列出：membership 来源、配置发布延迟、hash ring 版本、节点摘除策略，都应该可观测。
...
[← 上一题](../01-architecture-design/004.md) | [本章目录](README.md) | [下一题 →](../01-architecture-design/006.md)
