...
**Memcached 存进去的对象到底是什么？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **架构边界、KV 语义与“为什么做减法”**。

面试官通常不会满足于“术语定义”，而会沿着 `Memcached 存进去的对象到底是什么？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 对服务端来说是 key + flags + expiration + 可选 CAS + raw bytes；业务对象由客户端序列化/反序列化。
...
2. **把关键路径画出来：** 对 `Memcached 存进去的对象到底是什么？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 同时监控 value size 分布、序列化耗时、压缩比和反序列化失败率，而不是只看 Memcached latency。
4. **准备一个故障反例：** 一次 serialization library 升级若更改压缩 flag 或 object schema，可能造成大规模 deserialize error，看起来像缓存故障，实际是客户端协议契约破坏。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 不要说 Memcached“理解 JSON”。
...
**回答提示：** 先复述边界，再用本题核心机制回答：对服务端来说是 key + flags + expiration + 可选 CAS + raw bytes；业务对象由客户端序列化/反序列化。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：压缩阈值为什么会影响 slab 分布？
...
> **第一句给结论：** 对服务端来说是 key + flags + expiration + 可选 CAS + raw bytes；业务对象由客户端序列化/反序列化。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 跨语言客户端必须统一 serialization、compression、flags 和 key 规范；否则“同一个 key”可能被不同客户端以不同格式解释。  
> **第四层落生产：** 给出一个指标或算式，例如：同时监控 value size 分布、序列化耗时、压缩比和反序列化失败率，而不是只看 Memcached latency。

## 专家级展开（V2）

### A. 从第一性原理推导

Memcached 存的是带少量元数据的 raw bytes；序列化格式属于客户端契约。Item 负责 key/value 的物理布局，但不理解 JSON 字段、对象 schema 或业务类型。
...
git grep -n "\bnkey\b" -- memcached.c assoc.c items.c
git grep -n "\bnbytes\b" -- memcached.c assoc.c items.c
git grep -n "\bclient\b" -- memcached.c assoc.c items.c
```
...
跨语言客户端必须统一 serialization、compression、flags 和 key 规范；否则“同一个 key”可能被不同客户端以不同格式解释。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

一次 serialization library 升级若更改压缩 flag 或 object schema，可能造成大规模 deserialize error，看起来像缓存故障，实际是客户端协议契约破坏。
...
同时监控 value size 分布、序列化耗时、压缩比和反序列化失败率，而不是只看 Memcached latency。
...
[← 上一题](../01-architecture-design/006.md) | [本章目录](README.md) | [下一题 →](../01-architecture-design/008.md)
