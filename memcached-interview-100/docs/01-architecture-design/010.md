---
id: 010
title: "Cache miss 后 Memcached 会自动查数据库吗？"
chapter: 1
difficulty: P0
importance: "★★★★☆"
module: "基础架构与设计哲学"
...
# 第 010 题 · Cache miss 后 Memcached 会自动查数据库吗？
...
**Cache miss 后 Memcached 会自动查数据库吗？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **架构边界、KV 语义与“为什么做减法”**。

面试官通常不会满足于“术语定义”，而会沿着 `Cache miss 后 Memcached 会自动查数据库吗？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 不会。Memcached 返回 MISS；应用执行 DB/API 查询并决定是否回填，这是典型 Cache Aside。
...
2. **把关键路径画出来：** 对 `Cache miss 后 Memcached 会自动查数据库吗？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** origin_qps ≈ request_qps × miss_rate；再乘上单次回源成本和并发放大，评估数据库是否能承受。
4. **准备一个故障反例：** 热门 key 过期后，所有调用方同时 MISS 并回源会形成 stampede；因此 MISS 处理链通常比 HIT 链更值得做容量设计。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 把“数据库回源”说成 Memcached 内部能力是典型错误。
...
### 追问 1：为什么 miss 风暴会打垮数据库？

**回答提示：** 先复述边界，再用本题核心机制回答：不会。Memcached 返回 MISS；应用执行 DB/API 查询并决定是否回填，这是典型 Cache Aside。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：回填失败怎么处理？
...
> **第一句给结论：** 不会。Memcached 返回 MISS；应用执行 DB/API 查询并决定是否回填，这是典型 Cache Aside。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 如果把回源封装在客户端 SDK，看起来像 Memcached 自动查库，但那仍是应用/中间层逻辑，而非 Memcached server 能力。  
> **第四层落生产：** 给出一个指标或算式，例如：origin_qps ≈ request_qps × miss_rate；再乘上单次回源成本和并发放大，评估数据库是否能承受。

## 专家级展开（V2）

### A. 从第一性原理推导

Memcached 不承担回源；MISS 是正常返回值。Cache-aside 的核心是把“缓存是否存在”和“事实数据如何获取”解耦，应用决定回源、负缓存、限流和重建。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `item_get/assoc_find 返回 NULL；回源不在 memcached server 源码中`
...
如果把回源封装在客户端 SDK，看起来像 Memcached 自动查库，但那仍是应用/中间层逻辑，而非 Memcached server 能力。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

热门 key 过期后，所有调用方同时 MISS 并回源会形成 stampede；因此 MISS 处理链通常比 HIT 链更值得做容量设计。
...
origin_qps ≈ request_qps × miss_rate；再乘上单次回源成本和并发放大，评估数据库是否能承受。
...
[← 上一题](../01-architecture-design/009.md) | [本章目录](README.md) | [下一题 →](../02-item-set-get-source/011.md)
