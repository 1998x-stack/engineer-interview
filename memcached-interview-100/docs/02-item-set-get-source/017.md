...
面试官通常不会满足于“术语定义”，而会沿着 `delete foo 在内部发生什么？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 先按 hash 找 item，再从 assoc 删除、从 LRU unlink，降低 cache 持有引用；refcount 归零后 item_free/slabs_free 回收 chunk。
...
### 2. 仍被 GET/response 持有的 item 可暂缓物理释放。

从“对象生命周期”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。核心不变量是 construct-before-publish 与 unlink-before-free。Item 把 Hash 索引、LRU 生命周期、Slab 物理内存和并发引用连接起来。
### 3. 最终 freelist 接收归还的 chunk。
...
2. **把关键路径画出来：** 对 `delete foo 在内部发生什么？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 统计 delete 成功、get hit/miss 和 slab free chunk 变化，建立逻辑删除到物理回收的时间关系。
4. **准备一个故障反例：** 高并发 delete+get 是验证生命周期设计的好压力场景：未来 GET 应 MISS，但已开始的 GET 不能崩溃。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> DELETE 是观察多索引不变量最清晰的反向路径。
...
### 追问 1：只删 Hash 不删 LRU 会怎样？

**回答提示：** 先复述边界，再用本题核心机制回答：先按 hash 找 item，再从 assoc 删除、从 LRU unlink，降低 cache 持有引用；refcount 归零后 item_free/slabs_free 回收 chunk。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：延迟释放如何保证安全？
...
> **第一句给结论：** 先按 hash 找 item，再从 assoc 删除、从 LRU unlink，降低 cache 持有引用；refcount 归零后 item_free/slabs_free 回收 chunk。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** DELETE 成功后已有 reader 仍可能持有旧 item 指针并完成响应；“删除后任何线程立刻看不到内存内容”不是正确语义。  
> **第四层落生产：** 给出一个指标或算式，例如：统计 delete 成功、get hit/miss 和 slab free chunk 变化，建立逻辑删除到物理回收的时间关系。

## 专家级展开（V2）

### A. 从第一性原理推导

DELETE 是 SET publication 的逆过程：找到 item 后先让它从 Hash/LRU 逻辑不可见，再通过 refcount 决定何时物理 free；删除不是简单 free(pointer)。
...
git grep -n "\bassoc_delete\b" -- proto_text.c proto_parser.c items.c thread.c assoc.c slabs.c
git grep -n "\bdo_item_unlink_q\b" -- proto_text.c proto_parser.c items.c thread.c assoc.c slabs.c
...
DELETE 成功后已有 reader 仍可能持有旧 item 指针并完成响应；“删除后任何线程立刻看不到内存内容”不是正确语义。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

高并发 delete+get 是验证生命周期设计的好压力场景：未来 GET 应 MISS，但已开始的 GET 不能崩溃。
...
统计 delete 成功、get hit/miss 和 slab free chunk 变化，建立逻辑删除到物理回收的时间关系。
...
[← 上一题](../02-item-set-get-source/016.md) | [本章目录](README.md) | [下一题 →](../02-item-set-get-source/018.md)
