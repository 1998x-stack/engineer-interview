...
面试官通常不会满足于“术语定义”，而会沿着 `get foo 的源码逻辑如何走？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> hash key → item lock → assoc_find → 检查过期/flush → 持有引用并构造响应；访问还可能产生 LRU bump/touch 行为。
...
### 2. 命中后 value 通常通过 iovec/response 发送。

从“对象生命周期”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。核心不变量是 construct-before-publish 与 unlink-before-free。Item 把 Hash 索引、LRU 生命周期、Slab 物理内存和并发引用连接起来。
### 3. 过期 item 可在访问路径上被回收。
...
2. **把关键路径画出来：** 对 `get foo 的源码逻辑如何走？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 把 hit latency 按 key 频度分桶；若 Top1 key P99 明显高于长尾 key，检查 hot-key 与锁/cache-line 竞争。
4. **准备一个故障反例：** 热点单 key 即使完全命中也可能集中到同一 item-lock hash bucket、同一节点 NIC 和同一 CPU cache line，成为 tail latency 源。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> “Hash 查找 O(1)”不等于 GET 没有并发和网络成本。
...
**回答提示：** 先复述边界，再用本题核心机制回答：hash key → item lock → assoc_find → 检查过期/flush → 持有引用并构造响应；访问还可能产生 LRU bump/touch 行为。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：高热点 key 对 item lock 有何影响？
...
> **第一句给结论：** hash key → item lock → assoc_find → 检查过期/flush → 持有引用并构造响应；访问还可能产生 LRU bump/touch 行为。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** GET 平均 O(1) 不代表无写共享状态：引用计数、活跃标记、统计信息和可能的 LRU bump 都会引入并发成本。  
> **第四层落生产：** 给出一个指标或算式，例如：把 hit latency 按 key 频度分桶；若 Top1 key P99 明显高于长尾 key，检查 hot-key 与锁/cache-line 竞争。

## 专家级展开（V2）

### A. 从第一性原理推导

GET 的主链是 hash→item lock→assoc_find→有效性检查→临时引用→响应；现代 LRU 更新并不要求每次 GET 都同步移动双链表，因此读路径比教科书 strict LRU 更轻。
...
```bash
git grep -n "\bitem_get\b" -- proto_text.c proto_parser.c items.c thread.c assoc.c slabs.c
git grep -n "\bdo_item_get\b" -- proto_text.c proto_parser.c items.c thread.c assoc.c slabs.c
git grep -n "\bassoc_find\b" -- proto_text.c proto_parser.c items.c thread.c assoc.c slabs.c
git grep -n "\bitem_is_flushed\b" -- proto_text.c proto_parser.c items.c thread.c assoc.c slabs.c
...
GET 平均 O(1) 不代表无写共享状态：引用计数、活跃标记、统计信息和可能的 LRU bump 都会引入并发成本。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

热点单 key 即使完全命中也可能集中到同一 item-lock hash bucket、同一节点 NIC 和同一 CPU cache line，成为 tail latency 源。
...
把 hit latency 按 key 频度分桶；若 Top1 key P99 明显高于长尾 key，检查 hot-key 与锁/cache-line 竞争。
...
[← 上一题](../02-item-set-get-source/017.md) | [本章目录](README.md) | [下一题 →](../02-item-set-get-source/019.md)
