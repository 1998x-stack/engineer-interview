---
id: 021
title: "Memcached 为什么不用普通 malloc/free 管理所有 Item？"
chapter: 3
difficulty: P0
importance: "★★★★★"
...
# 第 021 题 · Memcached 为什么不用普通 malloc/free 管理所有 Item？
...
**Memcached 为什么不用普通 malloc/free 管理所有 Item？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **Slab 内存池、碎片、容量错配与 page 迁移**。

面试官通常不会满足于“术语定义”，而会沿着 `Memcached 为什么不用普通 malloc/free 管理所有 Item？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 不同尺寸对象的高频分配释放会带来碎片、allocator 元数据和锁竞争；Slab 用固定尺寸 chunk 的 freelist 把常见分配降为稳定 O(1)。
...
### 1. 按大小分 class，而不是每次精确 malloc。

通用 allocator 当然也很快，但在超高频、多尺寸对象和长期运行场景下，专用内存池更容易控制碎片、锁竞争和分配延迟的波动。
### 2. 牺牲可控内部碎片换可预测性能。

从“内存池模型”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。Slab 通过 size class 把不可控的通用分配问题转换为可控的 page/chunk/freelist 管理；优化对象分配稳定性，同时接受内部碎片。
### 3. Page 可批量切成 chunk，提高局部性和管理效率。

从“内存池模型”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。Slab 通过 size class 把不可控的通用分配问题转换为可控的 page/chunk/freelist 管理；优化对象分配稳定性，同时接受内部碎片。
### 4. 内存模型

从“内存池模型”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。Slab 通过 size class 把不可控的通用分配问题转换为可控的 page/chunk/freelist 管理；优化对象分配稳定性，同时接受内部碎片。
### 5. Slab 的价值不是“malloc 一定慢”，而是把任意大小、任意时刻的动态分配问题变成有限个 size class 的 freelist 管理。这样 allocator 行为更稳定，也能统计每个尺寸区间的命中、淘汰和 page 分配。

Slab 的设计核心是 size class + page + fixed-size chunk。它通过复用 chunk 避免在请求热路径频繁进入通用 allocator，但也意味着对象尺寸分布会直接决定内存利用率。
### 6. 代价是内部碎片：实际对象小于 chunk 时剩余空间无法给另一个对象使用。Growth factor 决定了“class 数量 vs 内部浪费”的基本曲线。
...
2. **把关键路径画出来：** 对 `Memcached 为什么不用普通 malloc/free 管理所有 Item？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** internal_waste_ratio = (chunk_size - item_total_size)/chunk_size；按真实 value 分布加权估算，而不是只看平均 value。
4. **准备一个故障反例：** 对象尺寸分布从 200B 突然变到 20KB 后，原来分给小 class 的 pages 可能暂时闲置，而大 class 频繁 eviction，形成局部容量失衡。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 答案重点是可预测性和碎片控制，不是“malloc 很慢”这一句。
...
### 追问 1：Slab 是否完全不调用 malloc？

**回答提示：** 先复述边界，再用本题核心机制回答：不同尺寸对象的高频分配释放会带来碎片、allocator 元数据和锁竞争；Slab 用固定尺寸 chunk 的 freelist 把常见分配降为稳定 O(1)。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：现代 allocator 已很快，为什么专用池仍有价值？
...
> **第一句给结论：** 不同尺寸对象的高频分配释放会带来碎片、allocator 元数据和锁竞争；Slab 用固定尺寸 chunk 的 freelist 把常见分配降为稳定 O(1)。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** Slab 仍会从系统/大块内存获得 page；它不是“完全不使用 malloc”。专用池降低的是请求热路径上的任意尺寸分配和不可预测行为。  
> **第四层落生产：** 给出一个指标或算式，例如：internal_waste_ratio = (chunk_size - item_total_size)/chunk_size；按真实 value 分布加权估算，而不是只看平均 value。

## 专家级展开（V2）

### A. 从第一性原理推导

Slab 不是因为“malloc 很慢”才存在，而是把长期、多尺寸、超高频分配转换成有限 size class 的 page/chunk/freelist 管理，获得更稳定的延迟、碎片可控性和 per-class 可观测性。
...
git grep -n "\bslabs_clsid\b" -- items.c slabs.c slabs.h slabs_mover.c
git grep -n "\bslabs_alloc\b" -- items.c slabs.c slabs.h slabs_mover.c
git grep -n "\bdo_slabs_alloc\b" -- items.c slabs.c slabs.h slabs_mover.c
git grep -n "\bslabs_free\b" -- items.c slabs.c slabs.h slabs_mover.c
```
...
Slab 仍会从系统/大块内存获得 page；它不是“完全不使用 malloc”。专用池降低的是请求热路径上的任意尺寸分配和不可预测行为。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

对象尺寸分布从 200B 突然变到 20KB 后，原来分给小 class 的 pages 可能暂时闲置，而大 class 频繁 eviction，形成局部容量失衡。
...
internal_waste_ratio = (chunk_size - item_total_size)/chunk_size；按真实 value 分布加权估算，而不是只看平均 value。
...
[← 上一题](../02-item-set-get-source/020.md) | [本章目录](README.md) | [下一题 →](../03-slab-memory/022.md)
