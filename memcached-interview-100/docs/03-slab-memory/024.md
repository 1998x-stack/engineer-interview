...
**Slab 解决了什么碎片，又引入了什么碎片？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **Slab 内存池、碎片、容量错配与 page 迁移**。

面试官通常不会满足于“术语定义”，而会沿着 `Slab 解决了什么碎片，又引入了什么碎片？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 它显著减少不可预测的外部碎片，但会产生 chunk_size - item_size 的内部碎片。
...
2. **把关键路径画出来：** 对 `Slab 解决了什么碎片，又引入了什么碎片？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 把 memory amplification 拆成 metadata + class rounding + unused chunks + hash/connection overhead 四部分。
4. **准备一个故障反例：** 平均 value 看似 1KB，但若 item_total 刚越过 1KB class 边界，实际 chunk 可能显著更大，导致容量估算偏乐观。
...
### 追问 1：如何通过 stats sizes 评估？

**回答提示：** 先复述边界，再用本题核心机制回答：它显著减少不可预测的外部碎片，但会产生 chunk_size - item_size 的内部碎片。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：压缩 value 为什么可能反而改善 slab 效率？
...
> **第一句给结论：** 它显著减少不可预测的外部碎片，但会产生 chunk_size - item_size 的内部碎片。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** “没有 external fragmentation”也不能推导出内存利用率接近 100%；header、CAS、key、chunk rounding 和闲置 pages 都会贡献 overhead。  
> **第四层落生产：** 给出一个指标或算式，例如：把 memory amplification 拆成 metadata + class rounding + unused chunks + hash/connection overhead 四部分。

## 专家级展开（V2）

### A. 从第一性原理推导

Slab 主要抑制通用 allocator 的外部碎片和随机分配波动，但引入 class 内部碎片：chunk 剩余空间不能借给另一个对象；另外还可能出现 class 之间的容量错配。
...
“没有 external fragmentation”也不能推导出内存利用率接近 100%；header、CAS、key、chunk rounding 和闲置 pages 都会贡献 overhead。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

平均 value 看似 1KB，但若 item_total 刚越过 1KB class 边界，实际 chunk 可能显著更大，导致容量估算偏乐观。
...
把 memory amplification 拆成 metadata + class rounding + unused chunks + hash/connection overhead 四部分。
...
[← 上一题](../03-slab-memory/023.md) | [本章目录](README.md) | [下一题 →](../03-slab-memory/025.md)
