---
id: 025
title: "如何根据 Item 大小选择 Slab Class？"
chapter: 3
difficulty: P1
importance: "★★★☆☆"
...
# 第 025 题 · 如何根据 Item 大小选择 Slab Class？
...
**如何根据 Item 大小选择 Slab Class？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **Slab 内存池、碎片、容量错配与 page 迁移**。

面试官通常不会满足于“术语定义”，而会沿着 `如何根据 Item 大小选择 Slab Class？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 先算完整 ntotal，包括 item header、CAS、key、flags metadata、value、CRLF 等，再找能容纳它的最小 class。
...
2. **把关键路径画出来：** 对 `如何根据 Item 大小选择 Slab Class？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 批量生成不同 key length/value length 的 item，记录 stats slabs 的 set_cmds/used_chunks，找 class 边界。
4. **准备一个故障反例：** key 命名规范从短数字 ID 改成长 URL 后，value 没变但 slab class 分布和内存使用突然变化。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 面试中若直接用 value 长度选 class，说明没有理解 item 布局。
...
### 追问 1：250B value 最终一定进 256B chunk 吗？

**回答提示：** 先复述边界，再用本题核心机制回答：先算完整 ntotal，包括 item header、CAS、key、flags metadata、value、CRLF 等，再找能容纳它的最小 class。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：如何设计对象序列化避免跨 class 边界？
...
> **第一句给结论：** 先算完整 ntotal，包括 item header、CAS、key、flags metadata、value、CRLF 等，再找能容纳它的最小 class。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 同样 value 长度但 key 更长、CAS/flags 布局不同，可能导致跨 class；容量规划不能只按 value 平均值。  
> **第四层落生产：** 给出一个指标或算式，例如：批量生成不同 key length/value length 的 item，记录 stats slabs 的 set_cmds/used_chunks，找 class 边界。

## 专家级展开（V2）

### A. 从第一性原理推导

选择 class 必须用 item 的总物理尺寸，不是只看 value：header、key、flags/CAS、CRLF/布局和可能的 chunk metadata 都会进入 ntotal，然后 slabs_clsid 找最小可容纳 class。
...
```bash
git grep -n "\bitem_make_header\b" -- items.c slabs.c slabs.h slabs_mover.c
git grep -n "\bITEM_ntotal\b" -- items.c slabs.c slabs.h slabs_mover.c
git grep -n "\bslabs_clsid\b" -- items.c slabs.c slabs.h slabs_mover.c
git grep -n "\bITEM_key\b" -- items.c slabs.c slabs.h slabs_mover.c
git grep -n "\bITEM_data\b" -- items.c slabs.c slabs.h slabs_mover.c
```
...
同样 value 长度但 key 更长、CAS/flags 布局不同，可能导致跨 class；容量规划不能只按 value 平均值。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

key 命名规范从短数字 ID 改成长 URL 后，value 没变但 slab class 分布和内存使用突然变化。
...
批量生成不同 key length/value length 的 item，记录 stats slabs 的 set_cmds/used_chunks，找 class 边界。
...
- [026. 为什么明明还有内存，一个 SET 仍可能发生 eviction？](../03-slab-memory/026.md)
...
[← 上一题](../03-slab-memory/024.md) | [本章目录](README.md) | [下一题 →](../03-slab-memory/026.md)
