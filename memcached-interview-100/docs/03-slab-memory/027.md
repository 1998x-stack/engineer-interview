---
id: 027
title: "Slab Reassign / Automove 是什么？"
chapter: 3
difficulty: P1
importance: "★★★☆☆"
...
# 第 027 题 · Slab Reassign / Automove 是什么？
...
**Slab Reassign / Automove 是什么？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **Slab 内存池、碎片、容量错配与 page 迁移**。

面试官通常不会满足于“术语定义”，而会沿着 `Slab Reassign / Automove 是什么？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 后台把 page 从富余 class 迁到紧张 class，缓解工作负载对象尺寸变化后的内存错配。
...
### 1. 迁移 page 时需处理其中仍有效/被引用的 item。

从“内存池模型”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。Slab 通过 size class 把不可控的通用分配问题转换为可控的 page/chunk/freelist 管理；优化对象分配稳定性，同时接受内部碎片。
### 2. automove 根据 eviction、age、free ratio 等信号做决策。

Eviction 是容量压力触发的主动牺牲，与业务 TTL 到期不同。分析 eviction 时必须落到具体 slab class，否则全局数字可能掩盖尺寸分布不均。
### 3. 它减少过去必须重启才能重新分布内存的场景。
...
2. **把关键路径画出来：** 对 `Slab Reassign / Automove 是什么？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 在不同对象尺寸 workload 切换前后记录 per-class pages，并观察 automove 收敛时间和 eviction 改善。
4. **准备一个故障反例：** 错误地频繁 reassign 可能导致维护线程和请求竞争；正确策略应看持续 eviction/空闲，而不是瞬时尖峰。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> Slab reassignment 是内存资源再平衡，不是 key shard migration。
...
**回答提示：** 先复述边界，再用本题核心机制回答：后台把 page 从富余 class 迁到紧张 class，缓解工作负载对象尺寸变化后的内存错配。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：busy item 如何处理？
...
> **第一句给结论：** 后台把 page 从富余 class 迁到紧张 class，缓解工作负载对象尺寸变化后的内存错配。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 移动 page 不是把任意活跃 item 无成本 memcpy 到新 class；需要安全处理 page 中对象，过程受锁、引用和 mover 状态约束。  
> **第四层落生产：** 给出一个指标或算式，例如：在不同对象尺寸 workload 切换前后记录 per-class pages，并观察 automove 收敛时间和 eviction 改善。

## 专家级展开（V2）

### A. 从第一性原理推导

Slab Reassign 是“移动 page”的机制，Automove 是“判断何时/从哪移到哪”的策略；两者共同解决长期 workload 变化造成的 class stagnation。
...
```bash
git grep -n "\bslabs_reassign\b" -- items.c slabs.c slabs.h slabs_mover.c
git grep -n "\bslabs_pick_any_for_reassign\b" -- items.c slabs.c slabs.h slabs_mover.c
git grep -n "\bslab_maintenance_thread\b" -- items.c slabs.c slabs.h slabs_mover.c
git grep -n "\bslab_automove_\b" -- items.c slabs.c slabs.h slabs_mover.c
```
...
移动 page 不是把任意活跃 item 无成本 memcpy 到新 class；需要安全处理 page 中对象，过程受锁、引用和 mover 状态约束。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

错误地频繁 reassign 可能导致维护线程和请求竞争；正确策略应看持续 eviction/空闲，而不是瞬时尖峰。
...
在不同对象尺寸 workload 切换前后记录 per-class pages，并观察 automove 收敛时间和 eviction 改善。
...
[← 上一题](../03-slab-memory/026.md) | [本章目录](README.md) | [下一题 →](../03-slab-memory/028.md)
