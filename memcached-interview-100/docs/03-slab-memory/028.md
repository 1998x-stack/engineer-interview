...
# 第 028 题 · Memcached 如何处理很大的 Item？

> **P2** · ★★★☆☆ · Slab Allocator 与内存管理  
> 本文是 PDF 对应题目的扩展版：保留原答案，并补充源码、复杂度、并发不变量、生产实践与实验。

## 题目

**Memcached 如何处理很大的 Item？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **Slab 内存池、碎片、容量错配与 page 迁移**。

面试官通常不会满足于“术语定义”，而会沿着 `Memcached 如何处理很大的 Item？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 现代实现可使用 item_chunk 链把较大对象跨多个 chunk 组装；item header 持有/关联 chunk chain。
...
2. **把关键路径画出来：** 对 `Memcached 如何处理很大的 Item？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** network_time_floor ≈ value_bytes*8/link_bps；再考虑并发和协议开销，估算单个大 response 的最低传输时间。
4. **准备一个故障反例：** 缓存 5MB JSON 即使命中，也可能比数据库返回精简字段更慢；cache hit 不是自动性能收益。
...
### 追问 1：chunk chain 对 GET 发送有什么影响？

**回答提示：** 先复述边界，再用本题核心机制回答：现代实现可使用 item_chunk 链把较大对象跨多个 chunk 组装；item header 持有/关联 chunk chain。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：为什么大对象会伤害命中率？
...
> **第一句给结论：** 现代实现可使用 item_chunk 链把较大对象跨多个 chunk 组装；item header 持有/关联 chunk chain。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** “支持大 item”不等于“应该缓存巨大 blob”；大 value 会占据更多 NIC 时间、降低并发，并容易形成 class 局部压力。  
> **第四层落生产：** 给出一个指标或算式，例如：network_time_floor ≈ value_bytes*8/link_bps；再考虑并发和协议开销，估算单个大 response 的最低传输时间。

## 专家级展开（V2）

### A. 从第一性原理推导

大 Item 受到 item_size_max、page/chunk 设计和网络带宽多重限制；现代 Memcached 可用 chunked item 链承载超过单 chunk 的对象，但大对象仍会放大内存、复制和尾延迟成本。
...
```bash
git grep -n "\bITEM_CHUNKED\b" -- items.c slabs.c slabs.h slabs_mover.c
git grep -n "\bitem_chunk\b" -- items.c slabs.c slabs.h slabs_mover.c
git grep -n "\bITEM_schunk\b" -- items.c slabs.c slabs.h slabs_mover.c
git grep -n "\bitem_size_max\b" -- items.c slabs.c slabs.h slabs_mover.c
```
...
“支持大 item”不等于“应该缓存巨大 blob”；大 value 会占据更多 NIC 时间、降低并发，并容易形成 class 局部压力。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

缓存 5MB JSON 即使命中，也可能比数据库返回精简字段更慢；cache hit 不是自动性能收益。
...
network_time_floor ≈ value_bytes*8/link_bps；再考虑并发和协议开销，估算单个大 response 的最低传输时间。
...
[← 上一题](../03-slab-memory/027.md) | [本章目录](README.md) | [下一题 →](../03-slab-memory/029.md)
