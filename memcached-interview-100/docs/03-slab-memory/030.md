---
id: 030
title: "Memcached 最大 Item 是不是固定 1MB？"
chapter: 3
difficulty: P0
importance: "★★★★☆"
...
# 第 030 题 · Memcached 最大 Item 是不是固定 1MB？

> **P0** · ★★★★☆ · Slab Allocator 与内存管理  
> 本文是 PDF 对应题目的扩展版：保留原答案，并补充源码、复杂度、并发不变量、生产实践与实验。

## 题目

**Memcached 最大 Item 是不是固定 1MB？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **Slab 内存池、碎片、容量错配与 page 迁移**。

面试官通常不会满足于“术语定义”，而会沿着 `Memcached 最大 Item 是不是固定 1MB？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 1MB 是常见默认限制，不是不可改变的协议常量；当前源码允许通过 item_size_max/-I 调整，并支持更大的上限。
...
2. **把关键路径画出来：** 对 `Memcached 最大 Item 是不是固定 1MB？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 按 value percentile 评估，不要为极少数 P99.99 大对象牺牲整个缓存；可考虑拆分或不缓存。
4. **准备一个故障反例：** 为了缓存 8MB 对象把 -I 调大，结果少数大对象占用高比例带宽并拉高 P99，整体命中率升但用户延迟反而变差。
...
**回答提示：** 先复述边界，再用本题核心机制回答：1MB 是常见默认限制，不是不可改变的协议常量；当前源码允许通过 item_size_max/-I 调整，并支持更大的上限。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：最大对象调到 64MB 会带来哪些风险？
...
> **第一句给结论：** 1MB 是常见默认限制，不是不可改变的协议常量；当前源码允许通过 item_size_max/-I 调整，并支持更大的上限。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 调大限制后客户端、代理、监控和上游序列化也要能处理同样大小；另外巨型 key/value 仍受协议和内部上限约束。  
> **第四层落生产：** 给出一个指标或算式，例如：按 value percentile 评估，不要为极少数 P99.99 大对象牺牲整个缓存；可考虑拆分或不缓存。

## 专家级展开（V2）

### A. 从第一性原理推导

1MB 是经典默认 item_size_max，不是不可变协议常数；配置可调整，但上调会影响 slab/page 行为、网络延迟和内存效率，因此不能只看“能不能存”。
...
调大限制后客户端、代理、监控和上游序列化也要能处理同样大小；另外巨型 key/value 仍受协议和内部上限约束。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

为了缓存 8MB 对象把 -I 调大，结果少数大对象占用高比例带宽并拉高 P99，整体命中率升但用户延迟反而变差。
...
按 value percentile 评估，不要为极少数 P99.99 大对象牺牲整个缓存；可考虑拆分或不缓存。
...
[← 上一题](../03-slab-memory/029.md) | [本章目录](README.md) | [下一题 →](../04-assoc-hashtable/031.md)
