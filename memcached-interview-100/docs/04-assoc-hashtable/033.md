---
id: 033
title: "Memcached 怎么处理 Hash Collision？"
chapter: 4
difficulty: P0
importance: "★★★★☆"
...
# 第 033 题 · Memcached 怎么处理 Hash Collision？
...
**Memcached 怎么处理 Hash Collision？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **本地 Hash 索引、碰撞、扩容与多索引不变量**。

面试官通常不会满足于“术语定义”，而会沿着 `Memcached 怎么处理 Hash Collision？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 多个 key 落到同一 bucket 后沿 h_next 遍历，并比较 key 长度与真实 key bytes，因此 hash 相同不会被视为同一个 key。
...
2. **把关键路径画出来：** 对 `Memcached 怎么处理 Hash Collision？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 构造可控 hash 分布（测试 hash 函数或 mock）测链长与 GET latency 的关系。
4. **准备一个故障反例：** 恶意或异常 key 分布若集中到少数 bucket，会让 CPU 飙升而 item 数并不高；安全版本也持续关注 hash/协议 DoS 类边界。
...
**回答提示：** 先复述边界，再用本题核心机制回答：多个 key 落到同一 bucket 后沿 h_next 遍历，并比较 key 长度与真实 key bytes，因此 hash 相同不会被视为同一个 key。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：恶意 key 是否可能构造 Hash DoS？
...
> **第一句给结论：** 多个 key 落到同一 bucket 后沿 h_next 遍历，并比较 key 长度与真实 key bytes，因此 hash 相同不会被视为同一个 key。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 极端碰撞会让单 bucket 查找退化为 O(k)，且 pointer chasing/锁持有时间增加；平均 O(1) 依赖良好分布和合适 table size。  
> **第四层落生产：** 给出一个指标或算式，例如：构造可控 hash 分布（测试 hash 函数或 mock）测链长与 GET latency 的关系。

## 专家级展开（V2）

### A. 从第一性原理推导

Memcached 使用链式方式处理 Hash collision：先按 hash 选 bucket，再沿 h_next 比较 nkey 和真实 key bytes；hash 相等不是 key 相等。
...
git grep -n "\bh_next\b" -- assoc.c items.c memcached.h
git grep -n "\bITEM_key\b" -- assoc.c items.c memcached.h
git grep -n "\bnkey\b" -- assoc.c items.c memcached.h
```
...
极端碰撞会让单 bucket 查找退化为 O(k)，且 pointer chasing/锁持有时间增加；平均 O(1) 依赖良好分布和合适 table size。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

恶意或异常 key 分布若集中到少数 bucket，会让 CPU 飙升而 item 数并不高；安全版本也持续关注 hash/协议 DoS 类边界。
...
构造可控 hash 分布（测试 hash 函数或 mock）测链长与 GET latency 的关系。
...
[← 上一题](../04-assoc-hashtable/032.md) | [本章目录](README.md) | [下一题 →](../04-assoc-hashtable/034.md)
