---
id: 036
title: "Internal Hash 与 Consistent Hash 是同一个东西吗？"
chapter: 4
difficulty: P0
importance: "★★★★☆"
...
# 第 036 题 · Internal Hash 与 Consistent Hash 是同一个东西吗？
...
**Internal Hash 与 Consistent Hash 是同一个东西吗？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **本地 Hash 索引、碰撞、扩容与多索引不变量**。

面试官通常不会满足于“术语定义”，而会沿着 `Internal Hash 与 Consistent Hash 是同一个东西吗？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 不是：consistent hashing 解决 key → server；assoc hash 解决 server 内 key → bucket。
...
2. **把关键路径画出来：** 对 `Internal Hash 与 Consistent Hash 是同一个东西吗？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 为每层分别定义 load factor：node load distribution 与 local bucket load distribution，不要只看一个“hash 均匀”。
4. **准备一个故障反例：** 节点扩容后 MISS 上升是路由层 remapping；单节点 CPU 异常可能是本地 Hash/热点，两者排查入口不同。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 把两者混淆是 Memcached 分布式面试的常见失分点。
...
### 追问 1：为什么客户端 Hash 变化会造成 MISS，但 assoc resize 不会？

**回答提示：** 先复述边界，再用本题核心机制回答：不是：consistent hashing 解决 key → server；assoc hash 解决 server 内 key → bucket。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：两层 Hash 各自优化什么？
...
> **第一句给结论：** 不是：consistent hashing 解决 key → server；assoc hash 解决 server 内 key → bucket。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** consistent hashing 不能解决本地 Hash collision；internal hash resize 也不会把 key 移到别的服务器。  
> **第四层落生产：** 给出一个指标或算式，例如：为每层分别定义 load factor：node load distribution 与 local bucket load distribution，不要只看一个“hash 均匀”。

## 专家级展开（V2）

### A. 从第一性原理推导

Internal Hash 解决“节点内 key→bucket”，Consistent Hash 解决“集群内 key→server”；一个优化本地查找，一个优化成员变更时的 remapping。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `server: hash/assoc_find；client/proxy: ketama/rendezvous routing`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\bserver\b" -- assoc.c items.c memcached.h
```
...
consistent hashing 不能解决本地 Hash collision；internal hash resize 也不会把 key 移到别的服务器。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

节点扩容后 MISS 上升是路由层 remapping；单节点 CPU 异常可能是本地 Hash/热点，两者排查入口不同。
...
为每层分别定义 load factor：node load distribution 与 local bucket load distribution，不要只看一个“hash 均匀”。
...
[← 上一题](../04-assoc-hashtable/035.md) | [本章目录](README.md) | [下一题 →](../04-assoc-hashtable/037.md)
