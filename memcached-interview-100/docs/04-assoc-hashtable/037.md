...
面试官通常不会满足于“术语定义”，而会沿着 `如果 Hash 函数分布不好会怎样？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 热点 bucket、长冲突链、更多 CPU cache miss 和更长 item lock 持有时间，最终表现为吞吐下降与 P99 升高。
...
2. **把关键路径画出来：** 对 `如果 Hash 函数分布不好会怎样？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 同时统计 key cardinality 分布与 access frequency Zipf 分布，分辨结构碰撞和业务热点。
4. **准备一个故障反例：** key 数量均匀分布但 50% 请求都打一个 key，Hash bucket 数再多也无法摊平该 key 的节点与锁热点。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 服务端内部 Hash 与客户端路由 Hash 要分别讨论。
...
### 追问 1：Murmur/Jenkins 的取舍是什么？

**回答提示：** 先复述边界，再用本题核心机制回答：热点 bucket、长冲突链、更多 CPU cache miss 和更长 item lock 持有时间，最终表现为吞吐下降与 P99 升高。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：Hash 算法切换会影响客户端路由吗？
...
> **第一句给结论：** 热点 bucket、长冲突链、更多 CPU cache miss 和更长 item lock 持有时间，最终表现为吞吐下降与 P99 升高。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 即使 cryptographic hash 分布好，也可能因为业务 hot key 造成访问热点；“hash 均匀”和“流量均匀”是两个概念。  
> **第四层落生产：** 给出一个指标或算式，例如：同时统计 key cardinality 分布与 access frequency Zipf 分布，分辨结构碰撞和业务热点。

## 专家级展开（V2）

### A. 从第一性原理推导

Hash 分布差会形成长 collision chain 和热点 lock bucket，成本包括 key 比较、随机内存访问、CPU cache miss 以及更长临界区，而不仅是 Big-O 变差。
...
```bash
git grep -n "\bhash\b" -- assoc.c items.c memcached.h
git grep -n "\bitem_lock\b" -- assoc.c items.c memcached.h
...
即使 cryptographic hash 分布好，也可能因为业务 hot key 造成访问热点；“hash 均匀”和“流量均匀”是两个概念。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

key 数量均匀分布但 50% 请求都打一个 key，Hash bucket 数再多也无法摊平该 key 的节点与锁热点。
...
同时统计 key cardinality 分布与 access frequency Zipf 分布，分辨结构碰撞和业务热点。
...
[← 上一题](../04-assoc-hashtable/036.md) | [本章目录](README.md) | [下一题 →](../04-assoc-hashtable/038.md)
