---
id: 038
title: "为什么 HashTable 存 item* 而不是复制 Value？"
chapter: 4
difficulty: P1
importance: "★★★☆☆"
...
# 第 038 题 · 为什么 HashTable 存 item* 而不是复制 Value？
...
**为什么 HashTable 存 item* 而不是复制 Value？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **本地 Hash 索引、碰撞、扩容与多索引不变量**。

面试官通常不会满足于“术语定义”，而会沿着 `为什么 HashTable 存 item* 而不是复制 Value？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 数据只保存一份，Hash 与 LRU 都通过指针引用同一 item，减少内存与一致性维护成本。
...
2. **把关键路径画出来：** 对 `为什么 HashTable 存 item* 而不是复制 Value？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 估算若 value 平均 2KB、1亿 items，索引复制一份 value 会额外消耗约 200GB，显然不可接受。
4. **准备一个故障反例：** 若索引复制 value，replace 时必须同步 Hash 副本和 LRU/Slab 副本，既浪费 RAM 又引入一致性 bug。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 节省内存的代价是必须非常谨慎地处理 dangling pointer。
...
**回答提示：** 先复述边界，再用本题核心机制回答：数据只保存一份，Hash 与 LRU 都通过指针引用同一 item，减少内存与一致性维护成本。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：为什么需要严格生命周期管理？
...
> **第一句给结论：** 数据只保存一份，Hash 与 LRU 都通过指针引用同一 item，减少内存与一致性维护成本。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 单份对象并不等于零复制网络发送；响应仍需经过 iovec/buffer/socket 路径。  
> **第四层落生产：** 给出一个指标或算式，例如：估算若 value 平均 2KB、1亿 items，索引复制一份 value 会额外消耗约 200GB，显然不可接受。

## 专家级展开（V2）

### A. 从第一性原理推导

HashTable 只存 item 指针实现 single source of object bytes：Hash、LRU、refcount 都围绕同一个 item 协作，避免 value 多份复制和同步问题。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `primary_hashtable bucket item*`
- `struct item`
- `ITEM_data`
...
git grep -n "\bITEM_data\b" -- assoc.c items.c memcached.h
```
...
单份对象并不等于零复制网络发送；响应仍需经过 iovec/buffer/socket 路径。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

若索引复制 value，replace 时必须同步 Hash 副本和 LRU/Slab 副本，既浪费 RAM 又引入一致性 bug。
...
估算若 value 平均 2KB、1亿 items，索引复制一份 value 会额外消耗约 200GB，显然不可接受。
...
[← 上一题](../04-assoc-hashtable/037.md) | [本章目录](README.md) | [下一题 →](../04-assoc-hashtable/039.md)
