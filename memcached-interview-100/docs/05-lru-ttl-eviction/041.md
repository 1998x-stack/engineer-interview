---
id: 041
title: "Memcached 当前还是最简单的一根 LRU 链吗？"
chapter: 5
difficulty: P0
importance: "★★★★★"
...
# 第 041 题 · Memcached 当前还是最简单的一根 LRU 链吗？
...
**Memcached 当前还是最简单的一根 LRU 链吗？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **segmented LRU、TTL、crawler 与淘汰工程**。

面试官通常不会满足于“术语定义”，而会沿着 `Memcached 当前还是最简单的一根 LRU 链吗？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 不是。现代 Memcached 默认采用 segmented LRU 思路，按 HOT/WARM/COLD 等区域组织，并由后台 maintainer 调整对象，减少高 QPS 下严格 LRU 的锁争用。
...
### 1. 传统“Hash + 一根双向链”适合解释基础，但不是当前完整实现。

哈希层需要同时考虑分布质量、计算成本、冲突链和扩容；“能正确 hash”只是第一层要求，生产性能更依赖均匀性与访问局部性。
### 2. 1.5 时代引入 segmented LRU、crawler、slab automove。
...
### 5. 教科书 strict LRU 在每次 GET 都移动链表头，会把热点读转化为共享写；在多核高 QPS 下，LRU lock 和 cache line bouncing 会成为瓶颈。

高并发下 strict LRU 会把每个读请求变成共享链表写操作。Memcached 通过分段和后台维护，在“淘汰质量”和“锁/缓存一致性开销”之间做工程折中。
### 6. Segmented LRU 的关键不是 HOT/WARM/COLD 三个名字，而是把“新对象”“被证明会复用的对象”“候选淘汰对象”分离，并把部分重排交给后台线程。
...
2. **把关键路径画出来：** 对 `Memcached 当前还是最简单的一根 LRU 链吗？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 构造 hot set + one-pass scan，对比热点命中率随扫描流量变化，观察 segmented policy 的保护效果。
4. **准备一个故障反例：** 扫描型 workload 一次访问海量冷 key，如果所有新 item 都能长期顶掉热点，会污染缓存；segmented LRU 让未被再次访问的对象更快流向 COLD。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 如果只背“访问就移到 LRU 头部”，容易被源码追问击穿。
...
### 追问 1：为什么 segmented LRU 更适合真实 workload？

**回答提示：** 先复述边界，再用本题核心机制回答：不是。现代 Memcached 默认采用 segmented LRU 思路，按 HOT/WARM/COLD 等区域组织，并由后台 maintainer 调整对象，减少高 QPS 下严格 LRU 的锁争用。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：TEMP LRU 是什么？
...
> **第一句给结论：** 不是。现代 Memcached 默认采用 segmented LRU 思路，按 HOT/WARM/COLD 等区域组织，并由后台 maintainer 调整对象，减少高 QPS 下严格 LRU 的锁争用。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** “LRU”在工程系统中通常是近似 recency policy，而不是每次访问严格更新全局顺序；准确性与吞吐是取舍。  
> **第四层落生产：** 给出一个指标或算式，例如：构造 hot set + one-pass scan，对比热点命中率随扫描流量变化，观察 segmented policy 的保护效果。

## 专家级展开（V2）

### A. 从第一性原理推导

现代 Memcached 不是一根 strict LRU；segmented LRU 把 item 分到 HOT/WARM/COLD（及特定用途 segment），用后台维护减少每次 GET 对共享双链表的同步写。
...
“LRU”在工程系统中通常是近似 recency policy，而不是每次访问严格更新全局顺序；准确性与吞吐是取舍。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

扫描型 workload 一次访问海量冷 key，如果所有新 item 都能长期顶掉热点，会污染缓存；segmented LRU 让未被再次访问的对象更快流向 COLD。
...
构造 hot set + one-pass scan，对比热点命中率随扫描流量变化，观察 segmented policy 的保护效果。
...
[← 上一题](../04-assoc-hashtable/040.md) | [本章目录](README.md) | [下一题 →](../05-lru-ttl-eviction/042.md)
