---
id: 044
title: "TTL 到了以后 Item 会在那一秒立刻被物理删除吗？"
chapter: 5
difficulty: P0
importance: "★★★★★"
...
# 第 044 题 · TTL 到了以后 Item 会在那一秒立刻被物理删除吗？
...
**TTL 到了以后 Item 会在那一秒立刻被物理删除吗？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **segmented LRU、TTL、crawler 与淘汰工程**。

面试官通常不会满足于“术语定义”，而会沿着 `TTL 到了以后 Item 会在那一秒立刻被物理删除吗？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 不一定。过期首先意味着逻辑上不可命中；物理空间可在 GET 发现、分配扫描或 crawler 后台遍历时回收。
...
### 2. 因此 curr_items/bytes 与“有效未过期对象”可能短暂不一致。
...
2. **把关键路径画出来：** 对 `TTL 到了以后 Item 会在那一秒立刻被物理删除吗？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 写入固定数量短 TTL key，不再访问；对比 TTL 到点、crawler 扫描、curr_items/bytes 的时间线。
4. **准备一个故障反例：** 大量一次性 key 统一 10 分钟 TTL，之后再无访问；若没有 crawler/内存压力，它们不会在第 600 秒瞬间全部从统计里消失。
...
### 追问 1：为什么 lazy expiration 是好设计？

**回答提示：** 先复述边界，再用本题核心机制回答：不一定。过期首先意味着逻辑上不可命中；物理空间可在 GET 发现、分配扫描或 crawler 后台遍历时回收。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：大量相同 TTL 会对 crawler 有什么压力？
...
> **第一句给结论：** 不一定。过期首先意味着逻辑上不可命中；物理空间可在 GET 发现、分配扫描或 crawler 后台遍历时回收。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 逻辑过期与物理回收的时间差意味着 curr_items/bytes 可能暂时包含 expired item。  
> **第四层落生产：** 给出一个指标或算式，例如：写入固定数量短 TTL key，不再访问；对比 TTL 到点、crawler 扫描、curr_items/bytes 的时间线。

## 专家级展开（V2）

### A. 从第一性原理推导

TTL 是“逻辑有效期”，不是定时器到点立即 free。GET 发现 expired 时可把它当 MISS 并回收；crawler 则负责后台主动扫描，避免无人访问的过期对象长期占空间。
...
git grep -n "\bdo_item_get\b" -- items.c crawler.c memcached.h
git grep -n "\bitem_is_flushed\b" -- items.c crawler.c memcached.h
...
逻辑过期与物理回收的时间差意味着 curr_items/bytes 可能暂时包含 expired item。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

大量一次性 key 统一 10 分钟 TTL，之后再无访问；若没有 crawler/内存压力，它们不会在第 600 秒瞬间全部从统计里消失。
...
写入固定数量短 TTL key，不再访问；对比 TTL 到点、crawler 扫描、curr_items/bytes 的时间线。
...
[← 上一题](../05-lru-ttl-eviction/043.md) | [本章目录](README.md) | [下一题 →](../05-lru-ttl-eviction/045.md)
