...
面试官通常不会满足于“术语定义”，而会沿着 `LRU Crawler 是干什么的？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 后台沿 LRU 检查过期对象并 reclaim，减少无人访问的 expired item 长期占 chunk，也可提供 metadump 等能力。
...
### 2. 扫描速率需要限流，避免争锁和 CPU 峰值。

从“生命周期与近似淘汰”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。现代 Memcached 不追求每次访问都精确维护 strict LRU，而是以 segmented LRU、后台 maintainer/crawler 降低共享写和锁争用。
### 3. crawler stats 可用于观察回收效率。
...
2. **把关键路径画出来：** 对 `LRU Crawler 是干什么的？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 关注 crawler reclaimed、checked、run time（具体 stats 字段依版本），计算 reclaim/scan 比率。
4. **准备一个故障反例：** TTL-heavy workload 如果 crawler 太慢，expired 内存积压；太激进又可能争用 CPU/LRU，因此需要观察扫描效率。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> Crawler 是“主动清过期”，不是新的淘汰算法。
...
**回答提示：** 先复述边界，再用本题核心机制回答：后台沿 LRU 检查过期对象并 reclaim，减少无人访问的 expired item 长期占 chunk，也可提供 metadump 等能力。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：它和 evict tail 的职责区别？
...
> **第一句给结论：** 后台沿 LRU 检查过期对象并 reclaim，减少无人访问的 expired item 长期占 chunk，也可提供 metadump 等能力。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** crawler 不是实时 TTL scheduler；它以扫描速度和资源预算运行，目标是后台维护而非精确到期删除。  
> **第四层落生产：** 给出一个指标或算式，例如：关注 crawler reclaimed、checked、run time（具体 stats 字段依版本），计算 reclaim/scan 比率。

## 专家级展开（V2）

### A. 从第一性原理推导

LRU Crawler 是后台遍历 LRU 的维护机制，用于识别 expired items 和收集 age 信息，减少 lazy expiration 对“必须再次访问”这一条件的依赖。
...
```bash
git grep -n "\blru_crawler_thread\b" -- items.c crawler.c memcached.h
git grep -n "\bcrawler_crawl_q\b" -- items.c crawler.c memcached.h
git grep -n "\bcrawlerstats\b" -- items.c crawler.c memcached.h
```
...
crawler 不是实时 TTL scheduler；它以扫描速度和资源预算运行，目标是后台维护而非精确到期删除。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

TTL-heavy workload 如果 crawler 太慢，expired 内存积压；太激进又可能争用 CPU/LRU，因此需要观察扫描效率。
...
关注 crawler reclaimed、checked、run time（具体 stats 字段依版本），计算 reclaim/scan 比率。
...
[← 上一题](../05-lru-ttl-eviction/044.md) | [本章目录](README.md) | [下一题 →](../05-lru-ttl-eviction/046.md)
