...
面试官通常不会满足于“术语定义”，而会沿着 `经典“30 天 TTL 陷阱”是什么？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> Basic Text 协议中较短 exptime 按相对秒解释，超过 30 天则按 Unix timestamp 绝对时间解释，因此直接传 31 天秒数可能得到完全不同的日期。
...
### 3. Meta protocol 可提供更丰富的 TTL 元信息。
...
2. **把关键路径画出来：** 对 `经典“30 天 TTL 陷阱”是什么？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 用 text protocol 分别设置 30d、30d+1s 和 now+31d 三种 exptime，立即 GET 验证。
4. **准备一个故障反例：** 应用直接发送 31*86400 期望 31 天，server 却把它当 1970 年附近绝对时间，导致立即过期。
...
**回答提示：** 先复述边界，再用本题核心机制回答：Basic Text 协议中较短 exptime 按相对秒解释，超过 30 天则按 Unix timestamp 绝对时间解释，因此直接传 31 天秒数可能得到完全不同的日期。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：跨时区有影响吗？
...
> **第一句给结论：** Basic Text 协议中较短 exptime 按相对秒解释，超过 30 天则按 Unix timestamp 绝对时间解释，因此直接传 31 天秒数可能得到完全不同的日期。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 客户端 SDK 可能隐藏这条规则并自动转换；面试应说明 server 协议语义，同时区分库层封装。  
> **第四层落生产：** 给出一个指标或算式，例如：用 text protocol 分别设置 30d、30d+1s 和 now+31d 三种 exptime，立即 GET 验证。

## 专家级展开（V2）

### A. 从第一性原理推导

经典 text protocol 把 exptime > 30天阈值解释为绝对 Unix 时间，而较小值解释为相对秒数；这是协议语义边界，不是“31天 TTL 不支持”。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `realtime/exptime conversion`
- `REALTIME_MAXDELTA`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\brealtime\b" -- items.c crawler.c memcached.h
git grep -n "\bREALTIME_MAXDELTA\b" -- items.c crawler.c memcached.h
```
...
客户端 SDK 可能隐藏这条规则并自动转换；面试应说明 server 协议语义，同时区分库层封装。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

应用直接发送 31*86400 期望 31 天，server 却把它当 1970 年附近绝对时间，导致立即过期。
...
用 text protocol 分别设置 30d、30d+1s 和 now+31d 三种 exptime，立即 GET 验证。
...
[← 上一题](../05-lru-ttl-eviction/045.md) | [本章目录](README.md) | [下一题 →](../05-lru-ttl-eviction/047.md)
