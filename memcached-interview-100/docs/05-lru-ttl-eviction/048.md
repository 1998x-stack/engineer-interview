---
id: 048
title: "刚被 GET 的 Item 会不会同时被另一个线程 Evict？"
chapter: 5
difficulty: P2
importance: "★★★☆☆"
...
# 第 048 题 · 刚被 GET 的 Item 会不会同时被另一个线程 Evict？

> **P2** · ★★★☆☆ · LRU、TTL 与 Eviction  
> 本文是 PDF 对应题目的扩展版：保留原答案，并补充源码、复杂度、并发不变量、生产实践与实验。

## 题目

**刚被 GET 的 Item 会不会同时被另一个线程 Evict？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **segmented LRU、TTL、crawler 与淘汰工程**。

面试官通常不会满足于“术语定义”，而会沿着 `刚被 GET 的 Item 会不会同时被另一个线程 Evict？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 可以先从 Hash/LRU 逻辑 unlink，但只要 GET/response 仍持有 refcount，物理 chunk 不能被重新分配。
...
2. **把关键路径画出来：** 对 `刚被 GET 的 Item 会不会同时被另一个线程 Evict？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 在 debug 环境人为延迟 GET response，同时制造内存压力触发 eviction，观察 item refcount 和 free 时点。
4. **准备一个故障反例：** GET 正在构造大 response 时 COLD LRU 选中该 item；正确实现必须避免 response 途中内存被重用。
...
**回答提示：** 先复述边界，再用本题核心机制回答：可以先从 Hash/LRU 逻辑 unlink，但只要 GET/response 仍持有 refcount，物理 chunk 不能被重新分配。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：为什么 refcount 仍需锁配合？
...
> **第一句给结论：** 可以先从 Hash/LRU 逻辑 unlink，但只要 GET/response 仍持有 refcount，物理 chunk 不能被重新分配。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 因此 eviction 不是对所有正在执行请求的强制取消，也不是立即把 chunk 交给新 item。  
> **第四层落生产：** 给出一个指标或算式，例如：在 debug 环境人为延迟 GET response，同时制造内存压力触发 eviction，观察 item refcount 和 free 时点。

## 专家级展开（V2）

### A. 从第一性原理推导

可以发生“逻辑上被 evict，但物理上仍活着”：reader 已经持有 refcount，eviction 从 Hash/LRU unlink 后未来 GET MISS，旧 reader 继续安全读完，最后引用归零再 free。
...
git grep -n "\bdo_item_unlink\b" -- items.c crawler.c memcached.h
git grep -n "\brefcount_incr\b" -- items.c crawler.c memcached.h
git grep -n "\bdo_item_remove\b" -- items.c crawler.c memcached.h
```
...
因此 eviction 不是对所有正在执行请求的强制取消，也不是立即把 chunk 交给新 item。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

GET 正在构造大 response 时 COLD LRU 选中该 item；正确实现必须避免 response 途中内存被重用。
...
在 debug 环境人为延迟 GET response，同时制造内存压力触发 eviction，观察 item refcount 和 free 时点。
...
[← 上一题](../05-lru-ttl-eviction/047.md) | [本章目录](README.md) | [下一题 →](../05-lru-ttl-eviction/049.md)
