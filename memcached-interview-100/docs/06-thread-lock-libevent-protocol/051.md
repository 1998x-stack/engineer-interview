...
# 第 051 题 · Memcached 是“一连接一线程”吗？

> **必刷题** · **P0** · ★★★★★ · 线程、锁、Libevent 与协议  
> 本文是 PDF 对应题目的扩展版：保留原答案，并补充源码、复杂度、并发不变量、生产实践与实验。

## 题目

**Memcached 是“一连接一线程”吗？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **事件驱动网络、线程分工、锁粒度与协议演进**。

面试官通常不会满足于“术语定义”，而会沿着 `Memcached 是“一连接一线程”吗？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 不是。典型架构是监听线程接入连接，多个 worker 各自运行 libevent event loop，同时管理大量 socket。
...
### 2. worker 还有配套 utility/background 线程。

从“I/O 与并发”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。一个 worker 可以借助 libevent 管理很多连接；性能的关键是减少共享锁、系统调用和 cache-line bouncing，而不是无限增加线程。
### 3. 这样避免每连接线程的栈内存和上下文切换。

从“I/O 与并发”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。一个 worker 可以借助 libevent 管理很多连接；性能的关键是减少共享锁、系统调用和 cache-line bouncing，而不是无限增加线程。
### 4. I/O 与并发

从“I/O 与并发”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。一个 worker 可以借助 libevent 管理很多连接；性能的关键是减少共享锁、系统调用和 cache-line bouncing，而不是无限增加线程。
### 5. Memcached 的 worker 不是阻塞等一个连接完成，而是每个 worker 拥有 event base，维护很多 conn 状态机。连接可能在 parsing、nread、write 等状态之间切换。

从“I/O 与并发”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。一个 worker 可以借助 libevent 管理很多连接；性能的关键是减少共享锁、系统调用和 cache-line bouncing，而不是无限增加线程。
### 6. 这使线程数量与连接数量解耦：几万连接并不意味着几万线程，系统资源主要转向 fd、buffer 与 event registration。
...
2. **把关键路径画出来：** 对 `Memcached 是“一连接一线程”吗？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 容量规划同时算 maxconns/fd 与 worker CPU；不要用“连接数/线程数”一比一估算。
4. **准备一个故障反例：** 10万 idle keep-alive 连接并不需要10万线程，但会占 fd、connection struct 和 buffer 内存。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 不要把多线程等同于 blocking thread-per-connection。
...
**回答提示：** 先复述边界，再用本题核心机制回答：不是。典型架构是监听线程接入连接，多个 worker 各自运行 libevent event loop，同时管理大量 socket。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：为什么 epoll/kqueue 适合这种模型？
...
> **第一句给结论：** 不是。典型架构是监听线程接入连接，多个 worker 各自运行 libevent event loop，同时管理大量 socket。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** worker 数仍受 CPU 核、锁竞争和网络模型影响，不是越多越好；后台 maintainer/crawler 等线程又是另一类角色。  
> **第四层落生产：** 给出一个指标或算式，例如：容量规划同时算 maxconns/fd 与 worker CPU；不要用“连接数/线程数”一比一估算。

## 专家级展开（V2）

### A. 从第一性原理推导

Memcached 不是一连接一线程，而是少量 worker thread 各自运行 libevent loop 并管理许多 connection；连接数与线程数因此可以解耦。
...
```bash
git grep -n "\bthread_init\b" -- memcached.c thread.c proto_text.c proto_parser.c
...
git grep -n "\bconn\b" -- memcached.c thread.c proto_text.c proto_parser.c
```
...
worker 数仍受 CPU 核、锁竞争和网络模型影响，不是越多越好；后台 maintainer/crawler 等线程又是另一类角色。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

10万 idle keep-alive 连接并不需要10万线程，但会占 fd、connection struct 和 buffer 内存。
...
容量规划同时算 maxconns/fd 与 worker CPU；不要用“连接数/线程数”一比一估算。
...
[← 上一题](../05-lru-ttl-eviction/050.md) | [本章目录](README.md) | [下一题 →](../06-thread-lock-libevent-protocol/052.md)
