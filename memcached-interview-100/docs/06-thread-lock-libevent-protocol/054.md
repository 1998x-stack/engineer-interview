...
# 第 054 题 · 为什么 Memcached 不是线程越多越快？
...
**为什么 Memcached 不是线程越多越快？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **事件驱动网络、线程分工、锁粒度与协议演进**。

面试官通常不会满足于“术语定义”，而会沿着 `为什么 Memcached 不是线程越多越快？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 线程增加到一定程度后会遭遇 lock contention、context switch、cache coherence、NUMA 和 scheduler 开销，吞吐可能反而下降。
...
2. **把关键路径画出来：** 对 `为什么 Memcached 不是线程越多越快？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 做 thread sweep：1/2/4/8/16/...，同时记录 QPS、P99、mutex wait、CPU util，找膝点。
4. **准备一个故障反例：** 从 8 增到 64 worker 后吞吐只涨 5%，P99 反而翻倍，说明已进入共享资源/调度主导区。
...
**回答提示：** 先复述边界，再用本题核心机制回答：线程增加到一定程度后会遭遇 lock contention、context switch、cache coherence、NUMA 和 scheduler 开销，吞吐可能反而下降。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：超线程核心该怎么算？
...
> **第一句给结论：** 线程增加到一定程度后会遭遇 lock contention、context switch、cache coherence、NUMA 和 scheduler 开销，吞吐可能反而下降。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 最佳线程数必须用目标 workload 压测；默认值不是硬规则，CPU 核数也不是直接等式。  
> **第四层落生产：** 给出一个指标或算式，例如：做 thread sweep：1/2/4/8/16/...，同时记录 QPS、P99、mutex wait、CPU util，找膝点。

## 专家级展开（V2）

### A. 从第一性原理推导

线程扩展受 Amdahl 定律和共享结构约束：更多 worker 增加并行能力，也增加 item/LRU/slab locks、cache coherence、scheduler 和内存带宽竞争。
...
```bash
git grep -n "\bsettings\b" -- memcached.c thread.c proto_text.c proto_parser.c
...
做 thread sweep：1/2/4/8/16/...，同时记录 QPS、P99、mutex wait、CPU util，找膝点。
...
[← 上一题](../06-thread-lock-libevent-protocol/053.md) | [本章目录](README.md) | [下一题 →](../06-thread-lock-libevent-protocol/055.md)
