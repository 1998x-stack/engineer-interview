...
# 第 056 题 · 为什么不能只用一个 Global Mutex？
...
**为什么不能只用一个 Global Mutex？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **事件驱动网络、线程分工、锁粒度与协议演进**。

面试官通常不会满足于“术语定义”，而会沿着 `为什么不能只用一个 Global Mutex？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 所有 GET/SET/DELETE 串行化会让多核退化为单核，并在热点下产生 convoy；细粒度/分片锁允许不相关 key 并行。
...
2. **把关键路径画出来：** 对 `为什么不能只用一个 Global Mutex？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 用 Little’s Law/排队直觉估算锁利用率 ρ=arrival_rate×hold_time，接近1时 tail latency 会急剧恶化。
4. **准备一个故障反例：** 如果全局锁平均持有 2µs，理论上单锁最多约50万次/s（还未算排队）；100万 QPS 已不可能仅靠加线程解决。
...
### 追问 1：为什么无锁 HashTable 不一定更快？

**回答提示：** 先复述边界，再用本题核心机制回答：所有 GET/SET/DELETE 串行化会让多核退化为单核，并在热点下产生 convoy；细粒度/分片锁允许不相关 key 并行。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：锁分片数量怎么选？
...
> **第一句给结论：** 所有 GET/SET/DELETE 串行化会让多核退化为单核，并在热点下产生 convoy；细粒度/分片锁允许不相关 key 并行。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 锁越细也不是越好：更多锁增加管理复杂度、锁序风险和 cache footprint，需要在 contention 与复杂性之间折中。  
> **第四层落生产：** 给出一个指标或算式，例如：用 Little’s Law/排队直觉估算锁利用率 ρ=arrival_rate×hold_time，接近1时 tail latency 会急剧恶化。

## 专家级展开（V2）

### A. 从第一性原理推导

单 Global Mutex 会把所有 GET/SET/DELETE 线性化，使多 worker 只增加排队；分片 item lock、独立 LRU lock 和专用 allocator lock 是把冲突限制到必要共享结构。
...
锁越细也不是越好：更多锁增加管理复杂度、锁序风险和 cache footprint，需要在 contention 与复杂性之间折中。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

如果全局锁平均持有 2µs，理论上单锁最多约50万次/s（还未算排队）；100万 QPS 已不可能仅靠加线程解决。
...
用 Little’s Law/排队直觉估算锁利用率 ρ=arrival_rate×hold_time，接近1时 tail latency 会急剧恶化。
...
[← 上一题](../06-thread-lock-libevent-protocol/055.md) | [本章目录](README.md) | [下一题 →](../06-thread-lock-libevent-protocol/057.md)
