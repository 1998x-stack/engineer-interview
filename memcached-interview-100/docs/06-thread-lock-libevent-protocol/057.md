...
面试官通常不会满足于“术语定义”，而会沿着 `Memcached 现在有哪些协议？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 官方当前主要支持 Basic Text 与 Meta Text；Binary Protocol 已 deprecated，不再作为新客户端优先方案。
...
### 2. Meta 用 flags/tokens 提供更高扩展性。

Meta Protocol 的价值在于用可扩展 flag/token 在一次往返中表达更多缓存状态，特别适合 anti-dogpiling、stale 和版本控制。
### 3. 协议选择影响功能、RTT 与客户端复杂度。
...
2. **把关键路径画出来：** 对 `Memcached 现在有哪些协议？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 协议选型不仅看 bytes，还比较 round trips、客户端实现成熟度、可观测性和所需语义。
4. **准备一个故障反例：** 老客户端只支持 Binary，服务端升级时功能仍可能工作，但长期维护、安全修复和新特性支持会受限。
...
### 追问 1：为什么 Binary 曾经流行？

**回答提示：** 当前官方将 Binary 标记为 deprecated；新客户端更推荐 Meta，它更易扩展且能表达 anti-dogpiling/stale 等语义。
### 追问 2：旧客户端如何迁移 Meta？

**回答提示：** 先复述边界，再用本题核心机制回答：官方当前主要支持 Basic Text 与 Meta Text；Binary Protocol 已 deprecated，不再作为新客户端优先方案。 最后补充一个生产后果或失败场景，避免只给术语。
...
> **第一句给结论：** 官方当前主要支持 Basic Text 与 Meta Text；Binary Protocol 已 deprecated，不再作为新客户端优先方案。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** “Binary 已 deprecated”不等于所有旧客户端立刻不能用；迁移需要看客户端和服务端版本，但新设计不应优先依赖 deprecated 协议。  
> **第四层落生产：** 给出一个指标或算式，例如：协议选型不仅看 bytes，还比较 round trips、客户端实现成熟度、可观测性和所需语义。

## 专家级展开（V2）

### A. 从第一性原理推导

当前应重点掌握 Basic Text 与 Meta Text；Binary protocol 已 deprecated。面试应说明协议演进原因：Meta 用可组合 flag 提供更丰富 cache 语义与较好的扩展性。
...
- `proto_text.c`
- `proto_parser.c`
- `meta commands mg/ms/md/ma`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\bproto_text\b" -- memcached.c thread.c proto_text.c proto_parser.c
git grep -n "\bproto_parser\b" -- memcached.c thread.c proto_text.c proto_parser.c
...
“Binary 已 deprecated”不等于所有旧客户端立刻不能用；迁移需要看客户端和服务端版本，但新设计不应优先依赖 deprecated 协议。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

老客户端只支持 Binary，服务端升级时功能仍可能工作，但长期维护、安全修复和新特性支持会受限。
...
协议选型不仅看 bytes，还比较 round trips、客户端实现成熟度、可观测性和所需语义。
...
[← 上一题](../06-thread-lock-libevent-protocol/056.md) | [本章目录](README.md) | [下一题 →](../06-thread-lock-libevent-protocol/058.md)
