...
面试官通常不会满足于“术语定义”，而会沿着 `一致性哈希解决什么？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 把节点和 key 映射到环/哈希空间，节点加入或退出时只让邻近范围 key 改变归属，降低重映射比例。
...
2. **把关键路径画出来：** 对 `一致性哈希解决什么？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 同时计算 remap ratio 和目标节点 load amplification，不能只看“移动了多少 key”。
4. **准备一个故障反例：** 一台节点故障后，它负责的约1/N key 被映射到邻近/其他节点，瞬间形成额外流量和冷 miss，需要 origin protection。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 名字里的“一致性”指路由稳定性，不是数据一致性。
...
### 追问 1：为什么一致性哈希不是强一致算法？

**回答提示：** 先复述边界，再用本题核心机制回答：把节点和 key 映射到环/哈希空间，节点加入或退出时只让邻近范围 key 改变归属，降低重映射比例。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：环上节点分布不均怎么办？
...
> **第一句给结论：** 把节点和 key 映射到环/哈希空间，节点加入或退出时只让邻近范围 key 改变归属，降低重映射比例。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 节点故障时受影响 key 仍然 MISS；一致性哈希只是减少“额外不相关 key”也被 remap。  
> **第四层落生产：** 给出一个指标或算式，例如：同时计算 remap ratio 和目标节点 load amplification，不能只看“移动了多少 key”。

## 专家级展开（V2）

### A. 从第一性原理推导

一致性哈希解决的是 membership change 下的最小 key 迁移和相对均匀路由，不提供数据复制、强一致性或自动 warmup。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `ketama continuum / rendezvous hash（客户端或 proxy 实现）`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\bketama\b" -- proto_proxy.c proxy_config.c
```
...
节点故障时受影响 key 仍然 MISS；一致性哈希只是减少“额外不相关 key”也被 remap。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

一台节点故障后，它负责的约1/N key 被映射到邻近/其他节点，瞬间形成额外流量和冷 miss，需要 origin protection。
...
同时计算 remap ratio 和目标节点 load amplification，不能只看“移动了多少 key”。
...
[← 上一题](../07-consistent-hashing-distributed/061.md) | [本章目录](README.md) | [下一题 →](../07-consistent-hashing-distributed/063.md)
