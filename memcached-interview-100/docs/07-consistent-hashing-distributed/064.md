---
id: 064
title: "一致性哈希是在 Memcached Server 内部完成的吗？"
chapter: 7
difficulty: P0
importance: "★★★★★"
...
# 第 064 题 · 一致性哈希是在 Memcached Server 内部完成的吗？
...
**一致性哈希是在 Memcached Server 内部完成的吗？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **客户端/代理路由、一致性哈希、节点故障与控制面**。

面试官通常不会满足于“术语定义”，而会沿着 `一致性哈希是在 Memcached Server 内部完成的吗？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 传统模式不是，客户端库维护节点列表并计算路由；服务器收到请求后只处理本地 key。
...
### 2. 也把 membership 一致性责任推给客户端配置。

从“路由与节点自治”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。传统 Memcached 把集群路由放到客户端：节点内部只管理自己的 KV。节点成员变化首先是“路由变化”，然后表现为缓存命中变化，而非数据库式数据迁移。
### 3. 内置 proxy 可把路由集中到代理层。
...
2. **把关键路径画出来：** 对 `一致性哈希是在 Memcached Server 内部完成的吗？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 对比 smart-client 与 proxy 两种架构的配置收敛时间、额外 RTT、可观测性和 blast radius。
4. **准备一个故障反例：** 更换客户端库 hash 算法会造成全局 remap；使用 proxy 可以集中控制，但引入代理容量和故障域。
...
**回答提示：** 先复述边界，再用本题核心机制回答：传统模式不是，客户端库维护节点列表并计算路由；服务器收到请求后只处理本地 key。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：proxy 有什么运维优势？
...
> **第一句给结论：** 传统模式不是，客户端库维护节点列表并计算路由；服务器收到请求后只处理本地 key。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 回答时应加“传统模式”限定，避免忽略现代 proxy；同时不要把 proxy 路由说成 server 数据自动复制。  
> **第四层落生产：** 给出一个指标或算式，例如：对比 smart-client 与 proxy 两种架构的配置收敛时间、额外 RTT、可观测性和 blast radius。

## 专家级展开（V2）

### A. 从第一性原理推导

传统 Memcached server 不知道集群 membership；一致性哈希由客户端库完成。现代 built-in proxy 可以把这层集中到代理，但那仍是独立路由层，不是 assoc 的本地 Hash。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `server assoc_*；proxy route/pool 配置`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\bserver\b" -- proto_proxy.c proxy_config.c
```
...
回答时应加“传统模式”限定，避免忽略现代 proxy；同时不要把 proxy 路由说成 server 数据自动复制。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

更换客户端库 hash 算法会造成全局 remap；使用 proxy 可以集中控制，但引入代理容量和故障域。
...
对比 smart-client 与 proxy 两种架构的配置收敛时间、额外 RTT、可观测性和 blast radius。
...
[← 上一题](../07-consistent-hashing-distributed/063.md) | [本章目录](README.md) | [下一题 →](../07-consistent-hashing-distributed/065.md)
