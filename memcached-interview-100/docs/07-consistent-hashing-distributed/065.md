---
id: 065
title: "加一台 Memcached 后，旧数据会自动迁过去吗？"
chapter: 7
difficulty: P0
importance: "★★★★☆"
...
# 第 065 题 · 加一台 Memcached 后，旧数据会自动迁过去吗？
...
**加一台 Memcached 后，旧数据会自动迁过去吗？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **客户端/代理路由、一致性哈希、节点故障与控制面**。

面试官通常不会满足于“术语定义”，而会沿着 `加一台 Memcached 后，旧数据会自动迁过去吗？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 传统模式不会主动迁移；路由变化后部分 key 去新节点产生 MISS，再由应用回源并 lazy warm。
...
### 2. 代价是扩容瞬间 origin 压力上升。

从“路由与节点自治”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。传统 Memcached 把集群路由放到客户端：节点内部只管理自己的 KV。节点成员变化首先是“路由变化”，然后表现为缓存命中变化，而非数据库式数据迁移。
### 3. 可用 warmup、双写或渐进流量缓解。
...
2. **把关键路径画出来：** 对 `加一台 Memcached 后，旧数据会自动迁过去吗？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 估算新增节点 remap_fraction × request_qps × origin_cost，决定是否需要 staged rollout。
4. **准备一个故障反例：** 上线新节点后一分钟内 hit rate 下降，若数据库没有 headroom，扩容反而触发事故；需要渐进加权或预热/限流。
...
**回答提示：** 先复述边界，再用本题核心机制回答：传统模式不会主动迁移；路由变化后部分 key 去新节点产生 MISS，再由应用回源并 lazy warm。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：为什么缓存比数据库更适合 lazy migration？
...
> **第一句给结论：** 传统模式不会主动迁移；路由变化后部分 key 去新节点产生 MISS，再由应用回源并 lazy warm。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 如果业务主动做预热，那是外部流程，不是 Memcached server 自动搬迁；预热本身也可能压垮 origin。  
> **第四层落生产：** 给出一个指标或算式，例如：估算新增节点 remap_fraction × request_qps × origin_cost，决定是否需要 staged rollout。

## 专家级展开（V2）

### A. 从第一性原理推导

经典扩容没有 durable shard migration：ring 改变后部分 key 路由到新节点产生 MISS，随后由正常请求逐步回源填充，属于 lazy warming。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `client membership/routing; server only GET/SET`
...
如果业务主动做预热，那是外部流程，不是 Memcached server 自动搬迁；预热本身也可能压垮 origin。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

上线新节点后一分钟内 hit rate 下降，若数据库没有 headroom，扩容反而触发事故；需要渐进加权或预热/限流。
...
估算新增节点 remap_fraction × request_qps × origin_cost，决定是否需要 staged rollout。
...
[← 上一题](../07-consistent-hashing-distributed/064.md) | [本章目录](README.md) | [下一题 →](../07-consistent-hashing-distributed/066.md)
