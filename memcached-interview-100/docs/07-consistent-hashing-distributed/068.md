...
面试官通常不会满足于“术语定义”，而会沿着 `为什么缓存系统不一定需要强复制？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 因为对象可由 authoritative source 重建，强复制的 RAM 与写放大成本可能高于偶发 MISS 的代价；真正要保护的是回源容量与恢复过程。
...
2. **把关键路径画出来：** 对 `为什么缓存系统不一定需要强复制？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** expected_failure_cost = failure_rate × affected_qps × rebuild_cost；与 replication 持续资源成本比较。
4. **准备一个故障反例：** 静态商品详情回源便宜可不复制；昂贵推荐结果重算500ms且高并发，副本或 stale 策略更值得。
...
### 追问 1：怎样做 selective replication？

**回答提示：** 先复述边界，再用本题核心机制回答：因为对象可由 authoritative source 重建，强复制的 RAM 与写放大成本可能高于偶发 MISS 的代价；真正要保护的是回源容量与恢复过程。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：什么时候缓存也需要强可用？
...
> **第一句给结论：** 因为对象可由 authoritative source 重建，强复制的 RAM 与写放大成本可能高于偶发 MISS 的代价；真正要保护的是回源容量与恢复过程。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** “可重建”不代表“重建便宜”；如果回源是昂贵 ML 推理或慢 SQL，缓存副本价值可能很高。  
> **第四层落生产：** 给出一个指标或算式，例如：expected_failure_cost = failure_rate × affected_qps × rebuild_cost；与 replication 持续资源成本比较。

## 专家级展开（V2）

### A. 从第一性原理推导

缓存不一定强复制，因为数据可重建且复制会消耗 RAM/网络；是否复制应由 origin 承压能力、warmup 时间和热点失效成本决定，而不是照搬数据库策略。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `architecture-level; optional proxy/client replication`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\barchitecture\b" -- proto_proxy.c proxy_config.c
```
...
expected_failure_cost = failure_rate × affected_qps × rebuild_cost；与 replication 持续资源成本比较。
...
[← 上一题](../07-consistent-hashing-distributed/067.md) | [本章目录](README.md) | [下一题 →](../07-consistent-hashing-distributed/069.md)
