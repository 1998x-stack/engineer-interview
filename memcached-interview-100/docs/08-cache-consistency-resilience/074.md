...
面试官通常不会满足于“术语定义”，而会沿着 `什么叫缓存穿透？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 大量请求访问本来就不存在的 key，Cache MISS 后持续打到 DB；常用负缓存、Bloom filter、输入校验、限流。
...
2. **把关键路径画出来：** 对 `什么叫缓存穿透？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 监控 cache_miss AND origin_not_found 的交集比例；它比总 miss 更能识别穿透。
4. **准备一个故障反例：** 攻击/bug 随机请求不存在 ID，key 基数巨大，负缓存本身也会污染 working set；此时参数校验/Bloom/rate limit 更重要。
...
**回答提示：** 先复述边界，再用本题核心机制回答：大量请求访问本来就不存在的 key，Cache MISS 后持续打到 DB；常用负缓存、Bloom filter、输入校验、限流。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：负缓存如何处理权限差异？
...
> **第一句给结论：** 大量请求访问本来就不存在的 key，Cache MISS 后持续打到 DB；常用负缓存、Bloom filter、输入校验、限流。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 负缓存 TTL 不能过长，否则新创建的数据可能在一段时间内仍被 NOT_FOUND 命中；要权衡穿透保护与新鲜度。  
> **第四层落生产：** 给出一个指标或算式，例如：监控 cache_miss AND origin_not_found 的交集比例；它比总 miss 更能识别穿透。

## 专家级展开（V2）

### A. 从第一性原理推导

缓存穿透是“请求的 key 在事实源也不存在”，因此每次都 MISS→origin miss；治理目标是阻止无意义重复回源，而不是单纯扩容 cache。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `negative cache via set/add TTL; Bloom filter at app layer`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\bnegative\b" -- proto_text.c items.c
```
...
负缓存 TTL 不能过长，否则新创建的数据可能在一段时间内仍被 NOT_FOUND 命中；要权衡穿透保护与新鲜度。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

攻击/bug 随机请求不存在 ID，key 基数巨大，负缓存本身也会污染 working set；此时参数校验/Bloom/rate limit 更重要。
...
监控 cache_miss AND origin_not_found 的交集比例；它比总 miss 更能识别穿透。
...
[← 上一题](../08-cache-consistency-resilience/073.md) | [本章目录](README.md) | [下一题 →](../08-cache-consistency-resilience/075.md)
