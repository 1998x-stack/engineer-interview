...
面试官通常不会满足于“术语定义”，而会沿着 `什么叫缓存雪崩？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 大量 key 同时失效或整个 cache 集群不可用，使回源流量在短时间内成倍增加。
...
2. **把关键路径画出来：** 对 `什么叫缓存雪崩？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 把 expiration timestamps 做直方图；如果过度集中，用随机 jitter/分批 refresh 打散。
4. **准备一个故障反例：** 1000万 key 都在整点 TTL 到期，数据库流量产生周期性尖峰；即使单 key 不热也会形成 aggregate avalanche。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 雪崩治理是系统工程而非单个 TTL 技巧。
...
**回答提示：** 先复述边界，再用本题核心机制回答：大量 key 同时失效或整个 cache 集群不可用，使回源流量在短时间内成倍增加。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：缓存全挂时哪些接口先降级？
...
> **第一句给结论：** 大量 key 同时失效或整个 cache 集群不可用，使回源流量在短时间内成倍增加。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 给 TTL 加 jitter 只解决同步过期，不解决整组节点断电或路由错误，因此需要多层防护。  
> **第四层落生产：** 给出一个指标或算式，例如：把 expiration timestamps 做直方图；如果过度集中，用随机 jitter/分批 refresh 打散。

## 专家级展开（V2）

### A. 从第一性原理推导

缓存雪崩是“大量 key 或大量容量同时失效”造成的 aggregate miss surge，区别于单 hot key 的击穿；典型原因包括同 TTL、集群故障、统一 flush 或部署变更。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `TTL + architecture; server no auto-origin protection`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\bTTL\b" -- proto_text.c items.c
```
...
给 TTL 加 jitter 只解决同步过期，不解决整组节点断电或路由错误，因此需要多层防护。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

1000万 key 都在整点 TTL 到期，数据库流量产生周期性尖峰；即使单 key 不热也会形成 aggregate avalanche。
...
把 expiration timestamps 做直方图；如果过度集中，用随机 jitter/分批 refresh 打散。
...
[← 上一题](../08-cache-consistency-resilience/075.md) | [本章目录](README.md) | [下一题 →](../08-cache-consistency-resilience/077.md)
