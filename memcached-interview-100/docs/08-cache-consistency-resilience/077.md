...
面试官通常不会满足于“术语定义”，而会沿着 `Hot Key 有什么危险？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 一致性哈希只做 key 级分片，一个超级热 key 仍集中到一台节点，造成单机 CPU/网络/item lock 热点。
...
### 2. 可做 key replica/多副本读，但写与一致性更复杂。

从“Cache 失败模型”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。Cache miss 是正常语义，但集中 miss 是生产事故来源。所有一致性与容灾设计都要围绕“如何保护 authoritative origin”展开。
### 3. 热点检测应看 per-key 或 proxy sampling。
...
2. **把关键路径画出来：** 对 `Hot Key 有什么危险？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** hot_key_bandwidth = qps × value_size ×8；按 TopK key 算节点级网络贡献。
4. **准备一个故障反例：** 一个 100KB 热配置 20k QPS 就约16Gbps 响应流量，单 NIC 先饱和，即使 CPU 很空。
...
### 追问 1：如何无侵入发现 hot key？

**回答提示：** 先复述边界，再用本题核心机制回答：一致性哈希只做 key 级分片，一个超级热 key 仍集中到一台节点，造成单机 CPU/网络/item lock 热点。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：复制热点 key 时如何选副本？
...
> **第一句给结论：** 一致性哈希只做 key 级分片，一个超级热 key 仍集中到一台节点，造成单机 CPU/网络/item lock 热点。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 增加普通节点数不一定缓解单 key；除非路由层做复制/拆 key/L1，否则热点仍落原节点。  
> **第四层落生产：** 给出一个指标或算式，例如：hot_key_bandwidth = qps × value_size ×8；按 TopK key 算节点级网络贡献。

## 专家级展开（V2）

### A. 从第一性原理推导

Hot Key 绕过了“节点数很多”的平均扩展性：同 key 通常固定路由到单节点，并集中 item-lock、CPU cache line、NIC 和 response bandwidth。
...
```bash
git grep -n "\brouting\b" -- proto_text.c items.c
git grep -n "\bitem_lock\b" -- proto_text.c items.c
git grep -n "\bassoc_find\b" -- proto_text.c items.c
```
...
增加普通节点数不一定缓解单 key；除非路由层做复制/拆 key/L1，否则热点仍落原节点。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

一个 100KB 热配置 20k QPS 就约16Gbps 响应流量，单 NIC 先饱和，即使 CPU 很空。
...
hot_key_bandwidth = qps × value_size ×8；按 TopK key 算节点级网络贡献。
...
[← 上一题](../08-cache-consistency-resilience/076.md) | [本章目录](README.md) | [下一题 →](../08-cache-consistency-resilience/078.md)
