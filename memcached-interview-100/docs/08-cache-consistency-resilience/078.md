...
**CAS 能不能解决缓存与数据库的强一致性？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **缓存一致性、回源治理、stampede/雪崩/热点**。

面试官通常不会满足于“术语定义”，而会沿着 `CAS 能不能解决缓存与数据库的强一致性？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 不能。CAS 只对 Memcached item 的版本竞争提供乐观控制，它不知道 DB transaction；跨系统仍需 CDC、version、事务消息等架构。
...
2. **把关键路径画出来：** 对 `CAS 能不能解决缓存与数据库的强一致性？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 定义 cache consistency lag：DB version time 到 cache version 可见时间，按 P99 监控而非宣称“CAS 强一致”。
4. **准备一个故障反例：** DB 已更新 v2，但 cache CAS 更新因超时失败，仍可能暂时读 v1；需要 TTL/CDC/retry/version check 收敛。
...
### 追问 1：如何用 DB row version 映射到 cache？

**回答提示：** 先复述边界，再用本题核心机制回答：不能。CAS 只对 Memcached item 的版本竞争提供乐观控制，它不知道 DB transaction；跨系统仍需 CDC、version、事务消息等架构。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：为什么双写问题本质上难？
...
> **第一句给结论：** 不能。CAS 只对 Memcached item 的版本竞争提供乐观控制，它不知道 DB transaction；跨系统仍需 CDC、version、事务消息等架构。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 即使把 DB version 写进 CAS，也只是帮助检测/排序，不能自动做到原子两阶段提交。  
> **第四层落生产：** 给出一个指标或算式，例如：定义 cache consistency lag：DB version time 到 cache version 可见时间，按 P99 监控而非宣称“CAS 强一致”。

## 专家级展开（V2）

### A. 从第一性原理推导

CAS 只约束 Memcached 单 key 的版本，不参与 DB transaction；因此它能防缓存内部 lost update，却不能消除 DB commit 与 cache update/delete 的跨系统失败窗口。
...
即使把 DB version 写进 CAS，也只是帮助检测/排序，不能自动做到原子两阶段提交。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

DB 已更新 v2，但 cache CAS 更新因超时失败，仍可能暂时读 v1；需要 TTL/CDC/retry/version check 收敛。
...
定义 cache consistency lag：DB version time 到 cache version 可见时间，按 P99 监控而非宣称“CAS 强一致”。
...
[← 上一题](../08-cache-consistency-resilience/077.md) | [本章目录](README.md) | [下一题 →](../08-cache-consistency-resilience/079.md)
