...
# 第 080 题 · Session 能不能存在 Memcached？
...
**Session 能不能存在 Memcached？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **缓存一致性、回源治理、stampede/雪崩/热点**。

面试官通常不会满足于“术语定义”，而会沿着 `Session 能不能存在 Memcached？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 可以，但前提是 session 丢失可被业务接受或有其他恢复机制；节点故障、eviction、restart 都可能让用户被迫重新登录。
...
### 2. 高价值 session 可采用持久 backing store。
...
2. **把关键路径画出来：** 对 `Session 能不能存在 Memcached？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 先定义 session loss budget（例如月度 forced re-login rate），再决定副本/持久层，而不是只看 latency。
4. **准备一个故障反例：** 全站登录 session 只存 Memcached，滚动重启造成大规模登出；技术上缓存正常，产品语义却不可接受。
...
### 追问 1：购物车是否适合只存在 Memcached？

**回答提示：** 先复述边界，再用本题核心机制回答：可以，但前提是 session 丢失可被业务接受或有其他恢复机制；节点故障、eviction、restart 都可能让用户被迫重新登录。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：如何做 graceful session loss？
...
> **第一句给结论：** 可以，但前提是 session 丢失可被业务接受或有其他恢复机制；节点故障、eviction、restart 都可能让用户被迫重新登录。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 复制或 sticky routing 可以降低丢失概率，但不能把 ephemeral cache 自动变成 durable session database。  
> **第四层落生产：** 给出一个指标或算式，例如：先定义 session loss budget（例如月度 forced re-login rate），再决定副本/持久层，而不是只看 latency。

## 专家级展开（V2）

### A. 从第一性原理推导

Session 可以缓存，但是否可接受取决于“session 丢失语义”。如果节点失败就让用户重新登录可接受，Memcached 可以；如果 session 是安全/交易权威状态，则需要 durable store 或恢复机制。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `ordinary GET/SET/TTL; no special session durability`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\bordinary\b" -- proto_text.c items.c
```
...
复制或 sticky routing 可以降低丢失概率，但不能把 ephemeral cache 自动变成 durable session database。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

全站登录 session 只存 Memcached，滚动重启造成大规模登出；技术上缓存正常，产品语义却不可接受。
...
先定义 session loss budget（例如月度 forced re-login rate），再决定副本/持久层，而不是只看 latency。
...
[← 上一题](../08-cache-consistency-resilience/079.md) | [本章目录](README.md) | [下一题 →](../09-observability-capacity-troubleshooting/081.md)
