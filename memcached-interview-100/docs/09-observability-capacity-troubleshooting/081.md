...
**生产 Memcached 第一眼看哪些指标？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **可观测性、容量规划、故障排查与 OS/硬件边界**。

面试官通常不会满足于“术语定义”，而会沿着 `生产 Memcached 第一眼看哪些指标？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 先看 hit/miss、cmd_get/cmd_set、bytes/limit、curr_items、evictions/reclaimed、connections/listen_disabled，再按 slab 下钻，并结合 CPU、RSS、网络与 P99。
...
2. **把关键路径画出来：** 对 `生产 Memcached 第一眼看哪些指标？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 推荐 dashboard：QPS、hit ratio、P50/P95/P99、evictions/s、miss→origin QPS、RSS/-m、network、connections、per-class age。
4. **准备一个故障反例：** hit rate 99% 看似很好，但 P99 MISS 查询耗时5s，用户仍然卡顿；或者 eviction 只集中在一个 class。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 监控应该回答“容量、命中、延迟、故障域”四类问题。
...
**回答提示：** 先复述边界，再用本题核心机制回答：先看 hit/miss、cmd_get/cmd_set、bytes/limit、curr_items、evictions/reclaimed、connections/listen_disabled，再按 slab 下钻，并结合 CPU、RSS、网络与 P99。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：为什么只看 hit rate 不够？
...
> **第一句给结论：** 先看 hit/miss、cmd_get/cmd_set、bytes/limit、curr_items、evictions/reclaimed、connections/listen_disabled，再按 slab 下钻，并结合 CPU、RSS、网络与 P99。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 单个 hit rate 或 memory usage 无法代表健康；tail latency 和 origin 负载必须一起看。  
> **第四层落生产：** 给出一个指标或算式，例如：推荐 dashboard：QPS、hit ratio、P50/P95/P99、evictions/s、miss→origin QPS、RSS/-m、network、connections、per-class age。

## 专家级展开（V2）

### A. 从第一性原理推导

生产第一屏应覆盖四类信号：流量（cmd_get/set）、效果（hits/misses）、容量/淘汰（bytes/items/evictions/reclaimed per class）、资源与连接（CPU/RSS/network/connections/listen_disabled）。
...
单个 hit rate 或 memory usage 无法代表健康；tail latency 和 origin 负载必须一起看。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

hit rate 99% 看似很好，但 P99 MISS 查询耗时5s，用户仍然卡顿；或者 eviction 只集中在一个 class。
...
推荐 dashboard：QPS、hit ratio、P50/P95/P99、evictions/s、miss→origin QPS、RSS/-m、network、connections、per-class age。

工程上至少给出一个可以**测量或计算**的量。没有数字的“性能更好”“内存更省”“更稳定”通常无法指导容量规划，也很难在故障现场验证。

## 关联题目

- [079. incr/decr 为什么适合做轻量计数器？](../08-cache-consistency-resilience/079.md)
...
[← 上一题](../08-cache-consistency-resilience/080.md) | [本章目录](README.md) | [下一题 →](../09-observability-capacity-troubleshooting/082.md)
