...
面试官通常不会满足于“术语定义”，而会沿着 `Hit Rate 怎么计算？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 通常 get_hits / (get_hits + get_misses)；但全局高 hit rate 可能掩盖少数极昂贵 MISS 或单个 slab/class 问题。
...
2. **把关键路径画出来：** 对 `Hit Rate 怎么计算？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** cost_saved_ratio=Σ(hit_request_origin_cost)/Σ(all_request_origin_cost)，可比纯 hit count 更贴近价值。
4. **准备一个故障反例：** 缓存大量廉价配置把 hit rate“刷高”，但真正昂贵 SQL key 经常 MISS，数据库仍高负载。
...
### 追问 1：99% hit rate 是否一定优秀？

**回答提示：** 先复述边界，再用本题核心机制回答：通常 get_hits / (get_hits + get_misses)；但全局高 hit rate 可能掩盖少数极昂贵 MISS 或单个 slab/class 问题。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：如何计算缓存节省的 DB CPU？
...
> **第一句给结论：** 通常 get_hits / (get_hits + get_misses)；但全局高 hit rate 可能掩盖少数极昂贵 MISS 或单个 slab/class 问题。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 99% 小对象命中 + 1% 巨型昂贵对象 MISS 可能比90%普通命中更糟，因此不能把 hit rate 当唯一 KPI。  
> **第四层落生产：** 给出一个指标或算式，例如：cost_saved_ratio=Σ(hit_request_origin_cost)/Σ(all_request_origin_cost)，可比纯 hit count 更贴近价值。

## 专家级展开（V2）

### A. 从第一性原理推导

基础 hit rate = hits/(hits+misses)，但它只是请求数量加权；业务更关心 byte hit rate、cost-weighted hit rate 和关键 key hit rate。
...
```bash
git grep -n "\bget_hits\b" -- stats_prefix.c items.c slabs.c memcached.c
git grep -n "\bget_misses\b" -- stats_prefix.c items.c slabs.c memcached.c
git grep -n "\bcmd_get\b" -- stats_prefix.c items.c slabs.c memcached.c
```
...
99% 小对象命中 + 1% 巨型昂贵对象 MISS 可能比90%普通命中更糟，因此不能把 hit rate 当唯一 KPI。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

缓存大量廉价配置把 hit rate“刷高”，但真正昂贵 SQL key 经常 MISS，数据库仍高负载。
...
cost_saved_ratio=Σ(hit_request_origin_cost)/Σ(all_request_origin_cost)，可比纯 hit count 更贴近价值。
...
[← 上一题](../09-observability-capacity-troubleshooting/081.md) | [本章目录](README.md) | [下一题 →](../09-observability-capacity-troubleshooting/083.md)
