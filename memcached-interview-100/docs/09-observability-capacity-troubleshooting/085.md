---
id: 085
title: "get_misses 突然暴涨，你怎么排查？"
chapter: 9
difficulty: P1
importance: "★★★☆☆"
module: "监控、容量规划与故障诊断"
must_do: false
version_baseline: "Memcached 1.6.45"
---

# 第 085 题 · get_misses 突然暴涨，你怎么排查？

> **P1** · ★★★☆☆ · 监控、容量规划与故障诊断  
> 本文是 PDF 对应题目的扩展版：保留原答案，并补充源码、复杂度、并发不变量、生产实践与实验。

## 题目

**get_misses 突然暴涨，你怎么排查？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **可观测性、容量规划、故障排查与 OS/硬件边界**。

面试官通常不会满足于“术语定义”，而会沿着 `get_misses 突然暴涨，你怎么排查？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 先排节点故障/路由变更，再看 key format/TTL/deploy/flush，随后看 evictions/slab imbalance、流量结构、warmup 和客户端序列化 bug。

面试时建议先讲上面这句话，再根据追问选择展开源码或生产侧影响，不要一开始就陷入函数细节。

## 心智模型

**可观测性闭环：** 不能只看 hit rate。必须把请求、容量、slab class、eviction、连接、CPU/网络和 origin 压力串成因果链。

```mermaid
flowchart LR
  M[Metrics] --> H[Hypothesis]
  H --> D[Drill-down: slabs/items/conns]
  D --> T[Test / change]
  T --> V[Verify P99 + origin load]
  V --> M
```

## 深度机制

### 1. 把时间点与发布/扩缩容事件对齐。

从“可观测性闭环”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。不能只看 hit rate。必须把请求、容量、slab class、eviction、连接、CPU/网络和 origin 压力串成因果链。
### 2. 对比所有节点而非平均值。

从“可观测性闭环”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。不能只看 hit rate。必须把请求、容量、slab class、eviction、连接、CPU/网络和 origin 压力串成因果链。
### 3. 检查 DB 同时是否出现 QPS/latency 抬升。

QPS 不能单独决定节点数。需要分别计算内存、CPU、网络和 P99 的约束，并用压测得到的安全单节点能力而非理论峰值。

## 系统不变量

- **不变量 1：** 监控结论必须能从现象追到具体资源/路径。
- **不变量 2：** 扩容或参数变更后要用命中率、P99 与 origin load 验证效果。

这些不变量比记忆具体函数名更稳定。版本升级可能调整代码组织，但破坏这些约束通常意味着正确性或生命周期出现问题。

## 复杂度与性能分析

- 任何指标都应看 rate/delta 而不是孤立累计值，并结合 P50/P95/P99 与 origin 指标。
- 容量诊断要按 slab class、连接和节点维度下钻，避免用全局平均掩盖局部瓶颈。

进一步分析时要区分三个层面：**算法复杂度**（如 Hash 平均 O(1)）、**微架构成本**（cache miss、pointer chasing、cache-line bouncing）和 **系统成本**（RTT、序列化、回源）。高水平回答会主动指出这三者不是一回事。

## 源码导航



建议优先阅读以下 Memcached 1.6.45 文件：

- [`items.c`](https://github.com/memcached/memcached/blob/1.6.45/items.c)
- [`slabs.c`](https://github.com/memcached/memcached/blob/1.6.45/slabs.c)
- [`memcached.c`](https://github.com/memcached/memcached/blob/1.6.45/memcached.c)

源码阅读方法：先定位入口函数，再跟踪 **Item 指针如何变化、何时加锁、何时 publication/unlink、何时释放引用**。不要按文件从第一行顺序阅读。

## 工程与生产实践

1. **先确认语义边界：** 本题讨论的是 可观测性、容量规划、故障排查与 OS/硬件边界，先区分 server 内部机制、客户端路由和应用回源三层，避免跨层归因。
2. **把关键路径画出来：** 对 `get_misses 突然暴涨，你怎么排查？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 用变更时间线关联 miss_rate，分节点/客户端版本/key namespace 切片定位。
4. **准备一个故障反例：** 发布后 miss rate 从2%到70%，但 evictions=0、bytes低；检查发现 key prefix 从 v1: 改为 v2:，本质是全量冷启动。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 排查顺序比背一堆可能性更重要。

再补充两个通用陷阱：

- 把“默认值”说成“协议永远固定值”；例如 item size、线程数等都应区分默认配置与不可变约束。
- 把“当前实现细节”说成“KV cache 必然原理”；面试中应先讲不变量，再讲 Memcached 的具体实现。

## 高频追问

### 追问 1：如何快速区分“路由错”与“内存不足”？

**回答提示：** 先复述边界，再用本题核心机制回答：先排节点故障/路由变更，再看 key format/TTL/deploy/flush，随后看 evictions/slab imbalance、流量结构、warmup 和客户端序列化 bug。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：某一节点 miss 特别高说明什么？

**回答提示：** 先复述边界，再用本题核心机制回答：先排节点故障/路由变更，再看 key format/TTL/deploy/flush，随后看 evictions/slab imbalance、流量结构、warmup 和客户端序列化 bug。 最后补充一个生产后果或失败场景，避免只给术语。

## 动手验证

```bash
printf "stats\r\nstats settings\r\nstats slabs\r\nstats items\r\n" | nc 127.0.0.1 11211
```

每隔固定时间采样一次并计算 delta/rate。不要只截一张累计值快照；把 `get_misses`、`evictions`、P99 和数据库 QPS 放在同一时间轴。

完成实验后，至少记录：**预期 → 实际 → 为什么 → 对生产设计有什么影响**。只跑通命令而没有解释，不算真正掌握。

## 面试表达模板

> **第一句给结论：** 先排节点故障/路由变更，再看 key format/TTL/deploy/flush，随后看 evictions/slab imbalance、流量结构、warmup 和客户端序列化 bug。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 不要第一反应扩容；如果根因是 key namespace 改动或 ring mismatch，加内存毫无帮助。  
> **第四层落生产：** 给出一个指标或算式，例如：用变更时间线关联 miss_rate，分节点/客户端版本/key namespace 切片定位。

## 专家级展开（V2）

### A. 从第一性原理推导

MISS 激增要按“路由→生命周期→容量→客户端契约→origin”顺序排查：节点/Hash ring、部署 key prefix、TTL/flush、eviction/class、序列化和真正数据变化。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `get_misses`
- `evictions`
- `stats settings/slabs; client ring version`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\bget_misses\b" -- stats_prefix.c items.c slabs.c memcached.c
git grep -n "\bevictions\b" -- stats_prefix.c items.c slabs.c memcached.c
git grep -n "\bstats\b" -- stats_prefix.c items.c slabs.c memcached.c
```

阅读函数时至少记录 5 个问题：**入参中的 key/hash/item 是谁创建的、当前持有哪些锁、item 是否已 `ITEM_LINKED`、refcount 谁持有、失败路径最终由谁释放资源**。

### C. 边界条件与反例

不要第一反应扩容；如果根因是 key namespace 改动或 ring mismatch，加内存毫无帮助。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

发布后 miss rate 从2%到70%，但 evictions=0、bytes低；检查发现 key prefix 从 v1: 改为 v2:，本质是全量冷启动。

遇到这类问题时，建议把故障链写成：

```text
触发条件 → Memcached 内部状态变化 → HIT/MISS/P99 变化 → origin/网络/CPU 后果 → 保护或恢复动作
```

这样回答能自然从源码层过渡到生产系统，而不是把“源码题”和“系统设计题”割裂开。

### E. 定量分析 / 可验证指标

用变更时间线关联 miss_rate，分节点/客户端版本/key namespace 切片定位。

工程上至少给出一个可以**测量或计算**的量。没有数字的“性能更好”“内存更省”“更稳定”通常无法指导容量规划，也很难在故障现场验证。

## 关联题目

- [083. evictions 持续增长说明什么？](../09-observability-capacity-troubleshooting/083.md)
- [084. 为什么 Item 过期后 curr_items 可能没有立即下降？](../09-observability-capacity-troubleshooting/084.md)
- [086. listen_disabled_num 很高说明什么？](../09-observability-capacity-troubleshooting/086.md)
- [087. 为什么应该使用 Persistent Connections？](../09-observability-capacity-troubleshooting/087.md)

## 官方参考

- **S11** [Server Maintenance](https://docs.memcached.org/serverguide/maintenance/) — hit rate、eviction、slab 监控与维护。
- **S12** [Server Configuring](https://docs.memcached.org/serverguide/configuring/) — 内存、连接、线程、item size、运行配置。
- **S17** [Performance and Efficiency](https://docs.memcached.org/serverguide/performance/) — 内存占用、回收与性能行为。

---

[← 上一题](../09-observability-capacity-troubleshooting/084.md) | [本章目录](README.md) | [下一题 →](../09-observability-capacity-troubleshooting/086.md)
