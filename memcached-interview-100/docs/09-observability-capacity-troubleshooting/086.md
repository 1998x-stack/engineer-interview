---
id: 086
title: "listen_disabled_num 很高说明什么？"
chapter: 9
difficulty: P1
importance: "★★★☆☆"
module: "监控、容量规划与故障诊断"
...
# 第 086 题 · listen_disabled_num 很高说明什么？
...
**listen_disabled_num 很高说明什么？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **可观测性、容量规划、故障排查与 OS/硬件边界**。

面试官通常不会满足于“术语定义”，而会沿着 `listen_disabled_num 很高说明什么？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 表示曾因连接容量限制停止/拒绝接收新连接；要检查 maxconns、fd limit、连接泄漏、客户端不复用连接及连接 churn。
...
2. **把关键路径画出来：** 对 `listen_disabled_num 很高说明什么？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 同时看 curr_connections、connection_structures、accept rate、TIME_WAIT（客户端/OS侧）、fd limit。
4. **准备一个故障反例：** 客户端每请求新建连接且不复用，连接 churn 把 server 推到上限，即使 QPS 并不高。
...
**回答提示：** 先复述边界，再用本题核心机制回答：表示曾因连接容量限制停止/拒绝接收新连接；要检查 maxconns、fd limit、连接泄漏、客户端不复用连接及连接 churn。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：TIME_WAIT 与 server 连接有什么关系？
...
> **第一句给结论：** 表示曾因连接容量限制停止/拒绝接收新连接；要检查 maxconns、fd limit、连接泄漏、客户端不复用连接及连接 churn。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 简单提高 maxconns 可能把问题转为 RSS/fd/worker 过载；必须先找连接泄漏、短连接风暴或池配置。  
> **第四层落生产：** 给出一个指标或算式，例如：同时看 curr_connections、connection_structures、accept rate、TIME_WAIT（客户端/OS侧）、fd limit。

## 专家级展开（V2）

### A. 从第一性原理推导

listen_disabled_num 高通常表示连接接收曾因 maxconns/fd 等限制被暂停；它是连接控制面拥塞信号，不等同于 KV 算法慢。
...
- `maxconns`
- `listen_disabled_num`
- `conn_new/listening state`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\bmaxconns\b" -- stats_prefix.c items.c slabs.c memcached.c
git grep -n "\blisten_disabled_num\b" -- stats_prefix.c items.c slabs.c memcached.c
git grep -n "\bconn_new\b" -- stats_prefix.c items.c slabs.c memcached.c
```
...
简单提高 maxconns 可能把问题转为 RSS/fd/worker 过载；必须先找连接泄漏、短连接风暴或池配置。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

客户端每请求新建连接且不复用，连接 churn 把 server 推到上限，即使 QPS 并不高。
...
同时看 curr_connections、connection_structures、accept rate、TIME_WAIT（客户端/OS侧）、fd limit。
...
[← 上一题](../09-observability-capacity-troubleshooting/085.md) | [本章目录](README.md) | [下一题 →](../09-observability-capacity-troubleshooting/087.md)
