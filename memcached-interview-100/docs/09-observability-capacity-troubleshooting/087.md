---
id: 087
title: "为什么应该使用 Persistent Connections？"
chapter: 9
difficulty: P0
importance: "★★★★☆"
module: "监控、容量规划与故障诊断"
...
# 第 087 题 · 为什么应该使用 Persistent Connections？
...
**为什么应该使用 Persistent Connections？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **可观测性、容量规划、故障排查与 OS/硬件边界**。

面试官通常不会满足于“术语定义”，而会沿着 `为什么应该使用 Persistent Connections？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> KV 本地操作极快，反复 TCP handshake/teardown 的成本会占据总延迟；连接池和长连接减少 RTT、syscall 与内核状态开销。
...
2. **把关键路径画出来：** 对 `为什么应该使用 Persistent Connections？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 分别测 connect+GET 与 warm-connection GET；如果差距远大于 server processing，优化客户端连接。
4. **准备一个故障反例：** 函数式/Serverless 客户端每次调用都新建连接，cache latency 主要变成网络握手；开启连接池后 P99 显著下降。
...
**回答提示：** 先复述边界，再用本题核心机制回答：KV 本地操作极快，反复 TCP handshake/teardown 的成本会占据总延迟；连接池和长连接减少 RTT、syscall 与内核状态开销。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：TLS 会进一步放大什么成本？
...
> **第一句给结论：** KV 本地操作极快，反复 TCP handshake/teardown 的成本会占据总延迟；连接池和长连接减少 RTT、syscall 与内核状态开销。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 长连接也要有池大小、空闲回收和故障重连；过大池会浪费 server fd/内存。  
> **第四层落生产：** 给出一个指标或算式，例如：分别测 connect+GET 与 warm-connection GET；如果差距远大于 server processing，优化客户端连接。

## 专家级展开（V2）

### A. 从第一性原理推导

Persistent Connection 消除重复 TCP handshake/slow-start/accept/conn alloc，并允许 multi-get/pipeline 更高效；对于亚毫秒 KV，连接建立成本往往比服务器查找本身更大。
...
```bash
git grep -n "\bconn\b" -- stats_prefix.c items.c slabs.c memcached.c
git grep -n "\baccept\b" -- stats_prefix.c items.c slabs.c memcached.c
git grep -n "\bclient\b" -- stats_prefix.c items.c slabs.c memcached.c
```
...
长连接也要有池大小、空闲回收和故障重连；过大池会浪费 server fd/内存。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

函数式/Serverless 客户端每次调用都新建连接，cache latency 主要变成网络握手；开启连接池后 P99 显著下降。
...
分别测 connect+GET 与 warm-connection GET；如果差距远大于 server processing，优化客户端连接。
...
[← 上一题](../09-observability-capacity-troubleshooting/086.md) | [本章目录](README.md) | [下一题 →](../09-observability-capacity-troubleshooting/088.md)
