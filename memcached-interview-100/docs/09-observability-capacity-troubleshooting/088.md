...
面试官通常不会满足于“术语定义”，而会沿着 `stats slabs 能帮助判断什么？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 观察每 class 的 chunk size、pages、free/used chunks、set/get/eviction 等，定位对象大小分布和局部内存压力。
...
2. **把关键路径画出来：** 对 `stats slabs 能帮助判断什么？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 制作 class_id×time 热力图展示 used_ratio/evictions，快速定位局部内存热点。
4. **准备一个故障反例：** 某 class free_chunks 接近0且 eviction高，邻近 class 大量 free，说明 page distribution 可能不匹配。
...
### 追问 1：如何识别“对象刚好跨 class 边界”？

**回答提示：** 先复述边界，再用本题核心机制回答：观察每 class 的 chunk size、pages、free/used chunks、set/get/eviction 等，定位对象大小分布和局部内存压力。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：为什么 total_pages 特别重要？
...
> **第一句给结论：** 观察每 class 的 chunk size、pages、free/used chunks、set/get/eviction 等，定位对象大小分布和局部内存压力。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 只截取一次 snapshot 容易误判；需要看时间序列和 workload change 前后差异。  
> **第四层落生产：** 给出一个指标或算式，例如：制作 class_id×time 热力图展示 used_ratio/evictions，快速定位局部内存热点。

## 专家级展开（V2）

### A. 从第一性原理推导

stats slabs 把内存按对象尺寸视角展开：chunk_size、chunks_per_page、used/free、pages、cmd/set/hit 等帮助判断对象分布、class 压力和 automove 需求。
...
```bash
git grep -n "\bslabs_stats\b" -- stats_prefix.c items.c slabs.c memcached.c
git grep -n "\bslabclass_t\b" -- stats_prefix.c items.c slabs.c memcached.c
...
只截取一次 snapshot 容易误判；需要看时间序列和 workload change 前后差异。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

某 class free_chunks 接近0且 eviction高，邻近 class 大量 free，说明 page distribution 可能不匹配。
...
制作 class_id×time 热力图展示 used_ratio/evictions，快速定位局部内存热点。
...
[← 上一题](../09-observability-capacity-troubleshooting/087.md) | [本章目录](README.md) | [下一题 →](../09-observability-capacity-troubleshooting/089.md)
