---
id: 090
title: "为什么 Swap 对 Memcached 特别危险？"
chapter: 9
difficulty: P0
importance: "★★★★★"
module: "监控、容量规划与故障诊断"
...
# 第 090 题 · 为什么 Swap 对 Memcached 特别危险？
...
**为什么 Swap 对 Memcached 特别危险？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **可观测性、容量规划、故障排查与 OS/硬件边界**。

面试官通常不会满足于“术语定义”，而会沿着 `为什么 Swap 对 Memcached 特别危险？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> Memcached 的价值来自可预测 RAM latency；一旦 item page 被 swap，GET 会触发 page fault 和磁盘访问，P99 可上升几个数量级。
...
### 2. 应控制系统内存 headroom 和 swappiness。

一旦工作集被换出，原本纳秒级/微秒级内存访问会变成页故障和存储 I/O，直接破坏缓存作为低延迟层的价值。
### 3. 容器 OOM 与 swap 策略也需理解。
...
2. **把关键路径画出来：** 对 `为什么 Swap 对 Memcached 特别危险？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 监控 major page faults、swap in/out、memory PSI 与 Memcached P99 的相关性。
4. **准备一个故障反例：** 节点内存压力触发 swap，CPU 反而下降但 latency 暴涨；误以为 Memcached CPU 不高所以 server 健康。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> Extstore 是显式分层设计，swap 是不可控内核换页。
...
### 追问 1：为什么宁可更小的无 swap cache？

**回答提示：** 先复述边界，再用本题核心机制回答：Memcached 的价值来自可预测 RAM latency；一旦 item page 被 swap，GET 会触发 page fault 和磁盘访问，P99 可上升几个数量级。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：Extstore 与 swap 有什么本质区别？
...
> **第一句给结论：** Memcached 的价值来自可预测 RAM latency；一旦 item page 被 swap，GET 会触发 page fault 和磁盘访问，P99 可上升几个数量级。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** “还有 swap”不是安全容量；缓存服务通常宁愿 eviction 也不愿依赖 swap 保存冷 item。  
> **第四层落生产：** 给出一个指标或算式，例如：监控 major page faults、swap in/out、memory PSI 与 Memcached P99 的相关性。

## 专家级展开（V2）

### A. 从第一性原理推导

Swap 直接破坏 Memcached 的核心延迟假设：item 指针命中后如果 page 被换出，原本纳秒级内存访问变成 page fault + 存储 I/O，P99/P999 会灾难性抖动。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `OS memory management; RSS/-m/swap metrics`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\bOS\b" -- stats_prefix.c items.c slabs.c memcached.c
```
...
“还有 swap”不是安全容量；缓存服务通常宁愿 eviction 也不愿依赖 swap 保存冷 item。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

节点内存压力触发 swap，CPU 反而下降但 latency 暴涨；误以为 Memcached CPU 不高所以 server 健康。
...
监控 major page faults、swap in/out、memory PSI 与 Memcached P99 的相关性。
...
[← 上一题](../09-observability-capacity-troubleshooting/089.md) | [本章目录](README.md) | [下一题 →](../10-advanced-source-system-design/091.md)
