...
面试官通常不会满足于“术语定义”，而会沿着 `什么是 Warm Restart？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 特定配置下进程重启可以尽可能复用已有缓存内存状态，降低重启后的 cold-cache 风暴，但它不提供数据库式持久化承诺。
...
2. **把关键路径画出来：** 对 `什么是 Warm Restart？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 比较 cold restart 与 warm restart 的恢复 hit-rate 曲线、origin QPS 峰值和达到95%稳态命中所需时间。
4. **准备一个故障反例：** 安全升级需要重启数十个节点，如果每台都 cold，数据库会周期性被打高；warm restart 能降低这一部署风险。
...
### 追问 1：Warm Restart 和 replication 谁解决的问题不同？

**回答提示：** 它降低进程重启后的冷缓存冲击，但不是 WAL/复制意义上的持久化保证。
### 追问 2：为什么不能把它当持久存储？

**回答提示：** 先复述边界，再用本题核心机制回答：特定配置下进程重启可以尽可能复用已有缓存内存状态，降低重启后的 cold-cache 风暴，但它不提供数据库式持久化承诺。 最后补充一个生产后果或失败场景，避免只给术语。
...
> **第一句给结论：** 特定配置下进程重启可以尽可能复用已有缓存内存状态，降低重启后的 cold-cache 风暴，但它不提供数据库式持久化承诺。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 机器掉电、内存丢失或不兼容配置仍可能无法复用；不能把它当 durability SLA。  
> **第四层落生产：** 给出一个指标或算式，例如：比较 cold restart 与 warm restart 的恢复 hit-rate 曲线、origin QPS 峰值和达到95%稳态命中所需时间。

## 专家级展开（V2）

### A. 从第一性原理推导

Warm Restart 目标是进程重启后复用已有缓存内存/元数据，减少全量 cold start 对 origin 的冲击；它优化 restart recovery，不是跨机器持久化。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `restart.c / slabs_fixup / restart metadata（具体实现以1.6.45源码为准）`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\brestart\b" -- storage.c extstore.c proto_proxy.c proxy_config.c proto_text.c
```
...
机器掉电、内存丢失或不兼容配置仍可能无法复用；不能把它当 durability SLA。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

安全升级需要重启数十个节点，如果每台都 cold，数据库会周期性被打高；warm restart 能降低这一部署风险。
...
比较 cold restart 与 warm restart 的恢复 hit-rate 曲线、origin QPS 峰值和达到95%稳态命中所需时间。
...
[← 上一题](../10-advanced-source-system-design/092.md) | [本章目录](README.md) | [下一题 →](../10-advanced-source-system-design/094.md)
