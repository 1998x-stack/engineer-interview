---
id: 094
title: "Meta Protocol 怎么实现 stale-while-revalidate？"
chapter: 10
difficulty: P2
importance: "★★★☆☆"
module: "高级特性、源码与系统设计"
...
# 第 094 题 · Meta Protocol 怎么实现 stale-while-revalidate？
...
**Meta Protocol 怎么实现 stale-while-revalidate？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **Extstore、Warm Restart、Meta/Proxy 与系统设计**。

面试官通常不会满足于“术语定义”，而会沿着 `Meta Protocol 怎么实现 stale-while-revalidate？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 允许过期/标 stale 对象在限定窗口继续被部分请求使用，同时通过 token/flags 让一个客户端承担刷新，其他客户端避免一起回源。
...
2. **把关键路径画出来：** 对 `Meta Protocol 怎么实现 stale-while-revalidate？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 观测 revalidation winner rate、stale served count、refresh latency 和 stale age P99。
4. **准备一个故障反例：** 首页推荐 key 过期，赢家去重算200ms，其余请求继续返回旧推荐而不是全部等/回源。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> Meta 的价值在复杂缓存语义，而不是换一种命令拼写。
...
**回答提示：** 先复述边界，再用本题核心机制回答：允许过期/标 stale 对象在限定窗口继续被部分请求使用，同时通过 token/flags 让一个客户端承担刷新，其他客户端避免一起回源。 最后补充一个生产后果或失败场景，避免只给术语。
### 追问 2：stale window 太长有什么风险？
...
> **第一句给结论：** 允许过期/标 stale 对象在限定窗口继续被部分请求使用，同时通过 token/flags 让一个客户端承担刷新，其他客户端避免一起回源。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** stale 服务必须有业务最大陈旧时间；价格、权限等强时效数据不能无限 stale。  
> **第四层落生产：** 给出一个指标或算式，例如：观测 revalidation winner rate、stale served count、refresh latency 和 stale age P99。

## 专家级展开（V2）

### A. 从第一性原理推导

stale-while-revalidate 的核心是把“过期”拆成“不能作为最终新鲜值”与“仍可短暂服务的 stale 副本”；Meta 允许选出一个 revalidator，其他请求继续吃 stale，抑制 stampede。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `Meta get flags for stale/winner/recache; ITEM_STALE; official Meta protocol`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\bMeta\b" -- storage.c extstore.c proto_proxy.c proxy_config.c proto_text.c
```
...
观测 revalidation winner rate、stale served count、refresh latency 和 stale age P99。
...
[← 上一题](../10-advanced-source-system-design/093.md) | [本章目录](README.md) | [下一题 →](../10-advanced-source-system-design/095.md)
