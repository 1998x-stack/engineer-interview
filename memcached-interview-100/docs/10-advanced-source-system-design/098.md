---
id: 098
title: "一个 Node 故障后，怎样避免数据库被 MISS 打死？"
chapter: 10
difficulty: P0
importance: "★★★★★"
module: "高级特性、源码与系统设计"
...
# 第 098 题 · 一个 Node 故障后，怎样避免数据库被 MISS 打死？
...
**一个 Node 故障后，怎样避免数据库被 MISS 打死？**

## 面试官在考什么

这道题用于判断候选人是否真正理解 **Extstore、Warm Restart、Meta/Proxy 与系统设计**。

面试官通常不会满足于“术语定义”，而会沿着 `一个 Node 故障后，怎样避免数据库被 MISS 打死？` 继续追问到：**状态如何变化、哪个模块拥有这份状态、并发下怎样保持不变量、生产中用什么指标证明判断**。优秀回答应先给 30 秒结论，再主动给出一个反例或故障场景。

## 30-90 秒标准回答

> 组合 request coalescing、origin 并发限流、熔断、stale serving、L1 cache、warm spare/replica、渐进 rehash 和业务降级，让恢复流量受控。
...
### 2. 所有客户端同时 failover 可能形成 thundering herd。

从“从机制到架构”看，这一结论的重点不是记住术语，而是明确职责边界和失败方式。高级特性不是功能清单：Extstore、Meta、Warm Restart、Proxy 分别解决容量、协议语义、重启冲击和路由控制面问题，系统设计要量化代价。
### 3. 恢复节点也要渐进 warm，避免 cache 填充抢占 DB。
...
2. **把关键路径画出来：** 对 `一个 Node 故障后，怎样避免数据库被 MISS 打死？` 至少能指出输入、关键状态、输出以及失败路径；如果涉及 Item，要明确 Hash/LRU/Slab/refcount 中哪些会变化。
3. **用指标证伪：** 明确 origin_budget，并令 allowed_miss_qps≤budget；超过部分必须命中其他层或被限流/降级。
4. **准备一个故障反例：** 单节点故障导致5万 QPS miss，DB只能多承受1万：必须先在应用层把4万以上请求用 stale/L1/coalescing/降级吸收。
5. **版本意识：** 本仓库源码锚定 Memcached `1.6.45`；默认参数与现代特性应以该版本官方源码/文档为准。

## 常见易错点

> 缓存灾备的核心是“控制回源”，不是让 cache 100% 不故障。
...
### 追问 1：如果 DB headroom 只有 10% 怎么办？

**回答提示：** 先限制并发回源、服务 stale、合并请求和降级；不能让 cache 故障把超出 headroom 的请求直接灌给 DB。
### 追问 2：哪些数据可以直接返回 stale？
...
> **第一句给结论：** 组合 request coalescing、origin 并发限流、熔断、stale serving、L1 cache、warm spare/replica、渐进 rehash 和业务降级，让恢复流量受控。  
> **第二层讲机制：** 用 1 张状态/调用链图说明“谁维护什么状态、何时改变”。  
> **第三层讲边界：** 仅让客户端快速 failover 到其他 cache 节点可能把幸存节点打满；容灾不能只追求“请求不报连接错误”。  
> **第四层落生产：** 给出一个指标或算式，例如：明确 origin_budget，并令 allowed_miss_qps≤budget；超过部分必须命中其他层或被限流/降级。

## 专家级展开（V2）

### A. 从第一性原理推导

节点故障后的核心任务是“限制 MISS 传播到 origin”：singleflight、stale、L1、限流、熔断、逐级降级、warm spare/replica 都是在削峰或减少重建次数。

这道题建议使用“**状态 → 所有者 → 转移条件 → 失败后果**”四步法推导，而不是背结论。先确定哪份状态是核心，再问由哪个模块维护，随后说明状态何时迁移，最后指出违反不变量会表现为什么 bug 或性能问题。

### B. 关键源码符号与阅读顺序

- `architecture-level; Meta stale/lease can help`

建议在 `memcached-1.6.45` 源码树中用下面的方式定位，而不是按文件顺序从头读：

```bash
git grep -n "\barchitecture\b" -- storage.c extstore.c proto_proxy.c proxy_config.c proto_text.c
```
...
仅让客户端快速 failover 到其他 cache 节点可能把幸存节点打满；容灾不能只追求“请求不报连接错误”。

面试中主动讲边界条件很加分，因为它能证明你区分了“默认实现”“协议语义”“架构经验”和“数学近似”。尤其要避免把平均 O(1)、默认 1MB、client-side hashing 等表述成无条件真理。

### D. 生产故障推演

单节点故障导致5万 QPS miss，DB只能多承受1万：必须先在应用层把4万以上请求用 stale/L1/coalescing/降级吸收。
...
明确 origin_budget，并令 allowed_miss_qps≤budget；超过部分必须命中其他层或被限流/降级。
...
[← 上一题](../10-advanced-source-system-design/097.md) | [本章目录](README.md) | [下一题 →](../10-advanced-source-system-design/099.md)
