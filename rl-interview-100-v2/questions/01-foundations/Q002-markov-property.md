...
[← Q001](Q001-mdp-five-tuple.md) · [章节首页](README.md) · [Q003 →](Q003-discount-factor.md)
...
> 给定当前状态 S_t 后，下一状态分布不再依赖更早历史：当前状态应是对决策所需历史信息的充分统计量。
...
Markov 性是“条件独立”而不是“无历史依赖”。历史信息可以存在，只要已经被当前状态充分编码。

### 数学/推导抓手

检验形式：P(sₜ₊₁|s₀,a₀,…,sₜ,aₜ)=P(sₜ₊₁|sₜ,aₜ)。在 POMDP 中可用 belief state bₜ(s)=P(sₜ=s|o₀:ₜ,a₀:ₜ₋₁) 恢复 Markov 表示。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 部分可观测任务可用 frame stack、RNN/Transformer history encoder 或显式 belief state。
- 如果同一 observation 下最优动作依赖早期历史，通常说明 state representation 不充分。
...
### 追问 1：frame stacking 为什么有效？

Markov 性是“条件独立”而不是“无历史依赖”。历史信息可以存在，只要已经被当前状态充分编码。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：history encoder 会不会自动让问题变成 MDP？
...
> “观测”不等于“状态”；传感器给出的 o_t 可能只是 partial observation。
...
> **结论先行**：给定当前状态 S_t 后，下一状态分布不再依赖更早历史：当前状态应是对决策所需历史信息的充分统计量。

继续展开时，先把它放回本章的统一问题框架：**形式上是条件独立，而不是物理世界“没有记忆”。；若观测 o_t 不具 Markov 性，可以堆叠帧、加入历史统计、使用 RNN/Transformer，或者维护 belief state。**。随后写出本题最关键的数学对象：`P(S_{t+1}|S_0,A_0,…,S_t,A_t) = P(S_{t+1}|S_t,A_t)`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“一句话解释 Markov Property。”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
