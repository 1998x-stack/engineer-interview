...
[← Q002](Q002-markov-property.md) · [章节首页](README.md) · [Q004 →](Q004-value-q-advantage.md)
...
> γ 让无限时域 return 有界，也表达时间偏好并控制 credit assignment 的有效跨度。

面试开场建议只讲这一层；如果面试官点头继续，再进入后面的推导与 failure mode。

## 3. 深入解析（PDF 原始要点）

- 数学层面：若奖励有界且 γ<1，则无穷和收敛。
- 优化层面：γ 越接近 1，远期影响更强，但方差、bootstrap error 与 credit assignment 难度也增加。
- 可用 1/(1-γ) 粗略理解有效时间尺度，但它不是严格 episode 长度。
...
γ 同时承担数学收敛、时间偏好和有效规划视野三个角色。它不是“越接近 1 越好”的无害超参。

### 数学/推导抓手

有界奖励 |r|≤Rmax 且 γ<1 时，|Gₜ|≤Rmax/(1−γ)。有效 horizon 常用 1/(1−γ) 粗略估计，但真正 credit horizon 还受 episode、λ 与 bootstrapping 影响。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- γ 增大通常放大 target 方差和 value scale。
- 比较实验时 reward scale、γ、episode length 应一起记录，否则 return 不可横向解释。
...
> 不要把 γ 只解释成“未来奖励不重要”，很多任务使用折扣也为了数学和估计稳定性。
...
> **结论先行**：γ 让无限时域 return 有界，也表达时间偏好并控制 credit assignment 的有效跨度。

继续展开时，先把它放回本章的统一问题框架：**数学层面：若奖励有界且 γ<1，则无穷和收敛。；优化层面：γ 越接近 1，远期影响更强，但方差、bootstrap error 与 credit assignment 难度也增加。**。随后写出本题最关键的数学对象：`H_eff ≈ 1/(1-γ)`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“为什么 Return 需要折扣因子 γ？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
