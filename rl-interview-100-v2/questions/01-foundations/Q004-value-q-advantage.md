...
[← Q003](Q003-discount-factor.md) · [章节首页](README.md) · [Q005 →](Q005-bellman-expectation.md)
...
> V 是状态价值，Q 是在状态 s 先执行 a 后的价值，A=Q-V 衡量动作相对该状态平均行为的好坏。
...
V 是策略下“平均动作”的状态价值，Q 是条件于首个动作的价值，A 则是相对于该状态平均水平的中心化增益。A 的中心化性质正是 policy gradient 方差控制的关键。

### 数学/推导抓手

Vπ(s)=Eₐ~π[Qπ(s,a)]，因此 Eₐ~π[Aπ(s,a)]=0。这个零均值性质解释了 advantage 作为 baseline-centered signal 的意义。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 连续动作 actor-critic 中 Q(s,a) 常用于直接优化 actor；PPO 常用 V(s) 形成 GAE。
- 检查 advantage 是否近似零均值、合理方差，是 PPO debug 的第一批指标。
...
> 不能说 advantage “就是 reward”；它包含未来回报与 baseline。
...
> **结论先行**：V 是状态价值，Q 是在状态 s 先执行 a 后的价值，A=Q-V 衡量动作相对该状态平均行为的好坏。

继续展开时，先把它放回本章的统一问题框架：**V^π(s)=E_{a~π}Q^π(s,a)。；Advantage 是 policy gradient 中高频对象：用相对价值代替绝对 return 可降低方差。**。随后写出本题最关键的数学对象：`A^π(s,a) = Q^π(s,a) - V^π(s)`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“V(s)、Q(s,a)、A(s,a) 有什么关系？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
