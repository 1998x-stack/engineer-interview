---
id: Q005
title: "推导 Bellman Expectation Equation。"
...
[← Q004](Q004-value-q-advantage.md) · [章节首页](README.md) · [Q006 →](Q006-bellman-optimality.md)
...
> Bellman 的核心是把长期 return 递归拆成“当前一步奖励 + 下一状态的折扣价值”。

面试开场建议只讲这一层；如果面试官点头继续，再进入后面的推导与 failure mode。

## 3. 深入解析（PDF 原始要点）

- 从 G_t=r_{t+1}+γG_{t+1} 出发，对策略和环境随机性取条件期望。
...
Bellman expectation equation 是“递归分解 return”的恒等式；它不是某个算法专属更新式。DP、TD、critic 学习都只是对这个固定点方程采用不同估计方式。

### 数学/推导抓手

Vπ(s)=Eπ[rₜ₊₁+γVπ(sₜ₊₁)|sₜ=s]。有限状态下写成向量式 V=Rπ+γPπV，因此 V=(I−γPπ)⁻¹Rπ（可逆时）。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 实现 critic target 时必须 stop-gradient/target network，避免把 target 两端同时更新成错误计算图。
- terminal state 应关闭 bootstrap：y=r+γ(1−done)V(s′)。
...
### 追问 1：Bellman operator 为什么是 contraction？
...
> **结论先行**：Bellman 的核心是把长期 return 递归拆成“当前一步奖励 + 下一状态的折扣价值”。

继续展开时，先把它放回本章的统一问题框架：**从 G_t=r_{t+1}+γG_{t+1} 出发，对策略和环境随机性取条件期望。；V 方程对动作按 π 加权，再对转移 P 加权。**。随后写出本题最关键的数学对象：`V^π(s)=Σ_a π(a|s) Σ_{s′} P(s′|s,a)[R(s,a,s′)+γV^π(s′)]`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“推导 Bellman Expectation Equation。”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
