---
id: Q007
title: "Dynamic Programming、Monte Carlo、TD 的区别？"
...
> **题型**：公开面经母题 ｜ **频率**：★★★★★ ｜ **难度**：★★★☆☆ ｜ **岗位**：全方向  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q006](Q006-bellman-optimality.md) · [章节首页](README.md) · [Q008 →](Q008-mc-vs-td.md)
...
> DP 要环境模型并 bootstrap；MC 不要模型但等完整 return；TD 不要模型且 bootstrap，可在线更新。
...
| 环境模型 | 需要 | 不需要 | 不需要 |
| Bootstrap | 是 | 否 | 是 |
| 是否等 episode 结束 | 否 | 通常是 | 否 |
| 典型风险 | model 假设 | 高方差 | bootstrap bias |

## 4.3 Repo v2 专业深化：从第一原则理解

三者最大的区分维度是：是否需要模型、是否 bootstrap、target 的 bias/variance。把它们看成 target construction 的三种方案，比背表格更可靠。

### 数学/推导抓手

MC target=完整 Gₜ；TD(0) target=r+γV(s′)；DP 则对已知 P,R 做期望而非采样。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 短 episode、可并行仿真可容忍 MC；continuing task 常依赖 TD。
- 模型可知但状态巨大时 DP 仍可能不可行，不能把“有模型”误等于“能做 DP”。
...
> “TD 一定优于 MC”错误；不同任务的 bias-variance 与非平稳程度不同。
...
> **结论先行**：DP 要环境模型并 bootstrap；MC 不要模型但等完整 return；TD 不要模型且 bootstrap，可在线更新。

继续展开时，先把它放回本章的统一问题框架：**比较三维：是否需要 model、是否等 episode 结束、是否 bootstrap。；MC target 更接近真实 return，bias 小但 variance 大；TD target 使用当前 value estimate，引入 bias 换更低 variance 和更快更新。**。随后写出本题最关键的数学对象：`TD(0) target = r_t + γV(s_{t+1})`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“Dynamic Programming、Monte Carlo、TD 的区别？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
