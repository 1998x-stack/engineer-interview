...
> **题型**：公开面经母题 ｜ **频率**：★★★★☆ ｜ **难度**：★★★☆☆ ｜ **岗位**：研究/控制  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q009](Q009-td-error.md) · [章节首页](README.md) · [Q011 →](Q011-n-step-return.md)
...
> 从 p(x) 采 N 个独立样本，用样本均值估计 E[f(X)]；标准误差随 N 以 1/√N 缩小。
...
```text
μ̂ = (1/N)Σ_i f(X_i),   Var(μ̂)=Var(f(X))/N
```
...
Monte Carlo 的核心是用样本均值近似期望，并用大数定律/中心极限定理解释一致性和误差缩放。面试时要能从普通期望自然迁移到 importance sampling。

### 数学/推导抓手

μ=E[f(X)]，μ̂=1/N∑f(Xᵢ)，Var(μ̂)=Var(f)/N；标准误差随 1/√N 缩放。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 报告 MC 结果时应带置信区间而非单个均值。
- rare-event 或权重长尾时，增加样本数并不一定是最有效的降方差手段。
...
### 追问 2：importance sampling 权重怎么设计？
...
> 要区分 estimator 的方差和单个样本 f(X) 的方差。
...
> **结论先行**：从 p(x) 采 N 个独立样本，用样本均值估计 E[f(X)]；标准误差随 N 以 1/√N 缩小。

继续展开时，先把它放回本章的统一问题框架：**先写 estimator，再写 expectation 与 variance。；若目标分布难采样可转向 importance sampling。**。随后写出本题最关键的数学对象：`μ̂ = (1/N)Σ_i f(X_i),   Var(μ̂)=Var(f(X))/N`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“手推一个 Monte Carlo 估计问题。”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
