---
id: Q011
title: "n-step Return 为什么连接了 MC 和 TD？"
...
[← Q010](Q010-monte-carlo-estimation.md) · [章节首页](README.md) · [Q012 →](Q012-on-vs-off-policy.md)
...
> n-step return 用前 n 步真实奖励，再在第 n 步 bootstrap；n=1 接近 TD，n 到 episode 末端接近 MC。
...
```text
G_t^(n)=Σ_{k=0}^{n-1}γ^k r_{t+k+1}+γ^nV(s_{t+n})
```
...
n-step return 是从“纯 bootstrap”到“纯采样回报”的连续桥梁。n 增大让真实奖励更快传播，但同时增加方差和 off-policy mismatch。

### 数学/推导抓手

Gₜ⁽ⁿ⁾=∑_{k=0}^{n−1}γᵏrₜ₊ₖ+γⁿV(sₜ₊ₙ)。TD(λ)/GAE 可视为对不同 n-step target 的指数加权。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 分布式 RL 中 n-step 还受 rollout chunk 边界影响，需正确处理 bootstrap value。
...
### 追问 2：Rainbow 为什么使用 multi-step return？
...
> **结论先行**：n-step return 用前 n 步真实奖励，再在第 n 步 bootstrap；n=1 接近 TD，n 到 episode 末端接近 MC。

继续展开时，先把它放回本章的统一问题框架：**n 增大通常降低 bootstrap bias，但环境与策略随机性造成的 variance 增大。；off-policy 下 n-step 还要考虑行为策略与目标策略的 mismatch。**。随后写出本题最关键的数学对象：`G_t^(n)=Σ_{k=0}^{n-1}γ^k r_{t+k+1}+γ^nV(s_{t+n})`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“n-step Return 为什么连接了 MC 和 TD？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
