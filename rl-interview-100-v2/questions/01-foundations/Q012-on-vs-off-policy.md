---
id: Q012
title: "On-policy 与 Off-policy 的本质区别？"
...
[← Q011](Q011-n-step-return.md) · [章节首页](README.md) · [Q013 →](Q013-importance-sampling.md)
...
> 看产生数据的 behavior policy μ 与要学习/评价的 target policy π 是否相同。
...
on/off-policy 的定义来自“数据分布由谁产生”，不是来自是否有 replay buffer。关键对象是 behavior policy μ 与 target policy π。

### 数学/推导抓手

off-policy 评价通常需要校正 d^μ(s)μ(a|s) 与目标分布，importance ratio π(a|s)/μ(a|s) 是最直接但高方差的校正。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- trajectory 必须带 policy version/log-prob 才能量化 policy lag。
- SAC/DQN 可 off-policy 重放；PPO 只能有限复用旧 rollout。
...
### 追问 1：PPO 为什么能重复用同一 rollout 多个 epoch？

on/off-policy 的定义来自“数据分布由谁产生”，不是来自是否有 replay buffer。关键对象是 behavior policy μ 与 target policy π。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：SAC 为什么能长期使用 replay buffer？
...
> 不要用“是否有 replay buffer”作为定义；那只是常见实现现象。
...
> **结论先行**：看产生数据的 behavior policy μ 与要学习/评价的 target policy π 是否相同。

继续展开时，先把它放回本章的统一问题框架：**On-policy 稳定但新旧数据很快过期；Off-policy 可以 replay，提高样本效率但分布偏移更难处理。；SARSA、PPO 常作 on-policy；Q-learning、DQN、DDPG、TD3、SAC 是典型 off-policy。**。随后写出本题最关键的数学对象：`on-policy: μ=π; off-policy: μ≠π`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“On-policy 与 Off-policy 的本质区别？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
