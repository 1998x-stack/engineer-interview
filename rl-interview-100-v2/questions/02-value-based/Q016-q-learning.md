---
id: Q016
title: "简述 Q-learning：目标、更新式、为什么 model-free？"
...
[← Q015](../01-foundations/Q015-exploration-exploitation.md) · [章节首页](README.md) · [Q017 →](Q017-dqn-loss-terminal-mask.md)
...
> Q-learning 用 Bellman optimality target 做 TD 更新，不需要显式知道环境转移模型 P，只依靠采样 transition。

面试开场建议只讲这一层；如果面试官点头继续，再进入后面的推导与 failure mode。

## 3. 深入解析（PDF 原始要点）

- 目标值使用 r+γmax Q(s′,a′)，因此直接逼近 Q*。
...
Q-learning 是 off-policy TD control：利用 Bellman optimality target 学 Q*，不需要环境转移模型。

### 数学/推导抓手

Q←Q+α[r+γmaxₐ′Q(s′,a′)−Q(s,a)]。在表格、充分探索与合适步长条件下可收敛。
...
- 函数逼近后不再自动继承表格收敛保证。
- terminal mask、reward clipping/scale、target stale 都会显著影响深度版本。
...
> **结论先行**：Q-learning 用 Bellman optimality target 做 TD 更新，不需要显式知道环境转移模型 P，只依靠采样 transition。

继续展开时，先把它放回本章的统一问题框架：**目标值使用 r+γmax Q(s′,a′)，因此直接逼近 Q*。；tabular 情况在标准条件下收敛；深度函数逼近后不再自动保证。**。随后写出本题最关键的数学对象：`Q(s,a) ← Q(s,a)+α[r+γmax_{a′}Q(s′,a′)-Q(s,a)]`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“简述 Q-learning：目标、更新式、为什么 model-free？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
