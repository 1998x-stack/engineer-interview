---
id: Q017
title: "写出 DQN Loss，并解释 terminal mask。"
...
> **题型**：公开面经真题 ｜ **频率**：★★★★★ ｜ **难度**：★★★☆☆ ｜ **岗位**：游戏/Agent  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q016](Q016-q-learning.md) · [章节首页](README.md) · [Q018 →](Q018-dqn-two-tricks.md)
...
> DQN 用 online Q 预测当前动作价值，用 frozen target network 构造 bootstrap target。terminal 时不能再 bootstrap。

面试开场建议只讲这一层；如果面试官点头继续，再进入后面的推导与 failure mode。

## 3. 深入解析（PDF 原始要点）

- target y=r+γ(1-done)max Q_target(s′,a′)。
- prediction 只取 Q_online(s,a) 对应执行动作的分量。
...
```text
L(θ)=E[(r+γ(1-d)max_{a′}Q_{θ−}(s′,a′)-Q_θ(s,a))²]
```
...
DQN loss 是对 Bellman optimality target 的回归。terminal mask 决定是否允许从 s′ bootstrap，是最常见且隐蔽的实现错误之一。

### 数学/推导抓手

y=r+γ(1−terminated)maxₐ′Qθ⁻(s′,a′)，L=Huber(Qθ(s,a)−stopgrad(y))。truncated 不一定等价 terminated。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 使用 gather 取已执行 action 的 Q；target 分支必须 no_grad。
- Gymnasium 中 terminated 与 truncated 要分开处理。
...
> 配套代码：[`code/dqn_loss.py`](../../code/dqn_loss.py)。先自己手写，再对照仓库版本。
...
### 追问 1：为什么 Huber 常比 MSE 稳？

DQN loss 是对 Bellman optimality target 的回归。terminal mask 决定是否允许从 s′ bootstrap，是最常见且隐蔽的实现错误之一。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：truncated 与 terminated 怎么处理？
...
> Gymnasium 中 terminated 和 truncated 语义不同；时间上限截断并不一定等价于真正终止。
...
> **结论先行**：DQN 用 online Q 预测当前动作价值，用 frozen target network 构造 bootstrap target。terminal 时不能再 bootstrap。

继续展开时，先把它放回本章的统一问题框架：**target y=r+γ(1-done)max Q_target(s′,a′)。；prediction 只取 Q_online(s,a) 对应执行动作的分量。**。随后写出本题最关键的数学对象：`L(θ)=E[(r+γ(1-d)max_{a′}Q_{θ−}(s′,a′)-Q_θ(s,a))²]`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“写出 DQN Loss，并解释 terminal mask。”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
