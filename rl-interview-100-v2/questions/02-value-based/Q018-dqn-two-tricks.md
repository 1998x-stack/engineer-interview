...
[← Q017](Q017-dqn-loss-terminal-mask.md) · [章节首页](README.md) · [Q019 →](Q019-experience-replay.md)
...
> Experience Replay 打破时间相关性并复用数据；Target Network 让 bootstrap target 在一段时间内相对稳定。
...
flowchart LR
  ...
  Replay --> Online[Online Q Network]
  Target[Target Q Network] --> TD[TD Target]
  Online --> Loss[TD Loss]
  TD --> Loss
  Loss --> Online
  Online -. periodic/soft copy .-> Target
```
...
Experience Replay 解决样本相关性与复用，Target Network 解决 moving target；二者分别从数据分布和优化目标两侧稳定 DQN。

### 数学/推导抓手

在线网络 θ 负责预测/选动作，目标网络 θ⁻ 在一段时间内近似固定 Bellman target。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- replay 太小导致强相关，太旧则 behavior distribution 可能严重失配。
- target 更新太快退化为 moving target，太慢则 target stale。
...
### 追问 1：为什么 CNN 不算这两个核心 trick？

Experience Replay 解决样本相关性与复用，Target Network 解决 moving target；二者分别从数据分布和优化目标两侧稳定 DQN。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：replay buffer 太小/太大会怎样？
...
> 回答只背名称不够，必须说清各自对应的 failure mode。
...
> **结论先行**：Experience Replay 打破时间相关性并复用数据；Target Network 让 bootstrap target 在一段时间内相对稳定。

继续展开时，先把它放回本章的统一问题框架：**Replay 把序列数据转成近似 iid minibatch，同时使算法更明显 off-policy。；Target network 解决 moving target；可 hard copy 或 Polyak averaging。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“DQN 最经典的两个 trick 是什么？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
