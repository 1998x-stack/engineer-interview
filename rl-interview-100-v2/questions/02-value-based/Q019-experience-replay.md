---
id: Q019
title: "Experience Replay 为什么有效？有什么代价？"
...
> **题型**：公开面经母题 ｜ **频率**：★★★★★ ｜ **难度**：★★★☆☆ ｜ **岗位**：游戏/Agent  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q018](Q018-dqn-two-tricks.md) · [章节首页](README.md) · [Q020 →](Q020-target-network.md)
...
> 它通过随机回放 transition 降低 temporal correlation，并让一次昂贵环境交互被多次利用。
...
Replay 的价值不只在“打乱相关性”，还在于把一次昂贵环境交互转成多次 SGD 更新；代价是数据陈旧与分布偏移。

### 数学/推导抓手

replay ratio≈每个环境 transition 被用于多少次 gradient update，是 sample efficiency 与 overfitting 的关键系统超参。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 监控 buffer age、action/reward 分布和 replay ratio。
- 非平稳环境中旧数据可能从资产变成负担。
...
### 追问 1：为什么 on-policy PPO 不常用长 replay？

Replay 的价值不只在“打乱相关性”，还在于把一次昂贵环境交互转成多次 SGD 更新；代价是数据陈旧与分布偏移。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：如何做 reservoir replay？
...
> Replay Buffer 不是越大越好：过旧数据可能增加 mismatch。
...
> **结论先行**：它通过随机回放 transition 降低 temporal correlation，并让一次昂贵环境交互被多次利用。

继续展开时，先把它放回本章的统一问题框架：**优势：sample efficiency、近 iid、可做 prioritized sampling。；代价：数据来自旧策略；环境非平稳时旧数据可能失效；buffer 分布会影响学习目标。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“Experience Replay 为什么有效？有什么代价？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
