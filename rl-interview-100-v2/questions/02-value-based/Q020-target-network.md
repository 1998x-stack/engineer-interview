---
id: Q020
title: "Target Network 为什么能稳定 DQN？"
...
[← Q019](Q019-experience-replay.md) · [章节首页](README.md) · [Q021 →](Q021-deadly-triad.md)
...
> 若 prediction 和 target 同时由快速变化的同一个网络产生，优化对象本身持续移动；target network 将这一变化放慢。

面试开场建议只讲这一层；如果面试官点头继续，再进入后面的推导与 failure mode。

## 3. 深入解析（PDF 原始要点）

- Hard update 每 C 步复制 θ−←θ。
...
Target Network 用慢变量把“自举目标”暂时冻结，使回归问题局部上更接近监督学习。

### 数学/推导抓手

hard: θ⁻←θ 每 C 步；Polyak: θ⁻←τθ+(1−τ)θ⁻。后者相当于参数的指数滑动平均。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 不要误把 τ 的定义方向写反；不同代码库常用 tau=0.005 或 polyak=0.995 两种记法。
...
### 追问 2：为什么 actor-critic 也常有 target critic？
...
> 不要说 target network “消除 bias”；它主要提高稳定性。
...
> **结论先行**：若 prediction 和 target 同时由快速变化的同一个网络产生，优化对象本身持续移动；target network 将这一变化放慢。

继续展开时，先把它放回本章的统一问题框架：**Hard update 每 C 步复制 θ−←θ。；Soft update θ−←τθ+(1-τ)θ−。**。随后写出本题最关键的数学对象：`θ− ← τθ + (1-τ)θ−`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“Target Network 为什么能稳定 DQN？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
