...
> **题型**：经典高频 ｜ **频率**：★★★★★ ｜ **难度**：★★★☆☆ ｜ **岗位**：游戏/Agent  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q021](Q021-deadly-triad.md) · [章节首页](README.md) · [Q023 →](Q023-dueling-dqn.md)
...
> 普通 DQN 用同一估计器做 action selection 与 evaluation，max 会偏好噪声高估项；Double DQN 把选择和评价拆开。

面试开场建议只讲这一层；如果面试官点头继续，再进入后面的推导与 failure mode。

## 3. 深入解析（PDF 原始要点）

- online network 选 a*=argmax Q_online(s′,a)。
- target network 评估 y=r+γQ_target(s′,a*)。
...
```text
a*=argmax_a Q_θ(s′,a);  y=r+γQ_{θ−}(s′,a*)
```
...
Double DQN 解耦“谁来选最大动作”和“谁来给这个动作估值”，降低 max 对噪声的系统性正偏。

### 数学/推导抓手

a*=argmaxₐQθ(s′,a)，y=r+γQθ⁻(s′,a*)。与 DQN 的 maxₐQθ⁻ 同时选择和评估不同。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- Double 并不意味着一定有两个完全独立网络；在线/目标网络已经提供了两套参数角色。
...
### 追问 2：Double Q-learning 与 TD3 双 critic 有何联系？
...
> “两个网络”不是 Double DQN 的本质；DQN 本来就有 online/target，关键是 selection/evaluation 解耦。
...
> **结论先行**：普通 DQN 用同一估计器做 action selection 与 evaluation，max 会偏好噪声高估项；Double DQN 把选择和评价拆开。

继续展开时，先把它放回本章的统一问题框架：**online network 选 a*=argmax Q_online(s′,a)。；target network 评估 y=r+γQ_target(s′,a*)。**。随后写出本题最关键的数学对象：`a*=argmax_a Q_θ(s′,a);  y=r+γQ_{θ−}(s′,a*)`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“Double DQN 解决什么问题？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
