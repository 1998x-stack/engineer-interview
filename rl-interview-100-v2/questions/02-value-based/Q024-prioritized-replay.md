---
id: Q024
title: "Prioritized Experience Replay 为什么使用 TD error？"
...
[← Q023](Q023-dueling-dqn.md) · [章节首页](README.md) · [Q025 →](Q025-dqn-n-step.md)
...
> |δ| 大的 transition 表示当前模型预测误差大，通常包含更多学习信息，因此提高其采样概率。
...
- P(i)∝p_i^α，p_i=|δ_i|+ε。
- 改变采样分布会引入 bias，因此使用 IS 权重 w_i=(1/(NP(i)))^β 修正。
...
PER 用 TD error 近似样本的“当前学习价值”，优先采样高 surprise transition；但这会改变训练分布，因此需要 IS 权重修正。

### 数学/推导抓手

P(i)=pᵢ^α/∑pⱼ^α，wᵢ=(N·P(i))^{-β}。α 控制 prioritization，β 控制 bias correction。
...
- priority 要防 0：p=|δ|+ε。
- 极端 outlier 可长期霸占采样，常需 clip 或 rank-based priority。
...
> 高 TD error 不等于“高价值样本”，可能来自随机奖励或异常点。
...
> **结论先行**：|δ| 大的 transition 表示当前模型预测误差大，通常包含更多学习信息，因此提高其采样概率。

继续展开时，先把它放回本章的统一问题框架：**P(i)∝p_i^α，p_i=|δ_i|+ε。；改变采样分布会引入 bias，因此使用 IS 权重 w_i=(1/(NP(i)))^β 修正。**。随后写出本题最关键的数学对象：`P(i)=p_i^α/Σ_kp_k^α`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“Prioritized Experience Replay 为什么使用 TD error？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
