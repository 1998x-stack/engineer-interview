---
id: Q025
title: "DQN 加 n-step return 为什么有效？"
...
[← Q024](Q024-prioritized-replay.md) · [章节首页](README.md) · [Q026 →](Q026-distributional-rl.md)
...
> 多步真实奖励让 credit 更快传播，减少纯一步 bootstrap 的慢传播问题。
...
n-step DQN 让稀疏/延迟 reward 更快传播到前面的 state-action，但 n 越大越依赖 behavior trajectory，off-policy 偏差也越明显。

### 数学/推导抓手

y⁽ⁿ⁾=∑_{k=0}^{n−1}γᵏrₖ+γⁿmaxₐQ(sₙ,a)。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 遇到 episode terminal 要提前截断 n-step 累积；并行 env 要独立维护 n-step queue。
...
### 追问 1：n-step 与 eligibility trace 的关系？

n-step DQN 让稀疏/延迟 reward 更快传播到前面的 state-action，但 n 越大越依赖 behavior trajectory，off-policy 偏差也越明显。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：replay 中如何存 n-step transition？
...
> **结论先行**：多步真实奖励让 credit 更快传播，减少纯一步 bootstrap 的慢传播问题。

继续展开时，先把它放回本章的统一问题框架：**在 sparse/delayed reward 场景尤其有价值。；n 增大后 variance 与 off-policy mismatch 增大。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“DQN 加 n-step return 为什么有效？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
