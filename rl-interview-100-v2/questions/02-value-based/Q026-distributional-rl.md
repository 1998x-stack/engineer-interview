---
id: Q026
title: "什么是 Distributional RL？为什么不只学期望 Q？"
...
> **题型**：经典高频 ｜ **频率**：★★★☆☆ ｜ **难度**：★★★★☆ ｜ **岗位**：研究/游戏  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q025](Q025-dqn-n-step.md) · [章节首页](README.md) · [Q027 →](Q027-rainbow-dqn.md)
...
> 传统 Q 只学 return 的均值；Distributional RL 学整个随机 return Z(s,a) 的分布，保留不确定性和分布形状信息。
...
Distributional RL 学 return 随机变量 Z(s,a) 的分布，而不是只学其均值 Q=E[Z]。不同未来风险结构可有相同 Q，但分布学习常提供更丰富的训练信号。

### 数学/推导抓手

distributional Bellman: Z(s,a) ᴰ= R+γZ(s′,A′)。C51 用固定原子 support + projection。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 不要把 distributional RL 等同于 risk-sensitive RL；若最终仍按期望选动作，策略目标可以仍是 risk-neutral。
...
### 追问 1：C51 projection 为什么需要？

Distributional RL 学 return 随机变量 Z(s,a) 的分布，而不是只学其均值 Q=E[Z]。不同未来风险结构可有相同 Q，但分布学习常提供更丰富的训练信号。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：quantile regression DQN 与 C51 区别？
...
> **结论先行**：传统 Q 只学 return 的均值；Distributional RL 学整个随机 return Z(s,a) 的分布，保留不确定性和分布形状信息。

继续展开时，先把它放回本章的统一问题框架：**C51 用固定 atoms 近似 value distribution。；即便最终决策仍按期望值，分布学习也可能改善 representation 与优化。**。随后写出本题最关键的数学对象：`Q(s,a)=E[Z(s,a)]`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“什么是 Distributional RL？为什么不只学期望 Q？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
