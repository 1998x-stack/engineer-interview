---
id: Q030
title: "写出 Policy Gradient Theorem，并解释 log-derivative trick。"
...
> **题型**：核心推导 ｜ **频率**：★★★★★ ｜ **难度**：★★★★☆ ｜ **岗位**：全方向  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q029](Q029-value-vs-policy.md) · [章节首页](README.md) · [Q031 →](Q031-reinforce.md)
...
> 把 trajectory probability 对参数的梯度写成 log probability 梯度，就能用采样估计不可微环境中的期望回报梯度。

面试开场建议只讲这一层；如果面试官点头继续，再进入后面的推导与 failure mode。

## 3. 深入解析（PDF 原始要点）

- 环境转移 P 通常不依赖 θ，因此 trajectory log probability 的 θ 梯度只剩 Σ log πθ(a_t|s_t)。
...
Policy Gradient Theorem 的核心价值是：梯度不需要对未知环境转移 P 求导；只需对策略 log-prob 求导并用 Q/advantage 加权。

### 数学/推导抓手

∇J=E_{dπ(s),a~π}[∇θlogπθ(a|s)Qπ(s,a)]。log-derivative: ∇π=π∇logπ。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 实现离散策略时使用 distribution.log_prob；连续 Gaussian policy 注意 tanh squash 的 log-det Jacobian。
...
> 不要把 policy gradient 当成对 action 本身求梯度；stochastic PG 对 log-prob 求梯度。
...
> **结论先行**：把 trajectory probability 对参数的梯度写成 log probability 梯度，就能用采样估计不可微环境中的期望回报梯度。

继续展开时，先把它放回本章的统一问题框架：**环境转移 P 通常不依赖 θ，因此 trajectory log probability 的 θ 梯度只剩 Σ log πθ(a_t|s_t)。；用 return/Q/advantage 作为权重，得到 REINFORCE / actor-critic。**。随后写出本题最关键的数学对象：`∇J(θ)=E[∇θ log πθ(a|s) · Q^π(s,a)]`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“写出 Policy Gradient Theorem，并解释 log-derivative trick。”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
