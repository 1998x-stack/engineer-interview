---
id: Q032
title: "为什么加 baseline 不改变 Policy Gradient 的期望？"
...
[← Q031](Q031-reinforce.md) · [章节首页](README.md) · [Q033 →](Q033-actor-critic.md)
...
> 因为对任意只依赖 state 的 b(s)，E_{a~π}[∇logπ(a|s)b(s)]=b(s)∇Σ_aπ(a|s)=0。
...
baseline 不改变期望，是因为对 action-independent b(s)，E_{a~π}[∇logπ(a|s)b(s)]=0；它只重排随机梯度的方差。

### 数学/推导抓手

b(s)∑ₐπ(a|s)∇logπ(a|s)=b(s)∇∑ₐπ(a|s)=0。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- baseline 不能依赖当前采样 action，否则一般会引入 bias，除非采用特殊 correction。
...
### 追问 1：baseline 能依赖 action 吗？

baseline 不改变期望，是因为对 action-independent b(s)，E_{a~π}[∇logπ(a|s)b(s)]=0；它只重排随机梯度的方差。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：为什么 advantage normalization 仍常见？
...
> **结论先行**：因为对任意只依赖 state 的 b(s)，E_{a~π}[∇logπ(a|s)b(s)]=b(s)∇Σ_aπ(a|s)=0。

继续展开时，先把它放回本章的统一问题框架：**baseline 只改变 estimator variance，不改变期望方向。；最优 baseline 与 Q 的条件结构有关；V(s) 是常用近似。**。随后写出本题最关键的数学对象：`E_a[∇logπ(a|s)b(s)] = 0`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“为什么加 baseline 不改变 Policy Gradient 的期望？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
