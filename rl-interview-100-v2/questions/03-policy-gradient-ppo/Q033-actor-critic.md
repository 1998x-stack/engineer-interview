---
id: Q033
title: "Actor-Critic 为什么比 REINFORCE 更实用？"
...
[← Q032](Q032-baseline-unbiasedness.md) · [章节首页](README.md) · [Q034 →](Q034-a2c-vs-a3c.md)
...
> Critic 用 learned value 提供低方差 advantage/TD signal，Actor 用该信号更新 policy，实现 policy improvement 与 evaluation 的闭环。
...
Actor-Critic 用 critic 把 MC return 替换为低方差的 value/advantage estimate，代价是 critic bias 会传给 actor。

### 数学/推导抓手

actor: −E[logπ(a|s)Â]；critic: E[(V(s)−Vtarget)²]。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- critic underfit：advantage 噪声大；critic overfit/错误 bootstrap：actor 被系统性误导。
- 监控 explained variance 比单看 value loss 更有解释力。
...
### 追问 2：actor 与 critic 是否共享 backbone？
...
> 不要把 critic 等同 reward model；critic 预测未来回报，reward 定义任务目标。
...
> **结论先行**：Critic 用 learned value 提供低方差 advantage/TD signal，Actor 用该信号更新 policy，实现 policy improvement 与 evaluation 的闭环。

继续展开时，先把它放回本章的统一问题框架：**critic 可学 V 或 Q；常见 on-policy AC 用 V 做 baseline。；优点是能在线、多步 bootstrap；缺点是 critic bias 会传给 actor。**。随后写出本题最关键的数学对象：`actor update ∝ ∇logπ(a|s)Â(s,a)`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“Actor-Critic 为什么比 REINFORCE 更实用？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
