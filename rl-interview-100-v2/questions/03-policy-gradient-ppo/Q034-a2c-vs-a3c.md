...
> **题型**：公开面经母题 ｜ **频率**：★★★★☆ ｜ **难度**：★★★☆☆ ｜ **岗位**：游戏/分布式RL  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q033](Q033-actor-critic.md) · [章节首页](README.md) · [Q035 →](Q035-a3c-staleness.md)
...
> A3C 多 worker 异步更新 global network；A2C 常同步收集多个环境 rollout 后统一 batch 更新。
...
A3C 的异步 worker 用参数服务器式更新提升并行探索；A2C 用同步批次换取更一致的梯度与现代 GPU 友好性。

### 数学/推导抓手

两者都用 advantage actor-critic objective，差别主要在 rollout/gradient synchronization 机制。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 现代实现更常 vectorized synchronous env + batched learner，因为 GPU 批处理效率通常优于原始异步 CPU worker。
...
### 追问 1：A3C 为什么不需要 replay buffer？

A3C 的异步 worker 用参数服务器式更新提升并行探索；A2C 用同步批次换取更一致的梯度与现代 GPU 友好性。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：同步 barrier 的代价是什么？
...
> **结论先行**：A3C 多 worker 异步更新 global network；A2C 常同步收集多个环境 rollout 后统一 batch 更新。

继续展开时，先把它放回本章的统一问题框架：**A3C 通过异步 actor 打破样本相关性，但会引入 stale gradient / policy lag。；A2C 更利于 GPU batch，工程上更简单可控。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“A2C 与 A3C 有什么区别？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
