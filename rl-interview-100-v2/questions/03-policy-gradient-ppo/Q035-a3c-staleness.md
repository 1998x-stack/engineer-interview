...
> **题型**：公开面经真题 ｜ **频率**：★★★★☆ ｜ **难度**：★★★★☆ ｜ **岗位**：分布式RL  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q034](Q034-a2c-vs-a3c.md) · [章节首页](README.md) · [Q036 →](Q036-gae.md)
...
> worker 用旧 policy 收集并计算梯度，而 global policy 已更新，造成 stale gradient 与 off-policy drift；并发越大，policy lag 可能越严重。
...
A3C 不稳定的根源之一是 stale policy/stale gradient：worker 采样和计算期间全局参数已经变化，梯度对应的分布与当前 learner 不一致。

### 数学/推导抓手

policy lag 可用 KL(π_actor || π_learner) 或 parameter/version gap 量化。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 增加 worker 并不保证线性加速；actor throughput 超过 learner 会积压越来越旧的数据。
...
### 追问 1：IMPALA 的 V-trace 解决什么？

A3C 不稳定的根源之一是 stale policy/stale gradient：worker 采样和计算期间全局参数已经变化，梯度对应的分布与当前 learner 不一致。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：actor/learner throughput 如何平衡？
...
> **结论先行**：worker 用旧 policy 收集并计算梯度，而 global policy 已更新，造成 stale gradient 与 off-policy drift；并发越大，policy lag 可能越严重。

继续展开时，先把它放回本章的统一问题框架：**不同 worker 的梯度方差叠加，环境分布也可能不同。；参数服务器写入次序、optimizer state 与异步更新会进一步复杂化。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“A3C 异步训练为什么可能不收敛？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
