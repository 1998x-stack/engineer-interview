---
id: Q039
title: "写出 PPO Clipped Objective，并解释每一项。"
...
[← Q038](Q038-why-ppo.md) · [章节首页](README.md) · [Q040 →](Q040-ppo-ratio.md)
...
> r_t 衡量新策略相对旧策略对已采 action 的概率变化；Â 表示该动作相对 baseline 的好坏；min+clip 构造悲观 surrogate，限制过激改动的收益。

面试开场建议只讲这一层；如果面试官点头继续，再进入后面的推导与 failure mode。

## 3. 深入解析（PDF 原始要点）

- r_t=exp(logπ_new-logπ_old) 数值更稳定。
- A>0 时不鼓励 r 超过 1+ε；A<0 时不鼓励 r 低于 1-ε。
...
```text
L^CLIP(θ)=E[min(r_t(θ)Â_t, clip(r_t(θ),1−ε,1+ε)Â_t)]；r_t(θ)=π_θ(a_t|s_t)/π_old(a_t|s_t)
```
...
Clipped objective 是对每个样本构造的悲观 surrogate：只有当新策略的概率变化仍处于可信区域时，优势带来的收益才继续增长。

### 数学/推导抓手

A>0: min(r,1+ε)A；A<0: max(r,1−ε)A（由 min 与负 A 等价推出）。因此正负 advantage 的有效截断方向相反。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 重点监控 ratio p1/p50/p99、clipfrac、approx_kl、advantage std。
- clipfrac 高不一定马上失败，但意味着很多样本进入“无继续收益”区，数据利用效率下降。
...
> PPO clip 的是 probability ratio，不是 reward、advantage 或 gradient。
...
> **结论先行**：r_t 衡量新策略相对旧策略对已采 action 的概率变化；Â 表示该动作相对 baseline 的好坏；min+clip 构造悲观 surrogate，限制过激改动的收益。

继续展开时，先把它放回本章的统一问题框架：**r_t=exp(logπ_new-logπ_old) 数值更稳定。；A>0 时不鼓励 r 超过 1+ε；A<0 时不鼓励 r 低于 1-ε。**。随后写出本题最关键的数学对象：`L^CLIP(θ)=E[min(r_t(θ)Â_t, clip(r_t(θ),1−ε,1+ε)Â_t)]；r_t(θ)=π_θ(a_t|s_t)/π_old(a_t|s_t)`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“写出 PPO Clipped Objective，并解释每一项。”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
