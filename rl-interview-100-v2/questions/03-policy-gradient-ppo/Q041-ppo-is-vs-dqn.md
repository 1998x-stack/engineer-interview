---
id: Q041
title: "为什么 PPO 需要 Importance Sampling，而 DQN 不需要？"
chapter: "第三章 Policy Gradient / Actor-Critic / PPO"
source_type: "公开面经真题"
frequency: "★★★★★"
difficulty: "★★★★☆"
roles: "PPO/游戏"
tags: ["ppo", "policy-gradient", "importance-sampling", "dqn", "value-based"]
---

# Q041 为什么 PPO 需要 Importance Sampling，而 DQN 不需要？

> **题型**：公开面经真题 ｜ **频率**：★★★★★ ｜ **难度**：★★★★☆ ｜ **岗位**：PPO/游戏  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q040](Q040-ppo-ratio.md) · [章节首页](README.md) · [Q042 →](Q042-ppo-clip-cases.md)
...
> PPO 用 π_old 数据估计 π_new 的 policy objective，因此需 likelihood ratio；Q-learning 通过 off-policy Bellman optimality backup 学 Q*，不是直接对 π_new likelihood 做同样估计。
...
PPO 的目标是当前策略下的 policy gradient，但数据来自旧策略，因此 ratio 是显式分布校正；DQN 的 Bellman optimality update 天生允许 behavior 与 target 不同。

### 数学/推导抓手

PPO: E_{a~πnew}[A] 用 old sample 重写为 E_{a~πold}[rA]；Q-learning 不做这个积分重写。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 如果 PPO rollout 太旧，ratio correction 也会因高方差失效，不能把 IS 当无限数据复用许可证。
...
### 追问 1：SAC actor update 为什么不需要 behavior policy ratio？

PPO 的目标是当前策略下的 policy gradient，但数据来自旧策略，因此 ratio 是显式分布校正；DQN 的 Bellman optimality update 天生允许 behavior 与 target 不同。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：IMPALA 为什么要 V-trace？
...
> 避免循环回答：“因为 PPO on-policy，DQN off-policy”。要从目标函数解释。
...
> **结论先行**：PPO 用 π_old 数据估计 π_new 的 policy objective，因此需 likelihood ratio；Q-learning 通过 off-policy Bellman optimality backup 学 Q*，不是直接对 π_new likelihood 做同样估计。

继续展开时，先把它放回本章的统一问题框架：**PPO 多 epoch 进一步加大新旧策略差异，所以 ratio/clip 很关键。；DQN 虽不使用 PPO 式 IS，仍面临 state/action distribution mismatch。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“为什么 PPO 需要 Importance Sampling，而 DQN 不需要？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
