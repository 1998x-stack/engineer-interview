...
[← Q046](Q046-ppo-multi-epoch.md) · [章节首页](README.md) · [Q048 →](Q048-implement-ppo-gae.md)
...
> 主要问题是 on-policy rollout 贵、critic 成本高、超参敏感、长 horizon credit assignment 难，且在 LLM 中序列长度和巨型 action space 放大这些问题。
...
PPO 的核心缺点是 on-policy rollout 昂贵、critic/advantage 依赖强、超参耦合，以及长序列/大动作空间下 token-level 更新与 sequence reward 的 credit mismatch。

### 数学/推导抓手

sample reuse 受 trust-region 限制；不能像 replay-based off-policy 算法那样任意重放。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 仿真便宜时 PPO 的稳定性常胜过 sample-efficiency；真实机器人或 LLM rollout 昂贵时成本压力更明显。
...
> **结论先行**：主要问题是 on-policy rollout 贵、critic 成本高、超参敏感、长 horizon credit assignment 难，且在 LLM 中序列长度和巨型 action space 放大这些问题。

继续展开时，先把它放回本章的统一问题框架：**样本效率：数据快速过期。；系统成本：actor/critic/reference/reward 多模型并存。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“PPO 有哪些缺点？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
