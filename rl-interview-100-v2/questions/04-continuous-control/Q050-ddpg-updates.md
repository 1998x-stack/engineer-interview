---
id: Q050
title: "DDPG 的 Actor 和 Critic 如何更新？"
...
> **题型**：公开面经母题 ｜ **频率**：★★★★☆ ｜ **难度**：★★★★☆ ｜ **岗位**：机器人/控制  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q049](Q049-continuous-action-dqn.md) · [章节首页](README.md) · [Q051 →](Q051-ddpg-instability.md)
...
> Critic 用 off-policy TD target 学 Q；Actor 通过 deterministic policy gradient 沿着 critic 对 action 的梯度方向提升 Q。

面试开场建议只讲这一层；如果面试官点头继续，再进入后面的推导与 failure mode。

## 3. 深入解析（PDF 原始要点）

- target action 来自 target actor μ−(s′)。
- critic target y=r+γQ−(s′,μ−(s′))。
- actor gradient 通过 ∂Q/∂a · ∂μ/∂θ 反传。
...
DDPG 是 deterministic actor + off-policy critic。critic 学 Bellman target，actor 沿 Q 对 action 的梯度把动作推向更高价值。

### 数学/推导抓手

y=r+γQφ⁻(s′,μθ⁻(s′)); ∇θJ=E[∇aQφ(s,a)|a=μθ(s) ∇θμθ(s)]。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- actor loss 常写 −Q(s,actor(s)).mean()。
- target actor/critic 使用 Polyak averaging。
...
> 配套代码：[`code/ddpg_losses.py`](../../code/ddpg_losses.py)。先自己手写，再对照仓库版本。
...
### 追问 1：DDPG exploration 怎么做？

DDPG 是 deterministic actor + off-policy critic。critic 学 Bellman target，actor 沿 Q 对 action 的梯度把动作推向更高价值。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：为什么 deterministic policy 还能 off-policy？
...
> actor 最大化的是 critic 估计，critic 偏差会直接被 actor exploit。
...
> **结论先行**：Critic 用 off-policy TD target 学 Q；Actor 通过 deterministic policy gradient 沿着 critic 对 action 的梯度方向提升 Q。

继续展开时，先把它放回本章的统一问题框架：**target action 来自 target actor μ−(s′)。；critic target y=r+γQ−(s′,μ−(s′))。**。随后写出本题最关键的数学对象：`∇θJ = E[∇_a Q(s,a)|_{a=μ(s)} ∇θ μθ(s)]`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“DDPG 的 Actor 和 Critic 如何更新？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
