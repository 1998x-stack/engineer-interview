---
id: Q056
title: "PPO、TD3、SAC 如何选？"
chapter: "第四章 DDPG / TD3 / SAC 连续控制"
source_type: "项目高频"
frequency: "★★★★★"
difficulty: "★★★☆☆"
roles: "机器人/控制"
tags: ["ppo", "policy-gradient", "importance-sampling", "sac", "maximum-entropy", "td3", "continuous-control", "temporal-difference"]
---

# Q056 PPO、TD3、SAC 如何选？

> **题型**：项目高频 ｜ **频率**：★★★★★ ｜ **难度**：★★★☆☆ ｜ **岗位**：机器人/控制  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q055](Q055-sac-temperature.md) · [章节首页](README.md) · [Q057 →](Q057-continuous-action-discretization.md)
...
> 仿真便宜、需要简单稳定并行时常选 PPO；真实采样昂贵时 off-policy TD3/SAC 更具样本效率；SAC 比 TD3 更强调 stochastic exploration 与稳健性。
...
| 算法 | 范式 | 动作 | Replay | 优势 | 典型代价 |
|---|---|---|---|---|---|
| PPO | on-policy | 离散/连续 | 否 | 简单稳定、并行友好 | 样本效率低 |
| TD3 | off-policy | 连续 | 是 | 高样本效率 | 确定性策略探索敏感 |
| SAC | off-policy | 连续 | 是 | 最大熵、稳定、探索强 | 温度/尺度需调 |

## 4.3 Repo v2 专业深化：从第一原则理解

算法选择要围绕环境交互成本、动作空间、是否允许 replay、稳定性与部署约束，而不是按 leaderboard 排名。

### 数学/推导抓手

PPO：on-policy stochastic；TD3：off-policy deterministic；SAC：off-policy maximum-entropy stochastic。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 昂贵真实交互优先考虑 off-policy；海量廉价并行仿真可优先 PPO。
- 多峰最优动作或需要随机策略时 SAC 通常比 TD3 更自然。
...
### 追问 1：真实机器人为什么不能只看 benchmark return？

算法选择要围绕环境交互成本、动作空间、是否允许 replay、稳定性与部署约束，而不是按 leaderboard 排名。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：action latency 如何进入设计？
...
> **结论先行**：仿真便宜、需要简单稳定并行时常选 PPO；真实采样昂贵时 off-policy TD3/SAC 更具样本效率；SAC 比 TD3 更强调 stochastic exploration 与稳健性。

继续展开时，先把它放回本章的统一问题框架：**PPO：on-policy、实现稳、数据消耗大。；TD3：deterministic，连续控制强，但探索与超参更敏感。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“PPO、TD3、SAC 如何选？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
