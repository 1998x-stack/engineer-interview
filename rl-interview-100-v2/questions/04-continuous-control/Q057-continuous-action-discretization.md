...
> **题型**：基础扩展 ｜ **频率**：★★★★☆ ｜ **难度**：★★☆☆☆ ｜ **岗位**：机器人/控制  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q056](Q056-ppo-td3-sac-selection.md) · [章节首页](README.md) · [Q058 →](Q058-topk-vs-sac.md)
...
> 动作维数 d、每维 K 档时组合数 K^d 指数爆炸；分辨率越细，动作空间越不可控。
...
离散化的主要问题是组合爆炸和控制分辨率/计算量 tradeoff。每维 K 档、d 维动作即 K^d 个组合。

### 数学/推导抓手

若要保持每维误差 ≤ε，K 常随区间/ε 增长，整体动作数指数依赖 d。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 可用 factorized action head 缓解但会引入动作维度独立性假设。
...
### 追问 1：hybrid action space 怎么做？

离散化的主要问题是组合爆炸和控制分辨率/计算量 tradeoff。每维 K 档、d 维动作即 K^d 个组合。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：离散 skill + 连续参数怎么建模？
...
> **结论先行**：动作维数 d、每维 K 档时组合数 K^d 指数爆炸；分辨率越细，动作空间越不可控。

继续展开时，先把它放回本章的统一问题框架：**离散化还会产生控制抖动和精度瓶颈。；某些低维控制仍可离散化，不能绝对化。**。随后写出本题最关键的数学对象：`N_actions = K^d`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“连续动作为什么不能简单 discretize？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
