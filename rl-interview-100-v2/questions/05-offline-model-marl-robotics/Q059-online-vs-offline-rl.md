---
id: Q059
title: "Online RL 与 Offline RL 有什么区别？"
...
> **题型**：高级高频 ｜ **频率**：★★★★☆ ｜ **难度**：★★★☆☆ ｜ **岗位**：推荐/机器人/研究  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q058](../04-continuous-control/Q058-topk-vs-sac.md) · [章节首页](README.md) · [Q060 →](Q060-offline-q-failure.md)
...
> Online RL 可持续与环境交互；Offline RL 只有固定 dataset，核心难点是学到的 policy 会访问数据外 OOD action/state。
...
Offline RL 的约束是训练期间不能通过新交互纠正错误，所以 dataset coverage 与 learned policy distribution shift 成为中心问题。

### 数学/推导抓手

D固定来自 behavior μ；目标 π 若访问 D 中低密度 (s,a)，Q 的 extrapolation error 可能被 policy optimization 放大。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 必须报告 dataset quality/coverage，而不能只报算法。
- 真实业务还要考虑 logging policy、selection bias 与 action support。
- 先描述数据覆盖和行为策略，再谈算法。
...
### 追问 1：offline-to-online 怎么做？

Offline RL 的约束是训练期间不能通过新交互纠正错误，所以 dataset coverage 与 learned policy distribution shift 成为中心问题。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：off-policy RL 就等于 offline RL 吗？
...
> off-policy 只描述数据策略与目标策略不同；offline 还意味着训练期间不能获取新数据。
...
> **结论先行**：Online RL 可持续与环境交互；Offline RL 只有固定 dataset，核心难点是学到的 policy 会访问数据外 OOD action/state。

继续展开时，先把它放回本章的统一问题框架：**Offline 的目标不只是 imitation，而是用 reward 实现 policy improvement。；无法在线纠错使 value extrapolation error 特别危险。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“Online RL 与 Offline RL 有什么区别？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
