---
id: Q060
title: "为什么普通 Q-learning 直接做 Offline RL 容易失败？"
chapter: "第五章 Offline RL / Model-based / MARL / Sim2Real"
source_type: "核心高级"
frequency: "★★★★☆"
difficulty: "★★★★☆"
roles: "研究/推荐"
tags: ["q-learning", "value-based", "offline-rl"]
---

# Q060 为什么普通 Q-learning 直接做 Offline RL 容易失败？

> **题型**：核心高级 ｜ **频率**：★★★★☆ ｜ **难度**：★★★★☆ ｜ **岗位**：研究/推荐  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q059](Q059-online-vs-offline-rl.md) · [章节首页](README.md) · [Q061 →](Q061-bc-vs-offline-rl.md)
...
> max/actor 会选择 dataset 未覆盖的动作，而 Q 网络对 OOD action 的值没有可靠监督，容易产生虚高估计并被策略 exploit。
...
普通 Q-learning 在 offline setting 会对未见动作做 max 并 bootstrap；函数逼近器对 OOD action 的任意误差会被 max 选择并反复放大。

### 数学/推导抓手

a*=argmaxₐQ(s,a) 可落在 μ(a|s)≈0 区域；随后 target 继续用这个 Q。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 观察 learned action 与 dataset action 的距离/密度，是离线 RL 诊断关键。
- 先描述数据覆盖和行为策略，再谈算法。
...
> **结论先行**：max/actor 会选择 dataset 未覆盖的动作，而 Q 网络对 OOD action 的值没有可靠监督，容易产生虚高估计并被策略 exploit。

继续展开时，先把它放回本章的统一问题框架：**bootstrap 会把 OOD 误差继续传播。；这形成 extrapolation error → policy selects OOD → 更大误差的反馈环。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“为什么普通 Q-learning 直接做 Offline RL 容易失败？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
