---
id: Q061
title: "Behavior Cloning 与 Offline RL 的区别？"
...
> **题型**：高级母题 ｜ **频率**：★★★★☆ ｜ **难度**：★★★☆☆ ｜ **岗位**：机器人/推荐  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q060](Q060-offline-q-failure.md) · [章节首页](README.md) · [Q062 →](Q062-cql.md)
...
> BC 只拟合行为策略动作分布，不使用 reward 做真正 policy improvement；Offline RL 试图在固定数据上利用 reward 超过 behavior policy。
...
BC 只拟合 behavior action，不利用 reward 做策略改进；Offline RL 希望在不出 support 太远的条件下利用 reward 对数据内行为重新加权/改进。

### 数学/推导抓手

BC: max E_D logπ(a|s)；AWR/IQL 类方法则用 exp(βA) 对高价值动作提高权重。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 高质量 expert-only 数据上 BC 往往是强 baseline；offline RL 不应默认一定胜过 BC。
- 先描述数据覆盖和行为策略，再谈算法。
...
### 追问 1：BC 为什么有 compounding error？
...
> 不要把 BC 称为 RL；它通常是 supervised imitation learning。
...
> **结论先行**：BC 只拟合行为策略动作分布，不使用 reward 做真正 policy improvement；Offline RL 试图在固定数据上利用 reward 超过 behavior policy。

继续展开时，先把它放回本章的统一问题框架：**BC 稳定、不会主动走向 OOD，但受限于示范质量。；Offline RL 更有潜力，但必须限制策略偏离 dataset support。**。随后写出本题最关键的数学对象：`L_BC=-E_D[logπ(a|s)]`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“Behavior Cloning 与 Offline RL 的区别？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
