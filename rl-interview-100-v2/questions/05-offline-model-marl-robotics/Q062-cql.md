...
[← Q061](Q061-bc-vs-offline-rl.md) · [章节首页](README.md) · [Q063 →](Q063-iql.md)
...
> CQL 在 Bellman error 外增加 conservative regularizer，压低整体尤其 OOD action 的 Q，同时保持 dataset action 的价值，降低 policy exploit 虚高 Q。

面试开场建议只讲这一层；如果面试官点头继续，再进入后面的推导与 failure mode。

## 3. 深入解析（PDF 原始要点）

- 典型项 logsumexp_a Q(s,a)-E_{a~D}Q(s,a)。
...
先写普通 TD loss，再增加“抬高数据动作 Q、压低全动作 Q”的保守正则；关键是把 OOD action 的乐观估计变得昂贵。

### 4.2 关键公式

```text
L_CQL = L_TD + α[logΣ_a exp Q(s,a) - E_{a~D}Q(s,a)]
```
...
CQL 的思想是“对未被数据支持的高 Q 保持悲观”：在 Bellman error 之外加入 Q regularizer，降低 policy 对 OOD action 的虚高估计。

### 数学/推导抓手

离散形式常见 α[E_s log∑ₐexpQ(s,a)−E_{(s,a)~D}Q(s,a)] + Bellman loss。第一项抬高“所有动作”惩罚压力，第二项把数据动作拉回。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- α 太大会过度保守，策略退化接近 behavior。
- 连续动作下 logsumexp 需用采样近似。
- 先描述数据覆盖和行为策略，再谈算法。
...
- α 太大怎样？
- continuous action 下 logsumexp 怎么近似？
...
### 追问 2：continuous action 下 logsumexp 怎么近似？
...
> **结论先行**：CQL 在 Bellman error 外增加 conservative regularizer，压低整体尤其 OOD action 的 Q，同时保持 dataset action 的价值，降低 policy exploit 虚高 Q。

继续展开时，先把它放回本章的统一问题框架：**典型项 logsumexp_a Q(s,a)-E_{a~D}Q(s,a)。；可理解为让 learned Q 对 policy value 更保守。**。随后写出本题最关键的数学对象：`L_CQL = L_TD + α[logΣ_a exp Q(s,a) - E_{a~D}Q(s,a)]`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“CQL 的核心思想是什么？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
