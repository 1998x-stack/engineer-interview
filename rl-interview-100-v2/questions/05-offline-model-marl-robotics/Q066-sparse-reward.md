---
id: Q066
title: "Sparse Reward 怎么解决？先诊断什么？"
...
> **题型**：公开项目真题 ｜ **频率**：★★★★★ ｜ **难度**：★★★☆☆ ｜ **岗位**：游戏/机器人  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q065](Q065-reward-shaping.md) · [章节首页](README.md) · [Q067 →](Q067-intrinsic-motivation.md)
...
> 先区分主要瓶颈是 exploration 还是 credit assignment；前者需要更好探索/课程，后者需要更好的 value/return 传播。
...
稀疏奖励必须先区分 exploration failure 与 credit-assignment failure：没到过成功状态，靠更好 advantage estimator 没用；到过成功但学不回去，才是 credit assignment 更核心。

### 数学/推导抓手

探索类：intrinsic/curriculum/demo/HER；credit 类：n-step/GAE/hierarchical return decomposition。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 统计“成功状态到访率”和“到访后前置动作学习速度”可帮助区分两类问题。
- 先描述数据覆盖和行为策略，再谈算法。
...
> 不要把所有 sparse reward 都粗暴改成 dense handcrafted reward。
...
> **结论先行**：先区分主要瓶颈是 exploration 还是 credit assignment；前者需要更好探索/课程，后者需要更好的 value/return 传播。

继续展开时，先把它放回本章的统一问题框架：**exploration：intrinsic reward、self-play、curriculum、demonstrations。；credit：n-step、GAE、hierarchical decomposition。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“Sparse Reward 怎么解决？先诊断什么？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
