---
id: Q067
title: "Intrinsic Motivation 的基本思想与 noisy-TV 问题？"
...
> **题型**：高级扩展 ｜ **频率**：★★★☆☆ ｜ **难度**：★★★★☆ ｜ **岗位**：游戏/研究  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q066](Q066-sparse-reward.md) · [章节首页](README.md) · [Q068 →](Q068-self-play-curriculum.md)
...
> 给 agent 额外内部奖励鼓励 novelty、prediction error 或信息增益，但随机不可预测却无任务价值的状态可能长期吸引 agent，形成 noisy-TV。
...
Intrinsic motivation 用预测误差、访问新颖度等内部信号补充外部 reward。noisy-TV 表明“不可预测”不等于“有学习价值”。

### 数学/推导抓手

r=r_ext+βr_int；若 r_int≈prediction error，随机噪声状态可能永久保持高误差。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 使用 episodic novelty、可控性/learning progress 等信号可降低 noisy-TV。
- 先描述数据覆盖和行为策略，再谈算法。
...
### 追问 1：intrinsic reward 何时 anneal？

Intrinsic motivation 用预测误差、访问新颖度等内部信号补充外部 reward。noisy-TV 表明“不可预测”不等于“有学习价值”。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：episodic novelty 和 lifelong novelty 区别？
...
> prediction error 高可能只是不可约噪声，不一定意味着值得探索。
...
> **结论先行**：给 agent 额外内部奖励鼓励 novelty、prediction error 或信息增益，但随机不可预测却无任务价值的状态可能长期吸引 agent，形成 noisy-TV。

继续展开时，先把它放回本章的统一问题框架：**count-based / pseudo-count 奖励稀有状态。；RND 用随机网络预测误差衡量 novelty。**。随后写出本题最关键的数学对象：`r = r_ext + β r_int`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“Intrinsic Motivation 的基本思想与 noisy-TV 问题？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
