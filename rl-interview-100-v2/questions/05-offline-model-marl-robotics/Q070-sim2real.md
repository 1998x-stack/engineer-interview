...
> **题型**：公开项目真题 ｜ **频率**：★★★★☆ ｜ **难度**：★★★★☆ ｜ **岗位**：机器人  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q069](Q069-marl-qmix.md) · [章节首页](README.md) · [Q071 →](../06-llm-post-training-rl/Q071-rlhf-pipeline.md)
...
> 核心是缩小 simulator 与现实动力学/观测差异：domain randomization、system identification、adaptation、residual policy、真实数据 fine-tune。
...
Sim2Real 的目标不是让 simulator 完美，而是让训练策略对真实系统误差不敏感。Domain Randomization 用参数分布覆盖可能的真实动力学。

### 数学/推导抓手

训练目标变成 E_{ξ~p(ξ)} J(π;M_ξ)，ξ 可包括质量、摩擦、延迟、噪声。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 随机范围过窄覆盖不足，过宽则任务变难且学到过度保守策略。
- 真实部署前应做 system identification 与安全约束测试。
- 先描述数据覆盖和行为策略，再谈算法。
...
### 追问 1：latency randomization 为什么重要？

Sim2Real 的目标不是让 simulator 完美，而是让训练策略对真实系统误差不敏感。Domain Randomization 用参数分布覆盖可能的真实动力学。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：安全 sim2real 如何做？
...
> Sim2Real 不只是视觉 domain shift，动力学与控制时延往往更关键。
...
> **结论先行**：核心是缩小 simulator 与现实动力学/观测差异：domain randomization、system identification、adaptation、residual policy、真实数据 fine-tune。

继续展开时，先把它放回本章的统一问题框架：**随机 mass、friction、delay、sensor noise、motor strength、terrain 等。；若随机范围太窄，覆盖不到现实；太宽则策略过度保守、难学。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“机器人 Sim2Real 怎么做？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
