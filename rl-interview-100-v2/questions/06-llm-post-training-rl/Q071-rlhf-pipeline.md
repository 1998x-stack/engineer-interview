...
[← Q070](../05-offline-model-marl-robotics/Q070-sim2real.md) · [章节首页](README.md) · [Q072 →](Q072-why-rl-after-sft.md)
...
> 经典流程是 SFT → preference data → Reward Model → PPO/策略优化，并用 reference/KL 控制策略偏离。现代体系还包括 DPO、verifier RL、GRPO 等变体。
...
```mermaid
flowchart LR
  PT[Pretrained LM] --> SFT
  SFT --> Pref[Preference Data]
  Pref --> RM[Reward Model]
  SFT --> PPO[Policy Optimization]
  RM --> PPO
...
经典 RLHF 可以拆成三种学习信号：SFT 提供行为先验，preference/RM 提供 sequence-level 目标，RL 在当前 policy 分布上搜索并优化；reference KL 防止策略无约束漂移。

### 数学/推导抓手

常见 RL objective: E_{y~πθ}[rφ(x,y)−β log(πθ(y|x)/πref(y|x))]。PPO 再用 old policy ratio 实现稳定更新。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 区分四个模型角色：actor、reference、reward、critic；它们的参数冻结与显存放置是系统设计核心。
- 实际系统要记录 prompt→response→reward components→policy version 全链路 provenance。
...
### 追问 1：RLAIF 与 RLHF 差别？

经典 RLHF 可以拆成三种学习信号：SFT 提供行为先验，preference/RM 提供 sequence-level 目标，RL 在当前 policy 分布上搜索并优化；reference KL 防止策略无约束漂移。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：为什么 reasoning RL 常用 verifier 而非 RM？
...
> 不要把“RLHF”只等同 PPO；它是更广的基于人类反馈对齐范式。
...
> **结论先行**：经典流程是 SFT → preference data → Reward Model → PPO/策略优化，并用 reference/KL 控制策略偏离。现代体系还包括 DPO、verifier RL、GRPO 等变体。

继续展开时，先把它放回本章的统一问题框架：**SFT 学人类示范行为分布。；RM 从偏好对学习 sequence-level scalar reward。**。随后写出本题最关键的数学对象：`max_π E[r_φ(x,y) - β KL(π || π_ref)]`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“完整讲一下 RLHF Pipeline。”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
