...
[← Q071](Q071-rlhf-pipeline.md) · [章节首页](README.md) · [Q073 →](Q073-reward-model.md)
...
> SFT 是 token-level behavior cloning，无法直接优化 correctness、preference、tool success 等 sequence-level 目标；RL 可以用不可微 verifier/reward 直接优化期望任务收益。
...
SFT 只能模仿数据中的 token-level 行为；RL 可以直接优化不可微、sequence-level、交互式的目标，并通过当前 policy rollout 探索 SFT 数据之外的新解。

### 数学/推导抓手

SFT 最小化 −logπ(y*|x)；RL 最大化 E_{y~π}[R(x,y)]，目标与数据似然不再等价。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 可验证任务（代码测试、数学 checker）是 RL 最有价值的区域之一；开放文本无可靠 reward 时 RL 风险更高。
...
### 追问 1：为什么 preference pair 不能简单 chosen-only SFT？
...
> **结论先行**：SFT 是 token-level behavior cloning，无法直接优化 correctness、preference、tool success 等 sequence-level 目标；RL 可以用不可微 verifier/reward 直接优化期望任务收益。

继续展开时，先把它放回本章的统一问题框架：**SFT 仍是重要 warm start，保证语言与指令基本能力。；RL 的价值在于 policy exploration：可以发现示范数据中没有的新解法。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“已经有 SFT，为什么还需要 RL？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
