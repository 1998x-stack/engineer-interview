---
id: Q074
title: "Reward Model 准确率很高，为什么 RL 还是可能训练坏？"
...
> **题型**：公开面经追问 ｜ **频率**：★★★★★ ｜ **难度**：★★★★☆ ｜ **岗位**：LLM Post-training  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q073](Q073-reward-model.md) · [章节首页](README.md) · [Q075 →](Q075-reference-model.md)
...
> RM 的离线准确率是在训练/验证分布上测的；policy 优化会把生成分布推向 RM 未见区域，并主动 exploit approximation error，形成 Goodhart/reward hacking。
...
RM 验证准确率高并不能保证被优化时稳健，因为 RL policy 会主动搜索 RM 的误差最大区域；这是“预测模型面对优化器”的 Goodhart/reward hacking 问题。

### 数学/推导抓手

训练数据分布 D_RM 与逐步更新后的 D_π 分离，argmax_y r̂(y) 会放大 r̂−r_true 的局部误差。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 同时跟踪 RM reward 与独立 verifier/human eval 的 gap。
- 用 reward ensemble、adversarial red-team、online relabeling 减轻。
...
> **结论先行**：RM 的离线准确率是在训练/验证分布上测的；policy 优化会把生成分布推向 RM 未见区域，并主动 exploit approximation error，形成 Goodhart/reward hacking。

继续展开时，先把它放回本章的统一问题框架：**policy shift 越大，RM OOD 越严重。；RM 与 policy 同源模型还可能共享盲点。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“Reward Model 准确率很高，为什么 RL 还是可能训练坏？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
