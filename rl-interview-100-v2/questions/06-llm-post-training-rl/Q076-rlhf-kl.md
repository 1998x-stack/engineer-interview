---
id: Q076
title: "KL 在 RLHF 中出现在哪里？过大和过小意味着什么？"
...
[← Q075](Q075-reference-model.md) · [章节首页](README.md) · [Q077 →](Q077-why-ppo-rlhf.md)
...
> 常见有两类 KL：policy 与 reference 的长期正则，以及 policy 新旧版本的 trust-region/训练稳定指标。过大表示漂移，过小可能更新过于保守。
...
```text
D_KL(π_θ || π_ref)=E_{y~π_θ}[logπ_θ(y)-logπ_ref(y)]
```
...
RLHF 中 KL 是“性能收益 vs 偏离先验”的控制旋钮。太大说明 update 激进或 reward 强诱导；太小说明 policy 几乎没学或 KL 系数太强。

### 数学/推导抓手

reward shaping 常用 r_total=r_RM−β(logπ−logπ_ref) 的 token/sequence 聚合近似。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 按 prompt 难度/长度切 KL 分布；平均 KL 会掩盖少数极端 response。
- 自适应 β 可围绕 target KL 调整。
...
### 追问 1：reverse KL 与 forward KL 行为差别？

RLHF 中 KL 是“性能收益 vs 偏离先验”的控制旋钮。太大说明 update 激进或 reward 强诱导；太小说明 policy 几乎没学或 KL 系数太强。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：sampled KL estimator 会为负吗？
...
> 单样本 log-ratio 可以为负，真正 KL 的期望非负。
...
> **结论先行**：常见有两类 KL：policy 与 reference 的长期正则，以及 policy 新旧版本的 trust-region/训练稳定指标。过大表示漂移，过小可能更新过于保守。

继续展开时，先把它放回本章的统一问题框架：**reference KL 可放到 reward shaping 或 loss penalty。；token-level KL 的估计方式与方向会影响数值。**。随后写出本题最关键的数学对象：`D_KL(π_θ || π_ref)=E_{y~π_θ}[logπ_θ(y)-logπ_ref(y)]`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“KL 在 RLHF 中出现在哪里？过大和过小意味着什么？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
