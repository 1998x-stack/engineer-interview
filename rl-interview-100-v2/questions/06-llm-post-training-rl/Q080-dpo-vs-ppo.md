---
id: Q080
title: "DPO vs PPO，什么时候选哪个？"
chapter: "第六章 RLHF / DPO / GRPO / DAPO / GSPO"
source_type: "公开面经真题"
frequency: "★★★★★"
difficulty: "★★★☆☆"
roles: "LLM Post-training"
tags: ["ppo", "policy-gradient", "importance-sampling", "dpo", "preference-optimization"]
---
...
[← Q079](Q079-dpo.md) · [章节首页](README.md) · [Q081 →](Q081-grpo-vs-ppo.md)
...
> 有高质量固定偏好对、追求简单稳定时 DPO 很合适；有可验证 reward、希望在线探索新解法时 PPO/GRPO 类 online RL 更有优势。
...
| 维度 | DPO | PPO/RLHF |
|---|---|---|
| 训练数据 | 固定 preference pair | 当前 policy rollout |
| Reward Model | 不显式需要 | 通常需要/或 verifier |
| Critic | 不需要 | PPO 通常需要 |
| 在线探索 | 弱 | 强 |
| 工程复杂度 | 低 | 高 |
...
DPO 与 PPO 的关键不是“新旧算法谁更强”，而是 offline preference fitting 与 online reward-driven exploration 的差别。

### 数学/推导抓手

DPO 只在固定 pair D 上优化；PPO/RL 可从当前 π 继续采新 y 并获得 verifier/reward。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 静态人类偏好、成本敏感场景：DPO 类更简单；可验证推理、需要发现新 trajectory：online RL 更有优势。
...
### 追问 1：DPO 能否 online 化？

DPO 与 PPO 的关键不是“新旧算法谁更强”，而是 offline preference fitting 与 online reward-driven exploration 的差别。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：为什么 reasoning 模型更偏 verifier RL？
...
> 不要把比较变成“DPO 新所以更好”；要看 reward 是否可在线验证。
...
> **结论先行**：有高质量固定偏好对、追求简单稳定时 DPO 很合适；有可验证 reward、希望在线探索新解法时 PPO/GRPO 类 online RL 更有优势。

继续展开时，先把它放回本章的统一问题框架：**DPO 训练成本近似普通 fine-tuning，但受数据边界约束。；在线 RL 可以持续生成 hard trajectories，但 rollout 与系统复杂度高。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“DPO vs PPO，什么时候选哪个？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
