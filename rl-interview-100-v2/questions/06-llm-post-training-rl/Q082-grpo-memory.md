---
id: Q082
title: "PPO vs GRPO 为什么 GRPO 更省显存？"
...
[← Q081](Q081-grpo-vs-ppo.md) · [章节首页](README.md) · [Q083 →](Q083-grpo-zero-signal-group.md)
...
> PPO 通常需要大规模 value/critic 模型及 optimizer states；GRPO 用组内 reward 做 baseline，省去 critic 及其梯度、optimizer、通信。
...
GRPO 省显存的直接来源是移除 critic/value model 及其 optimizer states/activation；但总训练成本不一定同比下降，因为每个 prompt 需要多 completion rollout。

### 数学/推导抓手

PPO memory≈actor+critic(+optimizers)+reference/RM；GRPO 去掉 critic 路径，但增加 G-way sampling。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 判断系统收益要看“显存、rollout tokens、learner FLOPs、wall-clock”四项，而不是只看模型个数。
...
> 显存节省与总 FLOPs/端到端 wall-clock 不是同一指标。
...
> **结论先行**：PPO 通常需要大规模 value/critic 模型及 optimizer states；GRPO 用组内 reward 做 baseline，省去 critic 及其梯度、optimizer、通信。

继续展开时，先把它放回本章的统一问题框架：**LLM 32B/70B 时少一个同规模模型价值巨大。；但 GRPO rollout 数量更高，推理侧算力可能上升。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“PPO vs GRPO 为什么 GRPO 更省显存？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
