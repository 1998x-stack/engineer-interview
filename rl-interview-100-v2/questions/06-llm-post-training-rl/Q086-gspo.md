---
id: Q086
title: "GSPO 为什么改用 Sequence-level Importance Ratio？"
chapter: "第六章 RLHF / DPO / GRPO / DAPO / GSPO"
source_type: "2026 真题"
frequency: "★★★★★"
difficulty: "★★★★★"
roles: "LLM Reasoning"
tags: ["gspo", "sequence-level"]
---

# Q086 GSPO 为什么改用 Sequence-level Importance Ratio？

> **题型**：2026 真题 ｜ **频率**：★★★★★ ｜ **难度**：★★★★★ ｜ **岗位**：LLM Reasoning  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q085](Q085-dapo.md) · [章节首页](README.md) · [Q087 →](Q087-ppo-grpo-dapo-gspo.md)
...
> GSPO 把优化单位从 token-level ratio 提升到 sequence-level ratio，使 importance correction 与 sequence-level reward 的粒度更一致，并改善长序列与 MoE RL 的稳定性。
...
```text
s_i(θ)=exp((1/|y_i|)Σ_t[logπ_θ(y_{i,t}|x,y_{i,<t})−logπ_old(y_{i,t}|x,y_{i,<t})])
```
...
GSPO 把 importance ratio 的基本单位从 token 改为 sequence likelihood，希望让优化粒度与 sequence-level reward 更一致，并提升尤其 MoE RL 的稳定性。

### 数学/推导抓手

sequence ratio 可由 response token log-ratio 聚合后指数化（论文定义含长度归一化细节）；同一 response 的 tokens 共享 sequence-level clipping/weighting。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 需要关注长序列下 log-ratio 聚合的数值稳定与长度归一化。
- GSPO牺牲部分 token 级细粒度 credit，换取 sequence 一致性。
...
### 追问 2：sequence-level clip 对 fine-grained credit assignment 有何代价？
...
> GSPO 的 ratio 不是简单“把 token ratio 相乘后直接 clip”；论文显式做 1/|y| 长度归一化。
...
> **结论先行**：GSPO 把优化单位从 token-level ratio 提升到 sequence-level ratio，使 importance correction 与 sequence-level reward 的粒度更一致，并改善长序列与 MoE RL 的稳定性。

继续展开时，先把它放回本章的统一问题框架：**GSPO 认为 GRPO 的 token-level importance weight 只基于每个位置的一次 token sample，容易把高方差噪声注入长序列梯度。；它对整条 response 做 sequence-level clipping，并让同一 response 的 token 在梯度中共享同一 sequence weight。**。随后写出本题最关键的数学对象：`s_i(θ)=exp((1/|y_i|)Σ_t[logπ_θ(y_{i,t}|x,y_{i,<t})−logπ_old(y_{i,t}|x,y_{i,<t})])`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“GSPO 为什么改用 Sequence-level Importance Ratio？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
