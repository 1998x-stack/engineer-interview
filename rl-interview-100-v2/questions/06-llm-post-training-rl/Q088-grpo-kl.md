---
id: Q088
title: "GRPO 中 KL 怎么加？与 PPO reference KL 的关系？"
...
[← Q087](Q087-ppo-grpo-dapo-gspo.md) · [章节首页](README.md) · [Q089 →](Q089-agentic-process-reward.md)
...
> GRPO 仍可对 policy 与 reference 计算 token/sequence KL 作为正则或 reward penalty；它去掉的是 critic，不是 reference 约束。
...
GRPO 的 KL 通常仍是 policy 对 reference policy 的正则，它与“old policy ratio 用于 PPO-style update”是两个不同参照关系：old 控制本轮更新，reference 控制长期偏离。

### 数学/推导抓手

可写 objective≈L_clip−β KL(πθ||πref)；old_logp 出现在 ratio，ref_logp 出现在 KL。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 日志中分别保存 old_logp 与 ref_logp，混用会产生很隐蔽的训练错误。
- KL 的 token 聚合/sequence 归一化会影响长度偏置。
...
### 追问 1：KL penalty 放 reward 还是 loss 有区别吗？

GRPO 的 KL 通常仍是 policy 对 reference policy 的正则，它与“old policy ratio 用于 PPO-style update”是两个不同参照关系：old 控制本轮更新，reference 控制长期偏离。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：reference refresh 会怎样影响探索？
...
> **结论先行**：GRPO 仍可对 policy 与 reference 计算 token/sequence KL 作为正则或 reward penalty；它去掉的是 critic，不是 reference 约束。

继续展开时，先把它放回本章的统一问题框架：**常见实现用 sampled log-ratio 近似 KL penalty。；β 控制 reward exploitation 与语言先验保持。**。随后写出本题最关键的数学对象：`r̃ = r_task - β·KL(πθ || πref)`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“GRPO 中 KL 怎么加？与 PPO reference KL 的关系？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
