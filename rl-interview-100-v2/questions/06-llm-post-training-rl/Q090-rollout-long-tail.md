---
id: Q090
title: "GRPO 长尾 rollout 导致 GPU 利用率低，怎么解决？"
chapter: "第六章 RLHF / DPO / GRPO / DAPO / GSPO"
source_type: "2026 工程真题"
frequency: "★★★★★"
difficulty: "★★★★★"
roles: "LLM RL Infra"
...
> **题型**：2026 工程真题 ｜ **频率**：★★★★★ ｜ **难度**：★★★★★ ｜ **岗位**：LLM RL Infra  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q089](Q089-agentic-process-reward.md) · [章节首页](README.md) · [Q091 →](../07-debug-infra-system-design/Q091-reward-all-zero-one.md)
...
> 自回归长度差异导致 straggler：短序列 GPU 空等长序列。要从 dynamic batching、length bucketing、async rollout、continuous batching、max-length curriculum 与资源解耦处理。
...
长尾 rollout 的系统问题是同步 batch 被最慢序列拖住：大多数 GPU 已完成却等待少数超长 response。解决方向是动态批处理、异步 rollout、长度感知调度与超长策略。

### 数学/推导抓手

同步 step wall-time≈max_i T_i，而有效计算利用取决于 mean(T)/max(T)。尾部越重浪费越大。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 按预测长度 bucketing；continuous batching；异步 producer-consumer；设置软长度惩罚/合理 max tokens。
- 不要只截断：需评估截断对 reward 与学习信号的偏置。
...
### 追问 2：为什么 speculative decoding 可能帮助？
...
> 不要只说“加 GPU”；straggler 是调度与长度分布问题。
...
> **结论先行**：自回归长度差异导致 straggler：短序列 GPU 空等长序列。要从 dynamic batching、length bucketing、async rollout、continuous batching、max-length curriculum 与资源解耦处理。

继续展开时，先把它放回本章的统一问题框架：**按预测长度/历史长度分桶，减少同 batch 长短混杂。；rollout 服务与 learner 解耦，使用异步队列隐藏长尾。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“GRPO 长尾 rollout 导致 GPU 利用率低，怎么解决？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
