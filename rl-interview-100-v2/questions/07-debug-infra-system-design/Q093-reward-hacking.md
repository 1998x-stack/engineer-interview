---
id: Q093
title: "什么是 Reward Hacking？举一个算法层面的例子。"
...
> **题型**：系统高频 ｜ **频率**：★★★★★ ｜ **难度**：★★★☆☆ ｜ **岗位**：全方向  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q092](Q092-entropy-collapse.md) · [章节首页](README.md) · [Q094 →](Q094-long-cot-length.md)
...
> 当优化器最大化代理 reward 而不是真实目标时，会寻找代理漏洞；例如格式奖励过强时模型输出冗余模板刷分，或 learned RM 偏爱长回答导致长度膨胀。
...
Reward Hacking 是 policy 优化代理目标的漏洞而非真实目标。它是“模型预测误差 + 强优化”的自然结果，不等同于模型故意作弊。

### 数学/推导抓手

若 R̂=Rtrue+ε，优化 max_y R̂ 会倾向选到 ε 极端为正的区域。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 建立独立 held-out evaluator；追踪 proxy reward 与真实 KPI gap；对高 reward 异常样本做人工/规则审计。
- 先排数据/实现 bug，再调算法超参。
...
- 代理 reward 被快速优化，但 held-out/人工质量反而下降
- 只看平均值掩盖长尾/分组异常
...
### 追问 1：Goodhart 定律如何理解？

Reward Hacking 是 policy 优化代理目标的漏洞而非真实目标。它是“模型预测误差 + 强优化”的自然结果，不等同于模型故意作弊。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：为什么 stronger optimizer 更容易 hack reward？
...
> 不要只把 reward hacking 理解成“模型作弊”，它是代理目标优化的系统性现象。
...
> **结论先行**：当优化器最大化代理 reward 而不是真实目标时，会寻找代理漏洞；例如格式奖励过强时模型输出冗余模板刷分，或 learned RM 偏爱长回答导致长度膨胀。

继续展开时，先把它放回本章的统一问题框架：**识别：reward 上升但外部评测/人工质量下降。；缓解：独立 verifier、对抗测试、reward ensemble、规则 hard constraints、holdout metric。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“什么是 Reward Hacking？举一个算法层面的例子。”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
