---
id: Q094
title: "为什么 Long-CoT RL 容易越来越长？"
chapter: "第七章 Debug / RL Infra / System Design"
source_type: "LLM 工程高频"
frequency: "★★★★★"
difficulty: "★★★★☆"
roles: "LLM Reasoning"
...
> **题型**：LLM 工程高频 ｜ **频率**：★★★★★ ｜ **难度**：★★★★☆ ｜ **岗位**：LLM Reasoning  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q093](Q093-reward-hacking.md) · [章节首页](README.md) · [Q095 →](Q095-rollout-system.md)
...
> 若 reward 只看正确性，长 reasoning 提供更多搜索/自检机会，policy 会学到“延长计算”作为提升成功率的策略；若无成本项就可能过度思考。
...
```text
R_total = R_correct - λ·cost(length)
```
...
Long-CoT 越来越长常来自 reward 对“更多尝试/更多自检”存在隐性正激励，而 token 成本未被纳入目标；硬截断又会引入不连续惩罚。

### 数学/推导抓手

若 P(correct|length) 上升而 reward 只看 correctness，则最优策略可能把长度推到上限。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 画 reward/accuracy vs length 曲线；加入 token cost、软 overlong shaping、长度分层评估。
- 先排数据/实现 bug，再调算法超参。
...
### 追问 1：如何避免模型为了短而牺牲正确率？

Long-CoT 越来越长常来自 reward 对“更多尝试/更多自检”存在隐性正激励，而 token 成本未被纳入目标；硬截断又会引入不连续惩罚。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：test-time compute 与 training reward 怎么匹配？
...
> **结论先行**：若 reward 只看正确性，长 reasoning 提供更多搜索/自检机会，policy 会学到“延长计算”作为提升成功率的策略；若无成本项就可能过度思考。

继续展开时，先把它放回本章的统一问题框架：**长度与正确率常非线性相关。；硬截断 len>L→0 reward 会制造不连续噪声。**。随后写出本题最关键的数学对象：`R_total = R_correct - λ·cost(length)`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“为什么 Long-CoT RL 容易越来越长？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
