---
id: Q096
title: "Actor-Learner 架构中的 Policy Lag 是什么？"
...
> **题型**：系统设计 ｜ **频率**：★★★★☆ ｜ **难度**：★★★★☆ ｜ **岗位**：分布式RL  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q095](Q095-rollout-system.md) · [章节首页](README.md) · [Q097 →](Q097-game-rl-design.md)
...
> actor 用旧版本 π_k 生成数据时 learner 已到 π_{k+m}，trajectory 对当前策略越来越 off-policy，导致 importance ratio 极端与训练不稳定。
...
Policy Lag 是 actor 采样 policy 与 learner 当前 policy 的差异。系统越异步、learner 更新越快，lag 越大，数据越 off-policy。

### 数学/推导抓手

可用 policy_version gap 或 sampled KL(π_actor||π_learner) 度量。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 设置最大允许 version lag；trajectory 超期丢弃/降权；加快 weight broadcast。
- 同时监控 queue age，避免只看参数版本。
- 先排数据/实现 bug，再调算法超参。
...
> **结论先行**：actor 用旧版本 π_k 生成数据时 learner 已到 π_{k+m}，trajectory 对当前策略越来越 off-policy，导致 importance ratio 极端与训练不稳定。

继续展开时，先把它放回本章的统一问题框架：**用 policy_version 标签测 lag。；限制允许最大版本差；及时 weight broadcast。**。随后写出本题最关键的数学对象：`lag = learner_version - rollout_version`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“Actor-Learner 架构中的 Policy Lag 是什么？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
