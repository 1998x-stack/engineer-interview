---
id: Q098
title: "System Design：设计完整 LLM Reasoning RL Pipeline。"
chapter: "第七章 Debug / RL Infra / System Design"
source_type: "终极系统题"
frequency: "★★★★★"
difficulty: "★★★★★"
roles: "LLM Reasoning/Infra"
tags: ["reinforcement-learning"]
---

# Q098 System Design：设计完整 LLM Reasoning RL Pipeline。

> **题型**：终极系统题 ｜ **频率**：★★★★★ ｜ **难度**：★★★★★ ｜ **岗位**：LLM Reasoning/Infra  
> **来源层级**：本页“PDF 原始要点”来自仓库内原版 PDF；“Repo 扩展解析”是在不改变原结论的前提下新增的理论、工程与面试组织内容。

[← Q097](Q097-game-rl-design.md) · [章节首页](README.md) · [Q099 →](Q099-implement-grpo.md)
...
> 从 prompt curriculum 到多样本 rollout、verifier、group statistics、advantage、policy update、KL/entropy、evaluation、hard prompt mining 建立闭环，并对数据版本和 policy 版本全链路可追踪。
...
```text
success = capability gain - λ1·drift - λ2·cost - λ3·regression
```

```mermaid
flowchart TD
  Pool[Prompt Curriculum] --> Rollout[Multi-sample Rollout]
  Rollout --> Verify[Verifier / RM]
  Verify --> Filter[Signal Filtering]
  Filter --> Adv[Advantage]
  Adv --> Update[GRPO/DAPO/GSPO Update]
  Update --> Eval[Held-out Evaluation]
  Eval --> Mine[Hard Prompt Mining]
  Mine --> Pool
```
...
LLM Reasoning RL pipeline 应形成闭环：prompt curriculum→多样 rollout→可验证 reward→有效样本选择→policy update→独立 eval→失败簇回流。算法与系统不能分开设计。

### 数学/推导抓手

核心统计单位既有 prompt/group、sequence，也有 token；每层 normalization 必须显式定义。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 至少记录 reward、accuracy/pass@k、KL、entropy、clipfrac、length、zero-variance groups、GPU utilization、policy lag。
- eval 与 training verifier 分离可降低 reward overfitting。
- 先排数据/实现 bug，再调算法超参。
...
### 追问 1：reward 上升但 held-out 降低怎么办？

LLM Reasoning RL pipeline 应形成闭环：prompt curriculum→多样 rollout→可验证 reward→有效样本选择→policy update→独立 eval→失败簇回流。算法与系统不能分开设计。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：如何做 checkpoint selection？
...
> 好的答案一定包含“训练指标不等于真实能力”，并设置独立 held-out 与防污染流程。
...
> **结论先行**：从 prompt curriculum 到多样本 rollout、verifier、group statistics、advantage、policy update、KL/entropy、evaluation、hard prompt mining 建立闭环，并对数据版本和 policy 版本全链路可追踪。

继续展开时，先把它放回本章的统一问题框架：**Prompt Pool：按难度、领域、失败簇分层采样。；Rollout：G responses/prompt，continuous batching。**。随后写出本题最关键的数学对象：`success = capability gain - λ1·drift - λ2·cost - λ3·regression`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“System Design：设计完整 LLM Reasoning RL Pipeline。”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
