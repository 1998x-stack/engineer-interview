...
> 至少同时监控 reward、task accuracy、KL、entropy、value/advantage stats、clipfrac、gradient norm、response/action length、success rate 与系统吞吐。

面试开场建议只讲这一层；如果面试官点头继续，再进入后面的推导与 failure mode。

## 3. 深入解析（PDF 原始要点）

- reward↑ + held-out↓：疑似 reward hacking/overfit。
- KL↑ + clipfrac↑：更新过大。
- entropy↓ + diversity↓：探索坍缩。
...
专业 RL 诊断必须把“数据→估计器→优化→策略分布→真实任务”指标联动起来。单个 loss 或 reward 没有足够因果解释力。

### 数学/推导抓手

常用：return/reward components、advantage/value/Q、ratio/KL/entropy、clipfrac、grad norm、success/pass@k、length/cost。

> **面试要求**：这里的公式不是“背出来就结束”。需要能解释每个期望是对什么随机变量取、哪些量来自 rollout、哪些量是 learned estimate、哪些分支必须 stop-gradient。

### 工程化检查点

- 建立 joint dashboard：reward↑但 success↓→reward hacking；KL↑+entropy↓→collapse；value loss↑+explained variance↓→critic failure；GPU util↓+p99 length↑→rollout straggler。
- 先排数据/实现 bug，再调算法超参。
...
### 追问 1：advantage mean/std 应该是什么样？

专业 RL 诊断必须把“数据→估计器→优化→策略分布→真实任务”指标联动起来。单个 loss 或 reward 没有足够因果解释力。

回答时继续补一层：先说明**为什么**，再指出一个**边界条件/失败现象**，最后给出一个可观测指标或实现检查点。

### 追问 2：gradient norm spike 怎么定位？
...
> **结论先行**：至少同时监控 reward、task accuracy、KL、entropy、value/advantage stats、clipfrac、gradient norm、response/action length、success rate 与系统吞吐。

继续展开时，先把它放回本章的统一问题框架：**reward↑ + held-out↓：疑似 reward hacking/overfit。；KL↑ + clipfrac↑：更新过大。**。随后写出本题最关键的数学对象：`见上文推导`。最后必须补一句工程判断：公式成立不代表实现健康，需要用本页列出的分布指标、边界条件和 failure mode 验证。
...
针对本题额外要求：把 **“训练中应该监控哪些 RL 指标？如何联动诊断？”** 对应的关键变量单独画分布或写断言；如果实验结果和理论预期相反，优先检查数据定义、mask/terminal、旧策略版本和归一化维度，再讨论超参数。
