...
# Q003 · 为什么搜索一定要有候选召回阶段？

[← 上一题](Q002-search-vs-recommendation.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q004-recall-prerank-rank-rerank.md)
...
这道题表面在问“为什么搜索一定要有候选召回阶段？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

因为全库精排的计算量与延迟不可接受。召回阶段用便宜的检索函数把十亿级文档缩小到千级/万级候选， 目标是高 Recall；后续再用昂贵模型提高排序精度。

## 5 分钟深度回答

- 若文档数为 N ，Cross-Encoder 对全库逐一计算的成本近似 O(N · Cmodel )。
- 倒排索引利用 term → postings 只访问与 query term 相关的文档；ANN 只探索向量空间的一小部
分。
- 召回阶段的核心错误是 false negative：相关文档一旦没进入候选，后面的 Ranker 再强也救不回来。
- 因此常见优化顺序是先看 Recall@1000/Recall@5000，再看 NDCG@10。
- 多路召回是工业常态：BM25、Dense、规则/实体、热门、新鲜内容等并行，再合并去重。

## 进一步深挖：从“会答”到“能做”

#### Cascade 的核心不变量

每一级都应该回答两个问题：①这一层允许丢掉多少 oracle-relevant candidates？②为了节省多少延迟/成本？建议记录每层 `candidate_count`、`oracle_recall`、`stage_latency` 和 `drop_reason`。这样当 NDCG 下跌时，可以判断损失来自 recall ceiling 还是 rank inversion。
...
用 N×模型成本说明全库精排不可行；解释 candidate generation 的目标是高 Recall 不是最终相关性；做 oracle rerank 判断 recall ceiling。
...
- 召回 TopK 越大越好吗？
- Recall@1000 很高但线上效果差，可能为什么？
- 如何衡量不同召回通道的增量价值？

## 高频失分点 / Gotcha

只说“为了快”太浅；应明确说明 cascade 的经济性：前层高召回、低成本，后层高精度、高成本。
