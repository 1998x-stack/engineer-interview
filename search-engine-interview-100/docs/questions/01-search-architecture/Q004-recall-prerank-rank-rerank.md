...
# Q004 · 召回、粗排、精排、重排分别优化什么？

[← 上一题](Q003-why-candidate-retrieval.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q005-database-like-vs-search.md)
...
这道题表面在问“召回、粗排、精排、重排分别优化什么？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

四层不是模型大小的简单递增，而是约束不同：召回保覆盖，粗排做低成本淘汰，精排最大化相关性与业 务收益，重排处理列表级约束。

## 5 分钟深度回答

- 召回：重点看 Recall@K、Coverage、通道增量与延迟。
- 粗排：重点是单位计算成本的排序能力，常用轻量 GBDT/DNN 或蒸馏模型。
- 精排：重点优化 NDCG、CTR/CVR、long-click 等目标，可使用 LambdaMART、深度排序模型、
Cross-Encoder。
- 重排：
处理 diversity、 去重、 类目配额、 freshness、 合规策略和多目标约束， 属于 list-level optimization。
- 每层都要有独立诊断指标，否则“整体 NDCG 降了”无法定位是召回漏了还是排序排错。
...
### 追问链：参考回答
**追问 1：PreRank 的训练标签能否与 Rank 完全相同？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 2：为什么 ReRank 不应该只看 pointwise score？**
...
给每阶段定义 candidate depth、目标 metric 和 latency budget；说明粗排允许较弱交互而精排可用更贵特征；重排还承担 diversity/policy。
...
- PreRank 的训练标签能否与 Rank 完全相同？
- 为什么 ReRank 不应该只看 pointwise score？
- 如何选择每层的候选数？

## 高频失分点 / Gotcha

不要把 ReRank 等同于“再跑一个更大的模型”；它经常承担业务与列表约束。
...
Recall 负责入场，Rank 负责座次，ReRank 负责整桌体验。
