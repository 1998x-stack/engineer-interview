...
# Q005 · 为什么不能用数据库 LIKE 代替搜索引擎？

[← 上一题](Q004-recall-prerank-rank-rerank.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q006-query-to-serp-request-lifecycle.md)
...
这道题表面在问“为什么不能用数据库 LIKE 代替搜索引擎？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

数据库索引主要解决结构化 lookup；全文搜索需要 tokenization、倒排索引、相关性评分、短语/位置检 索、纠错与排序。‘LIKE

## 5 分钟深度回答

- B-tree 的优势是有序键上的点查、范围查；前导通配符会破坏可利用的有序前缀。
- 倒排索引把 term 映射到 posting list，使查询只扫描包含相关 term 的文档。
- 全文检索还需要 TF、DF、字段长度、positions、offsets 等统计与结构。
- 搜索结果不是“匹配/不匹配”二值结果，而是需要 TopK 排序。
- 数据库也能做全文索引，但当需求进入多字段相关性、ANN、LTR、分布式检索后，通常需要专门搜索
系统。
...
**追问 2：数据库全文索引和 Lucene 有什么差异？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 3：什么时候直接 SQL 已经足够？**
...
区分 OLTP lookup 与 IR ranking；说明 suffix wildcard 会破坏 B-tree prefix locality；补充数据库全文索引本质上也在引入倒排/FTS 子系统。
...
- 前缀 LIKE ‘abc
- 数据库全文索引和 Lucene 有什么差异？
- 什么时候直接 SQL 已经足够？

## 高频失分点 / Gotcha

面试中不要绝对说“LIKE 一定全表扫”；是否走索引取决于模式、collation、数据库实现与查询计划。
