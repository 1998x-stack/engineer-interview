---
id: Q007
title: "为什么现代搜索几乎都是 Multi-stage Ranking？"
chapter: 1
...
# Q007 · 为什么现代搜索几乎都是 Multi-stage Ranking？

[← 上一题](Q006-query-to-serp-request-lifecycle.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q008-multi-objective-search-ranking.md)
...
这道题表面在问“为什么现代搜索几乎都是 Multi-stage Ranking？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

因为计算预算有限且候选数量逐层减少。不同阶段使用不同模型，让单位延迟获得最大的质量收益，这是 经典 cascade architecture。

## 5 分钟深度回答

- 召回对百万/十亿规模，必须使用亚线性或稀疏访问；精排只面对几百候选。
- 模型越昂贵，适用的候选集就越小：BM25/ANN → GBDT/bi-encoder → cross-encoder/LLM。
- 如果前层过于激进，会产生不可恢复的 recall loss；如果过于宽松，会把计算压力推给下游。
- 阶段间通常要做蒸馏或目标对齐，避免粗排把精排偏好的候选误删。
- 工业优化常用“quality-latency frontier”，而不是单点最大 NDCG。
...
从 cost funnel 解释 cascade；引入 early exit/query routing；讨论阶段间 distribution shift——上一阶段改变后，下游模型候选分布也会改变。
...
- 如何做 Ranker 到 PreRank 的蒸馏？
- 候选数变化为什么会影响模型分布？
- 什么时候可以省掉粗排？

## 高频失分点 / Gotcha

不要机械说“四层固定”；真实系统可能是 2 层、3 层或更多，取决于 QPS、候选量和模型成本。
