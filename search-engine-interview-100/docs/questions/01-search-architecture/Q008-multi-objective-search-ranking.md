...
# Q008 · 搜索系统的目标函数为什么是多目标的？

[← 上一题](Q007-why-multi-stage-ranking.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q009-why-not-rank-by-ctr.md)
...
这道题表面在问“搜索系统的目标函数为什么是多目标的？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

相关性是基础，但真实系统还要兼顾质量、新鲜度、可信度、多样性、点击/转化与延迟。常见做法是多任 务预测、线性/非线性融合或约束优化。

## 5 分钟深度回答

- 可抽象为 U = αR + βE + γQ + δF − λC，其中 C 可以代表延迟/成本或风险。
- 相关性通常不能被业务目标完全覆盖，否则会出现“高转化但不相关”的劣化。
- 不同 query intent 权重应不同：导航型更重准确命中，新闻型更重 freshness，电商型还看价格/库
存/转化。
- 多目标训练可用 shared-bottom/MTL，但最终 list-level policy 常仍需显式规则或约束。
- 长期指标与短期指标可能冲突，例如提高标题党 CTR 会损害 long-click 与留存。
...
### 追问链：参考回答
**追问 1：CTR 与 relevance 如何解耦？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 2：如何给 freshness 设动态权重？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 3：多目标融合为什么需要 calibration？**
...
把 relevance、quality、freshness、revenue、diversity 写成约束或多目标；说明线性加权并非唯一方法，可用 constrained optimization/pareto/规则 guardrail。
...
- CTR 与 relevance 如何解耦？
- 如何给 freshness 设动态权重？
- 多目标融合为什么需要 calibration？

## 高频失分点 / Gotcha

“把所有指标加权求和”只是起点；真正难点是权重随意图变化、尺度校准与不可违反的硬约束。
