---
id: Q010
title: "传统 Search 与 RAG Retrieval 的目标有什么不同？"
chapter: 1
...
# Q010 · 传统 Search 与 RAG Retrieval 的目标有什么不同？

[← 上一题](Q009-why-not-rank-by-ctr.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · 下一题：无 →
...
| 核心标签 | `search-architecture`, `retrieval`, `ranking`, `trie`, `rag` |

## 面试官到底在考什么

这道题表面在问“传统 Search 与 RAG Retrieval 的目标有什么不同？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

传统搜索优化用户看到的结果列表；RAG Retrieval 的结果会继续被生成模型消费，因此评价要延伸到 evidence coverage、context precision 与最终 answer correctness。

## 5 分钟深度回答

- 传统搜索更关心 NDCG/MRR/CTR；RAG 还关心 answer faithfulness、evidence recall、context
utilization。
- RAG 中“相关文档”不等于“足够回答问题的证据”，需要考虑多跳、互补证据与 chunk 粒度。
- 过多相似 chunk 会占满 context window，造成冗余和 lost-in-context。
- Reranker 的最优目标可能从“文档相关性”变成“对回答是否提供边际证据”。
- 因此 RAG 的检索评估最好做 end-to-end：Retriever → Reranker → Context Builder → Gener-
ator。
...
### 追问链：参考回答
**追问 1：如何定义 context recall？**

先看 oracle recall：正确结果是否已经在候选中。如果不在，ranker 再强也无能为力；如果在但排序差，则瓶颈在 rank/fusion。

**追问 2：Retriever 变好但 Answer Accuracy 不变可能为什么？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 3：RAG 是否应该用 query expansion？**
...
RAG retriever 的价值最终由 answer utility 决定；document-level relevance 不等于 chunk evidence sufficiency；需要联合看 context precision/recall 和 grounded answer。
...
- 如何定义 context recall？
- Retriever 变好但 Answer Accuracy 不变可能为什么？
- RAG 是否应该用 query expansion？

## 高频失分点 / Gotcha

不要把 RAG 简化成“向量搜索”；BM25、hybrid、reranking 和过滤同样重要。
...
Search 排文档；RAG 排“对回答有用的证据”。

第二章倒排索引与 Lucene 内核

理解搜索为什么能在海量文本上低延迟工作，以及性能优化真正发生在哪里。

题号 题目 难度 Q11 什么是倒排索引？ 2/5 Q12 为什么叫“倒排”？正排索引还有什么用？ 1/5 Q13 Posting List 中一般存哪些信息？ 3/5 Q14 两个有序 Posting List 的 AND 查询怎么做？ 2/5 Q15 Posting List 很长时，WAND / Block-Max WAND 在做什么？什么？ 5/5 Q16 为什么 DocID 常用 gap/delta 编码？ 3/5 Q17 Term Dictionary 为什么常用 FST，而不只是 HashMap？ 4/5 Q18 Trie 为什么适合做 Search Autocomplete？ 2/5 Q19 Lucene Segment 为什么设计成 immutable？ 4/5 Q20 Segment Merge 为什么既重要又危险？ 4/5
