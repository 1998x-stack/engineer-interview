...
← 上一题：无 · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q012-inverted-vs-forward-index.md)
...
这道题表面在问“什么是倒排索引？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

倒排索引把“文档包含哪些词”的正向关系反转成“每个词出现在哪些文档中”。核心结构是 Term Dictionary + Posting Lists。

## 5 分钟深度回答

- 正排：doc → terms；倒排：term → postings。
- posting 最少包含 docID，常进一步包含 term frequency、positions、offsets、payload。
- 查询 ‘A AND B‘ 变成两个有序 posting list 的交集，而不是扫描所有文档。
- 索引构建阶段要做 analyzer：normalization、
tokenization、 stemming/lemmatization、 stopword 等。
- 倒排索引是 BM25、phrase query、highlight 和多字段搜索的基础。
...
写清 term dictionary→posting list 映射；说明 docID、tf、positions、offsets、payload 的可选性；讨论字段与 segment 粒度。
...
- 为什么 posting list 要有序？
- positions 会增加多少空间？
- 中文分词变化后为什么必须重建索引？

## 高频失分点 / Gotcha

只回答“词到文档 ID 的映射”拿不到高分；应提 Term Dictionary、posting 元数据与查询执行。
...
倒排索引 = Dictionary 定位词 + Posting 定位文档。
