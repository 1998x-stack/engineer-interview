...
# Q012 · 为什么叫“倒排”？正排索引还有什么用？

[← 上一题](Q011-inverted-index.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q013-posting-list-contents.md)
...
这道题表面在问“为什么叫“倒排”？正排索引还有什么用？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

“倒排”是相对文档到词项的正排关系而言。正排仍用于文档展示、字段取回、摘要生成和部分特征读取； 倒排主要服务高效检索。

## 5 分钟深度回答

- Forward index 适合从 docID 读取文档字段。
- Inverted index 适合从 term 找候选文档。
- 真实搜索系统通常同时保存 source/document store、doc values/columnar features 与 inverted
index。
- Lucene 中 stored fields、doc values 与 postings 各承担不同访问模式。
...
**追问 2：Stored field 和 postings 为什么不合成一个结构？**
...
正排适合从 doc 找字段/存储/重建 snippet，倒排适合从 term 找 doc；现代引擎通常同时维护 stored fields/doc values 与 postings。
...
- 为什么排序特征适合列式存储？
- Stored field 和 postings 为什么不合成一个结构？
