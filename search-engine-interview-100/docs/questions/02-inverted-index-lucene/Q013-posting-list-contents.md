...
# Q013 · Posting List 中一般存哪些信息？

[← 上一题](Q012-inverted-vs-forward-index.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q014-posting-list-intersection.md)
...
这道题表面在问“Posting List 中一般存哪些信息？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

posting 的字 段设 计决 定 能支持哪 些 查询 和打分。最基本是 docID；要 做 BM25 需要词频， 要做 phrase/proximity 需要 positions，要做 highlight 需要 offsets。

## 5 分钟深度回答

- docID：文档标识，通常排序后做差分压缩。
- tf/freq：term 在该文档中的出现次数。
- positions：每次出现的 token position，支持 phrase 和 near query。
- offsets：字符起止位置，用于高亮和精确片段。
- payload：可附加自定义信息，但会增加索引空间与查询成本。
- Index options 常是功能与空间的 trade-off：不是每个字段都需要 positions/offsets。
...
### 追问链：参考回答
**追问 1：只存 docID 能不能算 BM25？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 2：Phrase Query 最少需要什么信息？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 3：为什么 postings 常按 block 存储？**
...
区分最小 posting(docID) 与 rich posting(tf/pos/offset/payload)；功能越丰富索引越大；phrase/highlight 是否需要 positions/offsets 是经典 schema trade-off。
...
- 只存 docID 能不能算 BM25？
- Phrase Query 最少需要什么信息？
- 为什么 postings 常按 block 存储？

## 高频失分点 / Gotcha

面试中要把“支持的查询能力”和“存储成本”联系起来。
