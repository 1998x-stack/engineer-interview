---
id: Q014
title: "两个有序 Posting List 的 AND 查询怎么做？"
chapter: 2
...
# Q014 · 两个有序 Posting List 的 AND 查询怎么做？

[← 上一题](Q013-posting-list-contents.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q015-wand-block-max-wand.md)
...
这道题表面在问“两个有序 Posting List 的 AND 查询怎么做？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

用双指针线性求交：较小 docID 的指针前进，相等时输出。复杂度 O(|A| + |B|)，在长短悬殊时可进一步 用 skip/galloping。

## 5 分钟深度回答

- 基础算法：‘i=j=0‘，若 ‘A[i]==B[j]‘ 输出并同时前进；否则移动较小者。
- 当一条 posting 很短、另一条很长时，可以对长表二分或 exponential/galloping search。
- 多 term AND 可先按 posting length 从短到长求交，尽早缩小候选。
- 现代引擎还会使用 skip data、block metadata 与 SIMD 优化。

## 数学 / 白板推导

### 白板伪代码：Posting List 交集
...
时间复杂度是 $O(|A|+|B|)$，前提是两个 posting list 已按 DocID 排序。工程实现会继续叠加 skip、block、SIMD 与动态剪枝。
...
用双指针线性合并；当长度高度不均衡时可对短表逐项 galloping/binary search；再解释 skip pointer/block metadata 如何减少解码。
...
def intersect(a, b):
    ...
    return out
```
复杂度是 `O(|A|+|B|)`；若一个 posting 极短、另一个极长，可考虑 galloping / binary search 让工作量更接近短表大小。
...
- OR 查询怎么做？
- 三条 posting list 的交集顺序如何选？
- 为什么 ‘AND‘ 仍可能很慢？

## 高频失分点 / Gotcha

仅写出双指针还不够，进阶回答要讨论数据分布与跳跃优化。
...
> **练习：** 手写两个 posting list 的 AND/OR 交集，并说明如何加入 skip pointer。
