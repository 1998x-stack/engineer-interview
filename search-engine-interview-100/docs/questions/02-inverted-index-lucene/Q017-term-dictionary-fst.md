---
id: Q017
title: "Term Dictionary 为什么常用 FST，而不只是 HashMap？"
chapter: 2
...
# Q017 · Term Dictionary 为什么常用 FST，而不只是 HashMap？

[← 上一题](Q016-docid-gap-delta-encoding.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q018-trie-autocomplete.md)
...
| 核心标签 | `inverted-index`, `lucene`, `indexing`, `fst`, `map` |

## 面试官到底在考什么

这道题表面在问“Term Dictionary 为什么常用 FST，而不只是 HashMap？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

搜索词典既要压缩大量共享前缀，又要支持有序遍历、prefix/automaton 查询。FST 将 trie-like 前缀 共享与状态合并结合，在内存效率与功能上优于朴素 HashMap。

## 5 分钟深度回答

- HashMap 适合 exact lookup，但键本身存储冗余，prefix/range 遍历能力弱。
- Trie 共享前缀，但节点数可能很大；FST 可合并等价后缀状态，进一步压缩。
- 有序 term dictionary 对 range、wildcard、prefix enumeration 很重要。
- 搜索引擎通常还需要 term metadata 指向 postings 的文件位置。
- 面试中不需要手推 FST 最小化算法，但要说清“为什么不是 hash”。
...
**追问 2：Wildcard query 为什么可能很贵？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 3：Term dictionary 能否完全放内存？**
...
FST 利用公共前缀/后缀压缩并支持有序遍历和 prefix seek；HashMap O(1) 期望查找但内存大、无序、难做 prefix；Lucene term dictionary 还要考虑磁盘与 mmap。
...
- FST 与 Trie 区别？
- Wildcard query 为什么可能很贵？
- Term dictionary 能否完全放内存？

## 高频失分点 / Gotcha

不要把 FST 仅仅理解为“更高级的 Trie”；其关键价值是有限状态机/转导器的共享与紧凑编码。
