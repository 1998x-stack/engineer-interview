---
id: Q018
title: "Trie 为什么适合做 Search Autocomplete？"
chapter: 2
...
# Q018 · Trie 为什么适合做 Search Autocomplete？

[← 上一题](Q017-term-dictionary-fst.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q019-lucene-immutable-segment.md)
...
| 核心标签 | `inverted-index`, `lucene`, `indexing`, `trie`, `autocomplete` |

## 面试官到底在考什么

这道题表面在问“Trie 为什么适合做 Search Autocomplete？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Autocomplete 是前缀检索。 Trie 让查找前缀只与前缀长度有关， 再从该节点向下枚举高质量 completion； 若每个节点缓存 TopN，则能做到非常低延迟。

## 5 分钟深度回答

- 共享前缀能降低字符串存储重复。
- 节点可维护 top suggestions、频率、最近热度等聚合统计。
- 大规模系统常使用压缩 Trie/FST，降低对象和指针开销。
- 个性化 autocomplete 常是“全局候选 + 用户历史候选 + 热点候选”融合。
- 需要额外处理 typo、语言混输、敏感词、趋势变化。
...
Trie/FST 能按 prefix 找子树；真正线上还要把 popularity/recency/personalization 作为 top-k 权重，并预计算 top suggestions 避免遍历巨大子树。
...
Trie 解决的是 candidate generation，不等于完整 autocomplete 排序系统。
