...
# Q021 · TF-IDF 的核心直觉是什么？

← 上一题：无 · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q022-idf-discriminativeness.md)
...
这道题表面在问“TF-IDF 的核心直觉是什么？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

一个词对文档的重要性由“在当前文档里出现得多”和“在整个语料里出现得少”共同决定，因此是局部 频率 TF 与全局稀有度 IDF 的组合。

## 5 分钟深度回答

- T F (t, d) 表示 term 在文档中的频率；DF (t) 表示包含该 term 的文档数。
- 常见 IDF (t) = log df
N t：越稀有的 term 权重越高。

- 实际实现常对 TF 做 log/sublinear 变换，对向量做 cosine normalization。
- TF-IDF 不等于 BM25；BM25 进一步显式建模 TF saturation 与 length normalization。
...
面试时要主动说明：不同系统对 TF 与 IDF 的平滑、对数底数和归一化方式可能不同，真正稳定的是“局部频率 × 全局稀有度”这个建模思想。
...
**追问 3：TF-IDF 和 cosine similarity 什么关系？**
...
TF 表示局部证据，IDF 表示全局辨识度；强调“高频但常见”与“低频但稀有”的差异；补充 raw TF 线性增长是后续 BM25 要修的问题。
...
- 为什么不能只用 TF？
- IDF 为负怎么办？
- TF-IDF 和 cosine similarity 什么关系？

## 高频失分点 / Gotcha

公式记忆不是重点，关键是解释“局部重要 + 全局辨识度”。
