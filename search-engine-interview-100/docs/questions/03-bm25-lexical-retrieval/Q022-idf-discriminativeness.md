...
# Q022 · 为什么 IDF 能衡量一个词的“辨识度”？

[← 上一题](Q021-tf-idf-intuition.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q023-tf-idf-limitations.md)
...
这道题表面在问“为什么 IDF 能衡量一个词的“辨识度”？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

如果一个词出现在几乎所有文档中，它无法区分候选；如果只出现在很少文档中，一旦 query 命中它，就 强烈缩小了候选空间。IDF 可理解为一种信息量近似。

## 5 分钟深度回答

- dft ↑⇒ idft ↓。
- 停止词如“the/的”通常 df 极高，因此贡献低。
- 实体名、型号、错误码等 rare terms 在 lexical retrieval 中非常有价值。
- IDF 依赖 collection，跨索引或跨 shard 的统计不一致会改变得分。

## 数学 / 白板推导

### 极限情况

当 $df(t)\to N$ 时，词几乎出现在所有文档里，IDF 接近最低；当 $df(t)$ 很小时，IDF 较高。面试官真正想听的是：**IDF 在近似衡量 term 对文档集合的区分能力，而不是衡量词在语言学上的重要性。**
...
从 self-information 直觉解释 rare event 信息量大；df 而不是 corpus term count 决定文档区分能力；极端常见词 IDF 近 0。
...
稀有不等于高质量：乱码 token 也可能很稀有，所以 analyzer/质量控制仍重要。
