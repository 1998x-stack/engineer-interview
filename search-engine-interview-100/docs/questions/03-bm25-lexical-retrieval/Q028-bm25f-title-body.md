---
id: Q028
title: "Title 和 Body 应该如何联合打分？什么是 BM25F 思想？"
chapter: 3
...
# Q028 · Title 和 Body 应该如何联合打分？什么是 BM25F 思想？

[← 上一题](Q027-bm25-b.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q029-bm25-failure-modes.md)
...
这道题表面在问“Title 和 Body 应该如何联合打分？什么是 BM25F 思想？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

不同字段的语义和统计分布不同，不能简单拼接成一个大文本。常见方法是 per-field BM25 + boost， 或用 BM25F 类方法先对字段内 TF/长度做归一化再融合。

## 5 分钟深度回答

- Title 命中往往比 Body 命中更强，因为 title 是高密度主题字段。
- 字段长度分布不同，应该有不同的 normalization。
- 可构造 score = wt BM 25t + wb BM 25b + · · · ，然后用 LTR 学习更复杂融合。
- 字段 boost 不宜纯人工拍脑袋，最好用 judgment/log 数据优化。
- 电商搜索还会加入 brand、category、attribute exact match 等结构化字段。
...
BM25F 的关键是 field-specific tf normalization 与权重，不是简单 score 相加；考虑 title exact、anchor、body、category 等字段的不同长度分布。
...
- 同一个 term 在多个字段出现是否重复计分？
- 字段分词策略可以不同吗？

## 高频失分点 / Gotcha

把字段拼在一起会丢失“在哪里命中”这一重要特征。
...
> **练习：** 为 title/body/anchor 三个字段设计一个可解释的 field-weighted lexical baseline，并给出离线调参方案。

完成标准：能在不看答案的情况下，先用 30 秒给结论，再用 5 分钟白板说明原理、至少两个 trade-off、一个 failure case 和验证指标。

## 一句话记忆

多字段搜索的关键是：字段语义不同，统计也不同。
