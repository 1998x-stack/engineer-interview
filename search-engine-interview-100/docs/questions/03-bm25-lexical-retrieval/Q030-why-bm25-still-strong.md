...
# Q030 · 为什么 BM25 在 2026 年仍然非常强？

[← 上一题](Q029-bm25-failure-modes.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · 下一题：无 →
...
这道题表面在问“为什么 BM25 在 2026 年仍然非常强？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

因为 exact match、rare entity、ID/型号、可解释性和成熟倒排执行仍是很多搜索场景的硬需求。Dense 模型补足语义，但常不能稳定替代这些 lexical signals，因此 hybrid 成为更常见方案。

## 5 分钟深度回答

- 零训练成本：没有标注也能获得强基线。
- 倒排索引非常成熟，CPU latency 与内存效率优秀。
- 对罕见实体、代码符号、错误码、SKU 等 OOD/长尾 token 有天然优势。
- 得分可拆解到 term contribution，便于 debug。
- 现代系统常将 BM25 score 作为 LTR 特征，即使主召回已经加入 Dense。
...
BM25 强在 rare term、标识符、无需训练、解释性和倒排剪枝；作为 hybrid anchor 能抵御 dense embedding 的语义漂移和冷启动。
...
- 如果 embedding 模型很强，BM25 还有什么独有价值？
- Hybrid 如何判断各路的边际收益？

## 高频失分点 / Gotcha

“传统 = 落后”是搜索面试中的危险心态；成熟系统最重视可控、互补与收益/成本比。
