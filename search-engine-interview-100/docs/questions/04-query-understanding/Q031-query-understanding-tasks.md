---
id: Q031
title: "Query Understanding 通常包括哪些任务？"
chapter: 4
...
# Q031 · Query Understanding 通常包括哪些任务？

← 上一题：无 · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q032-chinese-tokenization.md)
...
这道题表面在问“Query Understanding 通常包括哪些任务？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Query Understanding 是把用户原始字符串转换成检索系统可消费的结构化意图。典型任务包括 normalization、语言识别、分词、纠错、实体/属性抽取、意图分类、改写与扩展。

## 5 分钟深度回答

- Normalization：大小写、Unicode、全半角、数字/单位等。
- Segmentation/tokenization：尤其中文、日文等无天然空格语言。
- Spelling：纠错与 typo tolerance。
- NER/entity linking：将“苹果”识别为公司/水果等实体候选。
- Intent/slot：导航、信息、交易；电商还要抽 brand/category/price/color。
- Rewrite/expansion：生成更适合 lexical/dense retriever 的查询表示。
...
把 QU 拆成 normalization、spell、segmentation、NER/entity link、intent、attribute、rewrite；不是所有 query 都需全链路，应用 confidence-based routing。
...
- 哪些步骤适合规则，哪些适合模型？
- Query Understanding 的延迟预算通常为什么很紧？
- 如何离线评估 rewrite？

## 高频失分点 / Gotcha

不要把 Query Understanding 等同于“分词”。它是搜索从字符串到意图结构的完整前端。
