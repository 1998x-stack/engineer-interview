...
[← 上一题](Q031-query-understanding-tasks.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q033-search-tokenization-granularity.md)
...
这道题表面在问“中文搜索分词为什么比英文更难？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

中文没有天然空格边界，而且一个字符串可能有多种合法切分；搜索还要求 recall，往往不能只输出单一 分词。

## 5 分钟深度回答

- ‘南京市长江大桥 ‘ 存在词边界歧义；品牌、网络新词又不断出现。
- 搜索分词与语言学分词目标不同：搜索宁愿保留多个粒度以提高 recall。
- 常见做法是词典 + 统计模型 + 新词发现，并在索引端/查询端做多粒度 token。
- 实体词通常需要整体保留，例如 ‘北京大学 ‘ 不能只剩 ‘北京 ‘ 与 ‘大学 ‘。
- 分词策略改变会同时影响 DF、BM25 长度统计和索引大小。
...
中文无天然空格、词边界歧义、实体与新词多；讨论字粒度、词粒度与 subword 多路索引；搜索分词目标和 NLP 分词 gold standard 不完全相同。
