...
[← 上一题](Q032-chinese-tokenization.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q034-spelling-correction-candidate-ranking.md)
...
这道题表面在问“搜索分词是不是越细越好？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

不是。过细会丢失实体和短语语义，并显著扩大 postings；过粗又会导致召回不足。工业上通常采用多粒 度 token、短语字段或 shingles。

## 5 分钟深度回答

- ‘北京大学 ‘ 若只切为 ‘北京/大学 ‘，会把大量非北大的结果召回。
- 保留原实体 token 可提供高 IDF 的精确匹配。
- 细粒度 token 对 typo/部分匹配有帮助，但需要字段权重和最小匹配约束。
- 可将 exact phrase、bigram、word token 作为不同特征交给 ranker。
...
过细提高 recall 但损失 phrase/entity precision；过粗降低 recall；常用多粒度 token + phrase/entity boost，而不是强迫唯一切分。
...
- N-gram tokenizer 为什么索引膨胀严重？
- Edge n-gram 适合什么场景？

## 高频失分点 / Gotcha

“分得越细 recall 越高”只看到了 recall，没有看到 precision、index size 与实体语义。
