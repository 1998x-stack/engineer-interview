---
id: Q035
title: "“苹果”这样的 Query 为什么难？如何做意图消歧？"
chapter: 4
...
# Q035 · “苹果”这样的 Query 为什么难？如何做意图消歧？

[← 上一题](Q034-spelling-correction-candidate-ranking.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q036-query-expansion-recall-precision.md)
...
这道题表面在问““苹果”这样的 Query 为什么难？如何做意图消歧？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

短 query 信息量低，可能对应多个实体或意图。系统需要结合上下文、用户历史、地域、热度、结果反馈 与实体图谱估计 P (intent | q, u, c)。

## 5 分钟深度回答

- 同一 query 可有 company、fruit、movie/music 等不同实体解释。
- Head query 可从历史点击分布学习 dominant intent；tail query 需要语义模型/知识图谱。
- 结果页可通过 intent diversification 降低一次猜错带来的损失。
- 个性化应谨慎：用户刚搜索食谱时，“苹果”不应因过去看科技新闻就总被解释为 Apple。
- Query session 是很强上下文：前一条 query 往往能显著帮助当前消歧。
...
用 P(intent|query,user,context) 表述歧义；通过 entity popularity、session context、location、vertical prior 消歧；低置信可多意图并行召回。
...
- 如何评估 intent classifier？
- 如果两个意图概率接近，结果页怎么排？

## 高频失分点 / Gotcha

不要把“最热门意图”永久固定为唯一答案；搜索需要覆盖 uncertain intents。
