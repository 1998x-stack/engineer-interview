---
id: Q036
title: "什么是 Query Expansion？为什么会同时提升 Recall 和伤害 Precision？"
chapter: 4
...
# Q036 · 什么是 Query Expansion？为什么会同时提升 Recall 和伤害 Precision？

[← 上一题](Q035-query-intent-disambiguation-apple.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q037-synonym-rewrite-expansion.md)
...
这道题表面在问“什么是 Query Expansion？为什么会同时提升 Recall 和伤害 Precision？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Expansion 为原 query 添加同义、相关实体、缩写展开或伪相关反馈词，使更多相关文档进入候选；但 扩展词如果语义漂移，也会召回大量弱相关内容。

## 5 分钟深度回答

- 词典扩展：car → automobile；实体别名：NYC → New York City。
- Pseudo Relevance Feedback：从初检 Top docs 中提取扩展词，例如 RM3 思路。
- Neural/LLM expansion 可以生成语义更丰富的改写，但漂移风险更大。
- 实践中常给 original terms 更高权重，扩展词较低 boost。
- 评价应看 recall 增量是否真正转化成 NDCG/CTR，而不是只看候选数。
...
Expansion 通过增加同义/相关 term 扩大召回，但 query drift 会引入噪声；讨论 expansion weight、原词保留和 max expansions。
...
- PRF 为什么可能自我强化错误结果？
- Expansion 在 head 与 tail query 哪个更有价值？

## 高频失分点 / Gotcha

Expansion 是 recall 工具，不应直接被当作“理解正确”的证明。
