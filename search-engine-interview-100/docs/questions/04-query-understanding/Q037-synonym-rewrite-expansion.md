---
id: Q037
title: "Synonym、Query Rewrite、Query Expansion 有什么区别？"
chapter: 4
...
# Q037 · Synonym、Query Rewrite、Query Expansion 有什么区别？

[← 上一题](Q036-query-expansion-recall-precision.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q038-llm-query-rewrite.md)
...
这道题表面在问“Synonym、Query Rewrite、Query Expansion 有什么区别？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Synonym 通常是局部词项等价或近义替换；Expansion 是添加更多检索词；Rewrite 可以重构整个 query，包含删停用信息、纠错、实体规范化、意图补全等更广操作。

## 5 分钟深度回答

- Synonym：‘iphone ↔ 苹果手机 ‘。
- Expansion：保留 ‘iphone‘，额外加入 ‘Apple phone‘ 等候选词。
- Rewrite：‘北京明天天气怎么样 → 北京天气明天 ‘，可能改变结构。
- 现代搜索常同时生成 lexical rewrite 与 dense rewrite，因为两种 retriever 的理想 query 表达不
同。
...
Synonym 多为局部等价映射，Expansion 扩支持集，Rewrite 改整体表达/结构；三者在线风险和可逆性不同，最好分别版本化和评测。
...
- Synonym 应在索引时展开还是查询时展开？
- 多路 rewrite 如何融合？

## 高频失分点 / Gotcha

边界并非学术上绝对统一；面试时应说明你采用的工程定义。
...
Synonym 是局部，Rewrite 是整体，Expansion 是加宽。
