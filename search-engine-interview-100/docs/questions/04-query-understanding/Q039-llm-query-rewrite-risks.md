---
id: Q039
title: "LLM Query Rewrite 最大的风险是什么？"
chapter: 4
...
# Q039 · LLM Query Rewrite 最大的风险是什么？

[← 上一题](Q038-llm-query-rewrite.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q040-search-autocomplete-query-suggest.md)
...
这道题表面在问“LLM Query Rewrite 最大的风险是什么？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

最大的算法风险是 query drift：模型补充了用户没有表达的限制或改变了意图；最大的工程风险是延 迟/成本和不稳定输出。

## 5 分钟深度回答

- ‘Java GC‘ 被改成 ‘Java G1 GC tuning‘ 就是过度缩窄。
- 结构化 filter 幻觉更危险：一旦误加 ‘brand=Apple‘，后续再强的 ranker 都看不到其他结果。
- 防护：original-query parallel recall、
confidence gate、 schema validation、 rewrite diff logging。
- 需要专门构建“不可改写”样本集：实体、型号、代码、URL、股票代码等。
- 线上监控 query reformulation rate 和 zero-result rate 可发现系统性 drift。
...
最大风险是 semantic drift 与 hallucinated constraint；检测可用 original-vs-rewrite embedding/实体一致性、retrieval overlap、离线 human slice；低置信直接回退。
...
- 如何做 rewrite offline label？
- 是否应该让 LLM 直接输出最终检索结果？

## 高频失分点 / Gotcha

不要用“生成看起来更自然”作为 rewrite 质量标准；要看检索效用和意图保持。
