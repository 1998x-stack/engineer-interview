---
id: Q040
title: "如何设计 Search Autocomplete / Query Suggest？"
chapter: 4
chapter_title: "Query Understanding 与 Query Rewrite"
difficulty: 4
frequency: "S"
roles: "Query Suggest / 系统"
tags:
  - query-understanding
  - query-rewrite
  - nlp
...
# Q040 · 如何设计 Search Autocomplete / Query Suggest？

[← 上一题](Q039-llm-query-rewrite-risks.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · 下一题：无 →
...
| 难度 | 4/5 |
| 频率 | S |
| 适用岗位 | Query Suggest / 系统 |
| 所属章节 | [Query Understanding 与 Query Rewrite](README.md) |
| 核心标签 | `query-understanding`, `query-rewrite`, `nlp`, `autocomplete` |

## 面试官到底在考什么

这道题表面在问“如何设计 Search Autocomplete / Query Suggest？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Autocomplete 通常是低延迟的候选生成 + 排序系统：Prefix/FST 找候选，结合频率、近期趋势、个性 化、拼写和安全策略排序。

## 5 分钟深度回答

- 候选：全局 query log、用户历史、热门趋势、实体/商品词典。
- 前缀索引：Trie/FST/edge n-gram；每个 prefix 可缓存 topN。
- Rank features：prefix match、frequency、recency、CTR、session context、personalization。
- Freshness：热点 query 需要流式更新或单独 hot layer，不能等离线全量 rebuild。
- 安全：敏感、隐私泄露、低频个人 query 不能直接进入全局建议。
- 延迟通常比普通搜索更严格，因为用户每敲一个字符都可能触发请求。
...
候选生成与排序分离；prefix index 解决低延迟，ranker 加 popularity/recency/personalization；还要处理 trend、typo、安全过滤和 prefix privacy。
...
- 如何防止 query log 泄露隐私？
- 趋势词如何避免被刷榜？
- 个性化 suggest 如何与全局候选融合？

## 高频失分点 / Gotcha

Autocomplete 不是“Trie 面试题”，而是一个独立的搜索产品。
...
> **练习：** 实现 prefix trie Top-K suggest，并解释热门 query 更新与长尾 query 的内存权衡。

完成标准：能在不看答案的情况下，先用 30 秒给结论，再用 5 分钟白板说明原理、至少两个 trade-off、一个 failure case 和验证指标。

## 一句话记忆

Prefix 找候选，Ranker 决定你真正看到什么。

第五章 Learning to Rank：从 RankNet 到 LambdaMART

掌握搜索排序面试的核心数学链：Pointwise → Pairwise → Lambda → Tree-based Ranking。

题号 题目 难度 Q41 为什么需要 Learning to Rank？BM25 不够吗？ 3/5 Q42 Pointwise Ranking 是什么？优缺点？ 2/5 Q43 Pairwise Ranking 是什么？ 3/5 Q44 Listwise Ranking 是什么？为什么更贴近 NDCG？ 4/5 Q45 RankNet 的核心公式和直觉是什么？ 4/5 Q46 LambdaRank 为什么出现？Lambda 到底是什么？ 5/5 Q47 LambdaMART 是什么？为什么经典？ 5/5 Q48 深度学习时代，为什么 LambdaMART 仍然常见？ 3/5 Q49 搜索 Ranker 常见特征有哪些？如何分类？ 3/5 Q50 什么是 Query-independent Feature？为什么有价值？ 2/5 Q51 搜索排序中的 Feature Leakage 是什么？ 4/5 Q52 Ranker 为什么有时需要 Calibration？ 4/5
