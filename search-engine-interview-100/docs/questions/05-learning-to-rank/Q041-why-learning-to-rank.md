---
id: Q041
title: "为什么需要 Learning to Rank？BM25 不够吗？"
chapter: 5
...
# Q041 · 为什么需要 Learning to Rank？BM25 不够吗？

← 上一题：无 · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q042-pointwise-ranking.md)
...
| 核心标签 | `learning-to-rank`, `lambdamart`, `ranking`, `bm25` |

## 面试官到底在考什么

这道题表面在问“为什么需要 Learning to Rank？BM25 不够吗？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

BM25 只利用词项匹配与集合统计， 而真实相关性由数十到数千种 query、 document、 query-document、 user 和行为特征共同决定。LTR 用数据学习这些特征如何组合。

## 5 分钟深度回答

- 典型特征：BM25、
title exact match、 freshness、 authority、 CTR、 dwell、 price、 quality、 embedding sim。
- LTR 的学习对象是“相对排序”，与普通回归/分类的目标不同。
- 经典路线包括 Pointwise、Pairwise、Listwise；工业经典算法是 LambdaMART。
- 神经模型兴起后，LTR 仍常作为最终 feature fusion 层或蒸馏/重排层。
- 没有稳定 label 和去偏机制，模型再复杂也只会学习历史排序偏差。
...
LTR 让多个异构 relevance/quality/behavior 特征联合学习；BM25 只建模 lexical matching；但 LTR 仍依赖候选 recall 上限。
...
- 为什么不把所有特征直接线性加权？
- LTR 的 training group 为什么以 query 为单位？

## 高频失分点 / Gotcha

“LTR = 用 XGBoost 排序”太窄；应先讲 ranking learning problem。
