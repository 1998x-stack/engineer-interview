...
# Q043 · Pairwise Ranking 是什么？

[← 上一题](Q042-pointwise-ranking.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q044-listwise-ranking.md)
...
这道题表面在问“Pairwise Ranking 是什么？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Pairwise 把同一 query 下两个文档构成一对，学习 di 是否应排在 dj 前。RankSVM、RankNet 都是 典型方法。

## 5 分钟深度回答

- 将 graded labels 转为偏好对：若 yi > yj ，期望 si > sj 。
- 优点是直接对相对顺序建模，不需要 score 在 query 间可比较。
- 缺点是样本对数量可能 O(n2 )，需要采样。
- 所有 pair 同权仍不理想：把第 1、2 名排反和第 100、101 名排反，对业务影响不同。
- LambdaRank 的核心动机之一就是按 ranking metric 的变化对 pair gradient 加权。
...
Pairwise 直接学习 d+ > d-，更贴近排序；pair 数可 O(n^2)，需要采样；不同位置 pair 对 NDCG 价值不同。
...
Pairwise 解决了相对性，但还没有天然解决“顶部更重要”。
