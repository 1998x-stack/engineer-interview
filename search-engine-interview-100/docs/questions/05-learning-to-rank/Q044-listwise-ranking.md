---
id: Q044
title: "Listwise Ranking 是什么？为什么更贴近 NDCG？"
chapter: 5
...
# Q044 · Listwise Ranking 是什么？为什么更贴近 NDCG？

[← 上一题](Q043-pairwise-ranking.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q045-ranknet.md)
...
| 核心标签 | `learning-to-rank`, `lambdamart`, `ranking`, `ndcg` |

## 面试官到底在考什么

这道题表面在问“Listwise Ranking 是什么？为什么更贴近 NDCG？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Listwise 直接把一个 query 的候选列表视作训练单元，用整个 permutation/list distribution 构造损 失，因此天然更接近最终排序任务，但计算和优化更复杂。

## 5 分钟深度回答

- Pointwise：单文档；Pairwise：文档对；Listwise：整列表。
- ListNet/ListMLE 等方法对排列概率或 top-one probability 建模。
- 真实指标如 NDCG 不平滑、离散，通常仍需 surrogate 或 Lambda 技巧。
- Listwise loss 的 batch/group 构造必须保持 query boundary。
- 候选集合变化会改变 list distribution，因此训练与 serving 候选分布一致很重要。
...
Listwise 使用整个 query list/排列或 list loss，能利用组结构；难点是可微 surrogate、变长 list 与大候选成本；与 NDCG 更一致但不等于直接优化离散 NDCG。
...
- Listwise 是否一定比 Pairwise 好？
- 一个 query 候选数不等如何 batch？

## 高频失分点 / Gotcha

“Listwise = 直接对 NDCG 求梯度”不准确；很多 listwise 方法优化的是可微 surrogate。
...
排序是列表问题，但训练常要用 surrogate 绕开不可微指标。
