...
# Q049 · 搜索 Ranker 常见特征有哪些？如何分类？

[← 上一题](Q048-why-lambdamart-still-used.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q050-query-independent-features.md)
...
这道题表面在问“搜索 Ranker 常见特征有哪些？如何分类？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

最好按 Query、Document、Query-Document、User/Context、Behavior 五类回答，显示你理解特 征是否依赖 query、是否可离线预计算，以及是否存在 leakage。

## 5 分钟深度回答

- Query：长度、语言、频次、意图、是否 head/tail。
- Document：quality、authority、freshness、popularity、长度、类型。
- Query-Document：BM25、字段 exact/phrase match、embedding similarity、entity match。
- User/Context：地域、设备、历史偏好、session intent。
- Behavior：CTR、CVR、dwell、skip、reformulation，但必须做去偏和时间窗切分。
- 工程上还应标注特征：offline/static、nearline、realtime，以及获取成本。
...
**追问 3：为什么 query frequency 也可能是强特征？**
...
按 query、doc、query-doc、user/context、behavior 分 feature；额外强调 freshness/availability 等必须 point-in-time；不要把高基数 ID 当作无约束特征。
...
- 哪些特征可以预计算？
- 实时 CTR 如何做平滑？
- 为什么 query frequency 也可能是强特征？

## 高频失分点 / Gotcha

只列特征名不如说明“特征从哪里来、多久更新、线上怎么取”。
