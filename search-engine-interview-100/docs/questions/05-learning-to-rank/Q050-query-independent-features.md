---
id: Q050
title: "什么是 Query-independent Feature？为什么有价值？"
chapter: 5
...
# Q050 · 什么是 Query-independent Feature？为什么有价值？

[← 上一题](Q049-search-ranker-features.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q051-feature-leakage.md)
...
这道题表面在问“什么是 Query-independent Feature？为什么有价值？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Query-independent 特征只依赖文档本身， 例如 PageRank、 质量、 freshness、 spam score、 popularity。 它们可离线计算并缓存，对所有 query 复用。

## 5 分钟深度回答

- Query-dependent 特征如 BM25、embedding similarity 必须结合当前 query 计算。
- Query-independent signals 可防止“关键词匹配很好但内容低质”的结果占据顶部。
- 新鲜度通常需要按当前时间动态衰减，因此严格说会依赖 request time，但不依赖 query text。
- 预计算能显著降低 Rank 阶段特征获取成本。
...
### 追问链：参考回答
**追问 1：PageRank 为什么是 query-independent？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 2：Popularity 会产生什么偏差？**

曝光概率不是常数。需要随机 intervention 或 click model 估计 propensity，再用 IPS/SNIPS 等校正，同时控制小 propensity 造成的高方差。
...
PageRank、质量、freshness、popularity 可离线预计算并缓存；它们提供 query-independent prior，但过强会压制长尾精准匹配。
...
- PageRank 为什么是 query-independent？
- Popularity 会产生什么偏差？

## 高频失分点 / Gotcha

“不依赖 query”不等于“永远静态”；freshness/popularity 仍会随时间变化。
