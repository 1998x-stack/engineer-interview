---
id: Q051
title: "搜索排序中的 Feature Leakage 是什么？"
chapter: 5
...
# Q051 · 搜索排序中的 Feature Leakage 是什么？

[← 上一题](Q050-query-independent-features.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q052-ranker-calibration.md)
...
这道题表面在问“搜索排序中的 Feature Leakage 是什么？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

训练时使用了 serving 时刻不可获得、或由未来行为计算出的特征，就会产生 leakage，使离线指标虚高、 上线崩溃。搜索日志尤其容易发生时间穿越。

## 5 分钟深度回答

- 例：用“未来 7 天 CTR”训练今天的样本。
- 例：特征聚合窗口包含了该曝光之后发生的点击。
- 例：人工 judgment 只对历史 Top results 标注，导致 selection bias。
- 解决：point-in-time join、event time watermark、feature snapshot/version、严格 train/val
时间切分。
- 训练与 serving 还要避免 feature definition skew，例如离线 Python 逻辑和线上 C++ 实现不同。
...
未来 CTR、最终购买状态、重建后统计都是 leakage 典型；解决靠 event-time join、feature timestamp、backfill parity 和 time-split validation。
...
- 随机划分 train/test 为什么可能泄漏？
- 同一 query 的不同时间样本如何切分？

## 高频失分点 / Gotcha

搜索数据中“时间”不是普通字段，而是防泄漏的第一原则。
...
> **练习：** 列出 5 种 feature leakage，并逐个写出 point-in-time 修复方案。
