---
id: Q053
title: "Precision@K 与 Recall@K 分别衡量什么？"
chapter: 6
...
# Q053 · Precision@K 与 Recall@K 分别衡量什么？

← 上一题：无 · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q054-mrr.md)
...
这道题表面在问“Precision@K 与 Recall@K 分别衡量什么？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Precision@K 看前 K 个结果里有多少是相关的；Recall@K 看所有相关结果中有多少被前 K 个找到了。 召回阶段更关心 Recall，用户可见顶部排序更关心 Precision/NDCG。

## 5 分钟深度回答

- P @K = #relevant@K/K。
- R@K = #relevant@K/#all relevant。
- 当 total relevant 不可完整标注时，Recall 的估计会受 judgment pool 影响。
- RAG retrieval 中 Recall@K 常非常关键，因为漏掉证据后 generator 无法恢复。

## 数学 / 白板推导

### Precision / Recall

$$\mathrm{Precision@K}=\frac{|\mathrm{Relevant}\cap \mathrm{TopK}|}{K}$$

$$\mathrm{Recall@K}=\frac{|\mathrm{Relevant}\cap \mathrm{TopK}|}{|\mathrm{Relevant}|}$$

召回阶段通常首先关心 Recall；最终 SERP 顶部更强调 Precision/NDCG。
...
### 追问链：参考回答
**追问 1：Precision 与 Recall 能同时无限提高吗？**
...
Precision 看前 K 的纯度，Recall 看相关文档覆盖；召回阶段优先 Recall，SERP 顶部更关注 precision/NDCG；K 的选择必须与产品展示位一致。
...
- Precision 与 Recall 能同时无限提高吗？
- 为什么搜索常不用单一 F1？

## 高频失分点 / Gotcha

搜索是排名问题，仅 P/R 还没考虑相关结果出现的位置。
...
Precision 看“前面干不干净”，Recall 看“该找的漏没漏”。
