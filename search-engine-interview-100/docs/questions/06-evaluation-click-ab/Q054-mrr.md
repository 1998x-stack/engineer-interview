...
[← 上一题](Q053-precision-recall-at-k.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q055-map-average-precision.md)
...
| 核心标签 | `evaluation`, `click-bias`, `ab-testing`, `mrr` |

## 面试官到底在考什么

这道题表面在问“MRR 是什么？适合什么场景？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

MRR 只关心第一个 relevant result 的排名，因此非常适合导航搜索、问答、实体查找等“找到一个正 确答案就基本完成任务”的场景。

## 5 分钟深度回答

- 若第一个相关结果排名为 rq ，则 RRq = 1/rq 。
P
- M RR = |Q|   q RRq 。

- 第二个、第三个相关结果不会继续贡献，因此不适合需要多个相关结果的 broad search。
- MRR 对顶部位置极其敏感。
...
$$RR(q)=\frac{1}{r_q},\qquad MRR=\frac{1}{|Q|}\sum_q RR(q)$$

它只关注“第一个相关结果何时出现”，因此非常适合导航型搜索、QA、实体查找；不适合评价需要返回许多相关结果的任务。
...
### 追问链：参考回答
**追问 1：如果第一个 relevant 在 rank1 与 rank2，差多少？**

k1 控制 TF saturation。越大 TF 的额外出现次数越有影响；接近 0 时 TF 的重复贡献趋弱。

**追问 2：电商搜索为什么通常不能只用 MRR？**
...
MRR 只关心第一个 relevant 的倒数排名，适合导航/QA；多个相关结果质量不会被充分奖励，因此不适合所有 web/ecommerce 列表。
...
- 如果第一个 relevant 在 rank1 与 rank2，差多少？
- 电商搜索为什么通常不能只用 MRR？
