...
# Q055 · MAP 是什么？与 MRR 有何不同？

[← 上一题](Q054-mrr.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q056-ndcg.md)
...
| 核心标签 | `evaluation`, `click-bias`, `ab-testing`, `mrr`, `map` |

## 面试官到底在考什么

这道题表面在问“MAP 是什么？与 MRR 有何不同？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Average Precision 在每个 relevant result 出现的位置计算 Precision，并对这些值求平均；MAP 再 对 query 求均值。因此它会考虑多个 relevant results，而不是只看第一个。
...
- 如果相关文档都集中在前面，AP 高。
- MAP 适合 binary relevance；graded relevance 通常更偏向 NDCG。
- Incomplete judgments 会影响 MAP，尤其尾部未判文档。
...
其中 $R$ 是该 query 的相关文档总数。MAP 是对 query 的 AP 取平均。与 MRR 不同，AP 会奖励多个相关文档被持续排到前面。
...
AP 对每个 relevant 位置累计 precision，MAP 再跨 query 平均；相比 MRR 能评价多个相关结果，但通常假设二元 relevance。
...
- MAP 与 NDCG 谁更适合 graded labels？
- 为什么 AP 的分母是 relevant 总数？

## 高频失分点 / Gotcha

不要把 MAP 误解为 mean precision@K。
