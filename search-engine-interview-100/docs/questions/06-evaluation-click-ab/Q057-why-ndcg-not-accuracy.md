---
id: Q057
title: "为什么 NDCG 比 Accuracy 更适合 Search？"
chapter: 6
...
# Q057 · 为什么 NDCG 比 Accuracy 更适合 Search？

[← 上一题](Q056-ndcg.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q058-recall-up-ndcg-down.md)
...
这道题表面在问“为什么 NDCG 比 Accuracy 更适合 Search？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Accuracy 把每个样本当独立分类，无法区分“相关结果排第 1”和“排第 100”的巨大体验差异，也无 法自然处理 graded relevance。

## 5 分钟深度回答

- 搜索输出是 ordered list，不是单标签分类。
- 顶部位置暴露和用户注意力远高于尾部，因此 position discount 必要。
- 一个系统可能预测相关/不相关准确率很高，但把少数真正重要的相关结果全排在后面。
- 因此 ranking metrics 必须评价顺序而非仅标签预测。
...
Accuracy 不编码顺序和位置价值；NDCG 同时利用 graded label 与 position discount；搜索错在 rank1 比 rank100 更严重。
...
- ROC-AUC 适合排序吗？
- 为什么线上 CTR 也不是 NDCG 的替代品？

## 高频失分点 / Gotcha

分类指标可以辅助诊断，但不能替代 list-level ranking metric。
