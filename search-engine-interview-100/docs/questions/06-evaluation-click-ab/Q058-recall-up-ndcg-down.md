---
id: Q058
title: "Recall@1000 提升但 NDCG@10 下降，怎么解释？"
chapter: 6
...
# Q058 · Recall@1000 提升但 NDCG@10 下降，怎么解释？

[← 上一题](Q057-why-ndcg-not-accuracy.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q059-clicks-vs-relevance-labels.md)
...
这道题表面在问“Recall@1000 提升但 NDCG@10 下降，怎么解释？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

说明“候选更全”并不代表“顶部更好”。新召回通道可能引入大量 hard negatives，改变 Ranker 的候 选分布；也可能 Ranker 没学会利用新通道，或候选预算挤掉了原来高质量候选。

## 5 分钟深度回答

- 先做 oracle analysis：在新候选集合上，用真实 label 的理想排序看 NDCG 上限是否提高。
- 分析 relevant docs 是否确实更多进入 candidate set。
- 检查 Ranker training 是否包含新召回分布；若没有，会发生 distribution shift。
- 按 recall source 打 feature、score 和 topK occupancy，定位谁挤占了位置。
- 必要时对 source 做 quota、fusion 或重新训练 ranker。
...
先做 oracle 分解：新候选是否包含更多 relevant？ranker 是否见过这种新候选分布？可能是 recall channel 提升但 downstream ranker calibration/feature 缺失。
...
- Recall 增量为什么可能是“无效 recall”？
- 如何做 channel ablation？

## 高频失分点 / Gotcha

一个阶段指标上升不等于 end-to-end 上升；搜索必须按 cascade 做归因。
