---
id: Q059
title: "为什么 Click 不能直接当 Relevance Label？"
chapter: 6
...
# Q059 · 为什么 Click 不能直接当 Relevance Label？

[← 上一题](Q058-recall-up-ndcg-down.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q060-position-bias-ips.md)
...
这道题表面在问“为什么 Click 不能直接当 Relevance Label？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

用户只会点击被展示且被看到的结果，所以 click 同时受 relevance 与 examination 影响。历史排名制 造了曝光偏差，直接训练会把旧模型偏差复制到新模型。

## 5 分钟深度回答

- Position bias：rank1 获得更多 exposure。
- Selection bias：未进入历史 topK 的文档永远没有点击 label。
- trust/presentation bias：品牌与视觉样式影响点击。
- 点击还包含 attractiveness，而不纯是任务满意度。
- 可加入 dwell、return-to-SERP、reformulation 等后续行为改善 label。
...
Click = relevance × examination × presentation 等多因素；未点击不等于不相关；曝光机制由旧 ranker 决定，形成 selection bias。
...
- 未点击结果能否作为负样本？
- Skip-above click 能说明什么？

## 高频失分点 / Gotcha

“曝光未点击 = 不相关”同样不成立，尤其用户可能根本没看到该结果。
