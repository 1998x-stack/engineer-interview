---
id: Q060
title: "如何处理 Position Bias？什么是 IPS？"
chapter: 6
...
# Q060 · 如何处理 Position Bias？什么是 IPS？

[← 上一题](Q059-clicks-vs-relevance-labels.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q061-offline-up-online-down.md)
...
| 核心标签 | `evaluation`, `click-bias`, `ab-testing`, `ips` |

## 面试官到底在考什么

这道题表面在问“如何处理 Position Bias？什么是 IPS？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

核心思想是估计每个位置被 examination 的 propensity，并用其倒数对观察到的点击重新加权，使高位 点击不被系统性高估。随机化/intervention 常用于估计 propensity。

## 5 分钟深度回答

- 简化点击模型：P (C = 1) = P (E = 1 | position)P (R = 1 | q, d)。
- IPS 风险估计对样本乘 1/pi ；低 propensity 位置的点击权重更大。
- Propensity 很小时权重会爆炸，需要 clipping/self-normalization 等方差控制。
- Interleaving/random swaps 能获得更可靠的相对偏好，但要控制用户体验成本。
- Unbiased LTR 的难点从“拟合模型”转向“识别曝光机制”。

## 数学 / 白板推导

### IPS 的最小公式

若位置 $i$ 被观察到的 propensity 为 $p_i$，点击样本损失为 $\ell_i$，一个最简 propensity-weighted 估计形式是：

$$\hat{R}_{IPS}=\frac{1}{n}\sum_i\frac{c_i}{p_i}\ell_i$$

真正的难点不是写出 $1/p_i$，而是 propensity 如何由随机化/intervention 或可靠 click model 估计，以及极小 propensity 带来的方差爆炸。
...
**追问 2：如果 propensity model 错了会怎样？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 3：如何做 doubly robust？**
...
IPS 用 propensity 的倒数重加权已曝光点击；小 propensity 会导致高方差，需要 clipping/self-normalization；propensity 必须可识别或通过随机化估计。
...
$$\hat R=\frac{1}{n}\sum_i \frac{y_i\,\ell_i}{p_i}$$

其中 `p_i` 是被曝光/检查的 propensity。工程上常做 weight clipping 或 self-normalized IPS，防止非常小的 `p_i` 产生巨大方差。
...
- IPS 为什么方差大？
- 如果 propensity model 错了会怎样？
- 如何做 doubly robust？

## 高频失分点 / Gotcha

不要只说“随机打乱位置”；面试要说明为什么随机化能估计 examination propensity。
...
> **练习：** 设计一个小流量随机 swap experiment 来估计 position propensity，并讨论方差与用户风险。
