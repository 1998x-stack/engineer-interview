---
id: Q061
title: "Offline 指标涨了，Online CTR/满意度为什么可能下降？"
chapter: 6
...
# Q061 · Offline 指标涨了，Online CTR/满意度为什么可能下降？

[← 上一题](Q060-position-bias-ips.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q062-search-ab-testing.md)
...
这道题表面在问“Offline 指标涨了，Online CTR/满意度为什么可能下降？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

离线 judgment/log 数据只是线上用户分布的近似，而且不包含延迟、展示、系统交互与长期行为。离线 改进可能过拟合 label 或损害未被评价的目标。

## 5 分钟深度回答

- Label mismatch：人工 relevance 与用户点击/转化目标不同。
- Distribution shift：评测集 head query 过多，线上 tail query 更多。
- Latency regression：模型相关性更好，但 p99 增加导致用户流失。
- Diversity/freshness 未纳入离线指标。
- 日志本身有历史系统 selection bias。
- 局部指标提升可能伤害长期指标，例如短点击变多、long click 下降。
...
### 追问链：参考回答
**追问 1：如何设计 offline-online correlation study？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 2：为什么 slice metrics 很重要？**
...
离线 label/traffic 与线上用户目标不一致、latency/freshness 变化、novelty 和 feedback loop 都会造成反转；必须用 slice 和 guardrail 定位。
...
- 如何设计 offline-online correlation study？
- 为什么 slice metrics 很重要？
