---
id: Q063
title: "Dense Retrieval 与 BM25 的本质区别是什么？"
chapter: 7
chapter_title: "Dense Retrieval 与 ANN"
difficulty: 3
frequency: "S"
roles: "Dense Retrieval"
tags:
  - dense-retrieval
  - ann
  - vector-search
...
# Q063 · Dense Retrieval 与 BM25 的本质区别是什么？

← 上一题：无 · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q064-dual-encoder-retrieval.md)
...
| 难度 | 3/5 |
| 频率 | S |
| 适用岗位 | Dense Retrieval |
| 所属章节 | [Dense Retrieval 与 ANN](README.md) |
| 核心标签 | `dense-retrieval`, `ann`, `vector-search`, `bm25`, `trie` |

## 面试官到底在考什么

这道题表面在问“Dense Retrieval 与 BM25 的本质区别是什么？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

BM25 在词项空间做稀疏 lexical matching；Dense Retrieval 把 query/document 编码为低维向量， 通过内积、cosine 或 L2 找语义邻居。它解决 vocabulary mismatch，但会失去部分 exact sparse 信 号。

## 5 分钟深度回答

- BM25 的每个维度对应词表 term；dense embedding 的维度没有直接词义解释。
- Dense 可把表面词不同但语义接近的文本拉近。
- Dense 对实体、数字、型号等 exact matching 可能不如 BM25 稳。
- Dense 检索需要 ANN index；BM25 依赖 inverted index。
- 两者互补是 hybrid search 成为主流的重要原因。
...
Sparse lexical 与 dense semantic 的 inductive bias 不同；dense 缓解词汇失配但可能损失 rare identifier/exact constraint；最常见解法是 hybrid。
...
- Dense 是否还需要 tokenizer？
- 为什么 embedding normalization 很重要？

## 高频失分点 / Gotcha

不要说 Dense “理解语义所以总比 BM25 好”；它的错误模式不同。
