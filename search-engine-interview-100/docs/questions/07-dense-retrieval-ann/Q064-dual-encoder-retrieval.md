...
# Q064 · 为什么 Dual Encoder 适合召回？

[← 上一题](Q063-dense-retrieval-vs-bm25.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q065-dual-encoder-training.md)
...
| 核心标签 | `dense-retrieval`, `ann`, `vector-search`, `dual-encoder` |

## 面试官到底在考什么

这道题表面在问“为什么 Dual Encoder 适合召回？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Dual Encoder 独立编码 query 和 document，使 document embedding 可以离线预计算并建立 ANN 索引；在线只算一次 query embedding，再进行向量检索。

## 5 分钟深度回答

- q = Eq (query)，d = Ed (doc)，score 常为 q T d。
- 文档向量可复用给所有 query，这是其相对 Cross-Encoder 的决定性效率优势。
- 代价是 query-document token 不能在 Transformer 内部做全交互，表达能力受限。
- 因此常用于第一阶段 recall，再用 Cross-Encoder rerank。
...
### 追问链：参考回答
**追问 1：Query encoder 和 Doc encoder 是否共享参数？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 2：为什么 asymmetric retrieval 常不共享？**
...
doc embedding 可离线预计算，query 只在线编码一次，之后 ANN；Cross-Encoder 需每个 pair 联合前向，不能直接做全库索引。
...
- Query encoder 和 Doc encoder 是否共享参数？
- 为什么 asymmetric retrieval 常不共享？

## 高频失分点 / Gotcha

Dual Encoder 的价值不只是“快”，而是把在线 pair computation 变成可索引表示。
