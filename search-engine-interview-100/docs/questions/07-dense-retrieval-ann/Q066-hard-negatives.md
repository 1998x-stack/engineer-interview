---
id: Q066
title: "为什么 Hard Negative 对 Dense Retrieval 至关重要？"
chapter: 7
...
# Q066 · 为什么 Hard Negative 对 Dense Retrieval 至关重要？

[← 上一题](Q065-dual-encoder-training.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q067-in-batch-negatives.md)
...
| 核心标签 | `dense-retrieval`, `ann`, `vector-search`, `trie`, `hard-negative` |

## 面试官到底在考什么

这道题表面在问“为什么 Hard Negative 对 Dense Retrieval 至关重要？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

随机负样本通常与 query 完全无关，模型很快学会区分，梯度趋于无信息。Hard negative 在主题/词面 上很像正确答案却不真正 relevant，迫使模型学习细粒度语义边界。

## 5 分钟深度回答

- 经典做法：BM25 top results 中排除正样本，作为 hard negatives。
- 也可用当前 dense model 自己挖 top false positives，进行 iterative mining。
- Too-hard negative 可能其实是未标注 relevant（false negative），会伤害训练。
- 应混合 easy + medium + hard negatives，保持优化稳定。
- 负样本分布最好接近 serving candidate distribution。
...
随机负样本太容易导致梯度快速饱和；hard negatives 提供决策边界，但 false negative 会伤害表示；常用 BM25/ANN mining + teacher filter。
...
- BM25 negative 为什么特别有价值？
- 如何发现 false negatives？

## 高频失分点 / Gotcha

“越难越好”是错误的；错误标注的超难负样本会把真正相关文档推远。
...
> **练习：** 比较 random negatives、BM25 hard negatives、cross-encoder mined negatives 的收益与风险。

完成标准：能在不看答案的情况下，先用 30 秒给结论，再用 5 分钟白板说明原理、至少两个 trade-off、一个 failure case 和验证指标。

## 一句话记忆

Hard negative 要难，但必须真的负。
...
- **R11** [Dense Passage Retrieval](https://arxiv.org/abs/2004.04906)
