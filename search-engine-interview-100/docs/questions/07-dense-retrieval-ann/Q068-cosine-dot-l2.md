---
id: Q068
title: "Cosine、Inner Product、L2 在归一化向量下有什么关系？"
chapter: 7
...
# Q068 · Cosine、Inner Product、L2 在归一化向量下有什么关系？

[← 上一题](Q067-in-batch-negatives.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q069-why-ann-not-bruteforce.md)
...
这道题表面在问“Cosine、Inner Product、L2 在归一化向量下有什么关系？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

若向量都做 L2 normalization，则 cosine similarity 就是 inner product，而平方 L2 距离 ∥x − y∥2 = 2 − 2xT y，三者给出相同排序。

## 5 分钟深度回答

- 未归一化时 inner product 同时受方向与向量模长影响。
- 有些 retriever 训练本来就利用 norm 表达置信度，这时不能随意 normalize。
- Faiss 做 cosine 常采用 normalize vectors + IndexFlatIP/inner-product index。
- ANN 索引的 metric 必须与训练时的 similarity 定义一致。
...
因此 cosine、inner product 与 L2 排序可以互相转换。Faiss 文档也建议做 cosine search 时先归一化向量，再使用 inner product index。
...
### 追问链：参考回答
**追问 1：为什么 MIPS 不是普通 Euclidean NN？**
...
单位向量时 ||x-y||²=2-2x·y，因此 cosine、IP、L2 ranking 等价；未归一化时向量范数会成为额外信号，不能混淆。
...
$$\|x-y\|_2^2=\|x\|^2+\|y\|^2-2x^\top y=2-2x^\top y$$

因此最大化 cosine / inner product 等价于最小化 L2 距离；不做归一化时这个等价关系不成立。
...
- 为什么 MIPS 不是普通 Euclidean NN？
- 归一化是否会改变训练模型效果？

## 高频失分点 / Gotcha

不要在模型训练后无脑把所有向量归一化；先确认训练目标。
...
Unit norm 下：cosine = dot，L2 只是单调变换。
