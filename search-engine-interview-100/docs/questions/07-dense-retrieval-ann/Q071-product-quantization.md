---
id: Q071
title: "Product Quantization（PQ）是什么？"
chapter: 7
...
# Q071 · Product Quantization（PQ）是什么？

[← 上一题](Q070-ivf-index.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q072-hnsw-principles.md)
...
| 核心标签 | `dense-retrieval`, `ann`, `vector-search`, `pq` |

## 面试官到底在考什么

这道题表面在问“Product Quantization（PQ）是什么？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

PQ 将 d 维向量切成 m 个低维子向量，每个子空间独立学习码本，只存每个子向量对应的 code。距离可 用 lookup table 在压缩域近似计算。

## 5 分钟深度回答

- 若每个子量化器 256 个中心，只需 8 bit 表示一个 code。
- m = 16 时一个向量可压到 16 bytes（不含 id）
，远小于 768-d float32 的 3072 bytes。
- 查询时对每个子空间预计算 query 到 codewords 的距离表，再累加 code lookup。
- PQ 引入 quantization error，代码越短压缩越强但 recall 降低。
- OPQ 在 PQ 前学习旋转/线性变换，使子空间更适合量化。

## 数学 / 白板推导

### Product Quantization

把 $d$ 维向量拆成 $m$ 个子空间，每个子空间独立量化成 codeword。若每段用 8 bit，则每个子向量只需 1 byte 的 code index。PQ 的价值是用可控的量化误差换取数量级的内存与带宽下降。
...
**追问 2：IVFPQ 为什么通常对 residual 做 PQ？**
...
PQ 把向量切 m 个子空间，各自量化为 codebook index；压缩内存并可用 lookup table 近似距离；m、nbits 决定码长与失真。
...
若 `d=768, m=96, nbits=8`，每个子空间只存 1 byte code，总码长约 `96 bytes/vector`，相比 float32 原始 `3072 bytes` 大幅压缩；但还要额外存 coarse index、IDs、codebooks 等。
...
- 为什么 PQ 要分子空间？
- IVFPQ 为什么通常对 residual 做 PQ？
- ADC 是什么？

## 高频失分点 / Gotcha

PQ 不是把 float32 简单变 int8；它是学习式向量量化。
