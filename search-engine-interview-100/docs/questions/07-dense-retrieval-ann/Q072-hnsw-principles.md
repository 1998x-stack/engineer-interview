...
# Q072 · HNSW 的原理是什么？为什么分层？

[← 上一题](Q071-product-quantization.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q073-hnsw-parameters.md)
...
这道题表面在问“HNSW 的原理是什么？为什么分层？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

HNSW 构建多层 proximity graph：上层节点少、连接跨度大，用于快速粗粒度导航；逐层下降到更密 的底层图做局部精细搜索，类似 skip list 的多尺度跳跃。

## 5 分钟深度回答

- 每个向量随机分配最高层，层数概率指数衰减，因此高层非常稀疏。
- 搜索从顶层入口开始 greedy navigation，再到下一层，以当前最好节点为入口。
- 底层使用候选队列扩展邻居，‘efSearch‘ 控制探索宽度。
- 建图时也要搜索邻居并做 heuristic neighbor selection，‘efConstruction‘ 控制质量。
- 图方法在高 recall 区域通常非常强，但 graph links 带来显著内存开销。
...
它不是树，而是多层 proximity graph；查询过程是贪心/beam-like 图遍历。
...
HNSW 高层稀疏图做远距离导航，底层稠密图精细搜索；search 通过 ef 候选集贪心扩展；图质量、memory 与 update 相关。
...
HNSW 搜索可概括为“高层贪心下降 + 底层 ef 候选扩展”。高层负责快速靠近目标区域，底层维护一个 bounded candidate set 持续探索更近邻居。面试里应同时说出它为何快、为什么耗内存。
...
- HNSW 为什么不是严格 O(log N ) 保证？
- 删除向量为什么麻烦？
- 入口点如何选择？

## 高频失分点 / Gotcha

不要把 HNSW 描述成“多层 KNN 图”就结束；要解释层级带来的 long-range navigation。
