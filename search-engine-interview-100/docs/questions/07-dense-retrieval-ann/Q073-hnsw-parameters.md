---
id: Q073
title: "HNSW 的 M、efConstruction、efSearch 分别控制什么？"
chapter: 7
...
# Q073 · HNSW 的 M、efConstruction、efSearch 分别控制什么？

[← 上一题](Q072-hnsw-principles.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q074-hnsw-vs-ivf-pq.md)
...
| 核心标签 | `dense-retrieval`, `ann`, `vector-search`, `hnsw` |

## 面试官到底在考什么

这道题表面在问“HNSW 的 M、efConstruction、efSearch 分别控制什么？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

‘M‘ 决定图的连接度与内存；‘efConstruction‘ 决定建图时搜索宽度和图质量；‘efSearch‘ 决定查询时 探索宽度，是 recall-latency 最直接的在线旋钮。

## 5 分钟深度回答

- ‘M‘ 大：图更密、recall 通常更高、memory/index build cost 更高。
- ‘efConstruction‘ 大：建图更慢，但图连通和近邻质量更好。
- ‘efSearch‘ 大：查询访问更多节点，recall 更高、latency 更大。
- 经验参数不能脱离数据规模、维度、metric 和 QPS；必须跑 recall-latency curve。
- Faiss 当前文档明确把 M、efConstruction、efSearch 列为 HNSW 关键参数。

## 数学 / 白板推导

### HNSW 参数边界

- `M`：图连接数，增大通常提高 recall，也增加构建成本与内存。
- `efConstruction`：建图时搜索宽度，增大通常改善图质量。
- `efSearch`：查询时搜索宽度，是最直接的 recall-latency knob。

Faiss 的索引说明指出 HNSW 链接本身有明显内存开销，因此“高 recall”不是免费的。
...
M 控图连边/内存，efConstruction 控建图质量和时间，efSearch 控查询 recall/latency；三者需要用 exact oracle 画 recall-latency 曲线。
...
调参实验不要只报单点：固定数据集和 exact ground truth，画 `Recall@K vs p95 latency`；再加 `memory/vector`。`efSearch` 通常是线上最直接的质量-延迟旋钮。
...
- M 提高后 efSearch 是否可以降低？
- 为什么建库成本与查询成本可以独立调？

## 高频失分点 / Gotcha

最常见面试错误：把 efConstruction 和 efSearch 说反。
...
> **练习：** 对 HNSW 扫描 M、efConstruction、efSearch，并画三维成本表：recall、memory、build time、latency。

完成标准：能在不看答案的情况下，先用 30 秒给结论，再用 5 分钟白板说明原理、至少两个 trade-off、一个 failure case 和验证指标。

## 一句话记忆

M 管结构，efConstruction 管建图，efSearch 管查询。

## 参考资料

- **R6** [Faiss indexes wiki](https://github.com/facebookresearch/faiss/wiki/Faiss-indexes)
- **R5** [Efficient and Robust Approximate Nearest Neighbor Search Using HNSW](https://arxiv.org/abs/1603.09320)
