...
# Q074 · HNSW 与 IVF-PQ 如何选？

[← 上一题](Q073-hnsw-parameters.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · 下一题：无 →
...
| 核心标签 | `dense-retrieval`, `ann`, `vector-search`, `ivf`, `hnsw` |

## 面试官到底在考什么

这道题表面在问“HNSW 与 IVF-PQ 如何选？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

选择取决于内存预算、规模、更新模式、目标 recall、CPU/GPU 和 build cost。HNSW 通常高 recall、 低延迟但内存重；IVF-PQ 压缩强、适合超大规模和 GPU，但有训练与量化误差。

## 5 分钟深度回答

- HNSW：不依赖 k-means training，在线 recall-latency 好；graph overhead 可达每向量数百字
节量级。
- IVFPQ：向量可压到几十 bytes，适合百亿/十亿级内存约束。
- 频繁更新场景：HNSW 的删除/重建管理较复杂；IVF 也要处理 centroid drift 与 tombstone。
- GPU：Faiss IVF/PQ 路线非常成熟；HNSW 更常见于 CPU 内存索引。
- 最佳答案不是“哪个更强”，而是画出 recall/latency/memory/build/update 五维 trade-off。
...
HNSW 高 recall/低 latency但内存大；IVF-PQ 压缩强、适合超大规模/磁盘/GPU但有量化误差与训练；过滤和动态更新会改变选择。
...
- 1B 768-d、128GB RAM 怎么选？
- 如果要求 99.9
- 如果要求每日全量重建呢？

## 高频失分点 / Gotcha

任何不带约束的“选 HNSW/IVF”都是不完整答案。
