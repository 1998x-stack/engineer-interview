---
id: Q077
title: "什么是 Reciprocal Rank Fusion（RRF）？"
chapter: 8
...
# Q077 · 什么是 Reciprocal Rank Fusion（RRF）？

[← 上一题](Q076-bm25-dense-score-fusion.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q078-splade-sparse-neural-retrieval.md)
...
| 核心标签 | `hybrid-search`, `reranking`, `rag`, `rrf` |

## 面试官到底在考什么

这道题表面在问“什么是 Reciprocal Rank Fusion（RRF）？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

RRF 用文档在各个结果列表中的名次，而不是原始 score，计算 reciprocal-rank 加和。它简单、无需 训练、对不同 score scale 鲁棒，因此是 hybrid search 常见 baseline。

## 5 分钟深度回答

- 若文档在第 ri (d) 名，则贡献 1/(k + ri (d))。
- k 是平滑常数，降低 top rank 极端差异。
- 某文档被多路检索都排得较高，会得到累积优势。
- RRF 不需要知道 BM25 20 分与 cosine 0.82 哪个“更大”。
- 缺点是丢掉了原 score magnitude，且通道权重默认相同，需要扩展时可加权。
...
RRF 只依赖排名，不要求 BM25 与 dense similarity 在同一个数值尺度。Elasticsearch 当前 RRF retriever 默认 `rank_constant=60`；调大常数会让较低排名候选获得更大相对影响。
...
**追问 2：Weighted RRF 怎么做？**

RRF 适合异构 score scale 的无监督融合；之后仍可用 LTR/Cross-Encoder 进行 query-doc 细粒度重排，二者解决的问题不同。

**追问 3：为什么 rank-based fusion 可能损失信息？**
...
RRF(d)=Σ1/(k+rank_i)；只依赖 rank 不依赖 score 标度，简单稳健；k 越大越平滑，且要处理缺失候选与 source weights。
...
- RRF 的 k 怎么影响融合？
- Weighted RRF 怎么做？
- 为什么 rank-based fusion 可能损失信息？

## 高频失分点 / Gotcha

RRF 很强但不是万能：它牺牲 score 信息换鲁棒性。
...
> **练习：** 手算两个 rank list 的 RRF，并尝试不同 rank_constant 看排序变化。
