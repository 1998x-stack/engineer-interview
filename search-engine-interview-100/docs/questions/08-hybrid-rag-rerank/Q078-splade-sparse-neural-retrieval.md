---
id: Q078
title: "SPLADE 这类 Learned Sparse Retrieval 在做什么？"
chapter: 8
...
# Q078 · SPLADE 这类 Learned Sparse Retrieval 在做什么？

[← 上一题](Q077-reciprocal-rank-fusion.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q079-cross-encoder-reranking.md)
...
| 核心标签 | `hybrid-search`, `reranking`, `rag`, `trie`, `splade` |

## 面试官到底在考什么

这道题表面在问“SPLADE 这类 Learned Sparse Retrieval 在做什么？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

SPLADE 使用 Transformer 学习稀疏词表权重与 expansion，使文档/query 仍可通过倒排索引检索， 同时获得神经模型的语义扩展能力。

## 5 分钟深度回答

- 输出维度对应词表，而不是一个低维 dense vector。
- 模型可以给输入中没出现的词分配权重，实现 learned expansion。
- 稀疏正则/效率约束用于控制激活 term 数量，否则倒排索引会爆炸。
- 优势：仍支持 inverted index、exact lexical signal 与一定可解释性。
- 它位于 BM25 与 Dense Retrieval 之间，是“神经化稀疏检索”的代表。
...
### 追问链：参考回答
**追问 1：SPLADE 为什么需要 sparsity regularization？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 2：Learned expansion 会产生什么奇怪 token？**
...
SPLADE 用模型学习高维 sparse term weights 和 expansion，仍可落到 inverted index；代价是 index expansion/latency，需要 sparsity regularization。
...
- SPLADE 为什么需要 sparsity regularization？
- Learned expansion 会产生什么奇怪 token？

## 高频失分点 / Gotcha

不要把 SPLADE 说成“BERT 生成同义词”；它输出的是词表维度上的可学习 sparse weights。
...
SPLADE = 神经模型学会“哪些词该被激活、权重多大”。

## 参考资料

- **R8** [SPLADE v2](https://arxiv.org/abs/2109.10086)
