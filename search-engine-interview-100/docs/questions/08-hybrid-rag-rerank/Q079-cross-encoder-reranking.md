---
id: Q079
title: "为什么 Cross-Encoder 适合 Rerank，而不适合全库召回？"
chapter: 8
chapter_title: "Hybrid Search、Neural Reranking 与 RAG"
difficulty: 3
frequency: "S"
roles: "Neural Ranking"
tags:
  - hybrid-search
  - reranking
  - rag
...
# Q079 · 为什么 Cross-Encoder 适合 Rerank，而不适合全库召回？

[← 上一题](Q078-splade-sparse-neural-retrieval.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q080-dual-vs-cross-encoder.md)
...
这道题表面在问“为什么 Cross-Encoder 适合 Rerank，而不适合全库召回？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Cross-Encoder 把 query 和 document 一起输入 Transformer，token 级交互充分，相关性效果强； 但每个 query-document pair 都要单独前向，无法预计算文档向量，所以只适合小候选集。

## 5 分钟深度回答

- 输入类似 ‘[CLS] query [SEP] doc‘，self-attention 能建模细粒度匹配。
- 候选数 K=100 时做 100 次 pair inference；全库十亿文档显然不可能。
- 常在 BM25/Dense top100–1000 后做 rerank。
- 可用 distillation 将 Cross-Encoder 知识蒸馏给 Dual Encoder/PreRank。
- 长文档需要截断、passage aggregation 或 hierarchical scoring。
...
### 追问链：参考回答
**追问 1：Cross-Encoder 的复杂度如何随 doc length 增长？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 2：如何批量化降低 GPU latency？**
...
Cross-Encoder 允许 q-d token 全交互，精度高；但 K 个候选要 K 次联合编码，成本随 rerank depth 线性增长。
...
- Cross-Encoder 的复杂度如何随 doc length 增长？
- 如何批量化降低 GPU latency？

## 高频失分点 / Gotcha

Cross-Encoder 的最大优势和最大代价来自同一件事：query/doc 在模型内完全交互。
...
> **练习：** 为 Cross-Encoder 设计 candidate depth 与 batch size 的 latency-quality frontier。
...
- **R10** [Passage Re-ranking with BERT](https://arxiv.org/abs/1901.04085)
