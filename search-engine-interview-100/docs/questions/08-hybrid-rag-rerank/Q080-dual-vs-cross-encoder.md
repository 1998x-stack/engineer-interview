---
id: Q080
title: "Dual Encoder 与 Cross-Encoder 的经典 Trade-off 是什么？"
chapter: 8
...
# Q080 · Dual Encoder 与 Cross-Encoder 的经典 Trade-off 是什么？

[← 上一题](Q079-cross-encoder-reranking.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q081-colbert-late-interaction.md)
...
| 核心标签 | `hybrid-search`, `reranking`, `rag`, `dual-encoder` |

## 面试官到底在考什么

这道题表面在问“Dual Encoder 与 Cross-Encoder 的经典 Trade-off 是什么？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Dual Encoder 牺牲 token-level 交互换可索引、低延迟；Cross-Encoder 牺牲吞吐换更强的 pairwise relevance。工业 cascade 通常让它们分工，而不是二选一。

## 5 分钟深度回答

- Dual：doc embedding 可离线、ANN 支持、适合 recall。
- Cross：pair input、无法预计算、适合 rerank。
- Dual 的表示瓶颈是把整段压成一个或少数向量。
- Cross 的瓶颈是 K 倍模型前向与长上下文成本。
...
Dual：可索引、快、交互弱；Cross：不可预计算 pair、慢、交互强；常见架构是 dual recall + cross rerank，并用 distillation 缩小差距。
...
- ColBERT 为什么被称为二者之间？
- Bi-encoder 能否通过多向量缓解瓶颈？

## 高频失分点 / Gotcha

如果回答只说“一个快一个准”，要继续解释为什么。
