---
id: Q083
title: "Retrieval Recall 很高，为什么 RAG 仍会答错？"
chapter: 8
...
# Q083 · Retrieval Recall 很高，为什么 RAG 仍会答错？

[← 上一题](Q082-rag-chunk-size.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q084-agentic-search.md)
...
| 核心标签 | `hybrid-search`, `reranking`, `rag`, `trie` |

## 面试官到底在考什么

这道题表面在问“Retrieval Recall 很高，为什么 RAG 仍会答错？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

因为“证据被召回”只是必要条件。Reranker 可能没把证据放进最终 context，证据可能被冗余 chunk 淹没，生成模型也可能没有正确利用或推理。

## 5 分钟深度回答

- Candidate recall 高，但 final context recall 低：问题在 rerank/context builder。
- 证据存在但位置太靠后：可能出现 lost-in-context。
- 多跳问题只召回一半证据，单文档 relevance 指标看起来仍不错。
- 生成模型可能 hallucinate 或忽略证据，需要 faithfulness/attribution 评估。
- 诊断要分层：Retriever → Reranker → Context → Generator，而不是只看最终准确率。
...
### 追问链：参考回答
**追问 1：如何做 oracle generator 分析？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 2：如何区分 retrieval error 与 reasoning error？**
...
高 document recall 仍可能 chunk 证据不完整、rerank/packing 丢失、上下文冗余、LLM 未利用；要用 stage-wise oracle 做故障归因。
...
- 如何做 oracle generator 分析？
- 如何区分 retrieval error 与 reasoning error？

## 高频失分点 / Gotcha

RAG 评估必须可归因，否则只调 embedding 很容易陷入盲目优化。
