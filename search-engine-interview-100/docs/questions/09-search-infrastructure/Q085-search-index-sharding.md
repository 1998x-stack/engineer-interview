---
id: Q085
title: "为什么 Search Index 要做 Sharding？"
chapter: 9
...
# Q085 · 为什么 Search Index 要做 Sharding？

← 上一题：无 · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q086-scatter-gather-search.md)
...
这道题表面在问“为什么 Search Index 要做 Sharding？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

单机无法承载超大索引的容量、QPS 与故障风险，因此按文档或路由键拆成多个 shard。一次 query 并 发访问相关 shards，再聚合局部 TopK。

## 5 分钟深度回答

- 容量：索引大于单机 RAM/SSD。
- 吞吐：多 shard 并行查询，提高 aggregate QPS。
- 故障隔离：副本可接管。
- 但 shard 越多并非越好，fan-out 与协调开销会增加。
- 路由策略可按 hash 均衡，也可按 tenant/time/category 减少 fan-out，但会带数据倾斜风险。
...
sharding 解决容量和并行吞吐；shard key 决定负载和相关性统计；过度随机会 fan-out 全集，按 tenant/category 路由可减少查询范围。
...
- 按时间分 shard 的新闻搜索如何查询？
- 如何处理 hot shard？

## 高频失分点 / Gotcha

Sharding 解决规模，但会把“单机问题”变成“分布式协调问题”。
...
> **练习：** 给 1B documents × 20 shards 做 QPS / fan-out / replica 数量级估算。
