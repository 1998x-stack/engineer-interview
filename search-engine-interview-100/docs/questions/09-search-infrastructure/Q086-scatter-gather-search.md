---
id: Q086
title: "分布式 Search Query 的 Scatter-Gather 怎么工作？"
chapter: 9
...
# Q086 · 分布式 Search Query 的 Scatter-Gather 怎么工作？

[← 上一题](Q085-search-index-sharding.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q087-distributed-topk-pitfalls.md)
...
这道题表面在问“分布式 Search Query 的 Scatter-Gather 怎么工作？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Coordinator 把 query scatter 到多个 shards，各 shard 本地检索 TopK，再 gather 回协调节点做 全局 merge、可能的第二阶段 fetch/rerank。

## 5 分钟深度回答

- Query phase：各 shard 返回 docIDs + scores 的局部 topK。
- Merge：coordinator 用 heap 合并成 global topK。
- Fetch phase：只对最终文档取 stored fields/snippets，避免所有候选传大 payload。
- 尾延迟受最慢 shard/replica 影响，可用 hedged request 或 timeout/partial result。
- 请求 fan-out 大时 coordinator 本身可能成为瓶颈。
...
coordinator scatter 到 shards，各自 local top-k，再 gather/merge；deadline 和 partial result 关键；总体 p99 常由最慢 shard 决定。
...
merge / global top-k
```
真实系统应给 shard 子请求更短 deadline，预留 coordinator merge 与网络返回时间。
...
分布式搜索的 p99 常由 straggler 决定，而不是平均 shard latency。
