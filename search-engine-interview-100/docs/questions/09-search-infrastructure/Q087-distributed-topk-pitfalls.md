---
id: Q087
title: "为什么每个 Shard 只返回 Local TopK 可能有问题？"
chapter: 9
...
# Q087 · 为什么每个 Shard 只返回 Local TopK 可能有问题？

[← 上一题](Q086-scatter-gather-search.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q088-primary-vs-replica-shard.md)
...
这道题表面在问“为什么每个 Shard 只返回 Local TopK 可能有问题？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

全局 TopK 需要保证所有潜在赢家都被局部候选带回来。如果每 shard 的 K 太小、评分统计又不一致， 可能截断真正的全局候选或产生 score comparability 问题。

## 5 分钟深度回答

- 简单同分布 additive scoring 下，每 shard 返回 K 通常足以合 global K，但实际有 rescore/fu-
sion/feature fetch 等复杂阶段。
- 若后续 rerank 使用 shard 未参与的全局特征，local rank 与 global rank 可能差异很大，需要
over-fetch。
- BM25 的 shard-local DF/avgdl 可能造成 score 不可比，需要 global stats 或接受近似。
- Hybrid source fusion 也常需要从每路/每 shard 多取一些候选。
- 因此系统常设置 shard_size > final_size。
...
若每 shard 只回 K，某 shard 的 K+1 也可能进 global top-k（视 score/统计/merge 方式）；实际常 over-fetch K×factor 并统一 merge。
...
- 为什么 oversampling 能提升全局质量？
- Global IDF 如何获得？

## 高频失分点 / Gotcha

不要机械说“local TopK 一定错误”；要说明什么条件下会出问题，以及为什么工程上常 over-fetch。
