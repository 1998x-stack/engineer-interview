---
id: Q088
title: "Primary Shard 与 Replica 的区别是什么？"
chapter: 9
...
# Q088 · Primary Shard 与 Replica 的区别是什么？

[← 上一题](Q087-distributed-topk-pitfalls.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q089-over-sharding.md)
...
这道题表面在问“Primary Shard 与 Replica 的区别是什么？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

Primary 是某个 shard 的主写入副本，负责接收/协调写操作；Replica 保存冗余数据，提供容错并可分 担读查询。

## 5 分钟深度回答

- Replica 提高 read capacity 与 availability，但也增加 storage 和 replication traffic。
- Primary 故障后可选 replica 提升。
- 复制通常是近实时而非严格零延迟，系统要定义 consistency semantics。
- 查询路由应做 replica load balancing，避免某台机器热点。
...
### 追问链：参考回答
**追问 1：增加 replica 会提升 indexing throughput 吗？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 2：Replica lag 对 freshness 有何影响？**
...
primary 负责写入主顺序，replica 提供容灾与读扩展；要讨论 replica lag、refresh visibility、failover 和读一致性。
...
- 增加 replica 会提升 indexing throughput 吗？
- Replica lag 对 freshness 有何影响？

## 高频失分点 / Gotcha

Replica 主要提升读与容错，不会免费提升写吞吐。
