---
id: Q090
title: "什么是 Near Real-Time（NRT）Search？"
chapter: 9
...
# Q090 · 什么是 Near Real-Time（NRT）Search？

[← 上一题](Q089-over-sharding.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q091-refresh-interval-tradeoff.md)
...
这道题表面在问“什么是 Near Real-Time（NRT）Search？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

文档被写入后通常先进入内存 buffer/transaction log，经过 refresh/open new searcher 后才对搜索 可见，因此搜索可见性延迟是秒级/亚秒级的“近实时”，不是每次写后立即全局可见。

## 5 分钟深度回答

- Refresh 产生新的可搜索 segment/view，但不等同于持久化 fsync/commit。
- 频繁 refresh 提升 freshness，却增加小 segment 与 merge 压力。
- 不同系统可用 refresh interval 调节索引吞吐与可见性。
- 实时 get 与 search visibility 可能采用不同路径，因此语义要区分。
...
NRT 意味着写入成功与可搜索之间存在 refresh window；translog/WAL durability 与 search visibility 是不同概念；强一致读可能需特殊路径。
...
- Refresh 与 Flush/Commit 有什么区别？
- 为什么 refresh 不是免费的？

## 高频失分点 / Gotcha

不要把 NRT 简化成“有 1 秒延迟”；具体延迟由 refresh policy 与负载决定。
