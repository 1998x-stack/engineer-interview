---
id: Q091
title: "Refresh Interval 为什么存在 Freshness-Throughput Trade-off？"
chapter: 9
...
# Q091 · Refresh Interval 为什么存在 Freshness-Throughput Trade-off？

[← 上一题](Q090-near-real-time-search.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q092-mysql-search-cdc-sync.md)
...
这道题表面在问“Refresh Interval 为什么存在 Freshness-Throughput Trade-off？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

更短 refresh interval 让新文档更快可搜索，但会产生更多小 segment、更多 searcher reopen 与 merge；更长 interval 提高批量写效率，却牺牲可见性。

## 5 分钟深度回答

- 写密集 bulk indexing 时常临时增大或关闭自动 refresh，再在批量结束后恢复。
- 新闻/库存场景需要低 freshness lag；历史文档库则可以更偏 throughput。
- 系统应监控 refresh time、segment count、merge time、indexing pressure。
- 如果业务允许，hot layer + cold layer 可以把 freshness 与大索引稳定性拆开。
...
refresh 越频繁 freshness 越好，但 segment 增长与 merge 压力更大；批量导入常临时调大 interval，实时场景需按 SLA 选择。
...
- 为什么小 segment 多会拖慢 query？
- 实时商品库存是否一定要重建全文索引字段？

## 高频失分点 / Gotcha

所有“更实时”的功能都有资源成本，必须明确业务 SLA。
