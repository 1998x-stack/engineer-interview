...
# Q092 · 搜索索引如何与 MySQL/业务数据库保持同步？

[← 上一题](Q091-refresh-interval-tradeoff.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q093-search-caching.md)
...
这道题表面在问“搜索索引如何与 MySQL/业务数据库保持同步？”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

不要依赖不可原子化的双写。常见稳健方案是数据库作为 source of truth，通过 CDC/binlog/Kafka 或 transactional outbox 把变更可靠地投递给 indexer，并保证幂等、版本和重放。

## 5 分钟深度回答

- CDC：捕获 DB commit 后的变更，天然与事务顺序对齐。
- Indexer 必须 idempotent：同一事件重复消费不能破坏状态。
- 用 version/sequence 防止乱序旧事件覆盖新文档。
- 定期 reconciliation/backfill 修复漏事件或历史 bug。
- Delete 事件同样要可靠传播，否则索引出现“幽灵文档”
。
- 强一致通常成本很高；多数搜索接受 bounded eventual consistency。
...
### 追问链：参考回答
**追问 1：为什么直接 DB+ES 双写会出错？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 2：Kafka 重放如何避免重复索引？**

回答时先给定义，再给边界条件与可观测指标；如果是算法选择题，用“效果、延迟、内存、更新、可解释性”五维 trade-off 组织。

**追问 3：Schema change 怎么处理？**
...
避免 DB+Search 双写；用 CDC/outbox→queue→idempotent indexer；管理 offset、版本号、delete、replay 和 reconciliation 全量校验。
...
Kafka / durable log
  ↓
idempotent indexer (document_id, version)
  ↓
Search index
```
用 source version 做 compare-and-set，可防 replay/乱序事件覆盖新版本。
...
一致性设计要从失败场景开始：DB 成功、Index 失败；Index 成功、DB 失败；消息重复/乱序。
...
> **练习：** 设计 MySQL binlog → Kafka → Indexer → Search 的 CDC exactly-once-ish/idempotent 方案。
