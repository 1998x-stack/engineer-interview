---
id: Q095
title: "系统设计：从 0 设计一个 Google-like Web Search"
chapter: 10
...
# Q095 · 系统设计：从 0 设计一个 Google-like Web Search

← 上一题：无 · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q096-system-design-ecommerce-search.md)
...
这道题表面在问“系统设计：从 0 设计一个 Google-like Web Search”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

答题要同时覆盖 crawler/indexing、retrieval/ranking、distributed serving、freshness、quality/s- pam、logs/experiments。不要一上来画 Elasticsearch 集群，而应先做需求与数量级估算。

## 5 分钟深度回答

- 需求：文档规模、QPS、p99、freshness、语言、多租户/权限、相关性目标。
- 离线：crawl scheduler → parser → canonicalization/dedup → link graph/quality → index
builder → shard publish。
- 索引：inverted index + vector index + document store + static feature store。
- 在线：query understanding → lexical/dense recalls → fusion → LTR → neural rerank →
snippets。
- 分布式：sharding/replication、scatter-gather、cache、timeout/partial result、rolling index
update。
- 数据闭环：judgments + click logs → debias → training → offline eval → A/B。
- 扩展问题：spam、safe search、canonical URL、fresh documents、duplicate results。
...
**追问 4：如果 p99 要 100ms，Cross-Encoder 怎么放？**
...
Web Search 需 crawler/frontier、canonicalization、spam/quality、indexing、retrieval/rank、freshness；系统设计还要讨论 crawl politeness、duplicate 和 authority。
...
- PageRank 在哪里算？
- 如何做跨语言搜索？
- 如果 p99 要 100ms，Cross-Encoder 怎么放？

## 高频失分点 / Gotcha

系统设计最忌“组件堆砌”。每个组件都要回答：解决什么瓶颈、输入输出是什么、失败如何降级。
...
> **练习：** 30 分钟白板完成 Web Search：需求 5 分钟、估算 5 分钟、架构 10 分钟、瓶颈/降级/实验 10 分钟。

完成标准：能在不看答案的情况下，先用 30 秒给结论，再用 5 分钟白板说明原理、至少两个 trade-off、一个 failure case 和验证指标。

## 一句话记忆

先定约束，再定索引，再定 cascade，最后闭环。
