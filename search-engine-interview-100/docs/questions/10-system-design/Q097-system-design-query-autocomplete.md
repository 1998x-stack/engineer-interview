---
id: Q097
title: "系统设计：亿级 Query Autocomplete"
chapter: 10
...
# Q097 · 系统设计：亿级 Query Autocomplete

[← 上一题](Q096-system-design-ecommerce-search.md) · [章节索引](README.md) · [全局索引](../../INDEX.md) · [下一题 →](Q098-system-design-billion-vector-search.md)
...
| 核心标签 | `system-design`, `search-architecture`, `staff-interview`, `autocomplete` |

## 面试官到底在考什么

这道题表面在问“系统设计：亿级 Query Autocomplete”，实际主要看三件事：**是否掌握核心定义/机制、是否能说明边界与 trade-off、是否能把算法落到可观测的线上系统**。回答建议采用“结论 → 原理 → 极限/反例 → 工业实现 → 指标”的顺序。

## 30 秒回答

设计要覆盖候选索引、热度流式更新、个性化、安全与极低延迟。最稳的框架是离线全局 FST + 实时 hot layer + 用户历史层，最后轻量 ranker 融合。

## 5 分钟深度回答

- 离线从 query logs 聚合去隐私化的频率/质量，构建 prefix → topN completion。
- 全局索引用 FST/压缩 trie；实时趋势用内存 KV/streaming topK。
- 用户历史候选本地/用户缓存读取，避免把私人 query 放入全局库。
- Rank features：prefix length、frequency、recency、trend、locale、personalization、edit
distance。
- 系统目标常是 p99 20–50ms 级，因此不能依赖重型在线模型。
- 防刷与安全：低频 query、个人信息、违规词、机器人流量需要过滤。
...
### 追问链：参考回答
**追问 1：如何做 typo tolerant autocomplete？**
...
Autocomplete 的目标 p99 极低；离线从 query logs 构建 prefix→top suggestions，在线叠加 trend/personalization；要防敏感查询泄漏和热前缀。
...
- 如何做 typo tolerant autocomplete？
- 热点事件如何秒级生效？
- 如何估算 FST 内存？

## 高频失分点 / Gotcha

Autocomplete 是“每个 keystroke 的搜索”，它的 QPS 和 latency 压力往往比主搜索更高。
...
> **练习：** 给 autocomplete 设计 prefix index、热榜、个性化、敏感词/质量 policy 与 <50ms SLO。

完成标准：能在不看答案的情况下，先用 30 秒给结论，再用 5 分钟白板说明原理、至少两个 trade-off、一个 failure case 和验证指标。

## 一句话记忆

全局静态 + 实时热点 + 个人历史，三层候选融合。
